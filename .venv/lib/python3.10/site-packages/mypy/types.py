...
class TypeAliasType(Type):
    ...
    def _expand_once(self) -> Type:
        ...
        return replace_alias_tvars(
            self.alias.target,
            self.alias.alias_tvars,
            self.args,
            self.line,
            self.column,
        )

    def _partial_expansion(
        self, nothing_args: bool = False
    ) -> tuple[ProperType, bool]:
        # Private method mostly for debugging and testing.
        unroller = UnrollAliasVisitor(set())
        if nothing_args:
            alias = self.copy_modified(
                args=[UninhabitedType()] * len(self.args)
            )
        else:
            alias = self
    ...
    def is_recursive(self) -> bool:
        ...
        *definition* is recursive. The property value thus can be cached on the
        ...
        if is_recursive is None:
            is_recursive = (
                self.expand_all_if_possible(nothing_args=True) is None
            )
...
class TypeVarType(TypeVarLikeType):
    ...
    def __init__(
        ...
        assert (
            values is not None
        ), "No restrictions must be represented by empty list"
        self.values = values
...
class ParamSpecType(TypeVarLikeType):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            name, fullname, id, upper_bound, line=line, column=column
        )
        self.flavor = flavor
...
class TypeVarTupleType(TypeVarLikeType):
    ...
    def copy_modified(
        self, id: Bogus[TypeVarId | int] = _dummy
    ) -> TypeVarTupleType:
        return TypeVarTupleType(
            self.name,
            self.fullname,
...
class UnboundType(ProperType):
    ...
    def copy_modified(
        self, args: Bogus[Sequence[Type] | None] = _dummy
    ) -> UnboundType:
        if args is _dummy:
            args = self.args
        return UnboundType(
...
class TypeList(ProperType):
    ...
    def __init__(
        self, items: list[Type], line: int = -1, column: int = -1
    ) -> None:
        super().__init__(line, column)
        self.items = items
...
class AnyType(ProperType):
    ...
    def __init__(
        ...
        assert (
            type_of_any != TypeOfAny.from_another_any or source_any is not None
        )
...
class UninhabitedType(ProperType):
    ...
    is_noreturn: (
        bool  # Does this come from a NoReturn?  Purely for error messages.
    )
...
NOT_READY: Final = mypy.nodes.FakeInfo(
...
class ExtraAttrs:
    ...
    def copy(self) -> ExtraAttrs:
        return ExtraAttrs(
            self.attrs.copy(), self.immutable.copy(), self.mod_name
        )

    def __repr__(self) -> str:
        return (
            f"ExtraAttrs({self.attrs!r}, {self.immutable!r}, {self.mod_name!r})"
        )
...
class Instance(ProperType):
    ...
    def deserialize(cls, data: JsonDict | str) -> Instance:
        ...
        if "last_known_value" in data:
            inst.last_known_value = LiteralType.deserialize(
                data["last_known_value"]
            )
        return inst
...
class Parameters(ProperType):
    ...
    def var_arg(self) -> FormalArgument | None:
        ...
        for position, (type, kind) in enumerate(
            zip(self.arg_types, self.arg_kinds)
        ):
            if kind == ARG_STAR:
    ...
    def formal_arguments(
        ...
        for i in range(len(self.arg_types)):
            ...
            arg = FormalArgument(
                self.arg_names[i], pos, self.arg_types[i], required
            )
            args.append(arg)
        return args
    ...
    def deserialize(cls, data: JsonDict) -> Parameters:
        ...
        return Parameters(
            ...
            data["arg_names"],
            variables=[
                cast(TypeVarLikeType, deserialize_type(v))
...
class CallableType(FunctionLike):
    ...
    def copy_modified(
        ...
        modified = CallableType(
            ...
            definition=(
                definition if definition is not _dummy else self.definition
            ),
            variables=variables if variables is not _dummy else self.variables,
            ...
            special_sig=(
                special_sig if special_sig is not _dummy else self.special_sig
            ),
            from_type_type=(
                from_type_type
            ...
            bound_args=(
                bound_args if bound_args is not _dummy else self.bound_args
            ),
            def_extras=(
                def_extras
    ...
    def deserialize(cls, data: JsonDict) -> CallableType:
        ...
        return CallableType(
            ...
            bound_args=[
                (None if t is None else deserialize_type(t))
...
class TupleType(ProperType):
    ...
    def slice(
        self, begin: int | None, end: int | None, stride: int | None
    ) -> TupleType:
        return TupleType(
...
class TypedDictType(ProperType):
    ...
    def copy_modified(
        ...
        *,
        ...
        return TypedDictType(
            items, required_keys, fallback, self.line, self.column
        )
...
class UnionType(ProperType):
    ...
    @staticmethod
    def make_union(
        items: Sequence[Type], line: int = -1, column: int = -1
    ) -> Type: ...
    ...
    def relevant_items(self) -> list[Type]:
        ...
        if state.strict_optional:
            ...
            return [
                i
                for i in self.items
                if not isinstance(get_proper_type(i), NoneType)
            ]
...
class TypeType(ProperType):
    ...
    @staticmethod
    def make_normalized(
        item: Type, *, line: int = -1, column: int = -1
    ) -> ProperType:
        item = get_proper_type(item)
        if isinstance(item, UnionType):
            return UnionType.make_union(
                [
                    TypeType.make_normalized(union_item)
...
class PlaceholderType(ProperType):
    ...
    def __init__(
        self, fullname: str | None, args: list[Type], line: int
    ) -> None:
        super().__init__(line)
        self.fullname = (
    ...
    def serialize(self) -> str:
        ...
        assert (
            False
        ), f"Internal error: unresolved placeholder type {self.fullname}"
            ...
            for var in t.variables:
                if isinstance(var, TypeVarType):
                    ...
                    if var.values:
                        ...
                        var.upper_bound, "builtins.object"
                    ):
                        vs.append(
                            f"{var.name} <: {var.upper_bound.accept(self)}"
                        )
                    else:
                        vs.append(var.name)
                else:
...
class TrivialSyntheticTypeTranslator(
    TypeTranslator, SyntheticTypeVisitor[Type]
):
        ...
        if param_spec is not None:
            ...
            if isinstance(repl, CallableType) or isinstance(repl, Parameters):
                ...
                return t.copy_modified(
                    arg_types=[t.accept(self) for t in prefix.arg_types]
                    + t.arg_types,
...
def invalid_recursive_alias(
    seen_nodes: set[mypy.nodes.TypeAlias], target: Type
) -> bool:
    ...
    if isinstance(target, TypeAliasType):
        ...
        return invalid_recursive_alias(
