...
class CreatorDashboardHandler(base.BaseHandler[CreatorDashboardHandlerNormalizedPayloadDict, Dict[str, str]]):
    ...
    def get(self) -> None:
            ...
            displayable_exploration_summary_dicts.append(
                {
                    ...
                    'human_readable_contributors_summary': (exploration['human_readable_contributors_summary']),
        ...
        dashboard_stats_dict = {
            ...
            'total_open_feedback': feedback_services.get_total_open_threads(feedback_thread_analytics),
        }
        ...
        for index, subscriber_settings in enumerate(subscribers_settings):
            subscriber_summary = {
                ...
                'subscriber_impact': (user_services.get_user_impact_score(subscriber_ids[index])),
            }
        ...
        self.values.update(
            {
                ...
                'threads_for_created_suggestions_list': (threads_linked_to_suggestions_by_user),
                'threads_for_suggestions_to_review_list': (threads_linked_to_suggestions_which_can_be_reviewed),
...
class UploadExplorationHandler(base.BaseHandler[Dict[str, str], UploadExplorationHandlerNormalizedRequestDict]):
    ...
    def post(self) -> None:
        ...
        if constants.ALLOW_YAML_FILE_UPLOAD:
            exp_services.save_new_exploration_from_yaml_and_assets(self.user_id, yaml_content, new_exploration_id, [])
            self.render_json({EXPLORATION_ID_KEY: new_exploration_id})
        else:
