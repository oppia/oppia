...
class CreatorDashboardHandler(
    base.BaseHandler[CreatorDashboardHandlerNormalizedPayloadDict, Dict[str, str]]
):
    ...
    def get(self) -> None:
        ...
        subscribed_collection_summaries = (
            collection_services.get_collection_summaries_subscribed_to(self.user_id)
        )
        ...
        feedback_thread_analytics = (
            feedback_services.get_thread_analytics_multi(exploration_ids_subscribed_to)
        )
        ...
        for ind, exploration in enumerate(exp_summary_dicts):
            ...
            displayable_exploration_summary_dicts.append({
                ...
                'num_total_threads': (feedback_analytics_dict['num_total_threads'])
            })
        ...
        dashboard_stats_dict = {
            'num_ratings':
                dashboard_stats['num_ratings'],
            'average_ratings':
                dashboard_stats['average_ratings'],
            'total_plays':
                dashboard_stats['total_plays'],
            'total_open_feedback':
        ...
        threads_linked_to_suggestions_by_user = ([
            t.to_dict() for t in
            feedback_services.get_multiple_threads(ids_of_suggestions_created_by_user)
        ])
        threads_linked_to_suggestions_which_can_be_reviewed = ([
            t.to_dict() for t in feedback_services.
            get_multiple_threads(ids_of_suggestions_which_can_be_reviewed)
        ])
...
class UploadExplorationHandler(
    base.BaseHandler[Dict[str, str], UploadExplorationHandlerNormalizedRequestDict]
):
