...
class TopicEditorStoryHandler(
    base.BaseHandler[TopicEditorStoryHandlerNormalizedPayloadDict,
    ...
    def get(self, topic_id: str) -> None:
        ...
        canonical_stories = list(
            filter(None, story_fetchers.get_stories_by_ids(canonical_stories_ids))
        )
        canonical_stories_dict: Dict[str, story_domain.Story] = {
            canonical_story.id: canonical_story for canonical_story in canonical_stories
        }
...
class EditableTopicDataHandler(
    base.BaseHandler[EditableTopicDataHandlerNormalizedPayloadDict, Dict[str, str]]
):
    ...
    def get(self, topic_id: str) -> None:
        ...
        for topic_object in topics:
            skill_id_to_rubrics_dict_local, deleted_skill_ids = (
                skill_services.get_rubrics_of_skills(topic_object.get_all_skill_ids())
            )
        ...
        classroom_url_fragment = (
            classroom_config_services.get_classroom_url_fragment_for_topic_id(topic_id)
        )
        classroom_name = (
        ...
        self.values.update({
            ...
            'classroom_name': (
                None if
                (classroom_name == str(constants.CLASSROOM_NAME_FOR_UNATTACHED_TOPICS
                                      )) else classroom_name
            ),
    ...
    def put(self, topic_id: str) -> None:
        ...
        topic_and_subtopic_page_change_dicts = self.normalized_payload[
            'topic_and_subtopic_page_change_dicts']
        topic_and_subtopic_page_change_list: List[Union[
            subtopic_page_domain.SubtopicPageChange, topic_domain.TopicChange]] = []
...
class TopicRightsHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
    ...
    def get(self, topic_id: str) -> None:
        ...
        can_publish_topic = (
            role_services.ACTION_CHANGE_TOPIC_STATUS in user_actions_info.actions
        )
...
class TopicPublishSendMailHandler(
    base.BaseHandler[TopicPublishSendMailHandlerNormalizedPayloadDict, Dict[str, str]]
):
    ...
    def put(self, topic_id: str) -> None:
        ...
        if server_can_send_emails:
            email_manager.send_mail_to_admin(
                ...
                (self.username, self.normalized_payload['topic_name'], topic_url)
            )
    ...
    def get(self, topic_url_fragment: str) -> None:
        ...
        self.values.update({
            'topic_url_fragment_exists':
                (topic_services.does_topic_with_url_fragment_exist(topic_url_fragment))
        })
        self.render_json(self.values)
...
class TopicIdToTopicNameHandler(
    base.BaseHandler[Dict[str, str], TopicIdToTopicNameHandlerNormalizedRequestDict]
):
