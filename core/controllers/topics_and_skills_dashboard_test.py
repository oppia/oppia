...
class CategorizedAndUntriagedSkillsDataHandlerTests(BaseTopicsAndSkillsDashboardTests):
    ...
    def test_get(self) -> None:
        ...
        self.assertEqual(len(json_response['untriaged_skill_summary_dicts']), 1)
        self.assertEqual(
            json_response['untriaged_skill_summary_dicts'][0]['skill_id'], skill_id
        )
        self.assertEqual(len(json_response['categorized_skills_dict']), 1)
...
class SkillsDashboardPageDataHandlerTests(BaseTopicsAndSkillsDashboardTests):
    ...
    def test_post(self) -> None:
        ...
        self.assertEqual(
            json_response['skill_summary_dicts'][1]['id'], self.subtopic_skill_id
        )
        ...
        self.assertEqual(len(json_response['skill_summary_dicts']), 2)
        self.assertEqual(
            json_response['skill_summary_dicts'][0]['id'], self.subtopic_skill_id
        )
        self.assertEqual(
    ...
    def test_fetch_filtered_skills_with_given_keywords(self) -> None:
        ...
        self.assertEqual(
            json_response['skill_summary_dicts'][0]['description'], 'Description 3'
        )
        self.assertFalse(json_response['more'])
        self.assertEqual(json_response['next_cursor'], None)
        ...
        self.assertEqual(
            json_response['skill_summary_dicts'][0]['description'], 'Subtopic Skill'
        )
        self.assertFalse(json_response['more'])
        self.assertEqual(json_response['next_cursor'], None)
...
class NewSkillHandlerTests(BaseTopicsAndSkillsDashboardTests):
    ...
    def test_skill_creation(self) -> None:
        ...
        json_response = self.post_json(
            self.url, {
                ...
                'files': {
                    'img.png':
                        (base64.b64encode(self.original_image_content).decode('utf-8'))
                }
            },
            csrf_token=csrf_token
        )
    ...
    def test_skill_creation_in_invalid_explanation(self) -> None:
        ...
        json_response = self.post_json(
            self.url, corrupt_payload, csrf_token=csrf_token, expected_status_int=400
        )
        self.assertEqual(json_response['status_code'], 400)
    ...
    def test_skill_creation_in_duplicate_description(self) -> None:
        ...
        self.assertIn(
            'Skill description should not be a duplicate', response_dict['error']
        )
...
class MergeSkillHandlerTests(BaseTopicsAndSkillsDashboardTests):
    ...
    def setUp(self) -> None:
        ...
        self.question = self.save_new_question(
            ...
            [self.linked_skill_id], content_id_generator.next_content_id_index
        )
...
class TopicIdToDiagnosticTestSkillIdsHandlerTests(BaseTopicsAndSkillsDashboardTests):
    ...
    def test_topic_id_to_diagnostic_test_skill_ids_handler_returns_correctly(
        ...
        url = '%s/?comma_separated_topic_ids=%s' % (
            feconf.TOPIC_ID_TO_DIAGNOSTIC_TEST_SKILL_IDS_HANDLER, 'incorrect_topic_id'
        )
