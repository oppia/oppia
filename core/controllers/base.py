...
class BaseHandler(
    ...
    def validate_and_normalize_args(self) -> None:
        ...
        keys_that_correspond_to_default_values = list(
            set(normalized_arg_values.keys()) - set(payload_arg_keys + request_arg_keys)
        )
    ...
    def handle_exception(
        ...
        if isinstance(exception, self.NotLoggedInException):
            ...
            if (
                payload_exists
                or self.GET_HANDLER_ERROR_RETURN_TYPE == feconf.HANDLER_TYPE_JSON
            ):
                self.error(401)
                values: ResponseValueDict = {
...
class CsrfTokenManager:
    ...
    @classmethod
    def _create_token(
        cls, user_id: Optional[str], issued_on: float, nonce: Optional[str] = None
    ) -> str:
