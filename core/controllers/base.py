...
class BaseHandler(
    ...
    def __init__(  # pylint: disable=super-init-not-called
        ...
        try:
            ...
            auth_services.destroy_auth_session(self.response)
            self.redirect('/logout?redirect_url=%s' % feconf.PENDING_ACCOUNT_DELETION_URL)
            return
            ...
            self.current_user_is_super_admin = auth_claims is not None and auth_claims.role_is_super_admin
        ...
        if auth_claims:
            ...
            if user_settings is None:
                ...
                if 'signup?' in self.request.uri:
                    ...
                    logging.error('Cannot find user %s with email %s on page %s' % (auth_id, email, self.request.uri))
                    auth_services.destroy_auth_session(self.response)
                    return
            ...
            if user_settings.deleted:
            ...
            elif self.REDIRECT_UNFINISHED_SIGNUPS and not user_services.has_fully_registered_account(self.user_id):
                ...
                if user_settings.last_logged_in is None or not utils.are_datetimes_close(datetime.datetime.utcnow(), user_settings.last_logged_in):
    ...
    def dispatch(self) -> None:
        ...
        if request_split.netloc == 'oppiaserver.appspot.com' and not request_split.path.startswith(('/cron/', '/task/')):
            self.redirect('https://oppiatestserver.appspot.com', permanent=True)
            return

        if not self._is_requested_path_currently_accessible_to_user() and request_split.path != '/maintenance':
        ...
        if self.payload is not None and self.REQUIRE_PAYLOAD_CSRF_CHECK:
            try:
                ...
                if 'signup' in self.request.uri and not self.user_id:
                    raise self.UnauthorizedUserException('Registration session expired.')
                csrf_token = self.request.get('csrf_token')
                if not csrf_token:
                    raise self.UnauthorizedUserException('Missing CSRF token. Changes were not saved. Please report this bug.')

                is_csrf_token_valid = CsrfTokenManager.is_csrf_token_valid(self.user_id, csrf_token)

                if not is_csrf_token_valid:
                    raise self.UnauthorizedUserException('Your session has expired, and unfortunately your changes cannot be saved. Please refresh the page.')
    ...
    def validate_and_normalize_args(self) -> None:
        ...
        if handler_class_name in handler_schema_constants.HANDLER_CLASS_NAMES_WITH_NO_SCHEMA:
            ...
            if self.URL_PATH_ARGS_SCHEMAS or self.HANDLER_ARGS_SCHEMAS:
                raise self.InternalErrorException('Remove handler class name from HANDLER_CLASS_NAMES_WHICH_STILL_NEED_SCHEMAS')
            return
        ...
        extra_args_are_allowed = self.GET_HANDLER_ERROR_RETURN_TYPE == feconf.HANDLER_TYPE_HTML and request_method == 'GET'

        if self.URL_PATH_ARGS_SCHEMAS is None:
            raise NotImplementedError('Missing schema for url path args in %s handler class.' % (handler_class_name))

        schema_for_url_path_args = self.URL_PATH_ARGS_SCHEMAS
        self.request.route_kwargs, errors = payload_validator.validate_arguments_against_schema(url_path_args, schema_for_url_path_args, extra_args_are_allowed)

        if errors:
            raise self.InvalidInputException('At \'%s\' these errors are happening:\n%s' % (self.request.uri, '\n'.join(errors)))
        ...
        if request_method in ['GET', 'POST', 'PUT', 'DELETE'] and (getattr(self.__class__, request_method.lower()) == getattr(BaseHandler, request_method.lower())):
            return

        try:
            if self.HANDLER_ARGS_SCHEMAS is None:
                raise Exception('No \'HANDLER_ARGS_SCHEMAS\' Found for the handler class: %s' % handler_class_name)
            schema_for_request_method = self.HANDLER_ARGS_SCHEMAS[request_method]
        except Exception as e:
            raise NotImplementedError('Missing schema for %s method in %s handler class.' % (request_method, handler_class_name)) from e

        allow_string_to_bool_conversion = request_method in ['GET', 'DELETE']
        normalized_arg_values, errors = payload_validator.validate_arguments_against_schema(
            handler_args,
        ...
        normalized_payload = {arg: normalized_arg_values.get(arg) for arg in payload_arg_keys}
        normalized_request = {arg: normalized_arg_values.get(arg) for arg in request_arg_keys}
        ...
        keys_that_correspond_to_default_values = list(set(normalized_arg_values.keys()) - set(payload_arg_keys + request_arg_keys))
        ...
        self.payload = RaiseErrorOnGet('Use self.normalized_payload instead of self.payload.')
    ...
    def current_user_is_site_maintainer(self) -> bool:
        ...
        """
        return self.current_user_is_super_admin or feconf.ROLE_ID_RELEASE_COORDINATOR in self.roles
    ...
    def _is_requested_path_currently_accessible_to_user(self) -> bool:
        ...
        """
        return self.request.path in AUTH_HANDLER_PATHS or not feconf.ENABLE_MAINTENANCE_MODE or self.current_user_is_site_maintainer
    ...
    def render_json(self, values: Union[str, Sequence[Mapping[str, Any]], Mapping[str, Any]]) -> None:
        ...
        self.response.headers['Content-Disposition'] = 'attachment; filename="oppia-attachment.txt"'
    ...
    def render_downloadable_file(self, file: io.BytesIO, filename: str, content_type: str) -> None:
        ...
        *,
        ...
        if iframe_restriction is not None:
            if iframe_restriction == 'SAMEORIGIN':
                ...
                self.response.headers['Content-Security-Policy'] = 'frame-ancestors \'none\''
            else:
                raise Exception('Invalid iframe restriction value: %s' % iframe_restriction)

        self.response.expires = 'Mon, 01 Jan 1990 00:00:00 GMT'
        self.response.pragma = 'no-cache'
        self.response.write(load_template(filepath, template_is_aot_compiled=template_is_aot_compiled))

    def _render_exception_json_or_html(self, return_type: str, values: ResponseValueDict) -> None:
        ...
        if return_type == feconf.HANDLER_TYPE_HTML and method == 'GET':
            ...
            if return_type not in (
                ...
                logging.warning('Not a recognized return type: defaulting to render JSON.')
    ...
    def _render_exception(self, values: ResponseValueDict) -> None:
        ...
        if method == 'GET':
            self._render_exception_json_or_html(self.GET_HANDLER_ERROR_RETURN_TYPE, values)
        elif method == 'POST':
            self._render_exception_json_or_html(self.POST_HANDLER_ERROR_RETURN_TYPE, values)
        elif method == 'PUT':
            self._render_exception_json_or_html(self.PUT_HANDLER_ERROR_RETURN_TYPE, values)
        elif method == 'DELETE':
            self._render_exception_json_or_html(self.DELETE_HANDLER_ERROR_RETURN_TYPE, values)
        else:
            logging.warning('Not a recognized request method.')
            self._render_exception_json_or_html(feconf.HANDLER_TYPE_JSON, values)

    def _log_exception_message(self, exception_type: str, log_type: LogType, error_message: str) -> None:
    ...
    def handle_exception(self, exception: BaseException, unused_debug_mode: bool) -> None:
        ...
        if isinstance(exception, self.NotLoggedInException):
            ...
            payload_exists = self.payload is not None and not isinstance(self.payload, RaiseErrorOnGet)
            self._log_exception_message(exception_type, LogType.WARNING, 'Unauthenticated user')
            if payload_exists or self.GET_HANDLER_ERROR_RETURN_TYPE == feconf.HANDLER_TYPE_JSON:
                self.error(401)
                values: ResponseValueDict = {
        ...
        if isinstance(exception, self.NotFoundException):
            self._log_exception_message(exception_type, LogType.WARNING, 'Invalid URL requested')
            self.error(404)
            values = {
                'error': 'Could not find the resource %s.' % self.request.uri,
                'status_code': 404,
        ...
        self._log_exception_message(exception_type, LogType.EXCEPTION, 'Exception raised')
...
class CsrfTokenManager:
    ...
    def is_csrf_token_valid(cls, user_id: Optional[str], token: str) -> bool:
        ...
        try:
            ...
            if hmac.compare_digest(authentic_token.encode('utf-8'), token.encode('utf-8')):
                return True
