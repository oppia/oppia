...
class QuestionsListHandlerTests(BaseQuestionsListControllerTests):
    ...
    def test_get_questions_succeeds(self) -> None:
        ...
        with self.swap(constants, 'NUM_QUESTIONS_PER_PAGE', 2):
            json_response = self.get_json(
                '%s/%s,%s?offset=0'
                % (feconf.QUESTIONS_LIST_URL_PREFIX, self.skill_id, self.skill_id_2)
            )
            ...
            for i in range(2):
                ...
                self.assertEqual(
                    question_summary_dicts_2[i]['skill_difficulties'], [0.3, 0.5]
                )
            json_response = self.get_json(
            ...
            json_response = self.get_json(
                '%s/%s?offset=3' % (feconf.QUESTIONS_LIST_URL_PREFIX, self.skill_id)
            )
    ...
    def test_get_fails_when_offset_not_valid(self) -> None:
        self.get_json(
            '%s/%s?offset=a' % (feconf.QUESTIONS_LIST_URL_PREFIX, self.skill_id),
            expected_status_int=400,
        )

    def test_get_fails_when_skill_id_not_valid(self) -> None:
        self.get_json(
            '%s/%s?offset=0' % (feconf.QUESTIONS_LIST_URL_PREFIX, '1,2'),
            expected_status_int=400,
        )

    def test_get_fails_when_skill_does_not_exist(self) -> None:
        self.get_json(
            '%s/%s?offset=0' % (feconf.QUESTIONS_LIST_URL_PREFIX, self.skill_id_3),
...
class QuestionCountDataHandlerTests(BaseQuestionsListControllerTests):
    ...
    def test_get_question_count_fails_with_invalid_skill_ids(self) -> None:
        self.get_json(
            '%s/%s' % (feconf.QUESTION_COUNT_URL_PREFIX, 'id1'), expected_status_int=400
        )
