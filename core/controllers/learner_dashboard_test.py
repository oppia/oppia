...
class LearnerDashboardExplorationsProgressHandlerTests(
    ...
    def test_bfs_with_answer_groups_and_disconnected_states(self) -> None:
        """Ensure BFS traverses answer groups, defaults, and disconnected."""
        self.login(self.VIEWER_EMAIL)
        ...
        for state_name in ['Branch', 'End', 'Isolated']:
            ...
            current_state = exploration.states[state_name]
            current_state.content.html = ''
        ...
        self.set_interaction_for_state(
            intro_state, 'TextInput', content_id_generator
        )

        assert intro_state.interaction.default_outcome is not None
        intro_state.interaction.default_outcome.dest = 'Branch'
        branch_state = exploration.states['Branch']
        ...
        intro_state.update_interaction_answer_groups([answer_group])
        ...
        learner_progress_services.mark_exploration_as_incomplete(
            self.viewer_id, self.EXP_ID_3, 'Branch', 1
        )
        ...
        self.assertEqual(exp_summary['total_cards_count'], 4)
        self.assertEqual(exp_summary['visited_cards_count'], 2)
        self.logout()
    ...
    def test_bfs_with_multiple_answer_groups(self) -> None:
        """Test BFS with multiple answer groups and default outcomes."""
        self.login(self.VIEWER_EMAIL)

        # Create exploration with multiple states and transitions.
        # This will have answer groups and default outcomes.
        ...
        # Mark as incomplete at Middle state.
        learner_progress_services.mark_exploration_as_incomplete(
            self.viewer_id, self.EXP_ID_1, 'Middle', 1
        )
        ...
        self.assertGreater(exp_summary['total_cards_count'], 1)
        self.assertGreater(exp_summary['visited_cards_count'], 0)
        self.logout()

    def test_progress_annotation_handles_fetch_failures(self) -> None:
        """Ensure summaries are preserved when exploration fetch raises."""
        self.login(self.VIEWER_EMAIL)
        ...
        learner_progress_services.mark_exploration_as_incomplete(
            self.viewer_id, self.EXP_ID_2, 'Introduction', 1
        )

        def _raise_for_id(exp_id: str, strict: bool = True) -> None:
            raise Exception('fetch failure')

        with self.swap(exp_fetchers, 'get_exploration_by_id', _raise_for_id):
            response = self.get_json(
        ...
        self.assertEqual(len(response['incomplete_explorations_list']), 1)
        exp_summary = response['incomplete_explorations_list'][0]
        self.assertNotIn('total_cards_count', exp_summary)
        self.assertNotIn('visited_cards_count', exp_summary)
        self.logout()
