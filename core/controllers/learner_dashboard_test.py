...
class LearnerDashboardTopicsAndStoriesProgressHandlerTests(test_utils.GenericTestBase):
    ...
    def test_get_learner_dashboard_ids(self) -> None:
        ...
        self.assertEqual(
            learner_dashboard_activity_ids['completed_exploration_ids'], [self.EXP_ID_1]
        )
        self.assertEqual(
        ...
        self.assertEqual(
            learner_dashboard_activity_ids['exploration_playlist_ids'], [self.EXP_ID_3]
        )

        self.assertEqual(
            learner_dashboard_activity_ids['completed_collection_ids'], [self.COL_ID_1]
        )
        self.assertEqual(
            learner_dashboard_activity_ids['incomplete_collection_ids'], [self.COL_ID_2]
        )
        self.assertEqual(
            learner_dashboard_activity_ids['collection_playlist_ids'], [self.COL_ID_3]
        )

        self.assertEqual(
            learner_dashboard_activity_ids['completed_story_ids'], [self.STORY_ID_1]
        )

        self.assertEqual(
            learner_dashboard_activity_ids['learnt_topic_ids'], [self.TOPIC_ID_1]
        )
        self.assertEqual(
        ...
        self.assertEqual(
            learner_dashboard_activity_ids['topic_ids_to_learn'], [self.TOPIC_ID_3]
        )
...
class LearnerDashboardExplorationsProgressHandlerTests(test_utils.GenericTestBase):
    ...
    def test_can_see_subscription(self) -> None:
        ...
        self.assertEqual(
            response['subscription_list'][0]['creator_username'], self.OWNER_USERNAME
        )
        self.logout()
