...
class QuestionCreationHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
    ...
    def post(self) -> None:
        ...
        if ((question_dict['id'] is not None) or
            ('question_state_data' not in question_dict) or
            ('language_code' not in question_dict) or (question_dict['version'] != 0)):
            raise self.InvalidInputException(
        ...
        if filenames:
            ...
            for filename in filenames_list:
                ...
                fs_services.save_original_and_compressed_versions_of_image(
                    filename, feconf.ENTITY_TYPE_QUESTION, question.id, image, 'image',
                    image_is_compressible
                )
