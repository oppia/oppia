        ...
        try:
            ...
            if action == 'reload_exploration':
                ...
                if exploration_id is None:
                    raise Exception(
                        'The \'exploration_id\' must be provided when the action is reload_exploration.'
                    )
                ...
                if collection_id is None:
                    raise Exception(
                        'The \'collection_id\' must be provided when the action is reload_collection.'
                    )
                ...
                if num_dummy_exps_to_generate is None:
                    raise Exception(
                        'The \'num_dummy_exps_to_generate\' must be provided when the action is generate_dummy_explorations.'
                    )
                ...
                if num_dummy_exps_to_publish is None:
                    raise Exception(
                        'The \'num_dummy_exps_to_publish\' must be provided when the action is generate_dummy_explorations.'
                    )
                ...
                if num_dummy_translation_opportunities_to_generate is None:
                    raise Exception(
                        'The \'num_dummy_translation_opportunities_to_generate\' must be provided when the action is generate_dummy_translation_opportunities.'
                    )
                ...
                if blog_post_title is None:
                    raise Exception(
                        'The \'blog_post_title\' must be provided when the action is generate_dummy_blog_post.'
                    )
                ...
                if skill_id is None:
                    raise Exception(
                        'The \'skill_id\' must be provided when the action is _generate_dummy_question_suggestions.'
                    )
                num_dummy_question_suggestions_generate = (
                    self.normalized_payload.get(
                        'num_dummy_question_suggestions_generate'
                ...
                if num_dummy_question_suggestions_generate is None:
                    raise Exception(
                        'The \'num_dummy_question_suggestions_generate\' must be provided when the action is _generate_dummy_question_suggestions.'
                    )
                ...
                if topic_id is None:
                    raise Exception(
                        'The \'topic_id\' must be provided when the action is generate_dummy_stories.'
                    )
                ...
                if num_dummy_stories_to_generate is None:
                    raise Exception(
                        'The \'num_dummy_stories_to_generate\' must be provided when the action is generate_dummy_stories.'
                    )
                ...
                if story_id is None:
                    raise Exception(
                        'The \'story_id\' must be provided when the action is generate_dummy_chapters.'
                    )
                ...
                if num_dummy_chapters_to_generate is None:
                    raise Exception(
                        'The \'num_dummy_chapters_to_generate\' must be provided when the action is generate_dummy_chapters.'
                    )
                ...
                if data is None:
                    raise Exception(
                        'The \'data\' must be provided when the action is upload_topic_similarities.'
                    )
                ...
                if topic_id is None:
                    raise Exception(
                        'The \'topic_id\' must be provided when the action is regenerate_topic_related_opportunities.'
                    )
                ...
                if exp_id is None:
                    raise Exception(
                        'The \'exp_id\' must be provided when the action is rollback_exploration_to_safe_state.'
                    )
                ...
                if platform_param_name is None:
                    raise Exception(
                        'The \'platform_param_name\' must be provided when the action is update_platform_parameter_rules.'
                    )
                new_rules = self.normalized_payload.get('new_rules')
                if new_rules is None:
                    raise Exception(
                        'The \'new_rules\' must be provided when the action is update_platform_parameter_rules.'
                    )
                commit_message = self.normalized_payload.get('commit_message')
                if commit_message is None:
                    raise Exception(
                        'The \'commit_message\' must be provided when the action is update_platform_parameter_rules.'
                    )
                ...
                logging.info(
                    '[ADMIN] %s updated feature %s with new rules: %s.'
                    % (self.user_id, platform_param_name, new_rule_dicts)
                )
    ...
    def _generate_dummy_question_suggestions(
        ...
        if constants.DEV_MODE:
            if (feconf.ROLE_ID_QUESTION_ADMIN not in self.user.roles) and (
                ...
                raise Exception(
                    (
                        'User \'%s\' must be a question submitter or question admin in order to generate question suggestions.'
                    )
                    % self.username
                )
    ...
    def _generate_dummy_stories(
        ...
        if constants.DEV_MODE:
            if feconf.ROLE_ID_CURRICULUM_ADMIN not in self.user.roles:
                raise Exception(
                    (
                        'User \'%s\' must be a curriculum admin in order to generate stories.'
                    )
                    % self.username
                )
    ...
    def _generate_dummy_chapters(
        ...
        if constants.DEV_MODE:
            if feconf.ROLE_ID_CURRICULUM_ADMIN not in self.user.roles:
                raise Exception(
                    (
                        'User \'%s\' must be a curriculum admin in order to generate chapters.'
                    )
                    % self.username
                )
...
class DataExtractionQueryHandler(
    ...
    def get(self) -> None:
        ...
        if state_answers is None:
            raise Exception(
                'No state answer exists for the given exp_id: %s, exp_version: %s and state_name: %s'
                % (exp_id, exp_version, state_name)
            )
...
class UpdateUsernameHandler(
    ...
    def put(self) -> None:
        ...
        if not old_fs.isfile('profile_picture.png'):
            raise self.NotFoundException(
                'The user with username %s does not have a profile picture with png extension.'
                % old_username
            )

        if not old_fs.isfile('profile_picture.webp'):
            raise self.NotFoundException(
                'The user with username %s does not have a profile picture with webp extension.'
                % old_username
            )
...
class DeleteUserHandler(
    ...
    def delete(self) -> None:
        ...
        if user_id_from_username != user_id:
            raise self.InvalidInputException(
                'The user ID retrieved from the username and the user ID provided by admin differ.'
            )
...
class TranslationCoordinatorRoleHandler(
    ...
    def put(self) -> None:
        ...
        user_id = user_settings.user_id
        if action == 'assign':
            if feconf.ROLE_ID_TRANSLATION_COORDINATOR not in (
                user_settings.roles
            ):
                user_services.add_user_role(
