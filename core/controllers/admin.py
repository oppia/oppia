...
class QuestionAdminRoleHandler(
    ...
    def put(self) -> None:
        ...
        Raises:
            InvalidInputException. If the request payload is invalid.
            NotFoundException. If the user with the given username does not exist.
            UnauthorizedUserException. If the user has manager rights and cannot be deassigned.
        """
        if self.normalized_payload is None:
            raise self.InvalidInputException('Invalid request payload.')
        ...
        if user_settings is None:
            self.render_json({'error': 'User with given username does not exist.'}, 404)
            return

        user_id = user_settings.user_id

        if action == 'assign':
            if feconf.ROLE_ID_QUESTION_ADMIN not in user_settings.roles:
                user_services.add_user_role(user_id, feconf.ROLE_ID_QUESTION_ADMIN)

        elif action == 'deassign':
            if topic_fetchers.get_topic_rights_with_user(user_id):
                raise self.UnauthorizedUserException(
                    'User has manager rights and cannot be deassigned.'
                )
