...
class QuestionAdminRoleHandler(
    ...
    @acl_decorators.can_access_admin_page
    def put(self) -> None:
        """Adds or removes the question-admin role for a user.
...
class QuestionCoordinatorRoleHandlerNormalizedPayloadDict(TypedDict):
    """Dict representation of QuestionCoordinator's normalized_payload
    dictionary.
    """
...
class QuestionCoordinatorRoleHandler(
    base.BaseHandler[
        QuestionCoordinatorRoleHandlerNormalizedPayloadDict, Dict[str, str]
    ]
):
    """Handler to assign or deassigning a question coordinator."""
    ...
    @acl_decorators.can_access_admin_page
    def put(self) -> None:
        """Adds or removes the question-coordinator role for a user.
        ...
        user_id = user_settings.user_id
        if (action == 'assign' and
                not feconf.ROLE_ID_QUESTION_COORDINATOR in user_settings.roles):
            user_services.add_user_role(
                user_id, feconf.ROLE_ID_QUESTION_COORDINATOR)
