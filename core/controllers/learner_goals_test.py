...
class LearnerGoalsHandlerTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.save_new_topic(
            self.TOPIC_ID_4,
            self.owner_id,
            name=self.TOPIC_NAME_4,
            url_fragment='topic-four',
            description='A new topic',
            canonical_story_ids=[],
            additional_story_ids=[],
            uncategorized_skill_ids=[],
            subtopics=[self.subtopic_4],
            next_subtopic_id=1
        )
    ...
    def test_add_topic_to_learner_goal(self) -> None:
        ...
        self.post_json(
            '%s/%s/%s' % (
                feconf.LEARNER_GOALS_DATA_URL, constants.ACTIVITY_TYPE_LEARN_TOPIC,
                self.TOPIC_ID_1
            ), {},
        ...
        response = self.post_json(
            '%s/%s/%s' %
            (feconf.LEARNER_GOALS_DATA_URL, 'InvalidActivityType', self.TOPIC_ID_1), {},
            csrf_token=csrf_token,
            expected_status_int=400
        )
        self.assertIn(
            'Received InvalidActivityType which is not in the allowed '
            'range of choices: [\'learntopic\']', response['error']
        )
