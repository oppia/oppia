...
class BlogHomepageDataHandlerTest(test_utils.GenericTestBase):
    ...
    def test_get_blog_homepage_data(self) -> None:
        ...
        self.assertEqual(
            ...
            json_response['blog_post_summary_dicts'][0][
        ...
        self.assertEqual(len(json_response['blog_post_summary_dicts']), 1)
        self.assertEqual(
            json_response['blog_post_summary_dicts'][0]['title'], 'Sample Title'
        )
        self.assertEqual(
    ...
    def test_get_blog_homepage_data_with_author_account_deleted(self) -> None:
        ...
        blog_admin_model = user_models.UserSettingsModel.get_by_id(
            self.blog_admin_id
        )
    ...
    def test_pagination_beyond_page_2_with_many_blog_posts(self) -> None:
        """Test that pagination works correctly for page 3 and beyond.

        This test reproduces the bug described in issue #23323 where
        pagination fails beyond page 2 with a 500 error.
        """
        for i in range(24):
            blog_post = blog_services.create_new_blog_post(self.blog_admin_id)
            change_dict: blog_services.BlogPostChangeDict = {
                'title': 'Test Blog Post %s' % i,
                'thumbnail_filename': 'thumbnail.svg',
                'content': '<p>Content for blog post %s</p>' % i,
                'tags': ['Newsletter'],
            }
        ...
        # Page 1 (offset 0) - should return 10 posts.
        json_response = self.get_json(
            '%s?offset=0' % feconf.BLOG_HOMEPAGE_DATA_URL
        )
        self.assertEqual(len(json_response['blog_post_summary_dicts']), 10)
        self.assertEqual(json_response['no_of_blog_post_summaries'], 25)

        # Page 2 (offset 10) - should return 10 posts.
        json_response = self.get_json(
            '%s?offset=10' % feconf.BLOG_HOMEPAGE_DATA_URL
        )
        self.assertEqual(len(json_response['blog_post_summary_dicts']), 10)

        # Page 3 (offset 20) - should return 5 posts.
        # This is where the bug in issue #23323 occurs.
        json_response = self.get_json(
            '%s?offset=20' % feconf.BLOG_HOMEPAGE_DATA_URL
        )
        self.assertEqual(len(json_response['blog_post_summary_dicts']), 5)

        self.logout()

    def test_pagination_with_deleted_author_on_later_pages(self) -> None:
        """Test pagination when author accounts are deleted on later pages.

        This test verifies that the blog homepage works correctly when a blog
        post's author account has been deleted but no BlogAuthorDetailsModel
        exists. The system should return a placeholder author name instead of
        failing with a 'User not found' error (issue #23323).
        """
        second_author_email = 'secondauthor@example.com'
        second_author_username = 'secondauthor'
        self.signup(second_author_email, second_author_username)
        second_author_id = self.get_user_id_from_email(second_author_email)
        self.add_user_role(second_author_username, feconf.ROLE_ID_BLOG_ADMIN)
        for i in range(10):
            blog_post = blog_services.create_new_blog_post(self.blog_admin_id)
            change_dict: blog_services.BlogPostChangeDict = {
                'title': 'First Author Post %s' % i,
                'thumbnail_filename': 'thumbnail.svg',
                'content': '<p>Content from first author %s</p>' % i,
                'tags': ['Newsletter'],
            }
            blog_services.update_blog_post(blog_post.id, change_dict)
            blog_services.publish_blog_post(blog_post.id)
        for i in range(15):
            blog_post = blog_services.create_new_blog_post(second_author_id)
            change_dict = {
                'title': 'Second Author Post %s' % i,
                'thumbnail_filename': 'thumbnail.svg',
                'content': '<p>Content from second author %s</p>' % i,
                'tags': ['Learners'],
            }
            blog_services.update_blog_post(blog_post.id, change_dict)
            blog_services.publish_blog_post(blog_post.id)
        second_author_model = user_models.UserSettingsModel.get_by_id(
            second_author_id
        )
        second_author_model.deleted = True
        second_author_model.update_timestamps()
        second_author_model.put()
        self.login(self.user_email)
        ...
        for summary in response['blog_post_summary_dicts']:
            if 'Second Author Post' in summary['title']:
                self.assertEqual(
                    summary['displayed_author_name'], 'Account Deleted'
                )
                self.assertEqual(
                    summary['author_username'], 'author account deleted'
                )
...
class BlogPostDataHandlerTest(test_utils.GenericTestBase):
    ...
    def test_get_post_page_data(self) -> None:
        ...
        self.assertEqual(
            '<p>Hello Bloggers</p>', json_response['blog_post_dict']['content']
        )
        ...
        self.assertEqual(
            '<p>Hello Blog</p>', json_response['blog_post_dict']['content']
        )
    ...
    def test_should_get_correct_recommendations_for_post_page(self) -> None:
        ...
        blog_post_three_id = blog_services.create_new_blog_post(
            blog_editor_id
        ).id
        ...
        blog_post_four_id = blog_services.create_new_blog_post(
            blog_editor_id
        ).id
        ...
        json_response = self.get_json(
            '%s/%s'
            % (feconf.BLOG_HOMEPAGE_DATA_URL, blog_post_four.url_fragment)
        )
        self.assertEqual(len(json_response['summary_dicts']), 2)
        self.assertEqual(
        ...
        json_response = self.get_json(
            '%s/%s'
            % (feconf.BLOG_HOMEPAGE_DATA_URL, blog_post_three.url_fragment)
        )
        self.assertEqual(len(json_response['summary_dicts']), 2)
        self.assertEqual(
...
class BlogPostSearchHandlerTest(test_utils.GenericTestBase):
    ...
    def test_handler_with_given_query_and_tag(self) -> None:
        ...
        response_dict = self.get_json(
            feconf.BLOG_SEARCH_DATA_URL,
            params={'q': 'Welcome', 'tags': '("Science")'},
        )
