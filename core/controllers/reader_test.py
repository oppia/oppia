...
(exp_models, stats_models, translation_models) = models.Registry.import_models([
    models.Names.EXPLORATION, models.Names.STATISTICS, models.Names.TRANSLATION
])


def _get_change_list(state_name: str, property_name: str,
                     new_value: Union[bool, str]) -> List[exp_domain.ExplorationChange]:
...
class ReaderPermissionsTest(test_utils.GenericTestBase):
    ...
    def test_unpublished_explorations_are_visible_to_their_editors(self) -> None:
    ...
    def test_published_explorations_are_visible_to_logged_out_users(self) -> None:
        rights_manager.publish_exploration(self.editor, self.EXP_ID)

        self.get_html_response('%s/%s' % (feconf.EXPLORATION_URL_PREFIX, self.EXP_ID))

    def test_published_explorations_are_visible_to_logged_in_users(self) -> None:
        rights_manager.publish_exploration(self.editor, self.EXP_ID)
    ...
    def test_cannot_perform_exploration_pretests_without_prerequisite_skill_ids(
        ...
        with self.swap(feconf, 'NUM_PRETEST_QUESTIONS', 1):
            with self.swap_to_always_return(
                story_domain.Story, 'get_prerequisite_skill_ids_for_exp_id', None
            ):
...
class QuestionsUnitTest(test_utils.GenericTestBase):
    ...
    def test_question_count_more_than_available_returns_all_questions(self) -> None:
        # Call the handler.
        url = '%s?question_count=%s&skill_ids=%s&fetch_by_difficulty=%s' % (
    ...
    def test_multiple_skill_id_returns_questions(self) -> None:
        ...
        self.assertItemsEqual([self.question_id, self.question_id_2, question_id_3],
                              question_ids)
    ...
    def test_filter_multiple_skill_id_return_questions(self) -> None:
        ...
        for _ in range(feconf.MAX_QUESTIONS_FETCHABLE_AT_ONE_TIME):
            ...
            self.save_new_question(
                question_id, 'user',
                self._create_valid_question_data('ABC', content_id_generator),
                [skill_id], content_id_generator.next_content_id_index
            )
            question_services.create_new_question_skill_link(
        ...
        url = '%s?question_count=%s&skill_ids=%s&fetch_by_difficulty=%s' % (
            feconf.QUESTIONS_URL_PREFIX, feconf.MAX_QUESTIONS_FETCHABLE_AT_ONE_TIME,
    ...
    def test_invalid_fetch_by_difficulty_raises_invalid_input_exception(self) -> None:
        # Call the handler.
        url = '%s?question_count=%s&skill_ids=%s&fetch_by_difficulty=%s' % (
...
class RecommendationsHandlerTests(test_utils.EmailTestBase):
    ...
    def _get_recommendation_ids(
        ...
        collection_id_param = (
            '&collection_id=%s' % collection_id if collection_id is not None else ''
        )
        include_recommendations_param = (
            '&include_system_recommendations=%s' % (include_system_recommendations)
    ...
    def test_logged_in_with_no_sysexps_no_authexps_no_col_has_no_exps(self) -> None:
    ...
    def test_logged_in_with_some_sysexps_no_authexps_no_col_has_some_exps(self) -> None:
    ...
    def test_logged_in_with_no_sysexps_some_authexps_no_col_has_some_exps(self) -> None:
    ...
    def test_logged_in_with_sysexps_and_authexps_no_col_has_some_exps(self) -> None:
        """Check there are recommended explorations when a user is logged in,
    ...
    def test_logged_in_no_sysexps_no_authexps_first_exp_in_col_has_exp(self) -> None:
    ...
    def test_logged_in_no_sysexps_no_authexps_mid_exp_in_col_has_exps(self) -> None:
        """Check there are recommended explorations when a user is logged in
    ...
    def test_logged_in_no_sysexps_no_authexps_all_exps_in_col_has_no_exps(self) -> None:
    ...
    def test_logged_in_with_sysexps_no_authexps_first_exp_in_col_has_exp(self) -> None:
        """Check there is a recommended exploration when a user is logged in
    ...
    def test_logged_in_with_sysexps_no_authexps_mid_exp_in_col_has_exps(self) -> None:
        """Check there are recommended explorations when a user is logged in
        and completes a middle exploration of the collection (since more
    ...
    def test_logged_in_sysexps_no_authexps_all_exps_in_col_has_no_exps(self) -> None:
        """Check there are not recommended explorations when a user is logged in
    ...
    def test_logged_in_no_sysexps_with_authexps_first_exp_in_col_has_exps(self) -> None:
    ...
    def test_logged_in_no_sysexps_with_authexps_mid_exp_in_col_has_exps(self) -> None:
        """Check there are recommended explorations when a user is logged in
    ...
    def test_logged_in_no_sysexps_authexps_all_exps_in_col_has_exps(self) -> None:
    ...
    def test_logged_out_with_no_sysexps_no_authexps_no_col_has_no_exps(self) -> None:
    ...
    def test_logged_out_with_sysexps_no_authexps_no_col_has_some_exps(self) -> None:
    ...
    def test_logged_out_no_sysexps_some_authexps_no_col_has_some_exps(self) -> None:
        """Check there are some recommended explorations when a user is logged
    ...
    def test_logged_out_with_sysexps_and_authexps_no_col_has_some_exps(self) -> None:
        """Check there are recommended explorations when a user is logged in,
        finishes an exploration in-viewer, and there are both author-specified
        exploration IDs and recommendations from the system.
        """
    ...
    def test_logged_out_no_sysexps_no_authexps_first_exp_in_col_has_exp(self) -> None:
    ...
    def test_logged_out_no_sysexps_no_authexps_mid_exp_in_col_has_exp(self) -> None:
        """Check there is a recommended exploration when a user is logged out
    ...
    def test_logged_out_no_sysexps_no_authexps_last_exp_col_has_no_exps(self) -> None:
    ...
    def test_logged_out_with_sysexps_no_authexps_first_exp_in_col_has_exp(self) -> None:
        """Check there is a recommended exploration when a user is logged out
        and completes the first exploration of a collection. Note that even
        though the completed exploration has system recommendations, they are
        ignored in favor of the collection's own recommendations.
    ...
    def test_logged_out_with_sysexps_no_authexps_mid_exp_in_col_has_exp(self) -> None:
    ...
    def test_logged_out_sysexps_no_authexps_last_exp_in_col_has_no_exps(self) -> None:
        """Check there are not recommended explorations when a user is logged
    ...
    def test_logged_out_no_sysexps_but_authexps_first_exp_in_col_has_exps(self) -> None:
    ...
    def test_logged_out_no_sysexps_with_authexps_mid_exp_in_col_has_exps(self) -> None:
        """Check there are recommended explorations when a user is logged out
    ...
    def test_logged_out_no_sysexps_with_dup_authexps_mid_col_exp_has_exps(self) -> None:
    ...
    def test_logged_out_no_sysexps_authexps_last_exp_in_col_has_exps(self) -> None:
        """Check there are still recommended explorations when a user is logged
    ...
    def test_get_recommendation_ids_with_invalid_author_recommended_ids(self) -> None:
        self.get_json(
...
class LearnerProgressTest(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.save_new_default_collection(
            self.COL_ID_0, self.owner_id, title='Welcome', category='Architecture'
        )
    ...
    def test_exp_incomplete_event_handler_with_no_version_raises_error(self) -> None:
        self.login(self.USER_EMAIL)
        csrf_token = self.get_new_csrf_token()
    ...
    def test_remove_collection_from_incomplete_list_handler(self) -> None:
        ...
        self.delete_json(
            '%s/%s/%s' % (
                feconf.LEARNER_INCOMPLETE_ACTIVITY_DATA_URL,
                constants.ACTIVITY_TYPE_COLLECTION, self.COL_ID_0
            )
        )
        self.assertEqual(
            learner_progress_services.get_all_incomplete_collection_ids(self.user_id),
        ...
        self.delete_json(
            '%s/%s/%s' % (
                feconf.LEARNER_INCOMPLETE_ACTIVITY_DATA_URL,
                constants.ACTIVITY_TYPE_STORY, self.STORY_ID
            )
        )
        self.assertEqual(
            learner_progress_services.get_all_incomplete_story_ids(self.user_id), []
        ...
        self.delete_json(
            '%s/%s/%s' % (
                feconf.LEARNER_INCOMPLETE_ACTIVITY_DATA_URL,
                constants.ACTIVITY_TYPE_LEARN_TOPIC, self.TOPIC_ID
            )
        )
        self.assertEqual(
            learner_progress_services.get_all_partially_learnt_topic_ids(self.user_id),
...
class StorePlaythroughHandlerTest(test_utils.GenericTestBase):
    ...
    def test_new_exp_issue_gets_created(self) -> None:
        ...
        for it.
        """
        self.playthrough_data['issue_customization_args']['state_name']['value'
                                                                       ] = 'state_name2'
...
class StatsEventHandlerTest(test_utils.GenericTestBase):
    ...
    def test_stats_events_handler(self) -> None:
        ...
        self.assertEqual(
            exploration_stats.state_stats_mapping[self.state_name].total_hit_count_v2, 1
        )
        self.assertEqual(
            exploration_stats.state_stats_mapping[self.state_name].first_hit_count_v2, 1
        )
        self.assertEqual(
            exploration_stats.state_stats_mapping[self.state_name
        ...
        self.assertEqual(
            exploration_stats.state_stats_mapping[self.state_name].num_completions_v2, 1
        )
        self.assertEqual(
            exploration_stats.state_stats_mapping[self.state_name
    ...
    def test_stats_events_handler_raise_error_with_invalid_state_stats_property(
        ...
        self.aggregated_stats['state_stats_mapping']['Home'][
            'total_hit_count'] = 'invalid'  # type: ignore[assignment]
...
class AnswerSubmittedEventHandlerTest(test_utils.GenericTestBase):
    ...
    def test_submit_answer_for_exploration_raises_error_with_no_version(self) -> None:
        exp_id = '6'
        exp_services.delete_demo(exp_id)
        exp_services.load_demo(exp_id)
...
class ExplorationStartEventHandlerTests(test_utils.GenericTestBase):
    ...
    def test_user_checkpoint_progress_is_none_on_fetching_older_exploration(
        ...
        exp_services.update_exploration(
            owner_id, exp_id, change_list, 'Made What language state a checkpoint'
        )
        ...
        self.assertEqual(
            exploration_dict['furthest_reached_checkpoint_state_name'], 'Welcome!'
        )
        self.assertEqual(
            exploration_dict['most_recently_reached_checkpoint_exp_version'], 2
        )
        self.assertEqual(
            exploration_dict['most_recently_reached_checkpoint_state_name'], 'Welcome!'
        )
...
class ExplorationActualStartEventHandlerTests(test_utils.GenericTestBase):
    ...
    def test_cannot_actually_start_a_state_with_no_exploration_version(self) -> None:
        self.login(self.VIEWER_EMAIL)
        # Load demo exploration.
        exp_id = '6'
        exp_services.delete_demo(exp_id)
...
class LearnerAnswerDetailsSubmissionHandlerTests(test_utils.GenericTestBase):
    ...
    def test_submit_learner_answer_details_for_exploration_states(self) -> None:
        ...
        with self.swap(constants, 'ENABLE_SOLICIT_ANSWER_DETAILS_FEATURE', True):
            ...
            interaction_id = exploration_dict['exploration']['states'][state_name][
                'interaction']['id']
            state_reference = (
                stats_services.get_state_reference_for_exploration(exp_id, state_name)
            )
...
class CheckpointReachedEventHandlerTests(test_utils.GenericTestBase):
    ...
    def test_user_checkpoint_progress_is_updated_correctly(self) -> None:
        ...
        self.assertEqual(
            exp_user_data.most_recently_reached_checkpoint_state_name, 'Welcome!'
        )
        ...
        self.assertEqual(
            updated_exp_user_data.most_recently_reached_checkpoint_exp_version, 2
        )
        self.assertEqual(
...
class ExplorationRestartEventHandlerTests(test_utils.GenericTestBase):
    ...
    def test_user_checkpoint_progress_is_updated_correctly_on_restart(self) -> None:
        self.signup(self.OWNER_EMAIL, self.OWNER_USERNAME)
        self.signup(self.VIEWER_EMAIL, self.VIEWER_USERNAME)
...
class SaveTransientCheckpointProgressHandlerTests(test_utils.GenericTestBase):
    ...
    def test_logged_out_user_checkpoint_progress_is_saved_correctly(self) -> None:
        self.signup(self.OWNER_EMAIL, self.OWNER_USERNAME)
    ...
    def test_logged_out_progress_is_displayed_correctly_when_exp_version_is_same( # pylint: disable=line-too-long
        ...
        self.assertTrue(
            url_response.headers['Location'].endswith(
                '%s/%s?pid=%s' % (
                    feconf.EXPLORATION_URL_PREFIX, exp_user_data.exploration_id,
                    unique_progress_url_id
                )
            )
        )
        ...
        exploration_dict = self.get_json(
            '%s/%s?pid=%s' % (
                feconf.EXPLORATION_INIT_URL_PREFIX, exp_user_data.exploration_id,
                unique_progress_url_id
            )
        )
        self.assertEqual(exploration_dict['furthest_reached_checkpoint_exp_version'], 1)
...
class StateVersionHistoryHandlerUnitTests(test_utils.GenericTestBase):
    ...
    def test_raises_error_when_version_history_does_not_exist(self) -> None:
        ...
        vh_model = exp_models.ExplorationVersionHistoryModel.get(
            exp_models.ExplorationVersionHistoryModel.get_instance_id(self.EXP_ID, 2)
        )
        vh_model.delete()
        ...
        self.assertEqual(
            response_for_state_a, {
                ...
                'state_dict_in_previous_version':
                    exploration_v1.states[feconf.DEFAULT_INIT_STATE_NAME].to_dict(),
