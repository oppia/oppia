...
class StoryEditorTests(BaseStoryEditorControllerTests):
    ...
    def test_put_fails_with_long_commit_message(self) -> None:
        ...
        self.assertIn(
            'Schema validation for \'commit_message\' failed: Validation failed: has_length_at_most ({\'max_value\': 375}) for object',
            json_response['error'],
        )
    ...
    def test_put_can_not_access_story_handler_with_mismatch_of_story_versions(
        ...
        self.assertEqual(
            json_response['error'],
            'Trying to update version 1 of story from version 2, which is too old. Please reload the page and try again.',
        )
