...
class EditorTests(BaseEditorControllerTests):
    ...
    def test_publish_exploration(self) -> None:
        ...
        exploration_rights = self.put_json(publish_url, {}, csrf_token=csrf_token)[
            'rights'
        ]
    ...
    def test_lock_exploration(self) -> None:
        ...
        self.put_json(
            edits_allowed_url, {'edits_are_allowed': False}, csrf_token=csrf_token
        )
...
class UserExplorationEmailsIntegrationTest(BaseEditorControllerTests):
    ...
    def test_user_exploration_emails_handler(self) -> None:
        ...
        csrf_token = self.get_new_csrf_token()
        exp_email_preferences = user_services.get_email_preferences_for_exploration(
...
class FetchIssuesPlaythroughHandlerTests(test_utils.GenericTestBase):
    ...
    def test_fetch_issues_handler_with_disabled_exp_id(self) -> None:
        self.get_json(
            '/issuesdatahandler/5', params={'exp_version': 2}, expected_status_int=404
        )
...
class EditorAutosaveTest(BaseEditorControllerTests):
    ...
    INVALID_CHANGELIST: Final = [
        {'cmd': 'edit_exploration_property', 'property_name': 'title', 'new_value': 1}
    ]
    ...
    def test_exploration_loaded_with_draft_applied(self) -> None:
        response = self.get_json(
            '/createhandler/data/%s' % self.EXP_ID2, params={'apply_draft': True}
        )
    ...
    def test_exploration_loaded_without_draft_as_draft_does_not_exist(self) -> None:
        response = self.get_json(
            '/createhandler/data/%s' % self.EXP_ID3, params={'apply_draft': True}
        )
        # Title not updated because change list not applied.
        self.assertEqual(response['title'], 'A title')
    ...
    def test_draft_not_updated_validation_error(self) -> None:
        ...
        self.assertEqual(
            response,
            {'status_code': 400, 'error': 'Expected title to be a string, received 1'},
        )
...
class LearnerAnswerInfoHandlerTests(BaseEditorControllerTests):
    ...
    def setUp(self) -> None:
        ...
        self.state_reference = stats_services.get_state_reference_for_exploration(
    ...
    def test_delete_learner_answer_info_of_exploration_states(self) -> None:
        ...
        with self.swap(constants, 'ENABLE_SOLICIT_ANSWER_DETAILS_FEATURE', True):
            ...
            learner_answer_info_id = learner_answer_details.learner_answer_info_list[
                0
            ].id
            ...
            updated_learner_answer_details = stats_services.get_learner_answer_details(
                self.entity_type, self.state_reference
            )
...
class ImageUploadHandlerTests(BaseEditorControllerTests):
    ...
    def test_upload_successful_when_image_uploaded(self) -> None:
        ...
        response = self.post_json(
            publish_url,
            {'image': 'img', 'filename': filename, 'filename_prefix': filename_prefix},
            csrf_token=csrf_token,
            expected_status_int=200,
            upload_files=[('image', 'unused_filename', raw_image)],
        )
