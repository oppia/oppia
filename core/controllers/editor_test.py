...
class EditorTests(BaseEditorControllerTests):
    ...
    def test_editor_page(self) -> None:
        ...
        self.assertIn(
            b'<exploration-editor-page></exploration-editor-page>', response.body
        )
    ...
    def test_that_default_exploration_cannot_be_published(self) -> None:
        ...
        exp_id = self.post_json(feconf.NEW_EXPLORATION_URL, {}, csrf_token=csrf_token)[
    ...
    def test_publish_exploration(self) -> None:
        ...
        exploration_rights = self.put_json(publish_url, {},
...
"""
    ...
    def test_exploration_download_handler_for_default_exploration(self) -> None:
        ...
        self.assertEqual(
            zf_saved.namelist(), ['The title for ZIP download handler test.yaml']
        )
...
class ExplorationDeletionRightsTests(BaseEditorControllerTests):
    ...
    def test_deletion_rights_for_unpublished_exploration(self) -> None:
        ...
        rights_manager.assign_role_for_exploration(
            self.owner, unpublished_exp_id, self.editor_id, rights_domain.ROLE_EDITOR
        )

        self.login(self.EDITOR_EMAIL)
        self.delete_json(
            '/createhandler/data/%s' % unpublished_exp_id, expected_status_int=401
        )
        self.logout()
        ...
        self.login(self.OWNER_EMAIL)
        self.delete_json(
            '/createhandler/data/%s' % unpublished_exp_id, expected_status_int=200
        )
        self.logout()
    ...
    def test_deletion_rights_for_published_exploration(self) -> None:
        ...
        rights_manager.assign_role_for_exploration(
            self.owner, published_exp_id, self.editor_id, rights_domain.ROLE_EDITOR
        )
        rights_manager.assign_role_for_exploration(
    ...
    def test_logging_info_after_deletion(self) -> None:
        ...
        with self.swap(logging, 'info', mock_logging_function
...
class VersioningIntegrationTest(BaseEditorControllerTests):
    ...
    def test_get_with_disabled_exploration_id_raises_error(self) -> None:
        self.get_html_response(
            '%s/%s' % (feconf.EDITOR_URL_PREFIX, feconf.DISABLED_EXPLORATION_IDS[0]),
            expected_status_int=404
        )
...
class ExplorationRightsIntegrationTest(BaseEditorControllerTests):
    ...
    def test_for_deassign_editor_role(self) -> None:
        ...
        self.login(self.OWNER_EMAIL)
        self.delete_json(rights_url, params={
    ...
    def test_can_not_assign_roles_with_invalid_payload_version(self) -> None:
        ...
        self.assertEqual(
            response_dict['error'], 'At \'http://localhost/createhandler/rights/eid\' '
            'these errors are happening:\n'
...
class UserExplorationEmailsIntegrationTest(BaseEditorControllerTests):
    ...
    def test_user_exploration_emails_handler(self) -> None:
        ...
        exp_email_preferences = (
            user_services.get_email_preferences_for_exploration(self.owner_id, exp_id)
        )
...
class ModeratorEmailsTests(test_utils.EmailTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.save_new_valid_exploration(
            self.EXP_ID, self.editor_id, title='My Exploration', end_state_name='END'
        )
    ...
    def test_email_is_sent_correctly_when_unpublishing(self) -> None:
        ...
        self.assertEqual(
            messages[0].sender, 'Site Admin <%s>' % feconf.SYSTEM_EMAIL_ADDRESS
        )
...
class FetchIssuesPlaythroughHandlerTests(test_utils.GenericTestBase):
    ...
    def test_fetch_issues_handler(self) -> None:
        ...
        response = self.get_json(
            '/issuesdatahandler/%s' % self.EXP_ID, params={
                'exp_version': 1
            }
        )
    ...
    def test_fetch_playthrough_handler(self) -> None:
        ...
        response = self.get_json(
            '/playthroughdatahandler/%s/%s' % (self.EXP_ID, self.playthrough_id1)
        )
...
class EditorAutosaveTest(BaseEditorControllerTests):
    ...
    def test_exploration_loaded_with_draft_applied(self) -> None:
        response = self.get_json(
            '/createhandler/data/%s' % self.EXP_ID2, params={
                'apply_draft': True
            }
        )
    ...
    def test_exploration_loaded_without_draft_as_draft_does_not_exist(self) -> None:
        response = self.get_json(
            '/createhandler/data/%s' % self.EXP_ID3, params={
                'apply_draft': True
            }
        )
        # Title not updated because change list not applied.
    ...
    def test_get_returns_assigned_interaction_ids_of_exploration_states(self) -> None:
        with self.login_context(self.OWNER_EMAIL) as owner_id:
            ...
            self.save_new_linear_exp_with_state_names_and_interactions(
                exp_id, owner_id, ['A', 'B', 'End'], ['FractionInput', 'TextInput']
            )
...
class LearnerAnswerInfoHandlerTests(BaseEditorControllerTests):
    ...
    def setUp(self) -> None:
        ...
        stats_services.record_learner_answer_info(
            self.entity_type, self.state_reference, self.interaction_id, self.answer,
            self.answer_details
        )
    ...
    def test_get_learner_answer_details_of_question_states(self) -> None:
        ...
        customization_args = (
            question.question_state_data.interaction.to_dict()['customization_args']
        )
        ...
        with self.swap(constants, 'ENABLE_SOLICIT_ANSWER_DETAILS_FEATURE', True):
            ...
            response = self.get_json(
                '%s/%s/%s' % (
                    feconf.LEARNER_ANSWER_INFO_HANDLER_URL, feconf.ENTITY_TYPE_QUESTION,
                    question_id
                )
            )
            self.assertEqual(response, learner_answer_info_data)
        ...
        with self.swap(constants, 'ENABLE_SOLICIT_ANSWER_DETAILS_FEATURE', True):
            ...
            self.delete_json(
                '%s/%s/%s?learner_answer_info_id=%s' % (
                    feconf.LEARNER_ANSWER_INFO_HANDLER_URL,
                    feconf.ENTITY_TYPE_EXPLORATION, self.exp_id, learner_answer_info_id
                ),
                expected_status_int=400
            )
            self.delete_json(
    ...
    def test_delete_learner_answer_info_of_question_states(self) -> None:
        ...
        with self.swap(constants, 'ENABLE_SOLICIT_ANSWER_DETAILS_FEATURE', True):
            ...
            self.delete_json(
                '%s/%s/%s?learner_answer_info_id=%s' % (
                    feconf.LEARNER_ANSWER_INFO_HANDLER_URL, feconf.ENTITY_TYPE_QUESTION,
                    question_id, learner_answer_info_id
                )
            )
            updated_learner_answer_details = (
                stats_services.get_learner_answer_details(
...
class ImageUploadHandlerTests(BaseEditorControllerTests):
    ...
    def test_return_error_when_image_not_uploaded(self) -> None:
        ...
        publish_url = '%s/%s/%s' % (
            feconf.EXPLORATION_IMAGE_UPLOAD_PREFIX, feconf.ENTITY_TYPE_EXPLORATION,
            exp_id
        )
