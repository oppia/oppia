...
class BulkEmailWebhookEndpoint(base.BaseHandler[Dict[str, str], Dict[str, str]]):
    ...
    def post(self, unused_secret: str) -> None:
        ...
        if (self.normalized_request['data[list_id]'] != feconf.MAILCHIMP_AUDIENCE_ID):
            self.render_json({})
            return
...
class MailingListSubscriptionHandler(
    base.BaseHandler[MailingListSubscriptionHandlerNormalizedPayloadDict, Dict[str,
                                                                               str]]
):
    ...
    def put(self) -> None:
        ...
        for update in updates:
            ...
            if update_type == 'subject_interests':
                ...
                non_boolean_keys = [
                    key for key, value in data.items() if not isinstance(value, bool)
                ]
                if non_boolean_keys:
                    raise self.InvalidInputException(
...
class SignupHandler(
    ...
    def get(self) -> None:
        ...
        self.render_json({
            ...
            'has_ever_registered':
                bool(user_settings.username and user_settings.last_agreed_to_terms),
            'username':
                user_settings.username,
        })
...
class SiteLanguageHandler(
    ...
    LANGUAGE_ID_PROVIDER_FUNC: Callable[[Dict[
        str, str]], str] = (lambda language_dict: language_dict['id'])
    URL_PATH_ARGS_SCHEMAS: Dict[str, str] = {}
    HANDLER_ARGS_SCHEMAS = {
        'PUT': {
            'site_language_code': {
