...
from typing import (
    Dict, List, Optional, OrderedDict, Sequence, Tuple, TypedDict, Union
)

ListOfContributorDashboardStatsTypes = Sequence[
    Union[suggestion_registry.TranslationContributionStats,
...
ListOfContributorDashboardStatsDictTypes = Sequence[
    Union[suggestion_registry.TranslationContributionStatsFrontendDict,
...
class ContributionOpportunitiesHandler(
    base.BaseHandler[Dict[str, str],
                     ContributionOpportunitiesHandlerNormalizedRequestDict]
):
    ...
    def get(self, opportunity_type: str) -> None:
        ...
        if opportunity_type == constants.OPPORTUNITY_TYPE_SKILL:
            skill_opportunities, next_cursor, more = (
                self.
                _get_skill_opportunities_with_corresponding_topic_name(search_cursor)
            )
        ...
        self.values = {
            'opportunities': (
                skill_opportunities if opportunity_type
                == constants.OPPORTUNITY_TYPE_SKILL else translation_opportunities
            ),
            ...
            for skill_opportunity in skill_opportunities:
                if (skill_opportunity.id in classroom_topic_skill_id_to_topic_name):
                    ...
                    client_side_skill_opportunity_dict: (
                        ...
                        'topic_name': (
                            classroom_topic_skill_id_to_topic_name[skill_opportunity.id]
                        )
                    }
    ...
    def _get_translation_opportunity_dicts(
        self, language_code: str, topic_name: Optional[str],
        search_cursor: Optional[str]
    ) -> Tuple[List[opportunity_domain.PartialExplorationOpportunitySummaryDict],
               Optional[str], bool]:
...
class ReviewableOpportunitiesHandler(
    ...
    def _get_reviewable_exploration_opportunity_summaries(
        ...
        topic_exp_ids_targeted_by_in_review_suggestions = [
            exp_id for exp_id in topic_exp_ids
...
class TranslatableTextHandler(
    base.BaseHandler[Dict[str, str], TranslatableTextHandlerNormalizedRequestDict]
):
    ...
    def _get_state_names_to_not_set_content_id_mapping(
        self, state_names_to_content_id_mapping: Dict[str, Dict[
            str, translation_domain.TranslatableContent]]
        ...
        for state_name in state_names_to_content_id_mapping:
            ...
            if content_id_to_unsubmitted_translatable_item:
                final_mapping[state_name] = {
                    cid: translatable_item.to_dict() for cid, translatable_item in
...
class MachineTranslationStateTextsHandler(
    base.BaseHandler[Dict[str, str],
                     MachineTranslationStateTextsHandlerNormalizedRequestDict]
):
    ...
    def get(self) -> None:
        ...
        state_names_to_content_id_mapping: (
            Dict[str, Dict[str, translation_domain.TranslatableContent]]
        ) = (translation_services.get_translatable_text(exp, target_language_code))
        ...
        for content_id in content_ids:
            ...
            source_text = content_id_to_translatable_item_mapping[content_id
                                                                 ].content_value
...
class TranslationPreferenceHandler(
    base.BaseHandler[TranslationPreferenceHandlerNormalizedRequestDict, Dict[str, str]]
):
...
class ContributorStatsSummariesHandler(
    ...
    def get(
        ...
        if contribution_subtype not in [
            feconf.CONTRIBUTION_SUBTYPE_SUBMISSION, feconf.CONTRIBUTION_SUBTYPE_REVIEW
        ]:
            raise self.InvalidInputException(
        ...
        if contribution_type == feconf.CONTRIBUTION_TYPE_TRANSLATION:
            if contribution_subtype == feconf.CONTRIBUTION_SUBTYPE_SUBMISSION:
                ...
                self.values = {
                    'translation_contribution_stats': _get_client_side_stats(stats)
                }
...
def _get_client_side_stats(
    ...
    topic_name_by_topic_id = {
        topic_summary.id: topic_summary.name for topic_summary in topic_summaries
