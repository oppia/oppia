...
class ReviewableOpportunitiesHandler(
    ...
    HANDLER_ARGS_SCHEMAS = {
        'GET': {
            'topic_name': {'schema': {'type': 'basestring'}, 'default_value': None},
            'language_code': {'schema': {'type': 'basestring'}, 'default_value': None},
        }
    }
...
class LessonsPinningHandler(
    ...
    HANDLER_ARGS_SCHEMAS = {
        'PUT': {
            ...
            'opportunity_id': {'schema': {'type': 'basestring'}, 'default_value': None},
        },
    }
...
class TranslatableTextHandler(
    ...
    def get(self) -> None:
        ...
        state_names_to_content_id_mapping = translation_services.get_translatable_text(
            exp, language_code
        )
...
class MachineTranslationStateTextsHandler(
    ...
    def get(self) -> None:
        ...
        if state_name not in state_names_to_content_id_mapping:
            raise self.NotFoundException()
        content_id_to_translatable_item_mapping = state_names_to_content_id_mapping[
            state_name
        ]
...
class ContributorCertificateHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
    ...
    URL_PATH_ARGS_SCHEMAS = {
        ...
        'suggestion_type': {
            'schema': {'type': 'basestring', 'choices': feconf.SUGGESTION_TYPE_CHOICES}
        },
    }
    HANDLER_ARGS_SCHEMAS = {
        'GET': {
