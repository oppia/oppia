...
(app_feedback_report_models, exp_models, suggestion_models, user_models) = models.Registry.import_models(
    [
...
class CronMailReviewerNewSuggestionsHandlerTests(test_utils.GenericTestBase):
    ...
    def test_email_not_sent_if_sending_reviewer_emails_is_not_enabled(
        ...
        with mock.patch.object(email_manager, 'send_reviewer_notifications', new_callable=mock.Mock) as mock_send:
            self.login(self.CURRICULUM_ADMIN_EMAIL, is_super_admin=True)

            with self.testapp_swap:
                self.get_json('/cron/mail/reviewers/new_contributor_dashboard_suggestions')
    ...
    def test_email_sent_to_reviewer_if_sending_reviewer_emails_is_enabled(
        ...
        with mock.patch.object(email_manager, 'send_reviewer_notifications', new_callable=mock.Mock) as mock_send:
            ...
            with self.testapp_swap:
                self.get_json('/cron/mail/reviewers/new_contributor_dashboard_suggestions')
            mock_send.assert_called_once_with({'en': [self.reviewer_id]}, {'en': [mock.ANY]})
    ...
    def test_email_not_sent_if_reviewer_ids_is_empty(self) -> None:
        ...
        with mock.patch.object(email_manager, 'send_reviewer_notifications', new_callable=mock.Mock) as mock_send:
            ...
            user_services.remove_translation_review_rights_in_language(self.reviewer_id, self.language_code)
            user_services.remove_translation_review_rights_in_language(self.reviewer_id, 'hi')
            with self.testapp_swap:
                self.get_json('/cron/mail/reviewers/new_contributor_dashboard_suggestions')
...
class CronMailAdminContributorDashboardBottlenecksHandlerTests(test_utils.GenericTestBase):
    ...
    def test_cron_exploration_recommendations_handler(self) -> None:
        ...
        swap_with_checks = self.swap_with_checks(
            ...
            expected_kwargs=[
                {
                    'job_class': (exp_recommendation_computation_jobs.ComputeExplorationRecommendationsJob),
                }
            ],
        )
