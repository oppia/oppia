...
class StoryProgressHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
    ...
    def _record_node_completion(
        self, story_id: str, node_id: str, completed_node_ids: List[str],
    ...
    def get(self, story_id: str, node_id: str) -> None:
        ...
        if completed_nodes or node_id != ordered_nodes[0].id:
            self.redirect(
                '/learn/%s/%s/story/%s' %
                (classroom_url_fragment, topic_url_fragment, story_url_fragment)
            )
            return

        (next_exp_ids, next_node_id,
         _) = (self._record_node_completion(story_id, node_id, [], ordered_nodes))
    ...
    def post(self, story_id: str, node_id: str) -> None:
        ...
        exp_summaries = (
            summary_services.
            get_displayable_exp_summary_dicts_matching_ids(next_exp_ids)
        )
        ...
        questions_available = len(
            question_services.get_questions_by_skill_ids(1, acquired_skill_ids, False)
        ) > 0

        learner_completed_story = len(completed_node_ids) == len(ordered_nodes)
        learner_at_review_point_in_story = (
            len(exp_summaries) != 0 and
            (len(completed_node_ids) & constants.NUM_EXPLORATIONS_PER_REVIEW_TEST == 0)
        )
