...
class StoryProgressHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
    ...
    def post(self, story_id: str, node_id: str) -> None:
        ...
        (next_exp_ids, next_node_id, completed_node_ids) = self._record_node_completion(
            story_id, node_id, completed_node_ids, ordered_nodes
        )

        ready_for_review_test = False
        exp_summaries = summary_services.get_displayable_exp_summary_dicts_matching_ids(
            next_exp_ids
        )
        ...
        completed_story_ids = learner_progress_services.get_all_completed_story_ids(
            self.user_id
        )
        story_ids_in_topic = []
