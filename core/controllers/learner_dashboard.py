...
class LearnerDashboardTopicsAndStoriesProgressHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
    ...
    def get(self) -> None:
        ...
        learnt_topic_summary_dicts = learner_progress_services.get_displayable_topic_summary_dicts(
            self.user_id,
        ...
        all_topic_summary_dicts = learner_progress_services.get_displayable_topic_summary_dicts(
            self.user_id,
        ...
        self.values.update(
            {
                ...
                'number_of_nonexistent_topics_and_stories': (number_of_nonexistent_topics_and_stories),
...
class LearnerDashboardCollectionsProgressHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
    ...
    def get(self) -> None:
        ...
        for summary_dict in completed_collection_summary_dicts:
            summary_dict['completed_node_count'] = summary_dict['total_node_count']

        # Fetch completed node counts for incomplete collections.
        incomplete_collection_ids = [d['id'] for d in incomplete_collection_summary_dicts]
        completed_exps_per_collection = collection_services.get_explorations_completed_in_collections(self.user_id, incomplete_collection_ids)
        ...
        self.values.update(
            {
                ...
                'number_of_nonexistent_collections': (number_of_nonexistent_collections),
                'completed_to_incomplete_collections': (learner_progress.completed_to_incomplete_collections),
            }
        )
        self.render_json(self.values)
...
class LearnerDashboardExplorationsProgressHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
    ...
    def get(self) -> None:
        ...
        for summary_dict in incomplete_exp_summary_dicts + completed_exp_summary_dicts + exploration_playlist_summary_dicts:
        ...
        progress_by_exp_id = learner_progress_services.get_checkpoint_progress_for_explorations(self.user_id, exploration_ids_for_progress)
        default_progress_data: learner_progress_services.ExplorationCheckpointProgressDict = {
        ...
        for summary_dict in incomplete_exp_summary_dicts:
            progress_data = progress_by_exp_id.get(summary_dict['id'], default_progress_data)
            summary_dict['visited_checkpoints_count'] = progress_data['visited_checkpoints_count']
            summary_dict['total_checkpoints_count'] = progress_data['total_checkpoints_count']
        ...
        self.values.update(
            {
                ...
                'number_of_nonexistent_explorations': (number_of_nonexistent_explorations),
