...
class LearnerDashboardTopicsAndStoriesProgressHandler(
    ...
    def get(self) -> None:
        ...
        all_topic_summary_dicts = (
            learner_progress_services.get_displayable_topic_summary_dicts(
                self.user_id, learner_progress_in_topics_and_stories.all_topic_summaries
            )
        )
...
class LearnerCompletedChaptersCountHandler(
    ...
    def get(self) -> None:
        ...
        assert self.user_id is not None
        learner_progress_in_topics_and_stories = (
            learner_progress_services.get_topics_and_stories_progress(self.user_id)[0]
        )
    ...
    def get(self) -> None:
        ...
        for index, creator_settings in enumerate(creators_settings):
            subscription_summary = {
                ...
                'creator_impact': (
                    user_services.get_user_impact_score(creators_subscribed_to[index])
                )
            }
...
class LearnerDashboardIdsHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
    ...
    def get(self) -> None:
        ...
        self.values.update({
            'learner_dashboard_activity_ids': (learner_dashboard_activities.to_dict())
        })
        self.render_json(self.values)
