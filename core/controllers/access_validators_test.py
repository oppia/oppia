...
class ClassroomsPageAccessValidationHandlerTests(test_utils.GenericTestBase):
    ...
    def test_validation_returns_false_if_no_public_classrooms_are_present(self) -> None:
...
class ExplorationPlayerAccessValidationPageTests(test_utils.GenericTestBase):
    ...
    def test_exploration_player_page_with_valid_exploration_version(self) -> None:
    ...
    def test_handler_raises_error_with_invaild_collection(self) -> None:
        ...
        self.get_html_response(
            '%s/can_access_exploration_player_page/%s'
            % (ACCESS_VALIDATION_HANDLER_PREFIX, self.exploration.id),
            params={'v': self.exploration.version, 'collection_id': 'aZ9_______12'},
            expected_status_int=404,
        )
        self.logout()
...
class DiagnosticTestPlayerPageAccessValidationHandlerTests(test_utils.GenericTestBase):
    ...
    @test_utils.enable_feature_flags([feature_flag_list.FeatureNames.DIAGNOSTIC_TEST])
...
class EditLearnerGroupPageAccessValidationHandlerTests(test_utils.GenericTestBase):
    ...
    def test_validation_returns_false_with_user_not_being_a_facilitator(self) -> None:
        self.login(self.NEW_USER_EMAIL)
        self.get_json(
            '%s/can_access_edit_learner_group_page/%s'
            % (ACCESS_VALIDATION_HANDLER_PREFIX, self.LEARNER_GROUP_ID),
...
class CollectionEditorAccessValidationPage(test_utils.GenericTestBase):
    ...
    def test_validation_returns_false_if_given_collection_id_non_existent(self) -> None:
        self.login(self.guest_email)
        self.get_html_response(
