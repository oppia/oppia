...
class DiagnosticTestQuestionsHandler(
    ...
    def get(self, topic_id: str) -> None:
        ...
        request_data = cast(
            DiagnosticTestQuestionsHandlerNormalizedRequestDict, self.normalized_request
        )
        ...
        skill_id_to_questions_map: Dict[
            str, List[question_domain.Question]] = (collections.defaultdict(list))

        for skill_id in diagnostic_test_skill_ids:
            questions = question_services.get_questions_by_skill_ids(
                feconf.MAX_QUESTIONS_FETCHABLE_AT_ONE_TIME, [skill_id],
        ...
        # the same difficulty.
        skill_id_to_questions_dict: Dict[str, Dict[
            str, question_domain.QuestionDict]] = (collections.defaultdict(dict))
