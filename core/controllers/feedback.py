...
def update_original_and_last_message_author_id_in_feedback_thread_dicts(
    ...
    for feedback_thread_dict in feedback_thread_dicts:
        last_nonempty_message_author_id = feedback_thread_dict['last_nonempty_message_author_id']
        updated_feedback_thread_dict: UpdatedLastMessageAuthorFeedbackThreadDict = {
            ...
            'original_author_username': (user_services.get_username(feedback_thread_dict['original_author_id']) if feedback_thread_dict['original_author_id'] else None),
            ...
            'last_nonempty_message_text': (feedback_thread_dict['last_nonempty_message_text']),
            'last_nonempty_message_author': (user_services.get_username(last_nonempty_message_author_id) if last_nonempty_message_author_id else None),
        }
...
def update_author_id_in_message_dicts(
    ...
    for message_dict in message_dicts:
        updated_message_dict: UpdatedFeedbackMessageDict = {
            'author_username': (user_services.get_username(message_dict['author_id']) if message_dict['author_id'] else None),
...
class ThreadListHandler(base.BaseHandler[ThreadListHandlerNormalizedPayloadDict, Dict[str, str]]):
    ...
    def get(self, exploration_id: str) -> None:
        feedback_thread_dicts = [thread.to_dict() for thread in feedback_services.get_all_threads(feconf.ENTITY_TYPE_EXPLORATION, exploration_id, False)]
        self.values.update(
            {
...
class ThreadListHandlerForTopicsHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
    ...
    def get(self, topic_id: str) -> None:
        suggestion_thread_dicts = [thread.to_dict() for thread in feedback_services.get_all_threads(feconf.ENTITY_TYPE_TOPIC, topic_id, True)]
        self.values.update({'suggestion_thread_dicts': (update_original_author_id_in_feedback_thread_dicts(suggestion_thread_dicts))})
        self.render_json(self.values)
...
class ThreadHandler(base.BaseHandler[ThreadHandlerNormalizedPayloadDict, Dict[str, str]]):
    ...
    def get(self, thread_id: str) -> None:
        ...
        message_dicts = [message.to_dict() for message in feedback_services.get_messages(thread_id)]
    ...
    def post(self, thread_id: str) -> None:
        ...
        if suggestion and updated_status:
            raise self.InvalidInputException('Suggestion thread status cannot be changed manually.')

        messages = feedback_services.get_messages(thread_id)
        new_message = feedback_services.create_message(thread_id, self.user_id, updated_status, updated_subject, text)
...
class RecentFeedbackMessagesHandler(base.BaseHandler[Dict[str, str], RecentFeedbackMessagesHandlerNormalizedRequestDict]):
    ...
    HANDLER_ARGS_SCHEMAS = {'GET': {'cursor': {'schema': {'type': 'basestring'}, 'default_value': None}}}
    ...
    def get(self) -> None:
        ...
        all_feedback_messages, new_urlsafe_start_cursor, more = feedback_services.get_next_page_of_all_feedback_messages(urlsafe_start_cursor=urlsafe_start_cursor)
...
class FeedbackStatsHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
    ...
    @acl_decorators.can_play_exploration
    def get(self, exploration_id: str) -> None:
        feedback_thread_analytics = feedback_services.get_thread_analytics(exploration_id)
        self.values.update(
            {
...
class FeedbackThreadViewEventHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
    ...
    def post(self, thread_id: str) -> None:
        ...
        feedback_services.clear_feedback_message_references_transactional(self.user_id, exploration_id, thread_id)
        self.render_json(self.values)
