...
EXPECTED_THREAD_KEYS: Final = [
    ...
    'subject', 'last_updated_msecs', 'message_count', 'last_nonempty_message_text',
    'last_nonempty_message_author'
]
EXPECTED_MESSAGE_KEYS: Final = [
    'author_username', 'created_on_msecs', 'entity_type', 'message_id', 'entity_id',
...
class FeedbackThreadIntegrationTests(test_utils.GenericTestBase):
    ...
    def test_missing_thread_subject_raises_400_error(self) -> None:
        ...
        self.assertEqual(
            response_dict['error'], 'At \'http://localhost/threadlisthandler/0\' '
    ...
    def test_no_username_shown_for_logged_out_learners(self) -> None:
        ...
        response_dict = self.get_json(
            '%s/%s' % (feconf.FEEDBACK_THREAD_URL_PREFIX, threadlist[0]['thread_id'])
        )
    ...
    def test_message_id_assignment_for_multiple_posts_to_same_thread(self) -> None:
        ...
        self.assertEqual(
            response_dict['messages'][0]['author_username'], self.EDITOR_USERNAME
        )
...
class FeedbackThreadTests(test_utils.GenericTestBase):
    ...
    def test_post_feedback_threads_with_no_text_and_no_updated_status_raise_400(
        ...
        thread_id = feedback_services.create_thread(
            feconf.ENTITY_TYPE_EXPLORATION, self.EXP_ID, self.owner_id_1, 'a subject',
            'some text'
        )
    ...
    def test_post_feedback_threads_with_updated_suggestion_status_raises_400(
        ...
        suggestion_services.create_suggestion(
            ...
            self.EXP_ID, 1, self.owner_id_1, change, 'sample description'
        )
        ...
        self.assertEqual(
            response['error'], 'Suggestion thread status cannot be changed manually.'
        )
...
class ThreadListHandlerForTopicsHandlerTests(test_utils.GenericTestBase):
    ...
    def test_get_feedback_threads_linked_to_topics(self) -> None:
        ...
        response_dict = self.get_json(
            '%s/%s' % (feconf.FEEDBACK_THREADLIST_URL_PREFIX_FOR_TOPICS, self.topic_id)
        )
        ...
        self.assertEqual(
            suggestion_thread_dicts['original_author_username'], self.OWNER_USERNAME
        )
...
class RecentFeedbackMessagesHandlerTests(test_utils.GenericTestBase):
    ...
    def test_get_recently_posted_feedback_messages(self) -> None:
        ...
        feedback_services.create_thread(
            feconf.ENTITY_TYPE_EXPLORATION, self.exp_id, self.moderator_id, 'a subject',
            'some text'
        )
