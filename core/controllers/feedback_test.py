...
class FeedbackThreadIntegrationTests(test_utils.GenericTestBase):
    ...
    def test_message_id_assignment_for_multiple_posts_to_same_thread(self) -> None:
        ...
        for num in range(num_users):
            ...
            csrf_token = self.get_new_csrf_token()
            self.post_json(
                thread_url, {'text': 'New Message %s' % num}, csrf_token=csrf_token
            )
            self.logout()
...
class FeedbackThreadTests(test_utils.GenericTestBase):
    ...
    def test_feedback_threads(self) -> None:
        ...
        self.post_json(
            thread_url,
            {'updated_status': None, 'updated_subject': None, 'text': 'Message 2'},
            csrf_token=csrf_token,
        )
    ...
    def test_post_feedback_threads_with_updated_suggestion_status_raises_400(
        ...
        response = self.post_json(
            thread_url,
            {'text': 'Message 1', 'updated_subject': None, 'updated_status': 'open'},
            csrf_token=csrf_token,
            expected_status_int=400,
        )
