...
class ContributionOpportunitiesHandlerTest(test_utils.GenericTestBase):
    ...
    def test_get_translation_opportunities_fetches_matching_opportunities(self) -> None:
        ...
        self.assertEqual(
            response['opportunities'],
            [self.expected_opportunity_dict_1, self.expected_opportunity_dict_2],
        )
        self.assertFalse(response['more'])
        self.assertIsInstance(response['next_cursor'], str)
    ...
    def test_get_skill_opportunity_data_pagination(self) -> None:
        with self.swap(constants, 'OPPORTUNITIES_PAGE_SIZE', 1):
            ...
            self.assertEqual(
                response['opportunities'], [self.expected_skill_opportunity_dict_0]
            )
            ...
            self.assertEqual(
                next_response['opportunities'], [self.expected_skill_opportunity_dict_1]
            )
    ...
    def test_get_translation_opportunity_data_pagination(self) -> None:
        with self.swap(constants, 'OPPORTUNITIES_PAGE_SIZE', 1):
            ...
            self.assertEqual(len(next_response['opportunities']), 1)
            self.assertItemsEqual(
                next_response['opportunities'], [self.expected_opportunity_dict_2]
            )
            self.assertFalse(next_response['more'])
            self.assertIsInstance(next_response['next_cursor'], str)
    ...
    def test_get_reviewable_translation_opportunities_returns_in_review_suggestions(  # pylint: disable=line-too-long
        ...
        response = self.get_json(
            '%s' % feconf.REVIEWABLE_OPPORTUNITIES_URL, params={'topic_name': 'topic'}
        )
    ...
    def test_get_reviewable_translation_opportunities_filtering_language(  # pylint: disable=line-too-long
        ...
        response = self.get_json(
            '%s' % feconf.REVIEWABLE_OPPORTUNITIES_URL, params={'language_code': 'es'}
        )
        ...
        response = self.get_json(
            '%s' % feconf.REVIEWABLE_OPPORTUNITIES_URL, params={'language_code': 'hi'}
        )
        ...
        response = self.get_json(
            '%s' % feconf.REVIEWABLE_OPPORTUNITIES_URL, params={'language_code': 'pt'}
        )
        # Should be empty.
    ...
    def test_pin_translation_opportunity_with_language_code_set_to_none(self) -> None:
...
class TranslatableTextHandlerTest(test_utils.GenericTestBase):
    ...
    def test_handler_with_user_reviewable_language_code(self) -> None:
        self.login(self.OWNER_EMAIL)
        user_services.allow_user_to_review_translation_in_language(self.owner_id, 'hi')
    ...
    def test_handler_returns_correct_data(self) -> None:
        ...
        output = self.get_json(
            '/gettranslatabletexthandler', params={'language_code': 'hi', 'exp_id': '0'}
        )
    ...
    def test_get_featured_translation_languages(self) -> None:
        ...
        expected_response = {
            'featured_translation_languages': [
                ...
                {'language_code': 'pcm', 'explanation': 'For learners in Nigeria.'},
                {
                    'language_code': 'es',
                ...
                {'language_code': 'ha', 'explanation': 'For learners in Nigeria.'},
                {'language_code': 'ig', 'explanation': 'For learners in Nigeria.'},
                {'language_code': 'yo', 'explanation': 'For learners in Nigeria.'},
            ]
        }
...
class ContributorStatsSummariesHandlerTest(test_utils.GenericTestBase):
    ...
    def test_get_translation_review_stats(self) -> None:
        ...
        response = self.get_json(
            '/contributorstatssummaries/translation/review/%s' % (self.OWNER_USERNAME)
        )
    ...
    def test_get_question_contribution_stats(self) -> None:
        ...
        response = self.get_json(
            '/contributorstatssummaries/question/submission/%s' % (self.OWNER_USERNAME)
        )
    ...
    def test_get_question_review_stats(self) -> None:
        ...
        response = self.get_json(
            '/contributorstatssummaries/question/review/%s' % (self.OWNER_USERNAME)
        )
    ...
    def test_get_all_stats_of_other_users_raises_error(self) -> None:
        ...
        response = self.get_json(
            '/contributorstatssummaries/question/review/abc', expected_status_int=401
        )
