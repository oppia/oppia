...
class ExplorationImprovementsHandlerTests(ImprovementsTestBase):
    ...
    def get_url(self, exp_id: Optional[str] = None) -> str:
        ...
        """
        return '%s/%s/%s' % (
            feconf.IMPROVEMENTS_URL_PREFIX, constants.TASK_ENTITY_TYPE_EXPLORATION,
    ...
    def test_get_returns_open_tasks(self) -> None:
        ...
        with self.login_context(self.OWNER_EMAIL):
            self.assertEqual(
                self.get_json(self.get_url()), {
                    'open_tasks': [
                        improvements.get_task_dict_with_username_and_profile_picture(t)
    ...
    def test_get_returns_resolved_tasks(self) -> None:
        task_entries = [
            self._new_resolved_task(
                state_name=name, task_type=constants.TASK_TYPE_HIGH_BOUNCE_RATE
            ) for name in ['A', 'B', 'C']
        ]
        improvements_services.put_tasks(task_entries)
    ...
    def test_post_invalid_exploration_returns_invalid_input_page(self) -> None:
        with self.login_context(self.OWNER_EMAIL):
            self.post_json(
                self.get_url(exp_id='bad_exp_id'), {
                    'task_entries': [{
                        'entity_version': 1,
                        'task_type': 'high_bounce_rate',
                        'target_id': 'Introduction',
    ...
    def test_post_with_missing_issue_description_is_allowed(self) -> None:
        ...
        task_id = (
            improvements_models.ExplorationStatsTaskEntryModel.generate_task_id(
                constants.TASK_ENTITY_TYPE_EXPLORATION, self.exp.id, self.exp.version,
                constants.TASK_TYPE_HIGH_BOUNCE_RATE, constants.TASK_TARGET_TYPE_STATE,
    ...
    def test_post_can_create_new_open_task_in_storage(self) -> None:
        ...
        self.assertEqual(
            task_entry_model.entity_type, constants.TASK_ENTITY_TYPE_EXPLORATION
        )
        self.assertEqual(task_entry_model.entity_id, self.exp.id)
        self.assertEqual(task_entry_model.entity_version, self.exp.version)
        self.assertEqual(
            task_entry_model.task_type, constants.TASK_TYPE_HIGH_BOUNCE_RATE
        )
    ...
    def test_to_dict_with_invalid_resolver_id_raises_exception(self) -> None:
        ...
        task_entry = improvements_domain.TaskEntry(
            ...
            constants.TASK_STATUS_RESOLVED, invalid_resolver_id, self.MOCK_DATE
        )
    ...
    def test_to_dict_with_non_existing_resolver_id(self) -> None:
        task_entry = improvements_domain.TaskEntry(
            ...
            constants.TASK_TYPE_HIGH_BOUNCE_RATE, constants.TASK_TARGET_TYPE_STATE,
            feconf.DEFAULT_INIT_STATE_NAME, 'issue description',
            constants.TASK_STATUS_RESOLVED, None, self.MOCK_DATE
        )
...
class ExplorationImprovementsHistoryHandlerTests(ImprovementsTestBase):
    ...
    def get_url(
        self, exp_id: Optional[str] = None, cursor: Optional[str] = None
    ) -> str:
        """Returns the URL corresponding to the handler.
...
class ExplorationImprovementsConfigHandlerTests(test_utils.GenericTestBase):
    ...
    @test_utils.set_platform_parameters([
        ...
        (
            platform_parameter_list.ParamName.
            HIGH_BOUNCE_RATE_TASK_STATE_BOUNCE_RATE_OBSOLETION_THRESHOLD, 0
        ),  # pylint: disable=line-too-long
        (
            platform_parameter_list.ParamName.
            HIGH_BOUNCE_RATE_TASK_MINIMUM_EXPLORATION_STARTS, 0
        )  # pylint: disable=line-too-long
    ])
    def test_custom_high_bounce_rate_creation_threshold(self) -> None:
        ...
        self.assertAlmostEqual(
            json_response['high_bounce_rate_task_state_bounce_rate_creation_threshold'],
            0.35
        )
    ...
    @test_utils.set_platform_parameters([
        (
            platform_parameter_list.ParamName.
            HIGH_BOUNCE_RATE_TASK_STATE_BOUNCE_RATE_CREATION_THRESHOLD, 0
        ),  # pylint: disable=line-too-long
        (
            platform_parameter_list.ParamName.
            HIGH_BOUNCE_RATE_TASK_STATE_BOUNCE_RATE_OBSOLETION_THRESHOLD, 0.05
        ),  # pylint: disable=line-too-long
        (
            platform_parameter_list.ParamName.
            HIGH_BOUNCE_RATE_TASK_MINIMUM_EXPLORATION_STARTS, 0
    ...
    def test_custom_high_bounce_rate_obsoletion_threshold(self) -> None:
        ...
        self.assertAlmostEqual(
            json_response['high_bounce_rate_task_state_bounce_rate_obsoletion_threshold'
                         ], 0.05
        )
    ...
    @test_utils.set_platform_parameters([
        ...
        (
            platform_parameter_list.ParamName.
            HIGH_BOUNCE_RATE_TASK_MINIMUM_EXPLORATION_STARTS, 20
        )  # pylint: disable=line-too-long
    ])
    def test_custom_high_bounce_rate_task_minimum_exploration_starts(self) -> None:
        ...
        self.assertAlmostEqual(
            json_response['high_bounce_rate_task_minimum_exploration_starts'], 20
        )
