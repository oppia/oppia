...
class BasePracticeSessionsControllerTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.topic = topic_domain.Topic.create_default_topic(
            self.topic_id, 'public_topic_name', 'public-topic-name', 'description',
            'fragm'
        )
        ...
        self.topic = topic_domain.Topic.create_default_topic(
            self.topic_id_1, 'private_topic_name', 'private-topic-name', 'description',
            'fragm'
        )
        self.topic.thumbnail_filename = 'Topic.svg'
        self.topic.thumbnail_bg_color = (
...
class PracticeSessionsPageDataHandlerTests(BasePracticeSessionsControllerTests):
    ...
    def test_get_fails_when_skill_ids_dont_exist(self) -> None:
        topic = topic_domain.Topic.create_default_topic(
            'topic_id_3', 'topic_without_skills', 'noskills', 'description', 'fragm'
        )
    ...
    def test_any_user_can_access_practice_sessions_data(self) -> None:
        ...
        self.assertEqual(
            json_response['skill_ids_to_descriptions_map']['skill_id_1'], 'Skill 1'
        )
        self.assertEqual(
            json_response['skill_ids_to_descriptions_map']['skill_id_2'], 'Skill 2'
        )
