...
class SuggestionUnitTests(test_utils.GenericTestBase):
    ...
    def test_saves_new_images_uploaded_by_translation_submitter(self) -> None:
        ...
        exploration = self.save_new_linear_exp_with_state_names_and_interactions(
            exp_id, self.editor_id, ['State 1'], ['EndExploration'], category='Algebra'
        )
    ...
    def test_cannot_update_question_without_next_content_id_index(self) -> None:
        ...
        response = self.post_json(
            '%s/%s'
            % (feconf.UPDATE_QUESTION_SUGGESTION_URL_PREFIX, suggestion.suggestion_id),
            {'question_state_data': question_state_data, 'skill_difficulty': 0.6},
            csrf_token=csrf_token,
            expected_status_int=400,
        )
        ...
        question_state_dict: state_domain.StateDict = {
            ...
            'interaction': {
                'answer_groups': [
                    {
                        'rule_specs': [
                            {'rule_type': 'IsInRegion', 'inputs': {'x': 'Region1'}}
                        ],
                        'outcome': {
                            'dest': None,
                ...
                'hints': [
                    {'hint_content': {'html': '<p>assaas</p>', 'content_id': 'hint_3'}}
                ],
                'id': 'ImageClickInput',
                'solution': None,
            },
...
class UserSubmittedSuggestionsHandlerTest(test_utils.GenericTestBase):
    ...
    def test_translation_suggestions_fetches_extra_page_if_filtered_result_is_empty(  # pylint: disable=line-too-long
        ...
        response = self.get_json(
            '/getsubmittedsuggestions/exploration/translate_content',
            {'limit': 1, 'offset': 0, 'sort_key': constants.SUGGESTIONS_SORT_KEY_DATE},
        )
