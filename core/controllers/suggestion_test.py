...
class SuggestionUnitTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.exploration = (
            self.save_new_linear_exp_with_state_names_and_interactions(
                self.EXP_ID,
                self.editor_id, ['State 1', 'State 2', 'State 3'], ['TextInput'],
                category='Algebra'
            )
        )
        ...
        suggestion_services.create_suggestion(
            ...
            self.EXP_ID, self.exploration.version, self.author_id, {
        ...
        suggestion_services.create_suggestion(
            feconf.SUGGESTION_TYPE_EDIT_STATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            self.EXP_ID, self.exploration.version, self.author_id_2, {
                'cmd': exp_domain.CMD_EDIT_STATE_PROPERTY,
                'property_name': exp_domain.STATE_PROPERTY_CONTENT,
                'state_name': 'State 2',
                'old_value': {
    ...
    def test_suggestion_to_exploration_handler_with_invalid_suggestion_id(self) -> None:
        ...
        suggestion_to_accept = self.get_json(
            '%s?author_id=%s' % (feconf.SUGGESTION_LIST_URL_PREFIX, self.author_id)
        )['suggestions'][0]
        ...
        response = self.put_json(
            '%s/exploration/%s/%s' % (
                feconf.SUGGESTION_ACTION_URL_PREFIX, suggestion_to_accept['target_id'],
    ...
    def test_suggestion_to_exploration_handler_with_invalid_target_type(self) -> None:
        ...
        suggestion_services.create_suggestion(
            feconf.SUGGESTION_TYPE_ADD_QUESTION, feconf.ENTITY_TYPE_TOPIC, exp_id, 1,
            self.author_id, {
    ...
    def test_owner_of_exploration_cannot_respond_to_own_suggestion(self) -> None:
        self.login(self.EDITOR_EMAIL)
        ...
        suggestion_services.create_suggestion(
            feconf.SUGGESTION_TYPE_EDIT_STATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            exp_id, 1, self.editor_id, change_cmd, 'sample description'
        )
    ...
    def test_suggestion_to_exploration_handler_with_invalid_action(self) -> None:
        ...
        self.assertIn(
            ...
            'range of choices', response['error']
        )
    ...
    def test_reject_suggestion_to_exploration(self) -> None:
        ...
        self.put_json(
            '%s/exploration/%s/%s' % (
                feconf.SUGGESTION_ACTION_URL_PREFIX, suggestion_to_reject['target_id'],
                suggestion_to_reject['suggestion_id']
            ), {
                'action': 'reject',
                'review_message': 'Rejected!'
    ...
    def test_suggestion_to_exploration_handler_with_long_commit_mesage(self) -> None:
        ...
        self.assertIn(
            ...
            'failed: has_length_at_most', response['error']
        )
    ...
    def test_accept_suggestion(self) -> None:
        ...
        exploration = exp_fetchers.get_exploration_by_id(self.EXP_ID)
        self.assertEqual(
            exploration.states[suggestion_to_accept['change_cmd']['state_name']
                              ].content.html,
        ...
        suggestion_to_accept = self.get_json(
            '%s?author_id=%s' % (feconf.SUGGESTION_LIST_URL_PREFIX, self.author_id_2)
        )['suggestions'][0]
    ...
    def test_resubmit_rejected_suggestion(self) -> None:
        ...
        self.assertEqual(suggestion.status, suggestion_models.STATUS_IN_REVIEW)
        self.assertEqual(
            suggestion.change_cmd.new_value['html'], resubmit_change_content_html
        )
    ...
    def test_translation_accept_suggestion_by_reviewer(self) -> None:
        ...
        suggestion_to_accept = self.get_json(
            '%s?author_id=%s' % (feconf.SUGGESTION_LIST_URL_PREFIX, self.translator_id)
        )['suggestions'][0]

        csrf_token = self.get_new_csrf_token()
        with self.swap(
            opportunity_services,
            'update_translation_opportunity_with_accepted_suggestion', lambda x, _: x
        ):
            self.put_json(
                '%s/exploration/%s/%s' % (
                    feconf.SUGGESTION_ACTION_URL_PREFIX,
    ...
    def test_saves_new_images_uploaded_by_translation_submitter(self) -> None:
        ...
        exploration = (
            self.save_new_linear_exp_with_state_names_and_interactions(
                exp_id,
                self.editor_id, ['State 1'], ['EndExploration'],
                category='Algebra'
            )
        )
    ...
    def test_raises_exception_if_new_contentless_image_in_translation(self) -> None:
        exp_id = '12345678exp1'
        exploration = (
            self.save_new_linear_exp_with_state_names_and_interactions(
                exp_id,
    ...
    def test_raises_exception_if_target_exploration_image_is_not_in_target_exploration_assets( # pylint: disable=line-too-long
        ...
        self.assertIn(
            ...
            'before submitting this translation again.' % exp_id, response_dict['error']
        )
    ...
    def test_update_suggestion_updates_translation_html(self) -> None:
        ...
        self.put_json(
            '%s/%s' %
            (feconf.UPDATE_TRANSLATION_SUGGESTION_URL_PREFIX, suggestion.suggestion_id),
            {
    ...
    def test_update_suggestion_updates_question_suggestion_content(self) -> None:
        ...
        suggestion_change: Dict[str, Union[
            str, question_domain.QuestionSuggestionChangeDict, float]] = {
                'cmd': question_domain.CMD_CREATE_NEW_FULLY_SPECIFIED_QUESTION,
                'question_dict': {
                    'question_state_data':
        ...
        self.post_json(
            '%s/%s' %
            (feconf.UPDATE_QUESTION_SUGGESTION_URL_PREFIX, suggestion.suggestion_id), {
    ...
    def test_cannot_update_question_with_invalid_skill_difficulty(self) -> None:
        ...
        self.assertIn(
            ...
            'not convert str to float', response['error']
        )
        self.logout()
...
class QuestionSuggestionTests(test_utils.GenericTestBase):
    ...
    def test_query_question_suggestions(self) -> None:
        suggestions = self.get_json(
            '%s?suggestion_type=%s' %
            (feconf.SUGGESTION_LIST_URL_PREFIX, feconf.SUGGESTION_TYPE_ADD_QUESTION)
        )['suggestions']
        self.assertEqual(len(suggestions), 1)
    ...
    def test_accept_question_suggestion(self) -> None:
        ...
        self.assertEqual(
            merged_question_skill_links[0].skill_descriptions, [self.SKILL_DESCRIPTION]
        )
    ...
    def test_accept_question_suggestion_with_image_region_interactions(self) -> None:
        ...
        fs_services.save_original_and_compressed_versions_of_image(
            'image.png', 'question_suggestions', skill_id, original_image_content,
            'image', True
        )
        ...
        self.put_json(
            '%s/skill/%s/%s' % (
                feconf.SUGGESTION_ACTION_URL_PREFIX, suggestion.target_id,
                suggestion.suggestion_id
            ), {
                'action': 'accept',
                'review_message': 'This looks good!',
...
class SkillSuggestionTests(test_utils.GenericTestBase):
    ...
    def test_suggestion_to_skill_handler_with_invalid_target_type(self) -> None:
        ...
        suggestion_services.create_suggestion(
            feconf.SUGGESTION_TYPE_EDIT_STATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            exp_id, 1, self.author_id, change_cmd, 'sample description'
        )
        ...
        response = self.put_json(
            '%s/skill/%s/%s' %
            (feconf.SUGGESTION_ACTION_URL_PREFIX, self.skill_id, suggestion_id), {
                'action': 'reject',
                'review_message': 'Rejected!'
            },
            csrf_token=csrf_token,
    ...
    def test_suggestion_to_skill_handler_with_invalid_target_id(self) -> None:
        ...
        response = self.put_json(
            '%s/skill/%s/%s' % (
                feconf.SUGGESTION_ACTION_URL_PREFIX, 'skill_id',
                suggestion_to_accept['suggestion_id']
            ), {
                'action': 'reject',
                'review_message': 'Rejected!'
        ...
        self.assertEqual(
            response['error'], 'The skill id provided does not match the skill id '
            'present as part of the suggestion_id'
        )
...
class UserSubmittedSuggestionsHandlerTest(test_utils.GenericTestBase):
    ...
    def test_get_translation_suggestions_returns_null_exploration_content_html_for_obsolete_suggestions( # pylint: disable=line-too-long
        ...
        exp_100 = self.save_new_linear_exp_with_state_names_and_interactions(
            '100',
            self.owner_id, ['Introduction', continue_state_name, 'End state'],
            ['TextInput', 'Continue'],
            category='Algebra'
        )
        self.publish_exploration(self.owner_id, exp_100.id)
        self.create_story_for_translation_opportunity(
            self.owner_id, self.admin_id, 'story_id_100', self.TOPIC_ID, exp_100.id
        )
        self.logout()
    ...
    def test_translation_suggestions_fetches_extra_page_if_filtered_result_is_empty( # pylint: disable=line-too-long
        ...
        suggestion_services.create_suggestion(
            feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            exp_id, 1, self.author_id, add_translation_change_dict, 'test description'
        )
...
class ReviewableSuggestionsHandlerTest(test_utils.GenericTestBase):
    ...
    def test_exploration_handler_returns_data_with_valid_exploration_id(self) -> None:
        ...
        self.assertEqual(
            suggestion['suggestion_type'], feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT
        )
