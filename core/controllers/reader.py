...
class AnswerSubmittedEventHandler(
    ...
    def post(self, exploration_id: str) -> None:
        ...
        old_interaction_instance = interaction_registry.Registry.get_interaction_by_id(
            old_interaction.id
        )
...
class StateHitEventHandler(
    ...
    def post(self, exploration_id: str) -> None:
        ...
        client_time_spent_in_secs = (
            self.normalized_payload[  # pylint: disable=unused-variable
                'client_time_spent_in_secs'
            ]
        )
...
class ReaderFeedbackHandler(
    ...
    HANDLER_ARGS_SCHEMAS = {
        'POST': {
            ...
            'include_author': {'schema': {'type': 'bool'}, 'default_value': True},
            'state_name': {
                'schema': {
                    'type': 'basestring',
                    'validators': [
    ...
    def get(self, exploration_id: str) -> None:
        ...
        if collection_id:
            ...
            system_chosen_exp_ids = (
                recommendations_services.get_exploration_recommendations(exploration_id)
            )
            filtered_exp_ids = list(
                set(system_chosen_exp_ids) - set(author_recommended_exp_ids)
            )
            system_recommended_exp_ids = random.sample(
                filtered_exp_ids, min(MAX_SYSTEM_RECOMMENDATIONS, len(filtered_exp_ids))
            )
...
class SaveTransientCheckpointProgressHandler(
    base.BaseHandler[
        SaveTransientCheckpointProgressHandlerNormalizedPayloadDict, Dict[str, str]
    ]
):
...
class LearnerAnswerDetailsSubmissionHandler(
    base.BaseHandler[
        LearnerAnswerDetailsSubmissionHandlerNormalizedPayloadDict, Dict[str, str]
    ]
):
    ...
    HANDLER_ARGS_SCHEMAS = {
        'PUT': {
            'state_name': {'schema': {'type': 'basestring'}, 'default_value': None},
