...
class VoiceArtistManagementTests(test_utils.GenericTestBase):
    ...
    def test_voiceover_admin_can_manage_voice_artist_in_private_exp(self):
        # Technically, voiceover admins can manage voice artists in any
        # exploration, including private ones. Note, however, that this
        # does not mean that they can actually assign voice artists to
        # private explorations. The logic of assigning voice artists to
        # different types of explorations is handled in rights_manager.py.
        self.login(self.VOICEOVER_ADMIN_EMAIL)
        csrf_token = self.get_new_csrf_token()
        with self.swap(self, 'testapp', self.mock_testapp):
            self.post_json(
