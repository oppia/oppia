...
class StoryPageDataHandlerTests(BaseStoryViewerControllerTests):
    ...
    def test_can_not_access_story_viewer_page_with_unpublished_story(self) -> None:
        ...
        story = story_domain.Story.create_default_story(
            new_story_id, 'Title', 'Description', self.TOPIC_ID, new_story_url_fragment
        )
        story_services.save_new_story(self.admin_id, story)
        self.get_json(
    ...
    def test_can_not_access_story_viewer_page_with_unpublished_topic(self) -> None:
        ...
        self.save_new_topic(
            ...
            url_fragment='topics',
            description='A new topic',
            canonical_story_ids=[new_story_id],
            additional_story_ids=[],
            uncategorized_skill_ids=[],
            subtopics=[],
            next_subtopic_id=0
        ...
        story = story_domain.Story.create_default_story(
            new_story_id, 'Title', 'Description', 'topic_id_1', new_story_url_fragment
        )
        story_services.save_new_story(self.admin_id, story)
...
class StoryProgressHandlerTests(BaseStoryViewerControllerTests):
    ...
    def test_cannot_access_story_progress_handler_if_user_is_not_logged_in(
        ...
        response = self.get_json(
            '%s/staging/topic/%s/%s' %
            (feconf.STORY_PROGRESS_URL_PREFIX, self.STORY_URL_FRAGMENT, self.NODE_ID_3),
    ...
    def test_redirect_for_returning_user_with_completed_nodes(self) -> None:
        ...
        response = self.get_html_response(
            '%s/staging/topic/%s/%s' %
            (feconf.STORY_PROGRESS_URL_PREFIX, self.STORY_URL_FRAGMENT, self.NODE_ID_1),
            expected_status_int=302
        )
        self.assertEqual(
            'http://localhost/learn/staging/topic/story/title-one',
    ...
    def test_redirect_to_next_node(self) -> None:
        ...
        response = self.get_html_response(
            '%s/staging/topic/%s/%s' %
            (feconf.STORY_PROGRESS_URL_PREFIX, self.STORY_URL_FRAGMENT, self.NODE_ID_2),
            expected_status_int=302
        )
        self.assertEqual(
            'http://localhost/explore/1?classroom_url_fragment=staging'
            '&topic_url_fragment=topic&story_url_fragment=title-one'
            '&node_id=node_1', response.headers['location']
        )
    ...
    def test_post_fails_when_story_does_not_exist(self) -> None:
        ...
        self.post_json(
            '%s/staging/topic/%s/%s' %
            (feconf.STORY_PROGRESS_URL_PREFIX, 'invalid-story', self.NODE_ID_2), {},
            csrf_token=csrf_token,
            expected_status_int=404
        )
    ...
    def test_post_fails_when_node_does_not_exist(self) -> None:
        ...
        self.post_json(
            '%s/staging/topic/%s/%s' %
            (feconf.STORY_PROGRESS_URL_PREFIX, self.STORY_URL_FRAGMENT, 'node_0000'),
            {},
            csrf_token=csrf_token,
            expected_status_int=404
        )
    ...
    def test_post_fails_when_node_id_schema_is_invalid(self) -> None:
        ...
        self.post_json(
            '%s/staging/topic/%s/%s' %
            (feconf.STORY_PROGRESS_URL_PREFIX, self.STORY_URL_FRAGMENT, 'invalid_node'),
            {},
            csrf_token=csrf_token,
    ...
    def test_mark_story_and_topic_as_completed_and_learnt(self) -> None:
        ...
        self.assertEqual(
            len(learner_goals_services.get_all_topic_ids_to_learn(self.viewer_id)), 1
        )
        story_services.record_completed_node_in_story_context(
            self.viewer_id, self.STORY_ID, self.NODE_ID_2
        )
        ...
        self.assertEqual(
            len(learner_progress_services.get_all_learnt_topic_ids(self.viewer_id)), 1
        )
        self.assertEqual(
            len(learner_goals_services.get_all_topic_ids_to_learn(self.viewer_id)), 0
        )
        self.assertEqual(
