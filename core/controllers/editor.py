...
class ExplorationHandler(
    ...
    def put(self, exploration_id: str) -> None:
        ...
        if version > exploration.version:
            raise base.BaseHandler.InvalidInputException(
                'Trying to update version %s of exploration from version %s, which is not possible. Please reload the page and try again.'
...
class ResolveIssueHandler(
    ...
    def post(self, exp_id: str) -> None:
        ...
        if not issue_to_remove:
            raise self.NotFoundException(
                'Exploration issue does not exist in the list of issues for the exploration with ID %s'
                % exp_id
            )
