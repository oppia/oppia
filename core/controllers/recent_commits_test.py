...
class RecentCommitsHandlerUnitTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        commit1 = exp_models.ExplorationCommitLogEntryModel.create(
            'entity_1', 0, self.committer_1_id, 'create', 'created first commit', [],
            'public', True
        )
        commit2 = exp_models.ExplorationCommitLogEntryModel.create(
            'entity_1', 1, self.committer_2_id, 'edit', 'edited commit', [], 'public',
            True
        )
        commit3 = exp_models.ExplorationCommitLogEntryModel.create(
            'entity_2', 0, self.committer_1_id, 'create', 'created second commit', [],
            'private', False
        )
    ...
    def test_get_recent_commits_explorations(self) -> None:
        ...
        self.assertEqual(
            response_dict['exp_ids_to_exp_data']['exp_1']['title'], 'MyExploration'
        )
        self.logout()
