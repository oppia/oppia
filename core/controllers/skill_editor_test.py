...
class BaseSkillEditorControllerTests(test_utils.GenericTestBase):
    ...
    def _mock_update_skill_raise_exception(
        self, unused_committer_id: str, unused_skill_id: str,
        unused_change_list: List[skill_domain.SkillChange], unused_commit_message: str
    ) -> None:
...
class EditableSkillDataHandlerTest(BaseSkillEditorControllerTests):
    ...
    def test_new_user_can_not_delete_skill(self) -> None:
        ...
        response = self.delete_json(self.url, expected_status_int=401)
        self.assertEqual(
            response['error'], 'You do not have credentials to delete the skill.'
        )
    ...
    def test_skill_which_is_assigned_to_multiple_topics(self) -> None:
        ...
        self.assertEqual(
            json_response['assigned_skill_topic_data_dict']['Maths'], 'Addition'
        )
        self.assertEqual(
            json_response['assigned_skill_topic_data_dict']['Science'], 'Chemistry'
        )
        self.assertEqual(1, len(json_response['grouped_skill_summaries']['Name']))
        self.logout()
    ...
    def test_editable_skill_handler_fails_long_commit_message(self) -> None:
        ...
        json_response = self.put_json(
            self.url, put_payload_copy, csrf_token=csrf_token, expected_status_int=400
        )
        self.assertEqual(
            json_response['error'],
    ...
    def test_editable_skill_handler_put_fails(self) -> None:
        ...
        update_skill_swap = self.swap(
            skill_services, 'update_skill', self._mock_update_skill_raise_exception
        )
        ...
        self.put_json(
            self.url, self.put_payload, csrf_token=csrf_token, expected_status_int=400
        )
    ...
    def test_editable_skill_handler_delete_succeeds(self) -> None:
        ...
        skill_has_topics_swap = self.swap(
            topic_fetchers, 'get_all_skill_ids_assigned_to_some_topic', lambda: []
        )
