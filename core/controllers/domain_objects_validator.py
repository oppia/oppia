...
def validate_platform_params_values_for_blog_admin(
    ...
    for name, value in new_platform_parameter_values.items():
        if not isinstance(name, str):
            raise Exception(
                'Platform parameter name should be a string, received: %s'
                % name
            )

        if not isinstance(value, (bool, float, int, str)):
            raise Exception(
                'The value of %s platform parameter is not of valid type, it should be one of %s.'
        ...
        if not (
            ...
            raise Exception(
                'The value of platform parameter %s is of type \'%s\', expected it to be of type \'%s\''
