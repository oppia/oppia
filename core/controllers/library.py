...
def get_matching_activity_dicts(query_string, search_cursor):
    ...
    activity_list = []
    activity_list = (
        summary_tile_services.get_displayable_collection_summary_dicts_matching_ids( #pylint: disable=line-too-long
            collection_ids))
    activity_list += (
...
class LibraryIndexHandler(base.BaseHandler):
    ...
    def get(self):
        ...
        summary_dicts_by_category = summary_tile_services.get_library_groups([
            feconf.DEFAULT_LANGUAGE_CODE])
        recently_published_summary_dicts = (
            summary_tile_services.get_recently_published_exploration_summary_dicts( #pylint: disable=line-too-long
                feconf.RECENTLY_PUBLISHED_QUERY_LIMIT_FOR_LIBRARY_PAGE))
        top_rated_activity_summary_dicts = (
            summary_tile_services.get_top_rated_exploration_summary_dicts(
                [feconf.DEFAULT_LANGUAGE_CODE],
                feconf.NUMBER_OF_TOP_RATED_EXPLORATIONS_FOR_LIBRARY_PAGE))
        featured_activity_summary_dicts = (
            summary_tile_services.get_featured_activity_summary_dicts(
                [feconf.DEFAULT_LANGUAGE_CODE]))
