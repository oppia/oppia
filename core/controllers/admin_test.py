...
(
    audit_models, blog_models, exp_models, opportunity_models, user_models,
    voiceover_models
) = models.Registry.import_models([
    models.Names.AUDIT, models.Names.BLOG, models.Names.EXPLORATION,
    models.Names.OPPORTUNITY, models.Names.USER, models.Names.VOICEOVER
])
...
class AdminIntegrationTest(test_utils.GenericTestBase):
    ...
    def test_without_blog_post_title_generate_dummy_blog_post_is_not_performed(
        ...
        assert_raises_regexp_context_manager = self.assertRaisesRegex(
            Exception, 'The \'blog_post_title\' must be provided when the action '
    ...
    def test_without_num_dummy_exps_generate_dummy_exp_action_is_not_performed(
        ...
        assert_raises_regexp_context_manager = self.assertRaisesRegex(
            Exception, 'The \'num_dummy_exps_to_generate\' must be provided when the '
    ...
    def test_without_num_dummy_exps_to_publish_action_is_not_performed(self) -> None:
        ...
        assert_raises_regexp_context_manager = self.assertRaisesRegex(
            Exception, 'The \'num_dummy_exps_to_publish\' must be provided when the '
            'action is generate_dummy_explorations.'
        )
        with assert_raises_regexp_context_manager, self.prod_mode_swap:
            self.post_json(
    ...
    def test_without_skill_id_dummy_question_suggestions_action_is_not_performed( # pylint: disable=line-too-long
        ...
        assert_raises_regexp_context_manager = self.assertRaisesRegex(
            Exception, 'The \'skill_id\' must be provided when the '
    ...
    def test_without_commit_message_action_update_param_is_not_performed(self) -> None:
        ...
        assert_raises_regexp_context_manager = self.assertRaisesRegex(
            Exception, 'The \'commit_message\' must be provided when the action is '
            'update_platform_parameter_rules.'
        )
        with assert_raises_regexp_context_manager, self.prod_mode_swap:
            self.post_json(
    ...
    def test_load_new_structures_data(self) -> None:
        ...
        story_id = topic_fetchers.get_topic_by_id(
            topic_summaries[0].id
        ).canonical_story_references[0].story_id
        ...
        questions, _ = (
            question_fetchers.get_questions_and_skill_descriptions_by_skill_ids(
                10,
                [skill_summaries[0].id, skill_summaries[1].id, skill_summaries[2].id], 0
            )
        )
    ...
    def test_admin_topics_csv_download_handler(self) -> None:
        ...
        self.assertIn(
            ...
            b'Statistics,Welcome', response.body
        )
    ...
    def test_get_handler_includes_all_platform_params(self) -> None:
        ...
        with self.swap(
            platform_parameter_list, 'ALL_PLATFORM_PARAMS_LIST',
    ...
    def test_post_with_rules_changes_updates_platform_params(self) -> None:
        ...
        rule_dicts = [
            rule.to_dict() for rule in platform_parameter_registry.Registry.
            get_platform_parameter(param.name).rules
        ]
...
class AdminRoleHandlerTest(test_utils.GenericTestBase):
    ...
    def test_if_filter_criterion_is_username_and_username_is_not_provided(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception, 'The username must be provided when the filter criterion '
            'is \'username\'.'
        ):
            self.get_json(
                feconf.ADMIN_ROLE_HANDLER_URL, params={
                    'filter_criterion': 'username'
                }
            )
...
class TranslationCoordinatorRoleHandlerTest(test_utils.GenericTestBase):
    ...
    def test_adding_translation_coordinator_role_to_language(self) -> None:
        ...
        self.assertEqual(
            response_dict, {
                'roles':
                    [feconf.ROLE_ID_FULL_USER, feconf.ROLE_ID_TRANSLATION_COORDINATOR],
                'banned': False,
                'managed_topic_ids': [],
...
class BannedUsersHandlerTest(test_utils.GenericTestBase):
    ...
    def test_unmark_a_banned_user(self) -> None:
        ...
        self.delete_json('/bannedusershandler', params={
            'username': username
        })
...
class DataExtractionQueryHandlerTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        stats_services.record_answer(
            ...
            stats_domain.SubmittedAnswer(
                'first answer', 'TextInput', 0, 0, exp_domain.EXPLICIT_CLASSIFICATION,
        ...
        stats_services.record_answer(
            ...
            'TextInput',
            stats_domain.SubmittedAnswer(
                'second answer', 'TextInput', 0, 0, exp_domain.EXPLICIT_CLASSIFICATION,
                {}, 'a_session_id_val', 1.0
            )
        )
    ...
    def test_raises_error_if_no_state_answer_exists_while_data_extraction(self) -> None:
        ...
        self.assertEqual(
            response['error'], 'No state answer exists for the given exp_id: exp, '
...
class ClearSearchIndexTest(test_utils.GenericTestBase):
    ...
    def test_clear_search_index(self) -> None:
        ...
        result_explorations = search_services.search_explorations('Welcome', [], [],
                                                                  2)[0]
...
class UpdateUsernameHandlerTest(test_utils.GenericTestBase):
    ...
    def test_update_username_with_non_string_new_username(self) -> None:
        ...
        self.assertEqual(
            response['error'], 'At \'http://localhost/updateusernamehandler\' '
            'these errors are happening:\n'
...
class VerifyUserModelsDeletedHandlerTest(test_utils.GenericTestBase):
    ...
    def test_get_with_nonexistent_user_id_returns_true(self) -> None:
        response = self.get_json(
            '/verifyusermodelsdeletedhandler', params={
                'user_id': 'aaa'
            }
        )
