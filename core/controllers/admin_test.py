...
class AdminIntegrationTest(test_utils.GenericTestBase):
    ...
    def test_dummy_certificate_assessment_is_seeded(self) -> None:
        self.set_curriculum_admins([self.CURRICULUM_ADMIN_USERNAME])
        self.login(self.CURRICULUM_ADMIN_EMAIL, is_super_admin=True)
        csrf_token = self.get_new_csrf_token()
        # The dummy certificate offering references the topic generated by the
        # dummy new structures data, whose linked questions cover every
        # difficulty bucket required by the certificate validator.
        self.post_json(
            '/adminhandler',
            {'action': 'generate_dummy_new_structures_data'},
            csrf_token=csrf_token,
        ...
        self.assertEqual(len(certificate_offerings), 1)
        self.assertEqual(certificate_offerings[0].async_status, 'Available')
        math_classroom = (
            classroom_config_services.get_classroom_by_url_fragment('math')
        )
        assert math_classroom is not None
        self.assertEqual(
            certificate_offerings[0].classroom_id, math_classroom.classroom_id
        )
        ...
        self.assertEqual(
            attempts[0].version_data['certificate_id'],
            certificate_offerings[0].certificate_id,
        )
        self.logout()
