...
class BlogServicesUnitTests(test_utils.GenericTestBase):
    ...
    def test_get_blog_post_summary_from_model(self) -> None:
        ...
        blog_post_summary = (
            blog_services.get_blog_post_summary_from_model(blog_post_summary_model)
        )
        expected_blog_post_summary = (
            blog_domain.BlogPostSummary(
                self.blog_post_a_id, self.user_id_a, '', '', '', [],
    ...
    def test_generate_url_fragment(self) -> None:
        url_fragment = (
            blog_services.generate_url_fragment('Sample Url Fragment', 'ABC123EFG')
        )
        self.assertEqual(url_fragment, 'sample-url-fragment-abc123efg')

        url_fragment = (
            blog_services.generate_url_fragment('SaMple Url FrAgMent', 'ABC123Efgh')
        )
    ...
    def test_get_blog_post_summary_by_title(self) -> None:
        ...
        expected_blog_post_summary = (
            blog_domain.BlogPostSummary(
                self.blog_post_a_id, self.user_id_a, 'Hello Bloggers', '', '', [],
                blog_post_summary.thumbnail_filename, blog_post_summary.last_updated,
                blog_post_summary.published_on
            )
        )
        self.assertEqual(
