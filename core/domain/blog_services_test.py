...
class BlogServicesUnitTests(test_utils.GenericTestBase):
    ...
    def test_get_published_blog_post_summaries(self) -> None:
        ...
        number_of_published_blogs = blog_services.get_published_blog_post_summaries(
            0, 2
        )
    ...
    def test_get_blog_posy_by_invalid_url(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception, 'Blog Post URL fragment should be a string. Recieved:' r'\[123\]'
        ):
    ...
    def test_check_can_edit_blog_post(self) -> None:
        ...
        self.assertFalse(
            blog_services.check_can_edit_blog_post(user_info_b, blog_post_rights)
        )
        self.assertTrue(
            blog_services.check_can_edit_blog_post(user_info_a, blog_post_rights)
        )
    ...
    def test_delete_blog_post(self) -> None:
        blog_services.delete_blog_post(self.blog_post_a_id)
        self.assertIsNone(
            blog_services.get_blog_post_rights(self.blog_post_a_id, strict=False)
        )
        self.assertIsNone(
            blog_services.get_blog_post_by_id(self.blog_post_a_id, strict=False)
        )
        self.assertIsNone(
            blog_services.get_blog_post_summary_by_id(self.blog_post_a_id, strict=False)
        )
    ...
    def test_updated_blog_post_is_added_correctly_to_index(self) -> None:
        ...
        with add_docs_swap:
            ...
            if old_blog_post_summary.published_on:
                rank = math.floor(
                    utils.get_time_in_millisecs(old_blog_post_summary.published_on)
                )
            else:
                rank = 0
            ...
            if new_blog_post_summary.published_on:
                rank = math.floor(
                    utils.get_time_in_millisecs(new_blog_post_summary.published_on)
                )
            else:
                rank = 0
...
class BlogAuthorDetailsTests(test_utils.GenericTestBase):
    ...
    def test_update_blog_author_details_with_invalid_author_name(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception, ('Author name can only have alphanumeric characters and spaces.')
        ):
            blog_services.update_blog_author_details(
                self.user_id, new_author_name, new_author_bio
            )
...
class BlogPostSummaryQueriesUnitTests(test_utils.GenericTestBase):
    ...
    def test_search_blog_post_summaries(self) -> None:
        ...
        blog_post_ids, _ = blog_services.get_blog_post_ids_matching_query(
            'Oppia', [], feconf.MAX_NUM_CARDS_TO_DISPLAY_ON_BLOG_SEARCH_RESULTS_PAGE
        )
        self.assertEqual(
        ...
        blog_post_ids, _ = blog_services.get_blog_post_ids_matching_query(
            'Basic', [], feconf.MAX_NUM_CARDS_TO_DISPLAY_ON_BLOG_SEARCH_RESULTS_PAGE
        )
        self.assertEqual(
            sorted(blog_post_ids),
            sorted(
        ...
        blog_post_ids, _ = blog_services.get_blog_post_ids_matching_query(
            'Lessons', [], feconf.MAX_NUM_CARDS_TO_DISPLAY_ON_BLOG_SEARCH_RESULTS_PAGE
        )
        self.assertEqual(
            sorted(blog_post_ids),
            sorted(
    ...
    def test_blog_post_summaries_pagination_in_filled_search_results(self) -> None:
        ...
        with self.swap(
            ...
            self.assertEqual(
                sorted(found_blog_post_ids), sorted(self.all_blog_post_ids[:6])
            )
    ...
    def test_get_blog_post_ids_matching_query_with_stale_blog_post_ids(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                ...
                '%s, %s' % (self.all_blog_post_ids[0], self.all_blog_post_ids[1]),
