...
class BlogServicesUnitTests(test_utils.GenericTestBase):
    ...
    def test_get_published_blog_post_summaries(self) -> None:
        ...
        no_of_blog_posts = blog_services.get_published_blog_post_summaries()
        # Ruling out the possibility of None for mypy type checking.
        assert no_of_blog_posts is not None
        self.assertEqual(
            len(no_of_blog_posts),
            1
        )
    ...
    def test_get_published_blog_post_summaries_by_user_id(self) -> None:
        ...
        # Ruling out the possibility of None for mypy type checking.
        assert no_of_published_blog is not None
        self.assertEqual(
            len(no_of_published_blog), 1
        )
    ...
    def test_get_blog_post_by_invalid_url_string(self) -> None:
        with self.assertRaisesRegex(  # type: ignore[no-untyped-call]
            Exception,
            'No blog post exist for the given url fragment' +
            ' just-a-non-existing-url.'
        ):
            blog_services.get_blog_post_by_url_fragment(
                'just-a-non-existing-url'
            )
