...
class VerifyProfilePictureOneOffJobTests(test_utils.GenericTestBase):
    ...
    def _run_one_off_job(self):
        """Runs the one-off MapReduce job."""
        job_id = user_jobs_one_off.VerifyProfilePictureOneOffJob.create_new()
        user_jobs_one_off.VerifyProfilePictureOneOffJob.enqueue(job_id)
        self.assertEqual(
            self.count_jobs_in_mapreduce_taskqueue(
                taskqueue_services.QUEUE_NAME_ONE_OFF_JOBS), 1)
        self.process_and_flush_pending_mapreduce_tasks()
        stringified_output = (
            user_jobs_one_off.VerifyProfilePictureOneOffJob.get_output(job_id))
        eval_output = [ast.literal_eval(stringified_item) for
                       stringified_item in stringified_output]
        return eval_output
    ...
    def setUp(self):
        ...
        with self.swap(test_utils.TestBase, 'signup_superadmin_user', empty):
            super(VerifyProfilePictureOneOffJobTests, self).setUp()
        self.signup(self.OWNER_EMAIL, self.OWNER_USERNAME)
        self.owner_id = self.get_user_id_from_email(self.OWNER_EMAIL)
        user_services.generate_initial_profile_picture(self.owner_id)

    def test_correct_profile_picture(self):
        output = self._run_one_off_job()
