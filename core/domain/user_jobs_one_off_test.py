...
class PopulateStoriesAndTopicsInIncompleteActivitiesOneOffJobTests(
    ...
    def setUp(self):
        super(
            PopulateStoriesAndTopicsInIncompleteActivitiesOneOffJobTests,
            self).setUp()
...
class PopulateStoriesAndTopicsInCompletedActivitiesOneOffJobTests(
        test_utils.GenericTestBase):
    """Tests for the one-off populate story_ids and learnt_topics_ids
    in CompletedActivitiesModel job."""
    ...
    def _run_one_off_job(self):
        """Runs the one-off MapReduce job."""
        job_id = user_jobs_one_off.PopulateStoriesAndTopicsInCompletedActivitiesOneOffJob.create_new() # pylint: disable=line-too-long
        user_jobs_one_off.PopulateStoriesAndTopicsInCompletedActivitiesOneOffJob.enqueue(job_id) # pylint: disable=line-too-long
        self.assertEqual(
            self.count_jobs_in_mapreduce_taskqueue(
                taskqueue_services.QUEUE_NAME_ONE_OFF_JOBS), 1)
        self.process_and_flush_pending_mapreduce_tasks()
    ...
    def setUp(self):
        super(
            PopulateStoriesAndTopicsInCompletedActivitiesOneOffJobTests,
            self).setUp()
    ...
    def test_does_not_add_story_and_topic_for_one_incomplete_exp(self):
        # Mark an exploration as incomplete.
        state_name = 'state_name'
        version = 1
        learner_progress_services.mark_exploration_as_incomplete(
    ...
    def test_does_not_add_story_and_topic_for_multiple_incomplete_exp(self):
        # Mark 2 exploration in different stories as incomplete.
        state_name = 'state_name'
        version = 1
        learner_progress_services.mark_exploration_as_incomplete(
    ...
    def test_adds_story_and_topic_for_one_completed_and_one_incomplete_exp(
            self):
        # Mark an exploration as completed and other as incomplete in different
        # story.
        state_name = 'state_name'
        version = 1
        story_services.record_completed_node_in_story_context(
