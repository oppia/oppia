...
class StoryServicesUnitTests(test_utils.GenericTestBase):
    ...
    def test_commit_log_entry(self) -> None:
        story_commit_log_entry = story_models.StoryCommitLogEntryModel.get_commit(
            self.STORY_ID, 1
        )
    ...
    def test_update_story_node_properties(self) -> None:
        ...
        changelist = [
            story_domain.StoryChange(
                {'cmd': story_domain.CMD_DELETE_STORY_NODE, 'node_id': self.NODE_ID_1}
            ),
            story_domain.StoryChange(
                {
                    'cmd': story_domain.CMD_UPDATE_STORY_NODE_OUTLINE_STATUS,
    ...
    def test_exploration_context_model_is_modified_correctly(self) -> None:
        ...
        change_list = [
            ...
            story_domain.StoryChange(
                {'cmd': story_domain.CMD_DELETE_STORY_NODE, 'node_id': self.NODE_ID_2}
            ),
            story_domain.StoryChange(
                {
                    'cmd': story_domain.CMD_ADD_STORY_NODE,
    ...
    def test_cannot_update_story_with_exps_with_different_categories(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception, 'All explorations in a story should be of the ' 'same category'
        ):
            story_services.update_story(
                self.USER_ID, self.STORY_ID, change_list, 'Updated story node.'
            )
    ...
    def test_cannot_delete_node_with_invalid_node_id(self) -> None:
        change_list = [
            story_domain.StoryChange(
                {'cmd': story_domain.CMD_DELETE_STORY_NODE, 'node_id': 'invalid_node'}
            )
        ]
    ...
    def test_get_chapter_notifications_list(self) -> None:
        ...
        with self.swap(
            ...
            story_publcation_timeliness = story_domain.StoryPublicationTimeliness(
                canonical_story_id_1, 'title', 'Topic', ['Chapter 2'], ['Chapter 3']
            )
            self.assertEqual(
