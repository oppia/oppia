...
(story_models,
 user_models) = models.Registry.import_models([models.Names.STORY, models.Names.USER])
...
class StoryServicesUnitTests(test_utils.GenericTestBase):
    ...
    def test_update_story_properties(self) -> None:
        ...
        story_services.update_story(
            self.USER_ID, self.STORY_ID, changelist, 'Updated Title and Description.'
        )
        ...
        self.assertEqual(
            story.thumbnail_bg_color, constants.ALLOWED_THUMBNAIL_BG_COLORS['story'][0]
        )
    ...
    def test_update_story_node_properties(self) -> None:
        ...
        self.assertEqual(
            story.story_contents.nodes[1].thumbnail_size_in_bytes, len(raw_image)
        )
        self.assertEqual(
        ...
        self.assertEqual(story.version, 3)
        self.assertEqual(
            story.story_contents.nodes[1].status, constants.STORY_NODE_STATUS_PUBLISHED
        )
        self.assertEqual(
            story.story_contents.nodes[1].planned_publication_date_msecs,
            (datetime.datetime(2023, 1, 2, 0, 0).timestamp() * 1000),
            msg='Incorrect planned publication date in milliseconds received.'
        )
        self.assertEqual(
            story.story_contents.nodes[1].first_publication_date_msecs,
            (datetime.datetime(2023, 1, 1, 0, 0).timestamp() * 1000),
            msg='Incorrect first publication date in milliseconds received.'
        )
        self.assertEqual(
            story.story_contents.nodes[1].last_modified_msecs,
            (datetime.datetime(2023, 1, 1, 0, 0).timestamp() * 1000),
    ...
    def test_delete_story(self) -> None:
        ...
        self.assertEqual(
            story_fetchers.get_story_summary_by_id(self.STORY_ID, strict=False), None
        )
    ...
    def test_exploration_story_link_collision(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception, 'The exploration with ID 0 is already linked to story '
    ...
    def test_update_story_language_code(self) -> None:
        ...
        story_services.update_story(
            self.USER_ID, self.STORY_ID, change_list, 'Updated story language_code.'
        )
    ...
    def test_update_story_url_fragment(self) -> None:
        ...
        story_services.update_story(
            self.USER_ID, self.STORY_ID, change_list, 'Updated story url_fragment.'
        )
    ...
    def test_cannot_update_story_if_url_fragment_already_exists(self) -> None:
        ...
        story_id = story_services.get_new_story_id()
        self.save_new_story(
            story_id, self.USER_ID, topic_id, title='original', url_fragment='original'
        )
        change_list = [
            story_domain.StoryChange({
                'cmd': story_domain.CMD_UPDATE_STORY_PROPERTY,
    ...
    def test_validate_exploration_throws_an_exception(self) -> None:
        ...
        validate_fn_swap = self.swap(
            exp_services, 'validate_exploration_for_story', _mock_validate_function
        )
    ...
    def test_validate_exploration_returning_error_messages(self) -> None:
        ...
        validation_error_messages = (
            story_services.validate_explorations_for_story(['invalid_exp', 'exp_id_1'],
                                                           False)
        )
        message_1 = (
    ...
    def test_cannot_update_story_with_exps_with_different_categories(self) -> None:
        ...
        validation_error_messages = (
            story_services.validate_explorations_for_story(['exp_id_2', 'exp_id_1'],
                                                           False)
        )
        ...
        exp_services.update_exploration(
            ...
            [
                exp_domain.ExplorationChange({
                    ...
                    'property_name': 'param_specs',
                    'new_value': {
                        'param1': param_domain.ParamSpec('UnicodeString').to_dict()
                    }
                }),
                exp_domain.ExplorationChange({
                    'cmd':
    ...
    def test_cannot_update_story_with_mismatch_of_story_versions(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception, 'Unexpected error: trying to update version 1 of story '
        ...
        with self.assertRaisesRegex(
            Exception, 'Trying to update version 11 of story from version 2, '
    ...
    def test_cannot_update_initial_node_with_invalid_node_id(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception, 'The node with id new_initial_node_id is not part of this story'
        ):
            story_services.update_story(
                self.USER_ID, self.STORY_ID, change_list,
    ...
    def test_cannot_update_node_description_with_invalid_node_id(self) -> None:
        ...
        with self.assertRaisesRegex(
            ...
            story_services.update_story(
                self.USER_ID, self.STORY_ID, change_list, 'Update node description.'
            )
    ...
    def test_cannot_delete_starting_node_of_story(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception, 'The node with id %s is the starting node for the story, '
...
class StoryProgressUnitTests(test_utils.GenericTestBase):
    ...
    def _get_progress_model(self, user_id: str,
                            STORY_ID: str) -> Optional[user_models.StoryProgressModel]:
    ...
    def test_get_completed_nodes_in_story(self) -> None:
        ...
        for ind, completed_node in enumerate(
            story_fetchers.get_completed_nodes_in_story(self.owner_id, self.STORY_1_ID)
        ):
