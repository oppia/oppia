...
class StoryServicesUnitTests(test_utils.GenericTestBase):
    ...
    def test_update_story_which_not_corresponding_topic_id(self):
        topic_id = topic_services.get_new_topic_id()
        self.save_new_topic(
            topic_id, self.USER_ID, 'A New Topic', '', None,
    ...
    def test_get_story_by_version(self):
        ...
        story_id = story_services.get_new_story_id()
        self.save_new_topic(
            topic_id, self.USER_ID, 'A different topic', '', None,
            'A new topic', [], [], [], [], 0)
        self.save_new_story(
...
class StoryProgressUnitTests(StoryServicesUnitTests):
    ...
    def setUp(self):
        ...
        self.TOPIC_ID = topic_services.get_new_topic_id()
        self.save_new_topic(
            self.TOPIC_ID, self.USER_ID, 'New Topic', '', None,
            'A new topic', [], [], [], [], 0)
