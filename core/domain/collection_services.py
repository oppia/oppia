...
from typing import (
    Dict, Final, Iterable, List, Literal, Mapping, Optional, Sequence, Tuple, TypedDict,
    cast, overload
)
...
def get_collection_summary_from_model(
    ...
    return collection_domain.CollectionSummary(
        ...
        collection_summary_model.status, collection_summary_model.community_owned,
        collection_summary_model.owner_ids, collection_summary_model.editor_ids,
        collection_summary_model.viewer_ids, collection_summary_model.contributor_ids,
        collection_summary_model.contributors_summary, collection_summary_model.version,
...
def get_collection_and_collection_rights_by_id(
    collection_id: str
) -> Tuple[Optional[collection_domain.Collection],
...
def get_next_exploration_id_to_complete_by_user(user_id: str,
...
def get_collection_summaries_subscribed_to(
    ...
    """
    return [
        summary for summary in get_collection_summaries_matching_ids(
...
def _create_collection(
    committer_id: str, collection: collection_domain.Collection, commit_message: str,
...
def update_collection(
    committer_id: str, collection_id: str,
    change_list: Sequence[Mapping[str, change_domain.AcceptableChangeDictTypes]],
    commit_message: Optional[str]
) -> None:
...
def _compute_summary_of_collection(
    ...
    collection_summary = collection_domain.CollectionSummary(
        collection.id, collection.title, collection.category, collection.objective,
        collection.language_code, collection.tags, collection_rights.status,
        collection_rights.community_owned, collection_rights.owner_ids,
        collection_rights.editor_ids, collection_rights.viewer_ids, contributor_ids,
...
def save_collection_summary(
    ...
    collection_summary_model = (
        collection_models.CollectionSummaryModel.get_by_id(collection_summary.id)
    )
