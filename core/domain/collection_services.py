...
def _migrate_collection_contents_to_latest_schema(
    ...
    if not (
        ...
        raise Exception(
            'Sorry, we can only process v1-v%d collection schemas at present.'
            % feconf.CURRENT_COLLECTION_SCHEMA_VERSION
        )
...
def get_collection_from_model(
    ...
    versioned_collection_contents: collection_domain.VersionedCollectionDict = {
        ...
        'collection_contents': copy.deepcopy(
...
def get_multiple_collections_by_id(
    ...
    for index, cid in enumerate(uncached):
        ...
        if model:
            ...
            logging.info(
                'Tried to fetch collection with id %s, but no such collection exists in the datastore'
                % cid
            )
    ...
    for exploration_id in collection.exploration_ids:
        if rights_manager.is_exploration_private(exploration_id):
            raise utils.ValidationError(
                'Cannot reference a private exploration within a public collection, exploration ID: %s'
                % exploration_id
            )
...
def _save_collection(
    ...
    if not change_list:
        raise Exception(
            'Unexpected error: received an invalid change list when trying to save collection %s: %s'
            % (collection.id, change_list)
        )
    ...
    for collection_node in collection.nodes:
        if not exp_summaries_dict[collection_node.exploration_id]:
            raise utils.ValidationError(
                'Expected collection to only reference valid explorations, but found an exploration with ID: %s (was it deleted?)'
    ...
    if collection.version > collection_model.version:
        raise Exception(
            'Unexpected error: trying to update version %s of collection from version %s. Please reload the page and try again.'
            % (collection_model.version, collection.version)
        )

    if collection.version < collection_model.version:
        raise Exception(
            'Trying to update version %s of collection from version %s, which is too old. Please reload the page and try again.'
            % (collection_model.version, collection.version)
        )
...
def delete_collection_summaries(collection_ids: List[str]) -> None:
    ...
    """
    summary_models = collection_models.CollectionSummaryModel.get_multi(
        collection_ids
    )
...
def delete_demo(collection_id: str) -> None:
    ...
    if not collection:
        logging.info(
            'Collection with id %s was not deleted, because it does not exist.'
            % collection_id
        )
    else:
        delete_collection(
