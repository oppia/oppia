...
class StateDomainUnitTests(test_utils.GenericTestBase):
            ...
            state_domain.SubtitledHtml.from_dict(
                {
                    'content_id': 'content_0',
                    'html': (
                        '<p><oppia-noninteractive-skillreview text-with-value="&amp;quot;&amp;quot;" skill_id-with-value="&amp;quot;&amp;quot;"></oppia-noninteractive-skillreview></p>'
                    ),
                }
            )
        )
    ...
    def test_update_answer_groups_invalid_translatable_input(self) -> None:
        ...
        rg = state_domain.AnswerGroup(
            rule_specs=[
                state_domain.RuleSpec(
                    rule_type='Equals', inputs={'x': 'not_a_dict'}
                )
            ],
    ...
    def test_update_inapplicable_misconception_skill_ids(self) -> None:
        ...
        self.assertEqual(
            state.inapplicable_skill_misconception_ids, ['string_1']
        )
            ...
            'interaction': {
                ...
                'default_outcome': {
                    ...
                    'feedback': {
                        ...
                        'html': (
                            '<p><oppia-noninteractive-image filepath-with-value="&amp;quot;random.png&amp;quot;"></oppia-noninteractive-image>Hello this is test case to check image tag inside p tag</p>'
                        ),
                    },
                    'dest': 'Introduction',
                    'dest_if_really_stuck': None,
    ...
    def test_solution_validation(self) -> None:
        ...
        hints_list = [
            state_domain.Hint(state_domain.SubtitledHtml('hint_1', ''))
        ]
        init_state.update_interaction_hints(hints_list)
    ...
    def test_cannot_update_answer_groups_with_invalid_rule_input_value(
        ...
        with self.assertRaisesRegex(
            Exception,
            re.escape(
                'Value has the wrong type. It should be a TranslatableSetOfNormalizedString. The value is'
            ),
        ):
            exploration.init_state.update_interaction_answer_groups(
                [state_answer_group]
...
class RecordedVoiceoversDomainUnitTests(test_utils.GenericTestBase):
    ...
    def test_validation_with_invalid_content_id_list(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            re.escape(
                'Expected state recorded_voiceovers to match the listed content ids [\'invalid_content\']'
            ),
        ):
...
class VoiceoverDomainTests(test_utils.GenericTestBase):
    ...
    def test_validate_audio_extension(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            re.escape(
                'Invalid audio filename: it should have one of the following extensions: %s'
