...
class UserIdJobTests(test_utils.GenericTestBase):
    ...
    def _run_one_off_job(self):
        ...
        job_id = user_id_migration.UserIdJob.create_new()
        user_id_migration.UserIdJob.enqueue(job_id)
        self.assertEqual(
            self.count_jobs_in_taskqueue(
                taskqueue_services.QUEUE_NAME_ONE_OFF_JOBS), 1)
        self.process_and_flush_pending_tasks()
        stringified_output = (
            user_id_migration.UserIdJob.get_output(job_id))
    ...
    def setUp(self):
        ...
        with self.swap(test_utils.TestBase, 'signup_superadmin_user', empty):
            super(UserIdJobTests, self).setUp()
