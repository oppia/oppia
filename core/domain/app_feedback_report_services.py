...
def create_report_from_json(
    ...
    """
    return app_feedback_report_domain.AppFeedbackReport.from_submitted_feedback_dict(  # pylint: disable=line-too-long
        report_json
    )
...
def store_incoming_report_stats(
    ...
    unticketed_id = app_feedback_report_constants.UNTICKETED_ANDROID_REPORTS_STATS_TICKET_ID  # pylint: disable=line-too-long
...
def _update_report_stats_model_in_transaction(
    ...
    if stats_model is None:
        ...
        stats_dict = {
            stats_parameter_names.REPORT_TYPE.value: {report_type: 1},
            stats_parameter_names.COUNTRY_LOCALE_CODE.value: {country_locale_code: 1},
            stats_parameter_names.ENTRY_POINT_NAME.value: {entry_point_name: 1},
            stats_parameter_names.TEXT_LANGUAGE_CODE.value: {text_language_code: 1},
            stats_parameter_names.AUDIO_LANGUAGE_CODE.value: {audio_language_code: 1},
            stats_parameter_names.ANDROID_SDK_VERSION.value: {sdk_version: 1},
            stats_parameter_names.VERSION_NAME.value: {version_name: 1},
        }
        ...
        stats_dict[stats_parameter_names.REPORT_TYPE.value] = calculate_new_stats_count_for_parameter(
            stats_dict[stats_parameter_names.REPORT_TYPE.value],
            report_type,
            delta,
        )
        stats_dict[stats_parameter_names.COUNTRY_LOCALE_CODE.value] = calculate_new_stats_count_for_parameter(
            stats_dict[stats_parameter_names.COUNTRY_LOCALE_CODE.value],
            country_locale_code,
            delta,
        )
        stats_dict[stats_parameter_names.ENTRY_POINT_NAME.value] = calculate_new_stats_count_for_parameter(
            stats_dict[stats_parameter_names.ENTRY_POINT_NAME.value],
            entry_point_name,
            delta,
        )
        stats_dict[stats_parameter_names.AUDIO_LANGUAGE_CODE.value] = calculate_new_stats_count_for_parameter(
            stats_dict[stats_parameter_names.AUDIO_LANGUAGE_CODE.value],
            audio_language_code,
            delta,
        )
        stats_dict[stats_parameter_names.TEXT_LANGUAGE_CODE.value] = calculate_new_stats_count_for_parameter(
            stats_dict[stats_parameter_names.TEXT_LANGUAGE_CODE.value],
            text_language_code,
            delta,
        )
        stats_dict[stats_parameter_names.ANDROID_SDK_VERSION.value] = calculate_new_stats_count_for_parameter(
            stats_dict[stats_parameter_names.ANDROID_SDK_VERSION.value],
            sdk_version,
            delta,
        )
        stats_dict[stats_parameter_names.VERSION_NAME.value] = calculate_new_stats_count_for_parameter(
            stats_dict[stats_parameter_names.VERSION_NAME.value],
            version_name,
            delta,
        )
...
def get_ticket_from_model(
    ...
    return app_feedback_report_domain.AppFeedbackReportTicket(
        ticket_model.id,
        ticket_model.ticket_name,
        ticket_model.platform,
        ticket_model.github_issue_repo_name,
        ticket_model.github_issue_number,
        ticket_model.archived,
...
def get_stats_from_model(
    ...
    return app_feedback_report_domain.AppFeedbackReportDailyStats(
        stats_model.id,
        ticket_obj,
        stats_model.platform,
        stats_model.stats_tracking_date,
        stats_model.total_reports_submitted,
        param_stats,
    )
...
def get_android_report_from_model(
    ...
    user_supplied_feedback = app_feedback_report_domain.UserSuppliedFeedback(
        feedback_report.get_report_type_from_string(android_report_model.report_type),
        feedback_report.get_category_from_string(android_report_model.category),
        report_info_dict['user_feedback_selected_items'],
        report_info_dict['user_feedback_other_text_input'],
    )
    device_system_context = app_feedback_report_domain.AndroidDeviceSystemContext(
        ...
        feedback_report.get_android_network_type_from_string(report_info_dict['network_type']),
    )
    ...
    app_context = app_feedback_report_domain.AndroidAppContext(
        entry_point,
        android_report_model.text_language_code,
        android_report_model.audio_language_code,
        feedback_report.get_android_text_size_from_string(report_info_dict['text_size']),
        ...
        report_info_dict['event_logs'],
        report_info_dict['logcat_logs'],
    )
    return app_feedback_report_domain.AppFeedbackReport(
        ...
        android_report_model.platform,
        android_report_model.submitted_on,
        android_report_model.local_timezone_offset_hrs,
        android_report_model.ticket_id,
        android_report_model.scrubbed_by,
...
def save_feedback_report_to_storage(
    ...
    report_info_json = {
        'user_feedback_selected_items': (user_supplied_feedback.user_feedback_selected_items),
        'user_feedback_other_text_input': (user_supplied_feedback.user_feedback_other_text_input),
    }
    ...
    report_info_json = {
        ...
        'android_device_language_locale_code': (device_system_context.device_language_locale_code),
        ...
        'only_allows_wifi_download_and_update': str(app_context.only_allows_wifi_download_and_update),
    ...
    if new_incoming_report:
        app_feedback_report_models.AppFeedbackReportModel.create(
            ...
            entry_point.story_id,
            entry_point.exploration_id,
            entry_point.subtopic_id,
...
def reassign_ticket(
    ...
    if old_ticket_id is None:
        ...
        if len(old_ticket_obj.reports) == 0:
            ...
            if old_ticket_obj.newest_report_creation_timestamp == (report.submitted_on_timestamp):
    ...
    new_ticket_id = app_feedback_report_constants.UNTICKETED_ANDROID_REPORTS_STATS_TICKET_ID  # pylint: disable=line-too-long
