...
def get_report_models(
    ...
    """
    report_models = app_feedback_report_models.AppFeedbackReportModel.get_multi(
        report_ids
    )
    ...
    if strict:
        for index, report_model in enumerate(report_models):
            if report_model is None:
                raise Exception(
                    'No AppFeedbackReportModel exists for the id %s' % report_ids[index]
                )
    ...
    if stats_model is None:
        ...
        stats_dict[stats_parameter_names.REPORT_TYPE.value] = (
            calculate_new_stats_count_for_parameter(
                stats_dict[stats_parameter_names.REPORT_TYPE.value], report_type, delta
            )
        )
...
def calculate_new_stats_count_for_parameter(
    current_stats_map: Dict[str, int], current_value: str, delta: int
) -> Dict[str, int]:
...
def scrub_single_app_feedback_report(
    report: app_feedback_report_domain.AppFeedbackReport, scrubbed_by: str
) -> None:
...
def reassign_ticket(
    ...
    new_ticket_model = (
        app_feedback_report_models.AppFeedbackReportTicketModel.get_by_id(new_ticket_id)
    )
...
def edit_ticket_name(
    ticket: app_feedback_report_domain.AppFeedbackReportTicket, new_name: str
) -> None:
