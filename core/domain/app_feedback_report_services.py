...
def _update_report_stats_model_in_transaction(
    ticket_id: str, platform: str, date: datetime.datetime,
    report_obj: app_feedback_report_domain.AppFeedbackReport, delta: int
) -> None:
    ...
    if stats_model is None:
        ...
        stats_dict[stats_parameter_names.VERSION_NAME.value] = (
            calculate_new_stats_count_for_parameter(
                stats_dict[stats_parameter_names.VERSION_NAME.value], version_name,
                delta
            )
        )
...
def create_app_daily_stats_from_model_json(
    ...
    for (stats_name, stats_values_dict) in daily_param_stats.items():
        ...
        counts_obj = (
            ...
            ReportStatsParameterValueCounts(stats_values_dict)
        )
...
def get_android_report_from_model(
    ...
    app_context = app_feedback_report_domain.AndroidAppContext(
        ...
        report_info_dict['account_is_profile_admin'], report_info_dict['event_logs'],
    ...
    return app_feedback_report_domain.AppFeedbackReport(
        ...
        android_report_model.local_timezone_offset_hrs, android_report_model.ticket_id,
        android_report_model.scrubbed_by, user_supplied_feedback, device_system_context,
        app_context
    )
...
def save_feedback_report_to_storage(
    ...
    if new_incoming_report:
        app_feedback_report_models.AppFeedbackReportModel.create(
            ...
            report.local_timezone_offset_hrs, user_supplied_feedback.report_type.value,
            user_supplied_feedback.category.value, device_system_context.version_name,
            device_system_context.device_country_locale_code,
            device_system_context.sdk_version, device_system_context.device_model,
            entry_point.entry_point_name, entry_point.topic_id, entry_point.story_id,
            entry_point.exploration_id, entry_point.subtopic_id,
            app_context.text_language_code, app_context.audio_language_code, None, None
        )
