...
class ExplorationRightsTests(test_utils.GenericTestBase):
    ...
    def test_demo_exploration(self) -> None:
        ...
        self.assertTrue(
            rights_manager.check_can_voiceover_activity(self.user_a, exp_rights)
        )
        ...
        self.assertTrue(
            rights_manager.check_can_access_activity(self.user_moderator, exp_rights)
        )
        self.assertTrue(
            rights_manager.check_can_edit_activity(self.user_moderator, exp_rights)
        )
        self.assertTrue(
            rights_manager.check_can_voiceover_activity(self.user_moderator, exp_rights)
        )
        self.assertTrue(
            rights_manager.check_can_save_activity(self.user_moderator, exp_rights)
        )
        self.assertTrue(
            rights_manager.check_can_delete_activity(self.user_moderator, exp_rights)
        )
        self.assertTrue(
        ...
        self.assertFalse(
            rights_manager.check_can_manage_voice_artist_in_activity(self.user_a, None)
        )

    def test_check_can_modify_core_activity_roles_for_none_activity(self) -> None:
        self.assertFalse(
            rights_manager.check_can_modify_core_activity_roles(self.user_a, None)
        )
    ...
    def test_newly_created_exploration(self) -> None:
        ...
        self.assertFalse(
            rights_manager.check_can_voiceover_activity(self.user_b, exp_rights)
        )
        self.assertFalse(
            rights_manager.check_can_save_activity(self.user_b, exp_rights)
        )
        self.assertFalse(
    ...
    def test_get_activity_rights_raise_error_for_invalid_activity_type(self) -> None:
        with self.assertRaisesRegex(
            ...
            rights_manager._get_activity_rights(
                'invalid_type', self.user_id_a
    ...
    def test_assign_role_for_exploration_raises_error_for_invalid_activity_id(
        ...
        with self.assertRaisesRegex(Exception, 'No activity_rights exists'):
            rights_manager.assign_role_for_exploration(
                self.user_b, 'abcdefg', self.user_id_c, rights_domain.ROLE_VIEWER
            )
    ...
    def test_setting_rights_of_exploration(self) -> None:
        ...
        with self.assertRaisesRegex(Exception, 'Could not assign new role.'):
            rights_manager.assign_role_for_exploration(
                self.user_b, self.EXP_ID, self.user_id_c, rights_domain.ROLE_VIEWER
            )
        ...
        rights_manager.assign_role_for_exploration(
            self.user_b, self.EXP_ID, self.user_id_c, rights_domain.ROLE_OWNER
        )
        rights_manager.assign_role_for_exploration(
            self.user_b, self.EXP_ID, self.user_id_d, rights_domain.ROLE_EDITOR
        )
        rights_manager.assign_role_for_exploration(
            self.user_b, self.EXP_ID, self.user_id_f, rights_domain.ROLE_VIEWER
        )
    ...
    def test_publishing_and_unpublishing_exploration(self) -> None:
        ...
        self.assertFalse(
            rights_manager.check_can_unpublish_activity(self.user_a, exp_rights)
        )
    ...
    def test_unpublished_exploration_is_removed_from_completed_activities(self) -> None:
        ...
        self.assertEqual(
            learner_progress_services.get_all_completed_exp_ids(self.user_id_f),
            [self.EXP_ID],
        )
    ...
    def test_unpublished_exploration_is_removed_from_incomplete_activities(
        ...
        self.assertEqual(
            learner_progress_services.get_all_incomplete_exp_ids(self.user_id_e),
            [self.EXP_ID],
        )
    ...
    def test_check_exploration_rights(self) -> None:
        ...
        rights_manager.assign_role_for_exploration(
            self.user_a, self.EXP_ID, self.user_id_c, rights_domain.ROLE_EDITOR
        )
    ...
    def test_owner_cannot_be_reassigned_as_owner(self) -> None:
        ...
        with self.assertRaisesRegex(Exception, 'This user already owns this'):
            rights_manager.assign_role_for_exploration(
                self.user_a, self.EXP_ID, self.user_id_a, rights_domain.ROLE_OWNER
            )
    ...
    def test_deassign_editor_is_successful_with_all_valid_commit_messages(self) -> None:
        ...
        snapshots_data = exp_models.ExplorationRightsModel.get_snapshots_metadata(
            self.EXP_ID, [1]
        )
    ...
    def test_guest_user_cannot_perform_activity_actions(self) -> None:
        ...
        self.assertFalse(
            rights_manager.check_can_delete_activity(guest_user, collection_rights)
        )
        ...
        self.assertFalse(
            rights_manager.check_can_publish_activity(guest_user, collection_rights)
        )
...
class CollectionRightsTests(test_utils.GenericTestBase):
    ...
    def test_demo_collection(self) -> None:
        ...
        self.assertTrue(
            rights_manager.check_can_access_activity(self.user_a, collection_rights)
        )
        self.assertTrue(
            rights_manager.check_can_edit_activity(self.user_a, collection_rights)
        )
        self.assertFalse(
            rights_manager.check_can_delete_activity(self.user_a, collection_rights)
        )
    ...
    def test_newly_created_collection(self) -> None:
        ...
        self.assertFalse(
            rights_manager.check_can_access_activity(self.user_b, collection_rights)
        )
        self.assertFalse(
            rights_manager.check_can_edit_activity(self.user_b, collection_rights)
        )
        self.assertFalse(
            rights_manager.check_can_delete_activity(self.user_b, collection_rights)
        )
    ...
    def test_editor_can_be_reassigned_as_owner(self) -> None:
        ...
        rights_manager.assign_role_for_collection(
            self.user_a, self.COLLECTION_ID, self.user_id_b, rights_domain.ROLE_OWNER
        )
    ...
    def test_viewer_can_be_reassigned_as_owner(self) -> None:
        ...
        rights_manager.assign_role_for_collection(
            self.user_a, self.COLLECTION_ID, self.user_id_b, rights_domain.ROLE_VIEWER
        )
        collection_rights = rights_manager.get_collection_rights(self.COLLECTION_ID)
    ...
    def test_setting_rights_of_collection(self) -> None:
        ...
        rights_manager.assign_role_for_collection(
            self.user_b, self.COLLECTION_ID, self.user_id_c, rights_domain.ROLE_OWNER
        )
        rights_manager.assign_role_for_collection(
            self.user_b, self.COLLECTION_ID, self.user_id_d, rights_domain.ROLE_EDITOR
        )
        rights_manager.assign_role_for_collection(
            self.user_b, self.COLLECTION_ID, self.user_id_e, rights_domain.ROLE_VIEWER
        )
    ...
    def test_publishing_and_unpublishing_collection(self) -> None:
        ...
        self.assertFalse(
            rights_manager.check_can_unpublish_activity(self.user_a, collection_rights)
        )
...
class CheckCanReleaseOwnershipTest(test_utils.GenericTestBase):
    ...
    def test_owner_can_release_ownership_of_published_exploration(self) -> None:
        self.assertTrue(
            rights_manager.check_can_release_ownership(
                self.owner, rights_manager.get_exploration_rights(self.published_exp_id)
            )
        )
    ...
    def test_owner_cannot_release_ownership_of_private_exploration(self) -> None:
        self.assertFalse(
            rights_manager.check_can_release_ownership(
                self.owner, rights_manager.get_exploration_rights(self.private_exp_id)
            )
        )
...
class CheckCanUnpublishActivityTest(test_utils.GenericTestBase):
    ...
    def test_owner_cannot_unpublish_published_collection(self) -> None:
        self.assertFalse(
            rights_manager.check_can_unpublish_activity(
                self.owner, rights_manager.get_collection_rights(self.published_col_id)
            )
        )
