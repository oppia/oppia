...
class TranslationServiceTests(test_utils.GenericTestBase):
    ...
    def test_get_machine_translation_checks_datastore_first(self) -> None:
        with self.swap_to_always_raise(translate_services.CLIENT, 'translate', error=AssertionError):
            self.assertEqual(
                translation_services.get_and_cache_machine_translation('en', 'es', 'text to translate'),
                'texto para traducir',
            )
...
class EntityTranslationServicesTest(test_utils.GenericTestBase):
    ...
    def test_compute_translation_related_change_needs_update_for_language(
        ...
        self.assertEqual(entity_translation_model.entity_version, 5)
        self.assertEqual(
            [translation['needs_update'] for translation in entity_translation_model.translations.values()],
            [False, False],
        )
        ...
        entity_translation = entity_translation_models[0]
        self.assertItemsEqual(
            [translation['needs_update'] for translation in entity_translation.translations.values()],
            [False, True],
        )
    ...
    def test_compute_translation_related_changes_upon_revert(self) -> None:
        ...
        entity_translation_models, _ = translation_services.compute_translation_related_changes_upon_revert(  # pylint: disable=line-too-long
