...
(
    app_feedback_report_models, auth_models, blog_models, collection_models,
...
class WipeoutServicePreDeleteTests(test_utils.GenericTestBase):
    ...
    def test_pre_delete_user_email_subscriptions(self) -> None:
        ...
        self.assertItemsEqual(
            observed_log_messages, [
                ...
                self.USER_1_EMAIL, 'Logging project ID for debugging: dev-project-id'
            ]
        )
    ...
    def test_pre_delete_user_collection_ownership_is_released(self) -> None:
        ...
        rights_manager.assign_role_for_collection(
            user_services.get_system_user(), 'col_id', self.user_2_id,
            feconf.ROLE_EDITOR
        )
    ...
    def test_pre_delete_user_exploration_ownership_is_released(self) -> None:
        ...
        rights_manager.assign_role_for_exploration(
            user_services.get_system_user(), 'exp_id', self.user_2_id,
            feconf.ROLE_EDITOR
        )
    ...
    def test_pre_delete_user_user_is_deassigned_from_topics(self) -> None:
        ...
        topic_services.assign_role(
            user_services.get_system_user(), self.user_1_actions, feconf.ROLE_MANAGER,
            'top_id'
        )
...
class WipeoutServiceDeleteAppFeedbackReportModelsTests(test_utils.GenericTestBase):
    ...
    REPORT_ID_1: Final = '%s.%s.%s' % (
        PLATFORM_ANDROID, REPORT_SUBMITTED_TIMESTAMP_1.second, 'randomInteger123'
    )
    REPORT_ID_2: Final = '%s.%s.%s' % (
        PLATFORM_ANDROID, REPORT_SUBMITTED_TIMESTAMP_2.second, 'randomInteger321'
    )
    REPORT_ID_3: Final = '%s.%s.%s' % (
        PLATFORM_ANDROID, REPORT_SUBMITTED_TIMESTAMP_2.second, 'differentInt'
    )
...
class WipeoutServiceDeleteCollectionModelsTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        rights_manager.assign_role_for_collection(
            user_services.get_user_actions_info(self.user_1_id), self.COL_1_ID,
            self.user_2_id, feconf.ROLE_OWNER
        )
...
class WipeoutServiceDeleteExplorationModelsTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        rights_manager.assign_role_for_exploration(
            user_services.get_user_actions_info(self.user_1_id), self.EXP_1_ID,
            self.user_2_id, feconf.ROLE_OWNER
        )
...
class WipeoutServiceDeleteFeedbackModelsTests(test_utils.GenericTestBase):
    ...
    def test_one_feedback_is_pseudonymized(self) -> None:
        ...
        suggestion_model_model = (
            suggestion_models.GeneralSuggestionModel.get_by_id(self.FEEDBACK_1_ID)
        )
        self.assertEqual(
    ...
    def test_verify_user_delete_when_user_is_not_deleted_returns_false(self) -> None:
        ...
        task_entry_id = (
            improvements_models.ExplorationStatsTaskEntryModel.generate_task_id(
                constants.TASK_ENTITY_TYPE_EXPLORATION, self.EXP_2_ID, 1,
                constants.TASK_TYPE_HIGH_BOUNCE_RATE, constants.TASK_TARGET_TYPE_STATE,
                'State'
            )
        )
        task_entry_model = (
            improvements_models.ExplorationStatsTaskEntryModel.get(task_entry_id)
        )
...
class WipeoutServiceDeleteQuestionModelsTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.save_new_question(
            self.QUESTION_1_ID, self.user_1_id,
            self._create_valid_question_data('ABC', content_id_generator),
            [self.SKILL_1_ID], content_id_generator.next_content_id_index
        )
        wipeout_service.pre_delete_user(self.user_1_id)
        wipeout_service.pre_delete_user(self.user_2_id)
        self.process_and_flush_pending_tasks()
    ...
    def test_multiple_questions_are_pseudonymized(self) -> None:
        content_id_generator = translation_domain.ContentIdGenerator()
        self.save_new_question(
            self.QUESTION_2_ID, self.user_1_id,
            self._create_valid_question_data('ABC', content_id_generator),
            [self.SKILL_1_ID], content_id_generator.next_content_id_index
        )
    ...
    def test_multiple_questions_with_multiple_users_are_pseudonymized(self) -> None:
        content_id_generator = translation_domain.ContentIdGenerator()
        self.save_new_question(
            self.QUESTION_2_ID, self.user_2_id,
            self._create_valid_question_data('ABC', content_id_generator),
            [self.SKILL_1_ID], content_id_generator.next_content_id_index
        )
...
class WipeoutServiceDeleteTopicModelsTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.save_new_topic(self.TOP_1_ID, self.user_1_id)
        topic_services.assign_role(
            self.user_1_actions, self.user_1_actions, topic_domain.ROLE_MANAGER,
            self.TOP_1_ID
        )
        topic_services.assign_role(
            self.user_1_actions, self.user_2_actions, topic_domain.ROLE_MANAGER,
            self.TOP_1_ID
        )
    ...
    def test_one_topic_snapshot_metadata_is_pseudonymized(self) -> None:
        ...
        metadata_model = (
            topic_models.TopicSnapshotMetadataModel.get_by_id('%s-1' % self.TOP_1_ID)
        )
    ...
    def test_multiple_topics_are_pseudonymized(self) -> None:
        ...
        self.assertEqual(commit_log_model.user_id, topic_mappings[self.TOP_1_ID])
        metadata_model = (
            topic_models.TopicSnapshotMetadataModel.get_by_id('%s-1' % self.TOP_2_ID)
        )
...
class WipeoutServiceDeleteBlogPostModelsTests(test_utils.GenericTestBase):
    ...
    def test_one_blog_post_model_is_pseudonymized(self) -> None:
        ...
        blog_post_summary_model = (
            blog_models.BlogPostSummaryModel.get_by_id(self.BLOG_1_ID)
        )
        self.assertEqual(
        ...
        blog_post_rights_model = (
            blog_models.BlogPostRightsModel.get_by_id(self.BLOG_1_ID)
        )
...
class WipeoutServiceDeletelLearnerGroupModelsTests(test_utils.GenericTestBase):
    ...
    def test_delete_learner_is_successful(self) -> None:
        ...
        learner_group_model_1 = (
            learner_group_models.LearnerGroupModel.get_by_id(self.LEARNER_GROUP_ID_1)
        )
        learner_group_model_2 = (
            learner_group_models.LearnerGroupModel.get_by_id(self.LEARNER_GROUP_ID_2)
        )
...
class WipeoutServiceDeleteVersionHistoryModelsTests(test_utils.GenericTestBase):
    ...
    def test_multiple_version_history_models_are_pseudonymized(self) -> None:
        ...
        pseudonymized_models = (
            exp_models.ExplorationVersionHistoryModel.get_multi(version_history_ids)
        )
