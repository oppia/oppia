...
class WipeoutServiceDeleteFeedbackAndSuggestionModelsTests(
    ...
    def setUp(self):
        super(
            WipeoutServiceDeleteFeedbackAndSuggestionModelsTests, self).setUp()
        self.signup(self.USER_1_EMAIL, self.USER_1_USERNAME)
        self.signup(self.USER_2_EMAIL, self.USER_2_USERNAME)
        self.user_1_id = self.get_user_id_from_email(self.USER_1_EMAIL)
        self.user_2_id = self.get_user_id_from_email(self.USER_2_EMAIL)
    ...
    def test_one_feedback_is_pseudonymized(self):
        ...
        pending_deletion_model = (
            user_models.PendingDeletionRequestModel.get_by_id(self.user_1_id))
        feedback_thread_model_model = (
            feedback_models.GeneralFeedbackThreadModel.get_by_id(
                self.FEEDBACK_1_ID)
        )
        self.assertEqual(
            feedback_thread_model_model.original_author_id,
            pending_deletion_model
    ...
    def test_one_question_when_the_deletion_is_repeated_is_pseudonymized(self):
        ...
        feedback_thread_model_model.original_author_id = self.user_1_id
        feedback_thread_model_model.put()
...
class WipeoutServiceVerifyDeleteFeedbackAndSuggestionModelsTests(
        test_utils.GenericTestBase):
