...
class WipeoutServiceHelpersTests(test_utils.GenericTestBase):
    ...
    def test_get_number_of_pending_deletion_requests_returns_correct_number(
        ...
        wipeout_service.save_pending_deletion_requests(
            [
                wipeout_domain.PendingDeletionRequest.create_default(self.user_1_id, self.USER_1_USERNAME, self.USER_1_EMAIL),
                wipeout_domain.PendingDeletionRequest.create_default(self.user_2_id, self.USER_2_USERNAME, self.USER_2_EMAIL),
            ]
        )
        number_of_pending_deletion_requests = wipeout_service.get_number_of_pending_deletion_requests()
...
class WipeoutServicePreDeleteTests(test_utils.GenericTestBase):
    ...
    def test_pre_delete_user_email_subscriptions(self) -> None:
        ...
        self.assertItemsEqual(
            ...
            ['Email ID %s permanently deleted from bulk email provider\'s db. Cannot access API, since this is a dev environment' % self.USER_1_EMAIL] + (['Logging project ID for debugging: dev-project-id'] * 6),
        )
...
class WipeoutServiceDeleteConfigModelsTests(test_utils.GenericTestBase):
    ...
    def test_one_config_property_is_pseudonymized(self) -> None:
        ...
        metadata_model = config_models.PlatformParameterSnapshotMetadataModel.get_by_id('%s-1' % self.CONFIG_1_ID)
    ...
    def test_remove_user_from_translation_coordinators(self) -> None:
        ...
        existing_model = suggestion_models.TranslationCoordinatorsModel.get_by_id('hi')
        self.assertIn(self.user_1_id, existing_model.coordinator_ids)
        self.assertEqual(existing_model.coordinators_count, 2)

        wipeout_service.delete_user(wipeout_service.get_pending_deletion_request(self.user_1_id))

        updated_model = suggestion_models.TranslationCoordinatorsModel.get_by_id('hi')
    ...
    def test_multiple_config_properties_are_pseudonymized(self) -> None:
        ...
        config_mappings = user_models.PendingDeletionRequestModel.get_by_id(self.user_1_id).pseudonymizable_entity_mappings[models.Names.CONFIG.value]
        metadata_model_1 = config_models.PlatformParameterSnapshotMetadataModel.get_by_id('%s-1' % self.CONFIG_1_ID)
        self.assertEqual(metadata_model_1.committer_id, config_mappings[self.CONFIG_1_ID])

        metadata_model_2 = config_models.PlatformParameterSnapshotMetadataModel.get_by_id('%s-1' % self.CONFIG_2_ID)
    ...
    def test_multiple_config_properties_with_multiple_users_are_pseudonymized(
        ...
        metadata_model_3 = config_models.PlatformParameterSnapshotMetadataModel.get_by_id('%s-1' % self.CONFIG_2_ID)
    ...
    def test_one_config_property_with_multiple_users_is_pseudonymized(
        ...
        # Verify second user is not yet deleted.
        metadata_model_2 = config_models.PlatformParameterSnapshotMetadataModel.get_by_id('%s-2' % self.CONFIG_1_ID)
        self.assertEqual(metadata_model_2.committer_id, self.user_2_id)
        ...
        # Verify second user is deleted.
        config_mappings_2 = user_models.PendingDeletionRequestModel.get_by_id(self.user_2_id).pseudonymizable_entity_mappings[models.Names.CONFIG.value]
        metadata_model_3 = config_models.PlatformParameterSnapshotMetadataModel.get_by_id('%s-2' % self.CONFIG_1_ID)
...
class WipeoutServiceDeleteCollectionModelsTests(test_utils.GenericTestBase):
    ...
    def test_one_collection_with_missing_snapshot_is_pseudonymized(
        ...
        self.assertItemsEqual(
            log_messages,
            [
                '[WIPEOUT] The commit log model \'CollectionCommitLogEntryModel\' and snapshot models [\'CollectionSnapshotMetadataModel\', \'CollectionRightsSnapshotMetadataModel\'] IDs differ. Snapshots without commit logs: [], commit logs without snapshots: [\'%s\'].' % self.COL_2_ID,
            ],
        )
...
class WipeoutServiceDeleteExplorationModelsTests(test_utils.GenericTestBase):
    ...
    def test_one_exploration_with_missing_snapshot_is_pseudonymized(
        ...
        self.assertItemsEqual(
            log_messages,
            ['[WIPEOUT] The commit log model \'ExplorationCommitLogEntryModel\' and snapshot models [\'ExplorationSnapshotMetadataModel\', \'ExplorationRightsSnapshotMetadataModel\'] IDs differ. Snapshots without commit logs: [], commit logs without snapshots: [\'%s\'].' % self.EXP_2_ID],
        )
...
class WipeoutServiceDeleteImprovementsModelsTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.signup(self.USER_1_EMAIL, self.USER_1_USERNAME)
        self.user_1_id = self.get_user_id_from_email(self.USER_1_EMAIL)
        self.improvements_model_1_id = improvements_models.ExplorationStatsTaskEntryModel.create(
        ...
        self.improvements_model_2_id = improvements_models.ExplorationStatsTaskEntryModel.create(
            entity_type=constants.TASK_ENTITY_TYPE_EXPLORATION,
...
class WipeoutServiceVerifyDeleteImprovementsModelsTests(test_utils.GenericTestBase):
    ...
    def test_verify_user_delete_when_user_is_not_deleted_returns_false(
        ...
        task_entry_id = improvements_models.ExplorationStatsTaskEntryModel.generate_task_id(
...
class WipeoutServiceDeleteQuestionModelsTests(test_utils.GenericTestBase):
    ...
    def test_one_question_with_missing_snapshot_is_pseudonymized(self) -> None:
        ...
        self.assertEqual(
            log_messages,
            ['[WIPEOUT] The commit log model \'QuestionCommitLogEntryModel\' and snapshot models [\'QuestionSnapshotMetadataModel\'] IDs differ. Snapshots without commit logs: [], commit logs without snapshots: [\'%s\'].' % self.QUESTION_2_ID],
        )
...
class WipeoutServiceDeleteSkillModelsTests(test_utils.GenericTestBase):
    ...
    def test_one_skill_with_missing_snapshot_is_pseudonymized(self) -> None:
        ...
        self.assertEqual(
            log_messages,
            ['[WIPEOUT] The commit log model \'SkillCommitLogEntryModel\' and snapshot models [\'SkillSnapshotMetadataModel\'] IDs differ. Snapshots without commit logs: [], commit logs without snapshots: [\'%s\'].' % self.SKILL_2_ID],
        )
...
class WipeoutServiceDeleteStoryModelsTests(test_utils.GenericTestBase):
    ...
    def test_one_story_with_missing_snapshot_is_pseudonymized(self) -> None:
        ...
        self.assertEqual(
            log_messages,
            ['[WIPEOUT] The commit log model \'StoryCommitLogEntryModel\' and snapshot models [\'StorySnapshotMetadataModel\'] IDs differ. Snapshots without commit logs: [], commit logs without snapshots: [\'%s\'].' % self.STORY_2_ID],
        )
...
class WipeoutServiceDeleteSubtopicModelsTests(test_utils.GenericTestBase):
    ...
    def test_one_subtopic_with_missing_snapshot_is_pseudonymized(self) -> None:
        ...
        self.assertEqual(
            log_messages,
            ['[WIPEOUT] The commit log model \'SubtopicPageCommitLogEntryModel\' and snapshot models [\'SubtopicPageSnapshotMetadataModel\'] IDs differ. Snapshots without commit logs: [], commit logs without snapshots: [\'%s\'].' % self.SUBTOP_2_ID],
        )
...
class WipeoutServiceDeleteTopicModelsTests(test_utils.GenericTestBase):
    ...
    def test_one_topic_with_missing_snapshot_is_pseudonymized(self) -> None:
        ...
        self.assertItemsEqual(
            log_messages,
            ['[WIPEOUT] The commit log model \'TopicCommitLogEntryModel\' and snapshot models [\'TopicSnapshotMetadataModel\', \'TopicRightsSnapshotMetadataModel\'] IDs differ. Snapshots without commit logs: [], commit logs without snapshots: [\'%s\'].' % self.TOP_2_ID],
        )
...
class WipeoutServiceDeleteVersionHistoryModelsTests(test_utils.GenericTestBase):
    ...
    def test_multiple_version_history_models_are_pseudonymized(self) -> None:
        ...
        version_history_ids = [
            self.version_history_model_class.get_instance_id(self.EXPLORATION_ID_0, self.VERSION_1),
            self.version_history_model_class.get_instance_id(self.EXPLORATION_ID_1, self.VERSION_1),
        ]
...
class WipeoutServiceVerifyProfilePictureIsDeletedTests(test_utils.GenericTestBase):
    ...
    def test_log_error_when_profile_pictures_are_missing_while_deletion(
        ...
        self.assertEqual(
            logs,
            [
                '[WIPEOUT] Profile picture of username username2 in .png format does not exists.',
                '[WIPEOUT] Profile picture of username username2 in .webp format does not exists.',
            ],
        )
    ...
    def test_log_error_when_profile_pictures_are_missing_while_verification(
        ...
        self.assertEqual(
            logs,
            [
                '[WIPEOUT] Profile picture in .png format is not deleted for user having username username2.',
                '[WIPEOUT] Profile picture in .webp format is not deleted for user having username username2.',
            ],
        )
