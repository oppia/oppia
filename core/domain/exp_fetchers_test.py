...
class ExplorationRetrievalTests(test_utils.GenericTestBase):
    ...
    def test_get_exploration_summaries_from_models(self) -> None:
        ...
        exp_summary_dict = (
            exp_fetchers.get_exploration_summaries_from_models(exp_summary_models)
        )
    ...
    def test_get_exploration_summaries_where_user_has_role(self) -> None:
        exp_ids = [self.EXP_1_ID, self.EXP_2_ID, self.EXP_3_ID]
        exp_summaries = (
            exp_fetchers.get_exploration_summaries_where_user_has_role(self.owner_id)
        )
...
"""
        ...
        feconf.DEFAULT_INIT_STATE_NAME, feconf.CURRENT_STATE_SCHEMA_VERSION
    )
