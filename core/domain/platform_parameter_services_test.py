...
class PlatformFeatureServiceTest(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        caching_services.delete_multi(
            caching_services.CACHE_NAMESPACE_PLATFORM_PARAMETER, None, self.param_names
        )
        ...
        self.swap_all_platform_params_list = self.swap(
            platform_parameter_list, 'ALL_PLATFORM_PARAMS_LIST', self.param_name_enums
        )
    ...
    def test_get_all_platform_parameters_dicts(self) -> None:
        ...
        with self.swap_all_platform_params_list:
            self.assertEqual(
                parameter_services.get_all_platform_parameters_dicts(), expected_dicts
            )

    def test_get_platform_parameter_value(self) -> None:
        with self.swap_all_platform_params_list:
            self.assertEqual(
                parameter_services.get_platform_parameter_value(self.param_b.name),
                False
            )
    ...
    def test_evaluation_context_for_app_version_works_as_expected(self) -> None:
        with self.swap_all_platform_params_list:
            self.assertFalse(
                parameter_services.get_platform_parameter_value(self.param_c.name)
            )
