...
class ImageValidationServiceTests(test_utils.GenericTestBase):
    ...
    def _assert_image_validation_error(
        self, image: Union[str, bytes], filename: str, entity_type: str,
    ...
    def test_image_validation_checks(self) -> None:
        ...
        self._assert_image_validation_error(
            large_image, 'image.svg', feconf.ENTITY_TYPE_EXPLORATION,
        ...
        self._assert_image_validation_error(
            large_image, 'image.svg', feconf.ENTITY_TYPE_BLOG_POST,
            'Image exceeds file size limit of 1024 KB'
        )

        invalid_svg = b'<badsvg></badsvg>'
        self._assert_image_validation_error(
            invalid_svg, 'image.svg', feconf.ENTITY_TYPE_EXPLORATION,
            'Unsupported tags/attributes found in the SVG'
        )

        no_xmlns_attribute_svg = invalid_svg = b'<svg></svg>'
        self._assert_image_validation_error(
            no_xmlns_attribute_svg, 'image.svg', feconf.ENTITY_TYPE_EXPLORATION,
            'The svg tag does not contains the \'xmlns\' attribute.'
        )

        self._assert_image_validation_error(
            b'not an image', 'image.png', feconf.ENTITY_TYPE_EXPLORATION,
            'Image not recognized'
        )

        self._assert_image_validation_error(
            self.raw_image, '.png', feconf.ENTITY_TYPE_EXPLORATION, 'Invalid filename'
        )
        self._assert_image_validation_error(
            self.raw_image, 'image/image.png', feconf.ENTITY_TYPE_EXPLORATION,
            'Filenames should not include slashes'
        )
        self._assert_image_validation_error(
            self.raw_image, 'image', feconf.ENTITY_TYPE_EXPLORATION,
            'Image filename with no extension'
        )
        self._assert_image_validation_error(
            self.raw_image, 'image.pdf', feconf.ENTITY_TYPE_EXPLORATION,
            'Expected a filename ending in .png'
        )
        base64_encoded_string = 'SGVsbG8gV29ybGQh'
        self._assert_image_validation_error(
            base64_encoded_string, 'image.svg', feconf.ENTITY_TYPE_EXPLORATION,
            'Image not recognized'
        )
        ...
        validated_image = image_validation_services.validate_image_and_filename(
            base64_encoded_xmlns_attribute_svg, 'image.svg',
            feconf.ENTITY_TYPE_EXPLORATION
        )
