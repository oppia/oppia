...
class UserSettings(object):
    ...
    def __init__(
        ...
        if profile_picture_data_url:
            ...
            self.profile_picture_data_url = fetch_gravatar(email)
...
def fetch_gravatar(email):
    """Gets autogenerated avatar for a given email.

    Returns Default picture if there is an error.
    """
    base_url = "http://www.gravatar.com/avatar/"
    avatar_url = base_url + hashlib.md5(email.lower()).hexdigest() + "?"
    avatar_url += urllib.urlencode({'d':'retro', 's':str(PROFILE_PICTURE_SIZE)})
    try:
        request = urllib2.Request(avatar_url)
        response = urllib2.urlopen(request)
        if response.getcode() == 200:
            content_type = response.headers.getheader('content-type')
            encoded_body = base64.b64encode(response.read())
            return "data:{};base64,{}".format(content_type, encoded_body)
        else:
            return "/images/avatar/user_blue_72px.png"
    except urllib2.HTTPError, e:
        print "Fetching gravatar failed with error code- %s" %e.code
        return "/images/avatar/user_blue_72px.png"
