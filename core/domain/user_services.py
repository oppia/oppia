...
auth_models, user_models, audit_models, suggestion_models = (models.
    Registry.import_models([models.Names.AUTH, models.Names.USER, models.
    Names.AUDIT, models.Names.SUGGESTION]))
...
@overload
def get_user_id_from_username(username: str, *, strict: Literal[False]
    ) ->Optional[str]:
    ...
...
def get_user_id_from_username(username: str, strict: bool=False) ->Optional[str
    ...
    if user_model is None:
        if strict:
            raise Exception('No user_id found for the given username: %s' %
                username)
        return None
    else:
...
@overload
def get_multi_user_ids_from_usernames(usernames: List[str], *, strict:
    Literal[True]) ->List[str]:
    ...


@overload
def get_multi_user_ids_from_usernames(usernames: List[str]) ->List[Optional
    [str]]:
    ...
...
@overload
def get_multi_user_ids_from_usernames(usernames: List[str], *, strict:
    Literal[False]) ->List[Optional[str]]:
    ...


def get_multi_user_ids_from_usernames(usernames: List[str], strict: bool=False
    ...
    if len(usernames) == 0:
        return []
    normalized_usernames = [user_domain.UserSettings.normalize_username(
        username) for username in usernames]
    found_models: Sequence[user_models.UserSettingsModel
        ] = user_models.UserSettingsModel.query(user_models.
        UserSettingsModel.normalized_username.IN(normalized_usernames)).fetch()
    username_to_user_id_map = {model.normalized_username: model.id for
        model in found_models}
    user_ids = []
...
def get_user_settings_from_username(username: str) ->Optional[user_domain.
    UserSettings]:
...
def get_user_settings_from_email(email: str) ->Optional[user_domain.
    UserSettings]:
...
@overload
def get_users_settings(user_ids: Sequence[Optional[str]], *, strict:
    Literal[True], include_marked_deleted: bool=False) ->Sequence[user_domain
    .UserSettings]:
    ...
...
@overload
def get_users_settings(user_ids: Sequence[Optional[str]], *, strict:
    Literal[False], include_marked_deleted: bool=False) ->Sequence[Optional
    [user_domain.UserSettings]]:
    ...


@overload
def get_users_settings(user_ids: Sequence[Optional[str]], *, strict: bool=
    ..., include_marked_deleted: bool=False) ->Sequence[Optional[
    user_domain.UserSettings]]:
    ...


def get_users_settings(user_ids: Sequence[Optional[str]], strict: bool=
    False, include_marked_deleted: bool=False) ->Sequence[Optional[
    user_domain.UserSettings]]:
    ...
    user_settings_models = user_models.UserSettingsModel.get_multi(user_ids,
        include_deleted=include_marked_deleted)
    if strict:
    ...
    for i, model in enumerate(user_settings_models):
        if user_ids[i] == feconf.SYSTEM_COMMITTER_ID:
            result.append(user_domain.UserSettings(user_id=feconf.
                SYSTEM_COMMITTER_ID, email=feconf.SYSTEM_EMAIL_ADDRESS,
                roles=[feconf.ROLE_ID_FULL_USER, feconf.
                ROLE_ID_CURRICULUM_ADMIN, feconf.ROLE_ID_MODERATOR, feconf.
                ROLE_ID_VOICEOVER_ADMIN], banned=False, username='admin',
            ...
            result.append(_get_user_settings_from_model(model) if model is not
                None else None)
    return result
...
def get_gravatar_url(email: str) ->str:
    ...
    """
    return 'https://www.gravatar.com/avatar/%s?d=identicon&s=%s' % (hashlib
        .md5(email.encode('utf-8')).hexdigest(), GRAVATAR_SIZE_PX)
...
def fetch_gravatar(user_email: str) ->str:
    ...
    try:
        response = requests.get(gravatar_url, headers={b'Content-Type':
            b'image/png'}, allow_redirects=False, timeout=TIMEOUT_SECS)
    except Exception:
        logging.exception('Failed to fetch Gravatar from %s' % gravatar_url)
    else:
        if response.ok:
            if imghdr.what(None, h=response.content) == 'png':
                return utils.convert_image_binary_to_data_url(response.
                    content, 'png')
        else:
            logging.error('[Status %s] Failed to fetch Gravatar from %s' %
...
@overload
def get_user_settings(user_id: str, *, strict: Literal[True]
    ) ->user_domain.UserSettings:
    ...


@overload
def get_user_settings(user_id: str, *, strict: Literal[False]) ->Optional[
    user_domain.UserSettings]:
    ...


def get_user_settings(user_id: str, strict: bool=True) ->Optional[user_domain
    .UserSettings]:
...
@overload
def get_user_settings_by_auth_id(auth_id: str, *, strict: Literal[True]
    ) ->user_domain.UserSettings:
    ...


@overload
def get_user_settings_by_auth_id(auth_id: str) ->Optional[user_domain.
    UserSettings]:
    ...


@overload
def get_user_settings_by_auth_id(auth_id: str, *, strict: Literal[False]
    ) ->Optional[user_domain.UserSettings]:
    ...


def get_user_settings_by_auth_id(auth_id: str, strict: bool=False) ->Optional[
    user_domain.UserSettings]:
    """Return the user settings for a single user.
    ...
    user_settings_model = (None if user_id is None else user_models.
        UserSettingsModel.get_by_id(user_id))
...
def get_all_user_groups() ->List[user_domain.UserGroup]:
    ...
    """
    user_group_models: List[user_models.UserGroupModel] = list(user_models.
        UserGroupModel.get_all())
    user_groups_list = []
    for user_group_model in user_group_models:
        ...
        user_group = user_domain.UserGroup(user_group_model.id,
            user_group_model.name, member_usernames)
        user_groups_list.append(user_group)
    return user_groups_list


def _check_if_usernames_are_valid(name: str, member_usernames: List[str]
    ) ->None:
    ...
    """
    duplicates = [username for username in member_usernames if 
        member_usernames.count(username) > 1]
    duplicates = list(set(duplicates))
    if len(duplicates) > 0:
        raise Exception(f'Users list of user-group {name} contains ' +
            f'duplicates: {duplicates}.')
    filters = [(user_models.UserSettingsModel.username == username) for
        username in member_usernames]
    existing_users_settings: Sequence[user_models.UserSettingsModel
        ] = user_models.UserSettingsModel.query(datastore_services.any_of(*
        filters)).fetch()
    existing_members_usernames = [user_settings.username for user_settings in
        existing_users_settings]
    invalid_usernames = [username for username in member_usernames if 
        username not in existing_members_usernames]
    if len(invalid_usernames) > 0:
        raise Exception(f'Following users of user-group {name} ' +
            f'does not exist: {invalid_usernames}.')


def create_new_user_group(name: str, member_usernames: List[str]
    ...
    user_models.UserGroupModel(id=user_group.user_group_id, name=user_group
        .name, user_ids=user_ids).put()
    return user_group
...
def delete_user_group(user_group_id: str) ->None:
    ...
    """
    user_group_model = user_models.UserGroupModel.get(user_group_id, strict
        =False)
...
def update_user_group(user_group_id: str, name: str, member_usernames: List
    [str]) ->None:
...
def get_user_contribution_rights(user_id: str
    ) ->user_domain.UserContributionRights:
...
def get_users_contribution_rights(user_ids: List[str]) ->List[user_domain.
    ...
    """
    user_contribution_rights_models = (user_models.
        UserContributionRightsModel.get_multi(user_ids))
...
def get_reviewer_user_ids_to_notify() ->List[str]:
    ...
    reviewer_ids = [user_contribution_rights.id for
        user_contribution_rights in users_contribution_rights]
...
def get_all_reviewers_contribution_rights() ->List[user_domain.
    UserContributionRights]:
    ...
    """
    user_contribution_rights_models = (user_models.
        UserContributionRightsModel.get_all())
    return [_create_user_contribution_rights_from_model(
        user_contribution_rights_model) for user_contribution_rights_model in
        user_contribution_rights_models]


def _save_user_contribution_rights(user_contribution_rights: user_domain.
    UserContributionRights) ->None:
    ...
    user_models.UserContributionRightsModel(id=user_contribution_rights.id,
        can_review_translation_for_language_codes=user_contribution_rights.
        can_review_translation_for_language_codes,
        can_review_voiceover_for_language_codes=user_contribution_rights.
        can_review_voiceover_for_language_codes, can_review_questions=
        user_contribution_rights.can_review_questions, can_submit_questions
        =user_contribution_rights.can_submit_questions).put()


def _update_user_contribution_rights(user_contribution_rights: user_domain.
    UserContributionRights) ->None:
...
def _update_reviewer_counts_in_community_contribution_stats_transactional(
    ...
    future_languages_that_reviewer_can_review = set(
        ...
        can_review_translation_for_language_codes)
    past_languages_that_reviewer_can_review = set(past_user_contribution_rights
        .can_review_translation_for_language_codes)
...
def _update_reviewer_counts_in_community_contribution_stats(
    user_contribution_rights: user_domain.UserContributionRights) ->None:
    """Updates the reviewer counts in the community contribution stats based
...
def get_user_actions_info(user_id: Optional[str]
    ...
    """
    roles = get_user_roles_from_id(user_id) if user_id else [feconf.
        ROLE_ID_GUEST]
...
def convert_to_user_settings_model(user_settings: user_domain.UserSettings
...
def _get_user_settings_from_model(user_settings_model: user_models.
    UserSettingsModel) ->user_domain.UserSettings:
    ...
    """
    return user_domain.UserSettings(user_id=user_settings_model.id, email=
        user_settings_model.email, roles=user_settings_model.roles, banned=
        user_settings_model.banned, username=user_settings_model.username,
        last_agreed_to_terms=user_settings_model.last_agreed_to_terms,
        last_started_state_editor_tutorial=user_settings_model.
        last_started_state_editor_tutorial,
        last_started_state_translation_tutorial=user_settings_model.
        last_started_state_translation_tutorial, last_logged_in=
        user_settings_model.last_logged_in, last_edited_an_exploration=
        user_settings_model.last_edited_an_exploration,
        last_created_an_exploration=user_settings_model.
        last_created_an_exploration, default_dashboard=user_settings_model.
        default_dashboard, creator_dashboard_display_pref=
        user_settings_model.creator_dashboard_display_pref, user_bio=
        user_settings_model.user_bio, subject_interests=user_settings_model
        .subject_interests, first_contribution_msec=user_settings_model.
        first_contribution_msec, preferred_language_codes=
        user_settings_model.preferred_language_codes,
        preferred_site_language_code=user_settings_model.
        preferred_site_language_code, preferred_audio_language_code=
        user_settings_model.preferred_audio_language_code,
        preferred_translation_language_code=user_settings_model.
        preferred_translation_language_code, pin=user_settings_model.pin,
        display_alias=user_settings_model.display_alias, deleted=
        user_settings_model.deleted, created_on=user_settings_model.
        created_on, has_viewed_lesson_info_modal_once=user_settings_model.
...
def has_fully_registered_account(user_id: str) ->bool:
    ...
    if user_settings is None:
        return False
    return bool(user_settings.username and user_settings.
        last_agreed_to_terms and user_settings.last_agreed_to_terms >=
        feconf.TERMS_PAGE_LAST_UPDATED_UTC)


def get_all_profiles_auth_details_by_parent_user_id(parent_user_id: str
    ...
    return [auth_services.get_user_auth_details_from_model(model) for model in
        auth_services.get_all_profiles_by_parent_user_id(parent_user_id) if
        not model.deleted]
...
def create_new_user(auth_id: str, email: str) ->user_domain.UserSettings:
    ...
    user_settings = user_domain.UserSettings(user_id, email, [feconf.
        ROLE_ID_FULL_USER], False, False, preferred_language_codes=[
...
@transaction_services.run_in_transaction_wrapper
def _create_new_user_transactional(auth_id: str, user_settings: user_domain
    .UserSettings) ->None:
    ...
    user_contributions = get_or_create_new_user_contributions(user_settings
        .user_id)
    save_user_contributions(user_contributions)
    auth_services.associate_auth_id_with_user_id(auth_domain.
        AuthIdUserIdPair(auth_id, user_settings.user_id))


def create_new_profiles(auth_id: str, email: str, modifiable_user_data_list:
    List[user_domain.ModifiableUserData]) ->List[user_domain.UserSettings]:
    ...
    for modifiable_user_data in modifiable_user_data_list:
        ...
        user_id = user_models.UserSettingsModel.get_new_id()
        user_settings = user_domain.UserSettings(user_id, email, [feconf.
            ROLE_ID_MOBILE_LEARNER], False, False, preferred_language_codes
            =[constants.DEFAULT_LANGUAGE_CODE], pin=modifiable_user_data.pin)
...
@transaction_services.run_in_transaction_wrapper
def _create_new_profile_transactional(user_settings: user_domain.
    UserSettings, user_auth_details: auth_domain.UserAuthDetails) ->None:
    """Save user models for new users as a transaction.
...
def update_multiple_users_data(modifiable_user_data_list: List[user_domain.
...
def _save_existing_users_settings(user_settings_list: List[user_domain.
    UserSettings]) ->None:
...
def _save_existing_users_auth_details(user_auth_details_list: List[
    auth_domain.UserAuthDetails]) ->None:
...
def _save_user_auth_details(user_auth_details: auth_domain.UserAuthDetails
    ) ->None:
...
def get_multiple_user_auth_details(user_ids: List[Optional[str]]) ->List[
    ...
    return [auth_services.get_user_auth_details_from_model(model) for model in
        user_settings_models if model is not None]


@overload
def get_auth_details_by_user_id(user_id: str, *, strict: Literal[True]
    ) ->auth_domain.UserAuthDetails:
    ...


@overload
def get_auth_details_by_user_id(user_id: str) ->Optional[auth_domain.
    UserAuthDetails]:
    ...


@overload
def get_auth_details_by_user_id(user_id: str, *, strict: Literal[False]
    ) ->Optional[auth_domain.UserAuthDetails]:
    ...


def get_auth_details_by_user_id(user_id: str, strict: bool=False) ->Optional[
    auth_domain.UserAuthDetails]:
...
@overload
def get_usernames(user_ids: List[str], *, strict: Literal[False]) ->Sequence[
    Optional[str]]:
    ...


def get_usernames(user_ids: List[str], strict: bool=False) ->Sequence[Optional
    [str]]:
    ...
    non_system_users_settings = get_users_settings(non_system_user_ids,
        strict=strict, include_marked_deleted=True)
...
def update_profile_picture_data_url(username: str, profile_picture_data_url:
    str) ->None:
    ...
    png_binary = utils.convert_data_url_to_binary(profile_picture_data_url,
        'png')
...
def add_user_role(user_id: str, role: str) ->None:
    ...
    role_services.log_role_query(user_id, feconf.ROLE_ACTION_ADD, role=role,
...
def remove_user_role(user_id: str, role: str) ->None:
    ...
    role_services.log_role_query(user_id, feconf.ROLE_ACTION_REMOVE, role=
        role, username=user_settings.username)
    save_user_settings(user_settings)
...
def save_deleted_username(normalized_username: str) ->None:
    ...
    Args:
        normalized_username: str. Normalized version of the username to be
            saved.
    """
    hashed_normalized_username = utils.convert_to_hash(normalized_username,
        user_models.DeletedUsernameModel.ID_LENGTH)
    deleted_user_model = user_models.DeletedUsernameModel(id=
        hashed_normalized_username)
    deleted_user_model.update_timestamps()
    deleted_user_model.put()


def get_human_readable_user_ids(user_ids: List[str], strict: bool=True) ->List[
    str]:
    ...
    for ind, user_settings in enumerate(users_settings):
        if user_settings is None:
            ...
            usernames.append('[Awaiting user registration: %s]' %
...
def add_user_to_mailing_list(email: str, tag: str, name: Optional[str]=None
    ) ->bool:
    ...
    return bulk_email_services.add_or_update_user_status(email,
        merge_fields, tag, can_receive_email_updates=True)


def update_email_preferences(user_id: str, can_receive_email_updates: bool,
    can_receive_editor_role_email: bool, can_receive_feedback_email: bool,
    can_receive_subscription_email: bool, bulk_email_db_already_updated:
    bool=False) ->bool:
    ...
    """
    email_preferences_model = user_models.UserEmailPreferencesModel.get(user_id
        , strict=False)
    if email_preferences_model is None:
        email_preferences_model = user_models.UserEmailPreferencesModel(id=
            user_id)
    ...
    if not bulk_email_db_already_updated and server_can_send_emails:
        user_creation_successful = (bulk_email_services.
            add_or_update_user_status(email, {}, 'Account',
...
def get_email_preferences(user_id: str) ->user_domain.UserGlobalPrefs:
    ...
    if email_preferences_model is None:
        ...
        return user_domain.UserGlobalPrefs(email_preferences_model.
            site_updates, email_preferences_model.editor_role_notifications,
            email_preferences_model.feedback_message_notifications,
            email_preferences_model.subscription_notifications)


def get_users_email_preferences(user_ids: List[str]) ->List[user_domain.
    UserGlobalPrefs]:
    ...
    """
    user_email_preferences_models = (user_models.UserEmailPreferencesModel.
        get_multi(user_ids))
    result = []
...
def set_email_preferences_for_exploration(user_id: str, exploration_id: str,
    mute_feedback_notifications: Optional[bool]=None,
    mute_suggestion_notifications: Optional[bool]=None) ->None:
    ...
    """
    exploration_user_model = user_models.ExplorationUserDataModel.get(user_id,
        exploration_id)
...
def get_email_preferences_for_exploration(user_id: str, exploration_id: str
    ...
    if exploration_user_model is None:
        ...
        return user_domain.UserExplorationPrefs(exploration_user_model.
            mute_feedback_notifications, exploration_user_model.
            mute_suggestion_notifications)


def get_users_email_preferences_for_exploration(user_ids: List[str],
    exploration_id: str) ->List[user_domain.UserExplorationPrefs]:
    """Gives mute preferences for exploration with given exploration_id of user
    with given user_id.
    ...
    Returns:
        ...
        receive email updates for particular exploration.
    """
    user_id_exp_id_combinations = list(itertools.product(user_ids, [
        exploration_id]))
...
@overload
def get_user_contributions(user_id: str, *, strict: Literal[True]
    ) ->user_domain.UserContributions:
    ...


@overload
def get_user_contributions(user_id: str) ->Optional[user_domain.
    UserContributions]:
    ...


@overload
def get_user_contributions(user_id: str, *, strict: Literal[False]) ->Optional[
    user_domain.UserContributions]:
    ...


def get_user_contributions(user_id: str, strict: bool=False) ->Optional[
    user_domain.UserContributions]:
    ...
    if model is None:
        return None
    result = user_domain.UserContributions(model.id, model.
        created_exploration_ids, model.edited_exploration_ids)
    return result


def get_or_create_new_user_contributions(user_id: str
    ) ->user_domain.UserContributions:
    """Gets domain object representing the contributions for the given user_id.
...
def save_user_contributions(user_contributions: user_domain.UserContributions
    ) ->None:
...
def update_user_contributions(user_id: str, created_exploration_ids: List[
    str], edited_exploration_ids: List[str]) ->None:
...
def get_validated_user_contributions_model(user_contributions: user_domain.
    UserContributions) ->user_models.UserContributionsModel:
    ...
    return user_models.UserContributionsModel(id=user_contributions.user_id,
...
def migrate_dashboard_stats_to_latest_schema(versioned_dashboard_stats:
    user_models.UserStatsModel) ->None:
    ...
    if (not 1 <= stats_schema_version <= feconf.
        CURRENT_DASHBOARD_STATS_SCHEMA_VERSION):
        raise Exception(
...
def parse_date_from_string(datetime_str: str) ->Dict[str, int]:
    ...
    """
    datetime_obj = datetime.datetime.strptime(datetime_str, feconf.
        DASHBOARD_STATS_DATETIME_STRING_FORMAT)
    return {'year': datetime_obj.year, 'month': datetime_obj.month, 'day':
        datetime_obj.day}
...
def get_weekly_dashboard_stats(user_id: str) ->List[Dict[str,
    DashboardStatsDict]]:
...
def get_last_week_dashboard_stats(user_id: str) ->Optional[Dict[str,
    DashboardStatsDict]]:
...
def update_dashboard_stats_log(user_id: str) ->None:
    ...
    weekly_dashboard_stats = {get_current_date_as_string(): {'num_ratings':
        model.num_ratings or 0, 'average_ratings': model.average_ratings,
...
def can_review_translation_suggestions(user_id: str, language_code:
    ...
    reviewable_language_codes = (user_contribution_rights.
        can_review_translation_for_language_codes)
...
def allow_user_to_review_translation_in_language(user_id: str,
    ...
    """
    user_contribution_rights = get_user_contribution_rights(user_id)
    allowed_language_codes = set(user_contribution_rights.
        can_review_translation_for_language_codes)
    if language_code is not None:
...
def remove_translation_review_rights_in_language(user_id: str,
    language_code_to_remove: str) ->None:
    ...
    user_contribution_rights = get_user_contribution_rights(user_id)
    user_contribution_rights.can_review_translation_for_language_codes = [
        lang_code for lang_code in user_contribution_rights.
        can_review_translation_for_language_codes if lang_code !=
        language_code_to_remove]
    _update_user_contribution_rights(user_contribution_rights)


def allow_user_to_review_voiceover_in_language(user_id: str, language_code: str
    ) ->None:
...
def remove_voiceover_review_rights_in_language(user_id: str, language_code: str
    ) ->None:
...
def remove_contribution_reviewer(user_id: str) ->None:
    ...
    Args:
        user_id: str. The unique ID of the user.
    """
    user_contribution_rights_model = (user_models.
        UserContributionRightsModel.get_by_id(user_id))
    if user_contribution_rights_model is not None:
        user_contribution_rights = _create_user_contribution_rights_from_model(
            user_contribution_rights_model)
        user_contribution_rights.can_review_questions = False
...
def get_contributor_usernames(category: str, language_code: Optional[str]=None
    ) ->Sequence[str]:
    ...
    if category == constants.CD_USER_RIGHTS_CATEGORY_REVIEW_TRANSLATION:
        ...
        user_ids = (user_models.UserContributionRightsModel.
...
def log_username_change(committer_id: str, old_username: str, new_username: str
    ) ->None:
    ...
    audit_models.UsernameChangeAuditModel(id=model_id, committer_id=
        committer_id, old_username=old_username, new_username=new_username
        ).put()
...
def unmark_user_banned(user_id: str) ->None:
    ...
    user_auth_details = auth_services.get_user_auth_details_from_model(
        auth_models.UserAuthDetailsModel.get(user_id))
    user_settings = get_user_settings(user_id)
    user_settings.unmark_banned(feconf.ROLE_ID_FULL_USER if
        user_auth_details.is_full_user() else feconf.ROLE_ID_MOBILE_LEARNER)
    save_user_settings(user_settings)
...
def get_dashboard_stats(user_id: str) ->DashboardStatsDict:
    ...
    return {'total_plays': total_plays, 'num_ratings': num_ratings,
        'average_ratings': average_ratings}


def get_checkpoints_in_order(init_state_name: str, states: Dict[str,
    state_domain.State]) ->List[str]:
    ...
    while len(queue) > 0:
        ...
        if current_state_name not in visited_state_names:
            ...
            if (current_state.card_is_checkpoint and current_state_name not in
                checkpoint_state_names):
...
def update_learner_checkpoint_progress(user_id: str, exploration_id: str,
    state_name: str, exp_version: int) ->None:
    ...
    Args:
        ...
        exp_version: int. The exploration version of the most recently reached
            checkpoint.
    """
    exp_user_model = user_models.ExplorationUserDataModel.get(user_id,
        exploration_id)
    if exp_user_model is None:
        exp_user_model = user_models.ExplorationUserDataModel.create(user_id,
            exploration_id)
    ...
    if exp_user_model.furthest_reached_checkpoint_state_name is None:
        ...
        furthest_reached_checkpoint_exp = exp_fetchers.get_exploration_by_id(
            exploration_id, strict=True, version=exp_user_model.
...
def clear_learner_checkpoint_progress(user_id: str, exploration_id: str
    ) ->None:
...
@overload
def sync_logged_in_learner_checkpoint_progress_with_current_exp_version(user_id
    : str, exploration_id: str) ->Optional[user_domain.ExplorationUserData]:
    ...


@overload
def sync_logged_in_learner_checkpoint_progress_with_current_exp_version(user_id
    : str, exploration_id: str, *, strict: Literal[True]
...
@overload
def sync_logged_in_learner_checkpoint_progress_with_current_exp_version(user_id
    : str, exploration_id: str, *, strict: Literal[False]) ->Optional[
    user_domain.ExplorationUserData]:
    ...


def sync_logged_in_learner_checkpoint_progress_with_current_exp_version(user_id
    : str, exploration_id: str, strict: bool=False) ->Optional[user_domain.
    ExplorationUserData]:
    ...
    most_recently_reached_checkpoint_in_current_exploration = (
        get_most_distant_reached_checkpoint_in_current_exploration(
        get_checkpoints_in_order(latest_exploration.init_state_name,
        latest_exploration.states), get_checkpoints_in_order(
        most_recently_interacted_exploration.init_state_name,
        most_recently_interacted_exploration.states), exp_user_model.
    ...
    furthest_reached_checkpoint_in_current_exploration = (
        ...
        furthest_reached_exploration.states), exp_user_model.
...
def assign_coordinator(committer: user_domain.UserActionsInfo, assignee:
    user_domain.UserActionsInfo, language_id: str) ->None:
    ...
    if language_rights is None:
        model = suggestion_models.TranslationCoordinatorsModel(id=
            language_id, coordinator_ids=[assignee.user_id],
...
def deassign_coordinator(committer: user_domain.UserActionsInfo, assignee:
    user_domain.UserActionsInfo, language_id: str) ->None:
...
def get_translation_rights_from_model(translation_coordinator_model:
    ...
    return user_domain.TranslationCoordinatorStats(
        translation_coordinator_model.id, translation_coordinator_model.
        coordinator_ids, translation_coordinator_model.coordinators_count)


def get_translation_rights_with_user(user_id: str) ->List[user_domain.
    ...
    """
    translation_coordinator_models: Sequence[suggestion_models.
        TranslationCoordinatorsModel
        ] = suggestion_models.TranslationCoordinatorsModel.get_by_user(user_id)
    return [get_translation_rights_from_model(model) for model in
        translation_coordinator_models if model is not None]


def deassign_user_from_all_languages(committer: user_domain.UserActionsInfo,
    user_id: str) ->None:
    ...
    for translation_rights in translation_rights_list:
        ...
        language_rights = suggestion_models.TranslationCoordinatorsModel(id
            =translation_rights.language_id, coordinator_ids=
            translation_rights.coordinator_ids, coordinators_count=
            translation_rights.coordinators_count)
        suggestion_models.TranslationCoordinatorsModel.update_timestamps(
            language_rights, update_last_updated_time=True)
        suggestion_models.TranslationCoordinatorsModel.put(language_rights)
...
def check_user_is_coordinator(user_id: str, language_id: str) ->bool:
    ...
    """
    model = suggestion_models.TranslationCoordinatorsModel.get(language_id,
        strict=False)
    if model is None:
        return False
