...
def get_users_contribution_rights(
    ...
    """
    user_contribution_rights_models = user_models.UserContributionRightsModel.get_multi(
        user_ids
    )
...
def get_all_profiles_auth_details_by_parent_user_id(
    ...
    """
    if (
        auth_models.UserAuthDetailsModel.has_reference_to_user_id(parent_user_id)
        is False
    ):
...
def update_email_preferences(
    ...
    if not bulk_email_db_already_updated and server_can_send_emails:
        user_creation_successful = bulk_email_services.add_or_update_user_status(
            email, {}, 'Account', can_receive_email_updates=can_receive_email_updates
        )
...
def get_users_email_preferences(
    ...
    """
    user_email_preferences_models = user_models.UserEmailPreferencesModel.get_multi(
        user_ids
    )
    result = []
...
def remove_contribution_reviewer(user_id: str) -> None:
    ...
    Args:
        user_id: str. The unique ID of the user.
    """
    user_contribution_rights_model = user_models.UserContributionRightsModel.get_by_id(
        user_id
    )
    if user_contribution_rights_model is not None:
        user_contribution_rights = _create_user_contribution_rights_from_model(
...
def get_contributor_usernames(
    ...
    if category == constants.CD_USER_RIGHTS_CATEGORY_REVIEW_TRANSLATION:
        ...
        user_ids = (
            user_models.UserContributionRightsModel.get_translation_reviewer_user_ids(
                language_code
            )
        )
