...
SYSTEM_USERS = {
    constants.SYSTEM_COMMITTER_ID: constants.SYSTEM_COMMITTER_ID,
    feconf.MIGRATION_BOT_USER_ID: constants.MIGRATION_BOT_USERNAME
}
    ...
    def require_valid_username(cls, username):
        ...
        if not username:
            ...
            raise utils.ValidationError(
              ...
              constants.MIGRATION_BOT_USERNAME in username.lower().strip()):
...
def get_users_settings(user_ids):
    ...
    for ind, model in enumerate(user_settings_models):
        if user_ids[ind] == constants.SYSTEM_COMMITTER_ID:
...
def get_system_user():
    ...
    """
    system_user = UserActionsInfo(constants.SYSTEM_COMMITTER_ID)
    return system_user
...
def get_human_readable_user_ids(user_ids):
    ...
    for ind, user_settings in enumerate(users_settings):
        if user_settings is None:
            ...
            raise Exception('User not found.')
        elif user_settings.user_id == constants.SYSTEM_COMMITTER_ID:
