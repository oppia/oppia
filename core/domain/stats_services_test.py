...
class StatisticsServicesTests(test_utils.GenericTestBase):
    ...
    def test_update_stats_method(self) -> None:
        ...
        self.assertEqual(
            exploration_stats.state_stats_mapping['Home'].total_answers_count_v2, 1
        )
        self.assertEqual(
        ...
        self.assertEqual(
            exploration_stats.state_stats_mapping['🙂'].total_hit_count_v2, 1
        )
        self.assertEqual(
            exploration_stats.state_stats_mapping['🙂'].first_hit_count_v2, 1
        )
        self.assertEqual(
            exploration_stats.state_stats_mapping['🙂'].total_answers_count_v2, 1
        )
        self.assertEqual(
            exploration_stats.state_stats_mapping['🙂'].useful_feedback_count_v2, 1
        )
        self.assertEqual(
            exploration_stats.state_stats_mapping['🙂'].num_completions_v2, 1
        )
        self.assertEqual(
            exploration_stats.state_stats_mapping['🙂'].num_times_solution_viewed_v2, 1
        )
    ...
    def test_get_stats_for_new_exp_version(self) -> None:
        ...
        self.assertEqual(
            ...
            set(['Home', 'New state 2', 'End', 'New state'])
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            set(['Home', 'End', 'Renamed state', 'New state'])
        )
        ...
        self.assertEqual(
            exp_stats.state_stats_mapping['Renamed state'].first_hit_count_v2, 2
        )
        self.assertEqual(
            exp_stats.state_stats_mapping['End'].useful_feedback_count_v2, 4
        )
        ...
        self.assertEqual(
            exp_stats.state_stats_mapping['New state 4'].total_answers_count_v2, 0
        )
...
class ExplorationIssuesTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.exp = self.save_new_linear_exp_with_state_names_and_interactions(
            'exp_id', self.owner_id, ['A', 'B'], ['TextInput', 'EndExploration']
        )
    ...
    def _create_mis_playthrough(
        self, state_name: str, num_times_answered_incorrectly: int
    ) -> str:
    ...
    def _create_mis_exp_issue(
        ...
        is_valid = True
        return stats_domain.ExplorationIssue(
            'MultipleIncorrectSubmissions', issue_customization_args, playthrough_ids,
            stats_models.CURRENT_ISSUE_SCHEMA_VERSION, is_valid
        )
    ...
    def test_save_exp_issues_model(self) -> None:
        ...
        self.assertEqual(
            exp_issues.unresolved_issues[0].playthrough_ids, eq_playthrough_ids
        )
        self.assertEqual(
            exp_issues.unresolved_issues[1].playthrough_ids, mis_playthrough_ids
        )
        self.assertEqual(
            exp_issues.unresolved_issues[2].playthrough_ids, cst_playthrough_ids
        )
    ...
    def test_cst_exp_issue_is_updated_when_state_is_renamed(self) -> None:
        ...
        self.assertEqual(
            exp_issue.issue_customization_args['state_names']['value'], ['Z', 'B', 'Z']
        )
        ...
        self.assertEqual(
            actions[2]['action_customization_args']['dest_state_name']['value'], 'Z'
        )
        self.assertEqual(
    ...
    def test_eq_exp_issue_is_updated_when_state_is_renamed(self) -> None:
        stats_services.save_exp_issues_model(
            stats_domain.ExplorationIssues(
                self.exp.id, self.exp.version,
                [self._create_eq_exp_issue([self._create_eq_playthrough('A')], 'A')]
            )
        )
...
class AnswerEventTests(test_utils.GenericTestBase):
    ...
    def test_record_answer(self) -> None:
        ...
        event_services.AnswerSubmissionEventHandler.record(
            ...
            exp_domain.EXPLICIT_CLASSIFICATION, 'sid2', self.TIME_SPENT, self.PARAMS,
            'answer1'
        )
        # Answer is a dict.
        event_services.AnswerSubmissionEventHandler.record(
        ...
        event_services.AnswerSubmissionEventHandler.record(
            ...
            exp_domain.EXPLICIT_CLASSIFICATION, 'sid1', self.TIME_SPENT, self.PARAMS, 10
        )
        ...
        event_services.AnswerSubmissionEventHandler.record(
            ...
            exp_domain.EXPLICIT_CLASSIFICATION, 'sid3', self.TIME_SPENT, self.PARAMS,
            [2, 4, 8]
        )
        # Answer is a unicode string.
        event_services.AnswerSubmissionEventHandler.record(
            'eid', exp_version, second_state_name, 'TextInput', 1, 1,
            exp_domain.EXPLICIT_CLASSIFICATION, 'sid4', self.TIME_SPENT, self.PARAMS,
            self.UNICODE_TEST_STRING
        )
        # Answer is None (such as for Continue).
        event_services.AnswerSubmissionEventHandler.record(
            'eid', exp_version, third_state_name, 'Continue', 1, 1,
            exp_domain.EXPLICIT_CLASSIFICATION, 'sid5', self.TIME_SPENT, self.PARAMS,
            None
        )
...
class RecordAnswerTests(test_utils.GenericTestBase):
    ...
    def test_record_and_retrieve_single_answer(self) -> None:
        ...
        stats_services.record_answer(
            ...
            'TextInput',
            stats_domain.SubmittedAnswer(
                'some text', 'TextInput', 0, 1, exp_domain.EXPLICIT_CLASSIFICATION, {},
    ...
    def test_record_many_answers(self) -> None:
        ...
        submitted_answer_list = [
            stats_domain.SubmittedAnswer(
                'answer a', 'TextInput', 0, 1, exp_domain.EXPLICIT_CLASSIFICATION, {},
                'session_id_v', 10.0
            ),
            stats_domain.SubmittedAnswer(
                'answer ccc', 'TextInput', 1, 1, exp_domain.EXPLICIT_CLASSIFICATION, {},
                'session_id_v', 3.0
            ),
            stats_domain.SubmittedAnswer(
                'answer bbbbb', 'TextInput', 1, 0, exp_domain.EXPLICIT_CLASSIFICATION,
                {}, 'session_id_v', 7.5
            ),
        ]
    ...
    def test_record_many_answers_with_preexisting_entry(self) -> None:
        stats_services.record_answer(
            ...
            'TextInput',
            stats_domain.SubmittedAnswer(
                '1 answer', 'TextInput', 0, 0, exp_domain.EXPLICIT_CLASSIFICATION, {},
                'a_session_id_val', 1.0
            )
        )
        ...
        submitted_answer_list = [
            stats_domain.SubmittedAnswer(
                'answer aaa', 'TextInput', 0, 1, exp_domain.EXPLICIT_CLASSIFICATION, {},
                'session_id_v', 10.0
            ),
            stats_domain.SubmittedAnswer(
                'answer ccccc', 'TextInput', 1, 1, exp_domain.EXPLICIT_CLASSIFICATION,
                {}, 'session_id_v', 3.0
            ),
            stats_domain.SubmittedAnswer(
                'answer bbbbbbb', 'TextInput', 1, 0, exp_domain.EXPLICIT_CLASSIFICATION,
                {}, 'session_id_v', 7.5
            ),
        ]
        stats_services.record_answers(
...
class SampleAnswerTests(test_utils.GenericTestBase):
    ...
    def test_exactly_100_answers_returned_if_main_shard_has_100_answers(self) -> None:
        ...
        stats_services.record_answers(
            self.EXP_ID, self.exploration.version, self.exploration.init_state_name,
            'TextInput', submitted_answer_list * 100
        )
    ...
    def test_get_sample_answers_with_invalid_exp_id(self) -> None:
        sample_answers = stats_services.get_sample_answers(
            'invalid_exp_id', self.exploration.version, self.exploration.init_state_name
        )
...
class LearnerAnswerDetailsServicesTest(test_utils.GenericTestBase):
    ...
    def test_new_learner_answer_details_is_created(self) -> None:
        ...
        stats_services.record_learner_answer_info(
            feconf.ENTITY_TYPE_EXPLORATION, state_reference, interaction_id, answer,
            answer_details
        )
        learner_answer_details = stats_services.get_learner_answer_details(
            feconf.ENTITY_TYPE_EXPLORATION, state_reference
