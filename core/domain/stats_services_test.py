...
class StatisticsServicesTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.save_new_valid_exploration(self.exp_id, 'admin', title='Title 1', end_state_name='End')
    ...
    def test_update_stats_method(self) -> None:
        ...
        self.assertEqual(
            exploration_stats.state_stats_mapping['Home'].useful_feedback_count_v2,
            1,
        )
        ...
        self.assertEqual(
            exploration_stats.state_stats_mapping['🙂'].total_answers_count_v2,
            1,
        )
        self.assertEqual(
            exploration_stats.state_stats_mapping['🙂'].useful_feedback_count_v2,
            1,
        )
        self.assertEqual(exploration_stats.state_stats_mapping['🙂'].num_completions_v2, 1)
        self.assertEqual(
            exploration_stats.state_stats_mapping['🙂'].num_times_solution_viewed_v2,
            1,
        )
    ...
    def test_get_stats_for_new_exploration(self) -> None:
        ...
        exploration_stats_for_new_exploration = stats_services.get_stats_for_new_exploration(
            exploration.id,
            exploration.version,
    ...
    def test_get_stats_for_new_exp_version(self) -> None:
        ...
        exploration.version += 1
        change_list = [
            exp_domain.ExplorationChange({'cmd': 'delete_state', 'state_name': 'New state 4'}),
            exp_domain.ExplorationChange(
                {
                    'cmd': 'add_state',
                    'state_name': 'New state',
        ...
        self.assertEqual(
            exp_stats.state_stats_mapping['New state 4'].total_answers_count_v2,
            0,
        )

        # Test reverts.
        exploration.version += 1
        exploration_stats_for_new_exp_version = stats_services.get_stats_for_new_exp_version(
            exploration.id,
            exploration.version,
            list(exploration.states.keys()),
            None,
    ...
    def test_mis_exp_issue_is_invalidated_when_state_is_deleted(self) -> None:
        stats_services.save_exp_issues_model(
            stats_domain.ExplorationIssues(
                self.exp.id,
                self.exp.version,
                [self._create_mis_exp_issue([self._create_mis_playthrough('B', 2)], 'B', 2)],
            )
        )
    ...
    def test_mis_exp_issue_is_updated_when_state_is_renamed(self) -> None:
        stats_services.save_exp_issues_model(
            stats_domain.ExplorationIssues(
                self.exp.id,
                self.exp.version,
                [self._create_mis_exp_issue([self._create_mis_playthrough('A', 2)], 'A', 2)],
            )
        )
        ...
        stats_services.save_exp_issues_model(
            stats_domain.ExplorationIssues(
                ...
                [
                    ...
                    self._create_mis_exp_issue([self._create_mis_playthrough('B', 3)], 'B', 3),
                ],
            )
        )
...
class AnswerEventTests(test_utils.GenericTestBase):
    ...
    def test_record_answer(self) -> None:
        ...
        expected_submitted_answer_list3: List[Dict[str, Union[str, Optional[int], Dict[str, str], float]]] = [
            {
                'answer': None,
                'time_spent_in_sec': 5.0,
                'answer_group_index': 1,
...
class LearnerAnswerDetailsServicesTest(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.learner_answer_details_model_exploration = stats_models.LearnerAnswerDetailsModel.create_model_instance(
        ...
        self.learner_answer_details_model_question = stats_models.LearnerAnswerDetailsModel.create_model_instance(
