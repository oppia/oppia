...
class StatisticsServicesTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.exp_id = 'exp_id1'
        self.exp_version = 1
        self.stats_model_id = stats_models.ExplorationStatsModel.create(
    ...
    def test_get_stats_for_new_exploration(self) -> None:
        ...
        newly_created_exploration_stats = stats_services.get_exploration_stats_by_id(
    ...
    def test_get_stats_for_new_exp_version(self) -> None:
        ...
        exploration_stats_with_none = stats_services.get_exploration_stats_by_id(
            exploration.id, exploration.version
        )
        # Ruling out the possibility of None for mypy type checking.
        ...
        change_list = [
            exp_domain.ExplorationChange(
                {'cmd': 'delete_state', 'state_name': 'New state 4'}
            ),
            exp_domain.ExplorationChange(
                {
                    'cmd': 'add_state',
    ...
    def test_get_updated_exp_issues_models_for_new_exp_version(self) -> None:
        ...
        exploration_issues_model = stats_models.ExplorationIssuesModel.get(
            '%s.%s' % ('exp_id', 1)
        )
...
class ExplorationIssuesTests(test_utils.GenericTestBase):
    ...
    def _create_mis_playthrough(
        ...
        issue_customization_args: stats_domain.IssuesCustomizationArgsDictType = {
            'state_name': {'value': state_name},
            'num_times_answered_incorrectly': {'value': num_times_answered_incorrectly},
        }
        actions: List[stats_domain.LearnerActionDict] = [
            {
                'action_type': 'ExplorationStart',
    ...
    def test_delete_playthroughs_multi(self) -> None:
        ...
        self.assertEqual(
            stats_models.PlaythroughModel.get_multi(playthrough_ids), [None, None, None]
        )
    ...
    def test_cst_exp_issue_is_invalidated_when_state_is_deleted(self) -> None:
        ...
        exp_services.update_exploration(
            self.owner_id,
            self.exp.id,
            [exp_domain.ExplorationChange({'cmd': 'delete_state', 'state_name': 'B'})],
            'change',
        )
