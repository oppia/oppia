...
def get_exploration_opportunity_summary_from_model(
    ...
    return opportunity_domain.ExplorationOpportunitySummary(
        model.id, model.topic_id, model.topic_name, model.story_id, model.story_title,
        model.chapter_title, model.content_count,
        new_incomplete_translation_language_codes, model.translation_counts,
        model.language_codes_needing_voice_artists,
        model.language_codes_with_assigned_voice_artists, {}, False
    )
...
def create_exp_opportunity_summary(
    topic: topic_domain.Topic, story: story_domain.Story,
    ...
    exploration_opportunity_summary = (
        opportunity_domain.ExplorationOpportunitySummary(
            ...
            story_node.title, content_count, incomplete_translation_language_codes,
...
def update_exploration_opportunities(
    ...
    if old_story.title != new_story.title:
        ...
        for exp_id in unchanged_exp_ids:
            new_node = (
                new_story.story_contents.get_node_with_corresponding_exp_id(exp_id)
            )
            old_node = (
                old_story.story_contents.get_node_with_corresponding_exp_id(exp_id)
            )
...
def get_translation_opportunities(
    ...
    exp_opportunity_summary_models, cursor, more = (
        opportunity_models.ExplorationOpportunitySummaryModel.
        get_all_translation_opportunities(page_size, cursor, language_code, topic_name)
    )
