...
ALLOWED_SERVER_MODES: Final = [
    ServerMode.DEV.value, ServerMode.TEST.value, ServerMode.PROD.value
]
ALLOWED_FEATURE_STAGES: Final = [
    FeatureStages.DEV.value, FeatureStages.TEST.value, FeatureStages.PROD.value
]
...
class EvaluationContext:
    ...
    def __init__(
        self, platform_type: Optional[str], app_version: Optional[str],
        server_mode: ServerMode
    ) -> None:
    ...
    @classmethod
    def from_dict(
        cls, client_context_dict: ClientSideContextDict,
            ...
            for _, version in self._conditions:
                if not APP_VERSION_WITHOUT_HASH_REGEXP.match(version):
                    raise utils.ValidationError(
                        ...
                        'regexp %s.' % (version, APP_VERSION_WITHOUT_HASH_REGEXP)
                    )
...
class PlatformParameterRule:
    ...
    def to_dict(self) -> PlatformParameterRuleDict:
        ...
        """
        return {
            'filters': [filter_domain.to_dict() for filter_domain in self._filters],
...
class PlatformParameter:
    ...
    DATA_TYPE_PREDICATES_DICT: (Dict[str, Callable[[PlatformDataTypes], bool]]) = {
    ...
    def __init__(
        self, name: str, description: str, data_type: str,
        rules: List[PlatformParameterRule], rule_schema_version: int,
        default_value: PlatformDataTypes
    ) -> None:
        self._name = name
