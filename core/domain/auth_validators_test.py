...
class UserAuthDetailsModelValidatorTests(test_utils.AuditJobsTestBase):
    ...
    USER_NAME = 'username'
    PARENT_EMAIL = 'parentemail@example.com'
    PARENT_NAME = 'parentname'
    PROFILE_EMAIL = 'profileemail@example.com'
    PROFILE_NAME = 'profilename'

    def setUp(self):
        super(UserAuthDetailsModelValidatorTests, self).setUp()

        # Note: There will be a total of 4 UserSettingsModels (hence 4
        ...
        self.auth_id = self.get_auth_id_from_email(self.USER_EMAIL)
        self.auth_model_instance = (
            auth_models.UserAuthDetailsModel.get_by_id(self.user_id))

        self.signup(self.PARENT_EMAIL, self.PARENT_NAME)
        self.parent_user_id = self.get_user_id_from_email(self.PARENT_EMAIL)
        self.parent_auth_id = self.get_auth_id_from_email(self.PARENT_EMAIL)
        self.parent_auth_model_instance = (
            auth_models.UserAuthDetailsModel.get_by_id(self.parent_user_id))

        self.signup(self.PROFILE_EMAIL, self.PROFILE_NAME)
        self.profile_user_id = self.get_user_id_from_email(self.PROFILE_EMAIL)
        self.profile_auth_id = self.get_auth_id_from_email(self.PROFILE_EMAIL)
        self.profile_auth_model_instance = (
            auth_models.UserAuthDetailsModel.get_by_id(self.profile_user_id))
        self.profile_auth_model_instance.parent_user_id = self.parent_user_id
        self.profile_auth_model_instance.gae_id = None
        self.profile_auth_model_instance.update_timestamps(
            update_last_updated_time=False)
        self.profile_auth_model_instance.put()
    ...
    def test_audit_standard_operation_passes(self):
        expected_output = [
            u'[u\'fully-validated UserAuthDetailsModel\', 4]']
        self.run_job_and_check_output(
            expected_output, sort=False, literal_eval=False)

    def test_audit_with_created_on_greater_than_last_updated_fails(self):
        self.auth_model_instance.created_on = (
            self.auth_model_instance.last_updated + datetime.timedelta(days=1))
        self.auth_model_instance.update_timestamps()
        self.auth_model_instance.put()
        expected_output = [(
            ...
            '%s of last_updated field\']]') % (
                self.user_id, self.auth_model_instance.created_on,
                self.auth_model_instance.last_updated
            ), u'[u\'fully-validated UserAuthDetailsModel\', 3]']
        self.run_job_and_check_output(
            expected_output, sort=True, literal_eval=False)

    def test_audit_with_last_updated_greater_than_current_time_fails(self):
        auth_models.UserAuthDetailsModel.get_by_id(
            self.get_user_id_from_email('tmpsuperadmin@example.com')).delete()
        auth_models.UserAuthDetailsModel.get_by_id(
            self.parent_user_id).delete()
        auth_models.UserAuthDetailsModel.get_by_id(
            self.profile_user_id).delete()
        expected_output = [(
            u'[u\'failed validation check for current time check of '
            'UserAuthDetailsModel\', '
            '[u\'Entity id %s: The last_updated field has a '
            'value %s which is greater than the time when the job was run\']]'
        ) % (self.user_id, self.auth_model_instance.last_updated)]
    ...
    def test_audit_with_missing_user_identifiers_model_fails(self):
        auth_models.UserIdentifiersModel.get_by_user_id(self.user_id).delete()
        expected_output = [
            (
                u'[u\'failed validation check for auth_ids '
                'field check of UserAuthDetailsModel\', '
                '[u"Entity id %s: based on '
                'field auth_ids having value '
    ...
    def test_audit_with_missing_parent_user_settings_model_fails(self):
        user_models.UserSettingsModel.get_by_id(self.parent_user_id).delete()
        expected_output = [
            (
                u'[u\'failed validation check for user_settings_ids '
                'field check of UserAuthDetailsModel\', '
                ...
                '%s, expected model UserSettingsModel '
                'with id %s but it doesn\'t exist"]]') % (
                    self.parent_user_id, self.parent_user_id,
                    self.parent_user_id),
            (
                u'[u\'failed validation check for parent_user_settings_ids '
                'field check of UserAuthDetailsModel\', '
                '[u"Entity id %s: based on '
                'field parent_user_settings_ids having value '
                '%s, expected model UserSettingsModel '
                'with id %s but it doesn\'t exist"]]') % (
                    self.profile_user_id, self.parent_user_id,
                    self.parent_user_id),
...
class UserIdentifiersModelValidatorTests(test_utils.AuditJobsTestBase):
    ...
    def setUp(self):
        ...
        # test_utils.AuditJobsTestBase.
        self.model_instance = (
            auth_models.UserIdentifiersModel.get_by_id(self.auth_id))
        self.job_class = (
    ...
    def test_audit_with_last_updated_greater_than_current_time_fails(self):
        auth_models.UserIdentifiersModel.get_by_id(
