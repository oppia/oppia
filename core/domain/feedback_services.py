...
def _create_models_for_thread_and_first_message(
    entity_type: str, entity_id: str, original_author_id: Optional[str], subject: str,
    text: str, has_suggestion: bool
) -> str:
    ...
    thread.put()
    create_message(
        thread_id, original_author_id, feedback_models.STATUS_CHOICES_OPEN, subject,
        text
    )
    return thread_id
...
def create_thread(
    ...
    return _create_models_for_thread_and_first_message(
        entity_type, entity_id, original_author_id, subject, text, has_suggestion
    )
...
def create_message(
    ...
    """
    return create_messages([thread_id],
                           author_id,
                           updated_status,
                           updated_subject,
                           text,
...
def create_messages(
    ...
    # creation.
    message_ids = (
        feedback_models.GeneralFeedbackMessageModel.get_message_counts(thread_ids)
    )
    ...
    for thread_id, message_id in zip(thread_ids, message_ids):
        message_identifiers.append(
            feedback_domain.FullyQualifiedMessageIdentifier(thread_id, message_id)
        )
    ...
    if (
        server_can_send_emails and feconf.CAN_SEND_TRANSACTIONAL_EMAILS and
        author_id is not None and user_services.is_user_registered(author_id) and
        message_changed and should_send_email
    ):
        for index, thread_model in enumerate(thread_models):
            _add_message_to_email_buffer(
                author_id, thread_model.id, message_ids[index], len(text),
                old_statuses[index], new_statuses[index]
            )
...
def update_messages_read_by_the_user(
    ...
    feedback_thread_user_model = (
        feedback_models.GeneralFeedbackThreadUserModel.get(user_id, thread_id) or
        feedback_models.GeneralFeedbackThreadUserModel.create(user_id, thread_id)
    )
...
def add_message_ids_to_read_by_list(
    ...
    current_feedback_thread_user_models_with_possible_nones = (
        feedback_models.GeneralFeedbackThreadUserModel.get_multi(user_id, thread_ids)
    )
...
def _get_message_from_model(
    ...
    return feedback_domain.FeedbackMessage(
        ...
        message_model.updated_subject, message_model.text, message_model.created_on,
        message_model.last_updated, message_model.received_via_email
    )
...
def get_messages(thread_id: str) -> List[feedback_domain.FeedbackMessage]:
    ...
    """
    return [
        _get_message_from_model(model) for model in
...
def get_thread_analytics_multi(
    ...
    return [
        ...
        ) for exp_id, model in zip(exploration_ids, feedback_thread_analytics_models)
    ]
...
def get_multiple_threads(thread_ids: List[str]) -> List[feedback_domain.FeedbackThread]:
    ...
    return [
        ...
        for model in feedback_models.GeneralFeedbackThreadModel.get_multi(thread_ids)
...
def _get_thread_from_model(
    ...
    message_count = (
        ...
        feedback_models.GeneralFeedbackMessageModel.get_message_count(thread_model.id)
    )
    return feedback_domain.FeedbackThread(
        thread_model.id, thread_model.entity_type, thread_model.entity_id, None,
        thread_model.original_author_id, thread_model.status, thread_model.subject,
...
def get_exp_thread_summaries(
    ...
    exp_thread_user_models, exploration_models = (
        cast(
            Tuple[List[Optional[feedback_models.GeneralFeedbackThreadUserModel]],
    ...
    for thread, last_two_message_models, thread_user_model, exp_model in (
        ...
        thread_summaries.append(
            feedback_domain.FeedbackThreadSummary(
                ...
                last_message_model.text, thread.message_count, last_message_is_read,
                second_last_message_is_read, author_last_message,
                author_second_last_message, exp_model.title, exp_model.id, thread.id
            )
        )
    return thread_summaries, number_of_unread_threads


def get_threads(entity_type: str,
                entity_id: str) -> List[feedback_domain.FeedbackThread]:
...
def get_closed_threads(entity_type: str, entity_id: str,
                       has_suggestion: bool) -> List[feedback_domain.FeedbackThread]:
    ...
    """
    return [
        thread for thread in get_threads(entity_type, entity_id) if (
...
def get_all_threads(entity_type: str, entity_id: str,
                    has_suggestion: bool) -> List[feedback_domain.FeedbackThread]:
...
def _enqueue_feedback_thread_status_change_email_task_transactional(
    user_id: str, reference: feedback_domain.FeedbackMessageReference, old_status: str,
    new_status: str
) -> None:
...
def get_feedback_message_references(
    ...
    feedback_message_references = (() if model is None else
                                   model.feedback_message_references)
    return [
        feedback_domain.FeedbackMessageReference(
            reference['entity_type'], reference['entity_id'], reference['thread_id'],
...
def clear_feedback_message_references_transactional(
    ...
    updated_references = [
        reference for reference in model.feedback_message_references if (
...
def _get_all_recipient_ids(exploration_id: str, thread_id: str,
                           author_id: str) -> Tuple[List[str], List[str]]:
...
def _send_batch_emails(
    ...
    exploration_id: str, has_suggestion: bool
) -> None:
...
def _add_message_to_email_buffer(
    author_id: str, thread_id: str, message_id: int, message_length: int,
    ...
    if old_status != new_status:
        ...
        _send_feedback_thread_status_change_emails(
            other_recipient_ids, feedback_message_reference, old_status, new_status,
...
def handle_thread_status_changed(exp_id: str, old_status: str, new_status: str) -> None:
    ...
    if (
        old_status != feedback_models.STATUS_CHOICES_OPEN and
