...
class InitTopicMetaTagContentOneOffJobTests(test_utils.GenericTestBase):

    ALBERT_EMAIL = 'albert@example.com'
    ALBERT_NAME = 'albert'

    def setUp(self):
        super(InitTopicMetaTagContentOneOffJobTests, self).setUp()
    ...
    def test_job_skips_deleted_topic(self):
        ...
        # Start migration job on sample topic.
        job_id = (
            topic_jobs_one_off.InitTopicMetaTagContentOneOffJob.create_new())
        topic_jobs_one_off.InitTopicMetaTagContentOneOffJob.enqueue(job_id)
        ...
        output = (
            topic_jobs_one_off.InitTopicMetaTagContentOneOffJob.get_output(
                job_id))
        expected = [[u'topic_deleted',
                     [u'Encountered 1 deleted topics.']]]
        self.assertEqual(expected, [ast.literal_eval(x) for x in output])
    ...
    def test_job_skips_meta_tag_content_initialisation_if_not_none(self):
        """Tests that the one off job skips topics for which the meta tag
        content field is not None.
        """
        topic_model = topic_models.TopicModel(
            id=self.TOPIC_ID,
            name='Topic name',
    ...
    def test_job_initialises_meta_tag_content_correctly(self):
        """Tests that the one off job initialises the meta tag content field
        for topics.
        """
        topic_model = topic_models.TopicModel(
            id=self.TOPIC_ID,
