...
class RemoveDeprecatedTopicModelFieldsOneOffJobTests(
    ...
    # RemoveDeprecatedTopicModelFieldsOneOffJob was called one time
    ...
    def test_one_exp_models_with_deprecated_field(self):
        with self.swap(
            topic_models,
            'TopicModel',
            MockTopicModelWithDeprecatedFields
        ):
            ...
            topic_models.TopicRightsModel(
                ...
                'manager_id', 'Created new topic rights',
                [{'cmd': topic_domain.CMD_CREATE_NEW}])

            topic_model.commit(
                'manager_id', commit_message, [{
                    'cmd': topic_domain.CMD_CREATE_NEW,
                    'name': 'Topic name'
                }])

            self.assertIsNotNone(topic_model.abbreviated_name)

            self.assertIn('abbreviated_name', topic_model._values) # pylint: disable=protected-access
            self.assertIn('abbreviated_name', topic_model._properties) # pylint: disable=protected-access

            output = self._run_one_off_job()
            self.assertItemsEqual(
                [['SUCCESS_REMOVED - TopicModel', 1]], output)
            ...
            self.assertNotIn(
                'abbreviated_name', migrated_topic_model._properties)  # pylint: disable=protected-access
    ...
    def test_one_exploration_model_without_deprecated_fields(self):
        ...
        self.assertNotIn(
            'abbreviated_name', topic_model._values)  # pylint: disable=protected-access
        self.assertNotIn(
            'abbreviated_name', topic_model._properties)  # pylint: disable=protected-access
