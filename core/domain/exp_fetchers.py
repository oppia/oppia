...
def get_multiple_versioned_exp_interaction_ids_mapping_by_version(
    ...
    for index, exploration_model in enumerate(exploration_models):
        ...
        for state_name in exploration_model.states:
            states_to_interaction_id_mapping[state_name] = exploration_model.states[
                state_name
            ]['interaction']['id']
...
def get_exploration_summary_from_model(
    ...
    return exp_domain.ExplorationSummary(
        ...
        exp_summary_model.title,
        exp_summary_model.category,
        exp_summary_model.objective,
        exp_summary_model.language_code,
        exp_summary_model.tags,
        exp_summary_model.ratings,
        exp_summary_model.scaled_average_rating,
        exp_summary_model.status,
        exp_summary_model.community_owned,
        exp_summary_model.owner_ids,
        exp_summary_model.editor_ids,
        exp_summary_model.voice_artist_ids,
...
@overload
def get_exploration_by_id(
    exploration_id: str, *, strict: Literal[False], version: Optional[int] = None
