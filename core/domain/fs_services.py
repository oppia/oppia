...
ALLOWED_ENTITY_NAMES: List[str] = [
    ...
    feconf.ENTITY_TYPE_TOPIC, feconf.ENTITY_TYPE_SKILL, feconf.ENTITY_TYPE_STORY,
    feconf.ENTITY_TYPE_QUESTION, feconf.ENTITY_TYPE_USER, feconf.ENTITY_TYPE_CLASSROOM
]
...
class GcsFileSystem(GeneralFileSystem):
    ...
    def commit(
        ...
        storage_services.commit(
            self._bucket_name, self._get_gcs_file_url(filepath), raw_bytes, mimetype
        )
    ...
    def listdir(self, dir_name: str) -> List[str]:
        ...
        if dir_name.startswith('/') or dir_name.endswith('/'):
            raise IOError(
                'The dir_name should not start with / or end with / : %s' % dir_name
            )
...
def save_original_and_compressed_versions_of_image(
    filename: str, entity_type: str, entity_id: str, original_image_content: bytes,
    filename_prefix: str, image_is_compressible: bool
) -> None:
    ...
    if not fs.isfile(compressed_image_filepath):
        fs.commit(
            compressed_image_filepath, compressed_image_content, mimetype=mimetype
        )
...
def copy_images(
    source_entity_type: str, source_entity_id: str, destination_entity_type: str,
    destination_entity_id: str, filenames: List[str]
) -> None:
...
def validate_and_save_image(
    raw_image: bytes, filename: str, filename_prefix: str, entity_type: str,
    entity_id: str
) -> None:
    ...
    save_original_and_compressed_versions_of_image(
        filename, entity_type, entity_id, raw_image, filename_prefix, is_compressible
    )
...
def get_static_asset_url(filepath: str) -> str:
    ...
    if constants.EMULATOR_MODE:
        ...
        # loop.
        return 'http://localhost:8181/assetsstatic/%s' % (filepath)
