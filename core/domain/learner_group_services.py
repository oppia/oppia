...
def get_matching_learner_group_syllabus_to_add(
    ...
    if category != constants.DEFAULT_ADD_SYLLABUS_FILTER:
        ...
        published_topics_summary = topic_fetchers.get_published_topic_summaries()
        matching_topics = []
        for topic_summary in published_topics_summary:
            try:
                topic = topic_fetchers.get_topic_by_id(topic_summary.id, strict=True)
                matching_topics.append(topic)
            except Exception as e:
                raise Exception('Topic with id %s could not be fetched: %s' % (topic_summary.id, e)) from e
...
def can_user_be_invited(user_id: str, username: str, group_id: str) -> Tuple[bool, str]:
    ...
    if user_id in learner_group_model.learner_user_ids:
        ...
        return (
            False,
            'User with username %s has been already invited to join the group' % username,
        )
...
def remove_study_guide_reference_from_learner_groups(topic_id: str, subtopic_id: int) -> None:
    ...
    learner_group_model_cls = learner_group_models.LearnerGroupModel
    found_models: Sequence[learner_group_models.LearnerGroupModel] = learner_group_model_cls.get_all().filter(datastore_services.any_of(learner_group_model_cls.subtopic_page_ids == study_guide_id)).fetch()
