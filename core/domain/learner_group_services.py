...
def get_matching_learner_group_syllabus_to_add(
    ...
    if category != constants.DEFAULT_ADD_SYLLABUS_FILTER:
        ...
        matching_topics: List[topic_domain.Topic] = topic_fetchers.get_topics_by_ids(
            matching_topic_ids, strict=True
        )
    else:
...
def get_matching_story_syllabus_item_dicts(
    topic: topic_domain.Topic, group_story_ids: List[str], keyword: Optional[str] = None
...
def remove_learners_from_learner_group(
    ...
    learner_grps_users_models = learner_group_fetchers.get_learner_group_models_by_ids(
