...
class VoiceoverAutogenerationPolicy:
    ...
    def validate(self) -> None:
        ...
        for language_code in self.language_codes_mapping:
            ...
            self.validate_language_code_mapping(
                language_code, language_code_mapping)
    ...
    def validate_language_code_mapping(
        ...
        Args:
            language_code: str. The language code that must match
                with the language_code_mapping's language code.
            language_code_mapping: dict(str, bool). The language_code_mapping 
                whose type and format needs to be validated.
        """
        ...
        for language_accent_code, is_auto_voiceover_enabled in (
            ...
            extracted_language_code = language_accent_code.split('-')[0]
            if language_code != extracted_language_code:
                raise utils.ValidationError(
                    'language_accent_code must be formatted as '
                    '{{language}}-{{accent}} where language must correspond to'
                    ' language_code, received %s' % extracted_language_code)
