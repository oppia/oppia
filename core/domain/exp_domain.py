...
import bs4
from typing import (
    Callable, Dict, Final, List, Literal, Mapping, Optional, Sequence, Set, Tuple,
...
MATH_INTERACTION_TYPES: Final = [
    TYPE_VALID_ALGEBRAIC_EXPRESSION, TYPE_VALID_NUMERIC_EXPRESSION,
    TYPE_VALID_MATH_EQUATION
]
ALGEBRAIC_MATH_INTERACTIONS: Final = [
    TYPE_VALID_ALGEBRAIC_EXPRESSION, TYPE_VALID_MATH_EQUATION
]
...
def clean_math_expression(math_expression: str) -> str:
    ...
    for trig_fn in trig_fns:
        math_expression = re.sub(
            r'%s(\^\d)\((.)\)' % trig_fn, r'(%s(\2))\1' % trig_fn, math_expression
        )
...
class ExplorationChange(change_domain.BaseChange):
    ...
    EXPLORATION_PROPERTIES: List[str] = [
        ...
        'author_notes', 'param_specs', 'param_changes', 'init_state_name',
        'auto_tts_enabled', 'next_content_id_index', 'edits_allowed'
    ]
    ...
    ALLOWED_COMMANDS: List[feconf.ValidCmdDict] = [
        ...
        },
        {
            ...
            'required_attribute_names': ['content_id', 'language_code', 'state_name'],
            'optional_attribute_names': [],
            'user_id_attribute_names': [],
            'allowed_values': {},
            'deprecated_values': {}
        ...
        },
        {
            ...
            'required_attribute_names': ['content_id', 'language_code', 'translation'],
            'optional_attribute_names': [],
            'user_id_attribute_names': [],
            'allowed_values': {},
            'deprecated_values': {}
        ...
        },
        {
            ...
            'required_attribute_names': ['property_name', 'state_name', 'new_value'],
            'optional_attribute_names': ['old_value'],
            'user_id_attribute_names': [],
            'allowed_values': {
                'property_name': STATE_PROPERTIES
            },
        ...
        },
        {
            ...
            'user_id_attribute_names': [],
            'allowed_values': {
                'property_name': EXPLORATION_PROPERTIES
            },
            'deprecated_values': {}
        },
        {
    ...
    DEPRECATED_COMMANDS: List[str] = [
        'clone', 'add_gadget', 'edit_gadget_property', 'delete_gadget', 'rename_gadget'
    ]
...
class TransientCheckpointUrl:
    ...
    def __init__(
        self, exploration_id: str, furthest_reached_checkpoint_state_name: str,
        furthest_reached_checkpoint_exp_version: int,
        most_recently_reached_checkpoint_state_name: str,
        most_recently_reached_checkpoint_exp_version: int
    ) -> None:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.most_recently_reached_checkpoint_state_name, str):
            raise utils.ValidationError(
                ...
                ' str, received %s' % self.most_recently_reached_checkpoint_state_name
            )
...
class ExplorationCommitLogEntry:
    ...
    def __init__(
        self, created_on: datetime.datetime, last_updated: datetime.datetime,
        user_id: str, exploration_id: str, commit_type: str, commit_message: str,
        commit_cmds: Sequence[Mapping[str, change_domain.AcceptableChangeDictTypes]],
        version: int, post_commit_status: str, post_commit_community_owned: bool,
        post_commit_is_private: bool
    ) -> None:
...
class Exploration(translation_domain.BaseTranslatableObject):
    ...
    def create_default_exploration(
        ...
        return cls(
            ...
            feconf.CURRENT_STATE_SCHEMA_VERSION, init_state_name, states_dict, {}, [],
            0, feconf.DEFAULT_AUTO_TTS_ENABLED,
    ...
    def from_dict(
        ...
        for (state_name, sdict) in exploration_dict['states'].items():
            ...
            solution = (
                ...
                if idict['solution'] is not None and idict['id'] is not None else None
            )
            ...
            state.interaction = state_domain.InteractionInstance(
                idict['id'], customization_args, interaction_answer_groups,
                default_outcome, idict['confirmed_unclassified_answers'],
                [state_domain.Hint.from_dict(h) for h in idict['hints']], solution
            )

            state.recorded_voiceovers = (
                state_domain.RecordedVoiceovers.from_dict(sdict['recorded_voiceovers'])
            )
    ...
    def validate(self, strict: bool = False) -> None:
        ...
        if not isinstance(self.objective, str):
            raise utils.ValidationError(
                'Expected objective to be a string, received %s' % self.objective
            )
        ...
        if not isinstance(self.author_notes, str):
            raise utils.ValidationError(
                'Expected author_notes to be a string, received %s' % self.author_notes
            )
        ...
        if not isinstance(self.param_specs, dict):
            raise utils.ValidationError(
                'Expected param_specs to be a dict, received %s' % self.param_specs
            )
        ...
        if not isinstance(self.param_changes, list):
            raise utils.ValidationError(
                'Expected param_changes to be a list, received %s' % self.param_changes
            )
        for param_change in self.param_changes:
            ...
            if param_change.name in constants.INVALID_PARAMETER_NAMES:
                raise utils.ValidationError(
                    ...
                    'reserved. Please choose a different name.' % param_change.name
                )
        ...
        for state_name, state in self.states.items():
            ...
            for group in interaction.answer_groups:
                ...
                if group.outcome.dest not in all_state_names:
                    raise utils.ValidationError(
                        'The destination %s is not a valid state.' % group.outcome.dest
                    )
    ...
    def get_content_html(self, state_name: str,
                         content_id: str) -> Union[str, List[str]]:
    ...
    def add_state(
        self, state_name: str, content_id_for_state_content: str,
        content_id_for_default_outcome: str
    ) -> None:
    ...
    def get_metadata(self) -> ExplorationMetadata:
        ...
        return ExplorationMetadata(
            self.title, self.category, self.objective, self.language_code, self.tags,
            self.blurb, self.author_notes, self.states_schema_version,
            self.init_state_name, self.param_specs, self.param_changes,
            self.auto_tts_enabled, self.edits_allowed
        )
    ...
    def _convert_states_v41_dict_to_v42_dict(
        ...
        def migrate_rule_inputs_and_answers(
            new_type: str, value: str, choices: List[state_domain.SubtitledHtmlDict]
        ) -> str:
            ...

        @overload
        def migrate_rule_inputs_and_answers(
            new_type: str, value: List[str],
        ...
        @overload
        def migrate_rule_inputs_and_answers(
            new_type: str, value: List[List[str]],
            choices: List[state_domain.SubtitledHtmlDict]
        ) -> List[List[str]]:
            ...
            ...
            if interaction_id == 'ItemSelectionInput':
                ...
                if solution is not None:
                    ...
                    solution['correct_answer'] = (
                        migrate_rule_inputs_and_answers(
                            'SetOfTranslatableHtmlContentIds', list_of_html_contents,
                            choices
                        )
                    )
                ...
                if solution is not None:
                    ...
                    solution['correct_answer'] = (
                        migrate_rule_inputs_and_answers(
                            ...
                            list_of_html_content_list, choices
                        )
                    )
                ...
                for rule_spec_dict in answer_group_dict['rule_specs']:
                    ...
                    if interaction_id == 'DragAndDropSortInput':
                        ...
                        if rule_type in rule_types_with_list_of_sets:
                            ...
                            rule_inputs['x'] = migrate_rule_inputs_and_answers(
                                'TranslatableHtmlContentId', rule_inputs['x'], choices
                            )
                            ...
                            for rule_input_name in ['x', 'y']:
                                ...
                                rule_inputs[rule_input_name] = (
                                    migrate_rule_inputs_and_answers(
                                        'TranslatableHtmlContentId', rule_input_value,
                                        choices
                                    )
                                )
    ...
    def _convert_states_v43_dict_to_v44_dict(
        cls, states_dict: Dict[str, state_domain.StateDict], init_state_name: str
    ) -> Dict[str, state_domain.StateDict]:
        ...
        for state_dict in states_dict.values():
            ...
            if interaction_customisation_args:
                state_domain.State.convert_html_fields_in_state(
                    state_dict,
                    html_validation_service.convert_svg_diagram_tags_to_image_tags, 46
                )
        return states_dict
    ...
    def _choices_should_be_unique_and_non_empty(
        ...
        *,
        ...
        for content_id in content_ids_of_choices_to_update:
            ...
            choice_voiceovers = state_dict['recorded_voiceovers']['voiceovers_mapping'][
                content_id]
    ...
    def _set_lower_and_upper_bounds(
        cls, range_var: RangeVariableDict, lower_bound: Optional[float],
        upper_bound: Optional[float], *, lb_inclusive: bool, ub_inclusive: bool
    ) -> None:
    ...
    def _should_check_range_criteria(
        cls, earlier_rule: state_domain.RuleSpecDict,
    ...
    def _fix_continue_interaction(
        ...
        if len(text_value) > 20:
            ...
            # Here we use MyPy ignore because the latest schema of state
            # dict doesn't contains written_translations property.
            continue_button_translations = state_dict[
    ...
    def _fix_end_interaction(cls, state_dict: state_domain.StateDict) -> None:
        ...
        # type.
        recc_exp_ids = cast(
            List[str], state_dict['interaction']['customization_args']
            ['recommendedExplorationIds']['value']
        )
        # Should be at most 3 recommended explorations.
        state_dict['interaction']['customization_args']['recommendedExplorationIds'][
    ...
    def _fix_fraction_input_interaction(
        ...
        rules_that_can_have_improper_fractions = [
            'IsExactlyEqualTo', 'HasFractionalPartExactlyEqualTo'
        ]
    ...
    def _fix_multiple_choice_input_interaction(
        ...
        cls._choices_should_be_unique_and_non_empty(
            choices, answer_groups, state_dict, is_item_selection_interaction=False
        )
    ...
    def _fix_item_selection_input_interaction(
        ...
        # type.
        min_value = cast(
            int, state_dict['interaction']['customization_args']
        ...
        cls._choices_should_be_unique_and_non_empty(
            choices, answer_groups, state_dict, is_item_selection_interaction=True
        )
        ...
        state_dict['interaction']['customization_args']['minAllowableSelectionCount'][
            'value'] = min_value
        state_dict['interaction']['customization_args']['maxAllowableSelectionCount'][
            'value'] = max_value
        state_dict['interaction']['customization_args']['choices']['value'] = choices
        state_dict['interaction']['answer_groups'] = answer_groups
    ...
    def _update_rule_value_having_empty_choices(
        cls, empty_choices: List[state_domain.SubtitledHtmlDict],
        rule_value_x: List[List[str]], solution: Optional[List[List[str]]]
    ) -> None:
    ...
    def _fix_text_input_interaction(
        ...
        rows_value = cast(
            int, state_dict['interaction']['customization_args']['rows']['value']
        )
    ...
    def _update_state_rte(
        ...
        for state in states_dict.values():
            ...
            if state['interaction']['default_outcome'] is not None:
                default_feedback = state['interaction']['default_outcome']['feedback'][
                    'html']
    ...
    def update_states_from_model(
        cls, versioned_exploration_states: VersionedExplorationStatesDict,
        current_states_schema_version: int, init_state_name: str, language_code: str
    ) -> Optional[int]:
    ...
    def _convert_v60_dict_to_v61_dict(
        ...
        exploration_dict['states'] = (
            cls._convert_states_v55_dict_to_v56_dict(exploration_dict['states'])
        )
    ...
    def _migrate_to_latest_yaml_version(
        ...
        try:
            ...
            raise utils.InvalidInputException(
                ...
                'a zip file. The YAML parser returned the following error: %s' % e
            )
    ...
    def validate(self) -> None:
        ...
        if valid_rating_keys != actual_rating_keys:
            raise utils.ValidationError(
                ...
                ((', ').join(valid_rating_keys), (', ').join(actual_rating_keys))
            )
        ...
        all_user_ids_with_rights = (
            self.owner_ids + self.editor_ids + self.voice_artist_ids + self.viewer_ids
        )
...
class ExplorationChangeMergeVerifier:
    ...
    PROPERTIES_CONFLICTING_INTERACTION_ID_CHANGES: List[str] = [
        STATE_PROPERTY_INTERACTION_CUST_ARGS, STATE_PROPERTY_INTERACTION_SOLUTION,
        STATE_PROPERTY_INTERACTION_ANSWER_GROUPS
    ]
    ...
    PROPERTIES_CONFLICTING_CUST_ARGS_CHANGES: List[str] = [
        STATE_PROPERTY_INTERACTION_SOLUTION, STATE_PROPERTY_RECORDED_VOICEOVERS,
        STATE_PROPERTY_INTERACTION_ANSWER_GROUPS
    ]
    ...
    PROPERTIES_CONFLICTING_SOLUTION_CHANGES: List[str] = [
        STATE_PROPERTY_INTERACTION_ANSWER_GROUPS, STATE_PROPERTY_RECORDED_VOICEOVERS,
        STATE_PROPERTY_INTERACTION_CUST_ARGS
    ]
    ...
    PROPERTIES_CONFLICTING_VOICEOVERS_CHANGES: List[str] = [
        STATE_PROPERTY_CONTENT, STATE_PROPERTY_INTERACTION_SOLUTION,
        STATE_PROPERTY_INTERACTION_HINTS, STATE_PROPERTY_INTERACTION_ANSWER_GROUPS,
        STATE_PROPERTY_INTERACTION_DEFAULT_OUTCOME, STATE_PROPERTY_INTERACTION_CUST_ARGS
    ]
    ...
    NON_CONFLICTING_PROPERTIES: List[str] = [
        STATE_PROPERTY_UNCLASSIFIED_ANSWERS, STATE_PROPERTY_LINKED_SKILL_ID,
        STATE_PROPERTY_INAPPLICABLE_SKILL_MISCONCEPTION_IDS,
        STATE_PROPERTY_CARD_IS_CHECKPOINT
    ]
    ...
    def is_change_list_mergeable(
        self, change_list: List[ExplorationChange],
        exp_at_change_list_version: Exploration, current_exploration: Exploration
    ) -> Tuple[bool, bool]:
...
class ExplorationMetadata:
    ...
    def __init__(
        self, title: str, category: str, objective: str, language_code: str,
        tags: List[str], blurb: str, author_notes: str, states_schema_version: int,
        init_state_name: str, param_specs: Dict[str, param_domain.ParamSpec],
        param_changes: List[param_domain.ParamChange], auto_tts_enabled: bool,
        edits_allowed: bool
    ) -> None:
...
class MetadataVersionHistory:
    ...
    def __init__(
        self, last_edited_version_number: Optional[int], last_edited_committer_id: str
    ):
...
class ExplorationVersionHistory:
    ...
    def __init__(
        ...
        metadata_last_edited_committer_id: str, committer_ids: List[str]
    ) -> None:
