...
class TransientCheckpointUrl:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.furthest_reached_checkpoint_state_name, str):
            raise utils.ValidationError(
                'Expected furthest_reached_checkpoint_state_name to be a str,received %s'
                % self.furthest_reached_checkpoint_state_name
            )
        ...
        if not isinstance(
            ...
            raise utils.ValidationError(
                'Expected most_recently_reached_checkpoint_state_name to be a str, received %s'
                % self.most_recently_reached_checkpoint_state_name
            )
...
class Exploration(translation_domain.BaseTranslatableObject):
        ...
        for state_name, sdict in exploration_dict['states'].items():
            ...
            for pc in state.param_changes:
                if pc.name not in exploration.param_specs:
                    raise Exception(
                        'Parameter %s was used in a state but not declared in the exploration param_specs.'
                        % pc.name
                    )
    ...
    def validate(self, strict: bool = False) -> None:
        ...
        for tag in self.tags:
            if not isinstance(tag, str):
                raise utils.ValidationError(
                    'Expected each tag in \'tags\' to be a string, received \'%s\''
                    % tag
                )
            ...
            for answer_group in state.interaction.answer_groups:
                ...
                if outcome.dest_if_really_stuck is not None:
                    if not isinstance(outcome.dest_if_really_stuck, str):
                        raise utils.ValidationError(
                            'Expected dest_if_really_stuck to be a string, received %s'
        ...
        if self.init_state_name not in self.states:
            raise utils.ValidationError(
                'There is no state in %s corresponding to the exploration\'s initial state name %s.'
        ...
        if not isinstance(self.next_content_id_index, int):
            raise utils.ValidationError(
                'Expected next_content_id_index to be an int, received %s'
        ...
        if not isinstance(self.edits_allowed, bool):
            raise utils.ValidationError(
                'Expected edits_allowed to be a bool, received %s'
                % self.edits_allowed
            )
        ...
        for param_change in self.param_changes:
            ...
            if param_change.name in constants.INVALID_PARAMETER_NAMES:
                raise utils.ValidationError(
                    'The exploration-level parameter with name \'%s\' is reserved. Please choose a different name.'
                    % param_change.name
                )
        ...
        for state_name, state in self.states.items():
            for param_change in state.param_changes:
                ...
                if param_change.name in constants.INVALID_PARAMETER_NAMES:
                    raise utils.ValidationError(
                        'The parameter name \'%s\' is reserved. Please choose a different name for the parameter being set in state \'%s\'.'
                        % (param_change.name, state_name)
                    )
                if param_change.name not in self.param_specs:
                    raise utils.ValidationError(
                        'The parameter with name \'%s\' was set in state \'%s\', but it does not exist in the list of parameter specifications for this exploration.'
                        % (param_change.name, state_name)
                    )
        ...
        for state_name, state in self.states.items():
            ...
            if default_outcome is not None:
                ...
                if (
                    ...
                    raise utils.ValidationError(
                        'The destination for the stuck learner %s is not a valid state.'
                ...
                if (
                    ...
                    raise utils.ValidationError(
                        'The default outcome for state %s has a refresher exploration ID, but is not a self-loop.'
                        % state_name
                    )
            ...
            for group in interaction.answer_groups:
                ...
                if (
                    ...
                    raise utils.ValidationError(
                        'The outcome for an answer group in state %s has a refresher exploration ID, but is not a self-loop.'
                        % state_name
                    )
                ...
                for param_change in group.outcome.param_changes:
                    if param_change.name not in self.param_specs:
                        raise utils.ValidationError(
                            'The parameter %s was used in an answer group, but it does not exist in this exploration'
                            % param_change.name
                        )
        ...
        if strict:
            ...
            if not self.states[self.init_state_name].card_is_checkpoint:
                raise utils.ValidationError(
                    'Expected card_is_checkpoint of first state to be True but found it to be %s'
                ...
                if interaction.is_terminal:
                    if state_name != self.init_state_name:
                        if state.card_is_checkpoint:
                            raise utils.ValidationError(
                                'Expected card_is_checkpoint of terminal state to be False but found it to be %s'
            ...
            if not 1 <= checkpoint_count <= 8:
                raise utils.ValidationError(
                    'Expected checkpoint count to be between 1 and 8 inclusive but found it to be %s'
                    % checkpoint_count
                )
            ...
            for state_name, state in self.states.items():
                ...
                if default_outcome is not None:
                    ...
                    if (
                        ...
                        raise utils.ValidationError(
                            'The default outcome for state %s is labelled correct but is a self-loop.'
                            % state_name
                        )
                ...
                for group in interaction.answer_groups:
                    ...
                    if (
                        ...
                        raise utils.ValidationError(
                            'The outcome for an answer group in state %s is labelled correct but is a self-loop.'
                            % state_name
                        )
            ...
            if len(warnings_list) > 0:
                ...
                raise utils.ValidationError(
                    'Please fix the following issues before saving this exploration: %s'
                    % warning_str
                )
    ...
    def _verify_all_states_reachable(self) -> None:
        ...
        if len(self.states) != len(processed_queue):
            ...
            raise utils.ValidationError(
                'The following states are not reachable from the initial state: %s'
                % ', '.join(unseen_states)
            )
    ...
    def _verify_no_dead_ends(self) -> None:
        ...
        if len(self.states) != len(processed_queue):
            ...
            raise utils.ValidationError(
                'It is impossible to complete the exploration from the following states: %s'
                % ', '.join(sorted_dead_end_states)
            )
    ...
    def update_init_state_name(self, init_state_name: str) -> None:
        ...
        if init_state_name not in self.states:
            raise Exception(
                'Invalid new initial state name: %s; it is not in the list of states %s for this exploration.'
                % (init_state_name, list(self.states.keys()))
            )
        self.init_state_name = init_state_name
    ...
    def validate(self) -> None:
        ...
        for owner_id in self.owner_ids:
            if not isinstance(owner_id, str):
                raise utils.ValidationError(
                    'Expected each id in owner_ids to be string, received %s'
                    % owner_id
                )
        ...
        for editor_id in self.editor_ids:
            if not isinstance(editor_id, str):
                raise utils.ValidationError(
                    'Expected each id in editor_ids to be string, received %s'
                    % editor_id
                )
        ...
        for voice_artist_id in self.voice_artist_ids:
            if not isinstance(voice_artist_id, str):
                raise utils.ValidationError(
                    'Expected each id in voice_artist_ids to be string, received %s'
                    % voice_artist_id
                )
        ...
        for viewer_id in self.viewer_ids:
            if not isinstance(viewer_id, str):
                raise utils.ValidationError(
                    'Expected each id in viewer_ids to be string, received %s'
                    % viewer_id
                )
        ...
        if len(all_user_ids_with_rights) != len(set(all_user_ids_with_rights)):
            raise utils.ValidationError(
                'Users should not be assigned to multiple roles at once, received users: %s'
                % ', '.join(all_user_ids_with_rights)
            )
        ...
        for contributor_id in self.contributor_ids:
            if not isinstance(contributor_id, str):
                raise utils.ValidationError(
                    'Expected each id in contributor_ids to be string, received %s'
                    % contributor_id
                )
