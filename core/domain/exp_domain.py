...
class Exploration(python_utils.OBJECT):
    ...
    def _convert_states_v47_dict_to_v48_dict(cls, exp_id, states_dict):
        ...
        def get_image_sizes_in_bytes_dict_from_html(html_string):
        ...
        for state_dict in states_dict.values():
            ...
            # For content.
            html_string = state_dict['content']['html']
            image_sizes_in_bytes = (
                get_image_sizes_in_bytes_dict_from_html(html_string))
            state_dict['content']['image_sizes_in_bytes'] = (
                image_sizes_in_bytes)

            # For written translations.
            written_translations = (
                state_dict['written_translations']['translations_mapping'])
            for content_id in written_translations:
                for language_code in written_translations[content_id]:
                    translation = (
                        written_translations[content_id]
                        [language_code]['translation'])
                    image_sizes_in_bytes = (
                        get_image_sizes_in_bytes_dict_from_html(
                            translation))
                    translation_mapping = written_translations[content_id]
                    written_translation = translation_mapping[language_code]
                    written_translation['image_sizes_in_bytes'] = (
                        image_sizes_in_bytes)

            # For interactions - answer groups, default outcomes.
            # customization args, hints and solution.
            interaction = states_dict['interaction']
            answer_groups = interaction['answer_groups']
            for answer_group in answer_groups:
                # Answer groups -> rule specs.
                # Answer groups -> outcomes.
                outcome = answer_group['outcome']
                # Outcome -> feedback.
                feedback = outcome['feedback']
                image_sizes_in_bytes = (
                    get_image_sizes_in_bytes_dict_from_html(feedback['html']))
                feedback['image_sizes_in_bytes'] = image_sizes_in_bytes
                # Outcome -> paramchange.

            # For interactions - default outcome.
            default_outcome = interaction['default_outcome']
            feedback = default_outcome['feedback']
            image_sizes_in_bytes = (
                get_image_sizes_in_bytes_dict_from_html(feedback['html']))
            feedback['image_sizes_in_bytes'] = image_sizes_in_bytes

            # For interactions - customization args.

            # For interactions - hints.
            for hint in interaction['hints']:
                image_sizes_in_bytes = get_image_sizes_in_bytes_dict_from_html(
                    hint['hint_content']['html'])
                hint['hint_content']['image_sizes_in_bytes'] = (
                    image_sizes_in_bytes)
