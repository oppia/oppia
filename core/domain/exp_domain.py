        ...
        for change in change_list:
            if change.cmd == CMD_ADD_STATE:
                ...
                if old_state_name in added_state_names:
                    ...
                    new_to_old_state_names[new_state_name] = new_to_old_state_names.pop(
                        old_state_name
                    )
                else:
...
class Exploration(translation_domain.BaseTranslatableObject):
        ...
        for state_name, state in self.states.items():
            ...
            if default_outcome is not None:
                ...
                if (
                    ...
                    and default_outcome.dest_if_really_stuck not in all_state_names
                ):
                    raise utils.ValidationError(
                ...
                for content_id in translations_mapping:
                    if content_id in list_of_subtitled_unicode_content_ids:
                        for language_code in translations_mapping[content_id]:
                            written_translation = translations_mapping[content_id][
                                language_code
                            ]
        ...
        for answer_group in interaction['answer_groups']:
            ...
            for rule_spec in answer_group['rule_specs']:
                for param_name, value in rule_spec['inputs'].items():
                    ...
                    param_type = interaction_registry.Registry.get_interaction_by_id(
                        interaction_id
    ...
    def _is_enclosed_by(
        ...
        lb_satisfied = base_range['lower_bound'] < test_range['lower_bound'] or (
            base_range['lower_bound'] == test_range['lower_bound']
            and (not test_range['lb_inclusive'] or base_range['lb_inclusive'])
        )
        ub_satisfied = base_range['upper_bound'] > test_range['upper_bound'] or (
    ...
    @classmethod
    def _fix_rte_tags(
        cls, html: str, *, is_tags_nested_inside_tabs_or_collapsible: bool = False
    ) -> str:
        ...
        for state in states_dict.values():
            ...
            for answer_group in state['interaction']['answer_groups']:
                ...
                if not html_cleaner.is_html_empty(feedback):
                    answer_group['outcome']['feedback']['html'] = cls.fix_content(
                        feedback
                    )
