...
class EntityVoiceoversUnitTests(test_utils.GenericTestBase):
    ...
    def test_validate_automated_voiceover_content_id(self) -> None:
        ...
        # wrong inputs that we can normally catch by typing.
        with self.assertRaisesRegex(
            utils.ValidationError, 'content_id must be a string, received'
        ):
            (
    ...
    def test_validate_token_data_type(self) -> None:
        with self.assertRaisesRegex(
            utils.ValidationError, 'Token must be a string'
        ):
            (
                self.entity_voiceovers_instance.automated_voiceovers_audio_offsets_msecs[
                    'content_id_0'
    ...
    def test_validate_audio_offset_data_type(self) -> None:
        with self.assertRaisesRegex(
            utils.ValidationError, 'audio_offset_msecs must be a floating value'
        ):
            (
                self.entity_voiceovers_instance.automated_voiceovers_audio_offsets_msecs[
                    'content_id_0'
    ...
    def test_add_voiceovers_correctly(self) -> None:
        ...
        assert isinstance(manual_voiceover, state_domain.Voiceover)
        self.assertDictEqual(
            manual_voiceover.to_dict(), dummy_new_voiceover_dict
        )
