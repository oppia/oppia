...
def _deassign_role(
    ...
    if not user_can_deassign_role:
        logging.error(
            'User %s tried to remove user %s from an activity %s but was refused permission.'
...
def _publish_activity(
    ...
    if not check_can_publish_activity(committer, activity_rights):
        logging.error(
            'User %s tried to publish %s %s but was refused permission.'
            % (committer_id, activity_type, activity_id)
        )
...
def set_private_viewability_of_exploration(
    ...
    if old_viewable_if_private == viewable_if_private:
        raise Exception(
            'Trying to change viewability status of this exploration to %s, but that is already the current value.'
            % viewable_if_private
        )
