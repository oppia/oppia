...
def get_activity_rights_from_model(
    activity_rights_model: Union[collection_models.CollectionRightsModel,
                                 exp_models.ExplorationRightsModel], activity_type: str
...
def _save_activity_rights(
    committer_id: str, activity_rights: rights_domain.ActivityRights,
    activity_type: str, commit_message: str,
    ...
    assert (
        activity_type
        in (constants.ACTIVITY_TYPE_COLLECTION, constants.ACTIVITY_TYPE_EXPLORATION)
    )

    if activity_type == constants.ACTIVITY_TYPE_EXPLORATION:
        model: Union[exp_models.ExplorationRightsModel, collection_models.
                     CollectionRightsModel] = exp_models.ExplorationRightsModel.get(
...
def _get_activity_rights_where_user_is_owner(
    ...
    if activity_type == constants.ACTIVITY_TYPE_EXPLORATION:
        activity_rights_models: Sequence[
            Union[collection_models.CollectionRightsModel,
                  exp_models.ExplorationRightsModel]
...
@overload
def _get_activity_rights(activity_type: str,
                         activity_id: str) -> Optional[rights_domain.ActivityRights]:
    ...
...
def _deassign_role(
    committer: user_domain.UserActionsInfo, removed_user_id: str, activity_id: str,
    activity_type: str
) -> None:
...
def _change_activity_status(
    committer_id: str, activity_id: str, activity_type: str, new_status: str,
    commit_message: str
) -> None:
...
def _publish_activity(
    ...
    _change_activity_status(
        committer_id, activity_id, activity_type, rights_domain.ACTIVITY_STATUS_PUBLIC,
...
def _unpublish_activity(
    ...
    _change_activity_status(
        committer_id, activity_id, activity_type, rights_domain.ACTIVITY_STATUS_PRIVATE,
...
def assign_role_for_exploration(
    committer: user_domain.UserActionsInfo, exploration_id: str, assignee_id: str,
    new_role: str
) -> None:
...
def deassign_role_for_exploration(
    committer: user_domain.UserActionsInfo, exploration_id: str, removed_user_id: str
) -> None:
    ...
    """
    _deassign_role(
        committer, removed_user_id, exploration_id, constants.ACTIVITY_TYPE_EXPLORATION
    )
...
def assign_role_for_collection(
    committer: user_domain.UserActionsInfo, collection_id: str, assignee_id: str,
    new_role: str
) -> None:
...
def deassign_role_for_collection(
    committer: user_domain.UserActionsInfo, collection_id: str, removed_user_id: str
) -> None:
    ...
    """
    _deassign_role(
        committer, removed_user_id, collection_id, constants.ACTIVITY_TYPE_COLLECTION
    )
