...
class UpdateSubtopicThumbnailSizeOneOffJob(jobs.BaseMapReduceOneOffJobManager):
    ...
    def map(item):
        ...
        try:
            ...
            yield (UpdateSubtopicThumbnailSizeOneOffJob._SUCCESS_KEY, 1)
        except Exception as e:
            logging.exception(
                'Subtopic %s thumbnail size update failed: %s' % (item.id, e))
            yield (
                UpdateSubtopicThumbnailSizeOneOffJob._ERROR_KEY,
                'Subtopic %s thumbnail size update failed: %s' % (item.id, e))
            return
