...
def get_published_blog_post_summaries_by_user_id(
    ...
    blog_post_summary_models: Sequence[blog_models.BlogPostSummaryModel] = (
        blog_models.BlogPostSummaryModel.query(
            ...
            blog_models.BlogPostSummaryModel.published_on != None # fmt: skip # pylint: disable=singleton-comparison, inequality-with-none, line-too-long
        ).order(
...
def get_total_number_of_published_blog_post_summaries() -> int:
    ...
    return blog_models.BlogPostRightsModel.query(
        blog_models.BlogPostRightsModel.blog_post_is_published == True  # fmt: skip # pylint: disable=singleton-comparison, line-too-long
    ).count()
