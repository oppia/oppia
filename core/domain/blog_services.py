...
def get_blog_post_by_url_fragment(
    ...
    Args:
        ...
        strict: bool. Whether to fail noisily if no blog post with the given
            url_fragment exists. Default is True.

    Returns:
        BlogPost or None. The domain object representing a blog post with the
        given ID, or None if it does not exist ad strict == False.

    Raises:
        Exception. If the node id is not in the story contents dict.
    """
    blog_post_model = (
        blog_models.BlogPostModel.get_by_url_fragment(url_fragment))
    if strict and blog_post_model is None:
        raise Exception(
            'No blog post exist for the given url fragment %s.'
            % url_fragment)
    if blog_post_model is None and not strict:
        return None
...
def get_blog_post_summary_models_list_by_user_id(
    ...
    sort_post_summaries: Callable[[blog_domain.BlogPostSummary], float] = (
        lambda k: k.last_updated.timestamp() if k.last_updated else 0
    )
    return (
        sorted(
            blog_post_summaries,
            key=sort_post_summaries,
            reverse=True
        )
...
def get_published_blog_post_summaries_by_user_id(
    ...
    for model in blog_summary_models:
        if model is not None:
            blog_post_summaries.append(get_blog_post_summary_from_model(model))
...
def does_blog_post_with_url_fragment_exist(url_fragment: str) -> bool:
    ...
    if not isinstance(url_fragment, str):
        raise utils.ValidationError(
            ...
            '%s' % url_fragment)
    existing_blog_post = get_blog_post_by_url_fragment(
        url_fragment,
        strict=False
    )
...
def get_published_blog_post_summaries(
    ...
    Returns:
        list(BlogPostSummaries | None) | None . These are sorted in order of
        the date published. None if no blog post is published.
    """
...
def update_blog_models_author_and_published_on_date(
    ...
    blog_post_model = blog_models.BlogPostModel.get(blog_post.id)
