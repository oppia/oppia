...
class TopicFetchersUnitTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.topic: Optional[topic_domain.Topic] = topic_fetchers.get_topic_by_id(
            self.TOPIC_ID
        )
    ...
    def test_get_topic_from_model(self) -> None:
        topic_model: Optional[topic_models.TopicModel] = topic_models.TopicModel.get(
            self.TOPIC_ID
        )
    ...
    def test_get_topic_by_url_fragment(self) -> None:
        topic: Optional[topic_domain.Topic] = topic_fetchers.get_topic_by_url_fragment(
            'name-one'
        )
        # Ruling out the possibility of None for mypy type checking.
        assert topic is not None
    ...
    def test_get_topic_by_id(self) -> None:
        ...
        fake_topic_id = topic_fetchers.get_new_topic_id()
        fake_topic: Optional[topic_domain.Topic] = topic_fetchers.get_topic_by_id(
    ...
    def test_get_topics_by_id(self) -> None:
        ...
        assert self.topic is not None
        expected_topic = self.topic.to_dict()
        topics: List[Optional[topic_domain.Topic]] = topic_fetchers.get_topics_by_ids(
            [self.TOPIC_ID]
        )
        # Ruling out the possibility of None for mypy type checking.
    ...
    def test_commit_log_entry(self) -> None:
        topic_commit_log_entry: Optional[topic_models.TopicCommitLogEntryModel] = (
    ...
    def test_get_multi_summaries(self) -> None:
        topic_summaries: List[Optional[topic_domain.TopicSummary]] = (
            topic_fetchers.get_multi_topic_summaries([self.TOPIC_ID, 'invalid_id'])
        )
