...
def get_library_groups(language_codes: List[str]) -> List[LibraryGroupDict]:
    ...
    for group in _LIBRARY_INDEX_GROUPS:
        ...
        exp_models_result = (
            ...
            .order(-exp_models.ExpSummaryModel.scaled_average_rating)
            .fetch(8)
        )
        exp_summaries = [
        ...
        summary_dicts.extend(get_displayable_exp_summary_dicts(exp_summaries))
...
def get_top_rated_exploration_summary_dicts(
    ...
    sort_fnc: Callable[[exp_domain.ExplorationSummary], float] = (
