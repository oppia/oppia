...
class AppFeedbackReportDomainTests(test_utils.GenericTestBase):
    def setUp(self) -> None:
        ...
        android_user_supplied_feedback = app_feedback_report_domain.UserSuppliedFeedback(
            REPORT_TYPE_SUGGESTION,
            CATEGORY_SUGGESTION_OTHER,
            USER_SELECTED_ITEMS,
            USER_TEXT_INPUT,
        )
        android_device_system_context = app_feedback_report_domain.AndroidDeviceSystemContext(
    ...
    def test_to_dict_android_report(self) -> None:
        ...
        expected_dict = {
            ...
            'submitted_on_timestamp': utils.get_human_readable_time_string(utils.get_time_in_millisecs(REPORT_SUBMITTED_TIMESTAMP)),
    ...
    def test_android_report_creation_from_submitted_feedback(self) -> None:
        REPORT_JSON['platform_type'] = app_feedback_report_constants.PLATFORM_CHOICE_ANDROID
        result = self.android_report_obj.from_submitted_feedback_dict(REPORT_JSON)
        expected_report_datetime = datetime.datetime.fromtimestamp(REPORT_JSON['report_submission_timestamp_sec'])
        expected_report_schema_version = REPORT_JSON['android_report_info_schema_version']
        self.assertEqual(result.submitted_on_timestamp, expected_report_datetime)
    ...
    def test_report_android_schema_version_not_an_int_validation_fails(
        ...
        self._assert_validation_error(
            ...
            'The report schema version %r is invalid, expected an integer' % (self.android_report_obj.schema_version),
        )
    ...
    def test_report_android_schema_version_greater_than_max_validation_fails(
        ...
        self._assert_validation_error(
            self.android_report_obj,
            'The supported report schema versions for %s reports are' % (PLATFORM_ANDROID),
        )
    ...
    def test_report_scrubber_id_is_invalid_validation_fails(self) -> None:
        ...
        self._assert_validation_error(
            self.android_report_obj,
            'The scrubbed_by user id \'%s\' is invalid.' % (self.android_report_obj.scrubbed_by),
        )
    ...
    def test_get_category_from_string_returns_expected_category(self) -> None:
        ...
        for category in app_feedback_report_constants.ALLOWED_CATEGORIES:
            self.assertEqual(
                feedback_report.get_category_from_string(category.value),
                category,
            )
    ...
    def test_get_entry_point_from_json_returns_expected_entry_point_obj(
        ...
        entry_point_json['entry_point_name'] = app_feedback_report_constants.EntryPoint.STUDY_GUIDE.value
        study_guide_obj = feedback_report.get_entry_point_from_json(entry_point_json)
        self.assertTrue(isinstance(study_guide_obj, app_feedback_report_domain.StudyGuideEntryPoint))
    ...
    def test_raises_error_with_invalid_entry_point_during_entry_point_from_json(
        ...
        with self.assertRaisesRegex(Exception, 'No topic_id provided for StudyGuideEntryPoint.'):
            entry_point_json['entry_point_topic_id'] = None
            feedback_report.get_entry_point_from_json(entry_point_json)

        with self.assertRaisesRegex(Exception, 'No subtopic_id provided for StudyGuideEntryPoint.'):
            entry_point_json['entry_point_topic_id'] = 'topic_id'
    ...
    def test_get_android_network_type_from_string_returns_expected_network_type(
        ...
        for network_type in app_feedback_report_constants.AndroidNetworkType:
            self.assertEqual(
                feedback_report.get_android_network_type_from_string(network_type.value),
                network_type,
            )
...
class AndroidDeviceSystemContextTests(test_utils.GenericTestBase):
    def setUp(self) -> None:
        super().setUp()
        self.device_system_context = app_feedback_report_domain.AndroidDeviceSystemContext(
            ANDROID_PLATFORM_VERSION,
            ANDROID_PACKAGE_VERSION_CODE,
            COUNTRY_LOCALE_CODE_INDIA,
            LANGUAGE_LOCALE_CODE_ENGLISH,
    ...
    def test_validation_package_version_code_less_than_minimum_fails(
        ...
        self._assert_validation_error(
            self.device_system_context,
            'The package version code is not a valid int. The minimum supported version is %d' % (feconf.MINIMUM_ANDROID_PACKAGE_VERSION_CODE),
        )
...
class NavigationDrawerEntryPointDomainTests(test_utils.GenericTestBase):
    ...
    def test_to_dict(self) -> None:
        expected_dict = {
            'entry_point_name': (app_feedback_report_constants.EntryPoint.NAVIGATION_DRAWER.value)  # pylint: disable=line-too-long
        }
    ...
    def test_validation_name_is_invalid_fails(self) -> None:
        ...
        with self.assertRaisesRegex(
            ...
            'Expected entry point name %s' % (app_feedback_report_constants.EntryPoint.NAVIGATION_DRAWER.value),
...
class LessonPlayerEntryPointDomainTests(test_utils.GenericTestBase):
    ...
    def test_to_dict(self) -> None:
        expected_dict = {
            'entry_point_name': (app_feedback_report_constants.EntryPoint.LESSON_PLAYER.value),
            'topic_id': 'topic_id',
            'story_id': 'story_id',
    ...
    def test_validation_name_is_invalid_fails(self) -> None:
        ...
        self._assert_validation_error(
            self.entry_point,
            'Expected entry point name %s' % (app_feedback_report_constants.EntryPoint.LESSON_PLAYER.value),
        )
    ...
    def test_validation_with_valid_story_id(self, mock_get_story_id_linked_to_exploration: mock.MagicMock) -> None:
    ...
    def test_entry_point_with_invalid_exploration_id_type(
        ...
        with self.assertRaisesRegex(
            utils.ValidationError,
            'Exploration id should be a string, received: %r' % (self.entry_point.exploration_id),
        ):
            self.entry_point.require_valid_entry_point_exploration(
                exploration_id=self.entry_point.exploration_id,
                story_id=self.entry_point.story_id,
    ...
    def test_entry_point_with_mismatched_story_id_and_exploration_id(
        ...
        with self.assertRaisesRegex(
            utils.ValidationError,
            'Exploration with id %s is not part of story with id of %s' % (self.entry_point.exploration_id, self.entry_point.story_id),
        ):
            self.entry_point.require_valid_entry_point_exploration(
                exploration_id=self.entry_point.exploration_id,
                story_id=self.entry_point.story_id,
...
class StudyGuideEntryPointDomainTests(test_utils.GenericTestBase):
    def setUp(self) -> None:
        super().setUp()
        self.entry_point = app_feedback_report_domain.StudyGuideEntryPoint('topic_id', 'subtopic_id')

    def test_to_dict(self) -> None:
        expected_dict = {
            'entry_point_name': (app_feedback_report_constants.EntryPoint.STUDY_GUIDE.value),
            'topic_id': 'topic_id',
    ...
    def test_validation_name_is_invalid_fails(self) -> None:
        ...
        self._assert_validation_error(
            self.entry_point,
            'Expected entry point name %s' % (app_feedback_report_constants.EntryPoint.STUDY_GUIDE.value),
        )
...
class CrashEntryPointDomainTests(test_utils.GenericTestBase):
    ...
    def test_validation_name_is_invalid_fails(self) -> None:
        ...
        with self.assertRaisesRegex(
            utils.ValidationError,
            'Expected entry point name %s' % (app_feedback_report_constants.EntryPoint.CRASH.value),
        ):
            self.entry_point.validate()
...
class AppFeedbackReportTicketDomainTests(test_utils.GenericTestBase):
    ...
    def test_to_dict(self) -> None:
        expected_dict = {
            ...
            'archived': False,
            'newest_report_creation_timestamp_isoformat': (REPORT_SUBMITTED_TIMESTAMP.isoformat()),
    ...
    def test_validation_ticket_name_too_long_fails(self) -> None:
        long_name = 'too long%s' % 'x' * (app_feedback_report_constants.MAXIMUM_TICKET_NAME_LENGTH)
...
class AppFeedbackReportDailyStatsDomainTests(test_utils.GenericTestBase):
    def setUp(self) -> None:
        ...
        param_stats = {
            'platform': (app_feedback_report_domain.ReportStatsParameterValueCounts({PLATFORM_ANDROID: 1})),
            'report_type': (app_feedback_report_domain.ReportStatsParameterValueCounts({REPORT_TYPE_SUGGESTION.value: 1})),
            'country_locale_code': (app_feedback_report_domain.ReportStatsParameterValueCounts({COUNTRY_LOCALE_CODE_INDIA: 1})),
            'entry_point_name': (app_feedback_report_domain.ReportStatsParameterValueCounts({ENTRY_POINT_NAVIGATION_DRAWER: 1})),
            'text_language_code': (app_feedback_report_domain.ReportStatsParameterValueCounts({LANGUAGE_LOCALE_CODE_ENGLISH: 1})),
            'audio_language_code': (app_feedback_report_domain.ReportStatsParameterValueCounts({LANGUAGE_LOCALE_CODE_ENGLISH: 1})),
            'android_sdk_version': (app_feedback_report_domain.ReportStatsParameterValueCounts({str(ANDROID_SDK_VERSION): 1})),
            'version_name': (app_feedback_report_domain.ReportStatsParameterValueCounts({ANDROID_PLATFORM_VERSION: 1})),
        }
        self.stats_id = app_feedback_report_models.AppFeedbackReportStatsModel.calculate_id(
            PLATFORM_ANDROID,
            self.ticket_id,
    ...
    def test_validation_invalid_daily_param_stats_fails(self) -> None:
        self.stats_obj.daily_param_stats = {
            'invalid_stat_name': app_feedback_report_domain.ReportStatsParameterValueCounts({'invalid_stats': 0}),
        }
        self._assert_validation_error(
            self.stats_obj,
            'The parameter %s is not a valid parameter to aggregate stats on' % 'invalid_stat_name',
        )
