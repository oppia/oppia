...
class TopicServicesUnitTests(test_utils.GenericTestBase):
    ...
    def test_compute_summary(self) -> None:
        ...
        self.assertEqual(topic_summary.canonical_name, self.topic.canonical_name)
        ...
        self.assertEqual(topic_summary.thumbnail_filename, self.topic.thumbnail_filename)
        self.assertEqual(topic_summary.thumbnail_bg_color, self.topic.thumbnail_bg_color)
        self.assertEqual(topic_summary.topic_model_created_on, self.topic.created_on)
        self.assertEqual(topic_summary.topic_model_last_updated, self.topic.last_updated)

    def test_compute_summary_when_serial_chapter_launch_disabled(self) -> None:
        story_exp_ids = ['exp-1', 'exp-2']
        no_exp_ids: List[str] = []
        self._create_linked_explorations(self.TOPIC_ID, self.story_id_1, story_exp_ids)
        ...
        self.assertEqual(topic_summary.total_published_node_count, len(story_exp_ids))
        self.assertDictEqual(
            topic_summary.published_story_exploration_mapping,
            {self.story_id_1: story_exp_ids, self.story_id_2: no_exp_ids},
        )

    @test_utils.enable_feature_flags([feature_flag_list.FeatureNames.SERIAL_CHAPTER_LAUNCH_CURRICULUM_ADMIN_VIEW])
    def test_compute_summary_when_serial_chapter_launch_enabled(self) -> None:
        ...
        self._publish_story_chapters_with_explorations(self.TOPIC_ID, self.story_id_1, story_1_published_chapter_exp_ids)
        ...
        self.assertEqual(
            ...
            len(story_1_published_chapter_exp_ids) + len(story_2_published_chapter_exp_ids),
        )
        self.assertDictEqual(
            topic_summary.published_story_exploration_mapping,
            {
    ...
    def test_compute_summary_does_not_map_unpublished_stories(self) -> None:
        story_exp_ids = ['exp-1', 'exp-2']
        self._create_linked_explorations(self.TOPIC_ID, self.story_id_1, story_exp_ids)
        updated_topic = topic_fetchers.get_topic_by_id(self.TOPIC_ID)
        ...
        self.assertDictEqual(topic_summary.published_story_exploration_mapping, {})
    ...
    def _publish_story_chapters(self, topic_id: str, story_id: str, chapter_exp_ids: List[str]) -> None:
        ...
        change_list = [
            ...
            if story.story_contents.nodes[i].exploration_id == chapter_exp_ids[i]
        ]
        topic_services.update_story_and_topic_summary(
            self.user_id_admin,
            story_id,
            change_list,
            'Publish these story chapters only.',
            topic_id,
        )

    def _create_linked_explorations(self, topic_id: str, story_id: str, exp_ids: List[str]) -> None:
        """Creates explorations with exp_ids. Links them to the story given
        ...
        for exp_id in exp_ids:
            self.save_new_valid_exploration(exp_id, self.user_id_admin, end_state_name='end')
    ...
    def test_generate_topic_summary_when_deleting_published_story(self) -> None:
        topic_services.publish_story(self.TOPIC_ID, self.story_id_1, self.user_id_admin)
    ...
    def test_generate_topic_summary_when_publishing_story_chapter(self) -> None:
        linked_exp_ids = ['exp_1']
        self._create_linked_explorations(self.TOPIC_ID, self.story_id_1, linked_exp_ids)
        topic_services.publish_story(self.TOPIC_ID, self.story_id_1, self.user_id_admin)
        story = story_fetchers.get_story_by_id(self.story_id_1)

        with (
            self.swap_to_always_return(feature_flag_services, 'is_feature_flag_enabled', True),
        ...
        topic_summary = topic_fetchers.get_topic_summary_by_id(self.TOPIC_ID)
        self.assertEqual(topic_summary.id, self.TOPIC_ID)
        self.assertEqual(topic_summary.total_published_node_count, len(linked_exp_ids))
        self.assertDictEqual(
            topic_summary.published_story_exploration_mapping,
    ...
    def test_generate_topic_summary_when_unpublishing_story_chapter(
        ...
        topic_summary = topic_fetchers.get_topic_summary_by_id(self.TOPIC_ID)
        self.assertEqual(topic_summary.id, self.TOPIC_ID)
        self.assertEqual(topic_summary.total_published_node_count, len(linked_exp_ids) - 1)
        self.assertDictEqual(
            topic_summary.published_story_exploration_mapping,
            {self.story_id_1: []},
        )
    ...
    def test_generate_topic_summary_when_changing_exp_id_linked_to_published_story_chapter(  # pylint: disable=line-too-long
        ...
        exp_id_1 = 'exp_1'
        exp_id_2 = 'exp_2'
        self._create_linked_explorations(self.TOPIC_ID, self.story_id_1, [exp_id_1])
        topic_services.publish_story(self.TOPIC_ID, self.story_id_1, self.user_id_admin)
        story = story_fetchers.get_story_by_id(self.story_id_1)
        topic_services.update_story_and_topic_summary(
            self.user_id_admin,
        ...
        self.save_new_valid_exploration(exp_id_2, self.user_id_admin, end_state_name='end')
    ...
    def test_cannot_rearrange_story_with_missing_index_values(self) -> None:
        with self.assertRaisesRegex(
            Exception,
            ('The following required attributes are missing: from_index, to_index'),
        ):
            topic_domain.TopicChange(
                {
    ...
    def test_cannot_update_topic_property_with_invalid_changelist(self) -> None:
        with self.assertRaisesRegex(
            Exception,
            ('Value for property_name in cmd update_topic_property: invalid property is not allowed'),
        ):
            topic_domain.TopicChange(
                {
    ...
    def test_update_subtopic_property_without_study_guide(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            'Entity for class StudyGuideModel with id %s not found' % (f'{topic_id_1}-1'),
        ):
            study_guide_services.get_study_guide_by_id(topic_id_1, 1)

        # Test whether a subtopic without corresponding study guide
        # already existing in datastore can be edited.
        changelist = [
            topic_domain.TopicChange(
                {
                    'cmd': topic_domain.CMD_UPDATE_SUBTOPIC_PROPERTY,
    ...
    def test_publish_and_unpublish_story(self) -> None:
        ...
        with self.swap_with_call_counter(topic_services, 'generate_topic_summary') as (generate_topic_summary):
            topic_services.publish_story(self.TOPIC_ID, self.story_id_1, self.user_id_admin)
    ...
    def test_invalid_publish_and_unpublish_story(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            'Exploration with ID exp_id is not public. Please publish explorations before adding them to a story.',
        ):
        ...
        with self.assertRaisesRegex(
            Exception,
            'Expected story to only reference valid explorations, but found a reference to an invalid exploration with ID: exp_id',
        ):
            topic_services.publish_story(self.TOPIC_ID, 'story_id_new', self.user_id_admin)
    ...
    def test_update_topic(self) -> None:
        ...
        changelist = [
            ...
            topic_domain.TopicChange(
                {
                    'cmd': topic_domain.CMD_UPDATE_TOPIC_PROPERTY,
                    'property_name': (topic_domain.TOPIC_PROPERTY_PRACTICE_TAB_IS_DISPLAYED),
                    'old_value': False,
                    'new_value': True,
                }
            ),
            topic_domain.TopicChange(
                {
                    'cmd': topic_domain.CMD_UPDATE_TOPIC_PROPERTY,
                    'property_name': (topic_domain.TOPIC_PROPERTY_PAGE_TITLE_FRAGMENT_FOR_WEB),
                    'old_value': '',
        ...
        with self.assertRaisesRegex(
            Exception,
            ('The thumbnail dummy_thumbnail.svg for topic with id %s does not exist in the filesystem.' % self.TOPIC_ID),
        ):
            topic_services.update_topic_and_subtopic_pages(
                self.user_id_admin,
                self.TOPIC_ID,
    ...
    def test_simultaneous_subtopic_and_subtopic_page_changes(self) -> None:
            ...
            subtopic_page_domain.SubtopicPageChange(
                {
                    ...
                    'property_name': (subtopic_page_domain.SUBTOPIC_PAGE_PROPERTY_PAGE_CONTENTS_HTML),
                    'old_value': '',
                    'subtopic_id': 1,
                    'new_value': {
                        'html': '<p>New Value</p>',
        ...
        study_guide = study_guide_services.get_study_guide_by_id(self.TOPIC_ID, 1)
        ...
        study_guide = study_guide_services.get_study_guide_by_id(self.TOPIC_ID, 1)
        study_guide_section = study_guide.sections[0]
        self.assertEqual(study_guide_section.heading.unicode_str, 'Another New Title')
        self.assertEqual(study_guide_section.content.html, '<p>Another New Value</p>')
    ...
    def test_update_topic_and_subtopic_page(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            'The given new subtopic id 3 is not equal to the expected next subtopic id: 2',
        ):
        ...
        changelist = [
            ...
            topic_domain.TopicChange({'cmd': topic_domain.CMD_DELETE_SUBTOPIC, 'subtopic_id': 2}),
        ]
        with self.assertRaisesRegex(
            Exception,
            'The incoming changelist had simultaneous creation and deletion of subtopics.',
        ):
            topic_services.update_topic_and_subtopic_pages(
                self.user_id_admin,
                self.TOPIC_ID,
        ...
        changelist = [
            ...
            topic_domain.TopicChange({'cmd': topic_domain.CMD_DELETE_SUBTOPIC, 'subtopic_id': 1}),
            subtopic_page_domain.SubtopicPageChange(
                {
                    'cmd': subtopic_page_domain.CMD_UPDATE_SUBTOPIC_PAGE_PROPERTY,
                    'property_name': (subtopic_page_domain.SUBTOPIC_PAGE_PROPERTY_PAGE_CONTENTS_HTML),
            ...
            subtopic_page_domain.SubtopicPageChange(
                {
                    'cmd': subtopic_page_domain.CMD_UPDATE_SUBTOPIC_PAGE_PROPERTY,
                    'property_name': (subtopic_page_domain.SUBTOPIC_PAGE_PROPERTY_PAGE_CONTENTS_AUDIO),
    ...
    @test_utils.enable_feature_flags([feature_flag_list.FeatureNames.SHOW_RESTRUCTURED_STUDY_GUIDES])
    def test_update_topic_and_study_guide(self) -> None:
        changelist: List[Union[topic_domain.TopicChange, study_guide_domain.StudyGuideChange]] = [
            topic_domain.TopicChange(
                {
                    'cmd': topic_domain.CMD_ADD_SUBTOPIC,
                    'title': 'Title3',
        ...
        # attempt.
        study_guide = study_guide_services.get_study_guide_by_id(self.TOPIC_ID, 3, strict=False)
            ...
            study_guide_domain.StudyGuideChange(
                {
                    ...
                    'property_name': (study_guide_domain.STUDY_GUIDE_PROPERTY_SECTIONS),
                    'subtopic_id': 2,
                    'old_value': [
                        {
                            'heading': {
        ...
        topic_services.update_topic_and_subtopic_pages(self.user_id_admin, self.TOPIC_ID, changelist, 'Updated html data')
        study_guide = study_guide_services.get_study_guide_by_id(self.TOPIC_ID, 2)
        self.assertEqual(
        ...
        # was also deleted.
        study_guide = study_guide_services.get_study_guide_by_id(self.TOPIC_ID, 2, strict=False)
        self.assertIsNone(study_guide)
        ...
        changelist = [
            ...
            topic_domain.TopicChange({'cmd': topic_domain.CMD_DELETE_SUBTOPIC, 'subtopic_id': 3}),
        ...
        with self.assertRaisesRegex(Exception, 'The subtopic with id 3 doesn\'t exist'):
            topic_services.update_topic_and_subtopic_pages(
                self.user_id_admin,
                self.TOPIC_ID,
                changelist,
        ...
        study_guide = study_guide_services.get_study_guide_by_id(self.TOPIC_ID, 2, strict=False)
        self.assertIsNone(study_guide)
        study_guide = study_guide_services.get_study_guide_by_id(self.TOPIC_ID, 4, strict=False)
        self.assertIsNone(study_guide)
        study_guide = study_guide_services.get_study_guide_by_id(self.TOPIC_ID, 3, strict=False)
    ...
    def test_update_subtopic_page_without_study_guide(self) -> None:
        ...
        # Test whether a subtopic page without corresponding study guide
        # already existing in datastore can be edited.
        changelist = [
            subtopic_page_domain.SubtopicPageChange(
                {
        ...
        topic_services.update_topic_and_subtopic_pages(self.user_id_admin, topic_id_1, changelist, 'Updated html data')
        subtopic_page = subtopic_page_services.get_subtopic_page_by_id(topic_id_1, 1)
        self.assertEqual(subtopic_page.page_contents.subtitled_html.html, '<p>New Value</p>')
    ...
    def test_update_subtopic_skill_ids(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            'Skill id %s is not present in the given old subtopic' % self.skill_id_2,
        ):
            topic_services.update_topic_and_subtopic_pages(
                self.user_id_admin,
                self.TOPIC_ID,
    ...
    def test_does_not_update_subtopic_url_fragment_if_it_already_exists(
        ...
        self.save_new_topic(
            topic_id,
            self.user_id,
            name='topic-with-duplicate-subtopic',
            description='Description',
            canonical_story_ids=[],
            additional_story_ids=[],
            uncategorized_skill_ids=[],
            subtopics=[],
            next_subtopic_id=1,
            url_fragment='frag-dup-subtopic',
        )
        with self.assertRaisesRegex(
            Exception,
            'Subtopic url fragments are not unique across subtopics in the topic',
        ):
    ...
    def test_cannot_update_topic_and_subtopic_pages_with_empty_changelist(
        ...
        with self.assertRaisesRegex(
            Exception,
            'Unexpected error: received an invalid change list when trying to save topic',
        ):
    ...
    def test_cannot_update_topic_and_subtopic_pages_with_mismatch_of_versions(
        ...
        with self.assertRaisesRegex(
            Exception,
            'Unexpected error: trying to update version 1 of topic from version 2. Please reload the page and try again.',
        ):
        ...
        with self.assertRaisesRegex(
            Exception,
            'Trying to update version 101 of topic from version 2, which is too old. Please reload the page and try again.',
        ):
            topic_services.update_topic_and_subtopic_pages(self.user_id, self.TOPIC_ID, changelist, 'change language_code')
    ...
    def test_update_thumbnail_filename(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            'The thumbnail img.svg for topic with id %s does not exist in the filesystem.' % (self.TOPIC_ID),
        ):
    ...
    def test_update_subtopic_thumbnail_filename(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            'The thumbnail %s for subtopic with topic_id %s does not exist in the filesystem.' % ('new_image.svg', self.TOPIC_ID),
        ):
    ...
    def test_get_topic_id_to_diagnostic_test_skill_ids(self) -> None:
        ...
        self.save_new_topic(
            ...
            description='Description of addition.',
            canonical_story_ids=[self.story_id_1, self.story_id_2],
            additional_story_ids=[self.story_id_3],
            uncategorized_skill_ids=[self.skill_id_1, self.skill_id_2],
            subtopics=[],
        ...
        self.assertEqual(
            topic_services.get_topic_id_to_diagnostic_test_skill_ids([fractions_id, additions_id]),
            expected_dict,
        )
    ...
    def test_get_topic_id_to_topic_name_dict(self) -> None:
        ...
        self.assertEqual(
            topic_services.get_topic_id_to_topic_name_dict([fractions_id, additions_id]),
            expected_dict,
        )
    ...
    def test_populate_topic_summary_model_fields(self) -> None:
        ...
        self.assertDictEqual(
            populated_model.published_story_exploration_mapping,
            topic_summary.published_story_exploration_mapping,
        )
    ...
    def test_get_chapter_counts_in_topic_summaries(self) -> None:
        ...
        with self.swap(
            ...
            frontend_topic_summary: topic_domain.FrontendTopicSummaryDict = {
                ...
                'additional_story_count': topic_summary['canonical_story_count'],
                'uncategorized_skill_count': topic_summary['additional_story_count'],
                ...
                'published_story_exploration_mapping': topic_summary['published_story_exploration_mapping'],
            ...
            self.assertEqual(
                chapter_counts[self.TOPIC_ID].total_chapter_counts_for_each_story,
                [0, 0, 4],
            )
            self.assertEqual(
                chapter_counts[self.TOPIC_ID].published_chapter_counts_for_each_story,
                [0, 0, 1],
            )
    ...
    def test_get_chapter_counts_in_topic_summaries_logs_error(self) -> None:
        ...
        with self.swap(
            ...
            with self.capture_logging(min_level=logging.ERROR) as logs:
                ...
                topic_services.get_chapter_counts_in_topic_summaries([frontend_topic_summary])
                self.assertEqual(
                    logs,
                    ['Topic %s has an invalid story reference ID invalid_story_id' % self.TOPIC_ID],
                )
    ...
    def test_get_published_story_exploration_ids_ignores_exp_ids_from_unpublished_stories(  # pylint: disable=line-too-long
        ...
        self._create_linked_explorations(self.TOPIC_ID, self.story_id_1, topic_exp_ids)

        story_exp_ids = topic_services.get_all_published_story_exploration_ids(self.TOPIC_ID)
    ...
    def test_get_published_story_exploration_ids_in_all_topics_when_topic_id_not_given(  # pylint: disable=line-too-long
        ...
        self.save_new_topic(
            ...
            url_fragment='name-two',
            description='Description',
            canonical_story_ids=[story_id_4],
            additional_story_ids=[],
            uncategorized_skill_ids=[],
            subtopics=[],
            next_subtopic_id=1,
        ...
        self._publish_story_chapters_with_explorations(self.TOPIC_ID, self.story_id_1, exp_ids[:1])
        self._publish_story_chapters_with_explorations(self.TOPIC_ID, self.story_id_3, exp_ids[1:3])
        self._publish_story_chapters_with_explorations(topic_id_2, story_id_4, exp_ids[3:])
    ...
    def test_get_published_story_exploration_ids_when_mapping_not_precomputed(
        ...
        self._create_linked_explorations(self.TOPIC_ID, self.story_id_1, topic_exp_ids)
        topic_services.publish_story(self.TOPIC_ID, self.story_id_1, self.user_id_admin)
        topic_summary_without_exp_ids = topic_services.compute_summary_of_topic(self.topic)
