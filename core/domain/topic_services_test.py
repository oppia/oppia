...
class TopicServicesUnitTests(test_utils.GenericTestBase):
    ...
    def test_does_not_add_skill_with_superseding_skill_to_topic(
            self) -> None:
        self.save_new_skill(
            'supersede', self.user_id)
        self.save_new_skill(
            'has_superseding', self.user_id)
        changelist = [
        ...
        skill_services.update_skill(
            self.user_id, 'has_superseding', changelist,
            'Merging skill.')
        topic_id = topic_fetchers.get_new_topic_id()
        self.save_new_topic(
            topic_id, self.user_id, name='topic-supersede',
            description='desc', url_fragment='topic-url-frag',
