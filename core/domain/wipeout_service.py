...
def get_pending_deletion_request(user_id):
    ...
    """
    pending_deletion_request_model = user_models.PendingDeletionRequestModel.get_by_id(
        user_id
    )
...
def save_pending_deletion_requests(pending_deletion_requests):
    ...
    pending_deletion_request_models = user_models.PendingDeletionRequestModel.get_multi(
...
def pre_delete_user(user_id):
    ...
    if user_settings.role != feconf.ROLE_ID_LEARNER:
        subscribed_exploration_summaries = exp_fetchers.get_exploration_summaries_subscribed_to(
            user_id
        )
        ...
        subscribed_collection_summaries = collection_services.get_collection_summaries_subscribed_to(
            user_id
        )
        collections_to_be_deleted_ids = [
            col_summary.id
...
def _generate_entity_to_pseudonymized_ids_mapping(entity_ids):
    ...
    Args:
        entity_ids: list(str). List of entity IDs for which to generate
    ...
    Returns:
        dict(str, str). Mapping between the entity IDs and pseudonymous
        user IDs. For each entity (with distinct ID) we generate a new
...
def _delete_models(user_id, user_role, module_name):
    ...
    for model_class in models.Registry.get_storage_model_classes([module_name]):
        ...
        if (
            user_role != lowest_role_for_class
            and role_services.check_if_path_exists_in_roles_graph(
                lowest_role_for_class, user_role
            )
            is False
        ):
            continue
...
def _collect_and_save_entity_ids_from_snapshots_and_commit(
    ...
    Args:
        ...
        entity_category: models.NAMES. The category of the models that are
...
def _pseudonymize_config_models(pending_deletion_request):
    ...
    config_ids_to_pids = pending_deletion_request.activity_mappings[
        models.NAMES.config
    ]
    for config_id, pseudonymized_id in config_ids_to_pids.items():
        ...
        for i in python_utils.RANGE(
            0, len(config_related_models), MAX_NUMBER_OF_OPS_IN_TRANSACTION
        ):
...
def _pseudonymize_entity_models(
    ...
    def _pseudonymize_models(entity_related_models, pseudonymized_id):
        ...
        This function is run in a transaction, with the maximum number of
        entity_related_models being MAX_NUMBER_OF_OPS_IN_TRANSACTION.

        Args:
            entity_related_models: list(BaseModel). Models whose user IDs
                should be pseudonymized.
            pseudonymized_id: str. New pseudonymized user ID to be used for
                the models.
        """
    ...
    for entity_id, pseudonymized_id in entity_ids_to_pids.items():
        entity_related_models = [
            model for model in snapshot_metadata_models
            if model.get_unversioned_instance_id() == entity_id
        ] + [
            model for model in commit_log_models
            if getattr(model, commit_log_model_field_name) == entity_id
        ]
        for i in python_utils.RANGE(
                0,
                len(entity_related_models),
                MAX_NUMBER_OF_OPS_IN_TRANSACTION):
            transaction_services.run_in_transaction(
                _pseudonymize_models,
                entity_related_models[i:i + MAX_NUMBER_OF_OPS_IN_TRANSACTION],
                pseudonymized_id)
...
def _pseudonymize_col_or_exp_models(
    ...
    for col_or_exp_id, pseudonymized_id in col_or_exp_ids_to_pids.items():
        ...
        col_or_exp_related_rights_snapshots_ids = [
            model.id
            for model in col_or_exp_related_snapshot_metadata_models
            if isinstance(model, rights_snapshot_metadata_model_class)
        ]
        col_or_exp_related_snapshot_content_models = rights_snapshot_content_model_class.get_multi(
        ...
        for i in python_utils.RANGE(
            0, len(col_or_exp_related_models), MAX_NUMBER_OF_OPS_IN_TRANSACTION
        ):
            transaction_services.run_in_transaction(
                _pseudonymize_models,
                col_or_exp_related_models[
...
def _pseudonymize_topic_models(pending_deletion_request):
    ...
    topic_model_ids, topic_snapshot_metadata_models, topic_commit_log_models = _collect_activity_ids_from_snapshots_and_commit(
        pending_deletion_request.user_id,
        topic_metadata_model_classes,
    ...
    def _pseudonymize_models(topic_related_models, pseudonymized_id):
        ...
        snapshot_metadata_models = [
            model
            for model in topic_related_models
            if isinstance(model, metadata_model_classes)
        ]
        for snapshot_metadata_model in snapshot_metadata_models:
            for commit_cmd in snapshot_metadata_model.commit_cmds:
    ...
    topic_ids_to_pids = pending_deletion_request.activity_mappings[
        models.NAMES.topic
    ]
    for topic_id, pseudonymized_id in topic_ids_to_pids.items():
        topic_related_snapshot_metadata_models = [
            model
            for model in topic_snapshot_metadata_models
            if model.get_unversioned_instance_id() == topic_id
        ]

        topic_related_rights_snapshots_ids = [
            model.id
            for model in topic_related_snapshot_metadata_models
            if isinstance(model, topic_models.TopicRightsSnapshotMetadataModel)
        ]
        topic_related_snapshot_content_models = topic_models.TopicRightsSnapshotContentModel.get_multi(
        ...
        topic_related_models = (
            ...
            + [
                model
                for model in subtopic_snapshot_metadata_models
                if subtopic_id_to_topic_id[model.get_unversioned_instance_id()]
                == topic_id
            ]
            + [
                model
                for model in topic_commit_log_models
                if getattr(model, "topic_id") == topic_id
            ]
            + [
                model
                for model in subtopic_commit_log_models
                if subtopic_id_to_topic_id[getattr(model, "subtopic_id")]
                == topic_id
            ]
        )
...
def _pseudonymize_feedback_models(pending_deletion_request):
    ...
    for feedback_id, pseudonymized_id in feedback_ids_to_pids.items():
        feedback_related_models = (
            [
                model
                for model in feedback_thread_models
                if model.id == feedback_id
            ]
            + [
                model
                for model in feedback_message_models
                if model.thread_id == feedback_id
            ]
            + [
                model
                for model in general_suggestion_models
                if model.id == feedback_id
            ]
        )
        for i in python_utils.RANGE(
            0, len(feedback_related_models), MAX_NUMBER_OF_OPS_IN_TRANSACTION
        ):
            transaction_services.run_in_transaction(
                _pseudonymize_models,
                feedback_related_models[
    ...
    def _pseudonymize_models(voiceover_application_models):
        ...
        for voiceover_application_model in voiceover_application_models:
            if voiceover_application_model.author_id == user_id:
                voiceover_application_model.author_id = suggestion_ids_to_pids[
                    voiceover_application_model.id
                ]
            if voiceover_application_model.final_reviewer_id == user_id:
                voiceover_application_model.final_reviewer_id = suggestion_ids_to_pids[
                    voiceover_application_model.id
                ]
