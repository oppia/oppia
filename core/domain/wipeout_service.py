...
def save_pending_deletion_requests(
    ...
    for deletion_request_model, deletion_request in zip(pending_deletion_request_models, pending_deletion_requests):
        ...
        deletion_request_dict = {
            ...
            'normalized_long_term_username': (deletion_request.normalized_long_term_username),
            'deletion_complete': deletion_request.deletion_complete,
            'pseudonymizable_entity_mappings': (deletion_request.pseudonymizable_entity_mappings),
        }
...
def pre_delete_user(user_id: str) -> None:
    ...
    if feconf.ROLE_ID_MOBILE_LEARNER not in user_settings.roles:
        taskqueue_services.defer(
            feconf.FUNCTION_ID_TO_FUNCTION_NAME_FOR_DEFERRED_JOBS['FUNCTION_ID_REMOVE_USER_FROM_RIGHTS_MODELS'],
...
def remove_user_from_translation_coordinators(user_id: str) -> None:
    ...
    for coordinator_model in translation_coordinator_models:
        if user_id in coordinator_model.coordinator_ids:
            ...
            coordinator_model.coordinators_count = len(coordinator_model.coordinator_ids)
            models_to_update.append(coordinator_model)

    if models_to_update:
        suggestion_models.TranslationCoordinatorsModel.update_timestamps_multi(models_to_update)
...
def remove_user_from_user_groups(user_id: str) -> None:
    ...
    """
    user_group_models: Sequence[user_models.UserGroupModel] = user_models.UserGroupModel.query(user_models.UserGroupModel.user_ids == user_id).fetch()
...
def delete_user(
    ...
    if user_settings_model is None or feconf.ROLE_ID_MOBILE_LEARNER not in user_settings_model.roles:
...
def verify_user_deleted(user_id: str, include_delete_at_end_models: bool = False) -> bool:
    ...
    if not auth_services.verify_external_auth_associations_are_deleted(user_id):
        logging.error('External auth association is not deleted for user with ID %s' % (user_id))
        return False
    ...
    if not include_delete_at_end_models:
        ...
        if username is not None and not _verify_profile_picture_is_deleted(username):
            return False
        ...
        if snapshot_metadata_ids != commit_log_ids:
            logging.error(
                ...
                % (
                    ...
                    [snapshot_metadata_model_class.__name__ for snapshot_metadata_model_class in snapshot_metadata_model_classes],
...
def _pseudonymize_activity_models_without_associated_rights_models(
    ...
    """
    snapshot_metadata_models, commit_log_models = _collect_and_save_entity_ids_from_snapshots_and_commits(
        pending_deletion_request,
        activity_category,
    ...
    def _pseudonymize_models_transactional(
        ...
        for rights_snapshot_metadata_model in rights_snapshot_metadata_models:
            ...
            if assign_commit_message_match:
                rights_snapshot_metadata_model.commit_message = rights_domain.ASSIGN_ROLE_COMMIT_MESSAGE_TEMPLATE % (
            ...
            if deassign_commit_message_match:
                rights_snapshot_metadata_model.commit_message = rights_domain.DEASSIGN_ROLE_COMMIT_MESSAGE_TEMPLATE % (
                    pseudonymized_username,
...
def _pseudonymize_feedback_models(
    ...
    feedback_thread_models: Sequence[feedback_models.GeneralFeedbackThreadModel] = feedback_thread_model_class.query(
        datastore_services.any_of(
    ...
    general_suggestion_models: Sequence[suggestion_models.GeneralSuggestionModel] = suggestion_model_class.query(
        datastore_services.any_of(
...
def _pseudonymize_blog_post_models(
    ...
    def _pseudonymize_models_transactional(
        ...
        ] = [model for model in blog_posts_related_models if isinstance(model, blog_post_model_class)]
        ...
        ] = [model for model in blog_posts_related_models if isinstance(model, blog_post_summary_model_class)]
