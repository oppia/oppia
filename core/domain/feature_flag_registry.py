...
class Registry:
    ...
    def get_feature_flag(cls, name: str) -> feature_flag_domain.FeatureFlag:
        ...
        """
        feature_flag_config_from_storage = cls.load_feature_flag_config_from_storage(name)
        feature_flag_spec_values = FEATURE_FLAG_NAME_TO_DESCRIPTION_AND_FEATURE_STAGE.get(name)

        if feature_flag_spec_values is not None:
            feature_flag_spec = feature_flag_domain.FeatureFlagSpec(feature_flag_spec_values[0], feature_flag_spec_values[1])
        else:
            raise Exception('Feature flag not found: %s.' % name)

        if feature_flag_config_from_storage is not None:
            return feature_flag_domain.FeatureFlag(name, feature_flag_spec, feature_flag_config_from_storage)
        else:
            feature_flag_config = feature_flag_domain.FeatureFlagConfig(False, 0, [], None)
            return feature_flag_domain.FeatureFlag(name, feature_flag_spec, feature_flag_config)
    ...
    def update_feature_flag(
        ...
        feature_flag.feature_flag_config.set_force_enable_for_all_users(force_enable_for_all_users)
    ...
    @classmethod
    def load_feature_flag_config_from_storage(cls, name: str) -> Optional[feature_flag_domain.FeatureFlagConfig]:
        ...
        Returns:
            ...
            in storage.
        """
        feature_flag_config_model = config_models.FeatureFlagConfigModel.get(name, strict=False)
    ...
    @classmethod
    def _update_feature_flag_storage_model(cls, feature_flag: feature_flag_domain.FeatureFlag) -> None:
        """Updates feature flag storage model.

        Args:
            feature_flag: FeatureFlag. The feature flag domain object.
        """
        feature_flag.feature_flag_config.validate(feature_flag.feature_flag_spec.feature_stage)

        model_instance = config_models.FeatureFlagConfigModel.get(feature_flag.name, strict=False)
        ...
        model_instance.force_enable_for_all_users = feature_flag.feature_flag_config.force_enable_for_all_users
        model_instance.rollout_percentage = feature_flag.feature_flag_config.rollout_percentage
        model_instance.user_group_ids = feature_flag.feature_flag_config.user_group_ids
