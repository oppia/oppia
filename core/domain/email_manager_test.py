...
from typing import (
    Callable, DefaultDict, Dict, Final, List, Optional, Sequence, Set, Type, Union
)
...
class EmailRightsTest(test_utils.GenericTestBase):
    ...
    def test_sender_id_validation(self) -> None:
        sender_ids_to_test = [
            feconf.SYSTEM_COMMITTER_ID, self.admin_id, self.moderator_id, self.editor_id
        ]
        ...
        expected_validation_results = {
            ...
            feconf.EMAIL_INTENT_UNPUBLISH_EXPLORATION: (True, False, True, False),
    ...
    def test_role_emails_sent_are_correct(self) -> None:
        with self.can_send_editor_role_email_ctx:
            ...
            self.assertEqual(
                ...
                '%s <%s>' % (self.EDITOR_USERNAME, feconf.NOREPLY_EMAIL_ADDRESS)
            )
            self.assertEqual(
                sent_email_model.intent, feconf.EMAIL_INTENT_EDITOR_ROLE_NOTIFICATION
            )
...
class SignupEmailTests(test_utils.EmailTestBase):
    ...
    def _set_signup_email_content_platform_parameter(
        ...
        platform_parameter_registry.Registry.update_platform_parameter(
            ...
            self.admin_id, 'Updating email subject.', [
        ...
        platform_parameter_registry.Registry.update_platform_parameter(
            ...
            self.admin_id, 'Updating email body.', [
                platform_parameter_domain.PlatformParameterRule.from_dict(
                    {
                        'filters': [
                            {
    ...
    def _reset_signup_email_content_platform_parameters(self) -> None:
        ...
        platform_parameter_registry.Registry.update_platform_parameter(
            (platform_parameter_list.ParamName.SIGNUP_EMAIL_SUBJECT_CONTENT.value),
            self.admin_id, 'Resetting email subject.', [],
            platform_parameter_registry.Registry.get_platform_parameter(
                platform_parameter_list.ParamName.SIGNUP_EMAIL_SUBJECT_CONTENT.value
            ).default_value
        )
        platform_parameter_registry.Registry.update_platform_parameter(
            platform_parameter_list.ParamName.SIGNUP_EMAIL_BODY_CONTENT.value,
            self.admin_id, 'Resetting email body.', [],
            platform_parameter_registry.Registry.get_platform_parameter(
                platform_parameter_list.ParamName.SIGNUP_EMAIL_BODY_CONTENT.value
            ).default_value
        )
    ...
    def _reset_the_email_platform_params_value(self) -> None:
        ...
        platform_parameter_registry.Registry.update_platform_parameter(
            platform_parameter_list.ParamName.EMAIL_SENDER_NAME.value, self.admin_id,
        ...
        platform_parameter_registry.Registry.update_platform_parameter(
            platform_parameter_list.ParamName.EMAIL_FOOTER.value, self.admin_id,
    ...
    def test_email_not_sent_if_content_parameter_is_not_modified(self) -> None:
        ...
        with self.capture_logging(min_level=logging.ERROR) as logs:
            with log_new_error_ctx:
                ...
                self.assertEqual(
                    logs[0], 'Please ensure that the value for the admin platform '
    ...
    @test_utils.set_platform_parameters(
        [
            ...
            (platform_parameter_list.ParamName.SIGNUP_EMAIL_BODY_CONTENT, EMAIL_BODY)
        ]
    )
    def test_contents_of_signup_email_are_correct(self) -> None:
        ...
        self.assertEqual(
            messages[0].sender, 'Email Sender <%s>' % feconf.NOREPLY_EMAIL_ADDRESS
        )
...
class DuplicateEmailTests(test_utils.EmailTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.new_email_html_body = 'Hi %s,<br><br>%s<br><br>%s' % (
            ...
            'THIS IS A <b>PLACEHOLDER</b> AND SHOULD BE REPLACED.', self.new_footer
        )

        def _generate_hash_for_tests(
            unused_cls: Type[test_utils.TestBase], unused_recipient_id: str,
        ...
        with self.capture_logging(min_level=logging.ERROR) as logs:
            with duplicate_email_ctx, log_new_error_ctx:
                ...
                email_models.SentEmailModel.create(
                    self.new_user_id, self.NEW_USER_EMAIL, feconf.SYSTEM_COMMITTER_ID,
                    feconf.SYSTEM_EMAIL_ADDRESS, feconf.EMAIL_INTENT_SIGNUP,
                    self.new_email_subject, cleaned_plaintext_body,
    ...
    def test_sending_email_with_different_recipient_but_same_hash(self) -> None:
        ...
        with duplicate_email_ctx, self.generate_hash_ctx:
            ...
            email_models.SentEmailModel.create(
                'recipient_id', self.NEW_USER_EMAIL, feconf.SYSTEM_COMMITTER_ID,
                feconf.SYSTEM_EMAIL_ADDRESS, feconf.EMAIL_INTENT_SIGNUP,
                self.new_email_subject, self.new_email_html_body,
                datetime.datetime.utcnow()
            )
    ...
    def test_sending_email_with_different_subject_but_same_hash(self) -> None:
        ...
        with duplicate_email_ctx, self.generate_hash_ctx:
            ...
            email_models.SentEmailModel.create(
                self.new_user_id, self.NEW_USER_EMAIL, feconf.SYSTEM_COMMITTER_ID,
                feconf.SYSTEM_EMAIL_ADDRESS, feconf.EMAIL_INTENT_SIGNUP,
                '%s%s' % (self.new_email_subject, 1), self.new_email_html_body,
                datetime.datetime.utcnow()
            )
    ...
    def test_sending_email_with_different_body_but_same_hash(self) -> None:
        ...
        with duplicate_email_ctx, self.generate_hash_ctx:
            ...
            email_models.SentEmailModel.create(
                self.new_user_id, self.NEW_USER_EMAIL, feconf.SYSTEM_COMMITTER_ID,
                feconf.SYSTEM_EMAIL_ADDRESS, feconf.EMAIL_INTENT_SIGNUP,
                self.new_email_subject, '%s%s' % (self.new_email_html_body, 1),
                datetime.datetime.utcnow()
            )
    ...
    def test_duplicate_emails_are_sent_after_some_time_has_elapsed(self) -> None:
        ...
        with duplicate_email_ctx:
            ...
            email_models.SentEmailModel.create(
                self.new_user_id, self.NEW_USER_EMAIL, feconf.SYSTEM_COMMITTER_ID,
                feconf.SYSTEM_EMAIL_ADDRESS, feconf.EMAIL_INTENT_SIGNUP,
                self.new_email_subject, self.new_email_html_body, email_sent_time
            )
...
class FeedbackMessageBatchEmailTests(test_utils.EmailTestBase):
    ...
    def test_email_not_sent_if_can_send_transactional_emails_is_false(self) -> None:
        feedback_messages: Dict[str, email_manager.FeedbackMessagesDict] = {
            self.exploration.id: {
                'title': self.exploration.title,
                'messages': ['Message 1.1', 'Message 1.2', 'Message 1.3']
...
class SuggestionEmailTests(test_utils.EmailTestBase):
    ...
    def test_that_suggestion_emails_are_correct(self) -> None:
        ...
        with self.can_send_feedback_email_ctx:
            ...
            self.assertEqual(
                sent_email_model.intent, feconf.EMAIL_INTENT_SUGGESTION_NOTIFICATION
            )
...
class SubscriptionEmailTests(test_utils.EmailTestBase):
    ...
    def test_that_subscription_emails_are_correct(self) -> None:
        ...
        with self.can_send_subscription_email_ctx:
            ...
            self.assertEqual(
                sent_email_model.intent, feconf.EMAIL_INTENT_SUBSCRIPTION_NOTIFICATION
            )
...
class NotifyContributionAchievementEmailTests(test_utils.EmailTestBase):
    ...
    def test_that_translation_submitter_acceptance_ranking_email_is_sent(self) -> None:
        ...
        contributor_ranking_email_info = (
            suggestion_registry.ContributorMilestoneEmailInfo(
                self.user_id, 'translation', 'acceptance', 'hi', 'Initial Contributor'
            )
        )
        email_manager.send_mail_to_notify_contributor_ranking_achievement(
            contributor_ranking_email_info
    ...
    def test_that_question_submitter_acceptance_ranking_email_is_sent(self) -> None:
        ...
        contributor_ranking_email_info = (
            suggestion_registry.ContributorMilestoneEmailInfo(
                self.user_id, 'question', 'acceptance', None, 'Initial Contributor'
            )
        )
        email_manager.send_mail_to_notify_contributor_ranking_achievement(
            contributor_ranking_email_info
    ...
    def test_that_translation_reviewer_review_ranking_email_is_sent(self) -> None:
        ...
        contributor_ranking_email_info = (
            suggestion_registry.ContributorMilestoneEmailInfo(
                self.user_id, 'translation', 'review', 'hi', 'Initial Contributor'
            )
        )
        email_manager.send_mail_to_notify_contributor_ranking_achievement(
            contributor_ranking_email_info
...
class NotifyContributionDashboardReviewersEmailTests(test_utils.EmailTestBase):
    ...
    def _create_translation_suggestion_in_lang_with_html_and_datetime(
        self, language_code: str, translation_html: str,
    ...
    def _assert_email_data_stored_in_sent_email_model_is_correct(
        self, expected_email_html_body: str, reviewer_id: Optional[str],
        reviewer_email: str
    ) -> None:
        ...
        sent_email_models: Sequence[
            email_models.SentEmailModel] = email_models.SentEmailModel.get_all().filter(
                email_models.SentEmailModel.recipient_id == reviewer_id
            ).fetch()
        self.assertEqual(len(sent_email_models), 1)
        sent_email_model = sent_email_models[0]
        self.assertEqual(
            sent_email_model.subject, email_manager.
            CONTRIBUTOR_DASHBOARD_REVIEWER_NOTIFICATION_EMAIL_DATA['email_subject']
        )
    ...
    def test_email_not_sent_if_reviewer_notifications_is_disabled(self) -> None:
        with self.capture_logging(min_level=logging.ERROR) as logs:
            ...
            self.assertEqual(self.log_new_error_counter.times_called, 1)
            self.assertEqual(
                logs[0], 'The "contributor_dashboard_reviewer_emails_is_enabled" '
    ...
    def test_email_not_sent_if_reviewer_email_does_not_exist(self) -> None:
        with self.capture_logging(min_level=logging.ERROR) as logs:
            ...
            self.assertEqual(self.log_new_error_counter.times_called, 1)
            self.assertEqual(
                logs[0], 'There was no email for the given reviewer id: '
                'reviewer_id_with_no_email.'
            )
    ...
    def test_email_sent_to_reviewer_with_multi_questions_waiting_for_a_review(
        ...
        with self.log_new_error_ctx:
            with self.mock_datetime_utcnow(mocked_datetime_for_utcnow):
                (
                    email_manager.send_mail_to_notify_contributor_dashboard_reviewers(
                        [self.reviewer_1_id], [reviewable_suggestion_email_infos]
                    )
                )
    ...
    def test_email_sent_to_multi_reviewers_with_multi_suggestions_waiting(self) -> None:
        ...
        suggestion_1 = (
            self._create_translation_suggestion_in_lang_with_html_and_datetime(
                'en', '<p>Translation 1</p>', self.mocked_review_submission_datetime +
                datetime.timedelta(hours=1, minutes=1)
            )
        )
        ...
        suggestion_3 = (
            self._create_translation_suggestion_in_lang_with_html_and_datetime(
                'fr', '<p>Translation 2</p>', self.mocked_review_submission_datetime +
                datetime.timedelta(days=1, hours=1)
            )
        )
        ...
        suggestion_4 = (
            self._create_question_suggestion_with_question_html_and_datetime(
                '<p>Question 2</p>', self.mocked_review_submission_datetime +
                datetime.timedelta(days=1, hours=1)
            )
        )
        reviewer_1_suggestion_email_infos = (
...
class NotifyAdminsSuggestionsWaitingTooLongForReviewEmailTests(test_utils.EmailTestBase
    ...
    def _assert_email_data_stored_in_sent_email_model_is_correct(
        self, expected_email_html_body: str, admin_id: Optional[str], admin_email: str
    ) -> None:
        """Asserts that the created sent email model from the sent email
        contains the right information.
        """
        sent_email_models: Sequence[
            email_models.SentEmailModel] = email_models.SentEmailModel.get_all().filter(
                email_models.SentEmailModel.recipient_id == admin_id
            ).fetch()
        self.assertEqual(len(sent_email_models), 1)
        sent_email_model = sent_email_models[0]
        self.assertEqual(
    ...
    @test_utils.set_platform_parameters(
        [
            ...
            (
                platform_parameter_list.ParamName.
                ENABLE_ADMIN_NOTIFICATIONS_FOR_SUGGESTIONS_NEEDING_REVIEW, False
            )  # pylint: disable=line-too-long
        ]
    )
    def test_email_not_sent_if_notifying_admins_about_suggestions_is_disabled(
        ...
        with self.capture_logging(min_level=logging.ERROR) as logs:
            ...
            self.assertEqual(self.log_new_error_counter.times_called, 1)
            self.assertEqual(
                logs[0], 'The "notify_admins_suggestions_waiting_too_long" property '
    ...
    @test_utils.set_platform_parameters(
        [
            ...
            (
                platform_parameter_list.ParamName.
                ENABLE_ADMIN_NOTIFICATIONS_FOR_SUGGESTIONS_NEEDING_REVIEW, True
            )  # pylint: disable=line-too-long
        ]
    )
    def test_email_not_sent_if_admin_email_does_not_exist(self) -> None:
        with self.capture_logging(min_level=logging.ERROR) as logs:
            ...
            self.assertEqual(self.log_new_error_counter.times_called, 1)
            self.assertEqual(
                logs[0], 'There was no email for the given admin id: '
                'admin_id_without_email.'
            )
    ...
    def test_email_sent_to_admin_if_question_has_waited_too_long_for_a_review(
        ...
        # Make sure correct email model is stored.
        self._assert_email_data_stored_in_sent_email_model_is_correct(
            expected_email_html_body, self.admin_1_id, self.CURRICULUM_ADMIN_1_EMAIL
        )
            ...
            with self.swap(suggestion_models,
                ...
                with self.mock_datetime_utcnow(mocked_datetime_for_utcnow):
                    (
                        email_manager.
                        send_mail_to_notify_admins_suggestions_waiting_long(
                            [self.admin_1_id], [], [], reviewable_suggestion_email_infos
                        )
                    )
    ...
    def test_email_sent_to_admin_if_multi_suggestion_types_waiting_for_review(
        ...
        sent_email_models: Sequence[
            email_models.SentEmailModel] = email_models.SentEmailModel.get_all().filter(
                email_models.SentEmailModel.recipient_id == self.admin_1_id
            ).fetch()
        ...
        email_sort_fn: Callable[
            [email_models.SentEmailModel],
            str] = (lambda m: m.html_body if isinstance(m.html_body, str) else '')
...
class NotifyReviewersNewSuggestionsTests(test_utils.EmailTestBase):
    ...
    def test_email_not_sent_if_server_can_send_emails_is_false(self) -> None:
        with self.capture_logging(min_level=logging.ERROR) as logs:
            with self.log_new_error_ctx:
                reviewer_ids_by_language: DefaultDict[str,
                                                      List[str]] = DefaultDict(list)
                suggestions_by_language: DefaultDict[
                    str, List[suggestion_registry.
...
class NotifyAdminsContributorDashboardReviewersNeededTests(test_utils.EmailTestBase):
    ...
    def _assert_email_data_stored_in_sent_email_model_is_correct(
        ...
        self.assertEqual(
            sent_email_model.subject, email_manager.
            ADMIN_NOTIFICATION_FOR_REVIEWER_SHORTAGE_EMAIL_DATA['email_subject']
        )
        self.assertEqual(sent_email_model.recipient_id, admin_id)
        self.assertEqual(sent_email_model.recipient_email, admin_email)
        self.assertEqual(sent_email_model.html_body, expected_email_html_body)
    ...
    def test_email_sent_to_admins_if_mutli_suggestion_types_needing_reviewers(
        ...
        email_manager.send_mail_to_notify_admins_that_reviewers_are_needed(
            [self.admin_1_id], [self.admin_2_id], [], suggestion_types_needing_reviewers
        )
...
class QueryStatusNotificationEmailTests(test_utils.EmailTestBase):
    ...
    def test_that_correct_completion_email_is_sent(self) -> None:
        ...
        self.assertEqual(
            sent_email_model.intent, feconf.EMAIL_INTENT_QUERY_STATUS_NOTIFICATION
        )
    ...
    def test_send_user_query_email(self) -> None:
        ...
        email_manager.send_user_query_email(
            self.sender_id, self.recipient_ids, email_subject, email_body, email_intent
        )
...
class BulkEmailsTests(test_utils.EmailTestBase):
    ...
    def test_that_correct_email_is_sent(self) -> None:
        ...
        email_manager.send_user_query_email(
            self.sender_id, self.recipient_ids, email_subject, email_html_body,
...
class ModeratorActionEmailsTests(test_utils.EmailTestBase):
    ...
    def test_exception_raised_if_server_can_send_emails_is_false(self) -> None:
        with self.assertRaisesRegex(
                Exception, 'For moderator emails to be sent, please ensure that '
    ...
    def test_correct_moderator_action_email_sent(self) -> None:
        ...
        email_manager.send_moderator_action_email(
            self.moderator_id, self.recipient_id, email_intent, exploration_title,
            email_html_body
        )
...
class CDUserEmailTest(test_utils.EmailTestBase):
    ...
    def test_assign_translation_reviewer_email_for_invalid_category(self) -> None:
    ...
    def test_schema_of_new_cd_user_email_data_constant(self) -> None:
        ...
        for category_details in (email_manager.NEW_CD_USER_EMAIL_DATA.values()):
            ...
            self.assertTrue(
                'to_review' in category_details or ('to_submit' in category_details)
            )
    ...
    def test_remove_translation_reviewer_email_for_invalid_category(self) -> None:
        with self.assertRaisesRegex(Exception, 'Invalid category'):
            email_manager.send_email_to_removed_cd_user(
                self.translation_reviewer_id, 'invalid_category'
            )
...
class CurriculumAdminsChapterNotificationsReminderMailTests(test_utils.EmailTestBase):
    ...
    def test_email_sent_if_chapters_are_overdue(self) -> None:
        ...
        self.assertEqual(sent_email_model.sender_id, feconf.SYSTEM_COMMITTER_ID)
        self.assertEqual(
            sent_email_model.sender_email, '. <%s>' % feconf.NOREPLY_EMAIL_ADDRESS
        )
        self.assertEqual(
            sent_email_model.intent,
