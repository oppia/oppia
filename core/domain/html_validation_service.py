...
def add_image_id_and_image_assets_in_state_dict(state_dict, image_counter):
    """Adds image_id in state HTML, and add ImageAssets in state dict.
    This funtion is extracting HTMl of state and do 3 operations on each HTML
    content. The 3 operations and their sequence were
    ...
    Args:
        state_dict: dict. The state in dict format.
        image_counter: int. Counter for an image id.
    ...
    # Adding image_id, getting image info and removing filepath from image
    # tag present in state content
    content_html = state_dict['content']['html']
    (new_content_html, image_counter) = (
        add_image_id_in_image_tag(
            content_html, image_counter))

    image_info_list = image_info_list + (
        get_image_info_from_html(new_content_html))

    state_dict['content']['html'] = (remove_filepath_from_image_tag(
        new_content_html))

    # Adding image_id, getting image info and removing filepath from image
    # tag present in state interaction.
    if state_dict['interaction']['default_outcome']:
        ...
        image_info_list = image_info_list + (
            get_image_info_from_html(
                new_interaction_feedback_html))

        state_dict['interaction']['default_outcome']['feedback']['html'] = (
            remove_filepath_from_image_tag(new_interaction_feedback_html))

    # Adding image_id, getting image info and removing filepath from image
    # tag present in state interaction answer_groups.
    for answer_group_index, answer_group in enumerate(
        ...
        image_info_list = image_info_list + (
            get_image_info_from_html(new_answer_group_html))

        state_dict['interaction']['answer_groups'][
            answer_group_index]['outcome']['feedback']['html'] = (
                remove_filepath_from_image_tag(new_answer_group_html))
    ...
    # Adding image_id, getting image info and removing filepath from image
    # tag present in hint interaction.
    ...
    # Adding image_id, getting image info and removing filepath from image
    # tag present in solution interaction.
    if state_dict['interaction']['solution']:
        solution_html = state_dict[
            'interaction']['solution']['explanation']['html']
        (new_solution_html, image_counter) = (add_image_id_in_image_tag(
        ...
        state_dict['interaction']['solution']['explanation']['html'] = (
            remove_filepath_from_image_tag(new_solution_html))

    # Adding image_id, getting image info and removing filepath from image
    # tag present in following given below interactions.
    if state_dict['interaction']['id'] in (
            'MultipleChoiceInput', 'DragAndDropSortInput'):
        for value_index, value in enumerate(
                state_dict['interaction']['customization_args'][
                    'choices']['value']):
            (new_value, image_counter) = (add_image_id_in_image_tag(
                value, image_counter))
