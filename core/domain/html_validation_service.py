...
def convert_html_fields_in_answer_group(answer_group, conversion_fn):
    """Checks for HTML fields in an answer group and converts it according
    to the conversion function.

    Args:
        answer_group: dict. The answer group dict.
...
def convert_html_fields_in_customization_args(
        cust_arg, conversion_fn):
    """Checks for HTML fields in the customization Args and converts it
    according to the conversion function.

    Args:
        cust_arg: dict. The customization Args dict.
        conversion_fn: function. The function to be used for converting the
            HTML.
    Returns:
        cust_arg: dict. The converted customization Args dict.
    """
    if 'choices' in cust_arg.keys():
        for value_index, value in enumerate(
                cust_arg['choices']['value']):
            cust_arg['choices']['value'][value_index] = conversion_fn(value)
    return cust_arg


def convert_html_fields_in_written_translations(
        written_translations, conversion_fn):
    """Checks for HTML fields in the written translations and converts it
    according to the conversion function.

    Args:
        written_translations: dict. The written translations dict.
        conversion_fn: function. The function to be used for converting the
            HTML.
    Returns:
        written_translations: dict. The converted written translations dict.
    """
    for (content_id, language_code_to_written_translation) in (
            written_translations['translations_mapping'].items()):
    ...
    return written_translations


def convert_html_fields_in_default_outcome(default_outcome, conversion_fn):
    """Checks for HTML fields in the default outcome and converts it
    according to the conversion function.

    Args:
        default_outcome: dict. The default outcome dict.
        conversion_fn: function. The function to be used for converting the
            HTML.
    Returns:
        default_outcome: dict. The converted default outcome dict.
    """
    default_outcome['feedback']['html'] = (
        conversion_fn(default_outcome['feedback']['html']))
    return default_outcome


def convert_html_fields_in_hints(hints, conversion_fn):
    """Checks for HTML fields in the hints and converts it
    according to the conversion function.

    Args:
        hints: dict. The hints dict.
        conversion_fn: function. The function to be used for converting the
            HTML.
    Returns:
        default_outcome: dict. The converted hints dict.
    """
    for hint_index, hint in enumerate(hints):
        hints[hint_index]['hint_content']['html'] = (
            conversion_fn(hint['hint_content']['html']))
    return hints


def convert_html_fields_in_solution(solution, conversion_fn):
    """Checks for HTML fields in the solutions and converts it
    according to the conversion function.

    Args:
        solution: dict. The solution dict.
        conversion_fn: function. The function to be used for converting the
            HTML.
    Returns:
        solutions: dict. The converted solution dict.
    """
