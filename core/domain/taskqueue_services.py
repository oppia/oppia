...
def defer(fn_identifier: str, queue_name: str, *args: Any, **kwargs: Any) -> None:
    ...
    task = platform_taskqueue_services.create_http_task(queue_name=queue_name, url=feconf.TASK_URL_DEFERRED, payload=payload)
    assert task.name is not None
    cloud_task_model = create_new_cloud_task_model(new_cloud_task_model_id, task.name, fn_identifier)
...
def create_new_cloud_task_model(new_model_id: str, task_name: str, function_id: str) -> cloud_task_models.CloudTaskRunModel:
...
def update_cloud_task_run_model(
    ...
    """
    cloud_task_model = cloud_task_models.CloudTaskRunModel.get(cloud_task_run_domain_instance.task_run_id, strict=False)
    if cloud_task_model is None:
        raise ValueError('CloudTaskRunModel with id %s does not exist.' % cloud_task_run_domain_instance.task_run_id)

    cloud_task_model.latest_job_state = cloud_task_run_domain_instance.latest_job_state
    cloud_task_model.exception_messages_for_failed_runs = cloud_task_run_domain_instance.exception_messages_for_failed_runs
    cloud_task_model.current_retry_attempt = cloud_task_run_domain_instance.current_retry_attempt
    cloud_task_model.update_timestamps()
    cloud_task_model.put()
...
def get_cloud_task_run_by_model_id(
    ...
    """
    cloud_task_model = cloud_task_models.CloudTaskRunModel.get(model_id, strict=False)
...
def get_all_cloud_task_runs() -> List[cloud_task_domain.CloudTaskRun]:
    ...
    return [convert_cloud_task_run_model_to_domain_object(model) for model in cloud_task_run_models]
...
def convert_cloud_task_run_model_to_domain_object(
    ...
    model_dict: cloud_task_domain.CloudTaskRunDict = {
        ...
        'exception_messages_for_failed_runs': (cloud_task_model.exception_messages_for_failed_runs),
...
def get_cloud_task_run_by_given_params(
    ...
    Returns:
        ...
        specified queue ID.
    """
    cloud_task_run_models = cloud_task_models.CloudTaskRunModel.get_by_queue_id(queue_id)
    filtered_models = [model for model in cloud_task_run_models if (start_datetime <= model.last_updated.replace(tzinfo=datetime.timezone.utc) <= end_datetime)]
    return [convert_cloud_task_run_model_to_domain_object(model) for model in filtered_models]
