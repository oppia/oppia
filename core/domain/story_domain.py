...
class Story(object):
    ...
    @property
    def node_ids(self):
        """Returns a list of all the node IDs that are part of this
        story.

        Returns:
            list(str). List of node IDs.
        """
        return [node.id for node in self.story_contents.nodes]

    @property
    def first_node_id(self):
        """Returns the first element in the node list of the story, which
           corresponds to the first node that the user would encounter, or if
           the story is empty, returns None.

        Returns:
            str|None. The node ID of the first node,
                or None if the story is empty.
        """
        if len(self.story_contents.nodes) > 0:
    ...
    def get_next_node_id(self, completed_node_ids):
        ...
        Args:
            completed_node_ids: list(str). List of completed node
                ids.

        Returns:
            str|None. The node ID of the next node,
                or None if the story is completed.
        """
    ...
    def get_next_node_id_in_sequence(self, current_node_id):
        """Returns the node ID of the node just after the node
           corresponding to the current node id. If the user is on the
           last node, None is returned.

        Args:
            current_node_id: str. The id of node currently
                completed.

        Returns:
            str|None. The node ID of the next node,
                or None if the passed id is the last one in the story.
        """
        node_just_unlocked = None

        for index in range(0, len(self.story_contents.nodes) - 1):
            if self.story_contents.nodes[index].id == current_node_id:
                node_just_unlocked = self.story_contents.nodes[index + 1].id
                break

        return node_just_unlocked
