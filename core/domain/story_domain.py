...
class StoryNode:
    ...
    def to_dict(self) -> StoryNodeDict:
        ...
        return {
            ...
            'last_modified_msecs': (utils.get_time_in_millisecs(self.last_modified) if self.last_modified else None),
            'first_publication_date_msecs': (utils.get_time_in_millisecs(self.first_publication_date) if self.first_publication_date else None),
    ...
    def from_dict(cls, node_dict: StoryNodeDict) -> StoryNode:
        ...
        node = cls(
            ...
            (utils.convert_millisecs_time_to_datetime_object(planned_publication_date_msecs) if planned_publication_date_msecs else None),
            (utils.convert_millisecs_time_to_datetime_object(last_modified_msecs) if last_modified_msecs else None),
            (utils.convert_millisecs_time_to_datetime_object(first_publication_date_msecs) if first_publication_date_msecs else None),
...
class StoryContents:
    ...
    def from_dict(cls, story_contents_dict: StoryContentsDict) -> StoryContents:
        ...
        """
        story_contents = cls(
            [StoryNode.from_dict(story_node_dict) for story_node_dict in story_contents_dict['nodes']],
...
class Story:
    ...
    def create_default_story(
        ...
        if thumbnail_filename is not None:
            raw_image = b''
            with open(f'core/tests/data/{thumbnail_filename}', 'rt', encoding='utf-8') as svg_file:
    ...
    @classmethod
    def _convert_story_contents_v5_dict_to_v6_dict(cls, story_contents_dict: StoryContentsDict) -> StoryContentsDict:
    ...
    def update_story_contents_from_model(
        ...
        conversion_fn = getattr(
            cls,
            '_convert_story_contents_v%s_dict_to_v%s_dict' % (current_version, current_version + 1),
        )
...
class StorySummary:
    ...
    def to_dict(self) -> StorySummaryDict:
        ...
        return {
            ...
            'thumbnail_bg_color': self.thumbnail_bg_color,
            'url_fragment': self.url_fragment,
            'story_model_created_on': utils.get_time_in_millisecs(self.story_model_created_on),
            'story_model_last_updated': utils.get_time_in_millisecs(self.story_model_last_updated),
        }
