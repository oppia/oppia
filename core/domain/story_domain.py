...
class StoryNode:
    ...
    def validate(self) -> None:
        ...
        if self.exploration_id == '':
            raise utils.ValidationError(
                'Expected exploration ID to not be an empty string, received %s'
                % self.exploration_id
            )
        ...
        for skill_id in self.prerequisite_skill_ids:
            if not isinstance(skill_id, str):
                raise utils.ValidationError(
                    'Expected each prerequisite skill id to be a string, received %s'
                    % skill_id
                )
        ...
        for skill_id in self.acquired_skill_ids:
            if not isinstance(skill_id, str):
                raise utils.ValidationError(
                    'Expected each acquired skill id to be a string, received %s'
                    % skill_id
                )
        ...
        for skill_id in self.prerequisite_skill_ids:
            if skill_id in self.acquired_skill_ids:
                raise utils.ValidationError(
                    'Expected prerequisite skill ids and acquired skill ids to be mutually exclusive. The skill_id %s intersects '
                    % skill_id
                )
        ...
        if self.planned_publication_date and (
            ...
            raise utils.ValidationError(
                'Expected planned publication date to be a datetime, received %s'
                % self.planned_publication_date
            )
        ...
        if self.last_modified and (
            ...
            raise utils.ValidationError(
                'Expected last modified to be a datetime, received %s'
                % self.last_modified
            )
        ...
        if self.first_publication_date and (
            ...
            raise utils.ValidationError(
                'Expected first publication date to be a datetime, received %s'
...
class Story:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.story_contents_schema_version, int):
            raise utils.ValidationError(
                'Expected story contents schema version to be an integer, received %s'
    ...
    def update_thumbnail_filename(
        ...
        if fs.isfile(filepath):
            ...
            raise Exception(
                'The thumbnail %s for story with id %s does not exist in the filesystem.'
    ...
    def add_node(self, desired_node_id: str, node_title: str) -> None:
        ...
        if self.story_contents.next_node_id != desired_node_id:
            raise Exception(
                'The node id %s does not match the expected next node id for the story.'
                % desired_node_id
            )
    ...
    def update_node_thumbnail_filename(
        ...
        if fs.isfile(filepath):
            ...
            raise Exception(
                'The thumbnail %s for story node with id %s does not exist in the filesystem.'
                % (new_thumbnail_filename, self.id)
            )
    ...
    def rearrange_node_in_story(self, from_index: int, to_index: int) -> None:
        ...
        if not isinstance(from_index, int):
            raise Exception(
                'Expected from_index value to be a number, received %s'
                % from_index
            )
