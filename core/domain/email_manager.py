...
def send_email_to_removed_cd_user(
    ...
    if category == constants.CD_USER_RIGHTS_CATEGORY_SUBMIT_QUESTION:
        email_subject = (
            'You have been unassigned as a %s submitter'
            % (category_data['category'])
        )
    else:
        email_subject = (
            'You have been unassigned as a %s reviewer'
            % (category_data['category'])
        )
    ...
    email_body_template = 'Hi %s,<br><br>The Oppia team has removed you from the %s. You won\'t be able to %s any more, but you can still contribute %ss through the <a href="https://www.oppia.org/contributor-dashboard">Contributor Dashboard</a>.<br><br>Thanks, and happy contributing!<br><br>Best wishes,<br>The Oppia Community'
...
def send_emails_to_voiceover_admins(
    ...
    optional_message = (
        (
            '<br>You have also been cc’d on a separate email, sent to the voiceover tech lead, to address the failed voiceover synthesis. Please follow up on that email as needed.<br><br>'
        )
