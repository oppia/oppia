...
# This mock should be used only in and
# InteractionCustomizationArgsValidationJob.  The first job validates the html
# strings and produces as output the invalid strings. If we do not use mock
# validation for rte while updating states and saving exploration, the
# validation for subtitled html in state will fail, thereby resulting in failure
# of job.  The second job validates the customization args in html and if the
# mock is not used while updating states and saving explorations, the validation
# for subtitled html in state will fail, thereby resulting in failure of job.
