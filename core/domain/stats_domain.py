        ...
        for state_name in state_stats_mapping:
            for state_stats_property in state_stats_properties:
                if state_stats_property not in state_stats_mapping[state_name]:
                    raise utils.ValidationError(
                        '%s not in state stats mapping of %s in aggregated stats dict.'
                        % (state_stats_property, state_name)
                    )
                if not isinstance(
...
class Playthrough:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.issue_customization_args, dict):
            raise utils.ValidationError(
                'Expected issue_customization_args to be a dict, received %s'
    ...
    def validate(self) -> None:
        ...
        for playthrough_id in self.playthrough_ids:
            if not isinstance(playthrough_id, str):
                raise utils.ValidationError(
                    'Expected each playthrough_id to be a string, received %s'
                    % type(playthrough_id)
                )
...
class SubmittedAnswer:
    ...
    def validate(self) -> None:
        ...
        if self.rule_spec_str is not None and not isinstance(
            ...
            raise utils.ValidationError(
                'Expected rule_spec_str to be either None or a string, received %s'
                % str(self.rule_spec_str)
            )

        if self.answer_str is not None and not isinstance(self.answer_str, str):
            raise utils.ValidationError(
                'Expected answer_str to be either None or a string, received %s'
                % str(self.answer_str)
            )
...
class StateAnswersCalcOutput:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(
            ...
            raise utils.ValidationError(
                'Expected calculation output to be one of AnswerFrequencyList or CategorizedAnswerFrequencyLists, encountered: %s'
    ...
    def validate(self) -> None:
        ...
        if self.entity_type == feconf.ENTITY_TYPE_EXPLORATION:
            if len(split_state_reference) != 2:
                raise utils.ValidationError(
                    'For entity type exploration, the state reference should be of the form \'exp_id:state_name\', but received %s'
                    % (self.state_reference)
                )
        elif self.entity_type == feconf.ENTITY_TYPE_QUESTION:
            if len(split_state_reference) != 1:
                raise utils.ValidationError(
                    'For entity type question, the state reference should be of the form \'question_id\', but received %s'
                    % (self.state_reference)
                )
        else:
            raise utils.ValidationError(
        ...
        if not isinstance(self.learner_answer_info_list, list):
            raise utils.ValidationError(
                'Expected learner_answer_info_list to be a list, received %s'
        ...
        if not isinstance(self.learner_answer_info_schema_version, int):
            raise utils.ValidationError(
                'Expected learner_answer_info_schema_version to be an int, received %s'
                % self.learner_answer_info_schema_version
            )

        if not isinstance(self.accumulated_answer_info_json_size_bytes, int):
            raise utils.ValidationError(
                'Expected accumulated_answer_info_json_size_bytes to be an int received %s'
                % self.accumulated_answer_info_json_size_bytes
            )
