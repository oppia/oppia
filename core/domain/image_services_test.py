...
class ImageServicesUnitTests(test_utils.GenericTestBase):
    ...
    def test_invalid_scaling_factor_triggers_value_error(self) -> None:
        value_exception = self.assertRaisesRegex(
            ValueError,
            re.escape('Scaling factor should be in the interval (0, 1], received 1.100000.'),
        )
        ...
        value_exception = self.assertRaisesRegex(
            ValueError,
            re.escape('Scaling factor should be in the interval (0, 1], received 0.000000.'),
        )
        with value_exception:
        ...
        value_exception = self.assertRaisesRegex(
            ValueError,
            re.escape('Scaling factor should be in the interval (0, 1], received -1.000000.'),
        )
        with value_exception:
