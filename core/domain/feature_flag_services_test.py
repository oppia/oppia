...
class FeatureFlagServiceTest(test_utils.GenericTestBase):
    ...
    def _swap_feature_flags_list(
        ...
        swap_all_feature_flags = self.swap(feature_services, 'ALL_FEATURE_FLAGS', self.feature_name_enums)

        swap_all_feature_names_set = self.swap(feature_services, 'ALL_FEATURES_NAMES_SET', set(self.feature_names))
    ...
    def setUp(self) -> None:
        ...
        self.swap_all_feature_names_set = self.swap(feature_services, 'ALL_FEATURES_NAMES_SET', set(self.feature_names))
        ...
        self.dev_feature_flag = feature_flag_domain.FeatureFlag(
            FeatureNames.FEATURE_A.value,
            feature_flag_domain.FeatureFlagSpec('a feature in dev stage', FeatureStages.DEV),
            feature_flag_domain.FeatureFlagConfig(False, 0, [], None),
        )
        self.test_feature_flag = feature_flag_domain.FeatureFlag(
            FeatureNames.FEATURE_B.value,
            feature_flag_domain.FeatureFlagSpec('a feature in test stage', FeatureStages.TEST),
            feature_flag_domain.FeatureFlagConfig(False, 0, [], None),
        )
        self.prod_feature_flag = feature_flag_domain.FeatureFlag(
            FeatureNames.FEATURE_C.value,
            feature_flag_domain.FeatureFlagSpec('a feature in prod stage', FeatureStages.PROD),
            feature_flag_domain.FeatureFlagConfig(False, 0, [], None),
        )

        with self.swap_all_feature_names_set:
            with self.swap_name_to_description_feature_stage_registry_dict:
                feature_services.update_feature_flag(self.dev_feature_flag.name, True, 0, [])
                feature_services.update_feature_flag(self.test_feature_flag.name, True, 0, [])
                feature_services.update_feature_flag(self.prod_feature_flag.name, True, 0, [])

    def test_get_all_feature_flags_returns_correct_feature_flags(self) -> None:
        swap_name_to_description_feature_stage_registry_dict = self._swap_name_to_description_feature_stage_registry()
        with swap_name_to_description_feature_stage_registry_dict:
            expected_feature_flags_dict = [
                ...
                registry.Registry.get_feature_flag(self.test_feature_flag.name).to_dict(),
                registry.Registry.get_feature_flag(self.prod_feature_flag.name).to_dict(),
            ]
        swap_all_feature_flags, swap_all_feature_names_set = self._swap_feature_flags_list()
        with swap_all_feature_flags, swap_all_feature_names_set:
            with self.swap_name_to_description_feature_stage_dict:
                self.assertEqual(
                    [feature_flag.to_dict() for feature_flag in feature_services.get_all_feature_flags()],
                    expected_feature_flags_dict,
                )
    ...
    def test_feature_flag_is_correctly_fetched_when_not_present_in_storage(
        ...
        swap_all_feature_flags = self.swap(feature_services, 'ALL_FEATURE_FLAGS', feature_flag_name_enums)
        swap_all_feature_names_set = self.swap(feature_services, 'ALL_FEATURES_NAMES_SET', set(feature_flag_names))
    ...
    def test_get_all_features_dicts_raises_error_when_feature_not_present(
        ...
        with swap_all_feature_flags:
            with self.swap_name_to_description_feature_stage_dict:
                with self.assertRaisesRegex(Exception, 'Feature flag not found: feature_d'):
        ...
        with swap_all_feature_flags, swap_all_feature_names_set:
            with swap_name_to_description_feature_stage_registry_dict:
                with self.assertRaisesRegex(
                    ...
                    feature_services.is_feature_flag_enabled('feature_that_does_not_exist', self.owner_id)

    def test_updating_non_existing_feature_results_in_error(self) -> None:
        swap_name_to_description_feature_stage_registry_dict = self._swap_name_to_description_feature_stage_registry()
        with swap_name_to_description_feature_stage_registry_dict:
            with self.assertRaisesRegex(Exception, 'Unknown feature flag: unknown_feature'):
                feature_services.update_feature_flag('unknown_feature', True, 0, [])
    ...
    def test_get_all_feature_flag_configs_in_dev_returns_correct_values(
        ...
        with swap_all_feature_flags, swap_all_feature_names_set:
            with (
                self.swap_name_to_description_feature_stage_dict,
                self.swap(constants, 'DEV_MODE', True),
            ):
                self.assertEqual(
                    feature_services.evaluate_all_feature_flag_configs(self.owner_id),
                    {
        ...
        with swap_all_feature_flags, swap_all_feature_names_set:
            with swap_name_to_description_feature_stage_registry_dict:
                with self.swap(constants, 'DEV_MODE', True):
                    self.assertTrue(feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, self.owner_id))
    ...
    def test_evaluate_test_feature_flag_for_dev_server_returns_true(
        ...
        with swap_all_feature_flags, swap_all_feature_names_set:
            with swap_name_to_description_feature_stage_registry_dict:
                with self.swap(constants, 'DEV_MODE', True):
                    self.assertTrue(feature_services.is_feature_flag_enabled(self.test_feature_flag.name, self.owner_id))
    ...
    def test_evaluate_prod_feature_flag_for_dev_server_returns_true(
        ...
        with swap_all_feature_flags, swap_all_feature_names_set:
            with self.swap(constants, 'DEV_MODE', True):
                with swap_name_to_description_feature_stage_registry_dict:
                    self.assertTrue(feature_services.is_feature_flag_enabled(self.prod_feature_flag.name, self.owner_id))
            ...
            with self.swap(constants, 'DEV_MODE', False):
                with swap_name_to_description_feature_stage_registry_dict:
                    with self.swap(feconf, 'ENV_IS_OPPIA_ORG_PRODUCTION_SERVER', False):
                        self.assertFalse(feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, self.owner_id))
    ...
    def test_evaluate_prod_feature_flag_for_test_server_returns_true(
        ...
        swap_all_feature_flags, swap_all_feature_names_set = self._swap_feature_flags_list()
        swap_name_to_description_feature_stage_registry_dict = self._swap_name_to_description_feature_stage_registry()
        with swap_all_feature_flags, self.swap(constants, 'DEV_MODE', False):
            with swap_name_to_description_feature_stage_registry_dict:
                with (
                    swap_all_feature_names_set,
                    self.swap(feconf, 'ENV_IS_OPPIA_ORG_PRODUCTION_SERVER', False),
                ):
                    self.assertTrue(feature_services.is_feature_flag_enabled(self.prod_feature_flag.name, self.owner_id))
    ...
    def test_evaluate_dev_feature_flag_for_prod_server_returns_false(
        ...
        with swap_all_feature_flags, self.swap(constants, 'DEV_MODE', False):
            with swap_name_to_description_feature_stage_registry_dict:
                with (
                    swap_all_feature_names_set,
                    self.swap(feconf, 'ENV_IS_OPPIA_ORG_PRODUCTION_SERVER', True),
                ):
                    self.assertFalse(feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, self.owner_id))
        ...
        with swap_all_feature_flags, self.swap(constants, 'DEV_MODE', False):
            with swap_name_to_description_feature_stage_registry_dict:
                with (
                    ...
                    self.assertFalse(feature_services.is_feature_flag_enabled(self.test_feature_flag.name, self.owner_id))
    ...
    def test_feature_flag_enable_for_logged_out_user_with_force_enable_property(
        ...
        swap_name_to_description_feature_stage_registry_dict = self._swap_name_to_description_feature_stage_registry()
        with swap_name_to_description_feature_stage_registry_dict:
            self.assertTrue(feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, self.LOGGED_OUT_USER_ID))
    ...
    def test_feature_flag_not_enabled_for_logged_out_user(self) -> None:
        ...
        with swap_all_feature_names_set:
            with swap_name_to_description_feature_stage_registry_dict:
                feature_services.update_feature_flag(self.dev_feature_flag.name, False, 0, [])
                self.assertFalse(feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, self.LOGGED_OUT_USER_ID))
    ...
    def test_feature_flag_enabled_for_given_user_groups(self) -> None:
        (user_1_id, user_2_id, user_3_id) = self._signup_multiple_users_and_return_ids()
        user_models.UserGroupModel(
            id=self.USER_GROUP_1,
            name='USER_GROUP_1',
            user_ids=[user_1_id, user_2_id, user_3_id],
        ).put()
        user_models.UserGroupModel(
            id=self.USER_GROUP_2,
            name='USER_GROUP_2',
        ...
        with swap_all_feature_names_set:
            with swap_name_to_description_feature_stage_registry_dict:
                ...
                self.assertTrue(feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, user_1_id))
    ...
    def test_feature_flag_disabled_if_user_not_in_user_groups(self) -> None:
        ...
        with swap_all_feature_names_set:
            with swap_name_to_description_feature_stage_registry_dict:
                feature_services.update_feature_flag(self.dev_feature_flag.name, False, 0, [self.USER_GROUP_2])
                self.assertFalse(feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, 'user_id'))
    ...
    def test_feature_flag_config_is_same_for_user_with_every_retrieval(
        ...
        with swap_name_to_description_feature_stage_registry_dict:
            with swap_all_feature_names_set:
                feature_services.update_feature_flag(self.dev_feature_flag.name, False, 50, [])
                initial_user1_value = feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, user_1_id)
                initial_user2_value = feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, user_2_id)
                initial_user3_value = feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, user_3_id)
                for _ in range(500):
                    self.assertEqual(
                        initial_user1_value,
                        feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, user_1_id),
                    )
                    self.assertEqual(
                        initial_user2_value,
                        feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, user_2_id),
                    )
                    self.assertEqual(
                        initial_user3_value,
                        feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, user_3_id),
                    )
    ...
    def test_feature_flag_enabled_for_users_in_5_perc_when_increased_to_10_perc(
        ...
        with swap_name_to_description_feature_stage_registry_dict:
            with swap_all_feature_names_set:
                feature_services.update_feature_flag(self.dev_feature_flag.name, False, 5, [])
                for user_id in user_ids_list:
                    feature_is_enabled = feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, user_id)
                    if feature_is_enabled:
                        count_feature_flag_enabled_for_5_perc += 1
                        user_ids_for_which_feature_flag_enabled_for_5_perc.add(user_id)
                ...
                # (50 - 4 * 6.89, 50 + 4 * 6.89)
                self.assertTrue(count_feature_flag_enabled_for_5_perc in list(range(22, 78)))

                feature_services.update_feature_flag(self.dev_feature_flag.name, False, 10, [])
                for user_id in user_ids_list:
                    feature_is_enabled = feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, user_id)
                    if feature_is_enabled:
                        count_feature_flag_enabled_for_10_perc += 1
                        user_ids_for_which_feature_flag_enabled_for_10_perc.add(user_id)
                ...
                self.assertTrue(count_feature_flag_enabled_for_10_perc in list(range(62, 138)))

                self.assertTrue(user_ids_for_which_feature_flag_enabled_for_5_perc.issubset(user_ids_for_which_feature_flag_enabled_for_10_perc))
        ...
        with swap_name_to_description_feature_stage_registry_dict:
            with swap_all_feature_names_set:
                ...
                for user_id in user_ids:
                    feature_status_for_users.append(feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, user_id))
    ...
    def test_feature_flag_enabled_for_100_perc_logged_in_users(self) -> None:
        ...
        with swap_name_to_description_feature_stage_registry_dict:
            with swap_all_feature_names_set:
                feature_services.update_feature_flag(self.dev_feature_flag.name, False, 100, [])
                for user_id in user_ids:
                    feature_status_for_users.append(feature_services.is_feature_flag_enabled(self.dev_feature_flag.name, user_id))
