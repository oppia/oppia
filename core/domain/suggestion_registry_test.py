...
ChangeType = Dict[str, Union[str, float, Dict[str, Union[str, int,
...
class BaseSuggestionUnitTests(test_utils.GenericTestBase):
    ...
    def test_base_class_accept_raises_error(self) -> None:
        with self.assertRaisesRegex(
            NotImplementedError, 'Subclasses of BaseSuggestion should implement accept.'
        ):
            self.base_suggestion.accept('test_message')

    def test_base_class_pre_accept_validate_raises_error(self) -> None:
        with self.assertRaisesRegex(
            NotImplementedError, 'Subclasses of BaseSuggestion should implement'
            ' pre_accept_validate.'
        ):
...
class SuggestionEditStateContentUnitTests(test_utils.GenericTestBase):
    ...
    def test_create_suggestion_edit_state_content(self) -> None:
        ...
        observed_suggestion = suggestion_registry.SuggestionEditStateContent(
            ...
            expected_suggestion_dict['status'], self.author_id, self.reviewer_id,
    ...
    def test_validate_target_type(self) -> None:
        ...
        with self.assertRaisesRegex(
            utils.ValidationError, 'Expected target_type to be among allowed choices'
        ):
            suggestion.validate()
    ...
    def test_validate_target_version_at_submission(self) -> None:
        ...
        with self.assertRaisesRegex(
            utils.ValidationError, 'Expected target_version_at_submission to be an int'
        ):
            suggestion.validate()
    ...
    def test_validate_author_id_format(self) -> None:
        ...
        with self.assertRaisesRegex(
            utils.ValidationError, 'Expected author_id to be in a valid user ID format'
        ):
            suggestion.validate()
    ...
    def test_validate_change(self) -> None:
        ...
        with self.assertRaisesRegex(
            utils.ValidationError, 'Expected change_cmd to be an ExplorationChange'
        ):
            suggestion.validate()
    ...
    def test_get_all_html_content_strings(self) -> None:
        ...
        suggestion = suggestion_registry.SuggestionEditStateContent(
            self.suggestion_dict['suggestion_id'], self.suggestion_dict['target_id'],
            self.suggestion_dict['target_version_at_submission'],
            self.suggestion_dict['status'], self.author_id, self.reviewer_id,
            change_dict, self.suggestion_dict['score_category'],
            self.suggestion_dict['language_code'], False, self.fake_date, self.fake_date
        )
    ...
    def test_convert_html_in_suggestion_change(self) -> None:
        ...
        suggestion = suggestion_registry.SuggestionEditStateContent(
            ...
            self.suggestion_dict['target_version_at_submission'],
            self.suggestion_dict['status'], self.author_id, self.reviewer_id,
            change_cmd, self.suggestion_dict['score_category'],
            self.suggestion_dict['language_code'], False, self.fake_date, self.fake_date
        )
...
class SuggestionTranslateContentUnitTests(test_utils.GenericTestBase):
    ...
    def test_pre_update_validate_fails_for_invalid_change_cmd(self) -> None:
        ...
        with self.assertRaisesRegex(
            utils.ValidationError, 'The new change_cmd cmd must be equal to %s' %
    ...
    def test_validate_language_code_fails_when_language_codes_do_not_match(
        ...
        with self.assertRaisesRegex(
            utils.ValidationError, 'Expected language_code to be %s, '
    ...
    def test_accept_suggestion_adds_translation_in_exploration(self) -> None:
        ...
        suggestion = suggestion_registry.SuggestionTranslateContent(
            ...
            self.suggestion_dict['target_version_at_submission'],
            self.suggestion_dict['status'], self.author_id, self.reviewer_id,
            self.suggestion_dict['change_cmd'], self.suggestion_dict['score_category'],
            self.suggestion_dict['language_code'], False, self.fake_date, self.fake_date
        )
    ...
    def test_accept_suggestion_with_psedonymous_author_adds_translation(self) -> None:
        ...
        suggestion = suggestion_registry.SuggestionTranslateContent(
            ...
            expected_suggestion_dict['target_id'],
            expected_suggestion_dict['target_version_at_submission'],
            expected_suggestion_dict['status'], self.PSEUDONYMOUS_ID, self.reviewer_id,
            expected_suggestion_dict['change_cmd'],
            expected_suggestion_dict['score_category'],
            expected_suggestion_dict['language_code'], False, self.fake_date,
            self.fake_date
...
TestChangeDictType = Dict[str, Union[str, float, Dict[str, Union[state_domain.StateDict,
                                                                 int, str, List[str]]]]]
...
class SuggestionAddQuestionTest(test_utils.GenericTestBase):
    ...
    def test_validate_change_type(self) -> None:
        ...
        with self.assertRaisesRegex(
            utils.ValidationError, 'Expected change_cmd to be an '
    ...
    def test_validate_change_question_dict(self) -> None:
        ...
        with self.assertRaisesRegex(
            utils.ValidationError, 'Expected change_cmd to contain question_dict'
        ):
            suggestion.validate()
    ...
    def test_validate_change_question_state_data_schema_version(self) -> None:
        ...
        with self.assertRaisesRegex(
            utils.ValidationError, 'Expected question state schema version to be %s, '
    ...
    def test_validate_change_skill_difficulty_none(self) -> None:
        ...
        with self.assertRaisesRegex(
            utils.ValidationError, 'Expected change_cmd to contain skill_difficulty'
        ):
            suggestion.validate()
    ...
    def test_validate_change_skill_difficulty_invalid_value(self) -> None:
        ...
        with self.assertRaisesRegex(
            utils.ValidationError, 'Expected change_cmd skill_difficulty to be one of '
        ):
            suggestion.validate()
    ...
    def test_pre_update_validate_change_cmd(self) -> None:
        ...
        # inputs that we can normally catch by typing.
        with self.assertRaisesRegex(
            utils.ValidationError, 'The new change_cmd cmd must be equal to '
    ...
    def test_pre_update_validate_complains_if_nothing_changed(self) -> None:
        ...
        suggestion = suggestion_registry.SuggestionAddQuestion(
            ...
            self.author_id, self.reviewer_id, change, 'question.topic_1', 'en', False,
        ...
        with self.assertRaisesRegex(
            ...
            suggestion.pre_update_validate(
                question_domain.
                CreateNewFullySpecifiedQuestionSuggestionCmd(new_change)
            )
    ...
    def test_validate_language_code_fails_when_language_code_is_set_to_none(
        ...
        with self.assertRaisesRegex(
            utils.ValidationError, 'Expected language_code to be en, received None'
        ):
            suggestion.validate()
    ...
    def test_convert_html_in_suggestion_change(self) -> None:
        ...
        suggestion = suggestion_registry.SuggestionAddQuestion(
            ...
            suggestion_dict['target_version_at_submission'], suggestion_dict['status'],
            self.author_id, self.reviewer_id, suggestion_dict['change_cmd'],
            suggestion_dict['score_category'], suggestion_dict['language_code'], False,
            self.fake_date, self.fake_date
        )
        suggestion.convert_html_in_suggestion_change(
            html_validation_service.add_math_content_to_math_rte_components
    ...
    def test_accept_suggestion_with_image_region_interactions(self) -> None:
        ...
        fs_services.save_original_and_compressed_versions_of_image(
            'image.png', 'question_suggestions', 'skill1', original_image_content,
            'image', True
        )
        ...
        image_and_region_ca_dict: domain.ImageAndRegionDict = {
            ...
            'labeledRegions': [{
                ...
                'region': {
                    'regionType':
                        'Rectangle',
                    'area': [[0.2644628099173554, 0.21807065217391305],
    ...
    def test_contructor_updates_state_shema_in_change_cmd(self) -> None:
        ...
        suggestion = suggestion_registry.SuggestionAddQuestion(
            ...
            self.author_id, 'test_reviewer', change, score_category, 'en', False,
            self.fake_date, self.fake_date
        )
        # Ruling out the possibility of any other type for mypy type checking.
        assert isinstance(suggestion.change_cmd.question_dict, dict)
    ...
    def test_contructor_raise_exception_for_invalid_state_shema_version(self) -> None:
        ...
        with self.assertRaisesRegex(
            utils.ValidationError, 'Expected state schema version to be in between 25'
        ):
...
class CommunityContributionStatsUnitTests(test_utils.GenericTestBase):
    ...
    def test_initial_object_with_valid_arguments_has_correct_properties(self) -> None:
        community_contribution_stats = (
            suggestion_registry.CommunityContributionStats(
                ...
                self.question_reviewer_count, self.question_suggestion_count
            )
        )
    ...
    def test_validate_question_reviewer_count_fails_for_negative_count(self) -> None:
        ...
        with self.assertRaisesRegex(
            ...
            'received: %s.' % (community_contribution_stats.question_reviewer_count)
        ):
            community_contribution_stats.validate()
    ...
    def test_validate_question_suggestion_count_fails_for_negative_count(self) -> None:
        ...
        with self.assertRaisesRegex(
            ...
            'received: %s.' % (community_contribution_stats.question_suggestion_count)
        ):
            community_contribution_stats.validate()
    ...
    def test_initial_object_with_valid_arguments_has_correct_properties(self) -> None:
        reviewable_suggestion_email_info = (
            suggestion_registry.ReviewableSuggestionEmailInfo(
                self.suggestion_type, self.language_code, self.suggestion_content,
                self.submission_datetime
            )
        )

        self.assertEqual(
            reviewable_suggestion_email_info.suggestion_type, self.suggestion_type
        )
        self.assertEqual(
            reviewable_suggestion_email_info.language_code, self.language_code
        )
        self.assertEqual(
            reviewable_suggestion_email_info.suggestion_content, self.suggestion_content
        )
        self.assertEqual(
...
class TranslationReviewStatsUnitTests(test_utils.GenericTestBase):
    ...
    def test_create_translation_review_stats(self) -> None:
        ...
        actual_stats = suggestion_registry.TranslationReviewStats(
            ...
            self.REVIEWED_TRANSLATIONS_COUNT, self.REVIEWED_TRANSLATION_WORD_COUNT,
            self.ACCEPTED_TRANSLATIONS_COUNT, self.ACCEPTED_TRANSLATION_WORD_COUNT,
            self.ACCEPTED_TRANSLATIONS_WITH_REVIEWER_EDITS_COUNT,
...
class QuestionContributionStatsUnitTests(test_utils.GenericTestBase):
    ...
    def test_create_question_contribution_stats(self) -> None:
        ...
        actual_stats = suggestion_registry.QuestionContributionStats(
            self.CONTRIBUTOR_USER_ID, self.TOPIC_ID, self.SUBMITTED_QUESTION_COUNT,
...
class QuestionReviewStatsUnitTests(test_utils.GenericTestBase):
    ...
    def test_create_question_review_stats(self) -> None:
        ...
        actual_stats = suggestion_registry.QuestionReviewStats(
            self.CONTRIBUTOR_USER_ID, self.TOPIC_ID, self.REVIEWED_QUESTIONS_COUNT,
            self.ACCEPTED_QUESTIONS_COUNT,
            self.ACCEPTED_QUESTIONS_WITH_REVIEWER_EDITS_COUNT,
            self.FIRST_CONTRIBUTION_DATE, self.LAST_CONTRIBUTION_DATE
        )
...
class ContributorMilestoneEmailInfoUnitTests(test_utils.GenericTestBase):
    ...
    def test_create_contribution_milestone_email_info(self) -> None:
        actual_info = suggestion_registry.ContributorMilestoneEmailInfo(
            self.CONTRIBUTOR_USER_ID, self.CONTRIBUTION_TYPE, self.CONTRIBUTION_SUBTYPE,
            self.LANGUAGE_CODE, self.RANK_NAME
        )
...
class ContributorStatsSummaryUnitTests(test_utils.GenericTestBase):
    ...
    def test_create_contribution_stats_summary(self) -> None:
        ...
        translation_contribution_stats = (
            ...
            self.LANGUAGE_CODE, self.CONTRIBUTOR_USER_ID, self.TOPIC_ID,
            self.SUBMITTED_TRANSLATIONS_COUNT, self.SUBMITTED_TRANSLATION_WORD_COUNT,
            self.ACCEPTED_TRANSLATIONS_COUNT,
            (self.ACCEPTED_TRANSLATIONS_WITHOUT_REVIEWER_EDITS_COUNT),
            self.ACCEPTED_TRANSLATION_WORD_COUNT, self.REJECTED_TRANSLATIONS_COUNT,
        ...
        contribution_summary = suggestion_registry.ContributorStatsSummary(
            ...
            [question_contribution_stats], [translation_review_stats],
