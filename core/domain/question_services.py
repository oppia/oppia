...
def _save_question(
    ...
    if not change_list:
        raise Exception(
            'Unexpected error: received an invalid change list when trying to save question %s: %s'
            % (question.id, change_list)
        )
...
def update_question(
    ...
    if (version is not None) and (version < updated_question.version):
        raise Exception(
            'Unexpected error: trying to update version %s of question from version %s. Please reload the page and try again.'
            % (version, updated_question.version)
        )
    if (version is not None) and (version > updated_question.version):
        raise Exception(
            'Trying to update version %s of question from version %s, which is too old. Please reload the page and try again.'
            % (version, updated_question.version)
        )
