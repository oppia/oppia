...
def link_multiple_skills_for_question(
    user_id: str, question_id: str, skill_ids: List[str],
    ...
    _update_linked_skill_ids_of_question(
        user_id, question_id, list(set(new_linked_skill_ids)), question.linked_skill_ids
    )
...
def update_question_skill_link_difficulty(
    ...
    question_skill_link_id = (
        question_models.QuestionSkillLinkModel.get_model_id(question_id, skill_id)
    )
...
def _update_linked_skill_ids_of_question(
    user_id: str, question_id: str, new_linked_skill_ids: List[str],
...
def get_total_question_count_for_skill_ids(skill_ids: List[str]) -> int:
    ...
    question_count = (
        question_skill_link_model.get_total_question_count_for_skill_ids(skill_ids)
    )
...
def get_questions_by_skill_ids(
    total_question_count: int, skill_ids: List[str], require_medium_difficulty: bool
...
def get_question_skill_link_from_model(
    ...
    return question_domain.QuestionSkillLink(
        ...
        skill_description, question_skill_link_model.skill_difficulty
    )
...
def get_question_by_id(question_id: str, *,
                       strict: Literal[False]) -> Optional[question_domain.Question]:
    ...


def get_question_by_id(question_id: str,
                       strict: bool = True) -> Optional[question_domain.Question]:
...
def get_question_skill_links_of_skill(
    ...
    question_skill_links = [
        get_question_skill_link_from_model(model, skill_description) for model in
        question_models.QuestionSkillLinkModel.get_models_by_skill_id(skill_id)
    ]
...
def replace_skill_id_for_all_questions(
    ...
    old_question_skill_link_models = (
        question_models.QuestionSkillLinkModel.get_models_by_skill_id(curr_skill_id)
    )
...
def get_displayable_question_skill_link_details(
...
) -> Tuple[List[Optional[question_domain.QuestionSummary]],
    ...
    question_skill_link_models = (
        question_models.QuestionSkillLinkModel.get_question_skill_links_by_skill_ids(
            ...
            if change.cmd == question_domain.CMD_UPDATE_QUESTION_PROPERTY:
                if (change.property_name ==
                    ...
                    update_linked_skill_ids_cmd = cast(
                        question_domain.UpdateQuestionPropertyLinkedSkillIdsCmd, change
                    )
...
def _save_question(
    ...
    change_list: List[question_domain.QuestionChange], commit_message: str
) -> None:
...
def get_question_summary_from_model(
    ...
    return question_domain.QuestionSummary(
        ...
        question_summary_model.misconception_ids, question_summary_model.interaction_id,
...
def untag_deleted_misconceptions(
    committer_id: str, skill_id: str, skill_description: str,
...
def populate_question_model_fields(
    question_model: question_models.QuestionModel, question: question_domain.Question
