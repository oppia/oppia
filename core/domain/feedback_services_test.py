...
class FeedbackDeletionUnitTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.thread_2_id = feedback_services.create_thread(
            feconf.ENTITY_TYPE_EXPLORATION, self.EXP_2_ID, self.user_id, 'subject',
            'text'
        )
    ...
    def test_delete_feedback_threads_deletes_thread(self) -> None:
        self.assertIsNotNone(
            feedback_models.GeneralFeedbackThreadModel.get_by_id(self.thread_1_id)
        )
        feedback_services.delete_threads_for_multiple_entities(
            feconf.ENTITY_TYPE_EXPLORATION, [self.EXP_1_ID]
        )
    ...
    def test_delete_feedback_threads_deletes_multiple_feedbacks(self) -> None:
        ...
        self.assertIsNotNone(
            feedback_models.GeneralFeedbackThreadModel.get_by_id(self.thread_2_id)
        )
        feedback_services.delete_threads_for_multiple_entities(
...
class FeedbackThreadUnitTests(test_utils.GenericTestBase):
    ...
    def test_get_threads_single_exploration(self) -> None:
        ...
        feedback_services.create_thread(
            'exploration', self.EXP_ID_1, None, self.EXPECTED_THREAD_DICT['subject'],
            'not used here'
        )
        threads = feedback_services.get_threads('exploration', self.EXP_ID_1)
    ...
    def test_get_total_open_threads_for_multiple_explorations(self) -> None:
        ...
        feedback_services.create_message(
            threads_exp_1[0].id, self.user_id, feedback_models.STATUS_CHOICES_FIXED,
            None, 'feedback message not used here'
        )
    ...
    def test_get_thread_summaries(self) -> None:
        ...
        thread_3.put()
        feedback_services.create_message(
            'exploration.%s.%s' % (self.EXP_ID_3, self.THREAD_ID), self.user_id, None,
            None, 'not used here'
        )
        ...
        for summary, thread, exploration_title in zip(thread_summaries, threads,
                                                      exploration_titles):
            ...
            self.assertEqual(
                summary.author_last_message, user_services.get_username(self.user_id)
            )
    ...
    def test_get_thread_summaries_returns_correct_message_count(self) -> None:
        ...
        thread_id_2 = feedback_services.create_thread(
            'exploration', self.EXP_ID_2, None, self.EXPECTED_THREAD_DICT['subject'],
            'not used here'
        )
    ...
    def test_get_thread_summaries_only_returns_threads_for_explorations(self) -> None:
        exp_thread_id = feedback_services.create_thread(
            'exploration', self.EXP_ID_1, self.user_id, 'unused subject', 'unused text'
        )
    ...
    def test_add_message_ids_to_read_by_list_adds_msgs_to_threads_in_order(
        ...
        sample_thread_ids = [
            'sample_thread_id_1', 'sample_thread_id_2', 'sample_thread_id_3'
        ]
        ...
        message_identifiers = []
        for sample_thread_id, sample_message_id in zip(sample_thread_ids,
                                                       sample_message_ids):
    ...
    def test_only_exploration_threads_trigger_events(self) -> None:
        ...
        with self.swap(event_services.FeedbackThreadCreatedEventHandler, 'record',
                       event_handler_call_counter_exploration):
            feedback_services.create_thread(
                feconf.ENTITY_TYPE_EXPLORATION, exp_id, 'test_user', 'a subject',
                'some text'
            )
    ...
    def test_cache_update_after_create_thread_with_user_text(self) -> None:
        thread_id = feedback_services.create_thread(
            'exploration', self.EXP_ID_1, self.user_id, 'subject', 'initial text'
        )
...
class FeedbackMessageEmailTests(test_utils.EmailTestBase):
    ...
    def test_pop_feedback_message_references(self) -> None:
        with self.can_send_feedback_email_ctx:
            feedback_services.create_thread(
                'exploration', self.exploration.id, self.user_id_a, 'a subject',
                'some text'
            )
    ...
    def test_send_feedback_message_email(self) -> None:
        with self.can_send_feedback_email_ctx:
            ...
            self.assertDictEqual(
                model.feedback_message_references[0], expected_feedback_message_dict
            )
            self.assertEqual(model.retries, 0)
    ...
    def test_add_new_feedback_message(self) -> None:
        with self.can_send_feedback_email_ctx:
            ...
            self.assertDictEqual(
                model.feedback_message_references[0], expected_feedback_message_dict1
            )
            self.assertDictEqual(
                model.feedback_message_references[1], expected_feedback_message_dict2
            )
            self.assertEqual(model.retries, 0)
    ...
    def test_email_is_not_sent_recipient_has_muted_this_exploration(self) -> None:
        user_services.set_email_preferences_for_exploration(
            self.editor_id, self.exploration.id, mute_feedback_notifications=True
        )
    ...
    def test_that_emails_are_not_sent_for_anonymous_user(self) -> None:
        with self.can_send_feedback_email_ctx:
            feedback_services.create_thread(
                'exploration', self.exploration.id, 'test_id', 'a subject', 'some text'
            )
    ...
    def test_that_email_are_not_sent_to_author_himself(self) -> None:
        with self.can_send_feedback_email_ctx:
            feedback_services.create_thread(
                'exploration', self.exploration.id, self.editor_id, 'a subject',
                'A message'
            )
    ...
    def test_that_email_is_sent_for_changing_status_of_thread(self) -> None:
        with self.can_send_feedback_email_ctx:
            ...
            feedback_services.create_message(
                thread_id, self.editor_id, feedback_models.STATUS_CHOICES_FIXED, None,
                ''
            )
            # There are two jobs in the taskqueue: one for the realtime
...
class FeedbackMessageBatchEmailHandlerTests(test_utils.EmailTestBase):
    ...
    def test_that_emails_are_sent(self) -> None:
        ...
        with self.can_send_feedback_email_ctx:
            feedback_services.create_thread(
                'exploration', self.exploration.id, self.new_user_id, 'a subject',
                'some text'
            )
    ...
    def test_that_correct_emails_are_sent_for_multiple_feedback(self) -> None:
        ...
        with self.can_send_feedback_email_ctx:
            ...
            feedback_services.create_message(
                thread_id, self.new_user_id, feedback_models.STATUS_CHOICES_OPEN,
                'subject', 'more text'
            )
