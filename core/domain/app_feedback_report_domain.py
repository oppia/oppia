...
app_feedback_report_models, = models.Registry.import_models([models.Names.
    APP_FEEDBACK_REPORT])
...
class AppFeedbackReport:
    ...
    def __init__(self, report_id: str, schema_version: int, platform: str,
        submitted_on_timestamp: datetime.datetime,
        local_timezone_offset_hrs: int, ticket_id: Optional[str],
        scrubbed_by: Optional[str], user_supplied_feedback:
        UserSuppliedFeedback, device_system_context: DeviceSystemContext,
    ...
    def to_dict(self) ->AppFeedbackReportDict:
        ...
        """
        return {'report_id': self.report_id, 'schema_version': self.
            schema_version, 'platform': self.platform,
            ...
            'ticket_id': self.ticket_id, 'scrubbed_by': self.scrubbed_by,
    ...
    def validate(self) ->None:
        ...
        if (not app_feedback_report_constants.TIMEZONE_MINIMUM_OFFSET <=
            self.local_timezone_offset_hrs <= app_feedback_report_constants
            .TIMEZONE_MAXIMUM_OFFSET):
            raise utils.ValidationError(
                'Expected local timezone offset to be in [%d, %d], received: %d'
                 % (app_feedback_report_constants.TIMEZONE_MINIMUM_OFFSET,
                app_feedback_report_constants.TIMEZONE_MAXIMUM_OFFSET, self
        ...
        if not isinstance(self.device_system_context,
            AndroidDeviceSystemContext):
            raise utils.ValidationError(
                'Expected device and system context to be of type AndroidDeviceSystemContext for platform %s, received: %r'
    ...
    @classmethod
    def require_valid_schema_version(cls, platform: str, schema_version: int
        ) ->None:
        ...
        if not isinstance(schema_version, int) or schema_version <= 0:
            raise utils.ValidationError(
                'The report schema version %r is invalid, expected an integer in [%d, %d].'
                 % (schema_version, minimum_schema, current_schema))
        if not minimum_schema <= schema_version <= current_schema:
            raise utils.ValidationError(
                'The supported report schema versions for %s reports are [%d, %d], received: %d.'
    ...
    def require_valid_scrubber_id(cls, scrubber_id: str) ->None:
        ...
        if not utils.is_user_id_valid(scrubber_id
            ) and scrubber_id != feconf.APP_FEEDBACK_REPORT_SCRUBBER_BOT_ID:
            raise utils.ValidationError(
                'The scrubbed_by user id %r is invalid.' % scrubber_id)

    @classmethod
    def from_submitted_feedback_dict(cls, report_dict:
        AndroidFeedbackReportDict) ->AppFeedbackReport:
    ...
    def get_android_report_from_dict(cls, report_dict:
        ...
        user_supplied_feedback_obj = UserSuppliedFeedback(cls.
            get_report_type_from_string(user_supplied_feedback_json[
            'report_type']), cls.get_category_from_string(
        ...
        device_system_context_obj = AndroidDeviceSystemContext(
            system_context_json['platform_version'], system_context_json[
            'package_version_code'], system_context_json[
            'android_device_country_locale_code'], system_context_json[
            'android_device_language_locale_code'], device_context_json[
            'android_device_model'], device_context_json[
            'android_sdk_version'], device_context_json['build_fingerprint'
            ], cls.get_android_network_type_from_string(device_context_json
            ['network_type']))
        app_context_json = report_dict['app_context']
        entry_point_obj = cls.get_entry_point_from_json(app_context_json[
            'entry_point'])
        app_context_obj = AndroidAppContext(entry_point_obj,
            app_context_json['text_language_code'], app_context_json[
            'audio_language_code'], cls.get_android_text_size_from_string(
            app_context_json['text_size']), app_context_json[
            'only_allows_wifi_download_and_update'], app_context_json[
            'automatically_update_topics'], app_context_json[
            'account_is_profile_admin'], app_context_json['event_logs'],
            app_context_json['logcat_logs'])
        report_datetime = from_milliseconds_utc(report_dict[
            'report_submission_timestamp_sec'] * 1000.0)
        report_id = (app_feedback_report_models.AppFeedbackReportModel.
            generate_id(report_dict['platform_type'], report_datetime))
        report_obj = AppFeedbackReport(report_id, report_dict[
            'android_report_info_schema_version'], report_dict[
            'platform_type'], report_datetime, report_dict[
            'report_submission_utc_offset_hrs'], None, None,
            user_supplied_feedback_obj, device_system_context_obj,
            app_context_obj)
        return report_obj

    @classmethod
    def get_report_type_from_string(cls, report_type_name: str
    ...
    @classmethod
    def get_android_text_size_from_string(cls, text_size_name: str
    ...
    @classmethod
    def get_entry_point_from_json(cls, entry_point_json: EntryPointDict
        ) ->EntryPoint:
        ...
        if (entry_point_name == app_feedback_report_constants.EntryPoint.
            NAVIGATION_DRAWER.value):
            ...
            return LessonPlayerEntryPoint(entry_point_json[
                'entry_point_topic_id'], entry_point_json[
                'entry_point_story_id'], entry_point_json[
            ...
            return RevisionCardEntryPoint(entry_point_json[
                'entry_point_topic_id'], entry_point_json[
    ...
    @classmethod
    def get_android_network_type_from_string(cls, network_type_name: str
...
class UserSuppliedFeedback:
    ...
    def __init__(self, report_type: app_feedback_report_constants.
        ReportType, category: app_feedback_report_constants.Category,
        user_feedback_selected_items: List[str],
        user_feedback_other_text_input: str) ->None:
    ...
    def to_dict(self) ->UserSuppliedFeedbackDict:
        ...
        """
        return {'report_type': self.report_type.value, 'category': self.
            category.value, 'user_feedback_selected_items': self.
            user_feedback_selected_items, 'user_feedback_other_text_input':
    ...
    def validate(self) ->None:
        ...
        if self.user_feedback_other_text_input is None:
            raise utils.ValidationError(
                'No user_feedback_selected_items supplied for category %s.' %
                self.category)
        self.require_valid_user_feedback_items_for_category(self.category,
            self.user_feedback_selected_items, self.
            user_feedback_other_text_input)

    @classmethod
    def require_valid_report_type(cls, report_type:
        app_feedback_report_constants.ReportType) ->None:
        ...
        if (report_type not in app_feedback_report_constants.
            ALLOWED_REPORT_TYPES):
            raise utils.ValidationError(
                'Invalid report type %s, must be one of %s.' % (report_type,
                app_feedback_report_constants.ALLOWED_REPORT_TYPES))

    @classmethod
    def require_valid_category(cls, category: app_feedback_report_constants
        .Category) ->None:
        ...
        if category not in app_feedback_report_constants.ALLOWED_CATEGORIES:
            raise utils.ValidationError(
                'Invalid category %s, must be one of %s.' % (category,
                app_feedback_report_constants.ALLOWED_CATEGORIES))

    @classmethod
    def require_valid_user_feedback_items_for_category(cls, category:
        app_feedback_report_constants.Category, selected_items: List[str],
        other_text_input: str) ->None:
        ...
        """
        if (category in app_feedback_report_constants.
    ...
    @classmethod
    def require_valid_selected_items_for_category(cls, selected_items: List
        [str]) ->None:
...
class DeviceSystemContext:
    ...
    def __init__(self, version_name: str, device_country_locale_code: str
        ) ->None:
    ...
    def to_dict(self) ->DeviceSystemContextDict:
        ...
        """
        return {'version_name': self.version_name,
    ...
    def validate(self) ->None:
        ...
        """
        raise NotImplementedError(
            'Subclasses of DeviceSystemContext should implement domain validation.'
            )
...
class AndroidDeviceSystemContext(DeviceSystemContext):
    ...
    def __init__(self, version_name: str, package_version_code: int,
        device_country_locale_code: str, device_language_locale_code: str,
        device_model: str, sdk_version: int, build_fingerprint: str,
        network_type: app_feedback_report_constants.AndroidNetworkType) ->None:
    ...
    def to_dict(self) ->AndroidDeviceSystemContextDict:
        ...
        Returns:
            ...
            instance.
        """
        return {'version_name': self.version_name, 'package_version_code':
            self.package_version_code, 'device_country_locale_code': self.
            device_country_locale_code, 'device_language_locale_code': self
            .device_language_locale_code, 'device_model': self.device_model,
            'sdk_version': self.sdk_version, 'build_fingerprint': self.
            build_fingerprint, 'network_type': self.network_type.value}
    ...
    def validate(self) ->None:
        ...
        self.require_valid_locale_code('country', self.
            device_country_locale_code)
        self.require_valid_locale_code('language', self.
        ...
        if not isinstance(self.build_fingerprint, str):
            raise utils.ValidationError(
                'Build fingerprint must be a string, received: %r.' % self.
                build_fingerprint)
    ...
    def require_valid_version_name(cls, version_name: str) ->None:
        ...
        delimiter = (app_feedback_report_constants.
            ANDROID_VERSION_NAME_DELIMITER)
        if len(version_name.split(delimiter)) != 3:
            raise utils.ValidationError(
                'The version name is not a valid string format, received: %s.'
                 % version_name)

    @classmethod
    def require_valid_package_version_code(cls, package_version_code: int
        ) ->None:
        ...
        if package_version_code < feconf.MINIMUM_ANDROID_PACKAGE_VERSION_CODE:
            raise utils.ValidationError(
                'The package version code is not a valid int. The minimum supported version is %d, received: %d.'
                 % (feconf.MINIMUM_ANDROID_PACKAGE_VERSION_CODE,
                package_version_code))

    @classmethod
    def require_valid_locale_code(cls, locale_type: str, locale_code: str
        ) ->None:
        ...
        if not isinstance(locale_code, str):
            raise utils.ValidationError(
                "The device's %s locale code must be an string, received: %r."
                 % (locale_type, locale_code))
        if not cls._match_locale_code_string(locale_code):
            raise utils.ValidationError(
                "The device's %s locale code is not a valid string, received: %s."
                 % (locale_type, locale_code))
    ...
    def require_valid_sdk_version(cls, sdk_version: int) ->None:
        ...
        if (sdk_version < app_feedback_report_constants.
    ...
    def require_valid_network_type(cls, network_type:
        app_feedback_report_constants.AndroidNetworkType) ->None:
        ...
        if (network_type not in app_feedback_report_constants.
...
class AppContext:
    ...
    def __init__(self, entry_point: EntryPoint, text_language_code: str,
    ...
    def to_dict(self) ->AppContextDict:
        ...
        """
        return {'entry_point': self.entry_point.to_dict(),
    ...
    def validate(self) ->None:
        ...
        """
        raise NotImplementedError(
            'Subclasses of AppContext should implement their own validation checks.'
            )
...
class AndroidAppContext(AppContext):
    ...
    def __init__(self, entry_point: EntryPoint, text_language_code: str,
        audio_language_code: str, text_size: app_feedback_report_constants.
        AndroidTextSize, only_allows_wifi_download_and_update: bool,
        automatically_update_topics: bool, account_is_profile_admin: bool,
        event_logs: List[str], logcat_logs: List[str]) ->None:
    ...
    def to_dict(self) ->AndroidAppContextDict:
        ...
        return {'entry_point': self.entry_point.to_dict(),
            'text_language_code': self.text_language_code,
            'audio_language_code': self.audio_language_code, 'text_size':
            self.text_size.value, 'only_allows_wifi_download_and_update':
            self.only_allows_wifi_download_and_update,
            'automatically_update_topics': self.automatically_update_topics,
            'account_is_profile_admin': self.account_is_profile_admin,
            'event_logs': self.event_logs, 'logcat_logs': self.logcat_logs}
    ...
    def validate(self) ->None:
        ...
        if self.only_allows_wifi_download_and_update is None or not isinstance(
            ...
            raise utils.ValidationError(
                'only_allows_wifi_download_and_update field should be a boolean, received: %r'
                 % self.only_allows_wifi_download_and_update)
        if self.automatically_update_topics is None or not isinstance(self.
            automatically_update_topics, bool):
            raise utils.ValidationError(
                'automatically_update_topics field should be a boolean, received: %r'
                 % self.automatically_update_topics)
        if self.account_is_profile_admin is None or not isinstance(self.
            account_is_profile_admin, bool):
            raise utils.ValidationError(
                'account_is_profile_admin field should be a boolean, received: %r'
                 % self.account_is_profile_admin)
        ...
        if self.logcat_logs is None or not isinstance(self.logcat_logs, list):
            raise utils.ValidationError(
                'Should have a logcat log list, received: %r' % self.
                logcat_logs)

    @classmethod
    def require_valid_language_code(cls, language_type: str, language_code: str
        ) ->None:
        ...
        if language_code is None:
            raise utils.ValidationError('No app %s language code supplied.' %
                language_type)
        if not isinstance(language_code, str):
            raise utils.ValidationError(
                "Expected the app's %s language code to be a string, received: %r"
                 % (language_type, language_code))
        if not cls._match_language_code_string(language_code):
            raise utils.ValidationError(
                "The app's %s language code is not a valid string, received: %s."
                 % (language_type, language_code))
    ...
    def require_valid_text_size(cls, text_size:
        app_feedback_report_constants.AndroidTextSize) ->None:
        """Checks whether the package version code is a valid string code for
        Oppia Android.
        ...
        if (text_size not in app_feedback_report_constants.
...
class EntryPoint:
    ...
    def __init__(self, entry_point: app_feedback_report_constants.
        EntryPoint, topic_id: Optional[str]=None, story_id: Optional[str]=
        None, exploration_id: Optional[str]=None, subtopic_id: Optional[str
        ]=None) ->None:
    ...
    def to_dict(self) ->Any:
        ...
        """
        raise NotImplementedError(
            'Subclasses of EntryPoint should implement their own dict representations.'
            )
    ...
    def validate(self) ->None:
        ...
        """
        raise NotImplementedError(
            'Subclasses of EntryPoint should implement their own validation checks.'
            )

    @classmethod
    def require_valid_entry_point_name(cls, actual_name: str,
        expected_entry_point: app_feedback_report_constants.EntryPoint) ->None:
    ...
    @classmethod
    def require_valid_entry_point_exploration(cls, exploration_id: Optional
        [str], story_id: Optional[str]) ->None:
        ...
        if expected_story_id != story_id:
            raise utils.ValidationError(
                'Exploration with id %s is not part of story with id of %s, should be found in story with id of %s'
...
class NavigationDrawerEntryPoint(EntryPoint):
    ...
    def __init__(self) ->None:
        ...
        super().__init__(app_feedback_report_constants.EntryPoint.
            NAVIGATION_DRAWER, None, None, None, None)
...
class LessonPlayerEntryPoint(EntryPoint):
    ...
    def __init__(self, topic_id: str, story_id: str, exploration_id: str
        ) ->None:
        ...
        """
        super().__init__(app_feedback_report_constants.EntryPoint.
            LESSON_PLAYER, topic_id=topic_id, story_id=story_id,
    ...
    def to_dict(self) ->LessonPlayerEntryPointDict:
        ...
        """
        return {'entry_point_name': self.entry_point_name, 'topic_id': self
            .topic_id, 'story_id': self.story_id, 'exploration_id': self.
            exploration_id}
    ...
    def validate(self) ->None:
        ...
        self.require_valid_entry_point_exploration(self.exploration_id,
            self.story_id)
...
class RevisionCardEntryPoint(EntryPoint):
    ...
    def __init__(self, topic_id: str, subtopic_id: str) ->None:
        ...
        """
        super().__init__(app_feedback_report_constants.EntryPoint.
            REVISION_CARD, topic_id, None, None, subtopic_id)
    ...
    def to_dict(self) ->RevisionCardEntryPointDict:
        ...
        """
        return {'entry_point_name': self.entry_point_name, 'topic_id': self
            .topic_id, 'subtopic_id': self.subtopic_id}
    ...
    def validate(self) ->None:
        ...
        if not isinstance(self.subtopic_id, int):
            raise utils.ValidationError(
                'Expected subtopic id to be an int, received %s' % self.
                subtopic_id)
...
class CrashEntryPoint(EntryPoint):
    ...
    def __init__(self) ->None:
        ...
        super().__init__(app_feedback_report_constants.EntryPoint.CRASH,
            None, None, None, None)
...
class AppFeedbackReportTicket:
    ...
    def __init__(self, ticket_id: str, ticket_name: str, platform: str,
        github_issue_repo_name: Optional[str], github_issue_number:
        Optional[int], archived: bool, newest_report_creation_timestamp:
        Optional[datetime.datetime], reports: List[str]) ->None:
    ...
    def to_dict(self) ->AppFeedbackReportTicketDict:
        ...
        Returns:
            ...
            instance.
        """
        return {'ticket_id': self.ticket_id, 'ticket_name': self.
            ticket_name, 'platform': self.platform,
            'github_issue_repo_name': self.github_issue_repo_name,
            'github_issue_number': self.github_issue_number, 'archived':
            self.archived, 'newest_report_creation_timestamp_isoformat': 
            self.newest_report_creation_timestamp.isoformat() if self.
            newest_report_creation_timestamp else None, 'reports': self.reports
            }
    ...
    def validate(self) ->None:
        ...
        if self.github_issue_number is not None:
            if not isinstance(self.github_issue_number, int
                ) or self.github_issue_number < 1:
                raise utils.ValidationError(
                    'The Github issue number name must be a positive integer, received: %r'
    ...
    def require_valid_ticket_id(cls, ticket_id: str) ->None:
        ...
        if not isinstance(ticket_id, str):
            raise utils.ValidationError(
                'The ticket id should be a string, received: %s' % ticket_id)
        if len(ticket_id.split(app_feedback_report_constants.
            TICKET_ID_DELIMITER)) != 3:
    ...
    def require_valid_ticket_name(cls, ticket_name: str) ->None:
        ...
        if len(ticket_name
            ...
            raise utils.ValidationError(
                'The ticket name is too long, has %d characters but only allowed %d characters'
                 % (len(ticket_name), app_feedback_report_constants.
                MAXIMUM_TICKET_NAME_LENGTH))
    ...
    def require_valid_github_repo(cls, repo_name: str) ->None:
        ...
        if repo_name not in app_feedback_report_constants.GITHUB_REPO_CHOICES:
            raise utils.ValidationError(
                ...
                repo_name, app_feedback_report_constants.GITHUB_REPO_CHOICES))


AcceptableEntryPointClasses = Union[NavigationDrawerEntryPointDict,
    LessonPlayerEntryPointDict, RevisionCardEntryPointDict, CrashEntryPointDict
    ]
...
class AppFeedbackReportDailyStats:
    ...
    def __init__(self, stats_id: str, ticket: AppFeedbackReportTicket,
        platform: str, stats_tracking_date: datetime.date,
        total_reports_submitted: int, daily_param_stats: Dict[str,
        ReportStatsParameterValueCounts]) ->None:
    ...
    def to_dict(self) ->AppFeedbackReportDailyStatsDict:
        ...
        Returns:
            ...
            instance.
        """
        return {'stats_id': self.stats_id, 'ticket': self.ticket.to_dict(),
            'platform': self.platform, 'stats_tracking_date': self.
            stats_tracking_date.isoformat(), 'total_reports_submitted':
            self.total_reports_submitted, 'daily_param_stats': {param_name:
            param_counts_obj.to_dict() for param_name, param_counts_obj in
    ...
    def validate(self) ->None:
        ...
        if not isinstance(self.total_reports_submitted, int):
            raise utils.ValidationError(
                'The total number of submitted reports should be an int, received: %r'
                 % self.total_reports_submitted)
        if self.total_reports_submitted < 0:
            raise utils.ValidationError(
                'The total number of submitted reports should be a non-negative int, received: %d'
                 % self.total_reports_submitted)
    ...
    def require_valid_stats_id(cls, stats_id: str) ->None:
        ...
        if len(stats_id.split(app_feedback_report_constants.STATS_ID_DELIMITER)
            ) != 3:
            raise utils.ValidationError('The stats id %s is invalid' % stats_id
                )

    @classmethod
    def require_valid_daily_param_stats(cls, param_stats: Dict[str,
        ReportStatsParameterValueCounts]) ->None:
        ...
        if not isinstance(param_stats, dict):
            raise utils.ValidationError(
                'The parameter stats should be a dict, received: %r' %
                param_stats)
        allowed_parameter_names = [parameter.value for parameter in
        ...
        for param_name, param_count_obj in param_stats.items():
            if param_name not in allowed_parameter_names:
                raise utils.ValidationError(
                    'The parameter %s is not a valid parameter to aggregate stats on, must be one of %s'
    ...
    def validate(self) ->None:
        ...
        for param_value, param_count in self.parameter_value_counts.items():
            ...
            if not isinstance(param_count, int) or param_count < 0:
                raise utils.ValidationError(
                    'The parameter value count should be a non-negative int, received: %r'
                     % param_count)
...
class AppFeedbackReportFilter:
    ...
    def __init__(self, filter_field: app_feedback_report_constants.
        FilterFieldNames, filter_options: List[str]) ->None:
    ...
    def to_dict(self) ->AppFeedbackReportFilterDict:
        ...
        return {'filter_field': self.filter_field.name, 'filter_options':
            self.filter_options}
    ...
    def validate(self) ->None:
        ...
        """
        if (self.filter_field not in app_feedback_report_constants.
            ALLOWED_FILTERS):
            raise utils.ValidationError(
                'The filter field should be one of %s, received: %s' % ([
                item.name for item in app_feedback_report_constants.
                ALLOWED_FILTERS], self.filter_field.name))
        if not isinstance(self.filter_options, list):
            raise utils.ValidationError(
                'The filter options should be a list, received: %r' % self.
                filter_options)