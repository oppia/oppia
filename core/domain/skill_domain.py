...
class Rubric:
    ...
    def validate(self) -> None:
        ...
        if len(self.explanations) > 10:
            raise utils.ValidationError(
                'Expected number of explanations to be less than or equal to 10, received %d'
        ...
        for explanation in self.explanations:
            if len(explanation) > 300:
                raise utils.ValidationError(
                    'Explanation should be less than or equal to 300 chars, received %d chars'
                    % len(explanation)
                )
        if (
...
class SkillContents:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.explanation, state_domain.SubtitledHtml):
            raise utils.ValidationError(
                'Expected skill explanation to be a SubtitledHtml object, received %s'
                % self.explanation
            )
...
class Skill:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.misconceptions_schema_version, int):
            raise utils.ValidationError(
                'Expected misconceptions schema version to be an integer, received %s'
        ...
        if not isinstance(self.rubric_schema_version, int):
            raise utils.ValidationError(
                'Expected rubric schema version to be an integer, received %s'
        ...
        if not isinstance(self.skill_contents_schema_version, int):
            raise utils.ValidationError(
                'Expected skill contents schema version to be an integer, received %s'
        ...
        if not isinstance(self.skill_contents, SkillContents):
            raise utils.ValidationError(
                'Expected skill_contents to be a SkillContents object, received %s'
                % self.skill_contents
            )
        ...
        if not isinstance(self.rubrics, list):
            raise utils.ValidationError(
                'Expected rubrics to be a list, received %s'
                % self.skill_contents
            )
        ...
        for rubric in self.rubrics:
            if not isinstance(rubric, Rubric):
                raise utils.ValidationError(
                    'Expected each rubric to be a Rubric object, received %s'
                    % rubric
                )
        ...
        if not isinstance(self.misconceptions, list):
            raise utils.ValidationError(
                'Expected misconceptions to be a list, received %s'
                % self.misconceptions
            )
        ...
        for misconception in self.misconceptions:
            if not isinstance(misconception, Misconception):
                raise utils.ValidationError(
                    'Expected each misconception to be a Misconception object, received %s'
                    % misconception
                )
...
class SkillSummary:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.misconception_count, int):
            raise utils.ValidationError(
                'Expected misconception_count to be an int, received \'%s\''
                % self.misconception_count
            )

        if self.misconception_count < 0:
            raise utils.ValidationError(
                'Expected misconception_count to be non-negative, received \'%s\''
                % self.misconception_count
            )
