...
class Skill:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.rubrics, list):
            raise utils.ValidationError(
                'Expected rubrics to be a list, ' 'received %s' % self.skill_contents
            )
        ...
        for skill_id in self.prerequisite_skill_ids:
            if not isinstance(skill_id, str):
                raise utils.ValidationError(
                    'Expected each skill ID to be a string, ' 'received %s' % skill_id
                )
    ...
    def convert_html_fields_in_skill_contents(
        ...
        for value_index, value in enumerate(skill_contents_dict['worked_examples']):
            skill_contents_dict['worked_examples'][value_index]['question']['html'] = (
                conversion_fn(value['question']['html'])
            )
            skill_contents_dict['worked_examples'][value_index]['explanation'][
                'html'
            ] = conversion_fn(value['explanation']['html'])
        ...
        for topic_name in categorized_skills_dict:
            ...
            for subtopic_title in categorized_skills_dict[topic_name]:
                result_categorized_skills_dict[topic_name][subtopic_title] = [
                    ...
                    for short_skill_summary in categorized_skills_dict[topic_name][
                        subtopic_title
                    ]
                ]
