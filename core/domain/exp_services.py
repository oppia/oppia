...
def generate_version_history_models(exploration_id: str) -> dict:
    """Generates the version history models for a given exploration id."""
    exploration = exp_fetchers.get_exploration_by_id(exploration_id)
    response_dict = {}
    commit_log_models = (
    ...
    state_vhs: List[Dict[str, state_domain.StateVersionHistory]] = []
    metadata_vhs: List[exp_domain.MetadataVersionHistory] = []
    committer_ids: List[List[str]] = []
    ...
    state_vhs.append(states_vh_at_v1)
    metadata_vhs.append(metadata_vh_at_v1)
    committer_ids.append([committer_id_v1])
    ...
    for version in range(2, exploration.version + 1):
        ...
        if revert_to_version is not None:
            state_vhs.append(copy.deepcopy(
                state_vhs[revert_to_version - 1]))
            metadata_vhs.append(copy.deepcopy(
                metadata_vhs[revert_to_version - 1]))
            committer_ids.append(copy.deepcopy(
                committer_ids[revert_to_version - 1]))
        else:
            old_states_dict = {
                state_name: state.to_dict()
            ...
            old_states_vh = copy.deepcopy(state_vhs[version - 2])
            old_metadata_vh = copy.deepcopy(metadata_vhs[version - 2])
            try:
                new_states_vh = (
                    update_states_version_history(
                ...
                new_committer_ids = (
                    get_updated_committer_ids(
                        new_states_vh,
                        new_metadata_vh.last_edited_committer_id
                    )
                )
                state_vhs.append(new_states_vh)
                metadata_vhs.append(new_metadata_vh)
                committer_ids.append(new_committer_ids)
            except Exception:
                commit_cmds = []
                added_states = []
                deleted_states = []
                ...
                for i in range(1, version):
                    state_vh = {
                        ...
                        for state_name, vh in state_vhs[i-1].items()
                    }
                    states_vhs.append(state_vh)
                ...
                response_dict['states_vhs'] = states_vhs
                break

    if response_dict == {}:
        for version in range(1, exploration.version + 1):
            vh_model = vh_models[version - 1]
            if vh_model is not None:
                vh_model.state_version_history = {
                    state_name: state_vh.to_dict()
                    for state_name, state_vh in state_vhs[version - 1].items()
                }
                vh_model.metadata_last_edited_committer_id = metadata_vhs[version - 1].last_edited_committer_id
                vh_model.metadata_last_edited_version_number = metadata_vhs[version - 1].last_edited_version_number
                vh_model.committer_ids = committer_ids[version - 1]
            else:
                vh_model = exp_models.ExplorationVersionHistoryModel(
                    id=exp_models.ExplorationVersionHistoryModel.get_instance_id(exploration_id, version),
                    exploration_id=exploration_id,
                    ...
                    metadata_last_edited_committer_id=metadata_vhs[version - 1].last_edited_committer_id,
                    metadata_last_edited_version_number=metadata_vhs[version - 1].last_edited_version_number,
