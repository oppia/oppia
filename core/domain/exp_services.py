...
base_models, exp_models, stats_models, user_models = (models.Registry.
    import_models([models.Names.BASE_MODEL, models.Names.EXPLORATION,
    models.Names.STATISTICS, models.Names.USER]))
datastore_services = models.Registry.import_datastore_services()
AcceptableActivityModelTypes = Union[user_models.CompletedActivitiesModel,
...
def is_exp_summary_editable(exp_summary: exp_domain.ExplorationSummary,
    user_id: str) ->bool:
    ...
    """
    return user_id is not None and (user_id in exp_summary.editor_ids or 
        user_id in exp_summary.owner_ids or exp_summary.community_owned)


def get_exploration_titles_and_categories(exp_ids: List[str]) ->Dict[str,
    Dict[str, str]]:
    ...
    Returns:
        ...
        ids are excluded.
    """
    explorations = [(exp_fetchers.get_exploration_from_model(e) if e else
        None) for e in exp_models.ExplorationModel.get_multi(exp_ids,
    ...
    for exploration in explorations:
        if exploration is None:
            ...
            result[exploration.id] = {'title': exploration.title,
                'category': exploration.category}
    return result


def get_exploration_ids_matching_query(query_string: str, categories: List[
    str], language_codes: List[str], offset: Optional[int]=None) ->Tuple[
    ...
    for _ in range(MAX_ITERATIONS):
        ...
        invalid_exp_ids = []
        for ind, model in enumerate(exp_models.ExpSummaryModel.get_multi(
            exp_ids)):
            if model is not None:
                returned_exploration_ids.append(exp_ids[ind])
            else:
                invalid_exp_ids.append(exp_ids[ind])
        if len(returned_exploration_ids
            ) == feconf.SEARCH_RESULTS_PAGE_SIZE or search_offset is None:
            break
        logging.error('Search index contains stale exploration ids: %s' %
            ', '.join(invalid_exp_ids))
    if len(returned_exploration_ids
        ) < feconf.SEARCH_RESULTS_PAGE_SIZE and search_offset is not None:
        logging.error(
            'Could not fulfill search request for query string %s; at least %s retries were needed.'
             % (query_string, MAX_ITERATIONS))
    return returned_exploration_ids, search_offset


def get_non_private_exploration_summaries() ->Dict[str, exp_domain.
    ExplorationSummary]:
    ...
    """
    return exp_fetchers.get_exploration_summaries_from_models(exp_models.
        ExpSummaryModel.get_non_private())


def get_top_rated_exploration_summaries(limit: int) ->Dict[str, exp_domain.
    ExplorationSummary]:
    ...
    """
    return exp_fetchers.get_exploration_summaries_from_models(exp_models.
        ExpSummaryModel.get_top_rated(limit))


def get_recently_published_exp_summaries(limit: int) ->Dict[str, exp_domain
    .ExplorationSummary]:
    ...
    """
    return exp_fetchers.get_exploration_summaries_from_models(exp_models.
        ExpSummaryModel.get_recently_published(limit))
...
def get_story_id_linked_to_exploration(exp_id: str) ->Optional[str]:
    ...
    """
    exploration_context_model = exp_models.ExplorationContextModel.get(exp_id,
        strict=False)
...
def get_all_exploration_summaries() ->Dict[str, exp_domain.ExplorationSummary]:
    ...
    """
    return exp_fetchers.get_exploration_summaries_from_models(exp_models.
        ExpSummaryModel.get_all().fetch())


def export_to_zip_file(exploration_id: str, version: Optional[int]=None
    ) ->io.BytesIO:
    ...
    with zipfile.ZipFile(temp_file, mode='w', compression=zipfile.ZIP_DEFLATED
        ) as zfile:
        if not exploration.title:
            zfile.writestr('Unpublished_exploration.yaml', yaml_repr)
        else:
            exploration_file_name = re.sub('[^A-Za-z0-9_ -]+', '',
                exploration.title)
...
def export_states_to_yaml(exploration_id: str, version: Optional[int]=None,
    width: int=80) ->Dict[str, str]:
    ...
    for state in exploration.states:
        exploration_dict[state] = utils.yaml_from_dict(exploration.states[
            state].to_dict(), width=width)
    return exploration_dict


def apply_change_list(exploration_id: str, change_list: Sequence[exp_domain
    .ExplorationChange]) ->exp_domain.Exploration:
    ...
    try:
        ...
        for change in change_list:
            if change.cmd == exp_domain.CMD_ADD_STATE:
                ...
                exploration.add_state(add_state_cmd.state_name,
                ...
                rename_state_cmd = cast(exp_domain.
                    RenameExplorationStateCmd, change)
                exploration.rename_state(rename_state_cmd.old_state_name,
                    rename_state_cmd.new_state_name)
            elif change.cmd == exp_domain.CMD_DELETE_STATE:
                delete_state_cmd = cast(exp_domain.
                    DeleteExplorationStateCmd, change)
                exploration.delete_state(delete_state_cmd.state_name)
            elif change.cmd == exp_domain.CMD_EDIT_STATE_PROPERTY:
                state: state_domain.State = exploration.states[change.
                    state_name]
                if (change.property_name == exp_domain.
                    STATE_PROPERTY_PARAM_CHANGES):
                    edit_param_changes_cmd = cast(exp_domain.
                        EditExpStatePropertyParamChangesCmd, change)
                    state.update_param_changes(list(map(to_param_domain,
                        edit_param_changes_cmd.new_value)))
                elif change.property_name == exp_domain.STATE_PROPERTY_CONTENT:
                    edit_content_cmd = cast(exp_domain.
                        EditExpStatePropertyContentCmd, change)
                    ...
                    edit_linked_skill_id_cmd = cast(exp_domain.
                        EditExpStatePropertyLinkedSkillIdCmd, change)
                    state.update_linked_skill_id(edit_linked_skill_id_cmd.
                        new_value)
                elif change.property_name == exp_domain.STATE_PROPERTY_INAPPLICABLE_SKILL_MISCONCEPTION_IDS:
                    edit_inapplicable_skill_misconception_ids = cast(exp_domain
                        .
                    ...
                    edit_interaction_cust_arg_cmd = cast(exp_domain.
                        EditExpStatePropertyInteractionCustArgsCmd, change)
                    ...
                    edit_interaction_answer_group_cmd = cast(exp_domain.
                        EditExpStatePropertyInteractionAnswerGroupsCmd, change)
                    answer_groups = edit_interaction_answer_group_cmd.new_value
                    new_answer_groups = [state_domain.AnswerGroup.from_dict
                    ...
                    new_outcome = None
                    if change.new_value:
                        edit_interaction_default_outcome_cmd = cast(exp_domain
                            .
                    ...
                    edit_unclassified_answers_cmd = cast(exp_domain.
                        EditExpStatePropertyUnclassifiedAnswersCmd, change)
                    ...
                    edit_state_interaction_hints_cmd = cast(exp_domain.
                        EditExpStatePropertyInteractionHintsCmd, change)
                    ...
                    new_solution = None
                    edit_interaction_solution_cmd = cast(exp_domain.
                        EditExpStatePropertyInteractionSolutionCmd, change)
                    if edit_interaction_solution_cmd.new_value is not None:
                        ...
                        new_solution = state_domain.Solution.from_dict(state
                            .interaction.id, edit_interaction_solution_cmd.
                            new_value)
                    ...
                    edit_solicit_answer_details_cmd = cast(exp_domain.
                        EditExpStatePropertySolicitAnswerDetailsCmd, change)
                    ...
                    if not isinstance(change.new_value, bool):
                        raise Exception(
                            ...
                            'bool, received %s' % change.new_value)
                    edit_card_is_checkpoint_cmd = cast(exp_domain.
                        EditExpStatePropertyCardIsCheckpointCmd, change)
                    state.update_card_is_checkpoint(edit_card_is_checkpoint_cmd
                        .new_value)
                elif change.property_name == exp_domain.STATE_PROPERTY_RECORDED_VOICEOVERS:
                    if not isinstance(change.new_value, dict):
                        raise Exception(
                            'Expected recorded_voiceovers to be a dict, received %s'
                             % change.new_value)
                    edit_recorded_voiceovers_cmd = cast(exp_domain.
                        EditExpStatePropertyRecordedVoiceoversCmd, change)
                    new_voiceovers_mapping = (edit_recorded_voiceovers_cmd.
                        new_value['voiceovers_mapping'])
                    language_codes_to_audio_metadata = (new_voiceovers_mapping
                        .values())
                    for language_codes in language_codes_to_audio_metadata:
                        for audio_metadata in language_codes.values():
                            audio_metadata['duration_secs'] = float(
                                audio_metadata['duration_secs'])
                    recorded_voiceovers = (state_domain.RecordedVoiceovers.
                        from_dict(change.new_value))
                    state.update_recorded_voiceovers(recorded_voiceovers)
            elif change.cmd == exp_domain.CMD_EDIT_EXPLORATION_PROPERTY:
                if change.property_name == 'title':
                    edit_title_cmd = cast(exp_domain.
                        EditExplorationPropertyTitleCmd, change)
                    exploration.update_title(edit_title_cmd.new_value)
                elif change.property_name == 'category':
                    edit_category_cmd = cast(exp_domain.
                        EditExplorationPropertyCategoryCmd, change)
                    exploration.update_category(edit_category_cmd.new_value)
                elif change.property_name == 'objective':
                    edit_objective_cmd = cast(exp_domain.
                        EditExplorationPropertyObjectiveCmd, change)
                    exploration.update_objective(edit_objective_cmd.new_value)
                elif change.property_name == 'language_code':
                    edit_language_code_cmd = cast(exp_domain.
                        EditExplorationPropertyLanguageCodeCmd, change)
                    exploration.update_language_code(edit_language_code_cmd
                        .new_value)
                elif change.property_name == 'tags':
                    edit_tags_cmd = cast(exp_domain.
                        EditExplorationPropertyTagsCmd, change)
                    exploration.update_tags(edit_tags_cmd.new_value)
                elif change.property_name == 'blurb':
                    edit_blurb_cmd = cast(exp_domain.
                        EditExplorationPropertyBlurbCmd, change)
                    exploration.update_blurb(edit_blurb_cmd.new_value)
                elif change.property_name == 'author_notes':
                    edit_author_notes_cmd = cast(exp_domain.
                        EditExplorationPropertyAuthorNotesCmd, change)
                    exploration.update_author_notes(edit_author_notes_cmd.
                        new_value)
                elif change.property_name == 'param_specs':
                    edit_param_specs_cmd = cast(exp_domain.
                        EditExplorationPropertyParamSpecsCmd, change)
                    exploration.update_param_specs(edit_param_specs_cmd.
                        new_value)
                elif change.property_name == 'param_changes':
                    edit_exp_param_changes_cmd = cast(exp_domain.
                        EditExplorationPropertyParamChangesCmd, change)
                    exploration.update_param_changes(list(map(
                        to_param_domain, edit_exp_param_changes_cmd.new_value))
                        )
                elif change.property_name == 'init_state_name':
                    edit_init_state_name_cmd = cast(exp_domain.
                        EditExplorationPropertyInitStateNameCmd, change)
                    exploration.update_init_state_name(edit_init_state_name_cmd
                        .new_value)
                elif change.property_name == 'auto_tts_enabled':
                    edit_auto_tts_enabled_cmd = cast(exp_domain.
                        EditExplorationPropertyAutoTtsEnabledCmd, change)
                    exploration.update_auto_tts_enabled(
                        edit_auto_tts_enabled_cmd.new_value)
                elif change.property_name == 'next_content_id_index':
                    cmd = cast(exp_domain.
                        EditExplorationPropertyNextContentIdIndexCmd, change)
                    next_content_id_index = max(cmd.new_value, exploration.
                        next_content_id_index)
                    exploration.update_next_content_id_index(
                        next_content_id_index)
            elif change.cmd == exp_domain.CMD_MIGRATE_STATES_SCHEMA_TO_LATEST_VERSION:
                migrate_states_schema_cmd = cast(exp_domain.
                    MigrateStatesSchemaToLatestVersionCmd, change)
                target_version_is_current_state_schema_version = (
                    migrate_states_schema_cmd.to_version == str(feconf.
        ...
        logging.error('%s %s %s %s' % (e.__class__.__name__, e,
            exploration_id, pprint.pformat(change_list)))
        raise e


def populate_exp_model_fields(exp_model: exp_models.ExplorationModel,
    exploration: exp_domain.Exploration) ->exp_models.ExplorationModel:
    ...
    exp_model.states = {state_name: state.to_dict() for state_name, state in
...
def populate_exp_summary_model_fields(exp_summary_model: Optional[
    exp_models.ExpSummaryModel], exp_summary: exp_domain.ExplorationSummary
    ...
    """
    exp_summary_dict = {'title': exp_summary.title, 'category': exp_summary
        .category, 'objective': exp_summary.objective, 'language_code':
        exp_summary.language_code, 'tags': exp_summary.tags, 'ratings':
        exp_summary.ratings, 'scaled_average_rating': exp_summary.
        scaled_average_rating, 'exploration_model_last_updated':
        exp_summary.exploration_model_last_updated,
        'exploration_model_created_on': exp_summary.
        exploration_model_created_on, 'first_published_msec': exp_summary.
        first_published_msec, 'status': exp_summary.status,
        'community_owned': exp_summary.community_owned, 'owner_ids':
        exp_summary.owner_ids, 'editor_ids': exp_summary.editor_ids,
        'voice_artist_ids': exp_summary.voice_artist_ids, 'viewer_ids':
        exp_summary.viewer_ids, 'contributor_ids': list(exp_summary.
        contributors_summary.keys()), 'contributors_summary': exp_summary.
        contributors_summary, 'version': exp_summary.version}
...
def update_states_version_history(states_version_history: Dict[str,
    state_domain.StateVersionHistory], change_list: Sequence[exp_domain.
    ExplorationChange], old_states_dict: Dict[str, state_domain.StateDict],
    new_states_dict: Dict[str, state_domain.StateDict], current_version:
    int, committer_id: str) ->Dict[str, state_domain.StateVersionHistory]:
    ...
    for old_state_name, new_state_name in effective_old_to_new_state_names.items(
        ...
        states_version_history[new_state_name
            ] = state_domain.StateVersionHistory(prev_version,
            old_state_name, committer_id)
    states_which_were_not_renamed = []
    for state_name in old_states_dict:
        if (state_name not in exp_versions_diff.deleted_state_names and 
            state_name not in effective_old_to_new_state_names):
            states_which_were_not_renamed.append(state_name)
    state_property_changed_data = {state_name: (False) for state_name in
        states_which_were_not_renamed}
    state_property_ignore_list = [exp_domain.STATE_PROPERTY_RECORDED_VOICEOVERS
        ]
    for change in change_list:
        if (change.cmd == exp_domain.CMD_EDIT_STATE_PROPERTY and change.
            property_name not in state_property_ignore_list):
    ...
    for state_name, state_property_changed in state_property_changed_data.items(
        ...
        if state_property_changed:
            diff_dict = deepdiff.DeepDiff(old_states_dict[state_name],
                new_states_dict[state_name])
            if diff_dict:
                states_version_history[state_name
                    ] = state_domain.StateVersionHistory(prev_version,
                    state_name, committer_id)
...
def update_metadata_version_history(metadata_version_history: exp_domain.
    MetadataVersionHistory, change_list: Sequence[exp_domain.
    ExplorationChange], old_metadata_dict: exp_domain.
    ExplorationMetadataDict, new_metadata_dict: exp_domain.
    ExplorationMetadataDict, current_version: int, committer_id: str
    ...
    """
    prev_version = current_version - 1
    metadata_was_changed = any(change.cmd == exp_domain.
        CMD_EDIT_EXPLORATION_PROPERTY for change in change_list)
...
def get_updated_committer_ids(states_version_history: Dict[str,
    state_domain.StateVersionHistory], metadata_last_edited_committer_id: str
    ) ->List[str]:
    ...
    """
    committer_ids = {version_history.committer_id for version_history in
        states_version_history.values()}
    committer_ids.add(metadata_last_edited_committer_id)
    return list(committer_ids)


def get_updated_version_history_model(exploration: exp_domain.Exploration,
    change_list: Sequence[exp_domain.ExplorationChange], committer_id: str,
    old_states: Dict[str, state_domain.State], old_metadata: exp_domain.
    ExplorationMetadata) ->Optional[exp_models.ExplorationVersionHistoryModel]:
    ...
    """
    version_history_model_id = (exp_models.ExplorationVersionHistoryModel.
        get_instance_id(exploration.id, exploration.version - 1))
    ...
    if version_history_model is not None:
        old_states_dict = {state_name: state.to_dict() for state_name,
            state in old_states.items()}
        new_states_dict = {state_name: state.to_dict() for state_name,
            state in exploration.states.items()}
        old_metadata_dict = old_metadata.to_dict()
        new_metadata_dict = exploration.get_metadata().to_dict()
        states_version_history = {state_name: state_domain.
            StateVersionHistory.from_dict(state_version_history_dict) for 
            state_name, state_version_history_dict in version_history_model
            .state_version_history.items()}
        ...
        updated_version_history_model_id = (exp_models.
            ExplorationVersionHistoryModel.get_instance_id(exploration.id,
            exploration.version))
        updated_version_history_model = (exp_models.
            ExplorationVersionHistoryModel(id=
            updated_version_history_model_id, exploration_id=exploration.id,
            exploration_version=exploration.version, state_version_history=
            {state_name: version_history.to_dict() for state_name,
            version_history in updated_states_version_history.items()},
...
def _compute_models_for_updating_exploration(committer_id: str, exploration:
    exp_domain.Exploration, commit_message: Optional[str], change_list:
    Sequence[exp_domain.ExplorationChange]) ->List[base_models.BaseModel]:
    ...
    if exploration.version > exploration_model.version:
        raise Exception(
            'Unexpected error: trying to update version %s of exploration from version %s. Please reload the page and try again.'
             % (exploration_model.version, exploration.version))
    if exploration.version < exploration_model.version:
        raise Exception(
            'Trying to update version %s of exploration from version %s, which is too old. Please reload the page and try again.'
             % (exploration_model.version, exploration.version))
    old_states = exp_fetchers.get_exploration_from_model(exploration_model
        ).states
    old_metadata = exp_fetchers.get_exploration_from_model(exploration_model
        ).get_metadata()
    exploration_model = populate_exp_model_fields(exploration_model,
        exploration)
    change_list_dict = [change.to_dict() for change in change_list]
    models_to_put.extend(exploration_model.get_models_to_put_values(
        committer_id, commit_message, change_list_dict))
    caching_services.delete_multi(caching_services.
        CACHE_NAMESPACE_EXPLORATION, None, [exploration.id])
    exploration.version += 1
    exp_versions_diff = exp_domain.ExplorationVersionsDiff(change_list)
    version_history_model = get_updated_version_history_model(exploration,
        change_list, committer_id, old_states, old_metadata)
    if version_history_model is not None:
        models_to_put.append(version_history_model)
    new_exp_stats = stats_services.get_stats_for_new_exp_version(exploration
        .id, exploration.version, list(exploration.states.keys()),
        exp_versions_diff, revert_to_version=None)
    new_state_stats_mapping = stats_services.get_state_stats_mapping(
        new_exp_stats)
    new_exp_stats_instance_id = (stats_models.ExplorationStatsModel.
        get_entity_id(new_exp_stats.exp_id, new_exp_stats.exp_version))
    models_to_put.append(stats_models.ExplorationStatsModel(id=
        new_exp_stats_instance_id, exp_id=new_exp_stats.exp_id, exp_version
        =new_exp_stats.exp_version, num_starts_v1=new_exp_stats.
        num_starts_v1, num_starts_v2=new_exp_stats.num_starts_v2,
    ...
    models_to_put.extend(stats_services.
        get_updated_exp_issues_models_for_new_exp_version(exploration,
        exp_versions_diff, None))
    return models_to_put


def _create_exploration(committer_id: str, exploration: exp_domain.
    Exploration, commit_message: str, commit_cmds: List[exp_domain.
    ExplorationChange]) ->None:
    ...
    model = exp_models.ExplorationModel(id=exploration.id, category=
        exploration.category, title=exploration.title, objective=
        exploration.objective, language_code=exploration.language_code,
        tags=exploration.tags, blurb=exploration.blurb, author_notes=
        exploration.author_notes, states_schema_version=exploration.
        states_schema_version, init_state_name=exploration.init_state_name,
        states={state_name: state.to_dict() for state_name, state in
        exploration.states.items()}, param_specs=exploration.
        param_specs_dict, param_changes=exploration.param_change_dicts,
    ...
    exploration.version += 1
    version_history_model = exp_models.ExplorationVersionHistoryModel(id=
        exp_models.ExplorationVersionHistoryModel.get_instance_id(
        exploration.id, exploration.version), exploration_id=exploration.id,
        exploration_version=exploration.version, state_version_history={
        state_name: state_domain.StateVersionHistory(None, None,
        committer_id).to_dict() for state_name in exploration.states},
        metadata_last_edited_version_number=None,
        metadata_last_edited_committer_id=committer_id, committer_ids=[
        committer_id])
    ...
    stats_services.create_exp_issues_for_new_exploration(exploration.id,
        exploration.version)
    regenerate_exploration_summary_with_new_contributor(exploration.id,
        committer_id)


def save_new_exploration(committer_id: str, exploration: exp_domain.Exploration
    ) ->None:
    ...
    _create_exploration(committer_id, exploration, commit_message, [
        exp_domain.CreateNewExplorationCmd({'cmd': exp_domain.
        CMD_CREATE_NEW, 'title': exploration.title, 'category': exploration
        .category})])
...
def delete_exploration(committer_id: str, exploration_id: str,
    ...
    """
    delete_explorations(committer_id, [exploration_id], force_deletion=
        force_deletion)


def delete_explorations(committer_id: str, exploration_ids: List[str],
    force_deletion: bool=False) ->None:
    ...
    Args:
        ...
        force_deletion: bool. If True, completely deletes the storage models
            ...
            but keeps the corresponding models in the datastore.
    """
    exp_models.ExplorationRightsModel.delete_multi(exploration_ids,
        committer_id, '', force_deletion=force_deletion)
    exp_models.ExplorationModel.delete_multi(exploration_ids, committer_id,
        feconf.COMMIT_MESSAGE_EXPLORATION_DELETED, force_deletion=
        force_deletion)
    caching_services.delete_multi(caching_services.
        CACHE_NAMESPACE_EXPLORATION, None, exploration_ids)
    ...
    activity_services.remove_featured_activities(constants.
        ACTIVITY_TYPE_EXPLORATION, exploration_ids)
    feedback_services.delete_threads_for_multiple_entities(feconf.
        ENTITY_TYPE_EXPLORATION, exploration_ids)
    taskqueue_services.defer(taskqueue_services.
        FUNCTION_ID_DELETE_EXPS_FROM_USER_MODELS, taskqueue_services.
        QUEUE_NAME_ONE_OFF_JOBS, exploration_ids)
    taskqueue_services.defer(taskqueue_services.
        FUNCTION_ID_DELETE_EXPS_FROM_ACTIVITIES, taskqueue_services.
        QUEUE_NAME_ONE_OFF_JOBS, exploration_ids)
...
def delete_explorations_from_user_models(exploration_ids: List[str]) ->None:
    ...
    subscription_models: Sequence[user_models.UserSubscriptionsModel
        ] = user_models.UserSubscriptionsModel.query(user_models.
        UserSubscriptionsModel.exploration_ids.IN(exploration_ids)).fetch()
    for model in subscription_models:
        model.exploration_ids = [id_ for id_ in model.exploration_ids if 
            id_ not in exploration_ids]
    user_models.UserSubscriptionsModel.update_timestamps_multi(list(
        subscription_models))
    ...
    exp_user_data_models: Sequence[user_models.ExplorationUserDataModel
        ] = user_models.ExplorationUserDataModel.get_all().filter(user_models
        .ExplorationUserDataModel.exploration_id.IN(exploration_ids)).fetch()
    user_models.ExplorationUserDataModel.delete_multi(list(
    ...
    user_contributions_models: Sequence[user_models.UserContributionsModel
        ...
        datastore_services.any_of(user_models.UserContributionsModel.
        created_exploration_ids.IN(exploration_ids), user_models.
        UserContributionsModel.edited_exploration_ids.IN(exploration_ids))
        ).fetch()
    for contribution_model in user_contributions_models:
        contribution_model.created_exploration_ids = [exp_id for exp_id in
            contribution_model.created_exploration_ids if exp_id not in
            exploration_ids]
        contribution_model.edited_exploration_ids = [exp_id for exp_id in
            contribution_model.edited_exploration_ids if exp_id not in
            exploration_ids]
    user_models.UserContributionsModel.update_timestamps_multi(list(
        user_contributions_models))
    user_models.UserContributionsModel.put_multi(list(
        user_contributions_models))
...
def delete_explorations_from_activities(exploration_ids: List[str]) ->None:
    ...
    if not exploration_ids:
        return
    model_classes: List[Union[Type[user_models.CompletedActivitiesModel],
        Type[user_models.IncompleteActivitiesModel]]] = [user_models.
        CompletedActivitiesModel, user_models.IncompleteActivitiesModel]
    all_entities: List[AcceptableActivityModelTypes] = []
    for model_class in model_classes:
        entities: Sequence[AcceptableActivityModelTypes] = model_class.query(
            model_class.exploration_ids.IN(exploration_ids)).fetch()
...
def get_exploration_snapshots_metadata(exploration_id: str, allow_deleted:
    bool=False) ->List[SnapshotsMetadataDict]:
    ...
    return exp_models.ExplorationModel.get_snapshots_metadata(exploration_id,
        version_nums, allow_deleted=allow_deleted)
...
def publish_exploration_and_update_user_profiles(committer: user_domain.
    UserActionsInfo, exp_id: str) ->None:
    ...
    email_subscription_services.inform_subscribers(committer.user_id,
        exp_id, exp_title)
    contribution_time_msec = utils.get_current_time_in_millisecs()
    contributor_ids = exp_fetchers.get_exploration_summary_by_id(exp_id
        ).contributor_ids
    for contributor in contributor_ids:
        contributor_user_settings = user_services.get_user_settings(contributor
            , strict=False)
...
def validate_exploration_for_story(exp: exp_domain.Exploration, strict: bool
    ) ->List[str]:
    ...
    if (exp.language_code not in android_validation_constants.
        SUPPORTED_LANGUAGES):
        error_string = (
    ...
    for state_name in exp.states:
        ...
        if state.interaction.id == 'EndExploration':
            recommended_exploration_ids = cast(List[str], state.interaction
        ...
        if state.interaction.id == 'MultipleChoiceInput':
            choices = cast(List[state_domain.SubtitledHtml], state.
                interaction.customization_args['choices'].value)
            if len(choices) < 4:
                error_string = (
        ...
        for answer_group in state.interaction.answer_groups:
            if len(answer_group.training_data) > 0:
                ...
                validation_error_messages.append(error_string)
                break
        if state.interaction.default_outcome is not None and len(state.
            interaction.default_outcome.param_changes) > 0:
            error_string = (
...
def update_exploration(committer_id: str, exploration_id: str, change_list:
    Optional[Sequence[exp_domain.ExplorationChange]], commit_message:
    Optional[str], is_by_voice_artist: bool=False) ->None:
    ...
    models_to_put = compute_models_to_put_when_saving_new_exp_version(
        committer_id=committer_id, exploration_id=exploration_id,
        change_list=change_list, commit_message=commit_message,
    ...
    caching_services.delete_multi(caching_services.
        CACHE_NAMESPACE_EXPLORATION, None, [exploration_id])


def compute_models_to_put_when_saving_new_exp_version(committer_id: str,
    exploration_id: str, change_list: Optional[Sequence[exp_domain.
    ExplorationChange]], commit_message: Optional[str], is_by_voice_artist:
    bool=False) ->List[base_models.BaseModel]:
    ...
    models_to_put.extend(_compute_models_for_updating_exploration(
        committer_id, updated_exploration, commit_message, change_list))
    voiceover_services.update_exploration_voice_artist_link_model(committer_id,
        change_list, old_exploration, updated_exploration)
    new_content_id_set = set(updated_exploration.get_translatable_content_ids()
        )
    content_ids_corresponding_translations_to_remove = (old_content_id_set -
        new_content_id_set)
    voiceover_changes = []
    for change in change_list:
    ...
    translation_changes = []
    for change in change_list:
        if not change.cmd in [exp_domain.CMD_EDIT_TRANSLATION, exp_domain.
            CMD_REMOVE_TRANSLATIONS, exp_domain.
            CMD_MARK_TRANSLATIONS_NEEDS_UPDATE, exp_domain.
            CMD_MARK_TRANSLATION_NEEDS_UPDATE_FOR_LANGUAGE]:
            continue
        if change.cmd == exp_domain.CMD_REMOVE_TRANSLATIONS:
            content_ids_corresponding_translations_to_remove.add(change.
                content_id)
        translation_changes.append(change)
    new_translation_models, translation_counts = (translation_services.
        compute_translation_related_change(updated_exploration,
        translation_changes))
    ...
    if committer_id != feconf.MIGRATION_BOT_USER_ID:
        user_contributions = (user_services.
            get_or_create_new_user_contributions(committer_id))
        user_contributions.add_edited_exploration_id(exploration_id)
        models_to_put.append(user_services.
            get_validated_user_contributions_model(user_contributions))
        ...
        if user_settings is not None:
            ...
            if not rights_manager.is_exploration_private(exploration_id):
                user_settings.update_first_contribution_msec(utils.
                    get_current_time_in_millisecs())
            models_to_put.append(user_services.
                convert_to_user_settings_model(user_settings))
    if opportunity_services.is_exploration_available_for_contribution(
        exploration_id):
        models_to_put.extend(opportunity_services.
            compute_opportunity_models_with_updated_exploration(
            exploration_id, updated_exploration.get_content_count(),
            translation_counts))
    ...
    exp_summary = update_exploration_summary(updated_exploration,
        exp_rights, exp_fetchers.get_exploration_summary_from_model(
        exp_summary_model), skip_exploration_model_last_updated=True)
...
def regenerate_exploration_summary_with_new_contributor(exploration_id: str,
    ...
    if exploration is not None:
        ...
        if exp_summary is None:
            updated_exp_summary = generate_new_exploration_summary(exploration,
                exp_rights)
        else:
            updated_exp_summary = update_exploration_summary(exploration,
                exp_rights, exp_summary)
...
def regenerate_exploration_and_contributors_summaries(exploration_id: str
    ) ->None:
...
def update_exploration_summary(exploration: exp_domain.Exploration,
    exp_rights: rights_domain.ActivityRights, exp_summary: exp_domain.
    ExplorationSummary, skip_exploration_model_last_updated: bool=False
    ...
    if skip_exploration_model_last_updated:
        exploration_model_last_updated = (exp_summary.
            exploration_model_last_updated)
    else:
        exploration_model_last_updated = from_milliseconds_utc(
            get_last_updated_by_human_ms(exploration.id) / 1000.0 * 1000.0)
    ...
    return exp_domain.ExplorationSummary(exploration.id, exploration.title,
        exploration.category, exploration.objective, exploration.
        language_code, exploration.tags, exp_summary.ratings,
        scaled_average_rating, exp_rights.status, exp_rights.
        community_owned, exp_rights.owner_ids, exp_rights.editor_ids,
        exp_rights.voice_artist_ids, exp_rights.viewer_ids, contributor_ids,
        exp_summary.contributors_summary, exploration.version, exploration.
        created_on, exploration_model_last_updated, exp_rights.
        first_published_msec)


def generate_new_exploration_summary(exploration: exp_domain.Exploration,
    exp_rights: rights_domain.ActivityRights) ->exp_domain.ExplorationSummary:
    ...
    return exp_domain.ExplorationSummary(exploration.id, exploration.title,
        exploration.category, exploration.objective, exploration.
        language_code, exploration.tags, ratings, scaled_average_rating,
        exp_rights.status, exp_rights.community_owned, exp_rights.owner_ids,
        exp_rights.editor_ids, exp_rights.voice_artist_ids, exp_rights.
        viewer_ids, [], {}, exploration.version, exploration.created_on,
        exploration_model_last_updated, exp_rights.first_published_msec)


def compute_exploration_contributors_summary(exploration_id: str) ->Dict[
    str, int]:
...
def save_exploration_summary(exp_summary: exp_domain.ExplorationSummary
    ) ->None:
    ...
    """
    existing_exp_summary_model = exp_models.ExpSummaryModel.get(exp_summary
        .id, strict=False)
...
def delete_exploration_summaries(exploration_ids: List[str]) ->None:
    ...
    existing_summary_models = [summary_model for summary_model in
        summary_models if summary_model is not None]
    exp_models.ExpSummaryModel.delete_multi(existing_summary_models)


def revert_version_history(exploration_id: str, current_version: int,
    revert_to_version: int) ->None:
    ...
    """
    version_history_model_id = (exp_models.ExplorationVersionHistoryModel.
        get_instance_id(exploration_id, revert_to_version))
    version_history_model = exp_models.ExplorationVersionHistoryModel.get(
        version_history_model_id, strict=False)
    if version_history_model is not None:
        new_version_history_model = exp_models.ExplorationVersionHistoryModel(
            ...
            exploration_id, current_version + 1), exploration_id=
            exploration_id, exploration_version=current_version + 1,
            state_version_history=version_history_model.
            state_version_history, metadata_last_edited_version_number=
            version_history_model.metadata_last_edited_version_number,
            metadata_last_edited_committer_id=version_history_model.
            metadata_last_edited_committer_id, committer_ids=
            version_history_model.committer_ids)
        new_version_history_model.update_timestamps()
        new_version_history_model.put()


def get_exploration_validation_error(exploration_id: str, revert_to_version:
    int) ->Optional[str]:
    ...
    """
    exploration = exp_fetchers.get_exploration_by_id(exploration_id,
        version=revert_to_version)
    exploration_rights = rights_manager.get_exploration_rights(exploration.id)
    try:
        exploration.validate(exploration_rights.status == rights_domain.
...
def revert_exploration(committer_id: str, exploration_id: str,
    current_version: int, revert_to_version: int) ->None:
    ...
    exploration = exp_fetchers.get_exploration_by_id(exploration_id,
        version=revert_to_version)
    exploration_rights = rights_manager.get_exploration_rights(exploration.id)
    exploration_is_public = (exploration_rights.status != rights_domain.
        ACTIVITY_STATUS_PRIVATE)
    exploration.validate(strict=exploration_is_public)
    exp_models.ExplorationModel.revert(exploration_model, committer_id, 
    ...
    new_translation_models, translation_counts = (translation_services.
        compute_translation_related_changes_upon_revert(
    ...
    if opportunity_services.is_exploration_available_for_contribution(
        exploration_id):
        translation_and_opportunity_models_to_put.extend(opportunity_services
            .compute_opportunity_models_with_updated_exploration(
            exploration_id, exploration.get_content_count(),
            translation_counts))
    ...
    exp_issues_models_to_put = (stats_services.
        get_updated_exp_issues_models_for_new_exp_version(
...
def get_demo_exploration_components(demo_path: str) ->Tuple[str, List[Tuple
    [str, bytes]]]:
...
def save_new_exploration_from_yaml_and_assets(committer_id: str,
    yaml_content: str, exploration_id: str, assets_list: List[Tuple[str,
    bytes]], strip_voiceovers: bool=False) ->None:
...
def delete_demo(exploration_id: str) ->None:
    ...
    if not exploration:
        logging.info(
            'Exploration with id %s was not deleted, because it does not exist.'
             % exploration_id)
    else:
        delete_exploration(feconf.SYSTEM_COMMITTER_ID, exploration_id,
            force_deletion=True)
...
def load_demo(exploration_id: str) ->None:
    ...
    save_new_exploration_from_yaml_and_assets(feconf.SYSTEM_COMMITTER_ID,
        yaml_content, exploration_id, assets_list)
    publish_exploration_and_update_user_profiles(user_services.
        get_system_user(), exploration_id)
    index_explorations_given_ids([exploration_id])
    logging.info('Exploration with id %s was loaded.' % exploration_id)


def get_next_page_of_all_non_private_commits(page_size: int=feconf.
    COMMIT_LIST_PAGE_SIZE, urlsafe_start_cursor: Optional[str]=None,
    max_age: Optional[datetime.timedelta]=None) ->Tuple[List[exp_domain.
    ExplorationCommitLogEntry], Optional[str], bool]:
    ...
    results, new_urlsafe_start_cursor, more = (exp_models.
        ExplorationCommitLogEntryModel.get_all_non_private_commits(
        page_size, urlsafe_start_cursor, max_age=max_age))
    return [exp_domain.ExplorationCommitLogEntry(entry.created_on, entry.
        last_updated, entry.user_id, entry.exploration_id, entry.
        commit_type, entry.commit_message, entry.commit_cmds, entry.version,
        entry.post_commit_status, entry.post_commit_community_owned, entry.
        post_commit_is_private) for entry in results
        ], new_urlsafe_start_cursor, more


def get_image_filenames_from_exploration(exploration: exp_domain.Exploration
    ) ->List[str]:
    ...
    for state in exploration.states.values():
        if state.interaction.id == 'ImageClickInput':
            image_paths = cast(domain.ImageAndRegionDict, state.interaction
                .customization_args['imageAndRegions'].value)
            filenames.append(image_paths['imagePath'])
    html_list = exploration.get_all_html_content_strings()
    filenames.extend(html_cleaner.get_image_filenames_from_html_strings(
        html_list))
    return filenames
...
def index_explorations_given_ids(exp_ids: List[str]) ->None:
    ...
    """
    exploration_summaries = (exp_fetchers.
        get_exploration_summaries_matching_ids(exp_ids))
    search_services.index_exploration_summaries([exploration_summary for
        exploration_summary in exploration_summaries if exploration_summary
         is not None])


def is_voiceover_change_list(change_list: Sequence[exp_domain.
    ExplorationChange]) ->bool:
    ...
    Returns:
        ...
        allowed for voice artist to do.
    """
    voiceover_with_accent_feature_is_enabled = (feature_flag_services.
        is_feature_flag_enabled(feature_flag_list.FeatureNames.
        ADD_VOICEOVER_WITH_ACCENT.value, None))
    for change in change_list:
...
def get_composite_change_list(exp_id: str, from_version: int, to_version: int
    ...
    composite_change_list = [exp_domain.ExplorationChange(change) for
        change in composite_change_list_dict]
    return composite_change_list


def are_changes_mergeable(exp_id: str, change_list_version: int,
    change_list: List[exp_domain.ExplorationChange]) ->bool:
    ...
    if current_exploration.version < change_list_version:
        return False
    composite_change_list = get_composite_change_list(exp_id,
        change_list_version, current_exploration.version)
    exp_at_change_list_version = exp_fetchers.get_exploration_by_id(exp_id,
        version=change_list_version)
    changes_are_mergeable, send_email = (exp_domain.
        ExplorationChangeMergeVerifier(composite_change_list).
        is_change_list_mergeable(change_list, exp_at_change_list_version,
        current_exploration))
    if send_email:
        change_list_dict = [change.to_dict() for change in change_list]
        email_manager.send_not_mergeable_change_list_to_admin_for_review(exp_id
            , change_list_version, current_exploration.version,
            change_list_dict)
...
def get_user_exploration_data(user_id: str, exploration_id: str,
    apply_draft: bool=False, version: Optional[int]=None
    ) ->UserExplorationDataDict:
    """Returns a description of the given exploration."""
    exp_user_data = user_models.ExplorationUserDataModel.get(user_id,
        exploration_id)
    is_valid_draft_version = (is_version_of_draft_valid(exploration_id,
        exp_user_data.draft_change_list_exp_version) if exp_user_data and
        exp_user_data.draft_change_list_exp_version else None)
    if apply_draft:
        updated_exploration = get_exp_with_draft_applied(exploration_id,
            user_id)
    ...
    draft_changes = (exp_user_data.draft_change_list if exp_user_data and
        exp_user_data.draft_change_list else None)
    draft_change_list_id = (exp_user_data.draft_change_list_id if
        exp_user_data else 0)
    exploration_email_preferences = (user_services.
        get_email_preferences_for_exploration(user_id, exploration_id))
    editor_dict: UserExplorationDataDict = {'auto_tts_enabled': exploration
        .auto_tts_enabled, 'category': exploration.category,
        'draft_change_list_id': draft_change_list_id, 'exploration_id':
        exploration_id, 'init_state_name': exploration.init_state_name,
        'language_code': exploration.language_code, 'objective':
        exploration.objective, 'param_changes': exploration.
        param_change_dicts, 'param_specs': exploration.param_specs_dict,
        'rights': rights_manager.get_exploration_rights(exploration_id).
        to_dict(), 'show_state_editor_tutorial_on_load': False,
        'show_state_translation_tutorial_on_load': False, 'states': states,
        'tags': exploration.tags, 'title': exploration.title, 'version':
        exploration.version, 'is_version_of_draft_valid':
        is_valid_draft_version, 'draft_changes': draft_changes,
        'email_preferences': exploration_email_preferences.to_dict(),
        'next_content_id_index': exploration.next_content_id_index,
        'edits_allowed': exploration.edits_allowed, 'exploration_metadata':
        exploration.get_metadata().to_dict()}
    return editor_dict


def create_or_update_draft(exp_id: str, user_id: str, change_list: Sequence
    [exp_domain.ExplorationChange], exp_version: int, current_datetime:
    datetime.datetime, is_by_voice_artist: bool=False) ->None:
    ...
    if (exp_user_data and exp_user_data.draft_change_list and exp_user_data
    ...
    if exp_user_data is None:
        exp_user_data = user_models.ExplorationUserDataModel.create(user_id,
            exp_id)
...
def get_exp_with_draft_applied(exp_id: str, user_id: str) ->Optional[exp_domain
    .Exploration]:
    ...
    if exp_user_data:
        if exp_user_data.draft_change_list:
            draft_change_list_exp_version = (exp_user_data.
                draft_change_list_exp_version)
            draft_change_list = [exp_domain.ExplorationChange(change) for
                change in exp_user_data.draft_change_list]
            if (exploration.version > exp_user_data.
                draft_change_list_exp_version):
                logging.info(
                    "Exploration and draft versions out of sync, trying to upgrade draft version to match exploration's."
                    )
                new_draft_change_list = (draft_upgrade_services.
                    try_upgrading_draft_to_exp_version(draft_change_list,
    ...
    if (exp_user_data and exp_user_data.draft_change_list and
        are_changes_mergeable(exp_id, draft_change_list_exp_version,
        draft_change_list)):
...
def get_exp_user_data_model_with_draft_discarded(exp_id: str, user_id: str
...
def regenerate_missing_stats_for_exploration(exp_id: str) ->Tuple[List[str],
    List[str], int, int]:
    ...
    if all(exp_stats is None for exp_stats in exp_stats_list):
        for index, version in enumerate(exp_versions):
            exp_stats_for_version = (stats_services.
                get_stats_for_new_exploration(exp_id, version, list(
    ...
    snapshots = exp_models.ExplorationModel.get_snapshots_metadata(exp_id,
        exp_versions)
    change_lists = []
    for snapshot in snapshots:
        ...
        for commit_cmd in snapshot['commit_cmds']:
            try:
                change_list_for_snapshot.append(exp_domain.
                    ExplorationChange(commit_cmd))
            except utils.ValidationError:
                logging.error(
                    'Exploration(id=%r) snapshots contains invalid commit_cmd: %r'
                     % (exp_id, commit_cmd))
                continue
    ...
    for i, (exp_stats, exp, change_list) in enumerate(zipped_items):
        revert_to_version = next((int(change.version_number) for change in
            change_list if change.cmd == revert_commit_cmd), None)
        ...
        if exp_stats:
            ...
            new_exploration_stats = (stats_services.
                get_stats_for_new_exploration(exp_id, exp.version, list(exp
                .state_interaction_ids_dict.keys())))
            stats_services.create_stats_model(new_exploration_stats)
            missing_exp_stats_indices.append(i)
            missing_exp_stats.append(
                'ExplorationStats(exp_id=%r, exp_version=%r)' % (exp_id,
                exp.version))
            num_valid_state_stats += len(new_exploration_stats.
                state_stats_mapping)
            continue
        else:
            ...
            if exp_stats is None:
                new_exploration_stats = (stats_services.
                    get_stats_for_new_exploration(exp_id, exp.version, list
        ...
        for state_name in exp.state_interaction_ids_dict.keys():
            ...
            try:
                prev_interaction_id = (prev_exp.state_interaction_ids_dict[
                    prev_state_name] if prev_state_name in prev_exp.
                    state_interaction_ids_dict else None)
                ...
                if (current_interaction_id != prev_interaction_id or 
                    ...
                    exp_stats_list_item.state_stats_mapping[state_name
                        ] = prev_exp_stats.state_stats_mapping[prev_state_name
                        ].clone()
                ...
                raise Exception(
                    'Exploration(id=%r, exp_version=%r) has no State(name=%r): %r'
                     % (exp_id, exp_stats.exp_version, prev_state_name, {
                    'added_state_names': exp_versions_diff.
                    added_state_names, 'deleted_state_names':
                    exp_versions_diff.deleted_state_names,
                    'new_to_old_state_names': exp_versions_diff.
                    new_to_old_state_names, 'old_to_new_state_names':
                    exp_versions_diff.old_to_new_state_names,
                    'prev_exp.states': prev_exp.state_interaction_ids_dict.
                    keys(), 'prev_exp_stats': prev_exp_stats})) from e
    ...
    return (missing_exp_stats, missing_state_stats, num_valid_exp_stats,
        num_valid_state_stats)


def update_logged_out_user_progress(exploration_id: str,
    unique_progress_url_id: str, state_name: str, exp_version: int) ->None:
    ...
    current_exploration = exp_fetchers.get_exploration_by_id(exploration_id,
        strict=True, version=exp_version)
    if checkpoint_url_model.furthest_reached_checkpoint_state_name is None:
        ...
        furthest_reached_checkpoint_exp = exp_fetchers.get_exploration_by_id(
            exploration_id, strict=True, version=checkpoint_url_model.
        ...
        furthest_reached_checkpoint_in_current_exp = (user_services.
            get_most_distant_reached_checkpoint_in_current_exploration(
            checkpoints_in_current_exp, checkpoints_in_older_exp,
            checkpoint_url_model.furthest_reached_checkpoint_state_name))
...
@overload
def sync_logged_out_learner_checkpoint_progress_with_current_exp_version(
    exploration_id: str, unique_progress_url_id: str) ->Optional[exp_domain
    .TransientCheckpointUrl]:
    ...
...
def sync_logged_out_learner_checkpoint_progress_with_current_exp_version(
    ...
    furthest_reached_exploration = exp_fetchers.get_exploration_by_id(
        exploration_id, strict=True, version=checkpoint_url_model.
        furthest_reached_checkpoint_exp_version)
    most_recently_reached_checkpoint_in_current_exploration = (user_services
        .get_most_distant_reached_checkpoint_in_current_exploration(
        user_services.get_checkpoints_in_order(latest_exploration.
        init_state_name, latest_exploration.states), user_services.
        get_checkpoints_in_order(most_recently_interacted_exploration.
        init_state_name, most_recently_interacted_exploration.states),
        checkpoint_url_model.most_recently_reached_checkpoint_state_name))
    furthest_reached_checkpoint_in_current_exploration = (user_services.
        get_most_distant_reached_checkpoint_in_current_exploration(
        user_services.get_checkpoints_in_order(latest_exploration.
        init_state_name, latest_exploration.states), user_services.
        get_checkpoints_in_order(furthest_reached_exploration.
        init_state_name, furthest_reached_exploration.states),
        checkpoint_url_model.furthest_reached_checkpoint_state_name))
...
def sync_logged_out_learner_progress_with_logged_in_progress(user_id: str,
    exploration_id: str, unique_progress_url_id: str) ->None:
    ...
    if logged_out_user_data is None:
        return
    latest_exploration = exp_fetchers.get_exploration_by_id(exploration_id)
    exp_user_data = exp_fetchers.get_exploration_user_data(user_id,
        exploration_id)
    logged_in_user_model = user_models.ExplorationUserDataModel.get(user_id,
        exploration_id)
    if logged_in_user_model is None or exp_user_data is None:
        ...
        current_exploration = exp_fetchers.get_exploration_by_id(exploration_id
            , strict=True, version=logged_out_user_data.
            most_recently_reached_checkpoint_exp_version)
        recent_checkpoint_state_name = (exp_user_data.
            most_recently_reached_checkpoint_state_name)
        assert recent_checkpoint_state_name is not None
        most_recently_reached_checkpoint_index_in_logged_in_progress = (
            user_services.get_checkpoints_in_order(current_exploration.
            init_state_name, current_exploration.states).index(
        ...
        most_recently_interacted_exploration = (exp_fetchers.
            get_exploration_by_id(exploration_id, strict=True, version=
            exp_user_data.most_recently_reached_checkpoint_exp_version))
        furthest_reached_exploration = exp_fetchers.get_exploration_by_id(
            exploration_id, strict=True, version=exp_user_data.
            furthest_reached_checkpoint_exp_version)
        recent_checkpoint_state_name = (exp_user_data.
            most_recently_reached_checkpoint_state_name)
        assert recent_checkpoint_state_name is not None
        most_recently_reached_checkpoint_in_current_exploration = (
            ...
            init_state_name, most_recently_interacted_exploration.states),
            recent_checkpoint_state_name))
        furthest_checkpoint_state_name = (exp_user_data.
        ...
        most_recently_reached_checkpoint_index_in_logged_in_progress = (
            user_services.get_checkpoints_in_order(latest_exploration.
            init_state_name, latest_exploration.states).index(
            recent_checkpoint_state_name))
        most_recently_reached_checkpoint_index_in_logged_out_progress = (
            user_services.get_checkpoints_in_order(latest_exploration.
            init_state_name, latest_exploration.states).index(
...
def set_exploration_edits_allowed(exp_id: str, edits_are_allowed: bool) ->None:
    ...
    caching_services.delete_multi(caching_services.
        CACHE_NAMESPACE_EXPLORATION, None, [exp_id])
...
def rollback_exploration_to_safe_state(exp_id: str) ->int:
    ...
    models_to_delete: List[Union[exp_models.ExplorationSnapshotContentModel,
        exp_models.ExplorationSnapshotMetadataModel]] = []
    for version in range(current_version_in_exp_model, 1, -1):
        snapshot_content_model = (exp_models.
            ExplorationSnapshotContentModel.get('%s-%s' % (exp_id, version),
            strict=False))
        snapshot_metadata_model = (exp_models.
            ExplorationSnapshotMetadataModel.get('%s-%s' % (exp_id, version
            ), strict=False))
    ...
    if last_known_safe_version != current_version_in_exp_model:
        ...
        safe_exp_model = exp_models.ExplorationModel.get(exp_id, strict=
            True, version=last_known_safe_version)
        safe_exp_model.version = last_known_safe_version
        base_models.BaseModel.update_timestamps_multi([safe_exp_model,
            exp_summary_model])
