...
class FeedbackThreadDomainUnitTests(test_utils.GenericTestBase):
    ...
    def test_to_dict(self) -> None:
        ...
        observed_thread = feedback_domain.FeedbackThread(
            ...
            self.EXP_ID,
            expected_thread_dict['state_name'],
            self.viewer_id,
            expected_thread_dict['status'],
            expected_thread_dict['subject'],
            expected_thread_dict['summary'],
            False,
            1,
            fake_date,
            fake_date,
...
class FeedbackMessageDomainUnitTests(test_utils.GenericTestBase):
    ...
    def test_to_dict(self) -> None:
        ...
        observed_message = feedback_domain.FeedbackMessage(
            self.FULL_MESSAGE_ID,
            self.THREAD_ID,
            self.MESSAGE_ID,
            self.owner_id,
            expected_message_dict['updated_status'],
            expected_message_dict['updated_subject'],
            expected_message_dict['text'],
            fake_date,
            fake_date,
            False,
        )
...
class FeedbackAnalyticsDomainUnitTests(test_utils.GenericTestBase):
    ...
    def test_to_dict(self) -> None:
        ...
        self.assertDictEqual(
            expected_thread_analytics.to_dict(),
            {'num_open_threads': 1, 'num_total_threads': 2},
        )
...
class FeedbackMessageReferenceDomainTests(test_utils.GenericTestBase):
    ...
    def test_to_dict(self) -> None:
        ...
        observed_feedback_message_reference = feedback_domain.FeedbackMessageReference(
            feconf.ENTITY_TYPE_EXPLORATION, self.exp_id, self.thread_id, self.message_id
        )
