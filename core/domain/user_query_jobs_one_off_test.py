...
class UserQueryJobOneOffTests(test_utils.EmailTestBase):
    ...
    def test_user_has_not_logged_in_last_n_days(self):
        ...
        self._run_one_off_job(user_query_1.id)

        query_1 = user_models.UserQueryModel.get(user_query_1.id)
        ...
        self._run_one_off_job(user_query_2.id)

        query_2 = user_models.UserQueryModel.get(user_query_2.id)
        ...
        user_query_3 = user_query_services.save_new_user_query(
            self.submitter_id, has_not_logged_in_for_n_days=6)
        self._run_one_off_job(user_query_3.id)

        query = user_models.UserQueryModel.get(user_query_3.id)
    ...
    def test_user_is_inactive_in_last_n_days(self):
        ...
        self._run_one_off_job(user_query.id)

        query = user_models.UserQueryModel.get(user_query.id)
    ...
    def test_combination_of_query_params(self):
        ...
        query_1 = user_models.UserQueryModel.get(user_query_1.id)
        query_2 = user_models.UserQueryModel.get(user_query_2.id)
        query_combined = user_models.UserQueryModel.get(user_query_3.id)
    ...
    def test_that_correct_email_is_sent_upon_failure(self):
        ...
        self._run_one_off_job_resulting_in_failure(user_query.id)
        query = user_models.UserQueryModel.get(user_query.id)
