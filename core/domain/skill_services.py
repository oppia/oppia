    ...
    for topic in all_topics:
        ...
        if old_skill_id in topic.get_all_skill_ids():
            if new_skill_id in topic.get_all_skill_ids():
                raise Exception(
                    'Found topic \'%s\' contains the two skills to be merged. Please unassign one of these skills from topic and retry this operation.'
                    % topic.name
                )
            ...
            if change.cmd == skill_domain.CMD_UPDATE_SKILL_PROPERTY:
                if (
                    ...
                    if role_services.ACTION_EDIT_SKILL_DESCRIPTION not in (
                        ...
                        raise Exception(
                            'The user does not have enough rights to edit the skill description.'
                        )
...
def _save_skill(
    ...
    if not change_list:
        raise Exception(
            'Unexpected error: received an invalid change list when trying to save skill %s: %s'
            % (skill.id, change_list)
        )
    skill.validate()
    ...
    if skill.version > skill_model.version:
        raise Exception(
            'Unexpected error: trying to update version %s of skill from version %s. Please reload the page and try again.'
            % (skill_model.version, skill.version)
        )

    if skill.version < skill_model.version:
        raise Exception(
            'Trying to update version %s of skill from version %s, which is too old. Please reload the page and try again.'
            % (skill_model.version, skill.version)
        )
