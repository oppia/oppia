...
exp_models, user_models = models.Registry.import_models([models.Names.
    EXPLORATION, models.Names.USER])
...
def assign_rating_to_exploration(user_id: str, exploration_id: str,
    ...
    exploration = exp_fetchers.get_exploration_by_id(exploration_id, strict
        =False)
    ...
    def _update_user_rating_transactional() ->Optional[int]:
        ...
        before updation.
        """
        exp_user_data_model = user_models.ExplorationUserDataModel.get(user_id,
            exploration_id)
    ...
    event_services.RateExplorationEventHandler.record(exploration_id,
        user_id, new_rating, old_rating)
    exploration_summary.scaled_average_rating = (exp_services.
        get_scaled_average_rating(exploration_summary.ratings))
    exp_services.save_exploration_summary(exploration_summary)


def get_user_specific_rating_for_exploration(user_id: str, exploration_id: str
    ) ->Optional[int]:
...
def get_when_exploration_rated(user_id: str, exploration_id: str) ->Optional[
    datetime.datetime]:
