...
class ExplorationStatsTests(test_utils.GenericTestBase):
    ...
    def _get_exploration_stats_from_dict(
        ...
        return stats_domain.ExplorationStats(
            exploration_stats_dict['exp_id'], exploration_stats_dict['exp_version'],
    ...
    def test_to_dict(self) -> None:
        ...
        self.assertDictEqual(
            expected_exploration_stats_dict, observed_exploration_stats.to_dict()
        )
...
class StateStatsTests(test_utils.GenericTestBase):
    ...
    def test_from_dict(self) -> None:
        ...
        self.assertEqual(
            state_stats.total_hit_count_v1, expected_state_stats.total_hit_count_v1
        )
        self.assertEqual(
            state_stats.total_hit_count_v2, expected_state_stats.total_hit_count_v2
        )
        self.assertEqual(
            state_stats.first_hit_count_v1, expected_state_stats.first_hit_count_v1
        )
        self.assertEqual(
            state_stats.first_hit_count_v2, expected_state_stats.first_hit_count_v2
        )
        self.assertEqual(
        ...
        self.assertEqual(
            state_stats.num_completions_v1, expected_state_stats.num_completions_v1
        )
        self.assertEqual(
            state_stats.num_completions_v2, expected_state_stats.num_completions_v2
        )
    ...
    def test_aggregate_from_session_state_stats(self) -> None:
        ...
        self.assertEqual(
            state_stats,
            stats_domain.StateStats(10, 11, 10, 12, 10, 13, 10, 14, 15, 10, 16)
        )
...
class SessionStateStatsTests(test_utils.GenericTestBase):
    ...
    def test_aggregated_stats_validation_when_fully_valid(self) -> None:
        ...
        self.assertEqual(
            ...
            validate_aggregated_stats_dict(sessions_state_stats), sessions_state_stats
        )
...
class ExplorationIssueTests(test_utils.GenericTestBase):
    ...
    def test_update_exp_issue_from_model(self) -> None:
        ...
        convert_issue_dict_swap = self.swap(
            stats_domain.ExplorationIssue, '_convert_issue_v1_dict_to_v2_dict',
        ...
        self.assertEqual(
            exp_issue_from_model.unresolved_issues[0].
            issue_customization_args['time_spent_in_exp_in_msecs'],
            self.DUMMY_TIME_SPENT_IN_MSECS
        )
    ...
    def test_comparison_between_exploration_issues_returns_correctly(self) -> None:
        ...
        exp_issue1 = stats_domain.ExplorationIssue(
            'EarlyQuit', expected_customization_args, [], 1, True
        )
        exp_issue2 = stats_domain.ExplorationIssue(
            'EarlyQuit', expected_customization_args, [], 2, True
        )
...
class LearnerActionTests(test_utils.GenericTestBase):
    ...
    def _dummy_convert_action_v1_dict_to_v2_dict(
        self, action_dict: stats_domain.LearnerActionDict
    ...
    def test_update_learner_action_from_model(self) -> None:
        ...
        convert_action_dict_swap = self.swap(
            stats_domain.LearnerAction, '_convert_action_v1_dict_to_v2_dict',
...
class SubmittedAnswerTests(test_utils.GenericTestBase):
    ...
    def test_dict_may_not_include_rule_spec_str_or_answer_str(self) -> None:
        submitted_answer = stats_domain.SubmittedAnswer(
            'Text', 'TextInput', 0, 1, exp_domain.EXPLICIT_CLASSIFICATION, {}, 'sess',
            10.5
        )
        self.assertEqual(
            submitted_answer.to_dict(), {
...
class SubmittedAnswerValidationTests(test_utils.GenericTestBase):
    ...
    def test_time_spent_in_sec_must_be_positive(self) -> None:
        ...
        self._assert_validation_error(
            self.submitted_answer, 'Expected time_spent_in_sec to be non-negative'
        )
    ...
    def test_session_id_must_not_be_none(self) -> None:
        ...
        self._assert_validation_error(
            self.submitted_answer, 'SubmittedAnswers must have a provided session_id'
        )
    ...
    def test_answer_group_index_must_be_integer(self) -> None:
        ...
        self._assert_validation_error(
            self.submitted_answer, 'Expected answer_group_index to be an integer'
        )

    def test_answer_group_index_must_be_positive(self) -> None:
        self.submitted_answer.answer_group_index = -1
        self._assert_validation_error(
            self.submitted_answer, 'Expected answer_group_index to be non-negative'
        )
    ...
    def test_rule_spec_index_must_be_positive(self) -> None:
        ...
        self._assert_validation_error(
            self.submitted_answer, 'Expected rule_spec_index to be non-negative'
        )
    ...
    def test_classification_categorization_must_be_valid_category(self) -> None:
        ...
        self._assert_validation_error(
            self.submitted_answer, 'Expected valid classification_categorization'
        )
    ...
    def test_answer_str_must_be_none_or_string(self) -> None:
        ...
        self._assert_validation_error(
            self.submitted_answer, 'Expected answer_str to be either None or a string'
        )
...
class StateAnswersCalcOutputValidationTests(test_utils.GenericTestBase):
    ...
    def test_exploration_id_must_be_string(self) -> None:
        ...
        self._assert_validation_error(
            self.state_answers_calc_output, 'Expected exploration_id to be a string'
        )
    ...
    def test_state_name_must_be_string(self) -> None:
        self.state_answers_calc_output.state_name = [
            'state'
        ]  # type: ignore[assignment]
        self._assert_validation_error(
            self.state_answers_calc_output, 'Expected state_name to be a string'
        )
    ...
    def test_calculation_id_must_be_string(self) -> None:
        self.state_answers_calc_output.calculation_id = [
            'calculation id'
        ]  # type: ignore[assignment]
        self._assert_validation_error(
            self.state_answers_calc_output, 'Expected calculation_id to be a string'
        )
    ...
    def test_calculation_output_must_be_known_type(self) -> None:
        ...
        self._assert_validation_error(
            self.state_answers_calc_output, 'Expected calculation output to be one of'
        )
    ...
    def test_calculation_output_must_be_less_than_one_million_bytes(self) -> None:
        ...
        self._assert_validation_error(
            self.state_answers_calc_output, 'calculation_output is too big to be stored'
        )
...
class LearnerAnswerDetailsTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.learner_answer_details = stats_domain.LearnerAnswerDetails(
            'exp_id:state_name', feconf.ENTITY_TYPE_EXPLORATION, 'TextInput', [
                stats_domain.LearnerAnswerInfo(
    ...
    def test_from_dict(self) -> None:
        ...
        self.assertEqual(
            learner_answer_details.accumulated_answer_info_json_size_bytes, 4000
        )
    ...
    def test_state_reference_must_be_string(self) -> None:
        ...
        self._assert_validation_error(
            self.learner_answer_details, 'Expected state_reference to be a string'
        )
    ...
    def test_interaction_id_must_be_valid(self) -> None:
        ...
        self._assert_validation_error(
            self.learner_answer_details, 'Unknown interaction_id: MyInteraction'
        )
    ...
    def test_interaction_id_must_be_string(self) -> None:
        ...
        self._assert_validation_error(
            self.learner_answer_details, 'Expected interaction_id to be a string'
        )

    def test_continue_interaction_cannot_solicit_answer_details(self) -> None:
        self.learner_answer_details.interaction_id = 'Continue'
        self._assert_validation_error(
            self.learner_answer_details, 'The Continue interaction does not support '
...
class LearnerAnswerInfoTests(test_utils.GenericTestBase):
    ...
    def test_to_dict(self) -> None:
        ...
        self.assertEqual(
            expected_learner_answer_info_dict, self.learner_answer_info.to_dict()
        )
    ...
    def test_answer_must_not_be_empty_dict(self) -> None:
        ...
        self._assert_validation_error(
            self.learner_answer_info, 'The answer submitted cannot be an empty dict'
        )

    def test_answer_must_not_be_empty_string(self) -> None:
        self.learner_answer_info.answer = ''
        self._assert_validation_error(
            self.learner_answer_info, 'The answer submitted cannot be an empty string'
        )
    ...
    def test_answer_details_must_not_be_none(self) -> None:
        ...
        self._assert_validation_error(
            self.learner_answer_info, 'Expected answer_details to be a string'
        )
    ...
    def test_large_answer_details_must_not_be_stored(self) -> None:
        ...
        self._assert_validation_error(
            self.learner_answer_info, 'The answer details size is to large to be stored'
        )
    ...
    def test_created_on_must_be_datetime_type(self) -> None:
        ...
        self._assert_validation_error(
            self.learner_answer_info, 'Expected created_on to be a datetime'
        )
