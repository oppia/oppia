...
class StateStatsTests(test_utils.GenericTestBase):
    ...
    def test_repr(self) -> None:
        ...
        self.assertEqual(
            '%r' % (state_stats,),
            'StateStats(total_answers_count_v1=1, total_answers_count_v2=2, useful_feedback_count_v1=3, useful_feedback_count_v2=4, total_hit_count_v1=5, total_hit_count_v2=6, first_hit_count_v1=7, first_hit_count_v2=8, num_times_solution_viewed_v2=9, num_completions_v1=10, num_completions_v2=11)',
        )
    ...
    def test_str(self) -> None:
        ...
        self.assertEqual(
            '%s' % (state_stats,),
            'StateStats(total_answers_count=3, useful_feedback_count=7, total_hit_count=11, first_hit_count=15, num_times_solution_viewed=9, num_completions=21)',
        )
    ...
    def test_aggregate_from_state_stats(self) -> None:
        ...
        self.assertEqual(
            state_stats,
            stats_domain.StateStats(101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111),
        )
...
class SessionStateStatsTests(test_utils.GenericTestBase):
    ...
    def test_repr(self) -> None:
        ...
        self.assertEqual(
            ...
            'SessionStateStats(total_answers_count=1, useful_feedback_count=2, total_hit_count=3, first_hit_count=4, num_times_solution_viewed=5, num_completions=6)',
        )
    ...
    def test_aggregated_stats_validation_when_state_property_type_is_missing(
        ...
        with self.assertRaisesRegex(
            utils.ValidationError,
            'total_answers_count not in state stats mapping of Home in aggregated stats dict.',
        ):
            stats_domain.SessionStateStats.validate_aggregated_stats_dict(sessions_state_stats)
...
class ExplorationIssueTests(test_utils.GenericTestBase):
    ...
    def test_update_exp_issue_from_model(self) -> None:
        ...
        self.assertEqual(
            exp_issue_from_model.unresolved_issues[0].issue_customization_args['time_spent_in_exp_in_msecs'],
    ...
    def test_cannot_update_exp_issue_from_invalid_schema_version_model(
        ...
        with self.assertRaisesRegex(
            Exception,
            'Sorry, we can only process v1-v%d and unversioned issue schemas at present.' % stats_models.CURRENT_ISSUE_SCHEMA_VERSION,
        ):
    ...
    def test_cannot_update_exp_issue_with_no_schema_version(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            re.escape('unsupported operand type(s) for +=: \'NoneType\' and \'int\''),
        ):
            stats_services.get_exp_issues_from_model(exp_issues_model)
    ...
    def test_actual_update_exp_issue_from_model_raises_error(self) -> None:
        ...
        with self.assertRaisesRegex(
            NotImplementedError,
            re.escape('The _convert_issue_v1_dict_to_v2_dict() method is missing from the derived class. It should be implemented in the derived class.'),
        ):
...
class LearnerActionTests(test_utils.GenericTestBase):
    ...
    def test_cannot_update_learner_action_from_invalid_schema_version_model(
        ...
        with self.assertRaisesRegex(
            Exception,
            'Sorry, we can only process v1-v%d and unversioned action schemas at present.' % stats_models.CURRENT_ISSUE_SCHEMA_VERSION,
        ):
    ...
    def test_actual_update_learner_action_from_model_raises_error(self) -> None:
        ...
        with self.assertRaisesRegex(
            NotImplementedError,
            re.escape('The _convert_action_v1_dict_to_v2_dict() method is missing from the derived class. It should be implemented in the derived class.'),
        ):
        ...
        self.assertEqual(
            ...
            [
                ...
                },
                {
                    ...
                    'rule_spec_index': 0,
                    'classification_categorization': (exp_domain.DEFAULT_OUTCOME_CLASSIFICATION),
                    'params': {},
                    'session_id': 'sess',
...
class SubmittedAnswerValidationTests(test_utils.GenericTestBase):
    ...
    def test_answer_may_be_none_only_for_linear_interaction(self) -> None:
        ...
        self._assert_validation_error(
            ...
            'SubmittedAnswers must have a provided answer except for linear interactions',
        )
...
class CategorizedAnswerFrequencyListsDomainTests(test_utils.GenericTestBase):
    ...
    def test_create_list_from_raw_object(self) -> None:
        answer_frequency_lists = stats_domain.CategorizedAnswerFrequencyLists.from_raw_type(
            {
        ...
        answer_frequency_lists = stats_domain.CategorizedAnswerFrequencyLists(
            {
                ...
                'category b': stats_domain.AnswerFrequencyList([self.ANSWER_B, self.ANSWER_C]),
            }
        )
        self.assertEqual(
...
class LearnerAnswerDetailsTests(test_utils.GenericTestBase):
    ...
    def test_learner_answer_info_with_big_size_must_not_be_added(self) -> None:
        ...
        self.assertEqual(len(self.learner_answer_details.learner_answer_info_list), 1)
        for i in range(36):
            learner_answer_info = stats_domain.LearnerAnswerInfo('%s%s' % (id_base, i), answer, answer_details, created_on)
            self.learner_answer_details.add_learner_answer_info(learner_answer_info)
    ...
    def test_end_exploration_interaction_cannot_solicit_answer_details(
        ...
        self._assert_validation_error(
            self.learner_answer_details,
            'The EndExploration interaction does not support soliciting answer details',
        )
    ...
    def test_accumulated_answer_info_json_size_bytes_must_be_int(self) -> None:
        self.learner_answer_details.accumulated_answer_info_json_size_bytes = 'size'  # type: ignore[assignment]
        self._assert_validation_error(
            self.learner_answer_details,
