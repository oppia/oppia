...
class ExplorationIssueTests(test_utils.GenericTestBase):
    ...
    def test_cannot_update_exp_issue_from_invalid_schema_version_model(
        ...
        with self.assertRaisesRegex(
            Exception,
            'Sorry, we can only process v1-v%d and unversioned issue schemas at present.'
    ...
    def test_actual_update_exp_issue_from_model_raises_error(self) -> None:
        ...
        with self.assertRaisesRegex(
            NotImplementedError,
            re.escape(
                'The _convert_issue_v1_dict_to_v2_dict() method is missing from the derived class. It should be implemented in the derived class.'
            ),
        ):
...
class LearnerActionTests(test_utils.GenericTestBase):
    ...
    def test_cannot_update_learner_action_from_invalid_schema_version_model(
        ...
        with self.assertRaisesRegex(
            Exception,
            'Sorry, we can only process v1-v%d and unversioned action schemas at present.'
            % stats_models.CURRENT_ISSUE_SCHEMA_VERSION,
        ):
    ...
    def test_actual_update_learner_action_from_model_raises_error(self) -> None:
        ...
        with self.assertRaisesRegex(
            NotImplementedError,
            re.escape(
                'The _convert_action_v1_dict_to_v2_dict() method is missing from the derived class. It should be implemented in the derived class.'
            ),
        ):
