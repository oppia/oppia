...
base_models, beam_job_models, job_models, user_models = (models.Registry.
    import_models([models.Names.BASE_MODEL, models.Names.BEAM_JOB, models.
    Names.JOB, models.Names.USER]))
datastore_services = models.Registry.import_datastore_services()
MODEL_CLASSES_TO_MARK_AS_DELETED = {user_models.UserQueryModel: datetime.
    timedelta(days=30), beam_job_models.BeamJobRunModel: datetime.timedelta
    (days=180), beam_job_models.BeamJobRunResultModel: datetime.timedelta(
    days=180), job_models.JobModel: datetime.timedelta(days=180)}
...
def delete_models_marked_as_deleted() ->None:
    ...
    date_before_which_to_hard_delete = (date_now - feconf.
        PERIOD_TO_HARD_DELETE_MODELS_MARKED_AS_DELETED)
    for model_class in models.Registry.get_all_storage_model_classes():
        deleted_models: Sequence[base_models.BaseModel] = model_class.query(
            model_class.deleted == True).fetch()
        models_to_hard_delete: List[base_models.BaseModel] = [deleted_model for
            deleted_model in deleted_models if deleted_model.last_updated <
            date_before_which_to_hard_delete]
        if issubclass(model_class, base_models.VersionedModel):
            model_ids_to_hard_delete: List[str] = [model.id for model in
                models_to_hard_delete]
            model_class.delete_multi(model_ids_to_hard_delete, '', '',
                force_deletion=True)
        else:
...
def mark_outdated_models_as_deleted() ->None:
    ...
    for model_class, period_to_keep in MODEL_CLASSES_TO_MARK_AS_DELETED.items(
        ...
        date_before_which_to_mark_as_deleted = get_current_datetime_utc(
            ) - period_to_keep
        models_to_mark_as_deleted.extend(model_class.query(model_class.
            last_updated < date_before_which_to_mark_as_deleted).fetch())
