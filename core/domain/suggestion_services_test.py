...
class SuggestionServicesUnitTests(test_utils.GenericTestBase):
    ...
    def mock_accept_suggestion(
        self, suggestion_id: str, reviewer_id: str, commit_message: str,
        review_message: str
    ) -> None:
        ...
        with self.swap(
            ...
            with self.swap(
                exp_fetchers, 'get_exploration_by_id', self.mock_get_exploration_by_id
            ):
                with self.swap(
                    suggestion_registry.SuggestionEditStateContent,
                    'pre_accept_validate', self.mock_pre_accept_validate_does_nothing
                ):
                    with self.swap(
                        suggestion_registry.SuggestionEditStateContent,
        ...
        with self.swap(
            ...
            with self.swap(
                ...
                suggestion_services.create_suggestion(
                    ...
                    feconf.ENTITY_TYPE_EXPLORATION, target_id,
                    self.target_version_at_submission, self.author_id, self.change_cmd,
                    'test description'
                )
    ...
    explorations = [
        MockExploration('exp1', {
            'state_1': {},
            'state_2': {}
        }),
        MockExploration('exp2', {
            'state_1': {},
            'state_2': {}
        }),
        MockExploration('exp3', {
            'state_1': {},
            'state_2': {}
        })
    ]
    ...
    def test_cannot_create_suggestion_with_invalid_suggestion_type(self) -> None:
        with self.assertRaisesRegex(Exception, 'Invalid suggestion type'):
            suggestion_services.create_suggestion(
                'invalid_suggestion_type', feconf.ENTITY_TYPE_EXPLORATION,
                self.target_id, self.target_version_at_submission, self.author_id,
    ...
    def test_cannot_create_suggestion_with_invalid_author_id(self) -> None:
        with self.assertRaisesRegex(
            ...
            suggestion_services.create_suggestion(
                ...
                self.target_version_at_submission, 'invalid author ID', self.change_cmd,
                'test description'
            )
    ...
    def test_cannot_create_translation_suggestion_with_invalid_content_html_raise_error(  # pylint: disable=line-too-long
        ...
        with self.assertRaisesRegex(
            Exception, 'The Exploration content has changed since this translation '
            'was submitted.'
        ):
            suggestion_services.create_suggestion(
    ...
    def test_get_submitted_submissions(self) -> None:
        suggestion_services.create_suggestion(
            feconf.SUGGESTION_TYPE_EDIT_STATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            self.target_id, self.target_version_at_submission, self.author_id,
            self.change_cmd, ''
        )
        suggestion_services.create_suggestion(
            feconf.SUGGESTION_TYPE_EDIT_STATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            self.target_id, self.target_version_at_submission, self.author_id,
            self.change_cmd, 'test_description'
        )
    ...
    def test_accept_suggestion_and_send_email_to_author(self) -> None:
        ...
        with self.swap(feconf, 'ENABLE_RECORDING_OF_SCORES', True):
            with self.swap(feconf, 'SEND_SUGGESTION_REVIEW_RELATED_EMAILS', True):
                with self.swap(
                    ...
                    suggestion_services.accept_suggestion(
                        suggestion.suggestion_id, self.reviewer_id, self.COMMIT_MESSAGE,
                        'review message'
                    )
        ...
        self.assertEqual(
            user_proficiency_model.score, feconf.MINIMUM_SCORE_REQUIRED_TO_REVIEW
        )
    ...
    def test_accept_suggestion_does_not_send_email_if_users_score_is_too_low(
        ...
        assert user_proficiency_model is not None
        self.assertEqual(
            user_proficiency_model.score, suggestion_models.INCREMENT_SCORE_OF_AUTHOR_BY
        )
    ...
    def test_accept_suggestion_no_commit_message_failure(self) -> None:
        ...
        with self.assertRaisesRegex(Exception, 'Commit message cannot be empty.'):
            suggestion_services.accept_suggestion(
                self.suggestion_id, self.reviewer_id, self.EMPTY_COMMIT_MESSAGE, ''
            )
    ...
    def test_resubmit_rejected_suggestion_raises_exception_for_unhandled_input(
        ...
        # rejected yet.
        expected_exception_regexp = (
            'The suggestion with id %s is not yet handled.' % (self.suggestion_id)
        )
        resubmit_change = exp_domain.ExplorationChange({
            'cmd': exp_domain.CMD_RENAME_STATE,
            'new_state_name': '',
    ...
    def test_update_translation_suggestion_to_change_translation_html(self) -> None:
        exploration = (
            self.save_new_linear_exp_with_state_names_and_interactions(
                'exploration1',
                self.author_id, ['state 1'], ['TextInput'],
                category='Algebra'
            )
        )
        old_content = state_domain.SubtitledHtml('content',
                                                 '<p>old content html</p>').to_dict()
        ...
        suggestion = suggestion_services.create_suggestion(
            feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            'exploration1', self.target_version_at_submission, self.author_id,
            add_translation_change_dict, 'test description'
        )
        ...
        self.assertEqual(
            updated_suggestion.change_cmd.translation_html, '<p>Updated translation</p>'
        )
    ...
    def test_update_question_suggestion_to_change_question_state(self) -> None:
        ...
        suggestion_services.update_question_suggestion(
            suggestion.suggestion_id, suggestion.change_cmd.skill_difficulty,
            question_state_data, content_id_generator.next_content_id_index
        )
        updated_suggestion = suggestion_services.get_suggestion_by_id(
            suggestion.suggestion_id
        )
        ...
        self.assertEqual(
            new_question_state_data['content']['html'], '<p>Updated question</p>'
        )
        self.assertEqual(
            new_question_state_data['interaction']['solution'], new_solution_dict
        )
    ...
    def test_wrong_suggestion_raise_error_while_updating_translation_suggestion(
        ...
        with self.assertRaisesRegex(
            Exception, 'Expected SuggestionTranslateContent suggestion'
    ...
    def test_wrong_suggestion_raise_error_when_updating_add_question_suggestion(
        ...
        model = opportunity_models.ExplorationOpportunitySummaryModel(
            ...
            content_count=2,
            incomplete_translation_language_codes=(audio_language_codes - set(['en'])),
            translation_counts={},
        ...
        with self.assertRaisesRegex(
            Exception, 'Expected SuggestionAddQuestion suggestion but '
            'found: SuggestionTranslateContent.'
        ):
            suggestion_services.update_question_suggestion(
                suggestion.suggestion_id, 0.1, exploration.states['state 1'].to_dict(),
                5
            )
    ...
    def test_update_question_suggestion_to_change_skill_difficulty(self) -> None:
        ...
        suggestion_services.update_question_suggestion(
            suggestion.suggestion_id, 0.6, question_state_data,
            content_id_generator.next_content_id_index
        )
        updated_suggestion = suggestion_services.get_suggestion_by_id(
            suggestion.suggestion_id
...
class SuggestionGetServicesUnitTests(test_utils.GenericTestBase):
        ...
        with self.swap(
            ...
            with self.swap(
                ...
                translation_suggestion = suggestion_services.create_suggestion(
                    feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT,
                    feconf.ENTITY_TYPE_EXPLORATION, target_id, 1, self.author_id_1,
                    add_translation_change_dict, 'test description'
                )
    ...
    def setUp(self) -> None:
        ...
        self.opportunity_summary_ids = [
            self.explorations[0].id, self.explorations[1].id, self.explorations[2].id
        ]
        ...
        with self.swap(
            ...
            suggestion_services.create_suggestion(
                ...
                self.target_version_at_submission, self.author_id_1, self.change_cmd,
                'test description'
            )
            ...
            suggestion_services.create_suggestion(
                feconf.SUGGESTION_TYPE_EDIT_STATE_CONTENT,
                feconf.ENTITY_TYPE_EXPLORATION, self.target_id_1,
                self.target_version_at_submission, self.author_id_2, self.change_cmd,
                'test description'
            )

            suggestion_services.create_suggestion(
                feconf.SUGGESTION_TYPE_EDIT_STATE_CONTENT,
                feconf.ENTITY_TYPE_EXPLORATION, self.target_id_2,
                self.target_version_at_submission, self.author_id_2, self.change_cmd,
                'test description'
            )
    ...
    def test_get_translation_suggestions_in_review_by_exp_ids(self) -> None:
        suggestions = (
            suggestion_services.get_translation_suggestions_in_review_by_exp_ids([
                self.target_id_1, self.target_id_2, self.target_id_3
            ], 'en')
        )
    ...
    def test_get_by_target_id(self) -> None:
        queries = [('target_type', feconf.ENTITY_TYPE_EXPLORATION),
    ...
    def test_query_suggestions(self) -> None:
        queries = [('target_type', feconf.ENTITY_TYPE_EXPLORATION),
                   ('target_id', self.target_id_1), ('author_id', self.author_id_2)]
        self.assertEqual(len(suggestion_services.query_suggestions(queries)), 1)

        queries = [('target_type', feconf.ENTITY_TYPE_EXPLORATION),
                   ('target_id', self.target_id_1), ('author_id', self.author_id_1),
                   ('status', suggestion_models.STATUS_IN_REVIEW)]
        self.assertEqual(len(suggestion_services.query_suggestions(queries)), 3)
    ...
    def test_get_translation_suggestion_ids_with_exp_ids_with_one_exp(self) -> None:
        ...
        with self.swap(
            ...
            with self.swap(
                ...
                suggestion_services.create_suggestion(
                    feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT,
                    feconf.ENTITY_TYPE_EXPLORATION, self.target_id_1, 1,
    ...
    def test_get_translation_suggestion_ids_with_exp_ids_with_multiple_exps(
        ...
        with self.swap(
            ...
            with self.swap(
                ...
                suggestion_services.create_suggestion(
                    feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT,
                    feconf.ENTITY_TYPE_EXPLORATION, self.target_id_2, 1,
                    self.author_id_1, self.add_translation_change_dict,
                    'test description'
                )
        # Create the translation suggestion associated with exploration id
        ...
        with self.swap(
            ...
            with self.swap(
                ...
                suggestion_services.create_suggestion(
                    feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT,
                    feconf.ENTITY_TYPE_EXPLORATION, self.target_id_3, 1,
                    self.author_id_1, self.add_translation_change_dict,
                    'test description'
                )
    ...
    def test_get_translation_suggestion_ids_with_exp_ids_with_empty_exp_list(
        ...
        self.assertEqual(
            len(suggestion_services.get_translation_suggestion_ids_with_exp_ids([])), 0
        )
    ...
    def test_get_submitted_suggestions_by_offset(self) -> None:
        ...
        self.assertEqual(
            translatable_suggestions[0].status, suggestion_models.STATUS_IN_REVIEW
        )
        ...
        self.assertEqual(
            translatable_suggestions[1].status, suggestion_models.STATUS_IN_REVIEW
        )
        ...
        self.assertEqual(
            question_suggestions[0].suggestion_type, feconf.SUGGESTION_TYPE_ADD_QUESTION
        )
        self.assertEqual(
        ...
        self.assertEqual(
            question_suggestions[1].suggestion_type, feconf.SUGGESTION_TYPE_ADD_QUESTION
        )
        self.assertEqual(
    ...
    def test_get_translation_suggestions_in_review(self) -> None:
        ...
        suggestions = (
            suggestion_services.get_translation_suggestions_in_review(self.target_id_1)
        )
        ...
        self.assertEqual(suggestions[0].target_id, self.target_id_1)
        self.assertEqual(
            suggestions[0].suggestion_type, feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT
        )
        ...
        self.assertEqual(
            suggestions[1].suggestion_type, feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT
        )
    ...
    def test_get_reviewable_translation_suggestions_with_empty_exp_ids( # pylint: disable=line-too-long
        ...
        suggestions, offset = suggestion_services.get_reviewable_translation_suggestions_by_offset(
            self.reviewer_id_1, [], constants.OPPORTUNITIES_PAGE_SIZE, 0, None
        )
    ...
    def test_get_reviewable_translation_suggestions_with_none_exp_ids(self) -> None:
        ...
        suggestions, offset = (
            suggestion_services.get_reviewable_translation_suggestions_by_offset(
                self.reviewer_id_1, None, constants.OPPORTUNITIES_PAGE_SIZE, 0, None
            )
        )
    ...
    def test_get_target_ids_of_reviewable_translation_suggestions_for_user(
        ...
        target_ids = (
            suggestion_services.get_reviewable_translation_suggestion_target_ids(
                self.reviewer_id_1, language_code
            )
        )

        self.assertCountEqual(target_ids, [fetched_target_id_1, fetched_target_id_2])
    ...
    def test_get_translation_suggestions_waiting_longest_for_review_per_lang(
        ...
        suggestions = (
            suggestion_services.
            get_translation_suggestions_waiting_longest_for_review('hi')
        )
    ...
    def test_get_translation_suggestions_waiting_longest_for_review_wrong_lang(
        ...
        suggestions = (
            suggestion_services.get_translation_suggestions_waiting_longest_for_review(
                'wrong_language_code'
            )
        )
    ...
    def test_get_question_suggestions_waiting_longest_for_review_keeps_order(
        ...
        suggestion_services.resubmit_rejected_suggestion(
            suggestion_1.suggestion_id, 'resubmit summary message', self.author_id_1,
    ...
    def test_query_suggestions_that_can_be_reviewed_by_user(self) -> None:
        ...
        suggestion_models.GeneralSuggestionModel.create(
            feconf.SUGGESTION_TYPE_EDIT_STATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            'exp1', 1, suggestion_models.STATUS_IN_REVIEW, 'author_3', 'reviewer_2',
            self.change_cmd, 'category1', 'exploration.exp1.thread_1', None
        )
        suggestion_models.GeneralSuggestionModel.create(
            feconf.SUGGESTION_TYPE_EDIT_STATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            'exp1', 1, suggestion_models.STATUS_IN_REVIEW, 'author_3', 'reviewer_2',
            self.change_cmd, 'category2', 'exploration.exp1.thread_2', None
        )
        suggestion_models.GeneralSuggestionModel.create(
            feconf.SUGGESTION_TYPE_EDIT_STATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            'exp1', 1, suggestion_models.STATUS_IN_REVIEW, 'author_3', 'reviewer_2',
            self.change_cmd, 'category3', 'exploration.exp1.thread_3', None
        )
        # This suggestion does not count as a suggestion that can be reviewed
        # by a user because it has already been rejected.
        suggestion_models.GeneralSuggestionModel.create(
            feconf.SUGGESTION_TYPE_EDIT_STATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            'exp1', 1, suggestion_models.STATUS_REJECTED, 'author_3', 'reviewer_2',
            self.change_cmd, 'category1', 'exploration.exp1.thread_4', None
        )
        suggestion_models.GeneralSuggestionModel.create(
            feconf.SUGGESTION_TYPE_EDIT_STATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            'exp1', 1, suggestion_models.STATUS_IN_REVIEW, 'author_3', 'reviewer_2',
            self.change_cmd, 'category2', 'exploration.exp1.thread_5', None
        )

        self.assertEqual(
            len(
                suggestion_services.
                get_all_suggestions_that_can_be_reviewed_by_user('user1')
            ), 3
        )
        self.assertEqual(
            len(
                suggestion_services.
                get_all_suggestions_that_can_be_reviewed_by_user('user2')
            ), 0
        )
...
class SuggestionIntegrationTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        exploration = (
            self.save_new_linear_exp_with_state_names_and_interactions(
                self.EXP_ID,
                self.editor_id, ['State 1', 'State 2', 'End State'], ['TextInput'],
                category='Algebra'
            )
        )
        ...
        self.new_content = state_domain.SubtitledHtml('content',
                                                      '<p>new content</p>').to_dict()

        self.change_cmd: Dict[str, Union[str, state_domain.SubtitledHtmlDict]] = {
            'cmd': exp_domain.CMD_EDIT_STATE_PROPERTY,
            'property_name': exp_domain.STATE_PROPERTY_CONTENT,
            'state_name': 'State 1',
        ...
        story_services.update_story(
            self.owner_id, self.STORY_ID, story_change_list_to_add_an_exp,
            'Added exploration.'
        )
    ...
    def create_translation_suggestion_associated_with_exp(
        ...
        suggestion_services.create_suggestion(
            feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            exp_id, 1, author_id, add_translation_change_dict, 'test description'
        )
    ...
    def assert_created_suggestion_is_valid(
        ...
        """
        suggestions = suggestion_services.query_suggestions([('author_id', author_id),
                                                             ('target_id', target_id)])
        self.assertEqual(len(suggestions), 1)
        self.assertEqual(suggestions[0].status, suggestion_models.STATUS_IN_REVIEW)
    ...
    def _set_up_topics_and_100_stories_for_translations(
        ...
        for i in range(103):
            self.create_story_for_translation_opportunity(
                self.owner_id, self.admin_id, ('story_id_%s' % (i)), topic_id, '%s' % i
            )
    ...
    def test_get_translation_contribution_stats_for_invalid_id_with_strict_true(
        ...
        with self.assertRaisesRegex(
            Exception, 'The stats models do not exist for the stats_id invalid_id.'
        ):
    ...
    def test_update_translation_contribution_stats_when_submitting(self) -> None:
        ...
        self.assertEqual(
            (translation_contribution_stats_model.submitted_translation_word_count), 7
        )
        self.assertEqual(
        ...
        self.assertEqual(
            (translation_submitter_total_stats_model.submitted_translations_count), 2
        )
        self.assertEqual(
        ...
        self.assertEqual(
            (translation_submitter_total_stats_model.accepted_translations_count), 0
        )
        self.assertEqual((
            translation_submitter_total_stats_model.
    ...
    def test_update_translation_review_stats_when_suggestion_is_accepted(self) -> None:
        ...
        suggestion_services.update_translation_contribution_stats_at_submission(
            suggestion_services.get_suggestion_by_id(initial_suggestion.suggestion_id)
        )
        suggestion_services.update_translation_contribution_stats_at_submission(
            suggestion_services.get_suggestion_by_id(latest_suggestion.suggestion_id)
        )

        suggestion_services.accept_suggestion(
            initial_suggestion.suggestion_id, self.reviewer_id, 'Accepted', 'Accepted'
        )
        suggestion_services.accept_suggestion(
            latest_suggestion.suggestion_id, self.reviewer_id, 'Accepted', 'Accepted'
        )
        ...
        assert translation_contribution_stats_model is not None
        self.assertEqual(
            (translation_contribution_stats_model.accepted_translation_word_count), 7
        )
        self.assertEqual(
        ...
        self.assertEqual(
            translation_reviewer_total_stats_model.accepted_translations_count, 2
        )
        self.assertEqual(
            (translation_reviewer_total_stats_model.reviewed_translations_count), 2
        )
        assert translation_submitter_total_stats_model is not None
        self.assertEqual(
            (translation_submitter_total_stats_model.accepted_translation_word_count), 7
        )
        self.assertEqual(
            translation_submitter_total_stats_model.accepted_translations_count, 2
        )
        self.assertEqual(
            (translation_submitter_total_stats_model.recent_review_outcomes),
            ['accepted', 'accepted']
        )
        self.assertEqual((translation_submitter_total_stats_model.recent_performance),
                         2)
        self.assertEqual((translation_submitter_total_stats_model.overall_accuracy),
                         100.0)

        suggestion_services.accept_suggestion(
            topic_suggestion.suggestion_id, self.reviewer_id, 'Accepted', 'Accepted'
        )
        suggestion_services.update_translation_review_stats(
            suggestion_services.get_suggestion_by_id(topic_suggestion.suggestion_id)
        )
        ...
        self.assertItemsEqual(
            translation_reviewer_total_stats_model_for_a_topic.
            topic_ids_with_translation_reviews, ['0', 'A']
        )

        self.assertItemsEqual(
            translation_submitter_total_stats_model_for_a_topic.
            topic_ids_with_translation_submissions, ['0', 'A']
        )
    ...
    def test_update_translation_review_stats_when_suggestion_is_rejected(self) -> None:
        ...
        self.assertEqual(
            translation_reviewer_total_stats_model.accepted_translations_count, 0
        )
        self.assertEqual(
            (translation_reviewer_total_stats_model.accepted_translation_word_count), 0
        )
        assert translation_submitter_total_stats_model is not None
        self.assertEqual(
            translation_submitter_total_stats_model.rejected_translations_count, 2
        )
        self.assertEqual(
            (translation_submitter_total_stats_model.rejected_translations_count), 2
        )
    ...
    def test_update_translation_review_stats_without_a_reviewer_id(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception, 'The final_reviewer_id in the suggestion should not be None.'
        ):
    ...
    def test_update_translation_review_stats_when_suggestion_is_edited(self) -> None:
        ...
        self.assertEqual(
            (translation_contribution_stats_model.submitted_translation_word_count), 6
        )
        self.assertEqual(
            translation_contribution_stats_model.accepted_translations_count, 2
        )
        ...
        self.assertEqual(
            (translation_reviewer_total_stats_model.accepted_translation_word_count), 2
        )
        self.assertEqual(
            translation_reviewer_total_stats_model.
            accepted_translations_with_reviewer_edits_count, 2
    ...
    def test_increment_translation_stats_for_than_100_suggestions_accepted(
        ...
        for i in range(1, 102):
            ...
            suggestion = suggestion_services.create_suggestion(
                feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT,
                feconf.ENTITY_TYPE_EXPLORATION, ('%s' % (i)), i, self.author_id,
                common_change_dict, 'description'
            )
            suggestion_services.update_translation_contribution_stats_at_submission( # pylint: disable=line-too-long
                suggestion_services.get_suggestion_by_id(
                    suggestion.suggestion_id))
            suggestion_services.accept_suggestion(
                suggestion.suggestion_id, self.reviewer_id, 'Accepted', 'Accepted'
            )
            suggestion_services.update_translation_review_stats(
        ...
        assert updated_translation_submitter_total_stats_model is not None
        self.assertEqual(
            len(updated_translation_submitter_total_stats_model.recent_review_outcomes),
            100
        )
        ...
        self.assertEqual(
            rejected_translation_submitter_total_stats_model.recent_review_outcomes[99],
    ...
    def test_increment_translation_stats_for_than_100_suggestions_rejected(
        ...
        self.assertEqual(
            accepted_translation_submitter_total_stats_model.recent_review_outcomes[99],
    ...
    def _create_question_suggestion(
        ...
        """
        content_id_generator = translation_domain.ContentIdGenerator()
        suggestion_change: Dict[str, Union[str, float, Dict[str, Union[
    ...
    def test_update_question_stats_when_suggestion_is_accepted(self) -> None:
        ...
        question_review_stats_model = (
            suggestion_models.QuestionReviewStatsModel.get(self.reviewer_id, topic_id)
        )
        question_contribution_stats_model = (
            suggestion_models.QuestionContributionStatsModel.get(
                self.author_id, topic_id
        ...
        self.assertEqual((question_reviewer_total_stats_model.reviewed_questions_count),
                         2)
        assert question_submitter_total_stats_model is not None
        self.assertEqual(
            question_submitter_total_stats_model.submitted_questions_count, 2
    ...
    def test_increment_question_stats_for_than_100_suggestions_accepted(self) -> None:
        ...
        self.assertEqual(
            len(updated_question_submitter_total_stats_model.recent_review_outcomes),
            100
        )
        ...
        self.assertEqual(
            len(rejected_question_submitter_total_stats_model.recent_review_outcomes),
            100
        )

        self.assertEqual(
            rejected_question_submitter_total_stats_model.recent_review_outcomes[99],
            suggestion_models.REVIEW_OUTCOME_REJECTED
        )
    ...
    def test_increment_question_stats_for_than_100_suggestions_rejected(self) -> None:
        ...
        self.assertEqual(
            len(accepted_question_submitter_total_stats_model.recent_review_outcomes),
            100
        )

        self.assertEqual(
            accepted_question_submitter_total_stats_model.recent_review_outcomes[99],
            suggestion_models.REVIEW_OUTCOME_ACCEPTED
        )
    ...
    def test_create_and_accept_suggestion_with_message(self) -> None:
        ...
        suggestion_services.accept_suggestion(
            suggestion_id, self.reviewer_id, self.COMMIT_MESSAGE, 'Accept message'
        )
    ...
    def test_auto_reject_translation_suggestions_for_content_ids(self) -> None:
        ...
        last_message = thread_messages[len(thread_messages) - 1]
        self.assertEqual(
            last_message.text, constants.OBSOLETE_TRANSLATION_SUGGESTION_REVIEW_MSG
        )
        suggestion = suggestion_services.get_suggestion_by_id(suggestion_id)
    ...
    def test_get_suggestions_with_translatable_explorations(self) -> None:
        ...
        filtered_translatable_suggestions = (
            suggestion_services.
            get_suggestions_with_editable_explorations(translatable_suggestions)
        )
...
class UserContributionProficiencyUnitTests(test_utils.GenericTestBase):
    ...
    def test_get_all_user_ids_who_are_allowed_to_review(self) -> None:
        ...
        user_ids = (
            suggestion_services.get_all_user_ids_who_are_allowed_to_review('category1')
        )
        self.assertEqual(user_ids, [])
        user_ids = (
            suggestion_services.get_all_user_ids_who_are_allowed_to_review('category2')
        )
        self.assertEqual(user_ids, [self.user_1_id])

        self.assertFalse(
            suggestion_services.can_user_review_category(self.user_1_id, 'category1')
        )
        self.assertTrue(
            suggestion_services.can_user_review_category(self.user_1_id, 'category2')
        )
        self.assertFalse(
            suggestion_services.can_user_review_category(self.user_2_id, 'category1')
        )
        self.assertFalse(
            suggestion_services.can_user_review_category(self.user_2_id, 'category1')
        )
...
class ReviewableSuggestionEmailInfoUnitTests(test_utils.GenericTestBase):
    ...
    def _create_translation_suggestion_with_translation_html(
        ...
        return suggestion_services.create_suggestion(
            feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            self.target_id, feconf.CURRENT_STATE_SCHEMA_VERSION, self.author_id,
            add_translation_change_dict, 'test description'
        )
    ...
    def _create_question_suggestion_with_question_html_content(
        ...
        """
        with self.swap(feconf, 'DEFAULT_STATE_CONTENT_STR', question_html_content):
            content_id_generator = translation_domain.ContentIdGenerator()
    ...
    def _create_edit_state_content_suggestion(
        ...
        edit_state_content_change_dict: Dict[str, Union[str, Dict[str, str]]] = {
            'cmd': exp_domain.CMD_EDIT_STATE_PROPERTY,
            'property_name': exp_domain.STATE_PROPERTY_CONTENT,
            'state_name': 'Introduction',
            'new_value': {
        ...
        return suggestion_services.create_suggestion(
            feconf.SUGGESTION_TYPE_EDIT_STATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            self.target_id, feconf.CURRENT_STATE_SCHEMA_VERSION, self.author_id,
            edit_state_content_change_dict, None
        )
    ...
    def test_create_raises_for_suggestion_type_not_on_contributor_dashboard(
        ...
        with self.swap(
            ...
            with self.assertRaisesRegex(
                Exception, 'Expected suggestion type to be offered on the Contributor '
    ...
    def test_create_from_suggestion_returns_info_for_question_suggestion(self) -> None:
        ...
        expected_reviewable_suggestion_email_info = (
            suggestion_registry.ReviewableSuggestionEmailInfo(
                question_suggestion.suggestion_type, question_suggestion.language_code,
                'default question content', question_suggestion.last_updated
            )
        )
        ...
        self._assert_reviewable_suggestion_email_infos_are_equal(
            reviewable_suggestion_email_info, expected_reviewable_suggestion_email_info
        )
    ...
    def test_create_from_suggestion_returns_info_for_translation_suggestion(
        ...
        expected_reviewable_suggestion_email_info = (
            suggestion_registry.ReviewableSuggestionEmailInfo(
                ...
                translation_suggestion.language_code, 'default translation content',
    ...
    def test_create_from_suggestion_returns_info_with_no_trailing_whitespace(
        ...
        expected_reviewable_suggestion_email_info = (
            suggestion_registry.ReviewableSuggestionEmailInfo(
                translation_suggestion.suggestion_type,
                translation_suggestion.language_code, 'test whitespace',
                translation_suggestion.last_updated
            )
        )
    ...
    def test_create_returns_info_for_translation_suggestion_if_html_math_rte(
        ...
        expected_reviewable_suggestion_email_info = (
            suggestion_registry.ReviewableSuggestionEmailInfo(
                translation_suggestion.suggestion_type,
                translation_suggestion.language_code, 'translation with rte [Math]',
                translation_suggestion.last_updated
            )
        )
    ...
    def test_create_returns_info_for_translation_suggestion_if_html_image_rte(
        ...
        expected_reviewable_suggestion_email_info = (
            suggestion_registry.ReviewableSuggestionEmailInfo(
                translation_suggestion.suggestion_type,
                translation_suggestion.language_code, 'translation with rte [Image]',
                translation_suggestion.last_updated
            )
        )
    ...
    def test_create_returns_info_for_translation_suggestion_if_html_link_rte(
        ...
        expected_reviewable_suggestion_email_info = (
            suggestion_registry.ReviewableSuggestionEmailInfo(
                translation_suggestion.suggestion_type,
                translation_suggestion.language_code, 'translation with rte [Link]',
                translation_suggestion.last_updated
            )
        )
    ...
    def test_create_returns_info_for_translation_suggestion_if_html_rte_value(
        ...
        expected_reviewable_suggestion_email_info = (
            suggestion_registry.ReviewableSuggestionEmailInfo(
                translation_suggestion.suggestion_type,
                translation_suggestion.language_code, '[Link]',
                translation_suggestion.last_updated
            )
        )
    ...
    def test_create_returns_info_for_question_suggestion_if_html_has_math_rte(
        ...
        expected_reviewable_suggestion_email_info = (
            suggestion_registry.ReviewableSuggestionEmailInfo(
                question_suggestion.suggestion_type, question_suggestion.language_code,
                'question with rte [Math]', question_suggestion.last_updated
            )
        )
    ...
    def test_create_returns_info_for_question_suggestion_if_html_has_image_rte(
        ...
        expected_reviewable_suggestion_email_info = (
            suggestion_registry.ReviewableSuggestionEmailInfo(
                question_suggestion.suggestion_type, question_suggestion.language_code,
                'question with rte [Image]', question_suggestion.last_updated
            )
        )
    ...
    def test_create_returns_info_for_question_suggestion_if_html_has_link_rte(
        ...
        expected_reviewable_suggestion_email_info = (
            suggestion_registry.ReviewableSuggestionEmailInfo(
                question_suggestion.suggestion_type, question_suggestion.language_code,
                'question with rte [Link]', question_suggestion.last_updated
            )
        )
    ...
    def test_create_returns_info_for_question_suggestion_if_html_has_repeat_rte(
        ...
        expected_reviewable_suggestion_email_info = (
            suggestion_registry.ReviewableSuggestionEmailInfo(
                question_suggestion.suggestion_type, question_suggestion.language_code,
                'question with rte [Link] [Link]', question_suggestion.last_updated
            )
        )
    ...
    def test_create_returns_info_for_question_suggestion_if_html_has_multi_rte(
        ...
        expected_reviewable_suggestion_email_info = (
            suggestion_registry.ReviewableSuggestionEmailInfo(
                question_suggestion.suggestion_type, question_suggestion.language_code,
                'question with rte [Link] [Math]', question_suggestion.last_updated
            )
        )
    ...
    def test_create_returns_info_for_question_suggestion_if_html_has_rte_value(
        ...
        expected_reviewable_suggestion_email_info = (
            suggestion_registry.ReviewableSuggestionEmailInfo(
                question_suggestion.suggestion_type, question_suggestion.language_code,
                '[Link]', question_suggestion.last_updated
            )
        )
    ...
    def test_create_returns_info_for_suggestion_if_html_has_rte_with_multi_word(
        ...
        expected_reviewable_suggestion_email_info = (
            suggestion_registry.ReviewableSuggestionEmailInfo(
                question_suggestion.suggestion_type, question_suggestion.language_code,
                '[Link Test]', question_suggestion.last_updated
            )
        )
...
class GetSuggestionsWaitingForReviewInfoToNotifyReviewersUnitTests(
    ...
    def _create_translation_suggestion_with_language_code_and_author(
        ...
        return suggestion_services.create_suggestion(
            feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            self.target_id, feconf.CURRENT_STATE_SCHEMA_VERSION, author_id,
            add_translation_change_dict, 'test description'
        )
    ...
    def _create_question_suggestion_with_skill_id_and_author_id(
        ...
        return suggestion_services.create_suggestion(
            feconf.SUGGESTION_TYPE_ADD_QUESTION, feconf.ENTITY_TYPE_SKILL, skill_id,
            feconf.CURRENT_STATE_SCHEMA_VERSION, author_id, add_question_change_dict,
            'test description'
        )
    ...
    def _assert_reviewable_suggestion_email_infos_are_in_correct_order(
        ...
        for index, reviewable_suggestion_email_info in enumerate(
            ...
            self.assertEqual(
                reviewable_suggestion_email_info.suggestion_type,
                expected_reviewable_suggestion_email_infos[index].suggestion_type
            )
            self.assertEqual(
                reviewable_suggestion_email_info.language_code,
            ...
            self.assertEqual(
                reviewable_suggestion_email_info.suggestion_content,
                expected_reviewable_suggestion_email_infos[index].suggestion_content
            )
            self.assertEqual(
                reviewable_suggestion_email_info.submission_datetime,
                expected_reviewable_suggestion_email_infos[index].submission_datetime
            )
        for index in range(len(reviewable_suggestion_email_infos) - 1):
            self.assertLessEqual(
                reviewable_suggestion_email_infos[index].submission_datetime,
                reviewable_suggestion_email_infos[index + 1].submission_datetime
            )
    ...
    def setUp(self) -> None:
        ...
        exploration = self.save_new_valid_exploration(self.target_id, self.author_id)
        audio_language_codes = set(
            language['id'] for language in constants.SUPPORTED_AUDIO_LANGUAGES
        )
        model = opportunity_models.ExplorationOpportunitySummaryModel(
    ...
    def test_get_returns_empty_for_rejected_suggestions(self) -> None:
        ...
        suggestion_services.reject_suggestion(
            translation_suggestion.suggestion_id, self.reviewer_1_id, 'review message'
        )
    ...
    def test_get_returns_infos_for_translation_reviewer_past_limit_same_lang(
        ...
        with self.swap(
            suggestion_services, 'MAX_NUMBER_OF_SUGGESTIONS_TO_EMAIL_REVIEWER', 1
        ):
            reviewable_suggestion_email_infos = (
                suggestion_services.
                get_suggestions_waiting_for_review_info_to_notify_reviewers([
    ...
    def test_get_returns_infos_for_translation_reviewer_past_limit_diff_lang(
        ...
        with self.swap(
            suggestion_services, 'MAX_NUMBER_OF_SUGGESTIONS_TO_EMAIL_REVIEWER', 2
        ):
            reviewable_suggestion_email_infos = (
                suggestion_services.
                get_suggestions_waiting_for_review_info_to_notify_reviewers([
...
class CommunityContributionStatsUnitTests(test_utils.GenericTestBase):
    ...
    def _create_edit_state_content_suggestion(
        ...
        return suggestion_services.create_suggestion(
            feconf.SUGGESTION_TYPE_EDIT_STATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            self.target_id, feconf.CURRENT_STATE_SCHEMA_VERSION, self.author_id,
            edit_state_content_change_dict, 'test description'
        )
    ...
    def test_resubmit_edit_state_content_suggestion_does_not_change_the_counts(
        ...
        suggestion_services.resubmit_rejected_suggestion(
            edit_state_content_suggestion.suggestion_id, 'resubmit summary message',
            self.author_id, resubmit_suggestion_change
        )
    ...
    def test_accept_question_suggestion_decreases_question_suggestion_count(
        ...
        suggestion_services.accept_suggestion(
            question_suggestion.suggestion_id, self.reviewer_id, self.COMMIT_MESSAGE,
            'review message'
        )
    ...
    def test_reject_question_suggestion_decreases_question_suggestion_count(
        ...
        suggestion_services.reject_suggestion(
            question_suggestion.suggestion_id, self.reviewer_id, 'review message'
        )
    ...
    def test_reject_question_suggestions_decreases_question_suggestion_count(
        ...
        suggestion_services.reject_suggestions([
            question_suggestion_1.suggestion_id, question_suggestion_2.suggestion_id
        ], self.reviewer_id, 'review message')
    ...
    def test_accept_translation_suggestion_lowers_translation_suggestion_count(
        ...
        suggestion_services.accept_suggestion(
            translation_suggestion.suggestion_id, self.reviewer_id, self.COMMIT_MESSAGE,
            'review message'
        )
    ...
    def test_reject_translation_suggestion_lowers_translation_suggestion_count(
        ...
        suggestion_services.reject_suggestion(
            translation_suggestion.suggestion_id, self.reviewer_id, 'review message'
        )
    ...
    def test_reject_one_translation_suggestion_diff_lang_lowers_only_one_count(
        ...
        suggestion_services.reject_suggestion(
            translation_suggestion_1.suggestion_id, self.reviewer_id, 'review message'
        )
...
class GetSuggestionsWaitingTooLongForReviewInfoForAdminsUnitTests(
    ...
    mocked_datetime_utcnow: datetime.datetime = (datetime.datetime(2020, 6, 15, 5))
    ...
    def test_get_returns_empty_for_suggestion_type_not_on_contributor_dashboard(
        ...
        with self.swap(
            ...
            with self.swap(
                suggestion_models, 'SUGGESTION_REVIEW_WAIT_TIME_THRESHOLD_IN_DAYS', 0
            ):
    ...
    def test_get_returns_empty_if_suggestion_review_wait_time_diff_is_negative(
        ...
        with self.swap(
            suggestion_models, 'SUGGESTION_REVIEW_WAIT_TIME_THRESHOLD_IN_DAYS', 1
        ):
            info_about_suggestions_waiting_too_long_for_review = (
                suggestion_services.
                get_info_about_suggestions_waiting_too_long_for_review()
    ...
    def test_get_new_suggestions_for_reviewer_notifications_past_threshold(
        ...
        with self.swap(
            utils, 'get_current_time_in_millisecs', mock_get_current_time_in_millisecs
        ):
            with self.mock_datetime_utcnow(self.mocked_datetime_utcnow):
...
class GetSuggestionTypesThatNeedReviewersUnitTests(test_utils.GenericTestBase):
    ...
    def test_get_returns_reviewers_needed_if_translation_for_a_lang_no_reviewer(
        ...
        self.assertDictEqual(
            suggestion_types_needing_reviewers, {
                feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT: {self.sample_language_code}
            }
        )
...
class EmailsTaskqueueTests(test_utils.GenericTestBase):
    ...
    def test_create_new_instant_task(self) -> None:
        ...
        (
            suggestion_services.enqueue_contributor_ranking_notification_email_task(
                ...
                feconf.CONTRIBUTION_SUBTYPE_ACCEPTANCE, 'hi', 'Initial Contributor'
            )
        )

        self.assertEqual(
            self.count_jobs_in_taskqueue(taskqueue_services.QUEUE_NAME_EMAILS), 1
        )
        ...
        self.assertEqual(
            tasks[0].payload['contribution_type'], feconf.CONTRIBUTION_TYPE_TRANSLATION
        )
        self.assertEqual(
    ...
    def test_create_email_task_raises_exception_for_invalid_language_code(self) -> None:
        ...
        with self.assertRaisesRegex(Exception, 'Not supported language code: error'):
            (suggestion_services.enqueue_contributor_ranking_notification_email_task)(
                user_id, feconf.CONTRIBUTION_TYPE_TRANSLATION,
                feconf.CONTRIBUTION_SUBTYPE_ACCEPTANCE, 'error', 'Initial Contributor'
            )
    ...
    def test_create_email_task_raises_exception_for_invalid_contribution_type(
        ...
        with self.assertRaisesRegex(Exception, 'Invalid contribution type: test'):
            (suggestion_services.enqueue_contributor_ranking_notification_email_task)(
                user_id, 'test', feconf.CONTRIBUTION_SUBTYPE_ACCEPTANCE, 'hi',
                'Initial Contributor'
            )
    ...
    def test_create_email_task_raises_exception_for_wrong_contribution_subtype(
        ...
        with self.assertRaisesRegex(Exception, 'Invalid contribution subtype: test'):
            (suggestion_services.enqueue_contributor_ranking_notification_email_task)(
                user_id, feconf.CONTRIBUTION_TYPE_TRANSLATION, 'test', 'hi',
                'Initial Contributor'
            )
...
class ContributorCertificateTests(test_utils.GenericTestBase):
    ...
    def _calculate_translation_contribution_hours(self, numer_of_words: int) -> str:
    ...
    def _calculate_question_contribution_hours(self, images_included: bool) -> str:
    ...
    def test_create_translation_contributor_certificate(self) -> None:
        ...
        suggestion_models.GeneralSuggestionModel.create(
            feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            'exp1', 1, suggestion_models.STATUS_ACCEPTED, self.author_id, 'reviewer_1',
            change_cmd, score_category, 'exploration.exp1.thread_6', 'hi'
        )
    ...
    def test_create_translation_contributor_certificate_for_english(self) -> None:
        ...
        suggestion_models.GeneralSuggestionModel.create(
            feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT, feconf.ENTITY_TYPE_EXPLORATION,
            'exp1', 1, suggestion_models.STATUS_ACCEPTED, self.author_id, 'reviewer_1',
            change_cmd, score_category, 'exploration.exp1.thread_6', 'en'
        )
    ...
    def test_create_question_contributor_certificate(self) -> None:
        ...
        suggestion_models.GeneralSuggestionModel.create(
            feconf.SUGGESTION_TYPE_ADD_QUESTION, feconf.ENTITY_TYPE_SKILL, 'skill_1', 1,
            suggestion_models.STATUS_ACCEPTED, self.author_id, 'reviewer_2',
            suggestion_change, 'category1', 'thread_1', 'en'
        )
    ...
    def test_create_certificate_returns_none_for_no_question_suggestions(self) -> None:
        certificate_data = (
            suggestion_services.generate_contributor_certificate_data(
                self.username,
                feconf.SUGGESTION_TYPE_ADD_QUESTION,
