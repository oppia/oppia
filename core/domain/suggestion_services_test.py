...
class SuggestionServicesUnitTests(test_utils.GenericTestBase):
    ...
    suggestion_id = '.'.join(
        [suggestion_models.TARGET_TYPE_EXPLORATION, target_id, THREAD_ID])
    ...
    def test_accept_suggestion_successfully(self):
        ...
        with self.swap(
            ...
            with self.swap(
                ...
                self.assertEqual(suggestion.assigned_reviewer_id, '')
                thread_messages = feedback_services.get_messages(
                    self.target_id, self.THREAD_ID)
    ...
    def test_is_suggestion_valid_edit_state_content_success(self):
        ...
        with self.swap(
            ...
            suggestion_services.create_suggestion(
                ...
                suggestion_models.TARGET_TYPE_EXPLORATION,
                self.target_id, self.target_version_at_submission,
                self.author_id, change_cmd, self.score_category,
                'test description', self.assigned_reviewer_id, self.reviewer_id)
        suggestion = suggestion_services.get_suggestion_by_id(
            self.suggestion_id)
        with self.swap(
