...
def apply_change_list(
    ...
    modified_subtopic_pages_list = subtopic_page_services.get_subtopic_pages_with_ids(
...
def delete_canonical_story(user_id: str, topic_id: str, story_id: str) -> None:
    ...
    change_list = [
        topic_domain.TopicChange(
            {'cmd': topic_domain.CMD_DELETE_CANONICAL_STORY, 'story_id': story_id}
        )
    ]
    update_topic_and_subtopic_pages(
...
def delete_additional_story(user_id: str, topic_id: str, story_id: str) -> None:
    ...
    change_list = [
        topic_domain.TopicChange(
            {'cmd': topic_domain.CMD_DELETE_ADDITIONAL_STORY, 'story_id': story_id}
        )
    ]
    update_topic_and_subtopic_pages(
        user_id,
...
def delete_topic(
    ...
    topic_rights_model.delete(
        committer_id, feconf.COMMIT_MESSAGE_TOPIC_DELETED, force_deletion=force_deletion
    )
...
def deassign_manager_role_from_topic(
    ...
    commit_cmds = [
        topic_domain.TopicRightsChange(
            {'cmd': topic_domain.CMD_REMOVE_MANAGER_ROLE, 'removed_user_id': user_id}
        )
    ]
    save_topic_rights(
        topic_rights,
