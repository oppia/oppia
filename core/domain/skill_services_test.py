...
class SkillServicesUnitTests(test_utils.GenericTestBase):
    ...
    def test_get_rubrics_of_linked_skills(self) -> None:
        ...
        self.assertEqual(
            ...
            {
                'skill_id_1': [
                    skill_domain.Rubric(constants.SKILL_DIFFICULTIES[0], ['Explanation 1']).to_dict(),
                    skill_domain.Rubric(constants.SKILL_DIFFICULTIES[1], ['Explanation 2']).to_dict(),
                    skill_domain.Rubric(constants.SKILL_DIFFICULTIES[2], ['Explanation 3']).to_dict(),
                ],
                'skill_id_2': None,
            },
        )
    ...
    def test_get_filtered_skill_summaries(self) -> None:
        ...
        augmented_skill_summaries, next_cursor, more = skill_services.get_filtered_skill_summaries(
    ...
    def test_update_skill(self) -> None:
        changelist = [
            ...
            skill_domain.SkillChange(
                {
                    'cmd': skill_domain.CMD_UPDATE_SKILL_MISCONCEPTIONS_PROPERTY,
                    'property_name': (skill_domain.SKILL_MISCONCEPTIONS_PROPERTY_MUST_BE_ADDRESSED),
                    'misconception_id': self.skill.next_misconception_id,
                    'old_value': True,
                    'new_value': False,
                }
    ...
    def test_delete_skill_model_with_linked_suggestion(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            'The suggestion with id %s has already been accepted/rejected.' % suggestion.suggestion_id,
        ):
    ...
    def test_cannot_update_skill_with_empty_changelist(self) -> None:
        with self.assertRaisesRegex(
            Exception,
            'Unexpected error: received an invalid change list when trying to save skill',
        ):
    ...
    def test_mismatch_of_skill_versions(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            'Unexpected error: trying to update version 0 of skill from version 1. Please reload the page and try again.',
        ):
        ...
        with self.assertRaisesRegex(
            Exception,
            'Trying to update version 2 of skill from version 1, which is too old. Please reload the page and try again.',
        ):
            skill_services.update_skill(self.USER_ID, self.SKILL_ID, changelist, 'Change language code.')
    ...
    def test_get_topic_names_with_given_skill_in_diagnostic_test(self) -> None:
        ...
        self.assertEqual(
            skill_services.get_topic_names_with_given_skill_in_diagnostic_test('skill_id_1'),
            ['topic'],
        )
        self.assertEqual(
            skill_services.get_topic_names_with_given_skill_in_diagnostic_test('incorrect_skill_id'),
            [],
        )
...
class SkillMigrationTests(test_utils.GenericTestBase):
    def test_migrate_skill_contents_to_latest_schema(self) -> None:
        ...
        current_schema_version_swap = self.swap(feconf, 'CURRENT_SKILL_CONTENTS_SCHEMA_VERSION', 5)
