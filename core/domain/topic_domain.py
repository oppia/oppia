...
class Subtopic:
    ...
    def validate(self) -> None:
        ...
        if len(self.title) > title_limit:
            raise utils.ValidationError(
                'Expected subtopic title to be less than %d characters, received %s'
        ...
        if len(self.url_fragment) > url_fragment_limit:
            raise utils.ValidationError(
                'Expected subtopic url fragment to be less than or equal to %d characters, received %s'
...
class Topic:
    ...
    def delete_canonical_story(self, story_id: str) -> None:
        ...
        if not deleted:
            raise Exception(
                'The story_id %s is not present in the canonical story references list of the topic.'
                % story_id
            )
    ...
    def add_canonical_story(self, story_id: str) -> None:
        ...
        if story_id in canonical_story_ids:
            raise Exception(
                'The story_id %s is already present in the canonical story references list of the topic.'
                % story_id
            )
    ...
    def add_additional_story(self, story_id: str) -> None:
        ...
        if story_id in additional_story_ids:
            raise Exception(
                'The story_id %s is already present in the additional story references list of the topic.'
                % story_id
            )
    ...
    def delete_additional_story(self, story_id: str) -> None:
        ...
        if not deleted:
            raise Exception(
                'The story_id %s is not present in the additional story references list of the topic.'
                % story_id
            )
    ...
    def validate(self, strict: bool = False) -> None:
        ...
        if len(self.description) > description_limit:
            raise utils.ValidationError(
                'Topic description should be at most %d characters, received %s.'
        ...
        for subtopic in self.subtopics:
            ...
            if subtopic.id >= self.next_subtopic_id:
                raise utils.ValidationError(
                    'The id for subtopic %s is greater than or equal to next_subtopic_id %s'
        ...
        if len(skill_ids_for_diagnostic_that_are_not_in_topic) > 0:
            raise utils.ValidationError(
                'The skill_ids %s are selected for the diagnostic test but they are not associated with the topic.'
        ...
        if len(self.skill_ids_for_diagnostic_test) > 3:
            raise utils.ValidationError(
                'The skill_ids_for_diagnostic_test field should contain at most 3 skill_ids.'
            )
        ...
        for story_id in additional_story_ids:
            if story_id in canonical_story_ids:
                raise utils.ValidationError(
                    'Expected additional story ids list and canonical story ids list to be mutually exclusive. The story_id %s is present in both lists'
                    % story_id
                )
    ...
    def add_subtopic(
        ...
        if self.next_subtopic_id != new_subtopic_id:
            raise Exception(
                'The given new subtopic id %s is not equal to the expected next subtopic id: %s'
...
class TopicSummary:
    ...
    def validate(self) -> None:
        ...
        if self.canonical_story_count < 0:
            raise utils.ValidationError(
                'Expected canonical_story_count to be non-negative, received \'%s\''
                % self.canonical_story_count
            )

        if self.additional_story_count < 0:
            raise utils.ValidationError(
                'Expected additional_story_count to be non-negative, received \'%s\''
                % self.additional_story_count
            )

        if self.uncategorized_skill_count < 0:
            raise utils.ValidationError(
                'Expected uncategorized_skill_count to be non-negative, received \'%s\''
                % self.uncategorized_skill_count
            )

        if self.total_skill_count < 0:
            raise utils.ValidationError(
                'Expected total_skill_count to be non-negative, received \'%s\''
                % self.total_skill_count
            )

        if self.total_skill_count < self.uncategorized_skill_count:
            raise utils.ValidationError(
                'Expected total_skill_count to be greater than or equal to uncategorized_skill_count %s, received \'%s\''
                % (self.uncategorized_skill_count, self.total_skill_count)
            )

        if self.total_published_node_count < 0:
            raise utils.ValidationError(
                'Expected total_published_node_count to be non-negative, received \'%s\''
                % self.total_published_node_count
            )

        if self.subtopic_count < 0:
            raise utils.ValidationError(
                'Expected subtopic_count to be non-negative, received \'%s\''
                % self.subtopic_count
            )
