...
class BlogPostDomainUnitTests(test_utils.GenericTestBase):
    ...
    def test_blog_post_id_validation_for_blog_post(self) -> None:
        self.assertIsNone(blog_domain.BlogPost.require_valid_blog_post_id(self.blog_post.id))
    ...
    def test_title_without_non_strict_validation(self) -> None:
        self._assert_valid_title_for_blog_post(
            'Blog Post title should at most have 65 chars, received: Very long title with more than sixty five chars and therefore an invalid blog post title',
            'Very long title with more than sixty five chars and therefore an invalid blog post title',
        )
    ...
    def test_url_fragment_validation(self) -> None:
        ...
        self._assert_valid_url_fragment_for_blog_post(
            'Blog Post URL Fragment field should not exceed %d characters.' % (url_fragment_char_limit),
            url_fragment,
        )
        self._assert_valid_url_fragment_for_blog_post(
            'Blog Post URL Fragment field contains invalid characters.Only lowercase words, numbers separated by hyphens are allowed. Received %s.' % ('oppia-in-covid19-#'),
            'oppia-in-covid19-#',
        )
    ...
    def test_tags_validation_for_blog_post(self) -> None:
        ...
        self._assert_valid_tags_for_blog_post(
            'Expected each tag in \'tags\' to be a string, received: \'%s\'' % '123',
            [123],  # type: ignore[list-item]
        )
        self._assert_valid_tags_for_blog_post(
            'Tags should only contain alphanumeric characters and spaces, received: \'%s\'' % 'Alpha@',
            ['Alpha@'],
        )
        self._assert_valid_tags_for_blog_post(
            'Tags should not start or end with whitespace, received: \'%s\'' % ' a b',
            [' a b'],
        )
        self._assert_valid_tags_for_blog_post(
            'Tags should not start or end with whitespace, received: \'%s\'' % 'a b ',
        ...
        self._assert_valid_tags_for_blog_post(
            'Adjacent whitespace in tags should be collapsed, received: \'%s\'' % 'a    b',
            ['a    b'],
        )
...
class BlogPostSummaryUnitTests(test_utils.GenericTestBase):
    ...
    def test_to_dict_returns_correct_representation(self) -> None:
        ...
        expected_dict = {
            ...
            'published_on': utils.convert_naive_datetime_to_string(self.blog_post_summary.published_on),
            'last_updated': utils.convert_naive_datetime_to_string(self.blog_post_summary.last_updated),
        }
...
class BlogAuthorDetailsTests(test_utils.GenericTestBase):
    ...
    def test_to_human_readable_dict(self) -> None:
        ...
        expected_dict = {
            ...
            'last_updated': utils.convert_naive_datetime_to_string(self.author_details.last_updated),
        }
    ...
    def test_author_details_model_raises_error_for_invalid_bio(self) -> None:
        ...
        with self.assertRaisesRegex(
            ...
            'Expected Author Bio to be a string, received %s' % self.author_details.author_bio,
        ):
            self.author_details.validate()
