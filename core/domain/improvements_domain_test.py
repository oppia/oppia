...
class TaskEntryTests(test_utils.GenericTestBase):
    ...
    def test_task_id_has_expected_value(self) -> None:
        task_entry = improvements_domain.TaskEntry(
            ...
            constants.TASK_STATUS_RESOLVED, self.owner_id, self.MOCK_DATE
        )
        self.assertEqual(
            task_entry.task_id, 'exploration.eid.1.high_bounce_rate.state.Introduction'
        )
    ...
    def test_constructor_ignores_resolution_args_when_task_is_obsolete(self) -> None:
        task_entry = improvements_domain.TaskEntry(
            ...
            constants.TASK_TYPE_HIGH_BOUNCE_RATE, constants.TASK_TARGET_TYPE_STATE,
            feconf.DEFAULT_INIT_STATE_NAME, 'issue description',
            constants.TASK_STATUS_OBSOLETE, self.owner_id, self.MOCK_DATE
        )
