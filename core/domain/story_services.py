...
def _create_story(
    committer_id: str, story: story_domain.Story, commit_message: str,
            ...
            if change.cmd == story_domain.CMD_ADD_STORY_NODE:
                ...
                if (change.property_name == story_domain.STORY_NODE_PROPERTY_OUTLINE):
                    ...
                    update_node_description_cmd = cast(
                        story_domain.UpdateStoryNodePropertyDescriptionCmd, change
                    )
                    ...
                    update_node_thumbnail_filename_cmd = cast(
                        story_domain.UpdateStoryNodePropertyThumbnailFilenameCmd, change
                    )
                    ...
                    update_node_thumbnail_bg_color = cast(
                        story_domain.UpdateStoryNodePropertyThumbnailBGColorCmd, change
                    )
                    ...
                    update_node_acquired_skill_ids_cmd = cast(
                        story_domain.UpdateStoryNodePropertyAcquiredSkillIdsCmd, change
                    )
                    ...
                    update_node_exploration_id_cmd = cast(
                        story_domain.UpdateStoryNodePropertyExplorationIdCmd, change
                    )
                    ...
                    update_node_last_modified_cmd = cast(
                        story_domain.UpdateStoryNodePropertyLastModifiedCmd, change
                    )
                ...
                if (change.property_name == story_domain.INITIAL_NODE_ID):
                    ...
                    update_initial_node_id_cmd = cast(
                        story_domain.UpdateStoryContentsPropertyInitialNodeIdCmd, change
                    )
...
def _save_story(
    committer_id: str, story: story_domain.Story, commit_message: str,
    change_list: List[story_domain.StoryChange], story_is_published: bool
) -> None:
...
def compute_summary_of_story(story: story_domain.Story) -> story_domain.StorySummary:
    ...
    story_summary = story_domain.StorySummary(
        story.id, story.title, story.description, story.language_code, story.version,
        story_model_node_titles, story.thumbnail_bg_color, story.thumbnail_filename,
        story.url_fragment, story.created_on, story.last_updated
    )
...
def get_chapter_notifications_stories_list(
    ...
    for topic_model in topic_models:
        ...
        if topic_rights.topic_is_published:
            canonical_stories = [
                story for story in all_canonical_stories
            ...
            for story in canonical_stories:
                ...
                if upcoming_chapters or overdue_chapters:
                    story_timeliness = story_domain.StoryPublicationTimeliness(
                        story.id, story.title, topic_model.name, overdue_chapters,
                        upcoming_chapters
                    )
