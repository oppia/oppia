...
from typing import (
    Callable, Dict, Final, List, Literal, Mapping, Match, Optional, Sequence, Set,
...
if MYPY:  # pragma: no cover
    ...
    AllowedSuggestionClasses = Union[suggestion_registry.SuggestionEditStateContent,
...
SUGGESTION_TRANSLATE_CONTENT_HTML: Callable[
    [suggestion_registry.SuggestionTranslateContent],
    str] = lambda suggestion: suggestion.change_cmd.translation_html

SUGGESTION_ADD_QUESTION_HTML: Callable[
    [suggestion_registry.SuggestionAddQuestion],
    str] = lambda suggestion: suggestion.change_cmd.question_dict['question_state_data'
                                                                  ]['content']['html']
...
def create_suggestion(
    suggestion_type: Literal['add_question'], target_type: str, target_id: str,
    target_version_at_submission: int, author_id: str,
...
@overload
def create_suggestion(
    suggestion_type: Literal['translate_content'], target_type: str, target_id: str,
    target_version_at_submission: int, author_id: str,
    change_cmd: Mapping[str, change_domain.AcceptableChangeDictTypes],
    description: Optional[str]
) -> suggestion_registry.SuggestionTranslateContent:
...
@overload
def create_suggestion(
    suggestion_type: Literal['edit_exploration_state_content'], target_type: str,
    target_id: str, target_version_at_submission: int, author_id: str,
    change_cmd: Mapping[str, change_domain.AcceptableChangeDictTypes],
    description: Optional[str]
...
@overload
def create_suggestion(
    suggestion_type: str, target_type: str, target_id: str,
    target_version_at_submission: int, author_id: str,
    change_cmd: Mapping[str, change_domain.AcceptableChangeDictTypes],
    description: Optional[str]
...
def create_suggestion(
    ...
    if suggestion_type == feconf.SUGGESTION_TYPE_EDIT_STATE_CONTENT:
        ...
        suggestion: AllowedSuggestionClasses = (
            suggestion_registry.SuggestionEditStateContent(
                ...
                None, change_cmd, score_category, language_code, False,
        ...
        question_dict = cast(question_domain.QuestionDict, change_cmd['question_dict'])
        ...
        suggestion = suggestion_registry.SuggestionAddQuestion(
            thread_id, target_id, target_version_at_submission, status, author_id, None,
            change_cmd, score_category, add_question_language_code, False,
            datetime.datetime.utcnow(), datetime.datetime.utcnow()
        )
    else:
    ...
    suggestion_models.GeneralSuggestionModel.create(
        suggestion_type, target_type, target_id, target_version_at_submission, status,
        author_id, None, change_cmd, score_category, thread_id, suggestion.language_code
    )
...
def get_suggestion_from_model(
    ...
    return suggestion_domain_class(
        ...
        suggestion_model.target_version_at_submission, suggestion_model.status,
        suggestion_model.author_id, suggestion_model.final_reviewer_id,
        suggestion_model.change_cmd, suggestion_model.score_category,
        suggestion_model.language_code, suggestion_model.edited_by_reviewer,
        suggestion_model.last_updated, suggestion_model.created_on
    )
...
def get_translation_contribution_stats_models(
    ...
    """
    stats_models = (
        suggestion_models.TranslationContributionStatsModel.get_multi(list(stats_ids))
    )
...
def get_translation_review_stats_models(
    ...
    """
    stats_models = (
        suggestion_models.TranslationReviewStatsModel.get_multi(list(stats_ids))
    )
...
def get_question_contribution_stats_models(
    ...
    """
    stats_models = (
        suggestion_models.QuestionContributionStatsModel.get_multi(list(stats_ids))
    )
...
def get_question_review_stats_models(
    ...
    """
    stats_models = (
        suggestion_models.QuestionReviewStatsModel.get_multi(list(stats_ids))
    )
...
def query_suggestions(
    ...
    """
    return [
        get_suggestion_from_model(s) for s in suggestion_models.GeneralSuggestionModel.
        query_suggestions(query_fields_and_values)
    ]
...
def _update_suggestions(
    ...
    suggestion_models.GeneralSuggestionModel.update_timestamps_multi(
        suggestion_models_to_update, update_last_updated_time=update_last_updated_time
    )
...
def get_commit_message_for_suggestion(author_username: str, commit_message: str) -> str:
    ...
    """
    return '%s %s: %s' % (
        feconf.COMMIT_MESSAGE_ACCEPTED_SUGGESTION_PREFIX, author_username,
        commit_message
    )


def accept_suggestion(
    suggestion_id: str, reviewer_id: str, commit_message: str, review_message: str
) -> None:
...
def resubmit_rejected_suggestion(
    suggestion_id: str, summary_message: str, author_id: str,
    ...
    feedback_services.create_message(
        suggestion_id, author_id, feedback_models.STATUS_CHOICES_OPEN, None,
        summary_message
    )
...
def get_all_suggestions_that_can_be_reviewed_by_user(
    ...
    return (
        [
            ...
            get_in_review_suggestions_in_score_categories(score_categories, user_id)
        ]
    )
...
def get_reviewable_translation_suggestion_target_ids(
    ...
    language_codes = (
        allowed_language_codes_for_review if not filtering_by_language_code else
        [language_code] if language_code in allowed_language_codes_for_review else []
    )
...
def get_translation_suggestions_waiting_longest_for_review(
    ...
    translation_suggestion_models = (
        suggestion_models.GeneralSuggestionModel.
        get_translation_suggestions_waiting_longest_for_review(language_code)
    )
...
def get_translation_suggestions_in_review(
    ...
    suggestion_models_in_review = (
        suggestion_models.GeneralSuggestionModel.
        get_in_review_translation_suggestions_by_exp_id(exp_id)
    )
...
def get_translation_suggestions_in_review_by_exploration(
    ...
    suggestion_models_in_review = (
        suggestion_models.GeneralSuggestionModel.
        get_translation_suggestions_in_review_with_exp_id(exp_id, language_code)
    )
    return [get_suggestion_from_model(model) for model in suggestion_models_in_review]
...
def get_translation_suggestions_in_review_by_exp_ids(
    ...
    suggestion_models_in_review = (
        suggestion_models.GeneralSuggestionModel.
        get_in_review_translation_suggestions_by_exp_ids(exp_ids, language_code)
    )
    return [
...
def get_suggestions_with_editable_explorations(
    ...
    return list(
        filter(
            lambda suggestion: suggestion_exp_id_to_exp[suggestion.target_id].
            edits_allowed, suggestions
        )
    )
...
def _get_plain_text_from_html_content_string(html_content_string: str) -> str:
    ...
    def _replace_rte_tag(rte_tag: Match[str]) -> str:
        ...
        rte_tag_name = re.search(r'oppia-noninteractive-(\w|-)+', rte_tag_string)
...
def get_suggestions_waiting_for_review_info_to_notify_reviewers(
    ...
    for user_contribution_rights in users_contribution_rights:
        ...
        # efficiently sorted.
        suggestions_waiting_longest_heap: List[Tuple[datetime.datetime,
                                                     suggestion_registry.BaseSuggestion]
                                               ] = []
...
def get_submitted_suggestions_by_offset(
    user_id: str, suggestion_type: Literal['add_question'], limit: int, offset: int,
    sort_key: Optional[str]
...
@overload
def get_submitted_suggestions_by_offset(
    user_id: str, suggestion_type: Literal['translate_content'], limit: int,
    offset: int, sort_key: Optional[str]
...
@overload
def get_submitted_suggestions_by_offset(
    user_id: str, suggestion_type: str, limit: int, offset: int, sort_key: Optional[str]
...
def get_user_proficiency_from_model(
    ...
    return user_domain.UserContributionProficiency(
        ...
        user_proficiency_model.score, user_proficiency_model.onboarding_email_sent
    )
...
def get_all_scores_of_user(user_id: str) -> Dict[str, int]:
    ...
    """
    scores = {}
    for model in (user_models.UserContributionProficiencyModel.get_all_scores_of_user(
            user_id)):
...
def get_all_translation_contribution_stats(
    ...
    translation_contribution_stats_models = (
        suggestion_models.TranslationContributionStatsModel.get_all_by_user_id(user_id)
    )
    return [
...
def update_translation_suggestion(suggestion_id: str, translation_html: str) -> None:
    ...
    suggestion.change_cmd.translation_html = (
        ...
        if isinstance(translation_html, str) else translation_html
    )
...
def update_question_suggestion(
    suggestion_id: str, skill_difficulty: float,
    question_state_data: state_domain.StateDict, next_content_id_index: int
) -> Optional[suggestion_registry.BaseSuggestion]:
...
def _create_translation_review_stats_from_model(
    translation_review_stats_model: (suggestion_models.TranslationReviewStatsModel)
...
def get_all_translation_review_stats(
    ...
    translation_review_stats_models = (
        suggestion_models.TranslationReviewStatsModel.get_all_by_user_id(user_id)
    )
    return [
...
def get_all_question_contribution_stats(
    ...
    question_contribution_stats_models = (
        suggestion_models.QuestionContributionStatsModel.get_all_by_user_id(user_id)
    )
    return [
...
def get_all_contributor_stats(
    ...
    return suggestion_registry.ContributorStatsSummary(
        user_id, translation_contribution_stats, question_contribution_stats,
...
def _update_question_contribution_stats_models(
    question_contribution_stats: List[suggestion_registry.QuestionContributionStats]
) -> None:
...
def _update_translation_submitter_total_stats_model(
    translation_submitter_total_stats: suggestion_registry.
...
def _update_translation_reviewer_total_stats_models(
    translation_reviewer_total_stat: suggestion_registry.
...
def _update_question_submitter_total_stats_models(
    question_submitter_total_stats: suggestion_registry.
    QuestionSubmitterTotalContributionStats
) -> None:
...
def _update_question_reviewer_total_stats_models(
    question_reviewer_total_stats: suggestion_registry.
    QuestionReviewerTotalContributionStats
) -> None:
...
def update_translation_contribution_stats_at_submission(
    ...
    if translation_submitter_total_stat_model is None:
        ...
        if topic_id not in (translation_submitter_total_stat.
...
def create_stats_for_new_translation_models(
    suggestion_is_accepted: bool, edited_by_reviewer: bool, content_word_count: int
    ...
    return (
        ...
        rejected_translations_count, rejected_translation_word_count,
        accepted_translations_without_reviewer_edits_count, recent_review_outcomes,
        recent_performance, overall_accuracy
    )
...
def update_translation_contribution_stats_at_review(
    ...
    if translation_contribution_stat_model is None:
        ...
        increment_translation_contribution_stats_at_review(
            translation_contribution_stat, content_word_count, suggestion_is_accepted,
...
def update_translation_review_stats(
    ...
    translation_review_stat_model = (
        ...
        suggestion_models.TranslationReviewStatsModel.get(
            suggestion.change_cmd.language_code, suggestion.final_reviewer_id, topic_id
        )
    )
    ...
    if translation_reviewer_total_stat_model is None:
        ...
        translation_reviewer_total_stat = (
            contribution_stats_services.get_translation_reviewer_total_stats_from_model(
                translation_reviewer_total_stat_model
            )
        )
    ...
    if translation_review_stat_model is None:
        ...
        translation_review_stat = (
            _create_translation_review_stats_from_model(translation_review_stat_model)
        )

        increment_translation_review_stats(
            translation_review_stat, content_word_count, suggestion.last_updated,
            suggestion_is_accepted, suggestion.edited_by_reviewer
        )
...
def update_question_contribution_stats_at_submission(
    ...
    for topic in skill_services.get_all_topic_assignments_for_skill(suggestion.target_id
        ...
        question_submitter_total_stat = (
            contribution_stats_services.get_question_submitter_total_stats_from_model(
                question_submitter_total_stat_model
            )
        )
        ...
        if topic.topic_id not in (
            ...
            (question_submitter_total_stat.topic_ids_with_question_submissions).append(
                topic.topic_id
            )
...
def update_question_contribution_stats_at_review(
    ...
    for topic in skill_services.get_all_topic_assignments_for_skill(suggestion.target_id
        ...
        increment_question_submitter_total_stats_at_review(
            question_submitter_total_stat, suggestion_is_accepted,
            suggestion.edited_by_reviewer
        )
...
def update_question_review_stats(
    ...
    for topic in skill_services.get_all_topic_assignments_for_skill(suggestion.target_id
        ...
        question_review_stat = (
            _create_question_review_stats_from_model(question_review_stat_model)
        )

        increment_question_review_stats(
            question_review_stat, suggestion.last_updated, suggestion_is_accepted,
            suggestion.edited_by_reviewer
        )
    ...
    for topic in skill_services.get_all_topic_assignments_for_skill(suggestion.target_id
        ...
        question_reviewer_total_stat = (
            contribution_stats_services.get_question_reviewer_total_stats_from_model(
                question_reviewer_total_stat_model
            )
        )
        ...
        if topic.topic_id not in (
            ...
            (question_reviewer_total_stat.topic_ids_with_question_reviews).append(
                topic.topic_id
            )
...
def increment_translation_contribution_stats_at_review(
    translation_contribution_stat: (suggestion_registry.TranslationContributionStats),
    content_word_count: int, suggestion_is_accepted: bool, edited_by_reviewer: bool
) -> None:
...
def increment_translation_review_stats(
    ...
    content_word_count: int, last_contribution_date: datetime.datetime,
    suggestion_is_accepted: bool, edited_by_reviewer: bool
) -> None:
...
def increment_question_review_stats(
    ...
    last_contribution_date: datetime.datetime, suggestion_is_accepted: bool,
...
def increment_translation_submitter_total_stats_at_review(
    ...
    if suggestion_is_accepted:
        ...
        if (len(translation_submitter_total_stat.recent_review_outcomes)
                >= RECENT_REVIEW_OUTCOMES_LIMIT):
            oldest_outcome = (translation_submitter_total_stat.recent_review_outcomes
                              ).pop(0)
...
def increment_translation_reviewer_total_stats(
    translation_reviewer_total_stat: suggestion_registry.
    TranslationReviewerTotalContributionStats, content_word_count: int,
    last_contribution_date: datetime.datetime, suggestion_is_accepted: bool,
    edited_by_reviewer: bool
) -> None:
...
def increment_question_submitter_total_stats_at_review(
    ...
    if suggestion_is_accepted:
        ...
        if (len(question_submitter_total_stat.recent_review_outcomes)
                >= RECENT_REVIEW_OUTCOMES_LIMIT):
            oldest_outcome = (question_submitter_total_stat.recent_review_outcomes
                              ).pop(0)
            if oldest_outcome == suggestion_models.REVIEW_OUTCOME_REJECTED:
...
def increment_question_reviewer_total_stats(
    question_reviewer_total_stat: suggestion_registry.
    QuestionReviewerTotalContributionStats, last_contribution_date: datetime.datetime,
    suggestion_is_accepted: bool, edited_by_reviewer: bool
) -> None:
...
def enqueue_contributor_ranking_notification_email_task(
    ...
    taskqueue_services.enqueue_task(
        feconf.TASK_URL_CONTRIBUTOR_DASHBOARD_ACHIEVEMENT_NOTIFICATION_EMAILS, payload,
        0
    )


def generate_contributor_certificate_data(
    username: str, suggestion_type: str, language_code: Optional[str],
...
def _generate_translation_contributor_certificate_data(
    language_code: str, from_date: datetime.datetime, to_date: datetime.datetime,
    user_id: str
    ...
    if ' (' in language_description:
        language_description = language_description[language_description.find('(') +
                                                    1:language_description.find(')')]
    ...
    return suggestion_registry.ContributorCertificateInfo(
        from_date.strftime('%d %b %Y'), to_date.strftime('%d %b %Y'), signature,
        str(hours_contributed), language_description
    )
