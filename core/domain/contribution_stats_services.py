...
def get_translation_submitter_total_stats_from_model(
    ...
    return suggestion_registry.TranslationSubmitterTotalContributionStats(
        ...
        topic_ids_with_translation_submissions=(translation_submitter_model.topic_ids_with_translation_submissions),
        recent_review_outcomes=(translation_submitter_model.recent_review_outcomes),
        ...
        submitted_translations_count=(translation_submitter_model.submitted_translations_count),
        submitted_translation_word_count=(translation_submitter_model.submitted_translation_word_count),
        accepted_translations_count=(translation_submitter_model.accepted_translations_count),
        accepted_translations_without_reviewer_edits_count=(translation_submitter_model.accepted_translations_without_reviewer_edits_count),
        accepted_translation_word_count=(translation_submitter_model.accepted_translation_word_count),
        rejected_translations_count=(translation_submitter_model.rejected_translations_count),
        rejected_translation_word_count=(translation_submitter_model.rejected_translation_word_count),
        first_contribution_date=(translation_submitter_model.first_contribution_date),
        last_contribution_date=(translation_submitter_model.last_contribution_date),
    )
...
def get_translation_reviewer_total_stats_from_model(
    ...
    return suggestion_registry.TranslationReviewerTotalContributionStats(
        ...
        topic_ids_with_translation_reviews=(translation_reviewer_model.topic_ids_with_translation_reviews),
        reviewed_translations_count=(translation_reviewer_model.reviewed_translations_count),
        accepted_translations_count=(translation_reviewer_model.accepted_translations_count),
        accepted_translations_with_reviewer_edits_count=(translation_reviewer_model.accepted_translations_with_reviewer_edits_count),
        accepted_translation_word_count=(translation_reviewer_model.accepted_translation_word_count),
        rejected_translations_count=(translation_reviewer_model.rejected_translations_count),
        first_contribution_date=(translation_reviewer_model.first_contribution_date),
...
def get_question_submitter_total_stats_from_model(
    ...
    return suggestion_registry.QuestionSubmitterTotalContributionStats(
        ...
        topic_ids_with_question_submissions=(question_submitter_model.topic_ids_with_question_submissions),
        recent_review_outcomes=(question_submitter_model.recent_review_outcomes),
        recent_performance=question_submitter_model.recent_performance,
        overall_accuracy=question_submitter_model.overall_accuracy,
        submitted_questions_count=(question_submitter_model.submitted_questions_count),
        accepted_questions_count=(question_submitter_model.accepted_questions_count),
        accepted_questions_without_reviewer_edits_count=(question_submitter_model.accepted_questions_without_reviewer_edits_count),
        rejected_questions_count=(question_submitter_model.rejected_questions_count),
        first_contribution_date=(question_submitter_model.first_contribution_date),
        last_contribution_date=(question_submitter_model.last_contribution_date),
    )
...
def get_question_reviewer_total_stats_from_model(
    ...
    return suggestion_registry.QuestionReviewerTotalContributionStats(
        ...
        topic_ids_with_question_reviews=(question_reviewer_model.topic_ids_with_question_reviews),
        reviewed_questions_count=(question_reviewer_model.reviewed_questions_count),
        accepted_questions_count=(question_reviewer_model.accepted_questions_count),
        accepted_questions_with_reviewer_edits_count=(question_reviewer_model.accepted_questions_with_reviewer_edits_count),
        rejected_questions_count=(question_reviewer_model.rejected_questions_count),
        first_contribution_date=(question_reviewer_model.first_contribution_date),
    ...
    Returns:
        3-tuple(sorted_results, next_offset, more). where:
                ...
                more: bool. If True, there are (probably) more results after
                    ...
                    after this batch.
    """
    translation_submitter_models, next_offset, more = suggestion_models.TranslationSubmitterTotalContributionStatsModel.fetch_page(
        page_size=page_size,
        offset=offset,
    ...
    Returns:
        3-tuple(sorted_results, next_offset, more). where:
            ...
            more: bool. If True, there are (probably) more results after
                ...
                after this batch.
    """
    translation_reviewer_models, next_offset, more = suggestion_models.TranslationReviewerTotalContributionStatsModel.fetch_page(
        page_size=page_size,
        offset=offset,
        language_code=language_code,
        sort_by=sort_by,
...
def get_question_submitter_total_stats(
    ...
    topic_ids: Optional[List[str]],
    max_days_since_last_activity: Optional[int],
) -> Tuple[List[suggestion_registry.QuestionSubmitterTotalContributionStats], int, bool]:
    """Returns the list of domain objects according to values specified.
    ...
    Returns:
        3-tuple(sorted_results, next_offset, more). where:
            ...
            more: bool. If True, there are (probably) more results after
                ...
                after this batch.
    """
    question_submitter_models, next_offset, more = suggestion_models.QuestionSubmitterTotalContributionStatsModel.fetch_page(
        page_size=page_size,
        offset=offset,
        sort_by=sort_by,
        topic_ids=topic_ids,
...
def get_question_reviewer_total_stats(
    ...
    sort_by: Optional[str],
    max_days_since_last_activity: Optional[int],
) -> Tuple[List[suggestion_registry.QuestionReviewerTotalContributionStats], int, bool]:
    """Returns the list of domain objects according to values specified.
    ...
    Returns:
        3-tuple(sorted_results, next_offset, more). where:
            ...
            more: bool. If True, there are (probably) more results after
                ...
                after this batch.
    """
    question_reviewer_models, next_offset, more = suggestion_models.QuestionReviewerTotalContributionStatsModel.fetch_page(
        page_size=page_size,
        offset=offset,
        sort_by=sort_by,
        max_days_since_last_activity=max_days_since_last_activity,
