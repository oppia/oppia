...
class QuestionServicesUnitTest(test_utils.GenericTestBase):
    ...
    def test_get_questions_by_skill_ids_with_fetch_by_difficulty(self) -> None:
        ...
        def sort_fn(question: question_domain.Question) -> float:
            return question.last_updated.timestamp() if question.last_updated else 0
    ...
    def test_get_questions_by_skill_ids_raise_error_with_high_question_count(
        ...
        with self.assertRaisesRegex(
            Exception,
            'Question count is too high, please limit the question count to %d.' % feconf.MAX_QUESTIONS_FETCHABLE_AT_ONE_TIME,
        ):
    ...
    def test_create_multi_question_skill_links_for_question(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            'Skill difficulties and skill ids should match. The lengths of the two lists are different.',
        ):
    ...
    def test_create_and_get_question_skill_link(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            re.escape('Entity for class QuestionModel with id %s not found' % (question_id_2)),
        ):
        ...
        with self.assertRaisesRegex(
            Exception,
            'Querying linked question summaries for more than 3 skills at a time is not supported currently.',
        ):
        ...
        self.assertItemsEqual(
            question_ids,
            [question_skill_link.question_id for question_skill_link in merged_question_skill_links],
        )
        ...
        with self.assertRaisesRegex(
            Exception,
            'The question with ID %s is already linked to skill skill_1' % (self.question_id),
        ):
            question_services.create_new_question_skill_link(self.editor_id, self.question_id, 'skill_1', 0.3)
    ...
    def test_delete_question(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            ('Entity for class QuestionModel with id %s not found' % (self.question_id)),
        ):
            question_models.QuestionModel.get(self.question_id)

        with self.assertRaisesRegex(
            Exception,
            ('Entity for class QuestionSummaryModel with id %s not found' % (self.question_id)),
        ):
    ...
    def test_cannot_update_question_with_no_change_list(self) -> None:
        with self.assertRaisesRegex(
            Exception,
            'Unexpected error: received an invalid change list when trying to save question',
        ):
    ...
    def test_cannot_update_question_with_invalid_change_list(self) -> None:
        ...
        self.assertRegex(
            ...
            'object has no attribute \'cmd\' %s invalid_change_list' % self.question_id,
        )
    ...
    def test_cannot_update_question_with_mismatch_of_versions(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            'Trying to update version 2 of question from version 1, which is too old. Please reload the page and try again.',
        ):
            question_services.update_question(
                self.editor_id,
                self.question_id_2,
        ...
        with self.assertRaisesRegex(
            Exception,
            'Unexpected error: trying to update version 1 of question from version 100. Please reload the page and try again.',
        ):
            question_services.update_question(
                self.editor_id,
                self.question_id_2,
    ...
    def test_compute_summary_of_question(self) -> None:
        ...
        self.assertEqual(question_summary.question_content, feconf.DEFAULT_STATE_CONTENT_STR)
    ...
    def test_get_skills_of_question(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            'Entity for class QuestionModel with id non_existent_question_id not found',
        ):
...
class QuestionMigrationTests(test_utils.GenericTestBase):
    ...
    def test_migrate_question_state_from_v31_to_latest(self) -> None:
        answer_group = {
            ...
            'rule_specs': [{'inputs': {'x': ['A', 'B', 'C']}, 'rule_type': 'HasElementsIn'}],
            'training_data': [],
            'tagged_skill_misconception_id': None,
        }
        question_state_dict = {
    ...
    def test_migrate_question_state_from_v35_to_latest(self) -> None:
        ...
        question_state_dict = {
            ...
            'written_translations': {'translations_mapping': {'explanation': {'en': {'html': '<p>test</p>', 'needs_update': True}}}},
            'interaction': {
                'answer_groups': [],
                'confirmed_unclassified_answers': [],
                'customization_args': {},
    ...
    def test_migrate_question_state_from_v41_with_drag_and_drop_sort_input_interaction_to_latest(  # pylint: disable=line-too-long
        ...
        answer_group = {
            ...
            'rule_specs': [
                {
                    'inputs': {'x': [['<p>Choice 1</p>', '<p>Choice 2</p>', 'invalid']]},
