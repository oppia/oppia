...
class SuggestionFormatAuditJob(jobs.BaseMapReduceOneOffJobManager):
    """Job that checks the format of suggestions."""
    ...
    @staticmethod
    def map(item):
        if item.suggestion_type == 'edit_exploration_state_content':
            change_cmd = item.change_cmd
            bad = False
            # Some old suggestions have old_value: None
            if set(change_cmd.keys()) - set([
                'state_name', 'cmd', 'new_value', 'property_name',
                'old_value']):
                bad = True
            if change_cmd['cmd'] != 'edit_state_property':
                bad = True
            if change_cmd['property_name'] != 'content':
                bad = True
            new_value = change_cmd['new_value']
            if set(new_value.keys()) - set(['html', 'content_id']):
                bad = True
            if not isinstance(new_value['html'], basestring):
                bad = True

            if ('old_value' in change_cmd and
                change_cmd['old_value'] is not None):
                bad = True

            if bad:
                yield (
                    'INDIV - Bad content suggestion format %s' % item.id,
                    str(change_cmd))
            else:
                if 'old_value' in change_cmd:
                    yield ('INDIV - Content suggestion with old_value', item.id)
                elif 'content_id' in change_cmd['new_value']:
                    yield (
                        'INDIV - Content suggestion with content_id', item.id)
                else:
                    yield ('Good content suggestion format', 1)
        elif item.suggestion_type == 'translate_content':
            change_cmd = item.change_cmd
            bad = False
            if set(change_cmd.keys()) != set([
                'content_html', 'translation_html', 'state_name',
                'language_code', 'content_id', 'cmd']):
                bad = True
            if change_cmd['cmd'] != 'add_translation':
                bad = True

            if bad:
                yield ('Bad content translation format', str(change_cmd))
            else:
                yield ('Good content translation format', 1)
        else:
            yield (
                'INDIV - Invalid suggestion type: %s' % item.suggestion_type,
                item.id)
