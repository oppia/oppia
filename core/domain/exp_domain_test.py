    ...
    def test_exp_change_object_with_deprecated_cmd(self) -> None:
        with self.assertRaisesRegex(
            ...
            exp_domain.ExplorationChange(
                {'cmd': 'clone', 'property_name': 'content', 'old_value': 'old_value'}
            )
...
class ExplorationVersionsDiffDomainUnitTests(test_utils.GenericTestBase):
    ...
    def test_revert_exploration_commit(self) -> None:
        exp_change = exp_domain.ExplorationChange(
            {'cmd': exp_models.ExplorationModel.CMD_REVERT_COMMIT, 'version_number': 1}
        )

        self.assertEqual(exp_change.version_number, 1)

        exp_change = exp_domain.ExplorationChange(
            {'cmd': exp_models.ExplorationModel.CMD_REVERT_COMMIT, 'version_number': 2}
        )
...
class ExplorationDomainUnitTests(test_utils.GenericTestBase):
    ...
    def test_collapsible_rte_tag(self) -> None:
        ...
        self._assert_validation_error(
            self.new_exploration, 'No collapsible content is present ' 'inside the tag.'
        )
    ...
    def test_numeric_interaction(self) -> None:
        ...
        test_ans_group_for_numeric_interaction = [
            state_domain.AnswerGroup.from_dict(
                {
                    'rule_specs': [
                        {'rule_type': 'IsLessThanOrEqualTo', 'inputs': {'x': 7}},
                        {
                        ...
                        {'rule_type': 'IsLessThanOrEqualTo', 'inputs': {'x': 7}},
                        {'rule_type': 'IsGreaterThanOrEqualTo', 'inputs': {'x': 10}},
                        {'rule_type': 'IsGreaterThanOrEqualTo', 'inputs': {'x': 15}},
                    ],
                    'outcome': {
                        'dest': 'EXP_1_STATE_1',
                        'feedback': {'content_id': 'feedback_0', 'html': '<p>good</p>'},
            ...
            state_domain.AnswerGroup.from_dict(
                {
                    'rule_specs': [
                        ...
                        {'rule_type': 'HasDenominatorEqualTo', 'inputs': {'x': 4}},
                        {
                            'rule_type': 'HasFractionalPartExactlyEqualTo',
                            'inputs': {
                                'f': {
            ...
            state_domain.AnswerGroup.from_dict(
                {
                    'rule_specs': [
                        ...
                        },
                        {
                            ...
                            'inputs': {'x': 'ca_choices_0', 'y': 'ca_choices_0'},
                        },
                        {'rule_type': 'IsEqualToOrdering', 'inputs': {'x': empty_list}},
                        {
                            'rule_type': 'HasElementXAtPositionY',
    ...
    def test_validation(self) -> None:
        ...
        rule_spec.inputs = {
            'x': {'contentId': 'rule_input_Equals', 'normalizedStrSet': ['Test']}
        }
        rule_spec.rule_type = 'Contains'
        exploration.validate()
    ...
    def test_validate_exploration_param_name(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception, 'No parameter named \'invalid\' exists in this ' 'exploration'
        ):
            exploration.validate()
...
class ExplorationChangesMergeabilityUnitTests(
            ...
            exp_domain.ExplorationChange(
                {
                    ...
                    'old_value': {
                        'refresher_exploration_id': None,
                        'feedback': {'html': '', 'content_id': 'default_outcome'},
                        'missing_prerequisite_skill_id': None,
                        'labelled_as_correct': False,
                        'dest': 'End',
                        'dest_if_really_stuck': None,
    ...
    def test_changes_are_mergeable_when_translations_changes_do_not_conflict(
        ...
        change_list_5 = [
            exp_domain.ExplorationChange(
                {'content_id': 'content', 'cmd': 'mark_translations_needs_update'}
            )
        ]
    ...
    def test_changes_are_mergeable_when_voiceovers_changes_do_not_conflict(
        ...
        change_list = [
            ...
            exp_domain.ExplorationChange(
                {
                    ...
                    'old_value': {
                        'placeholder': {
                            'value': {'unicode_str': '', 'content_id': 'cust_arg_1'}
                        },
                        'rows': {'value': 1},
                        'catchMisspellings': {'value': False},
                    },
            ...
            exp_domain.ExplorationChange(
                {
                    'property_name': 'content',
                    'old_value': {'content_id': 'content_6', 'html': ''},
                    'state_name': 'End',
                    'cmd': 'edit_state_property',
                    'new_value': {
                        'content_id': self.content_id_generator.generate(
            ...
            exp_domain.ExplorationChange(
                {
                    ...
                    'old_value': {
                        'placeholder': {
                            'value': {'unicode_str': '', 'content_id': 'cust_arg_5'}
                        },
                        'rows': {'value': 1},
                        'catchMisspellings': {'value': False},
                    },
