...
class ExplorationGraphUnitTests(test_utils.GenericTestBase):
    ...
    def test_next_explorations(self) -> None:
        ...
        self.assertEqual(
            collection.first_exploration_id, collection.get_next_exploration_id([])
        )
        ...
        self.assertEqual(collection.get_next_exploration_id(['exp_id_0']), 'exp_id_1')
        self.assertEqual(
            collection.get_next_exploration_id(['exp_id_0', 'exp_id_1']), 'exp_id_2'
        )
    ...
    def test_next_explorations_in_sequence(self) -> None:
        ...
        # available explorations thereafter.
        self.assertEqual(
            collection.get_next_exploration_id_in_sequence(exploration_id), None
        )

        collection.add_node('exp_id_1')
        collection.add_node('exp_id_2')
        self.assertEqual(
            collection.get_next_exploration_id_in_sequence(exploration_id), 'exp_id_1'
        )

        self.assertEqual(
            collection.get_next_exploration_id_in_sequence('exp_id_1'), 'exp_id_2'
        )
...
class YamlCreationUnitTests(test_utils.GenericTestBase):
    ...
    def test_from_yaml_with_no_schema_version_specified_raises_error(self) -> None:
        ...
        collection = collection_domain.Collection(
            self.COLLECTION_ID, 'title', 'category', 'objective', 'en', [], None, [], 0
    ...
    def test_from_yaml_with_invalid_schema_version_raises_error(self) -> None:
        collection = collection_domain.Collection(
            self.COLLECTION_ID, 'title', 'category', 'objective', 'en', [], 0, [], 0
        )
...
class SchemaMigrationMethodsUnitTests(test_utils.GenericTestBase):
    ...
    def test_correct_collection_schema_conversion_methods_exist(self) -> None:
        ...
        self.assertFalse(
            hasattr(
                collection_domain.Collection, '_convert_v%s_dict_to_v%s_dict' % (
                    current_collection_schema_version,
                    current_collection_schema_version + 1
                )
            )
...
class CollectionSummaryTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.collection_summary = collection_domain.CollectionSummary(
            ...
            constants.ACTIVITY_STATUS_PRIVATE, True, ['owner_id'], ['editor_id'],
            ['viewer_id'], ['contributor_id'], {}, 1, 1, current_time, current_time
        )
    ...
    def test_is_private(self) -> None:
        ...
        self.collection_summary = collection_domain.CollectionSummary(
            'col_id', 'title', 'category', 'objective', 'en', [],
            constants.ACTIVITY_STATUS_PUBLIC, True, ['owner_id'], ['editor_id'],
            ['viewer_id'], ['contributor_id'], {}, 1, 1, datetime.datetime.utcnow(),
    ...
    def test_is_editable_by(self) -> None:
        ...
        self.collection_summary = collection_domain.CollectionSummary(
            'col_id', 'title', 'category', 'objective', 'en', [],
            constants.ACTIVITY_STATUS_PUBLIC, False, ['owner_id'], ['editor_id'],
            ['viewer_id'], ['contributor_id'], {}, 1, 1, datetime.datetime.utcnow(),
            datetime.datetime.utcnow()
        )
    ...
    def test_is_solely_owned_by_user_one_owner(self) -> None:
        ...
        self.collection_summary = collection_domain.CollectionSummary(
            'col_id', 'title', 'category', 'objective', 'en', [],
            constants.ACTIVITY_STATUS_PUBLIC, True, ['other_id'], ['editor_id'],
            ['viewer_id'], ['contributor_id'], {}, 1, 1, datetime.datetime.utcnow(),
            datetime.datetime.utcnow()
        )
    ...
    def test_add_new_contribution_for_user_does_not_add_system_user(self) -> None:
        ...
        self.assertNotIn(
            feconf.SYSTEM_COMMITTER_ID, self.collection_summary.contributors_summary
        )
        self.assertNotIn(
