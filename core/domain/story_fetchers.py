...
def _migrate_story_contents_to_latest_schema(
    versioned_story_contents: story_domain.VersionedStoryContentsDict, story_id: str
) -> None:
...
def get_story_from_model(story_model: story_models.StoryModel) -> story_domain.Story:
    ...
    return story_domain.Story(
        ...
        story_model.corresponding_topic_id, story_model.version,
        story_model.url_fragment, story_model.meta_tag_content, story_model.created_on,
...
def get_story_summary_from_model(
    ...
    return story_domain.StorySummary(
        ...
        story_summary_model.thumbnail_bg_color, story_summary_model.thumbnail_filename,
        story_summary_model.url_fragment, story_summary_model.story_model_created_on,
...
def get_story_by_id(story_id: str,
                    strict: bool = True,
                    version: Optional[int] = None) -> Optional[story_domain.Story]:
...
def get_story_summary_by_id(story_id: str,
                            strict: bool = True) -> Optional[story_domain.StorySummary]:
...
@overload
def get_stories_by_ids(story_ids: List[str], *,
                       strict: Literal[True]) -> List[story_domain.Story]:
    ...
...
@overload
def get_stories_by_ids(story_ids: List[str], *,
                       strict: Literal[False]) -> List[Optional[story_domain.Story]]:
    ...


def get_stories_by_ids(story_ids: List[str],
                       strict: bool = False) -> Sequence[Optional[story_domain.Story]]:
    ...
    for index, story_model in enumerate(all_story_models):
        if story_model is None:
            if strict:
                raise Exception(
                    'No story model exists for the story_id: %s' % story_ids[index]
                )
...
def get_completed_nodes_in_story(user_id: str,
                                 story_id: str) -> List[story_domain.StoryNode]:
...
def get_user_progress_in_story_chapters(
    ...
    exp_ids = [
        node.exploration_id for node in all_valid_story_nodes if node.exploration_id
    ]
