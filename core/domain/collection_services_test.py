...
class CollectionQueriesUnitTests(CollectionServicesUnitTests):
    ...
    def test_get_collection_titles_and_categories(self) -> None:
        ...
        self.assertEqual(
            ...
            {'A': {'category': 'A category', 'title': 'TitleA'}},
        )
...
class CollectionProgressUnitTests(CollectionServicesUnitTests):
    ...
    def test_get_completed_exploration_ids(self) -> None:
        ...
        self.assertEqual(
            collection_services.get_completed_exploration_ids('Fake', self.COL_ID_0), []
        )
        self.assertEqual(
            collection_services.get_completed_exploration_ids(self.owner_id, 'Fake'), []
        )
...
class CollectionSummaryQueriesUnitTests(CollectionServicesUnitTests):
    ...
    def setUp(self) -> None:
        ...
        self.save_new_default_collection(
            self.COL_ID_5,
            self.owner_id,
            title='Private collection in Spanish',
            category='Languages',
        )
...
class CollectionCreateAndDeleteUnitTests(CollectionServicesUnitTests):
    ...
    def test_soft_deletion_of_collection(self) -> None:
        ...
        self.assertIsNone(
            collection_models.CollectionSummaryModel.get_by_id(self.COLLECTION_0_ID)
        )
        ...
        collection_snapshot_id = collection_models.CollectionModel.get_snapshot_id(
    ...
    def test_soft_deletion_of_multiple_collections(self) -> None:
        ...
        self.assertIsNone(
            collection_models.CollectionSummaryModel.get_by_id(self.COLLECTION_1_ID)
        )
        ...
        # The snapshot models exist.
        collection_0_snapshot_id = collection_models.CollectionModel.get_snapshot_id(
            self.COLLECTION_0_ID, 1
        )
        collection_1_snapshot_id = collection_models.CollectionModel.get_snapshot_id(
    ...
    def test_save_and_retrieve_collection(self) -> None:
        ...
        collection_services._save_collection(  # pylint: disable=protected-access
            self.owner_id, collection, '', _get_collection_change_list('title', '')
        )
    ...
    def test_save_and_retrieve_collection_summary(self) -> None:
        ...
        retrieved_collection_summary = collection_services.get_collection_summary_by_id(
            self.COLLECTION_0_ID
        )
...
class CollectionSnapshotUnitTests(CollectionServicesUnitTests):
    ...
    def test_get_collection_snapshots_metadata(self) -> None:
        ...
        snapshots_metadata = collection_services.get_collection_snapshots_metadata(
            self.COLLECTION_0_ID
        )
        ...
        self.assertDictContainsSubset(
            {
                'commit_cmds': [
                    {'cmd': 'create_new', 'title': 'A title', 'category': 'A category'}
                ],
                'committer_id': self.owner_id,
                'commit_message': ('New collection created with title \'A title\'.'),
                'commit_type': 'create',
    ...
    def test_versioning_with_add_and_delete_nodes(self) -> None:
        ...
        collection = collection_services.get_collection_by_id(self.COLLECTION_0_ID)
        collection.add_node(
            self.save_new_valid_exploration('new_exploration_id', self.owner_id).id
        )
        collection_services._save_collection(  # pylint: disable=protected-access
            'committer_id_2',
            collection,
...
class CollectionSummaryTests(CollectionServicesUnitTests):
    ...
    def test_is_editable_by(self) -> None:
        ...
        rights_manager.assign_role_for_collection(
            self.owner, self.COLLECTION_0_ID, self.viewer_id, rights_domain.ROLE_VIEWER
        )
        rights_manager.assign_role_for_collection(
            self.owner, self.COLLECTION_0_ID, self.editor_id, rights_domain.ROLE_EDITOR
        )
    ...
    def test_contributor_ids(self) -> None:
        ...
        rights_manager.assign_role_for_collection(
            albert, self.COLLECTION_0_ID, self.viewer_id, rights_domain.ROLE_VIEWER
        )
        rights_manager.assign_role_for_collection(
            albert, self.COLLECTION_0_ID, self.editor_id, rights_domain.ROLE_EDITOR
        )
        ...
        self.assertItemsEqual(
            collection_summary.contributor_ids, [self.albert_id, self.bob_id]
        )
    ...
    def test_contributor_summary(self) -> None:
        ...
        self._check_contributors_summary(
            self.COLLECTION_0_ID, {self.albert_id: 1, self.bob_id: 2}
        )

        # Have Albert update that collection. Version 4.
        collection_services.update_collection(
            self.albert_id, self.COLLECTION_0_ID, changelist_cmds, 'Changed title.'
        )
        self._check_contributors_summary(
            self.COLLECTION_0_ID, {self.albert_id: 2, self.bob_id: 2}
        )
