...
def get_topic_from_model(topic_model: topic_models.TopicModel) -> topic_domain.Topic:
    ...
    return topic_domain.Topic(
        topic_model.id, topic_model.name, topic_model.abbreviated_name,
        topic_model.url_fragment, topic_model.thumbnail_filename,
        topic_model.thumbnail_bg_color, topic_model.thumbnail_size_in_bytes,
        topic_model.description, [
            topic_domain.StoryReference.from_dict(reference)
            for reference in versioned_canonical_story_references['story_references']
        ], [
            topic_domain.StoryReference.from_dict(reference)
            for reference in versioned_additional_story_references['story_references']
        ...
        topic_model.language_code, topic_model.version,
        feconf.CURRENT_STORY_REFERENCE_SCHEMA_VERSION, topic_model.meta_tag_content,
        topic_model.practice_tab_is_displayed, topic_model.page_title_fragment_for_web,
...
@overload
def get_topic_by_id(
    topic_id: str, *, version: Optional[int] = None
) -> topic_domain.Topic:
    ...
...
def get_topic_by_id(topic_id: str,
                    strict: bool = True,
                    version: Optional[int] = None) -> Optional[topic_domain.Topic]:
    ...
    cached_topic = caching_services.get_multi(
        caching_services.CACHE_NAMESPACE_TOPIC, sub_namespace, [topic_id]
    ).get(topic_id)
...
@overload
def get_topics_by_ids(topic_ids: List[str], *,
                      strict: Literal[True]) -> List[topic_domain.Topic]:
    ...
...
@overload
def get_topics_by_ids(topic_ids: List[str], *,
                      strict: Literal[False]) -> List[Optional[topic_domain.Topic]]:
    ...


def get_topics_by_ids(topic_ids: List[str],
                      strict: bool = False) -> Sequence[Optional[topic_domain.Topic]]:
    ...
    for index, topic_model in enumerate(all_topic_models):
        if topic_model is None:
            if strict:
                raise Exception(
                    'No topic model exists for the topic_id: %s' % topic_ids[index]
                )
...
def get_topic_by_name(topic_name: str,
                      *,
                      strict: Literal[False] = ...) -> Optional[topic_domain.Topic]:
    ...


def get_topic_by_name(topic_name: str,
                      strict: bool = False) -> Optional[topic_domain.Topic]:
    """Returns a domain object representing a topic.
...
@overload
def get_topic_rights(topic_id: str, *,
                     strict: Literal[False]) -> Optional[topic_domain.TopicRights]:
    ...


def get_topic_rights(topic_id: str,
                     strict: bool = True) -> Optional[topic_domain.TopicRights]:
...
def get_topic_rights_from_model(
    ...
    return topic_domain.TopicRights(
        topic_rights_model.id, topic_rights_model.manager_ids,
...
def get_all_topic_summaries() -> List[topic_domain.TopicSummary]:
    ...
    topic_summaries: List[topic_domain.TopicSummary] = [
        get_topic_summary_from_model(summary) for summary in topic_summaries_models
    ]
...
def get_published_topic_summaries() -> List[topic_domain.TopicSummary]:
    ...
    published_topic_ids = [
        topic_id for topic_id, topic_rights in topic_id_to_topic_rights.items()
        if topic_rights.topic_is_published
    ]
    topic_summaries_list = [
        topic_summary
        for topic_summary in get_multi_topic_summaries(published_topic_ids)
...
def get_topic_summary_from_model(
    ...
    return topic_domain.TopicSummary(
        ...
        topic_summary_model.subtopic_count, topic_summary_model.total_skill_count,
        topic_summary_model.total_published_node_count,
        topic_summary_model.thumbnail_filename, topic_summary_model.thumbnail_bg_color,
...
def get_topic_summary_by_id(topic_id: str,
                            strict: bool = True) -> Optional[topic_domain.TopicSummary]:
    ...
    """
    topic_summary_model: Optional[
        topic_models.TopicSummaryModel
    ] = (topic_models.TopicSummaryModel.get(topic_id, strict=strict))
...
@overload
def get_multi_topic_rights(topic_ids: List[str], *,
                           strict: Literal[True]) -> List[topic_domain.TopicRights]:
    ...
...
def get_canonical_story_dicts(user_id: str,
                              topic: topic_domain.Topic) -> List[CannonicalStoryDict]:
