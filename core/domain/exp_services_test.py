...
""" % (
    ...
    def test_export_to_zip_file(self):
        ...
        with zipfile.ZipFile(zip_file_output) as zf:
            ...
            # we compare.
            with zf.open('A title.yaml').read().decode('utf-8') as zf_temp:
                self.assertEqual(
                zf_temp,
    ...
    def test_export_to_zip_file_with_assets(self):
        ...
        with zipfile.ZipFile(zip_file_output) as zf:
            ...
            # we compare.
            with zf.open('A title.yaml').read().decode('utf-8') as zf_temp:
                self.assertEqual(zf_temp, self.SAMPLE_YAML_CONTENT)
            with zf.open('assets/image/abc.png').read() as zf_temp:
                self.assertEqual(zf_temp, raw_image)
