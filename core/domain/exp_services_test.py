...
class ExplorationCommitLogUnitTests(ExplorationServicesUnitTests):
    ...
    def test_latest_updates(self):
        ...
        self.assertEqual(len(exp_commits_mapping_albert), 2)
        self.assertEqual(len(exp_commits_mapping_albert[self.EXP_ID_1]), 0)
        self.assertEqual(len(exp_commits_mapping_albert[self.EXP_ID_2]), 0)

        exp_commits_mapping_bob = (
            exp_services.get_last_updates_for_exp_ids(
