...
class ExplorationChangesMergeabilityUnitTests(ExplorationServicesUnitTests):
    ...
    def test_changes_are_not_mergeable_when_exploration_is_reverted(self):
        self.save_new_valid_exploration(
            self.EXP_0_ID, self.owner_id, end_state_name='End')
        ...
        # Check that when exploration is reverted in backend
        # and we try to make changes in the version more than
        # that of latest version, then the changes are not mergeable.
        changes_are_not_mergeable = exp_services.are_changes_mergeable(
            self.EXP_0_ID, 2, change_list_2)
        self.assertEqual(changes_are_not_mergeable, False)
