...
class ExplorationSummaryQueriesUnitTests(ExplorationServicesUnitTests):
    ...
    def test_search_exploration_summaries(self) -> None:
        # Due to the update in search logic from exact matching to
        # fuzzy search, the results may now include additional
        # exploration IDs that approximately match the search query.
        # As a result, assertions in this test have been updated
        # from strict equality checks (assertEqual) to more flexible
        # checks using assertTrue with issubset . This ensures that
        # the expected explorations are present in the results, even
        # if other fuzzy matches are also included.

        # Search within the 'Architecture' category.
        exp_ids, _ = exp_services.get_exploration_ids_matching_query(
            '', ['Architecture'], [])
        self.assertTrue(
            set([self.EXP_ID_0, self.EXP_ID_1]).issubset(set(exp_ids)))

        # Search for explorations in Finnish.
        exp_ids, _ = exp_services.get_exploration_ids_matching_query(
            '', [], ['fi'])
        self.assertTrue(
            set([self.EXP_ID_1, self.EXP_ID_5]).issubset(set(exp_ids)))
        ...
        exp_ids, _ = exp_services.get_exploration_ids_matching_query(
            'Oppia Introduce', [], [])
        self.assertTrue(
            set([self.EXP_ID_2, self.EXP_ID_3]).issubset(set(exp_ids)))
        ...
        exp_ids, _ = exp_services.get_exploration_ids_matching_query(
            'in', [], [])
        self.assertTrue(
            set([self.EXP_ID_3, self.EXP_ID_6]).issubset(set(exp_ids)))
        ...
        self.assertTrue(
            set([self.EXP_ID_0, self.EXP_ID_3]).issubset(set(exp_ids)))
