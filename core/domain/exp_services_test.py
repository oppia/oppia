...
TestCustArgDictType = Dict[str, Dict[str, Union[bool, Dict[str, Union[str, List[Dict[
    str, Union[str, Dict[str, Union[str, List[List[float]]]]]]]]]]]]
...
class ExplorationSummaryQueriesUnitTests(ExplorationServicesUnitTests):
    ...
    def test_get_exploration_summaries_with_no_query(self) -> None:
        ...
        (exp_ids,
         search_offset) = (exp_services.get_exploration_ids_matching_query('', [], []))
        self.assertEqual(
            sorted(exp_ids), [
                self.EXP_ID_0, self.EXP_ID_1, self.EXP_ID_2, self.EXP_ID_3,
                self.EXP_ID_4, self.EXP_ID_5, self.EXP_ID_6
    ...
    def test_get_exploration_summaries_with_deleted_explorations(self) -> None:
        ...
        # explorations.
        self.assertEqual(
            exp_services.get_exploration_ids_matching_query('', [], []), ([], None)
        )
    ...
    def test_get_exploration_summaries_with_deleted_explorations_multi(self) -> None:
        ...
        exp_services.delete_explorations(
            self.owner_id,
            [self.EXP_ID_0, self.EXP_ID_1, self.EXP_ID_3, self.EXP_ID_5, self.EXP_ID_6]
        )
...
class ExplorationCreateAndDeleteUnitTests(ExplorationServicesUnitTests):
    ...
    def test_soft_deletion_of_exploration(self) -> None:
        ...
        self.assertIsNotNone(
            exp_models.ExplorationSnapshotMetadataModel.get_by_id(exp_snapshot_id)
        )
        self.assertIsNotNone(
            exp_models.ExplorationSnapshotContentModel.get_by_id(exp_snapshot_id)
        )
        ...
        self.assertIsNotNone(
            ...
            get_by_id(exp_rights_snapshot_id)
        )
        self.assertIsNotNone(
    ...
    def test_soft_deletion_of_multiple_explorations(self) -> None:
        ...
        self.assertIsNotNone(
            exp_models.ExplorationSnapshotMetadataModel.get_by_id(exp_0_snapshot_id)
        )
        self.assertIsNotNone(
            exp_models.ExplorationSnapshotContentModel.get_by_id(exp_0_snapshot_id)
        )
        self.assertIsNotNone(
            exp_models.ExplorationSnapshotMetadataModel.get_by_id(exp_1_snapshot_id)
        )
        self.assertIsNotNone(
            exp_models.ExplorationSnapshotContentModel.get_by_id(exp_1_snapshot_id)
        )
        ...
        self.assertIsNotNone(
            exp_models.ExplorationRightsSnapshotMetadataModel.
            get_by_id(exp_0_rights_snapshot_id)
        )
        self.assertIsNotNone(
            exp_models.ExplorationRightsSnapshotContentModel.
            get_by_id(exp_0_rights_snapshot_id)
        ...
        self.assertIsNotNone(
            exp_models.ExplorationRightsSnapshotMetadataModel.
            get_by_id(exp_1_rights_snapshot_id)
        )
        self.assertIsNotNone(
            exp_models.ExplorationRightsSnapshotContentModel.
            get_by_id(exp_1_rights_snapshot_id)
    ...
    def test_activities_of_deleted_explorations_are_deleted(self) -> None:
        ...
        self.assertEqual(
            user_models.CompletedActivitiesModel.get(self.editor_id,
                                                     strict=True).exploration_ids, []
        )
        self.assertEqual(
            user_models.IncompleteActivitiesModel.get(self.owner_id,
                                                      strict=True).exploration_ids, []
        )
    ...
    def test_user_data_of_deleted_explorations_are_deleted(self) -> None:
        ...
        self.assertIsNone(
            user_models.ExplorationUserDataModel.get(self.owner_id, self.EXP_0_ID)
        )
        self.assertIsNone(
            user_models.ExplorationUserDataModel.get('other_user_id', self.EXP_0_ID)
        )
        self.assertIsNone(
            user_models.ExplorationUserDataModel.get(self.owner_id, self.EXP_1_ID)
        )
    ...
    def test_feedbacks_belonging_to_exploration_are_deleted(self) -> None:
        ...
        thread_1_id = feedback_services.create_thread(
            feconf.ENTITY_TYPE_EXPLORATION, self.EXP_0_ID, self.owner_id, 'subject',
            'text'
        )
        thread_2_id = feedback_services.create_thread(
            feconf.ENTITY_TYPE_EXPLORATION, self.EXP_0_ID, self.owner_id, 'subject 2',
            'text 2'
        )
    ...
    def test_apply_change_list(self) -> None:
        self.save_new_linear_exp_with_state_names_and_interactions(
            self.EXP_0_ID,
            self.owner_id, ['State 1', 'State 2'], ['TextInput'],
            category='Algebra'
        )
    ...
    def test_validation_fail_for_answer_groups(self) -> None:
        ...
        exploration.states[feconf.DEFAULT_INIT_STATE_NAME].interaction.answer_groups = [
            state_domain.AnswerGroup(
                state_domain.Outcome(
                    ...
                    'cheerful', 'merry', 'ecstatic', 'glad', 'overjoyed', 'pleased',
                    'thrilled', 'smile'
                ], None
            )
        ]
        error_string = (
            ...
            'its answer groups.' % (feconf.DEFAULT_INIT_STATE_NAME, exploration.id)
        )
        errors = exp_services.validate_exploration_for_story(exploration, False)
        self.assertEqual(len(errors), 1)
        self.assertEqual(errors[0], error_string)
    ...
    def test_validation_fail_for_default_outcome(self) -> None:
        ...
        exploration.states[
            feconf.DEFAULT_INIT_STATE_NAME].interaction.default_outcome = (
                state_domain.Outcome(
                    'state 1', None,
                    state_domain.SubtitledHtml(
...
"""
    ...
    def test_can_load_yaml_with_voiceovers(self) -> None:
        ...
        self.assertEqual(
            answer_group_voiceovers['en'].filename, self.ANSWER_GROUP_AUDIO_FILE
        )
        self.assertEqual(
            default_outcome_voiceovers['en'].filename, self.DEFAULT_OUTCOME_AUDIO_FILE
        )
    ...
    def test_can_load_yaml_with_stripped_voiceovers(self) -> None:
        exp_services.save_new_exploration_from_yaml_and_assets(
            self.owner_id,
            self.YAML_WITH_AUDIO_TRANSLATIONS,
            self.EXP_ID, [],
    ...
    def test_cannot_load_yaml_with_no_schema_version(self) -> None:
        ...
        """
            ...
            self.DEFAULT_OUTCOME_AUDIO_FILE, self.HINT_AUDIO_FILE,
...
class GetImageFilenamesFromExplorationTests(ExplorationServicesUnitTests):
    ...
    def test_get_image_filenames_from_exploration(self) -> None:
        ...
        customization_args_dict1: Dict[str, Dict[str, Union[
            bool, domain.ImageAndRegionDict]]] = {
        ...
        customization_args_dict2: Dict[str, Dict[str, Union[
            bool, List[state_domain.SubtitledHtmlDict]]]] = {
                'choices': {
                    'value': customization_args_choices
                },
                'showChoicesInShuffledOrder': {
                    'value': True
                }
        ...
        expected_output = [
            's1ImagePath.png', 's1Content.png', 's2Choice1.png', 's2Choice2.png',
            's3Choice1.png', 's3Choice2.png', 's3Choice3.png', 's2Hint1.png',
            's2AnswerGroup.png'
        ]
...
""" % (
            ...
            DUMMY_IMAGE_TAG, feconf.CURRENT_STATE_SCHEMA_VERSION
        )
    )
...
""" % (
    ...
    def test_export_to_zip_file(self) -> None:
        ...
        exploration = self.save_new_valid_exploration(
            self.EXP_0_ID, self.owner_id, objective='The objective', category='Algebra'
        )
        content_id_generator = translation_domain.ContentIdGenerator(
            exploration.next_content_id_index
        )
        ...
        # we compare.
        self.assertEqual(
            zf.open('A title.yaml').read().decode('utf-8'), self.SAMPLE_YAML_CONTENT
        )
    ...
    def test_export_by_versions(self) -> None:
        ...
        # we compare.
        self.assertEqual(
            zf.open('A title.yaml').read().decode('utf-8'), self.UPDATED_YAML_CONTENT
        )
...
def _get_change_list(
    state_name: str, property_name: str,
...
class UpdateStateTests(ExplorationServicesUnitTests):
    ...
    def test_delete_state_cmd_rejects_obsolete_translation_suggestions(self) -> None:
        ...
        suggestion = suggestion_services.create_suggestion(
            ...
            self.EXP_0_ID, 1, self.owner_id, add_translation_change_dict,
            'test description'
        )
    ...
    def test_update_interaction_id(self) -> None:
        ...
        change_list = _get_change_list(
            self.init_state_name, exp_domain.STATE_PROPERTY_INTERACTION_ID, 'Continue'
        ) + _get_change_list(
            self.init_state_name, exp_domain.STATE_PROPERTY_INTERACTION_CUST_ARGS, {
                'buttonText': {
                    'value': {
    ...
    def test_update_interaction_customization_args(self) -> None:
        ...
        assert isinstance(
            exploration.init_state.interaction.customization_args['choices'].value, list
        )
        ...
        button_text_subtitle_unicode = cast(
            state_domain.SubtitledUnicode, customization_args['buttonText'].value
        )
    ...
    def test_update_content(self) -> None:
        ...
        exploration = exp_fetchers.get_exploration_by_id(self.EXP_0_ID)
        self.assertEqual(
            exploration.init_state.content.html, '<p><strong>Test content</strong></p>'
        )
    ...
    def test_update_solicit_answer_details(self) -> None:
        ...
        exp_services.update_exploration(
            self.owner_id, self.EXP_0_ID,
            _get_change_list(
                self.init_state_name, exp_domain.STATE_PROPERTY_SOLICIT_ANSWER_DETAILS,
                True
            ), ''
        )
        exploration = exp_fetchers.get_exploration_by_id(self.EXP_0_ID)
    ...
    def test_update_solicit_answer_details_with_non_bool_fails(self) -> None:
        ...
        with self.assertRaisesRegex(
            ...
            exp_services.update_exploration(
                ...
                _get_change_list(
                    self.init_state_name,
                    exp_domain.STATE_PROPERTY_SOLICIT_ANSWER_DETAILS, 'abc'
                ), ''
            )
        exploration = exp_fetchers.get_exploration_by_id(self.EXP_0_ID)
        self.assertEqual(exploration.init_state.solicit_answer_details, False)
    ...
    def test_update_inapplicable_skill_misconception_ids(self) -> None:
        ...
        change_list = _get_change_list(
            'State1', exp_domain.STATE_PROPERTY_INAPPLICABLE_SKILL_MISCONCEPTION_IDS,
            ['string_2']
        )
        changes_are_mergeable = exp_services.are_changes_mergeable(
            self.EXP_0_ID, 3, change_list
    ...
    def test_update_card_is_checkpoint_with_non_bool_fails(self) -> None:
        ...
        with self.assertRaisesRegex(
            ...
            exp_services.update_exploration(
                self.owner_id, self.EXP_0_ID,
                _get_change_list(
                    self.init_state_name, exp_domain.STATE_PROPERTY_CARD_IS_CHECKPOINT,
                    'abc'
                ), ''
            )
        exploration = exp_fetchers.get_exploration_by_id(self.EXP_0_ID)
        ...
        change_list = _get_change_list(
            self.init_state_name, exp_domain.STATE_PROPERTY_CARD_IS_CHECKPOINT, 'abc'
        )
        changes_are_mergeable = exp_services.are_changes_mergeable(
            self.EXP_0_ID, 1, change_list
        )
...
class CommitMessageHandlingTests(ExplorationServicesUnitTests):
    ...
    def test_record_commit_message(self) -> None:
        ...
        exp_services.update_exploration(
            self.owner_id, self.EXP_0_ID,
            _get_change_list(
                self.init_state_name, exp_domain.STATE_PROPERTY_INTERACTION_STICKY,
                False
            ), 'A message'
        )
    ...
    def test_unpublished_explorations_can_accept_commit_message(self) -> None:
        ...
        exp_services.update_exploration(
            self.owner_id, self.EXP_0_ID,
            _get_change_list(
                self.init_state_name, exp_domain.STATE_PROPERTY_INTERACTION_STICKY, True
            ), ''
        )
...
class ExplorationSnapshotUnitTests(ExplorationServicesUnitTests):
    ...
    def test_get_exploration_snapshots_metadata(self) -> None:
        ...
        exp_services.update_exploration(
            second_committer_id, self.EXP_0_ID, new_change_list, 'Second commit.'
        )
    ...
    def test_reverts_exp_to_safe_state_when_content_model_is_missing(self) -> None:
        ...
        snapshot_content_model = (
            exp_models.ExplorationSnapshotContentModel.get('0-5', strict=True)
        )
...
class ExplorationSearchTests(ExplorationServicesUnitTests):
    ...
    def test_index_explorations_given_ids(self) -> None:
        ...
        def mock_add_documents_to_index(docs: List[Dict[str, str]],
                                        index: str) -> List[str]:
            self.assertEqual(index, exp_services.SEARCH_INDEX_EXPLORATIONS)
...
class ExplorationSummaryTests(ExplorationServicesUnitTests):
    ...
    def test_is_exp_summary_editable(self) -> None:
        ...
        self.assertTrue(
            exp_services.is_exp_summary_editable(exp_summary, user_id=self.owner_id)
        )
        self.assertFalse(
            exp_services.is_exp_summary_editable(exp_summary, user_id=self.editor_id)
        )
        self.assertFalse(
            exp_services.is_exp_summary_editable(exp_summary, user_id=self.viewer_id)
        )
    ...
    def test_regenerate_summary_with_new_contributor_with_invalid_exp_id(self) -> None:
        ...
        self.assertEqual(
            observed_log_messages, ['Could not find exploration with ID dummy_id']
        )
...
class ExplorationSummaryGetTests(ExplorationServicesUnitTests):
    ...
    def test_get_non_private_exploration_summaries(self) -> None:
        ...
        simple_props = [
            ...
            'scaled_average_rating', 'status', 'community_owned', 'owner_ids',
            'editor_ids', 'voice_artist_ids', 'viewer_ids', 'contributor_ids',
            'version', 'exploration_model_created_on', 'exploration_model_last_updated'
        ]
    ...
    def test_get_top_rated_exploration_summaries(self) -> None:
        ...
        top_rated_summaries_model = (
            exp_fetchers.get_exploration_summaries_from_models(top_rated_summaries)
        )
    ...
    def test_get_recently_published_exp_summaries(self) -> None:
        ...
        recently_publshed_summaries_model = (
            exp_fetchers.
            get_exploration_summaries_from_models(recently_published_summaries)
        )
    ...
    def test_get_user_exploration_data(self) -> None:
        ...
        exploration_description = (
            exp_services.get_user_exploration_data(self.albert_id, self.EXP_0_ID)
        )
        ...
        exploration_description_draft_applied = (
            exp_services.get_user_exploration_data(self.albert_id, self.EXP_0_ID, True)
        )
        self.assertTrue(
        ...
        exploration_draft_not_applied = (
            exp_services.get_user_exploration_data(self.bob_id, self.EXP_1_ID, True)
        )
...
"""
    ...
    def test_update_language_code(self) -> None:
        ...
        self.assertTrue(changes_are_mergeable)
        exp_services.update_exploration(
            self.albert_id, self.NEW_EXP_ID, change_list, 'Changed language code again.'
        )
    ...
    def test_update_exploration_author_notes(self) -> None:
        ...
        self.assertTrue(changes_are_mergeable)
        exp_services.update_exploration(
            self.albert_id, self.NEW_EXP_ID, change_list, 'Changed author_notes.'
        )
    ...
    def test_update_exploration_with_mark_translation_needs_update_changes(
        ...
        translation_services.add_new_translation(
            ...
            translation_domain.TranslatedContent(
                'Translation', translation_domain.TranslatableContentFormat.HTML, False
            )
        )
        entity_translations = (
    ...
    def test_update_exploration_with_remove_translation_changes(self) -> None:
        ...
        translation_services.add_new_translation(
            ...
            exploration.version, 'hi', 'content_0',
            translation_domain.TranslatedContent(
                'Translation 1', translation_domain.TranslatableContentFormat.HTML,
                False
            )
        )
        translation_services.add_new_translation(
            ...
            translation_domain.TranslatedContent(
                'Translation 2', translation_domain.TranslatableContentFormat.HTML,
                False
            )
        )
        entity_translations = (
    ...
    def test_update_unclassified_answers(self) -> None:
        ...
        exploration = exp_fetchers.get_exploration_by_id(self.NEW_EXP_ID)
        self.assertEqual(
            exploration.init_state.interaction.confirmed_unclassified_answers, ['test']
        )
    ...
    def test_get_exploration_validation_error(self) -> None:
        ...
        validate_swap = self.swap(
            exp_domain.Exploration, 'validate', _mock_exploration_validate_function
        )
        with validate_swap:
            info = exp_services.get_exploration_validation_error(self.NEW_EXP_ID, 0)
...
class ExplorationTranslationCountTests(ExplorationServicesUnitTests):
    ...
    def test_translation_count_upon_adding_new_interaction(self) -> None:
        ...
        translation_services.add_new_translation(
            ...
            translation_domain.TranslatedContent(
                'Translation 3', translation_domain.TranslatableContentFormat.HTML,
                False
            )
        )
    ...
    def test_translation_count_upon_revert(self) -> None:
        ...
        translation_services.add_new_translation(
            ...
            translation_domain.TranslatedContent(
                'Translation 4', translation_domain.TranslatableContentFormat.HTML,
                False
            )
        )
        translation_services.add_new_translation(
            ...
            translation_domain.TranslatedContent(
                'Translation 5', translation_domain.TranslatableContentFormat.HTML,
                False
            )
        )
        translation_services.add_new_translation(
            ...
            translation_domain.TranslatedContent(
                'Translation 6', translation_domain.TranslatableContentFormat.HTML,
                False
            )
        )
        translation_services.add_new_translation(
            ...
            translation_domain.TranslatedContent(
                'Translation 7', translation_domain.TranslatableContentFormat.HTML,
                False
            )
        )
        translation_services.add_new_translation(
            ...
            translation_domain.TranslatedContent(
                'Translation 8', translation_domain.TranslatableContentFormat.HTML,
                False
            )
        )
        translation_services.add_new_translation(
            ...
            translation_domain.TranslatedContent(
                'Translation 9', translation_domain.TranslatableContentFormat.HTML,
                False
            )
        )
...
class EditorAutoSavingUnitTests(test_utils.GenericTestBase):
    ...
    def test_create_or_update_draft_when_newer_draft_exists(self) -> None:
        exp_services.create_or_update_draft(
            self.EXP_ID1, self.USER_ID, self.NEW_CHANGELIST, 5, self.OLDER_DATETIME
        )
        exp_user_data = user_models.ExplorationUserDataModel.get(
            self.USER_ID, self.EXP_ID1
        )
    ...
    def test_create_or_update_draft_when_draft_does_not_exist(self) -> None:
        exp_services.create_or_update_draft(
            self.EXP_ID3, self.USER_ID, self.NEW_CHANGELIST, 5, self.NEWER_DATETIME
        )
        exp_user_data = user_models.ExplorationUserDataModel.get(
    ...
    def test_create_or_update_draft_with_exploration_model_not_created(self) -> None:
        ...
        exp_services.create_or_update_draft(
            'exp_id', self.editor_id, self.NEW_CHANGELIST, 1, self.NEWER_DATETIME
        )
        exp_user_data = user_models.ExplorationUserDataModel.get(
            self.editor_id, 'exp_id'
        )
...
class UpdateVersionHistoryUnitTests(ExplorationServicesUnitTests):
    ...
    def test_creating_new_exploration_creates_version_history_model(self) -> None:
        ...
        self.assertEqual(
            version_history_model.metadata_last_edited_committer_id, self.owner_id
        )
    ...
    def test_soft_deletion_does_not_delete_version_history_models(self) -> None:
        version_history_models_before_deletion: Sequence[
            ...
            self.version_history_model_class.query(
                self.version_history_model_class.exploration_id == self.exploration.id
            ).fetch()
        )
    ...
    def test_version_history_on_rename_state(self) -> None:
        ...
        self.assertEqual(
            new_model.state_version_history.get(feconf.DEFAULT_INIT_STATE_NAME), None
        )
        self.assertEqual(
    ...
    def test_version_history_on_cancelled_rename_state(self) -> None:
        ...
        new_state_name = 'Another name'
        expected_dict = state_domain.StateVersionHistory(None, None,
...
"""
    ...
    def test_logged_out_user_checkpoint_progress_is_updated_correctly(self) -> None:
        ...
        self.assertEqual(
            logged_out_user_data.furthest_reached_checkpoint_state_name, 'Introduction'
        )
        self.assertEqual(
        ...
        self.assertEqual(
            logged_out_user_data.furthest_reached_checkpoint_state_name, 'New state'
        )
        self.assertEqual(
        ...
        self.assertEqual(
            logged_out_user_data.most_recently_reached_checkpoint_state_name, 'Intro'
        )
...
class RegenerateMissingExpStatsUnitTests(test_utils.GenericTestBase):
    ...
    def test_when_exp_and_state_stats_models_exist(self) -> None:
        ...
        self.assertEqual(
            exp_services.regenerate_missing_stats_for_exploration('ID'), ([], [], 1, 1)
        )
    ...
    def test_handle_state_name_is_not_found_in_state_stats_mapping(self) -> None:
        ...
        exp_stats_list = (
            stats_services.get_multiple_exploration_stats_by_version(exp_id, [1, 2, 3])
        )
    ...
    def test_should_be_able_to_create_entity_voiceovers_models(self) -> None:
        ...
        models_to_put = (
            exp_services.compute_models_to_put_when_saving_new_exp_version(
                self.owner_id, 'test_exp_id', voiceover_changes, 'Added voiceover',
                False
            )
        )
