...
class RemoveInvalidPlaythroughsOneOffJob(jobs.BaseMapReduceOneOffJobManager):
    ...
    def map(playthrough_issues_model):
        ...
        if exp_id not in whitelisted_exploration_ids:
            ...
            enumerated_unresolved_issues = list(enumerate(unresolved_issues))
            # Iterate through the issues in reverse so we can pop elements
            # without invalidating the indices to others.
            for i, unresolved_issue in reversed(enumerated_unresolved_issues):
                ...
                # We must update the actual list of playthroughs so that the
                # deleted models are no longer associated.
