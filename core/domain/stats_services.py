...
def advance_version_of_exp_stats(
    ...
    if exp_versions_diff is None:
        raise Exception(
            'ExplorationVersionsDiff cannot be None when the change is' ' not a revert.'
        )
    ...
    for state_name in unchanged_state_names:
        new_state_name_stats_mapping[state_name] = exp_stats.state_stats_mapping[
            state_name
        ].clone()
...
def get_multiple_exploration_stats_by_version(
    ...
    """
    exploration_stats = []
    exploration_stats_models = stats_models.ExplorationStatsModel.get_multi_versions(
...
def get_playthrough_from_model(
    ...
    return stats_domain.Playthrough(
        playthrough_model.exp_id,
        playthrough_model.exp_version,
...
def get_state_reference_for_question(question_id: str) -> str:
    ...
    return stats_models.LearnerAnswerDetailsModel.get_state_reference_for_question(
        question_id
    )
