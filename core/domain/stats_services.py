...
def _update_stats_transactional(
    exp_id: str, exp_version: int, aggregated_stats: stats_domain.AggregatedStatsDict
) -> None:
    ...
    if exploration.version != exp_version:
        logging.error(
            ...
            'the current version is %s.' % (exp_version, exp_id, exploration.version)
        )
        return
    ...
    if exp_stats is None:
        raise Exception(
            'ExplorationStatsModel id="%s.%s" does not exist' % (exp_id, exp_version)
        )
...
def advance_version_of_exp_stats(
    exp_version: int, exp_versions_diff: Optional[exp_domain.ExplorationVersionsDiff],
...
def assign_playthrough_to_corresponding_issue(
    playthrough: stats_domain.Playthrough, exp_issues: stats_domain.ExplorationIssues,
    ...
    if len(issue.playthrough_ids) < feconf.MAX_PLAYTHROUGHS_FOR_ISSUE:
        issue.playthrough_ids.append(
            stats_models.PlaythroughModel.create(
                playthrough.exp_id, playthrough.exp_version, playthrough.issue_type,
...
def _get_corresponding_exp_issue(
    ...
    for issue in exp_issues.unresolved_issues:
        if issue.issue_type == playthrough.issue_type:
            ...
            identifying_arg = (
                feconf.CUSTOMIZATION_ARG_WHICH_IDENTIFIES_ISSUE[issue.issue_type]
            )
...
def get_updated_exp_issues_models_for_new_exp_version(
    ...
    for playthrough_model in playthrough_models:
        ...
        if 'state_names' in playthrough.issue_customization_args:
            ...
            state_names = cast(
                List[str], playthrough.issue_customization_args['state_names']['value']
            )
        ...
        for action in playthrough.actions:
            ...
            if 'dest_state_name' in action_customization_args:
                ...
                action_customization_args['dest_state_name']['value'] = (
                    dest_state_name if dest_state_name not in old_to_new_state_names
    ...
    for exp_issue in exp_issues.unresolved_issues:
        if 'state_names' in exp_issue.issue_customization_args:
            ...
            # customization arg to have values of type List[str].
            state_names = cast(
                List[str], exp_issue.issue_customization_args['state_names']['value']
            )
...
@overload
def get_exp_issues(exp_id: str, exp_version: int, *,
                   strict: Literal[False]) -> Optional[stats_domain.ExplorationIssues]:
    ...


@overload
def get_exp_issues(exp_id: str,
                   exp_version: int,
                   *,
                   strict: bool = ...) -> Optional[stats_domain.ExplorationIssues]:
    ...


def get_exp_issues(exp_id: str,
                   exp_version: int,
                   strict: bool = True) -> Optional[stats_domain.ExplorationIssues]:
...
def get_multiple_exploration_stats_by_version(
    ...
    exploration_stats_models = (
        stats_models.ExplorationStatsModel.get_multi_versions(exp_id, version_numbers)
    )
    for exploration_stats_model in exploration_stats_models:
        exploration_stats.append(
            None if exploration_stats_model is
            None else get_exploration_stats_from_model(exploration_stats_model)
        )
    return exploration_stats
...
def get_exploration_stats_from_model(
    ...
    return stats_domain.ExplorationStats(
        ...
        exploration_stats_model.num_starts_v1, exploration_stats_model.num_starts_v2,
        ...
        exploration_stats_model.num_completions_v2, new_state_stats_mapping
    )
...
def get_playthrough_from_model(
    ...
    return stats_domain.Playthrough(
        ...
        playthrough_model.issue_type, playthrough_model.issue_customization_args,
        actions
    )
...
def create_stats_model(exploration_stats: stats_domain.ExplorationStats) -> str:
    ...
    instance_id = stats_models.ExplorationStatsModel.create(
        ...
        exploration_stats.num_starts_v1, exploration_stats.num_starts_v2,
        exploration_stats.num_actual_starts_v1, exploration_stats.num_actual_starts_v2,
        exploration_stats.num_completions_v1, exploration_stats.num_completions_v2,
        new_state_stats_mapping
    )
    return instance_id
...
def get_exp_issues_model_from_domain_object(
    ...
    unresolved_issues_dicts = [
        unresolved_issue.to_dict() for unresolved_issue in exp_issues.unresolved_issues
    ]
    instance_id = stats_models.ExplorationIssuesModel.get_entity_id(
...
def get_exploration_stats_multi(
    ...
    exploration_stats_models = (
        ...
        get_multi_stats_models(exp_version_references)
    )
...
def record_answer(
    exploration_id: str, exploration_version: int, state_name: str, interaction_id: str,
...
def get_state_answers(exploration_id: str, exploration_version: int,
                      state_name: str) -> Optional[stats_domain.StateAnswers]:
...
def get_learner_answer_details_from_model(
    ...
    return stats_domain.LearnerAnswerDetails(
        ...
        learner_answer_details_model.interaction_id, [
            ...
            for learner_answer_info_dict in
...
def record_learner_answer_info(
    entity_type: str, state_reference: str, interaction_id: str,
    answer: Union[str, int, Dict[str, str], List[str]], answer_details: str
) -> None:
    ...
    learner_answer_info = stats_domain.LearnerAnswerInfo(
        learner_answer_info_id, answer, answer_details, datetime.datetime.utcnow()
    )
