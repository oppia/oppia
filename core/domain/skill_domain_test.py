...
class SkillDomainUnitTests(test_utils.GenericTestBase):
    ...
    def test_update_contents_from_model(self) -> None:
        versioned_skill_contents: skill_domain.VersionedSkillContentsDict = {
            ...
            'skill_contents': {
                ...
                'worked_examples': [
                    {
                        'question': {'html': '<p>A Question</p>', 'content_id': 'id'},
                        'explanation': {
    ...
    def test_update_misconceptions_from_model(self) -> None:
        ...
        self.assertEqual(
            versioned_misconceptions['misconceptions'][0]['must_be_addressed'], True
        )
        ...
        self.assertEqual(
            versioned_misconceptions['misconceptions'][0]['feedback'], expected_feedback
        )
        self.skill.update_misconceptions_from_model(
            versioned_misconceptions, versioned_misconceptions['schema_version']
        )
    ...
    def test_find_prerequisite_skill_id_index(self) -> None:
        ...
        self.assertEqual(
            self.skill._find_prerequisite_skill_id_index(
                'skill_id_2'
    ...
    def test_rubrics_validation(self) -> None:
        ...
        self.skill.rubrics = [
            skill_domain.Rubric(
                constants.SKILL_DIFFICULTIES[0], ['<p>%s</p>' % ('Explanation' * 30)]
            )
        ]
        self._assert_validation_error(
    ...
    def test_skill_contents_validation(self) -> None:
        ...
        example = skill_domain.WorkedExample(
            state_domain.SubtitledHtml('2', '<p>Example Question 1</p>'), 'explanation'
        )  # type: ignore[arg-type]
        self.skill.skill_contents.worked_examples = [example]
        self._assert_validation_error(
    ...
    def test_update_worked_examples(self) -> None:
        ...
        worked_examples_dict_list: List[skill_domain.WorkedExampleDict] = [
            {'question': question_1, 'explanation': explanation_1},
            {'question': question_2, 'explanation': explanation_2},
        ]
    ...
    def test_generate_skill_misconception_id(self) -> None:
        ...
        self.assertEqual(
            self.skill.generate_skill_misconception_id(0), '%s-%d' % (self.skill.id, 0)
        )
        self.assertEqual(
            self.skill.generate_skill_misconception_id(1), '%s-%d' % (self.skill.id, 1)
        )
...
class SkillSummaryTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.skill_summary = skill_domain.SkillSummary(
            'skill_id', 'description', 'en', 1, 1, 1, current_time, current_time
        )
    ...
    def test_validation_fails_with_negative_worked_examples_count(self) -> None:
        ...
        with self.assertRaisesRegex(
            utils.ValidationError,
            ('Expected worked_examples_count to be non-negative, ' 'received \'-1\''),
        ):
            self.skill_summary.validate()
        ...
        self.assertEqual(
            self.categorized_skills.to_dict(),
            {
                'Topic Name': {
                    'uncategorized': [],
                    'Subtopic Title 1': [
                        {'skill_id': 'skill_2', 'skill_description': 'Description 2'}
                    ],
                    'Subtopic Title 2': [
                        {'skill_id': 'skill_3', 'skill_description': 'Description 3'}
                    ],
                }
            },
        )
