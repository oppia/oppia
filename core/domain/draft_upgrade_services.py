...
AllowedDraftChangeListTypes = Union[state_domain.SubtitledHtmlDict,
                                    ...
                                    List[state_domain.AnswerGroupDict], str]
...
def try_upgrading_draft_to_exp_version(
    draft_change_list: List[exp_domain.ExplorationChange], current_draft_version: int,
    to_exp_version: int, exp_id: str
    ...
    commits_list = (
        exp_models.ExplorationCommitLogEntryModel.get_multi(exp_id, exp_versions)
    )
    upgrade_times = 0
        ...
        for i, change in enumerate(draft_change_list):
            if (change.cmd == exp_domain.CMD_EDIT_STATE_PROPERTY and
                ...
                content_ids_to_audio_translations_cmd = cast(
                    exp_domain.
                    EditExpStatePropertyContentIdsToAudioTranslationsDeprecatedCmd,  # pylint: disable=line-too-long
                    change
                )
