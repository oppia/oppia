...
def get_random_int(upper_bound: int) -> int:
    ...
    """
    assert upper_bound >= 0 and isinstance(upper_bound,
                                           int), ('Only positive integers allowed')
...
def get_hex_color_for_category(category: str) -> str:
    ...
    color: str = (
        ...
        if category in constants.CATEGORIES_TO_COLORS else constants.DEFAULT_COLOR
    )
    return color
...
def get_thumbnail_icon_url_for_category(category: str) -> str:
    ...
    """
    icon_name = (
        category if category in constants.CATEGORIES_TO_COLORS else
...
def partition(
    ...
    return ((i for i, predicate_is_true in true_part if predicate_is_true),
