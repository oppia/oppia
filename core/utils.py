...
def get_time_in_millisecs(datetime_obj: datetime.datetime) -> float:
    ...
    return msecs + (datetime_obj.microsecond // 1000.0) # type: ignore[no-any-return, no-untyped-call]
...
def get_human_readable_time_string(time_msec: float) -> str:
    ...
    return time.strftime(
        '%B %d %H:%M:%S', time.gmtime((time_msec // 1000.0))) # type: ignore[arg-type, no-untyped-call]
