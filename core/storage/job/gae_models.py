...
# Allowed transitions: processing --> completed --> archived
...
# Keys for query_parameters dictionary.
# Values corresponding to key is value specified by query submitter.
# A query can have any combination of following parameters.
# Valid parameters names.
# pylint: disable=C0103
QUERY_PARAMETERS = utils.create_enum(
...
class QueryModel(base_models.BaseModel):
    ...
    """
    # parameters for query given by submitter.
    query_parameters = ndb.JsonProperty()
    ...
    @classmethod
    def create(cls, submitter_id, query_parameters_domain):
        query_id = cls.get_new_id('')
        cls(id=query_id,
            query_parameters=query_parameters_domain.to_dict(),
            submitter_id=submitter_id,
            query_status=QUERY_STATUS_PROCESSING,
            user_ids=[]).put()
        return query_id
