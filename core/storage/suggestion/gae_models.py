...
from typing import (
    Dict, Final, List, Literal, Mapping, Optional, Sequence, Tuple, TypedDict, Union
)
...
REVIEW_OUTCOME_CHOICES: Final = [
    REVIEW_OUTCOME_ACCEPTED, REVIEW_OUTCOME_ACCEPTED_WITH_EDITS, REVIEW_OUTCOME_REJECTED
]
...
SCORE_TYPE_CHOICES: Final = [
    SCORE_TYPE_CONTENT, SCORE_TYPE_TRANSLATION, SCORE_TYPE_QUESTION
]
...
class GeneralSuggestionModel(base_models.BaseModel):
    ...
    target_type = datastore_services.StringProperty(
        required=True, indexed=True, choices=feconf.SUGGESTION_TARGET_TYPE_CHOICES
    )
    ...
    @classmethod
    def create(
        cls, suggestion_type: str, target_type: str, target_id: str,
        ...
        score_category: str, thread_id: str, language_code: Optional[str]
    ) -> None:
    ...
    def get_translation_suggestions_in_review_with_exp_id(
        ...
        return cls.get_all().filter(
            datastore_services.all_of(
                cls.status == STATUS_IN_REVIEW, cls.language_code == language_code,
    ...
    @classmethod
    def get_translation_suggestion_ids_with_exp_ids(cls,
                                                    exp_ids: List[str]) -> List[str]:
    ...
    def get_all_stale_suggestion_ids(cls) -> List[str]:
        ...
        suggestion_models: Sequence[GeneralSuggestionModel] = (
            cls.get_all().filter(cls.status == STATUS_IN_REVIEW
    ...
    def get_suggestions_waiting_too_long_for_review(
        ...
        threshold_time = (
            datetime.datetime.utcnow() -
            datetime.timedelta(days=SUGGESTION_REVIEW_WAIT_TIME_THRESHOLD_IN_DAYS)
        )
        return cls.get_all().filter(
            datastore_services.all_of(
                cls.status == STATUS_IN_REVIEW, cls.last_updated < threshold_time,
                cls.suggestion_type.IN(feconf.CONTRIBUTOR_DASHBOARD_SUGGESTION_TYPES)
            )
        ).order(cls.last_updated).fetch(MAX_NUMBER_OF_SUGGESTIONS_TO_EMAIL_ADMIN)
    ...
    def get_new_suggestions_waiting_for_review(
        ...
        return (
            ...
            ).order(-cls.created_on).fetch(MAX_NUMBER_OF_SUGGESTIONS_TO_EMAIL_ADMIN)
        )

    @classmethod
    def get_translation_suggestions_submitted_within_given_dates(
        cls, from_date: datetime.datetime, to_date: datetime.datetime, user_id: str,
        language_code: str
    ) -> Sequence[GeneralSuggestionModel]:
        ...
        return cls.get_all().filter(
            datastore_services.all_of(
                cls.created_on <= to_date, cls.created_on >= from_date,
                cls.author_id == user_id,
                cls.suggestion_type == feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT,
                cls.language_code == language_code, cls.status == STATUS_ACCEPTED
            )
    ...
    def get_in_review_suggestions_in_score_categories(
        ...
        return cls.get_all().filter(
            datastore_services.all_of(
                cls.status == STATUS_IN_REVIEW, cls.score_category.IN(score_categories),
                cls.author_id != user_id
            )
        ).fetch(feconf.DEFAULT_SUGGESTION_QUERY_LIMIT)
    ...
    def get_in_review_translation_suggestions(
        ...
        return cls.get_all().filter(
            datastore_services.all_of(
                cls.status == STATUS_IN_REVIEW,
                cls.suggestion_type == feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT,
                cls.author_id != user_id, cls.language_code.IN(language_codes)
            )
        ).fetch(feconf.DEFAULT_SUGGESTION_QUERY_LIMIT)

    @classmethod
    def get_in_review_translation_suggestions_by_offset(
        cls, limit: Optional[int], offset: int, user_id: str, sort_key: Optional[str],
    ...
    def get_in_review_translation_suggestion_target_ids(
        ...
        """
        fetched_models: Sequence[GeneralSuggestionModel] = cls.get_all().filter(
            datastore_services.all_of(
                cls.status == STATUS_IN_REVIEW,
                cls.suggestion_type == feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT,
                cls.author_id != user_id, cls.language_code.IN(language_codes)
    ...
    def get_reviewable_translation_suggestions(
        ...
        suggestion_query = cls.get_all().filter(
            datastore_services.all_of(
                cls.status == STATUS_IN_REVIEW,
                cls.suggestion_type == feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT,
                cls.language_code == language_code, cls.target_id == exp_id
            )
        ).order(-cls.created_on)
    ...
    def get_in_review_translation_suggestions_with_exp_ids_by_offset(
        cls, limit: Optional[int], offset: int, user_id: str, sort_key: Optional[str],
        language_codes: List[str], exp_ids: List[str]
    ) -> Tuple[Sequence[GeneralSuggestionModel], int]:
    ...
    def get_in_review_translation_suggestions_by_exp_ids(
        ...
        return cls.get_all().filter(
            datastore_services.all_of(
                cls.status == STATUS_IN_REVIEW,
                cls.suggestion_type == feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT,
                cls.target_id.IN(exp_ids), cls.language_code == language_code
            )
        ).fetch(feconf.DEFAULT_SUGGESTION_QUERY_LIMIT)
    ...
    def get_in_review_question_suggestions_by_offset(
        ...
        if sort_key == constants.SUGGESTIONS_SORT_KEY_DATE:
            ...
            while len(sorted_results) < limit:
                suggestion_models: Sequence[GeneralSuggestionModel] = (
                    suggestion_query.fetch(num_suggestions_per_fetch, offset=offset)
                )
                if not suggestion_models:
                    break
                for suggestion_model in suggestion_models:
...
class TranslationContributionStatsModel(base_models.BaseModel):
    ...
    def create(
        ...
        rejected_translation_word_count: int, contribution_dates: List[datetime.date]
    ) -> str:
    ...
    @classmethod
    def get_all_by_user_id(cls,
                           user_id: str) -> Sequence[TranslationContributionStatsModel]:
        ...
        """
        return cls.get_all().filter(cls.contributor_user_id == user_id
                                   ).fetch(feconf.DEFAULT_SUGGESTION_QUERY_LIMIT)
    ...
    def has_reference_to_user_id(cls, user_id: str) -> bool:
        ...
        Returns:
            bool. Whether any models refer to the given user ID.
        """
        return cls.query(cls.contributor_user_id == user_id
                        ).get(keys_only=True) is not None
...
class TranslationReviewStatsModel(base_models.BaseModel):
    ...
    def create(
        ...
        accepted_translation_word_count: int, first_contribution_date: datetime.date,
    ...
    def get_all_by_user_id(cls, user_id: str) -> Sequence[TranslationReviewStatsModel]:
        ...
        """
        return cls.get_all().filter(cls.reviewer_user_id == user_id
                                   ).fetch(feconf.DEFAULT_SUGGESTION_QUERY_LIMIT)
    ...
    def has_reference_to_user_id(cls, user_id: str) -> bool:
        ...
        Returns:
            bool. Whether any models refer to the given user ID.
        """
        return cls.query(cls.reviewer_user_id == user_id
                        ).get(keys_only=True) is not None
...
class QuestionContributionStatsModel(base_models.BaseModel):
    ...
    @classmethod
    def create(
        cls, contributor_user_id: str, topic_id: str, submitted_questions_count: int,
    ...
    @classmethod
    def get_all_by_user_id(cls,
                           user_id: str) -> Sequence[QuestionContributionStatsModel]:
...
class QuestionReviewStatsModel(base_models.BaseModel):
    ...
    @classmethod
    def create(
        cls, reviewer_user_id: str, topic_id: str, reviewed_questions_count: int,
        accepted_questions_count: int,
...
class TranslationSubmitterTotalContributionStatsModel(base_models.BaseModel):
    ...
    @classmethod
    def fetch_page(
        cls, page_size: int, offset: int, language_code: str,
        ...
        next_result_model: Sequence[TranslationSubmitterTotalContributionStatsModel] = (
    ...
    def get_all_by_user_id(
        ...
        Returns:
            ...
            TranslationSubmitterTotalContributionStatsModel.
        """
        return cls.get_all().filter(cls.contributor_id == user_id
                                   ).fetch(feconf.DEFAULT_SUGGESTION_QUERY_LIMIT)
...
class TranslationReviewerTotalContributionStatsModel(base_models.BaseModel):
    ...
    def fetch_page(
        ...
        # Check whether we have more results.
        next_result_model: Sequence[TranslationReviewerTotalContributionStatsModel] = (
...
class QuestionSubmitterTotalContributionStatsModel(base_models.BaseModel):
    ...
    @classmethod
    def create(
        cls, contributor_id: str, topic_ids_with_question_submissions: List[str],
        recent_review_outcomes: List[str], recent_performance: int,
    ...
    @classmethod
    def fetch_page(
        cls, page_size: int, offset: int, sort_by: Optional[SortChoices.value],
        topic_ids: Optional[List[str]], max_days_since_last_activity: Optional[int]
        ...
        # Check whether we have more results.
        next_result_model: Sequence[QuestionSubmitterTotalContributionStatsModel] = (
            sort_query.fetch(offset=next_offset)
        )
        more: bool = len(next_result_model) != 0
...
class QuestionReviewerTotalContributionStatsModel(base_models.BaseModel):
    ...
    @classmethod
    def create(
        cls, contributor_id: str, topic_ids_with_question_reviews: List[str],
    ...
    def fetch_page(
        ...
        if max_days_since_last_activity is not None:
            ...
            next_offset = offset
            while len(sorted_results) < page_size:
                result_models: Sequence[QuestionReviewerTotalContributionStatsModel] = (
                    sort_query.fetch(NUM_MODELS_PER_FETCH, offset=next_offset)
                )
                if not result_models:
                    break
