...
DEFAULT_SUGGESTION_ACCEPT_MESSAGE: Final = (
    'Automatically accepting suggestion after' ' %d days' % THRESHOLD_DAYS_BEFORE_ACCEPT
)
...
class GeneralSuggestionModel(base_models.BaseModel):
    ...
    def create(
        ...
        if cls.get_by_id(instance_id):
            raise Exception(
                'There is already a suggestion with the given' ' id: %s' % instance_id
            )
    ...
    def get_reviewable_translation_suggestions(
        ...
        sorted_results: List[GeneralSuggestionModel] = []
        offset = 0
        suggestion_models: Sequence[GeneralSuggestionModel] = suggestion_query.fetch(
            offset=offset
        )
        for suggestion_model in suggestion_models:
            offset += 1
...
class CommunityContributionStatsModel(base_models.BaseModel):
    ...
    translation_suggestion_counts_by_lang_code = datastore_services.JsonProperty(
        required=True
    )
...
class TranslationReviewStatsModel(base_models.BaseModel):
    ...
    def has_reference_to_user_id(cls, user_id: str) -> bool:
        ...
        """
        return (
            cls.query(cls.reviewer_user_id == user_id).get(keys_only=True) is not None
        )
...
class QuestionReviewStatsModel(base_models.BaseModel):
    ...
    accepted_questions_with_reviewer_edits_count = datastore_services.IntegerProperty(
        required=True, indexed=True
    )
