...
from typing import Dict, Final, List, Literal, Mapping, Optional, Sequence, Tuple, TypedDict, Union
...
REVIEW_OUTCOME_CHOICES: Final = [REVIEW_OUTCOME_ACCEPTED,
    REVIEW_OUTCOME_ACCEPTED_WITH_EDITS, REVIEW_OUTCOME_REJECTED]
...
SUGGESTION_MINIMUM_ROLE_FOR_REVIEW: Final = {feconf.
    SUGGESTION_TYPE_EDIT_STATE_CONTENT: feconf.ROLE_ID_FULL_USER}
SCORE_TYPE_CONTENT: Final = 'content'
SCORE_TYPE_TRANSLATION: Final = 'translation'
SCORE_TYPE_QUESTION: Final = 'question'
SCORE_TYPE_CHOICES: Final = [SCORE_TYPE_CONTENT, SCORE_TYPE_TRANSLATION,
    SCORE_TYPE_QUESTION]
...
DEFAULT_SUGGESTION_ACCEPT_MESSAGE: Final = (
    'Automatically accepting suggestion after %d days' %
    THRESHOLD_DAYS_BEFORE_ACCEPT)
...
INVALID_STORY_REJECT_TRANSLATION_SUGGESTIONS_MSG: Final = (
    'This text snippet has been removed from the story, and no longer needs translation. Sorry about that!'
    )
...
class SortChoices(enum.Enum):
    ...
    SORT_KEY_INCREASING_REVIEWED_TRANSLATIONS = (
        'IncreasingReviewedTranslations')
    SORT_KEY_DECREASING_REVIEWED_TRANSLATIONS = (
...
class GeneralSuggestionModel(base_models.BaseModel):
    ...
    suggestion_type = datastore_services.StringProperty(required=True,
        indexed=True, choices=feconf.SUGGESTION_TYPE_CHOICES)
    target_type = datastore_services.StringProperty(required=True, indexed=
        True, choices=feconf.SUGGESTION_TARGET_TYPE_CHOICES)
    target_id = datastore_services.StringProperty(required=True, indexed=True)
    target_version_at_submission = datastore_services.IntegerProperty(required
        =True, indexed=True)
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{
            ...
            'target_type': base_models.EXPORT_POLICY.EXPORTED, 'target_id':
            base_models.EXPORT_POLICY.EXPORTED,
            'target_version_at_submission': base_models.EXPORT_POLICY.
            EXPORTED, 'status': base_models.EXPORT_POLICY.EXPORTED,
    ...
    def has_reference_to_user_id(cls, user_id: str) ->bool:
        ...
        """
        return cls.query(datastore_services.any_of(cls.author_id == user_id,
            cls.final_reviewer_id == user_id)).get(keys_only=True) is not None
    ...
    def create(cls, suggestion_type: str, target_type: str, target_id: str,
        ...
        final_reviewer_id: Optional[str], change_cmd: Mapping[str,
        change_domain.AcceptableChangeDictTypes], score_category: str,
        thread_id: str, language_code: Optional[str]) ->None:
        ...
        if cls.get_by_id(instance_id):
            raise Exception(
                'There is already a suggestion with the given id: %s' %
                instance_id)
        cls(id=instance_id, suggestion_type=suggestion_type, target_type=
            target_type, target_id=target_id, target_version_at_submission=
            target_version_at_submission, status=status, author_id=
            author_id, final_reviewer_id=final_reviewer_id, change_cmd=
            change_cmd, score_category=score_category, language_code=
            language_code).put()

    @classmethod
    def query_suggestions(cls, query_fields_and_values: List[Tuple[str, str]]
    ...
    @classmethod
    def get_translation_suggestions_in_review_with_exp_id(cls, exp_id: str,
        ...
        """
        return cls.get_all().filter(datastore_services.all_of(cls.status ==
            STATUS_IN_REVIEW, cls.language_code == language_code, cls.
            suggestion_type == feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT, 
            cls.target_id == exp_id)).fetch(feconf.
            DEFAULT_SUGGESTION_QUERY_LIMIT)

    @classmethod
    def get_translation_suggestion_ids_with_exp_ids(cls, exp_ids: List[str]
        ) ->List[str]:
        ...
        """
        query = cls.get_all().filter(datastore_services.all_of(cls.
            suggestion_type == feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT,
            cls.target_id.IN(exp_ids)))
        suggestion_models: List[GeneralSuggestionModel] = []
        offset, more = 0, True
        while more:
            results: Sequence[GeneralSuggestionModel] = query.fetch(feconf.
                DEFAULT_SUGGESTION_QUERY_LIMIT, offset=offset)
            if len(results):
    ...
    @classmethod
    def get_suggestions_waiting_too_long_for_review(cls) ->Sequence[
    ...
    @classmethod
    def get_new_suggestions_waiting_for_review(cls) ->Sequence[
        GeneralSuggestionModel]:
        ...
        threshold_datetime = datetime.datetime.utcfromtimestamp(
            current_time_millisecs / 1000.0) - datetime.timedelta(days=
            SUGGESTION_REVIEW_WAIT_TIME_NOTIFICATION)
        return cls.get_all().filter(datastore_services.all_of(cls.status ==
            STATUS_IN_REVIEW, cls.suggestion_type == feconf.
            SUGGESTION_TYPE_TRANSLATE_CONTENT, cls.created_on >
            threshold_datetime)).order(-cls.created_on).fetch(
            MAX_NUMBER_OF_SUGGESTIONS_TO_EMAIL_ADMIN)

    @classmethod
    def get_translation_suggestions_submitted_within_given_dates(cls,
        from_date: datetime.datetime, to_date: datetime.datetime, user_id:
        str, language_code: str) ->Sequence[GeneralSuggestionModel]:
        ...
        """
        return cls.get_all().filter(datastore_services.all_of(cls.
            created_on <= to_date, cls.created_on >= from_date, cls.
            author_id == user_id, cls.suggestion_type == feconf.
            SUGGESTION_TYPE_TRANSLATE_CONTENT, cls.language_code ==
            language_code, cls.status == STATUS_ACCEPTED)).order(cls.created_on
            ).fetch()

    @classmethod
    def get_question_suggestions_submitted_within_given_dates(cls,
        ...
        return cls.get_all().filter(datastore_services.all_of(cls.
            created_on <= to_date, cls.created_on >= from_date, cls.
            author_id == user_id, cls.suggestion_type == feconf.
            SUGGESTION_TYPE_ADD_QUESTION, cls.status == STATUS_ACCEPTED)
            ).order(cls.created_on).fetch()

    @classmethod
    def get_in_review_suggestions_in_score_categories(cls, score_categories:
        List[str], user_id: str) ->Sequence[GeneralSuggestionModel]:
        ...
        return cls.get_all().filter(datastore_services.all_of(cls.status ==
            STATUS_IN_REVIEW, cls.score_category.IN(score_categories), cls.
            author_id != user_id)).fetch(feconf.DEFAULT_SUGGESTION_QUERY_LIMIT)

    @classmethod
    def get_in_review_translation_suggestions(cls, user_id: str,
        ...
        return cls.get_all().filter(datastore_services.all_of(cls.status ==
            STATUS_IN_REVIEW, cls.suggestion_type == feconf.
            SUGGESTION_TYPE_TRANSLATE_CONTENT, cls.author_id != user_id,
            cls.language_code.IN(language_codes))).fetch(feconf.
            DEFAULT_SUGGESTION_QUERY_LIMIT)

    @classmethod
    def get_in_review_translation_suggestions_by_offset(cls, limit:
        Optional[int], offset: int, user_id: str, sort_key: Optional[str],
        language_codes: List[str]) ->Tuple[Sequence[GeneralSuggestionModel],
        int]:
        ...
        if sort_key == constants.SUGGESTIONS_SORT_KEY_DATE:
            suggestion_query = cls.get_all().filter(datastore_services.
                all_of(cls.status == STATUS_IN_REVIEW, cls.suggestion_type ==
                feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT, cls.language_code
                .IN(language_codes))).order(-cls.created_on)
        ...
        suggestion_query = cls.get_all().filter(datastore_services.all_of(
            cls.status == STATUS_IN_REVIEW, cls.suggestion_type == feconf.
            SUGGESTION_TYPE_TRANSLATE_CONTENT, cls.author_id != user_id,
            cls.language_code.IN(language_codes)))
        results: Sequence[GeneralSuggestionModel] = suggestion_query.fetch(
            limit, offset=offset
            ) if limit is not None else suggestion_query.fetch(offset=offset)
        next_offset = offset + len(results)
        return results, next_offset

    @classmethod
    def get_in_review_translation_suggestion_target_ids(cls, user_id: str,
        ...
        """
        fetched_models: Sequence[GeneralSuggestionModel] = cls.get_all(
            ).filter(datastore_services.all_of(cls.status ==
            STATUS_IN_REVIEW, cls.suggestion_type == feconf.
            SUGGESTION_TYPE_TRANSLATE_CONTENT, cls.author_id != user_id,
            cls.language_code.IN(language_codes))).fetch(projection=[cls.
            target_id])
        return list({model.target_id for model in fetched_models})

    @classmethod
    def get_reviewable_translation_suggestions(cls, user_id: str,
        language_code: str, exp_id: str) ->Tuple[Sequence[
        GeneralSuggestionModel], int]:
        ...
        suggestion_query = cls.get_all().filter(datastore_services.all_of(
            cls.status == STATUS_IN_REVIEW, cls.suggestion_type == feconf.
            SUGGESTION_TYPE_TRANSLATE_CONTENT, cls.language_code ==
            language_code, cls.target_id == exp_id)).order(-cls.created_on)
        sorted_results: List[GeneralSuggestionModel] = []
        offset = 0
        suggestion_models: Sequence[GeneralSuggestionModel
            ] = suggestion_query.fetch(offset=offset)
    ...
    @classmethod
    def get_in_review_translation_suggestions_with_exp_ids_by_offset(cls,
        limit: Optional[int], offset: int, user_id: str, sort_key: Optional
        [str], language_codes: List[str], exp_ids: List[str]) ->Tuple[
        Sequence[GeneralSuggestionModel], int]:
        ...
        if sort_key == constants.SUGGESTIONS_SORT_KEY_DATE:
            suggestion_query = cls.get_all().filter(datastore_services.
                all_of(cls.status == STATUS_IN_REVIEW, cls.suggestion_type ==
                feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT, cls.language_code
                .IN(language_codes), cls.target_id.IN(exp_ids))).order(-cls
                .created_on)
            sorted_results: List[GeneralSuggestionModel] = []
            if limit is None:
                suggestion_models: Sequence[GeneralSuggestionModel
        ...
        suggestion_query = cls.get_all().filter(datastore_services.all_of(
            cls.status == STATUS_IN_REVIEW, cls.suggestion_type == feconf.
            SUGGESTION_TYPE_TRANSLATE_CONTENT, cls.author_id != user_id,
            cls.language_code.IN(language_codes), cls.target_id.IN(exp_ids)))
        results: Sequence[GeneralSuggestionModel] = suggestion_query.fetch(
            limit, offset=offset
            ) if limit is not None else suggestion_query.fetch(offset=offset)
        next_offset = offset + len(results)
    ...
    @classmethod
    def get_in_review_translation_suggestions_by_exp_id(cls, exp_id: str
        ) ->Sequence[GeneralSuggestionModel]:
        ...
        return cls.get_all().filter(datastore_services.all_of(cls.status ==
            STATUS_IN_REVIEW, cls.suggestion_type == feconf.
            SUGGESTION_TYPE_TRANSLATE_CONTENT, cls.target_id == exp_id)).fetch(
            feconf.DEFAULT_SUGGESTION_QUERY_LIMIT)

    @classmethod
    def get_in_review_translation_suggestions_by_exp_ids(cls, exp_ids: List
        [str], language_code: str) ->Sequence[GeneralSuggestionModel]:
        """Gets all in-review translation suggestions matching the supplied
        ...
        return cls.get_all().filter(datastore_services.all_of(cls.status ==
            STATUS_IN_REVIEW, cls.suggestion_type == feconf.
            SUGGESTION_TYPE_TRANSLATE_CONTENT, cls.target_id.IN(exp_ids), 
            cls.language_code == language_code)).fetch(feconf.
            DEFAULT_SUGGESTION_QUERY_LIMIT)

    @classmethod
    def get_in_review_question_suggestions_by_offset(cls, limit: int,
        offset: int, user_id: str, sort_key: Optional[str], skill_ids:
        Optional[List[str]]) ->Tuple[Sequence[GeneralSuggestionModel], int]:
        ...
        """
        filters = [cls.status == STATUS_IN_REVIEW, cls.suggestion_type ==
        ...
        if sort_key == constants.SUGGESTIONS_SORT_KEY_DATE:
            suggestion_query = cls.get_all().filter(datastore_services.
                all_of(*filters)).order(-cls.created_on)
            sorted_results: List[GeneralSuggestionModel] = []
            num_suggestions_per_fetch = 1000
            while len(sorted_results) < limit:
                suggestion_models: Sequence[GeneralSuggestionModel
                    ] = suggestion_query.fetch(num_suggestions_per_fetch,
                    offset=offset)
                if not suggestion_models:
                    break
                for suggestion_model in suggestion_models:
                    ...
                    if suggestion_model.author_id == user_id:
                        continue
                    if (skill_ids is not None and suggestion_model.
                        target_id not in skill_ids):
                        continue
                    sorted_results.append(suggestion_model)
                    if len(sorted_results) == limit:
                        break
    ...
    @classmethod
    def get_question_suggestions_waiting_longest_for_review(cls) ->Sequence[
        GeneralSuggestionModel]:
        ...
        return cls.get_all().filter(datastore_services.all_of(cls.status ==
            STATUS_IN_REVIEW, cls.suggestion_type == feconf.
            SUGGESTION_TYPE_ADD_QUESTION)).order(cls.last_updated).fetch(
            MAX_QUESTION_SUGGESTIONS_TO_FETCH_FOR_REVIEWER_EMAILS)

    @classmethod
    def get_translation_suggestions_waiting_longest_for_review(cls,
        language_code: str) ->Sequence[GeneralSuggestionModel]:
    ...
    @classmethod
    def get_user_created_suggestions_of_suggestion_type(cls,
        suggestion_type: str, user_id: str) ->Sequence[GeneralSuggestionModel]:
        ...
        """
        return cls.get_all().filter(datastore_services.all_of(cls.
            suggestion_type == suggestion_type, cls.author_id == user_id)
            ).order(-cls.created_on).fetch(feconf.
            DEFAULT_SUGGESTION_QUERY_LIMIT)

    @classmethod
    def get_user_created_suggestions_by_offset(cls, limit: int, offset: int,
        suggestion_type: str, user_id: str, sort_key: Optional[str]) ->Tuple[
        Sequence[GeneralSuggestionModel], int]:
    ...
    @classmethod
    def export_data(cls, user_id: str) ->Dict[str,
        GeneralSuggestionExportDataDict]:
        ...
        suggestion_models: Sequence[GeneralSuggestionModel] = cls.get_all(
            ).filter(cls.author_id == user_id).fetch()
        for suggestion_model in suggestion_models:
            user_data[suggestion_model.id] = {'suggestion_type':
                suggestion_model.suggestion_type, 'target_type':
                suggestion_model.target_type, 'target_id': suggestion_model
                .target_id, 'target_version_at_submission':
                suggestion_model.target_version_at_submission, 'status':
                suggestion_model.status, 'change_cmd': suggestion_model.
                change_cmd, 'language_code': suggestion_model.language_code,
...
class CommunityContributionStatsModel(base_models.BaseModel):
    ...
    translation_reviewer_counts_by_lang_code = datastore_services.JsonProperty(
        required=True)
    translation_suggestion_counts_by_lang_code = (datastore_services.
        JsonProperty(required=True))
    question_reviewer_count = datastore_services.IntegerProperty(required=True)
    question_suggestion_count = datastore_services.IntegerProperty(required
        =True)

    @classmethod
    def get(cls) ->CommunityContributionStatsModel:
        ...
        if community_contribution_stats_model is None:
            community_contribution_stats_model = cls(id=
                COMMUNITY_CONTRIBUTION_STATS_MODEL_ID,
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        """
        return dict(super(cls, cls).get_export_policy(), **{
            'translation_reviewer_counts_by_lang_code': base_models.
            EXPORT_POLICY.NOT_APPLICABLE,
            'translation_suggestion_counts_by_lang_code': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'question_reviewer_count':
            base_models.EXPORT_POLICY.NOT_APPLICABLE,
            'question_suggestion_count': base_models.EXPORT_POLICY.
            NOT_APPLICABLE})
...
class TranslationContributionStatsModel(base_models.BaseModel):
    ...
    submitted_translations_count = datastore_services.IntegerProperty(required
        =True, indexed=True)
    submitted_translation_word_count = datastore_services.IntegerProperty(
        required=True, indexed=True)
    accepted_translations_count = datastore_services.IntegerProperty(required
        =True, indexed=True)
    accepted_translations_without_reviewer_edits_count = (datastore_services
        .IntegerProperty(required=True, indexed=True))
    accepted_translation_word_count = datastore_services.IntegerProperty(
        required=True, indexed=True)
    rejected_translations_count = datastore_services.IntegerProperty(required
        =True, indexed=True)
    ...
    @classmethod
    def create(cls, language_code: str, contributor_user_id: str, topic_id:
        str, submitted_translations_count: int,
        submitted_translation_word_count: int, accepted_translations_count:
        int, accepted_translations_without_reviewer_edits_count: int,
        accepted_translation_word_count: int, rejected_translations_count:
        int, rejected_translation_word_count: int, contribution_dates: List
        [datetime.date]) ->str:
        """Creates a new TranslationContributionStatsModel instance and returns
        its ID.
        """
        entity_id = cls.construct_id(language_code, contributor_user_id,
            topic_id)
        entity = cls(id=entity_id, language_code=language_code,
            contributor_user_id=contributor_user_id, topic_id=topic_id,
            submitted_translations_count=submitted_translations_count,
            submitted_translation_word_count=
            submitted_translation_word_count, accepted_translations_count=
    ...
    @staticmethod
    def construct_id(language_code: str, contributor_user_id: str, topic_id:
        str) ->str:
    ...
    @classmethod
    def get(cls, language_code: str, contributor_user_id: str, topic_id: str
    ...
    @classmethod
    def get_all_by_user_id(cls, user_id: str) ->Sequence[
        ...
        """
        return cls.get_all().filter(cls.contributor_user_id == user_id).fetch(
            feconf.DEFAULT_SUGGESTION_QUERY_LIMIT)
    ...
    def has_reference_to_user_id(cls, user_id: str) ->bool:
        ...
        Returns:
            bool. Whether any models refer to the given user ID.
        """
        return cls.query(cls.contributor_user_id == user_id).get(keys_only=True
            ) is not None
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        """Model contains data to export corresponding to a user."""
        return dict(super(cls, cls).get_export_policy(), **{'language_code':
            base_models.EXPORT_POLICY.EXPORTED, 'contributor_user_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'topic_id':
            base_models.EXPORT_POLICY.EXPORTED,
            'submitted_translations_count': base_models.EXPORT_POLICY.
            EXPORTED, 'submitted_translation_word_count': base_models.
            EXPORT_POLICY.EXPORTED, 'accepted_translations_count':
            base_models.EXPORT_POLICY.EXPORTED,
            'accepted_translations_without_reviewer_edits_count':
            base_models.EXPORT_POLICY.EXPORTED,
            'accepted_translation_word_count': base_models.EXPORT_POLICY.
            EXPORTED, 'rejected_translations_count': base_models.
            EXPORT_POLICY.EXPORTED, 'rejected_translation_word_count':
            base_models.EXPORT_POLICY.EXPORTED, 'contribution_dates':
            base_models.EXPORT_POLICY.EXPORTED})
    ...
    def apply_deletion_policy(cls, user_id: str) ->None:
        ...
        """
        datastore_services.delete_multi(cls.query(cls.contributor_user_id ==
            user_id).fetch(keys_only=True))

    @classmethod
    def export_data(cls, user_id: str) ->Dict[str, Dict[str, Union[str, int,
        List[str]]]]:
        ...
        for model in stats_models:
            ...
            user_data[id_without_user_id] = {'language_code': model.
                language_code, 'topic_id': model.topic_id,
                'submitted_translations_count': model.
                submitted_translations_count,
                'submitted_translation_word_count': model.
                submitted_translation_word_count,
                'accepted_translations_count': model.
                accepted_translations_count,
                'accepted_translations_without_reviewer_edits_count': model
                .accepted_translations_without_reviewer_edits_count,
                'accepted_translation_word_count': model.
                accepted_translation_word_count,
                'rejected_translations_count': model.
                rejected_translations_count,
                'rejected_translation_word_count': model.
...
class TranslationReviewStatsModel(base_models.BaseModel):
    ...
    language_code = datastore_services.StringProperty(required=True,
        indexed=True)
    reviewer_user_id = datastore_services.StringProperty(required=True,
        indexed=True)
    topic_id = datastore_services.StringProperty(required=True, indexed=True)
    reviewed_translations_count = datastore_services.IntegerProperty(required
        =True, indexed=True)
    reviewed_translation_word_count = datastore_services.IntegerProperty(
        required=True, indexed=True)
    accepted_translations_count = datastore_services.IntegerProperty(required
        =True, indexed=True)
    accepted_translations_with_reviewer_edits_count = (datastore_services.
        IntegerProperty(required=True, indexed=True))
    accepted_translation_word_count = datastore_services.IntegerProperty(
        required=True, indexed=True)
    ...
    @classmethod
    def create(cls, language_code: str, reviewer_user_id: str, topic_id:
        str, reviewed_translations_count: int,
        reviewed_translation_word_count: int, accepted_translations_count:
        int, accepted_translations_with_reviewer_edits_count: int,
        accepted_translation_word_count: int, first_contribution_date:
        datetime.date, last_contribution_date: datetime.date) ->str:
        ...
        entity = cls(id=entity_id, language_code=language_code,
            reviewer_user_id=reviewer_user_id, topic_id=topic_id,
    ...
    @staticmethod
    def construct_id(language_code: str, reviewer_user_id: str, topic_id: str
        ) ->str:
    ...
    @classmethod
    def get(cls, language_code: str, reviewer_user_id: str, topic_id: str
    ...
    def get_all_by_user_id(cls, user_id: str) ->Sequence[
        ...
        Returns:
            ...
            TranslationReviewStatsModel.
        """
        return cls.get_all().filter(cls.reviewer_user_id == user_id).fetch(
            feconf.DEFAULT_SUGGESTION_QUERY_LIMIT)
    ...
    def has_reference_to_user_id(cls, user_id: str) ->bool:
        ...
        Returns:
            bool. Whether any models refer to the given user ID.
        """
        return cls.query(cls.reviewer_user_id == user_id).get(keys_only=True
            ) is not None
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        """Model contains data to export corresponding to a user."""
        return dict(super(cls, cls).get_export_policy(), **{'language_code':
            base_models.EXPORT_POLICY.EXPORTED, 'reviewer_user_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'topic_id':
            base_models.EXPORT_POLICY.EXPORTED,
            'reviewed_translations_count': base_models.EXPORT_POLICY.
            EXPORTED, 'reviewed_translation_word_count': base_models.
            EXPORT_POLICY.EXPORTED, 'accepted_translations_count':
            base_models.EXPORT_POLICY.EXPORTED,
            'accepted_translations_with_reviewer_edits_count': base_models.
            EXPORT_POLICY.EXPORTED, 'accepted_translation_word_count':
            base_models.EXPORT_POLICY.EXPORTED, 'first_contribution_date':
            base_models.EXPORT_POLICY.EXPORTED, 'last_contribution_date':
            base_models.EXPORT_POLICY.EXPORTED})
    ...
    def apply_deletion_policy(cls, user_id: str) ->None:
        ...
        Args:
            user_id: str. The ID of the user whose data should be deleted.
        """
        datastore_services.delete_multi(cls.query(cls.reviewer_user_id ==
            user_id).fetch(keys_only=True))
    ...
    def export_data(cls, user_id: str) ->Dict[str, Dict[str, Union[str, int,
        ...
        """
        user_data = {}
        stats_models: Sequence[TranslationReviewStatsModel] = cls.get_all(
            ).filter(cls.reviewer_user_id == user_id).fetch()
        for model in stats_models:
            splitted_id = model.id.split('.')
            id_without_user_id = '%s.%s' % (splitted_id[0], splitted_id[2])
            user_data[id_without_user_id] = {'language_code': model.
                language_code, 'topic_id': model.topic_id,
                'reviewed_translations_count': model.
                reviewed_translations_count,
                'reviewed_translation_word_count': model.
                reviewed_translation_word_count,
                'accepted_translations_count': model.
                accepted_translations_count,
                'accepted_translations_with_reviewer_edits_count': model.
                accepted_translations_with_reviewer_edits_count,
                'accepted_translation_word_count': model.
                accepted_translation_word_count, 'first_contribution_date':
                model.first_contribution_date.isoformat(),
                'last_contribution_date': model.last_contribution_date.
                isoformat()}
        return user_data
...
class QuestionContributionStatsModel(base_models.BaseModel):
    ...
    contributor_user_id = datastore_services.StringProperty(required=True,
        indexed=True)
    topic_id = datastore_services.StringProperty(required=True, indexed=True)
    submitted_questions_count = datastore_services.IntegerProperty(required
        =True, indexed=True)
    accepted_questions_count = datastore_services.IntegerProperty(required=
        True, indexed=True)
    accepted_questions_without_reviewer_edits_count = (datastore_services.
        IntegerProperty(required=True, indexed=True))
    first_contribution_date = datastore_services.DateProperty(indexed=True)
    last_contribution_date = datastore_services.DateProperty(indexed=True)

    @classmethod
    def create(cls, contributor_user_id: str, topic_id: str,
        submitted_questions_count: int, accepted_questions_count: int,
        accepted_questions_without_reviewer_edits_count: int,
        first_contribution_date: datetime.date, last_contribution_date:
        datetime.date) ->str:
        """Creates a new QuestionContributionStatsModel instance and returns
        its ID.
        """
        entity_id = cls.construct_id(contributor_user_id, topic_id)
        entity = cls(id=entity_id, contributor_user_id=contributor_user_id,
            topic_id=topic_id, submitted_questions_count=
    ...
    @classmethod
    def get(cls, contributor_user_id: str, topic_id: str) ->Optional[
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{
            ...
            'accepted_questions_without_reviewer_edits_count': base_models.
            EXPORT_POLICY.EXPORTED, 'first_contribution_date': base_models.
            EXPORT_POLICY.EXPORTED, 'last_contribution_date': base_models.
            EXPORT_POLICY.EXPORTED})
    ...
    def export_data(cls, user_id: str) ->Dict[str, Dict[str, Union[str, int,
        ...
        """
        user_data = {}
        stats_models: Sequence[QuestionContributionStatsModel] = cls.get_all(
            ).filter(cls.contributor_user_id == user_id).fetch()
        for model in stats_models:
            splitted_id = model.id.split('.')
            id_without_user_id = '%s' % splitted_id[1]
            user_data[id_without_user_id] = {'topic_id': model.topic_id,
                'submitted_questions_count': model.
                submitted_questions_count, 'accepted_questions_count':
                model.accepted_questions_count,
                'accepted_questions_without_reviewer_edits_count': model.
                accepted_questions_without_reviewer_edits_count,
                'first_contribution_date': model.first_contribution_date.
                isoformat(), 'last_contribution_date': model.
                last_contribution_date.isoformat()}
        return user_data
...
class QuestionReviewStatsModel(base_models.BaseModel):
    ...
    reviewer_user_id = datastore_services.StringProperty(required=True,
        indexed=True)
    topic_id = datastore_services.StringProperty(required=True, indexed=True)
    reviewed_questions_count = datastore_services.IntegerProperty(required=
        True, indexed=True)
    accepted_questions_count = datastore_services.IntegerProperty(required=
        True, indexed=True)
    accepted_questions_with_reviewer_edits_count = (datastore_services.
        IntegerProperty(required=True, indexed=True))
    first_contribution_date = datastore_services.DateProperty(indexed=True)
    last_contribution_date = datastore_services.DateProperty(indexed=True)

    @classmethod
    def create(cls, reviewer_user_id: str, topic_id: str,
        ...
        entity = cls(id=entity_id, reviewer_user_id=reviewer_user_id,
    ...
    @classmethod
    def get(cls, reviewer_user_id: str, topic_id: str) ->Optional[
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{
            ...
            'accepted_questions_count': base_models.EXPORT_POLICY.EXPORTED,
            'accepted_questions_with_reviewer_edits_count': base_models.
            EXPORT_POLICY.EXPORTED, 'first_contribution_date': base_models.
            EXPORT_POLICY.EXPORTED, 'last_contribution_date': base_models.
            EXPORT_POLICY.EXPORTED})
    ...
    def export_data(cls, user_id: str) ->Dict[str, Dict[str, Union[str, int,
        ...
        """
        user_data = {}
        stats_models: Sequence[QuestionReviewStatsModel] = cls.get_all(
            ).filter(cls.reviewer_user_id == user_id).fetch()
        for model in stats_models:
            splitted_id = model.id.split('.')
            id_without_user_id = '%s' % splitted_id[1]
            user_data[id_without_user_id] = {'topic_id': model.topic_id,
                ...
                'accepted_questions_with_reviewer_edits_count': model.
                accepted_questions_with_reviewer_edits_count,
                'first_contribution_date': model.first_contribution_date.
                isoformat(), 'last_contribution_date': model.
                last_contribution_date.isoformat()}
...
class TranslationSubmitterTotalContributionStatsModel(base_models.BaseModel):
    ...
    language_code = datastore_services.StringProperty(required=True,
        indexed=True)
    contributor_id = datastore_services.StringProperty(required=True,
        indexed=True)
    topic_ids_with_translation_submissions = datastore_services.StringProperty(
        repeated=True, indexed=True)
    recent_review_outcomes = datastore_services.StringProperty(repeated=
        True, indexed=True, choices=REVIEW_OUTCOME_CHOICES)
    recent_performance = datastore_services.IntegerProperty(required=True,
        indexed=True)
    overall_accuracy = datastore_services.FloatProperty(required=True,
        indexed=True)
    submitted_translations_count = datastore_services.IntegerProperty(required
        =True, indexed=True)
    submitted_translation_word_count = datastore_services.IntegerProperty(
    ...
    @classmethod
    def create(cls, language_code: str, contributor_id: str,
        ...
        int, accepted_translations_without_reviewer_edits_count: int,
        accepted_translation_word_count: int, rejected_translations_count:
        int, rejected_translation_word_count: int, first_contribution_date:
        datetime.date, last_contribution_date: datetime.date) ->str:
        ...
        entity = cls(id=entity_id, language_code=language_code,
            ...
            topic_ids_with_translation_submissions, recent_review_outcomes=
            recent_review_outcomes, recent_performance=recent_performance,
    ...
    @classmethod
    def get(cls, language_code: str, contributor_id: str) ->Optional[
    ...
    @classmethod
    def fetch_page(cls, page_size: int, offset: int, language_code: str,
        sort_by: Optional[SortChoices.value], topic_ids: Optional[List[str]
        ], max_days_since_last_activity: Optional[int]) ->Tuple[Sequence[
        TranslationSubmitterTotalContributionStatsModel], int, bool]:
        ...
        Returns:
            3-tuple(sorted_results, next_offset, more). where:
                ...
                more: bool. If True, there are (probably) more results after
                    ...
                    after this batch.
        """
        sort_options_dict = {SortChoices.SORT_KEY_INCREASING_LAST_ACTIVITY.
            value: -cls.last_contribution_date, SortChoices.
            SORT_KEY_DECREASING_LAST_ACTIVITY.value: cls.
            last_contribution_date, SortChoices.
            SORT_KEY_INCREASING_PERFORMANCE.value: cls.recent_performance,
            SortChoices.SORT_KEY_DECREASING_PERFORMANCE.value: -cls.
            recent_performance, SortChoices.SORT_KEY_DECREASING_ACCURACY.
            value: -cls.overall_accuracy, SortChoices.
            SORT_KEY_INCREASING_ACCURACY.value: cls.overall_accuracy,
            SortChoices.SORT_KEY_DECREASING_SUBMISSIONS.value: -cls.
            submitted_translations_count, SortChoices.
            SORT_KEY_INCREASING_SUBMISSIONS.value: cls.
            submitted_translations_count}
        sort = -cls.recent_performance
        if sort_by is not None:
            sort = sort_options_dict[sort_by]
        if topic_ids is not None:
            sort_query = cls.query(datastore_services.all_of(cls.
                language_code == language_code, cls.
                topic_ids_with_translation_submissions.IN(topic_ids))).order(
                sort)
        else:
            sort_query = cls.query(datastore_services.all_of(cls.
                language_code == language_code)).order(sort)
        ...
        if max_days_since_last_activity is not None:
            ...
            while len(sorted_results) < page_size:
                result_models: Sequence[
                    TranslationSubmitterTotalContributionStatsModel
                    ] = sort_query.fetch(NUM_MODELS_PER_FETCH, offset=
                    next_offset)
    ...
    def get_all_by_user_id(cls, user_id: str) ->Sequence[
        ...
        Returns:
            ...
            TranslationSubmitterTotalContributionStatsModel.
        """
        return cls.get_all().filter(cls.contributor_id == user_id).fetch(feconf
            .DEFAULT_SUGGESTION_QUERY_LIMIT)
    ...
    def has_reference_to_user_id(cls, user_id: str) ->bool:
        ...
        Returns:
            bool. Whether any models refer to the given user ID.
        """
        return cls.query(cls.contributor_id == user_id).get(keys_only=True
            ) is not None
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        """Model contains data to export corresponding to a user."""
        return dict(super(cls, cls).get_export_policy(), **{'language_code':
            base_models.EXPORT_POLICY.EXPORTED, 'contributor_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE,
            'topic_ids_with_translation_submissions': base_models.
            EXPORT_POLICY.EXPORTED, 'recent_review_outcomes': base_models.
            EXPORT_POLICY.EXPORTED, 'recent_performance': base_models.
            EXPORT_POLICY.EXPORTED, 'overall_accuracy': base_models.
            EXPORT_POLICY.EXPORTED, 'submitted_translations_count':
            base_models.EXPORT_POLICY.EXPORTED,
            'submitted_translation_word_count': base_models.EXPORT_POLICY.
            EXPORTED, 'accepted_translations_count': base_models.
            EXPORT_POLICY.EXPORTED,
            'accepted_translations_without_reviewer_edits_count':
            base_models.EXPORT_POLICY.EXPORTED,
            'accepted_translation_word_count': base_models.EXPORT_POLICY.
    ...
    def apply_deletion_policy(cls, user_id: str) ->None:
        ...
        Args:
            user_id: str. The ID of the user whose data should be deleted.
        """
        datastore_services.delete_multi(cls.query(cls.contributor_id ==
            user_id).fetch(keys_only=True))
    ...
    def export_data(cls, user_id: str) ->Dict[str, Dict[str, Union[str, int,
        ...
        for model in stats_models:
            ...
            user_data[language_code] = {'language_code': model.
                language_code, 'topic_ids_with_translation_submissions':
                ...
                rejected_translations_count,
                'rejected_translation_word_count': model.
                rejected_translation_word_count, 'first_contribution_date':
                model.first_contribution_date.isoformat(),
                'last_contribution_date': model.last_contribution_date.
                isoformat()}
        return user_data
...
class TranslationReviewerTotalContributionStatsModel(base_models.BaseModel):
    ...
    def create(cls, language_code: str, contributor_id: str,
        ...
        reviewed_translations_count: int, accepted_translations_count: int,
        accepted_translations_with_reviewer_edits_count: int,
        accepted_translation_word_count: int, rejected_translations_count:
        int, first_contribution_date: datetime.date, last_contribution_date:
        datetime.date) ->str:
        ...
        entity = cls(id=entity_id, language_code=language_code,
            ...
            topic_ids_with_translation_reviews, reviewed_translations_count
    ...
    @classmethod
    def fetch_page(cls, page_size: int, offset: int, language_code: str,
        sort_by: Optional[SortChoices.value], max_days_since_last_activity:
        Optional[int]) ->Tuple[Sequence[
        TranslationReviewerTotalContributionStatsModel], int, bool]:
        """Returns the models according to values specified.
        ...
        sort_options_dict = {SortChoices.SORT_KEY_INCREASING_LAST_ACTIVITY.
            ...
            SORT_KEY_DECREASING_LAST_ACTIVITY.value: cls.
            last_contribution_date, SortChoices.
            SORT_KEY_INCREASING_REVIEWED_TRANSLATIONS.value: cls.
            reviewed_translations_count, SortChoices.
            SORT_KEY_DECREASING_REVIEWED_TRANSLATIONS.value: -cls.
            reviewed_translations_count}
        sort = -cls.reviewed_translations_count
        if sort_by is not None:
            sort = sort_options_dict[sort_by]
        sort_query = cls.query(datastore_services.all_of(cls.language_code ==
            language_code)).order(sort)
        ...
        if max_days_since_last_activity is not None:
            ...
            next_offset = offset + len(sorted_results)
        next_result_model: Sequence[
            TranslationReviewerTotalContributionStatsModel] = sort_query.fetch(
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{'language_code':
            base_models.EXPORT_POLICY.EXPORTED, 'contributor_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE,
            'topic_ids_with_translation_reviews': base_models.EXPORT_POLICY
            .EXPORTED, 'reviewed_translations_count': base_models.
            EXPORT_POLICY.EXPORTED, 'accepted_translations_count':
            base_models.EXPORT_POLICY.EXPORTED,
            'accepted_translations_with_reviewer_edits_count': base_models.
            EXPORT_POLICY.EXPORTED, 'accepted_translation_word_count':
            base_models.EXPORT_POLICY.EXPORTED,
            'rejected_translations_count': base_models.EXPORT_POLICY.
            EXPORTED, 'first_contribution_date': base_models.EXPORT_POLICY.
            EXPORTED, 'last_contribution_date': base_models.EXPORT_POLICY.
            EXPORTED})
    ...
    def export_data(cls, user_id: str) ->Dict[str, Dict[str, Union[str, int,
        ...
        """
        user_data = {}
        stats_models: Sequence[TranslationReviewerTotalContributionStatsModel
            ] = cls.get_all().filter(cls.contributor_id == user_id).fetch()
        for model in stats_models:
            splitted_id = model.id.split('.')
            language_code = splitted_id[0]
            user_data[language_code] = {'language_code': model.
                language_code, 'topic_ids_with_translation_reviews': model.
                topic_ids_with_translation_reviews,
                'reviewed_translations_count': model.
                reviewed_translations_count, 'accepted_translations_count':
                ...
                accepted_translation_word_count,
                'rejected_translations_count': model.
                rejected_translations_count, 'first_contribution_date':
                model.first_contribution_date.isoformat(),
                'last_contribution_date': model.last_contribution_date.
                isoformat()}
        return user_data
...
class QuestionSubmitterTotalContributionStatsModel(base_models.BaseModel):
    ...
    accepted_questions_without_reviewer_edits_count = (datastore_services.
        IntegerProperty(required=True, indexed=True))
    rejected_questions_count = datastore_services.IntegerProperty(required=
        True, indexed=True)
    first_contribution_date = datastore_services.DateProperty(indexed=True)
    last_contribution_date = datastore_services.DateProperty(indexed=True)

    @classmethod
    def create(cls, contributor_id: str,
        ...
        accepted_questions_without_reviewer_edits_count: int,
        rejected_questions_count: int, first_contribution_date: datetime.
        date, last_contribution_date: datetime.date) ->str:
        ...
        entity = cls(id=entity_id, contributor_id=contributor_id,
            topic_ids_with_question_submissions=
            topic_ids_with_question_submissions, recent_review_outcomes=
            recent_review_outcomes, recent_performance=recent_performance,
    ...
    @classmethod
    def fetch_page(cls, page_size: int, offset: int, sort_by: Optional[
        SortChoices.value], topic_ids: Optional[List[str]],
        max_days_since_last_activity: Optional[int]) ->Tuple[Sequence[
        QuestionSubmitterTotalContributionStatsModel], int, bool]:
        """Returns the models according to values specified.
        ...
        sort_options_dict = {SortChoices.SORT_KEY_INCREASING_LAST_ACTIVITY.
            ...
            SORT_KEY_INCREASING_ACCURACY.value: cls.overall_accuracy,
            SortChoices.SORT_KEY_DECREASING_SUBMISSIONS.value: -cls.
            submitted_questions_count, SortChoices.
            SORT_KEY_INCREASING_SUBMISSIONS.value: cls.
        ...
        if topic_ids is not None:
            sort_query = cls.query(datastore_services.all_of(cls.
                topic_ids_with_question_submissions.IN(topic_ids))).order(sort)
        else:
        ...
        if max_days_since_last_activity is not None:
            ...
            next_offset = offset + len(sorted_results)
        next_result_model: Sequence[
            QuestionSubmitterTotalContributionStatsModel] = sort_query.fetch(
            offset=next_offset)
        more: bool = len(next_result_model) != 0
        return sorted_results, next_offset, more
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{
            ...
            'topic_ids_with_question_submissions': base_models.
            EXPORT_POLICY.EXPORTED, 'recent_review_outcomes': base_models.
            EXPORT_POLICY.EXPORTED, 'recent_performance': base_models.
            EXPORT_POLICY.EXPORTED, 'overall_accuracy': base_models.
            EXPORT_POLICY.EXPORTED, 'submitted_questions_count':
            base_models.EXPORT_POLICY.EXPORTED, 'accepted_questions_count':
            base_models.EXPORT_POLICY.EXPORTED,
            'accepted_questions_without_reviewer_edits_count': base_models.
            EXPORT_POLICY.EXPORTED, 'rejected_questions_count': base_models
            .EXPORT_POLICY.EXPORTED, 'first_contribution_date': base_models
            .EXPORT_POLICY.EXPORTED, 'last_contribution_date': base_models.
            EXPORT_POLICY.EXPORTED})
    ...
    def export_data(cls, user_id: str) ->Dict[str, Dict[str, Union[str, int,
        ...
        """
        user_data = {}
        stats_models: Sequence[QuestionSubmitterTotalContributionStatsModel
            ] = cls.get_all().filter(cls.contributor_id == user_id).fetch()
        for model in stats_models:
            user_data = {'topic_ids_with_question_submissions': model.
                topic_ids_with_question_submissions,
                'recent_review_outcomes': model.recent_review_outcomes,
                'recent_performance': model.recent_performance,
                'overall_accuracy': model.overall_accuracy,
...
class QuestionReviewerTotalContributionStatsModel(base_models.BaseModel):
    ...
    @classmethod
    def create(cls, contributor_id: str, topic_ids_with_question_reviews:
        List[str], reviewed_questions_count: int, accepted_questions_count:
        int, accepted_questions_with_reviewer_edits_count: int,
        rejected_questions_count: int, first_contribution_date: datetime.
        date, last_contribution_date: datetime.date) ->str:
        ...
        Returns:
            str. The ID of the model created.
        """
        entity = cls(id=contributor_id, contributor_id=contributor_id,
    ...
    @classmethod
    def fetch_page(cls, page_size: int, offset: int, sort_by: Optional[
        SortChoices.value], max_days_since_last_activity: Optional[int]
        ...
        sort_options_dict = {SortChoices.SORT_KEY_INCREASING_LAST_ACTIVITY.
            ...
            SORT_KEY_DECREASING_LAST_ACTIVITY.value: cls.
            last_contribution_date, SortChoices.
            SORT_KEY_INCREASING_REVIEWED_QUESTIONS.value: cls.
            reviewed_questions_count, SortChoices.
            SORT_KEY_DECREASING_REVIEWED_QUESTIONS.value: -cls.
        ...
        if max_days_since_last_activity is not None:
            ...
            sorted_results = list(sort_query.fetch(page_size, offset=offset))
            next_offset = offset + len(sorted_results)
        next_result_model: Sequence[QuestionReviewerTotalContributionStatsModel
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{
            'contributor_id': base_models.EXPORT_POLICY.NOT_APPLICABLE,
            'topic_ids_with_question_reviews': base_models.EXPORT_POLICY.
            EXPORTED, 'reviewed_questions_count': base_models.EXPORT_POLICY
            .EXPORTED, 'accepted_questions_count': base_models.
            EXPORT_POLICY.EXPORTED,
            'accepted_questions_with_reviewer_edits_count': base_models.
            EXPORT_POLICY.EXPORTED, 'rejected_questions_count': base_models
            .EXPORT_POLICY.EXPORTED, 'first_contribution_date': base_models
    ...
    def export_data(cls, user_id: str) ->Dict[str, Dict[str, Union[str, int,
        ...
        for model in stats_models:
            user_data = {'topic_ids_with_question_reviews': model.
                topic_ids_with_question_reviews, 'reviewed_questions_count':
                model.reviewed_questions_count, 'accepted_questions_count':
                model.accepted_questions_count,
                'accepted_questions_with_reviewer_edits_count': model.
                accepted_questions_with_reviewer_edits_count,
                'rejected_questions_count': model.rejected_questions_count,
...
class TranslationCoordinatorsModel(base_models.BaseModel):
    ...
    """
    coordinator_ids = datastore_services.StringProperty(indexed=True,
        repeated=True)
    coordinators_count = datastore_services.IntegerProperty(indexed=True,
        required=True)
    ...
    def has_reference_to_user_id(cls, user_id: str) ->bool:
        ...
        Returns:
            bool. Whether any models refer to the given user ID.
        """
        return cls.query(cls.coordinator_ids == user_id).get(keys_only=True
            ) is not None
    ...
    def export_data(cls, user_id: str) ->Dict[str, List[str]]:
        ...
        """
        coordinated_languages = cls.get_all().filter(cls.coordinator_ids ==
            user_id)
        coordinated_language_ids = [language.id for language in
            coordinated_languages]
        return {'coordinated_language_ids': coordinated_language_ids}

    @classmethod
    def get_by_user(cls, user_id: str) ->Sequence[TranslationCoordinatorsModel
        ]:
