...
class GalleryModel(base_models.BaseModel):
    """Model for storing publicized explorations keyed by rights status.
    Generated by MapReduce GalleryAggregator MapReduce job.
    """

    # TODO(msl): delete because not used any more?

    # List of dicts, each having keys 'id', 'title', 'category', etc
    # (see exp_jobs for details).
    list_of_exp_dicts = ndb.JsonProperty(repeated=True)
...
class ExpSummaryModel(base_models.BaseModel):
    ...
    # tests in exp_jobs_test don't work if auto_now=True (b/c would 
    # get incorrect expected last_updated)
    # also want this to be convertible to JSON, so use float
    #last_updated = ndb.DateTimeProperty(auto_now=True, indexed=True)
    ...
    @classmethod
    def summarize_exploration(cls, exp_model,
                              put_in_datastore=True):
        """Create ExpSummaryModel for an exploration model. If there is 
        already a summary in the datastore with the same exploration id,
        overwrite it.
        """
        exp_rights = ExplorationRightsModel.get_by_id(exp_model.id)

        last_updated = None
        if exp_model.last_updated:
            last_updated = utils.get_time_in_millisecs(
                exp_model.last_updated)

        created_on = None
        if exp_model.created_on:
            created_on = utils.get_time_in_millisecs(
                exp_model.created_on)

        expsum = ExpSummaryModel(
            id=exp_model.id,
            title=exp_model.title,
            category=exp_model.category,
            objective=exp_model.objective,
            language_code=exp_model.language_code,
            skill_tags=exp_model.skill_tags,
