...
class ExplorationCommitLogEntryModel(base_models.BaseCommitLogEntryModel):
    ...
    def get_multi(  # type: ignore[override]
        ...
        """
        instance_ids = [cls.get_instance_id(exp_id, exp_version) for exp_version in exp_versions]

        return super(ExplorationCommitLogEntryModel, cls).get_multi(instance_ids)
...
class ExplorationModel(base_models.VersionedModel):
    ...
    states_schema_version = datastore_services.IntegerProperty(required=True, default=0, indexed=True)
    # The name of the initial state of this exploration.
    init_state_name = datastore_services.StringProperty(required=True, indexed=True)
    ...
    param_changes = datastore_services.JsonProperty(repeated=True, indexed=False)
    # A boolean indicating whether automatic text-to-speech is enabled in
    # this exploration.
    auto_tts_enabled = datastore_services.BooleanProperty(default=True, indexed=True)
    ...
    # exploration.
    edits_allowed = datastore_services.BooleanProperty(default=True, indexed=True)
    # A boolean indicating whether correctness feedback is enabled in this
    # exploration.
    # DEPRECATED. Do not use.
    correctness_feedback_enabled = datastore_services.BooleanProperty(default=True, indexed=True)
    ...
    def delete_multi(  # type: ignore[override]
        ...
        if not force_deletion:
            ...
            versioned_and_exp_rights_models = zip(versioned_exp_models, exp_rights_models)
            ...
            # True.
            versioned_exp_models_without_none = [model for model in versioned_exp_models if model is not None]
            version_history_keys = []
            for model in versioned_exp_models_without_none:
                for version in range(1, model.version + 1):
                    version_history_id = ExplorationVersionHistoryModel.get_instance_id(model.id, version)
                    version_history_keys.append(datastore_services.Key(ExplorationVersionHistoryModel, version_history_id))
...
class ExplorationRightsSnapshotMetadataModel(base_models.BaseSnapshotMetadataModel):
    """Storage model for the metadata for an exploration rights snapshot."""

    pass


class ExplorationRightsSnapshotContentModel(base_models.BaseSnapshotContentModel):
    ...
    def has_reference_to_user_id(cls, user_id: str) -> bool:
        ...
        """
        return ExplorationRightsSnapshotMetadataModel.query(ExplorationRightsSnapshotMetadataModel.content_user_ids == user_id).get(keys_only=True) is not None
...
class ExplorationRightsModel(base_models.VersionedModel):
    ...
    # loose typing. So, once we explicitly type those NDB JSON properties, we
    # can remove Any type from the argument of '_reconstitute' method.
    def _reconstitute(self, snapshot_dict: Dict[str, Any]) -> ExplorationRightsModel:
        """Populates the model instance with the snapshot.
        ...
        """
        self.populate(**ExplorationRightsModel.convert_to_valid_dict(snapshot_dict))
        return self
    ...
    def compute_models_to_commit(
        ...
        for commit_cmd in commit_cmds:
            user_id_attribute_names = next(cmd['user_id_attribute_names'] for cmd in feconf.EXPLORATION_RIGHTS_CHANGE_ALLOWED_COMMANDS if cmd['name'] == commit_cmd['cmd'])
    ...
    def export_data(cls, user_id: str) -> Dict[str, List[str]]:
        ...
        voiced_explorations = cls.get_all().filter(cls.voice_artist_ids == user_id)
        ...
        editable_exploration_ids = [exp.key.id() for exp in editable_explorations]
        voiced_exploration_ids = [exp.key.id() for exp in voiced_explorations]
        viewable_exploration_ids = [exp.key.id() for exp in viewable_explorations]
...
class TransientCheckpointUrlModel(base_models.BaseModel):
    ...
    @classmethod
    def create(cls, exploration_id: str, unique_progress_url_id: str) -> TransientCheckpointUrlModel:
    ...
    def get_new_progress_id(cls) -> str:
        ...
        for _ in range(base_models.MAX_RETRIES):
            new_id = '%s' % ''.join(random.choice(string.ascii_letters) for _ in range(constants.MAX_PROGRESS_URL_ID_LENGTH))
...
class ExpSummaryModel(base_models.BaseModel):
    ...
    exploration_model_last_updated = datastore_services.DateTimeProperty(indexed=True)
    ...
    # model was created).
    exploration_model_created_on = datastore_services.DateTimeProperty(indexed=True)
    ...
    def get_top_rated(cls, limit: int) -> Sequence[ExpSummaryModel]:
        ...
        """
        return ExpSummaryModel.query().filter(ExpSummaryModel.status == constants.ACTIVITY_STATUS_PUBLIC).filter(not ExpSummaryModel.deleted).order(-ExpSummaryModel.scaled_average_rating).fetch(limit)

    @classmethod
    def get_private_at_least_viewable(cls, user_id: str) -> Sequence[ExpSummaryModel]:
        ...
        return (
            ...
            .filter(not ExpSummaryModel.deleted)
    ...
    def get_recently_published(cls, limit: int) -> Sequence[ExpSummaryModel]:
        ...
        """
        return ExpSummaryModel.query().filter(ExpSummaryModel.status == constants.ACTIVITY_STATUS_PUBLIC).filter(not ExpSummaryModel.deleted).order(-ExpSummaryModel.first_published_msec).fetch(limit)
...
class ExplorationVersionHistoryModel(base_models.BaseModel):
    ...
    exploration_version = datastore_services.IntegerProperty(required=True, indexed=True)
    ...
    state_version_history = datastore_services.JsonProperty(default={}, indexed=False)
    ...
    # the exploration.
    metadata_last_edited_version_number = datastore_services.IntegerProperty(indexed=True)
    # The user id of the user who committed the latest changes to the
    # exploration metadata.
    metadata_last_edited_committer_id = datastore_services.StringProperty(required=True, indexed=True)
    ...
    committer_ids = datastore_services.StringProperty(indexed=True, repeated=True)
    ...
    def has_reference_to_user_id(cls, user_id: str) -> bool:
        ...
        Returns:
            bool. Whether any models refer to the given user ID.
        """
        return ExplorationVersionHistoryModel.query(ExplorationVersionHistoryModel.committer_ids == user_id).get(keys_only=True) is not None
