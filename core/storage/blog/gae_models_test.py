...
class BlogPostModelTest(test_utils.GenericTestBase):
    ...
    def test_get_export_policy(self) -> None:
        ...
        self.assertEqual(
            blog_models.BlogPostModel.get_export_policy(), expected_export_policy_dict
        )
    ...
    def test_has_reference_to_user_id(self) -> None:
        self.assertTrue(
            blog_models.BlogPostModel.has_reference_to_user_id(self.USER_ID)
        )
        self.assertFalse(
            blog_models.BlogPostModel.has_reference_to_user_id(self.NONEXISTENT_USER_ID)
        )
    ...
    def test_raise_exception_by_mocking_collision(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception, 'New blog post id generator is producing too many collisions.'
        ):
            # Swap dependent method get_by_id to simulate collision every time.
            with self.swap(
                blog_post_model_cls,
    ...
    def test_creating_new_blog_post_model_instance(self) -> None:
        ...
        blog_post_model_instance = blog_models.BlogPostModel.create(
...
class BlogPostSummaryModelTest(test_utils.GenericTestBase):
    ...
    def test_has_reference_to_user_id(self) -> None:
        self.assertTrue(
            blog_models.BlogPostSummaryModel.has_reference_to_user_id(self.USER_ID)
        )
        self.assertFalse(
            blog_models.BlogPostSummaryModel.has_reference_to_user_id(
...
class BlogPostRightsModelTest(test_utils.GenericTestBase):
    ...
    def test_has_reference_to_user_id(self) -> None:
        self.assertTrue(
            blog_models.BlogPostRightsModel.has_reference_to_user_id(self.USER_ID)
        )
        self.assertTrue(
            blog_models.BlogPostRightsModel.has_reference_to_user_id(self.USER_ID_NEW)
        )
        self.assertFalse(
            blog_models.BlogPostRightsModel.has_reference_to_user_id(
                self.NONEXISTENT_USER_ID
            )
        )
    ...
    def test_export_data_on_uninvolved_user(self) -> None:
        ...
        blog_post_ids = blog_models.BlogPostRightsModel.export_data(
            self.NONEXISTENT_USER_ID
        )
    ...
    def test_raise_exception_by_mocking_collision(self) -> None:
        ...
        # Test create method.
        with self.assertRaisesRegex(
            Exception, 'Blog Post ID conflict on creating new blog post rights model.'
        ):
            #  Swap dependent method get_by_id to simulate collision every time.
            with self.swap(
    ...
    def test_creating_new_blog_post_rights_model(self) -> None:
        blog_post_model_id = blog_models.BlogPostModel.generate_new_blog_post_id()
        blog_post_rights_model_instance = blog_models.BlogPostRightsModel.create(
            blog_post_model_id, self.USER_ID
        )
...
class BlogAuthorDetailsModelTest(test_utils.GenericTestBase):
    ...
    def test_raise_exception_by_mocking_collision(self) -> None:
        ...
        # Test create method.
        with self.assertRaisesRegex(
            Exception, 'A blog author details model for given user already exists.'
        ):
        ...
        # Test generate_new_blog_post_id method.
        with self.assertRaisesRegex(
            Exception, 'New instance id generator is producing too many collisions.'
        ):
            # Swap dependent method get_by_id to simulate collision every time.
            with self.swap(
                blog_author_details_model_cls,
    ...
    def test_has_reference_to_user_id(self) -> None:
        ...
        self.assertTrue(
            blog_models.BlogAuthorDetailsModel.has_reference_to_user_id(self.USER_1_ID)
        )
