...
class BlogPostModel(base_models.BaseModel):
    ...
    url_fragment = datastore_services.StringProperty(indexed=True, required=True)
    ...
    def generate_new_blog_post_id(cls) -> str:
        ...
        for _ in range(base_models.MAX_RETRIES):
            ...
            if not cls.get_by_id(blog_post_id):
                return blog_post_id
        raise Exception('New blog post id generator is producing too many collisions.')
    ...
    def create(cls, blog_post_id: str, author_id: str) -> BlogPostModel:
        ...
        if cls.get_by_id(blog_post_id):
            raise Exception('A blog post with the given blog post ID exists already.')
    ...
    def export_data(cls, user_id: str) -> Dict[str, BlogPostModelDataDict]:
        ...
        blog_post_models: Sequence[BlogPostModel] = cls.get_all().filter(cls.author_id == user_id).fetch()
        for blog_post_model in blog_post_models:
            published_on = utils.get_time_in_millisecs(blog_post_model.published_on) if blog_post_model.published_on is not None else None
...
class BlogPostRightsModel(base_models.BaseModel):
    ...
    blog_post_is_published = datastore_services.BooleanProperty(indexed=True, required=True, default=False)
    ...
    def get_published_models_by_user(
        ...
        query = cls.query(
            cls.editor_ids == user_id,
            cls.blog_post_is_published,
        ).order(-cls.last_updated)
        return list(query.fetch(limit, offset=offset) if limit is not None else query.fetch(offset=offset))

    @classmethod
    def get_draft_models_by_user(cls, user_id: str, limit: Optional[int] = None) -> List[BlogPostRightsModel]:
        ...
        query = cls.query(
            cls.editor_ids == user_id,
            not cls.blog_post_is_published,
        ).order(-cls.last_updated)
    ...
    def export_data(cls, user_id: str) -> Dict[str, List[str]]:
        ...
        """
        editable_blog_posts: Sequence[BlogPostRightsModel] = cls.query(cls.editor_ids == user_id).fetch()
    ...
    def create(cls, blog_post_id: str, author_id: str) -> BlogPostRightsModel:
        ...
        """
        if cls.get_by_id(blog_post_id):
            raise Exception('Blog Post ID conflict on creating new blog post rights model.')
    ...
    def generate_new_instance_id(cls) -> str:
        ...
        for _ in range(base_models.MAX_RETRIES):
            ...
            if not cls.get_by_id(instance_id):
                return instance_id
        raise Exception('New instance id generator is producing too many collisions.')

    @classmethod
    def create(cls, author_id: str, displayed_author_name: str, author_bio: str) -> None:
        ...
        if cls.get_by_author(author_id):
            raise Exception('A blog author details model for given user already exists.')
