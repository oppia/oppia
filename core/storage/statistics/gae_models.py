...
ALLOWED_ISSUE_TYPES: Final = [ISSUE_TYPE_EARLY_QUIT,
    ISSUE_TYPE_MULTIPLE_INCORRECT_SUBMISSIONS,
    ISSUE_TYPE_CYCLIC_STATE_TRANSITIONS]
ALLOWED_ACTION_TYPES: Final = [ACTION_TYPE_EXPLORATION_START,
    ACTION_TYPE_ANSWER_SUBMIT, ACTION_TYPE_EXPLORATION_QUIT]
ALLOWED_ENTITY_TYPES: Final = [feconf.ENTITY_TYPE_EXPLORATION, feconf.
...
class StateCounterModel(base_models.BaseModel):
    ...
    """
    first_entry_count = datastore_services.IntegerProperty(default=0,
        indexed=False)
    subsequent_entries_count = datastore_services.IntegerProperty(default=0,
        indexed=False)
    resolved_answer_count = datastore_services.IntegerProperty(default=0,
        indexed=False)
    active_answer_count = datastore_services.IntegerProperty(default=0,
        indexed=False)
    ...
    @classmethod
    def get_or_create(cls, exploration_id: str, state_name: str
        ) ->StateCounterModel:
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{
            ...
            'subsequent_entries_count': base_models.EXPORT_POLICY.
            NOT_APPLICABLE, 'resolved_answer_count': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'active_answer_count':
...
class AnswerSubmittedEventLogEntryModel(base_models.BaseModel):
    ...
    def get_new_event_entity_id(cls, exp_id: str, session_id: str) ->str:
        ...
        return cls.get_new_id('%s:%s:%s' % (utils.get_time_in_millisecs(
            timestamp), exp_id, session_id))

    @classmethod
    def create(cls, exp_id: str, exp_version: int, state_name: str,
        session_id: str, time_spent_in_state_secs: float,
        is_feedback_useful: bool) ->str:
        """Creates a new answer submitted event."""
        entity_id = cls.get_new_event_entity_id(exp_id, session_id)
        answer_submitted_event_entity = cls(id=entity_id, exp_id=exp_id,
            exp_version=exp_version, state_name=state_name, session_id=
            session_id, time_spent_in_state_secs=time_spent_in_state_secs,
            is_feedback_useful=is_feedback_useful, event_schema_version=
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        """Model doesn't contain any data directly corresponding to a user."""
        return dict(super(cls, cls).get_export_policy(), **{'exp_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'exp_version':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'state_name':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'session_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE,
            'time_spent_in_state_secs': base_models.EXPORT_POLICY.
            NOT_APPLICABLE, 'is_feedback_useful': base_models.EXPORT_POLICY
            .NOT_APPLICABLE, 'event_schema_version': base_models.
...
class ExplorationActualStartEventLogEntryModel(base_models.BaseModel):
    ...
    def create(cls, exp_id: str, exp_version: int, state_name: str,
        ...
        entity_id = cls.get_new_event_entity_id(exp_id, session_id)
        actual_start_event_entity = cls(id=entity_id, exp_id=exp_id,
            exp_version=exp_version, state_name=state_name, session_id=
            session_id, event_schema_version=feconf.
...
class SolutionHitEventLogEntryModel(base_models.BaseModel):
    ...
    @classmethod
    def create(cls, exp_id: str, exp_version: int, state_name: str,
        session_id: str, time_spent_in_state_secs: float) ->str:
        """Creates a new solution hit event."""
        entity_id = cls.get_new_event_entity_id(exp_id, session_id)
        solution_hit_event_entity = cls(id=entity_id, exp_id=exp_id,
            exp_version=exp_version, state_name=state_name, session_id=
            session_id, time_spent_in_state_secs=time_spent_in_state_secs,
...
class StartExplorationEventLogEntryModel(base_models.BaseModel):
    ...
    play_type = datastore_services.StringProperty(indexed=True, choices=[
    ...
    @classmethod
    def create(cls, exp_id: str, exp_version: int, state_name: str,
        session_id: str, params: Dict[str, str], play_type: str,
        unused_version: int=1) ->str:
        ...
        """
        entity_id = cls.get_new_event_entity_id(exp_id, session_id)
        start_event_entity = cls(id=entity_id, event_type=feconf.
            EVENT_TYPE_START_EXPLORATION, exploration_id=exp_id,
            exploration_version=exp_version, state_name=state_name,
            session_id=session_id, client_time_spent_in_secs=0.0, params=
            params, play_type=play_type, event_schema_version=feconf.
            CURRENT_EVENT_MODELS_SCHEMA_VERSION)
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        """Model doesn't contain any data directly corresponding to a user."""
        return dict(super(cls, cls).get_export_policy(), **{'event_type':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'exploration_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'exploration_version':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'state_name':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'session_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE,
            'client_time_spent_in_secs': base_models.EXPORT_POLICY.
            NOT_APPLICABLE, 'params': base_models.EXPORT_POLICY.
            NOT_APPLICABLE, 'play_type': base_models.EXPORT_POLICY.
            NOT_APPLICABLE, 'event_schema_version': base_models.
            EXPORT_POLICY.NOT_APPLICABLE})
...
class MaybeLeaveExplorationEventLogEntryModel(base_models.BaseModel):
    ...
    @classmethod
    def create(cls, exp_id: str, exp_version: int, state_name: str,
        session_id: str, client_time_spent_in_secs: float, params: Dict[str,
        str], play_type: str) ->str:
        ...
        """
        entity_id = cls.get_new_event_entity_id(exp_id, session_id)
        leave_event_entity = cls(id=entity_id, event_type=feconf.
            EVENT_TYPE_MAYBE_LEAVE_EXPLORATION, exploration_id=exp_id,
            exploration_version=exp_version, state_name=state_name,
            session_id=session_id, client_time_spent_in_secs=
            client_time_spent_in_secs, params=params, play_type=play_type,
            event_schema_version=feconf.CURRENT_EVENT_MODELS_SCHEMA_VERSION)
...
class CompleteExplorationEventLogEntryModel(base_models.BaseModel):
    ...
    def create(cls, exp_id: str, exp_version: int, state_name: str,
        ...
        """
        entity_id = cls.get_new_event_entity_id(exp_id, session_id)
        complete_event_entity = cls(id=entity_id, event_type=feconf.
            EVENT_TYPE_COMPLETE_EXPLORATION, exploration_id=exp_id,
            exploration_version=exp_version, state_name=state_name,
            session_id=session_id, client_time_spent_in_secs=
            client_time_spent_in_secs, params=params, play_type=play_type,
            event_schema_version=feconf.CURRENT_EVENT_MODELS_SCHEMA_VERSION)
...
class RateExplorationEventLogEntryModel(base_models.BaseModel):
    ...
    def get_new_event_entity_id(cls, exp_id: str, user_id: str) ->str:
        ...
        timestamp = get_current_datetime_utc()
        return cls.get_new_id('%s:%s:%s' % (utils.get_time_in_millisecs(
            timestamp), exp_id, user_id))

    @classmethod
    def create(cls, exp_id: str, user_id: str, rating: int, old_rating:
        Optional[int]) ->str:
        ...
        cls(id=entity_id, event_type=feconf.EVENT_TYPE_RATE_EXPLORATION,
            exploration_id=exp_id, rating=rating, old_rating=old_rating,
            event_schema_version=feconf.CURRENT_EVENT_MODELS_SCHEMA_VERSION
            ).put()
        return entity_id
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{'event_type':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'exploration_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'rating': base_models
            .EXPORT_POLICY.NOT_APPLICABLE, 'old_rating': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'event_schema_version':
            base_models.EXPORT_POLICY.NOT_APPLICABLE})
...
class StateHitEventLogEntryModel(base_models.BaseModel):
    ...
    @classmethod
    def create(cls, exp_id: str, exp_version: int, state_name: str,
        session_id: str, params: Dict[str, str], play_type: str) ->str:
        ...
        """
        entity_id = cls.get_new_event_entity_id(exp_id, session_id)
        state_event_entity = cls(id=entity_id, event_type=feconf.
            EVENT_TYPE_STATE_HIT, exploration_id=exp_id,
            exploration_version=exp_version, state_name=state_name,
            session_id=session_id, params=params, play_type=play_type,
            event_schema_version=feconf.CURRENT_EVENT_MODELS_SCHEMA_VERSION)
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{'event_type':
            ...
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'state_name':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'session_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'params': base_models
            .EXPORT_POLICY.NOT_APPLICABLE, 'play_type': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'event_schema_version':
            base_models.EXPORT_POLICY.NOT_APPLICABLE})
...
class StateCompleteEventLogEntryModel(base_models.BaseModel):
    ...
    def create(cls, exp_id: str, exp_version: int, state_name: str,
        ...
        entity_id = cls.get_new_event_entity_id(exp_id, session_id)
        state_finish_event_entity = cls(id=entity_id, exp_id=exp_id,
            exp_version=exp_version, state_name=state_name, session_id=
            session_id, time_spent_in_state_secs=time_spent_in_state_secs,
            event_schema_version=feconf.CURRENT_EVENT_MODELS_SCHEMA_VERSION)
...
class LeaveForRefresherExplorationEventLogEntryModel(base_models.BaseModel):
    ...
    @classmethod
    def create(cls, exp_id: str, refresher_exp_id: str, exp_version: int,
        state_name: str, session_id: str, time_spent_in_state_secs: float
        ) ->str:
        """Creates a new leave for refresher exploration event."""
        entity_id = cls.get_new_event_entity_id(exp_id, session_id)
        leave_for_refresher_exp_entity = cls(id=entity_id, exp_id=exp_id,
            refresher_exp_id=refresher_exp_id, exp_version=exp_version,
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        """Model doesn't contain any data directly corresponding to a user."""
        return dict(super(cls, cls).get_export_policy(), **{'exp_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'refresher_exp_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'exp_version':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'state_name':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'session_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE,
...
class ExplorationStatsModel(base_models.BaseModel):
    ...
    @classmethod
    def get_model(cls, exp_id: str, exp_version: int) ->Optional[
    ...
    @classmethod
    def create(cls, exp_id: str, exp_version: int, num_starts_v1: int,
        num_starts_v2: int, num_actual_starts_v1: int, num_actual_starts_v2:
        int, num_completions_v1: int, num_completions_v2: int,
        state_stats_mapping: Dict[str, Dict[str, int]]) ->str:
        ...
        """
        instance_id = cls.get_entity_id(exp_id, exp_version)
        stats_instance = cls(id=instance_id, exp_id=exp_id, exp_version=
            exp_version, num_starts_v1=num_starts_v1, num_starts_v2=
            num_starts_v2, num_actual_starts_v1=num_actual_starts_v1,
            num_actual_starts_v2=num_actual_starts_v2, num_completions_v1=
            num_completions_v1, num_completions_v2=num_completions_v2,
    ...
    @classmethod
    def get_multi_versions(cls, exp_id: str, version_numbers: List[int]
        ...
        Returns:
            ...
            given versions.
        """
        entity_ids = [cls.get_entity_id(exp_id, version) for version in
            version_numbers]
        exploration_stats_models = cls.get_multi(entity_ids)
        return exploration_stats_models

    @classmethod
    def get_multi_stats_models(cls, exp_version_references: List[exp_domain
        .ExpVersionReference]) ->List[Optional[ExplorationStatsModel]]:
        ...
        """
        entity_ids = [cls.get_entity_id(exp_version_reference.exp_id,
            exp_version_reference.version) for exp_version_reference in
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{'exp_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'exp_version':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'num_starts_v1':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'num_starts_v2':
            base_models.EXPORT_POLICY.NOT_APPLICABLE,
            'num_actual_starts_v1': base_models.EXPORT_POLICY.
            NOT_APPLICABLE, 'num_actual_starts_v2': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'num_completions_v1': base_models
            .EXPORT_POLICY.NOT_APPLICABLE, 'num_completions_v2':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'state_stats_mapping':
            base_models.EXPORT_POLICY.NOT_APPLICABLE})
...
class ExplorationIssuesModel(base_models.BaseModel):
    ...
    exp_version = datastore_services.IntegerProperty(indexed=True, required
        =True)
    ...
    @classmethod
    def create(cls, exp_id: str, exp_version: int, unresolved_issues: List[
        stats_domain.ExplorationIssueDict]) ->str:
        ...
        """
        instance_id = cls.get_entity_id(exp_id, exp_version)
        exp_issues_instance = cls(id=instance_id, exp_id=exp_id,
            exp_version=exp_version, unresolved_issues=unresolved_issues)
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{'exp_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'exp_version':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'unresolved_issues':
            base_models.EXPORT_POLICY.NOT_APPLICABLE})
...
class PlaythroughModel(base_models.BaseModel):
    ...
    exp_version = datastore_services.IntegerProperty(indexed=True, required
        =True)
    issue_type = datastore_services.StringProperty(indexed=True, required=
        True, choices=ALLOWED_ISSUE_TYPES)
    ...
    def _generate_id(cls, exp_id: str) ->str:
        ...
        for _ in range(base_models.MAX_RETRIES):
            new_id = '%s.%s' % (exp_id, utils.convert_to_hash(str(utils.
    ...
    @classmethod
    def create(cls, exp_id: str, exp_version: int, issue_type: str,
        issue_customization_args: stats_domain.
        IssuesCustomizationArgsDictType, actions: List[stats_domain.
        LearnerActionDict]) ->str:
        ...
        cls(id=instance_id, exp_id=exp_id, exp_version=exp_version,
            issue_type=issue_type, issue_customization_args=
            issue_customization_args, actions=actions).put()
        return instance_id
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(PlaythroughModel, cls).get_export_policy(), **{
            ...
            'issue_customization_args': base_models.EXPORT_POLICY.
            NOT_APPLICABLE, 'actions': base_models.EXPORT_POLICY.
            NOT_APPLICABLE})
...
class LearnerAnswerDetailsModel(base_models.BaseModel):
    ...
    """
    state_reference = datastore_services.StringProperty(required=True,
        indexed=True)
    entity_type = datastore_services.StringProperty(required=True, indexed=
        True, choices=ALLOWED_ENTITY_TYPES)
    interaction_id = datastore_services.StringProperty(required=True,
        indexed=True)
    learner_answer_info_list = datastore_services.JsonProperty(repeated=
        True, indexed=False)
    learner_answer_info_schema_version = datastore_services.IntegerProperty(
        indexed=True, default=feconf.CURRENT_LEARNER_ANSWER_INFO_SCHEMA_VERSION
        )
    accumulated_answer_info_json_size_bytes = (datastore_services.
        IntegerProperty(indexed=True, required=False, default=0))
    ...
    @classmethod
    def get_state_reference_for_exploration(cls, exp_id: str, state_name: str
        ) ->str:
    ...
    @classmethod
    def create_model_instance(cls, entity_type: str, state_reference: str,
        interaction_id: str, learner_answer_info_list: List[stats_domain.
        LearnerAnswerInfo], learner_answer_info_schema_version: int,
        accumulated_answer_info_json_size_bytes: int) ->None:
        ...
        answer_details_instance = cls(id=instance_id, entity_type=
            entity_type, state_reference=state_reference, interaction_id=
            interaction_id, learner_answer_info_list=[learner_answer_info.
            to_dict() for learner_answer_info in learner_answer_info_list],
    ...
    @classmethod
    def get_model_instance(cls, entity_type: str, state_reference: str
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{
            ...
            'learner_answer_info_list': base_models.EXPORT_POLICY.
            NOT_APPLICABLE, 'learner_answer_info_schema_version':
            base_models.EXPORT_POLICY.NOT_APPLICABLE,
            'accumulated_answer_info_json_size_bytes': base_models.
            EXPORT_POLICY.NOT_APPLICABLE})
...
class ExplorationAnnotationsModel(base_models.BaseMapReduceBatchResultsModel):
    ...
    @classmethod
    def get_entity_id(cls, exploration_id: str, exploration_version: int
        ) ->str:
    ...
    @classmethod
    def create(cls, exp_id: str, version: str, num_starts: int,
        num_completions: int, state_hit_counts: Dict[str, int]) ->None:
        ...
        cls(id=entity_id, exploration_id=exp_id, version=version,
            num_starts=num_starts, num_completions=num_completions,
    ...
    def get_versions(cls, exploration_id: str) ->List[str]:
        ...
        annotations_result: Sequence[ExplorationAnnotationsModel
            ] = cls.get_all().filter(cls.exploration_id == exploration_id
...
class StateAnswersModel(base_models.BaseModel):
    ...
    interaction_id = datastore_services.StringProperty(indexed=True,
        required=True)
    shard_count = datastore_services.IntegerProperty(indexed=True, required
        =False)
    accumulated_answer_json_size_bytes = datastore_services.IntegerProperty(
        indexed=False, required=False, default=0)
    submitted_answer_list = datastore_services.JsonProperty(repeated=True,
        indexed=False)
    schema_version = datastore_services.IntegerProperty(indexed=True,
        default=feconf.CURRENT_STATE_ANSWERS_SCHEMA_VERSION)
    ...
    @classmethod
    def _get_model(cls, exploration_id: str, exploration_version: int,
        state_name: str, shard_id: int) ->Optional[StateAnswersModel]:
        ...
        """
        entity_id = cls._get_entity_id(exploration_id, exploration_version,
            state_name, shard_id)
        return cls.get(entity_id, strict=False)

    @classmethod
    def get_master_model(cls, exploration_id: str, exploration_version: int,
        state_name: str) ->Optional[StateAnswersModel]:
        ...
        """
        main_shard = cls._get_model(exploration_id, exploration_version,
            state_name, 0)
        return main_shard if main_shard else None

    @classmethod
    def get_all_models(cls, exploration_id: str, exploration_version: int,
        state_name: str) ->Optional[List[StateAnswersModel]]:
        ...
        """
        main_shard = cls.get_master_model(exploration_id,
            exploration_version, state_name)
        if main_shard is not None:
            all_models = [main_shard]
            if main_shard.shard_count > 0:
                shard_ids = [cls._get_entity_id(exploration_id,
                    exploration_version, state_name, shard_id) for shard_id in
                    range(1, main_shard.shard_count + 1)]
    ...
    def _insert_submitted_answers_unsafe_transactional(cls, exploration_id:
        str, exploration_version: int, state_name: str, interaction_id: str,
        ...
        if not main_shard:
            entity_id = cls._get_entity_id(exploration_id,
                exploration_version, state_name, 0)
            main_shard = cls(id=entity_id, exploration_id=exploration_id,
                exploration_version=exploration_version, state_name=
                state_name, shard_id=0, interaction_id=interaction_id,
                shard_count=0, submitted_answer_list=[])
            last_shard = main_shard
        elif main_shard.shard_count > 0:
            last_shard = cls._get_model(exploration_id, exploration_version,
                state_name, main_shard.shard_count)
        assert last_shard is not None
        sharded_answer_lists, sharded_answer_list_sizes = cls._shard_answers(
            last_shard.submitted_answer_list, last_shard.
            accumulated_answer_json_size_bytes, new_submitted_answer_dict_list)
        new_shard_count = main_shard.shard_count + (len(
            sharded_answer_lists) - 1)
        entities_to_put = []
        ...
        for i in range(1, len(sharded_answer_lists)):
            ...
            entity_id = cls._get_entity_id(exploration_id,
                exploration_version, state_name, shard_id)
            new_shard = cls(id=entity_id, exploration_id=exploration_id,
                exploration_version=exploration_version, state_name=
                state_name, shard_id=shard_id, interaction_id=
                interaction_id, submitted_answer_list=sharded_answer_lists[
                i], accumulated_answer_json_size_bytes=
                sharded_answer_list_sizes[i])
    ...
    @classmethod
    def insert_submitted_answers(cls, exploration_id: str,
        exploration_version: int, state_name: str, interaction_id: str,
        new_submitted_answer_dict_list: List[stats_domain.SubmittedAnswerDict]
        ) ->None:
        ...
        Args:
            ...
            new_submitted_answer_dict_list: list(dict). List of new submitted
                answers each of which is stored as a JSON blob.
        """
        cls._insert_submitted_answers_unsafe_transactional(exploration_id,
            exploration_version, state_name, interaction_id,
            new_submitted_answer_dict_list)

    @classmethod
    def _get_entity_id(cls, exploration_id: str, exploration_version: int,
        state_name: str, shard_id: int) ->str:
        ...
        """
        return ':'.join([exploration_id, str(exploration_version),
            state_name, str(shard_id)])

    @classmethod
    def _shard_answers(cls, current_answer_list: List[stats_domain.
        SubmittedAnswerDict], current_answer_list_size: int,
        new_answer_list: List[stats_domain.SubmittedAnswerDict]) ->Tuple[
        List[List[stats_domain.SubmittedAnswerDict]], List[int]]:
        ...
        """
        new_answer_size_list = [(answer_dict, cls._get_answer_dict_size(
            answer_dict)) for answer_dict in new_answer_list]
        new_answer_list_sorted = sorted(new_answer_size_list, key=lambda x:
            x[1])
        ...
        for answer_dict, answer_size in new_answer_list_sorted:
            if sharded_answer_list_sizes[-1
                ] + answer_size <= cls._MAX_ANSWER_LIST_BYTE_SIZE:
    ...
    @classmethod
    def _get_answer_dict_size(cls, answer_dict: stats_domain.
        SubmittedAnswerDict) ->int:
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{
            ...
            'accumulated_answer_json_size_bytes': base_models.EXPORT_POLICY
            .NOT_APPLICABLE, 'submitted_answer_list': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'schema_version': base_models.
            EXPORT_POLICY.NOT_APPLICABLE})
...
class StateAnswersCalcOutputModel(base_models.BaseMapReduceBatchResultsModel):
    ...
    exploration_id = datastore_services.StringProperty(indexed=True,
        required=True)
    exploration_version = datastore_services.StringProperty(indexed=True,
        required=True)
    state_name = datastore_services.StringProperty(indexed=True, required=True)
    interaction_id = datastore_services.StringProperty(indexed=True)
    calculation_id = datastore_services.StringProperty(indexed=True,
        required=True)
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{
            ...
            'calculation_output_type': base_models.EXPORT_POLICY.
            NOT_APPLICABLE, 'calculation_output': base_models.EXPORT_POLICY
            .NOT_APPLICABLE})