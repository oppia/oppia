...
class LearnerGoalsModel(base_models.BaseModel):
    """Keeps track of all the topics selected by the learner to keep a track of.
    ...
    @staticmethod
    def export_data(user_id):
        """(Takeout) Export LearnerGoalsModel's user properties.

        Args:
            user_id: str. The user_id denotes which user's data to extract.

        Returns:
            dict. A dict with one keys, 'topic_ids_to_learn'.
            The corresponding values are lists of the IDs of the
            topics which the given user has selected to keep a track of.
            If there is no model for the given user_id, the function returns
            an empty dict.
        """
