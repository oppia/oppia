...
from typing import (
    Dict, Final, List, Literal, Optional, Sequence, Tuple, TypedDict, Union, overload
)
...
class UserSettingsModel(base_models.BaseModel):
    ...
    default_dashboard = datastore_services.StringProperty(
        ...
        indexed=True,
        choices=[
            constants.DASHBOARD_TYPE_LEARNER, constants.DASHBOARD_TYPE_CREATOR,
    ...
    def export_data(
        ...
        return {
            ...
            'last_agreed_to_terms_msec': (
                ...
                if user.last_agreed_to_terms else None
            ),
            'last_started_state_editor_tutorial_msec': (
                utils.get_time_in_millisecs(user.last_started_state_editor_tutorial)
                if user.last_started_state_editor_tutorial else None
            ),
            'last_started_state_translation_tutorial_msec': (
                utils.get_time_in_millisecs(
                    user.last_started_state_translation_tutorial
                ) if user.last_started_state_translation_tutorial else None
            ),
            'last_logged_in_msec': (
                utils.get_time_in_millisecs(user.last_logged_in)
                if user.last_logged_in else None
            ),
            'last_edited_an_exploration_msec': (
                utils.get_time_in_millisecs(user.last_edited_an_exploration)
                if user.last_edited_an_exploration else None
            ),
            'last_created_an_exploration_msec': (
                utils.get_time_in_millisecs(user.last_created_an_exploration)
                if user.last_created_an_exploration else None
            ),
            'default_dashboard':
    ...
    def is_normalized_username_taken(cls, normalized_username: str) -> bool:
        ...
        return (
            cls.query().filter(cls.normalized_username == normalized_username
                              ).get() is not None or
            DeletedUsernameModel.get(hashed_normalized_username,
    ...
    def get_by_normalized_username(
        ...
        """
        return cls.get_all().filter(cls.normalized_username == normalized_username
                                   ).get()
...
class UserEmailPreferencesModel(base_models.BaseModel):
    ...
    def export_data(user_id: str) -> Dict[str, bool]:
        ...
        if user_email_preferences:
            return {
                'site_updates':
                    user_email_preferences.site_updates,
...
class UserSubscriptionsModel(base_models.BaseModel):
    ...
    def has_reference_to_user_id(cls, user_id: str) -> bool:
        ...
        """
        return (
            cls.query(cls.creator_ids == user_id).get(keys_only=True) is not None or
    ...
    def export_data(user_id: str) -> Dict[str, Union[List[str], float, None]]:
        ...
        user_data = {
            'exploration_ids':
                user_model.exploration_ids,
            'collection_ids':
                user_model.collection_ids,
...
class UserSubscribersModel(base_models.BaseModel):
    ...
    def has_reference_to_user_id(cls, user_id: str) -> bool:
        ...
        """
        return (
            cls.query(cls.subscriber_ids == user_id).get(keys_only=True) is not None or
            cls.get_by_id(user_id) is not None
        )
        ...
        for user_model in found_models:
            user_data[user_model.exploration_id] = {
                ...
                'rated_on_msec': (
                    ...
                    if user_model.rated_on else None
                ),
                'draft_change_list':
                ...
                'draft_change_list_last_updated_msec': (
                    ...
                    ) if user_model.draft_change_list_last_updated else None
                ),
                'draft_change_list_exp_version':
...
class CollectionProgressModel(base_models.BaseModel):
    ...
    def get_multi( # type: ignore[override]
        ...
        """
        instance_ids = [
            cls._generate_id(user_id, collection_id) for collection_id in collection_ids
        ]
...
class StoryProgressModel(base_models.BaseModel):
    ...
    @overload
    @classmethod
    def get(cls, user_id: str, story_id: str, *,
            strict: Literal[False]) -> Optional[StoryProgressModel]:
        ...
    ...
    def get(cls,
            ...
            story_id: str,
            *,
            strict: bool = ...) -> Optional[StoryProgressModel]:
        ...
...
class UserQueryModel(base_models.BaseModel):
    ...
    def fetch_page(
        ...
        fetch_result: Tuple[
            Sequence[UserQueryModel], datastore_services.Cursor,
...
class UserContributionProficiencyModel(base_models.BaseModel):
    ...
    def get_all_users_with_score_above_minimum_for_category(
        ...
        return cls.get_all().filter(
            datastore_services.all_of(
                cls.score_category == score_category, cls.score
                >= feconf.MINIMUM_SCORE_REQUIRED_TO_REVIEW
            )
        ).fetch()
...
class LearnerGroupsUserModel(base_models.BaseModel):
    ...
    learner_groups_user_details_schema_version = (
        datastore_services.IntegerProperty(required=True, default=0, indexed=True)
    )
    ...
    def export_data(
        cls, user_id: str
    ) -> LearnerGroupsUserDataDict:  # type: ignore[override]
...
class PinnedOpportunityModel(base_models.BaseModel):
    ...
    @classmethod
    def create(
        cls, user_id: str, language_code: str, topic_id: str, opportunity_id: str
    ...
    @classmethod
    def get_model(cls, user_id: str, language_code: str,
                  topic_id: str) -> Optional[PinnedOpportunityModel]:
