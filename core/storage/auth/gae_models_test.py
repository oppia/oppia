...
class UserAuthDetailsModelTests(test_utils.GenericTestBase):
    ...
    def test_export_data_trivial(self) -> None:
        ...
        exported_dict = (
            auth_models.UserAuthDetailsModel.export_data(self.NONEXISTENT_USER_ID)
        )
    ...
    def test_export_data_nontrivial(self) -> None:
        ...
        exported_dict = (
            auth_models.UserAuthDetailsModel.export_data(self.PROFILE_2_ID)
        )
        expected_dict = {
    ...
    def test_has_reference_to_existing_user_id_is_true(self) -> None:
        # For a full user.
        self.assertTrue(
            auth_models.UserAuthDetailsModel.has_reference_to_user_id(self.USER_ID)
        )

        # For a profile user.
        self.assertTrue(
            auth_models.UserAuthDetailsModel.has_reference_to_user_id(self.PROFILE_ID)
        )
...
class UserIdentifiersModelTests(test_utils.GenericTestBase):
    ...
    def test_has_reference_to_existing_user_id_is_true(self) -> None:
        # For a full user.
        self.assertTrue(
            auth_models.UserIdentifiersModel.has_reference_to_user_id(self.USER_ID)
        )
...
class UserIdByFirebaseAuthIdModelTests(test_utils.GenericTestBase):
    ...
    def test_apply_deletion_policy_for_registered_user_deletes_them(self) -> None:
        ...
        self.assertIsNone(
            auth_models.UserIdByFirebaseAuthIdModel.get(self.USER_ID, strict=False)
        )
...
class CsrfSecretModelUnitTests(test_utils.GenericTestBase):
    ...
    def test_get_export_policy(self) -> None:
        ...
        self.assertEqual(
            auth_models.CsrfSecretModel.get_export_policy(), expected_export_policy_dict
        )
