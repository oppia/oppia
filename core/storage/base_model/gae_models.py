...
from typing import Any, Dict, List, Mapping, Optional, Sequence, Tuple, Type, Union, TypeVar, cast, overload
SELF_BASE_MODEL = TypeVar('SELF_BASE_MODEL', bound='BaseModel')
SELF_BASE_HUMAN_MAINTAINED_MODEL = TypeVar('SELF_BASE_HUMAN_MAINTAINED_MODEL',
    bound='BaseHumanMaintainedModel')
SELF_BASE_COMMIT_LOG_ENTRY_MODEL = TypeVar('SELF_BASE_COMMIT_LOG_ENTRY_MODEL',
    bound='BaseCommitLogEntryModel')
SELF_VERSIONED_MODEL = TypeVar('SELF_VERSIONED_MODEL', bound='VersionedModel')
SELF_BASE_SNAPSHOT_METADATA_MODEL = TypeVar('SELF_BASE_SNAPSHOT_METADATA_MODEL'
    , bound='BaseSnapshotMetadataModel')
SELF_BASE_SNAPSHOT_CONTENT_MODEL = TypeVar('SELF_BASE_SNAPSHOT_CONTENT_MODEL',
    bound='BaseSnapshotContentModel')
MYPY = False
if MYPY:
    from core.domain import change_domain
    from mypy_imports import datastore_services
    from mypy_imports import transaction_services
    AllowedCommitCmdsListType = Sequence[Mapping[str, change_domain.
...
class BaseModel(datastore_services.Model):
    ...
    created_on = datastore_services.DateTimeProperty(indexed=True, required
        =True)
    ...
    def _pre_put_hook(self) ->None:
        ...
        if not self._last_updated_timestamp_is_fresh:
            raise Exception('%s did not call update_timestamps() yet' %
                type(self).__name__)
        self._last_updated_timestamp_is_fresh = False
    ...
    def get_deletion_policy() ->DELETION_POLICY:
        ...
        raise NotImplementedError(
            'The get_deletion_policy() method is missing from the derived class. It should be implemented in the derived class.'
            )
    ...
    def has_reference_to_user_id(cls, user_id: str) ->bool:
        ...
        """
        raise NotImplementedError(
            'The has_reference_to_user_id() method is missing from the derived class. It should be implemented in the derived class.'
            )
    ...
    def export_data(user_id: str) ->Dict[str, Any]:
        ...
        """
        raise NotImplementedError(
            'The export_data() method is missing from the derived class. It should be implemented in the derived class.'
            )
    ...
    def get_model_association_to_user() ->MODEL_ASSOCIATION_TO_USER:
        ...
        """
        raise NotImplementedError(
            'The get_model_association_to_user() method is missing from the derived class. It should be implemented in the derived class.'
            )

    @classmethod
    def get_export_policy(cls) ->Dict[str, EXPORT_POLICY]:
        """Model doesn't contain any data directly corresponding to a user."""
        return {'created_on': EXPORT_POLICY.NOT_APPLICABLE, 'last_updated':
            EXPORT_POLICY.NOT_APPLICABLE, 'deleted': EXPORT_POLICY.
            NOT_APPLICABLE}
    ...
    @overload
    @classmethod
    def get(cls: Type[SELF_BASE_MODEL], entity_id: str) ->SELF_BASE_MODEL:
        ...

    @overload
    @classmethod
    def get(cls: Type[SELF_BASE_MODEL], entity_id: str, *, strict: Literal[
        True]) ->SELF_BASE_MODEL:
        ...

    @overload
    @classmethod
    def get(cls: Type[SELF_BASE_MODEL], entity_id: str, *, strict: Literal[
        False]) ->Optional[SELF_BASE_MODEL]:
        ...

    @overload
    @classmethod
    def get(cls: Type[SELF_BASE_MODEL], entity_id: str, *, strict: bool=...
        ) ->Optional[SELF_BASE_MODEL]:
        ...

    @classmethod
    def get(cls: Type[SELF_BASE_MODEL], entity_id: str, strict: bool=True
        ) ->Optional[SELF_BASE_MODEL]:
        ...
        if strict and entity is None:
            raise cls.EntityNotFoundError(
                'Entity for class %s with id %s not found' % (cls.__name__,
                entity_id))
        return entity

    @classmethod
    def get_multi(cls: Type[SELF_BASE_MODEL], entity_ids: Sequence[Optional
        [str]], include_deleted: bool=False) ->List[Optional[SELF_BASE_MODEL]]:
    ...
    @classmethod
    def update_timestamps_multi(cls, entities: List[SELF_BASE_MODEL],
        ...
        Args:
            ...
            update_last_updated_time: bool. Whether to update the
                last_updated field of the model.
        """
        datastore_services.update_timestamps_multi(entities,
    ...
    @classmethod
    def get_all(cls: Type[SELF_BASE_MODEL], include_deleted: bool=False
        ...
        """
        return cls.query() if include_deleted else cls.query().filter(cls.
            deleted == False)
    ...
    def get_new_id(cls, entity_name: str) ->str:
        ...
        for _ in range(MAX_RETRIES):
            new_id = utils.convert_to_hash('%s%s' % (entity_name, utils.
                get_random_int(RAND_RANGE)), ID_LENGTH)
            if not cls.get_by_id(new_id):
                return new_id
        raise Exception('New id generator is producing too many collisions.')

    @classmethod
    def _fetch_page_sorted_by_last_updated(cls: Type[SELF_BASE_MODEL],
        query: datastore_services.Query, page_size: int,
        urlsafe_start_cursor: Optional[str]) ->Tuple[Sequence[
        SELF_BASE_MODEL], Optional[str], bool]:
        ...
        start_cursor: Optional[datastore_services.Cursor
            ] = datastore_services.make_cursor(urlsafe_cursor=
            urlsafe_start_cursor) if urlsafe_start_cursor else None
        last_updated_query = query.order(-cls.last_updated)
        fetch_result: Tuple[Sequence[SELF_BASE_MODEL], datastore_services.
            Cursor, bool] = last_updated_query.fetch_page(page_size,
            start_cursor=start_cursor)
        query_models, next_cursor, _ = fetch_result
        fetch_result = last_updated_query.fetch_page(page_size + 1,
            start_cursor=start_cursor)
        plus_one_query_models, _, _ = fetch_result
        return query_models, next_cursor.urlsafe().decode('utf-8'
            ) if next_cursor else None, len(plus_one_query_models
            ) == page_size + 1
...
class BaseHumanMaintainedModel(BaseModel):
    ...
    """
    last_updated_by_human = datastore_services.DateTimeProperty(indexed=
        True, required=True)
    ...
    @classmethod
    def put_multi_for_human(cls, instances: List[
        SELF_BASE_HUMAN_MAINTAINED_MODEL]) ->None:
    ...
    @classmethod
    def put_multi_for_bot(cls, instances: List[
        SELF_BASE_HUMAN_MAINTAINED_MODEL]) ->None:
...
class BaseCommitLogEntryModel(BaseModel):
    ...
    post_commit_status = datastore_services.StringProperty(indexed=True,
        required=True)
    post_commit_community_owned = datastore_services.BooleanProperty(indexed
        =True)
    ...
    def get_export_policy(cls) ->Dict[str, EXPORT_POLICY]:
        ...
        """
        return dict(BaseModel.get_export_policy(), **{'user_id':
            EXPORT_POLICY.NOT_APPLICABLE, 'commit_type': EXPORT_POLICY.
            NOT_APPLICABLE, 'commit_message': EXPORT_POLICY.NOT_APPLICABLE,
    ...
    def has_reference_to_user_id(cls, user_id: str) ->bool:
        ...
        """
        return cls.query(cls.user_id == user_id).get(keys_only=True
            ) is not None

    @classmethod
    def create(cls: Type[SELF_BASE_COMMIT_LOG_ENTRY_MODEL], entity_id: str,
        version: int, committer_id: str, commit_type: str, commit_message:
        Optional[str], commit_cmds: AllowedCommitCmdsListType, status: str,
        community_owned: bool) ->SELF_BASE_COMMIT_LOG_ENTRY_MODEL:
        ...
        """
        return cls(id=cls.get_instance_id(entity_id, version), user_id=
            committer_id, commit_type=commit_type, commit_message=
            commit_message, commit_cmds=commit_cmds, version=version,
            post_commit_status=status, post_commit_community_owned=
            community_owned, post_commit_is_private=status == constants.
    ...
    def get_instance_id(cls, target_entity_id: str, version: int) ->str:
        ...
        """
        raise NotImplementedError(
            'The get_instance_id() method is missing from the derived class. It should be implemented in the derived class.'
            )

    @classmethod
    def get_all_commits(cls: Type[SELF_BASE_COMMIT_LOG_ENTRY_MODEL],
        page_size: int, urlsafe_start_cursor: Optional[str]) ->Tuple[
        Sequence[SELF_BASE_COMMIT_LOG_ENTRY_MODEL], Optional[str], bool]:
        ...
        Returns:
            ...
            where:
                ...
                more: bool. If True, there are (probably) more results after
                    ...
                    this batch.
        """
        return cls._fetch_page_sorted_by_last_updated(cls.query(),
            page_size, urlsafe_start_cursor)

    @classmethod
    def get_commit(cls: Type[SELF_BASE_COMMIT_LOG_ENTRY_MODEL],
        target_entity_id: str, version: int) ->Optional[
...
class VersionedModel(BaseModel):
    ...
    COMMIT_TYPE_CHOICES: List[str] = [feconf.COMMIT_TYPE_CREATE, feconf.
        COMMIT_TYPE_REVERT, feconf.COMMIT_TYPE_EDIT, feconf.COMMIT_TYPE_DELETE]
    ...
    def _reconstitute(self: SELF_VERSIONED_MODEL, snapshot_dict: Dict[str, Any]
    ...
    def _reconstitute_from_snapshot_id(self: SELF_VERSIONED_MODEL,
        snapshot_id: str) ->SELF_VERSIONED_MODEL:
    ...
    def compute_models_to_commit(self, committer_id: str, commit_type: str,
        commit_message: Optional[str], commit_cmds:
        AllowedCommitCmdsListType, unused_additional_models: Mapping[str,
        BaseModel]) ->ModelsToPutDict:
        ...
        return {'snapshot_metadata_model': snapshot_metadata_instance,
            'snapshot_content_model': snapshot_content_instance,
            'versioned_model': self}

    def delete(self, committer_id: str, commit_message: str, force_deletion:
        bool=False) ->None:
        ...
        if force_deletion:
            ...
            snapshot_ids = [self.get_snapshot_id(self.id, version_number) for
                version_number in version_numbers]
            metadata_keys = [datastore_services.Key(self.
                SNAPSHOT_METADATA_CLASS, snapshot_id) for snapshot_id in
                snapshot_ids]
            content_keys = [datastore_services.Key(self.
                SNAPSHOT_CONTENT_CLASS, snapshot_id) for snapshot_id in
                snapshot_ids]
            commit_log_keys: List[datastore_services.Key] = []
            if self.COMMIT_LOG_ENTRY_CLASS is not None:
                commit_log_ids = (self.COMMIT_LOG_ENTRY_CLASS.
                    get_instance_id(self.id, version_number) for
                    version_number in version_numbers)
                commit_log_keys = [datastore_services.Key(self.
                    COMMIT_LOG_ENTRY_CLASS, commit_log_id) for
                    commit_log_id in commit_log_ids]
            datastore_services.delete_multi(content_keys + metadata_keys +
                commit_log_keys)
            super().delete()
        else:
            ...
            models_to_put = self.compute_models_to_commit(committer_id,
                feconf.COMMIT_TYPE_DELETE, commit_message, commit_cmds,
    ...
    @classmethod
    def delete_multi(cls, entity_ids: List[str], committer_id: str,
        commit_message: str, force_deletion: bool=False) ->None:
        ...
        Raises:
            Exception. This model instance has been already deleted.
        """
        versioned_models_with_none = cls.get_multi(entity_ids,
            include_deleted=force_deletion)
        versioned_models: List[VersionedModel] = [versioned_model for
            versioned_model in versioned_models_with_none if 
            versioned_model is not None]
        if force_deletion:
            ...
            for model in versioned_models:
                ...
                model_snapshot_ids = [model.get_snapshot_id(model.id,
                    version_number) for version_number in model_version_numbers
                    ]
                all_models_metadata_keys.extend([datastore_services.Key(
                    model.SNAPSHOT_METADATA_CLASS, snapshot_id) for
                    snapshot_id in model_snapshot_ids])
                all_models_content_keys.extend([datastore_services.Key(
                    model.SNAPSHOT_CONTENT_CLASS, snapshot_id) for
                    snapshot_id in model_snapshot_ids])
                if model.COMMIT_LOG_ENTRY_CLASS is not None:
                    assert cls.COMMIT_LOG_ENTRY_CLASS is not None
                    commit_log_ids = (cls.COMMIT_LOG_ENTRY_CLASS.
                        get_instance_id(model.id, version_number) for
                        version_number in model_version_numbers)
                    all_models_commit_keys.extend([datastore_services.Key(
                        model.COMMIT_LOG_ENTRY_CLASS, commit_log_id) for
                        commit_log_id in commit_log_ids])
            versioned_models_keys = [model.key for model in versioned_models]
            all_models_keys = (all_models_metadata_keys +
                all_models_content_keys + all_models_commit_keys +
                versioned_models_keys)
            for i in range(0, len(all_models_keys), feconf.
                MAX_NUMBER_OF_OPS_IN_TRANSACTION):
                datastore_services.delete_multi_transactional(all_models_keys
                    [i:i + feconf.MAX_NUMBER_OF_OPS_IN_TRANSACTION])
        else:
            for model in versioned_models:
                model._require_not_marked_deleted()
                model.deleted = True
            ...
            for model in versioned_models:
                ...
                snapshot_metadata_models.append(model.
                    SNAPSHOT_METADATA_CLASS.create(snapshot_id,
                    committer_id, feconf.COMMIT_TYPE_DELETE, commit_message,
                    commit_cmds))
                assert model.SNAPSHOT_CONTENT_CLASS is not None
                snapshot_content_models.append(model.SNAPSHOT_CONTENT_CLASS
                    .create(snapshot_id, snapshot))
    ...
    def put(self, *args: Any, **kwargs: Any) ->None:
        ...
        raise NotImplementedError(
            'The put() method is missing from the derived class. It should be implemented in the derived class.'
            )

    def get_models_to_put_values(self, committer_id: str, commit_message:
        Optional[str], commit_cmds: Sequence[Mapping[str, change_domain.
        AcceptableChangeDictTypes]]) ->List[BaseModel]:
        ...
        """
        commit_type = (feconf.COMMIT_TYPE_CREATE if self.version == 0 else
            feconf.COMMIT_TYPE_EDIT)
        models_to_put = self.compute_models_to_commit(committer_id,
            commit_type, commit_message, commit_cmds, self.
            _prepare_additional_models()).values()
        models_to_put_values = []
    ...
    def commit(self, committer_id: str, commit_message: Optional[str],
        commit_cmds: AllowedCommitCmdsListType) ->None:
        ...
        for commit_cmd in commit_cmds:
            ...
            if commit_cmd['cmd'].startswith(self._AUTOGENERATED_PREFIX):
                raise Exception('Invalid change list command: %s' %
                    commit_cmd['cmd'])
        models_to_put = self.get_models_to_put_values(committer_id,
            commit_message, commit_cmds)
        BaseModel.update_timestamps_multi(models_to_put)
        BaseModel.put_multi_transactional(models_to_put)

    @classmethod
    def revert(cls: Type[SELF_VERSIONED_MODEL], model: SELF_VERSIONED_MODEL,
        committer_id: str, commit_message: str, version_number: int) ->None:
        ...
        commit_cmds: List[Dict[str, Union[str, int]]] = [{'cmd': model.
            CMD_REVERT_COMMIT, 'version_number': version_number}]
        ...
        models_to_put = new_model.compute_models_to_commit(committer_id,
            feconf.COMMIT_TYPE_REVERT, commit_message, commit_cmds,
    ...
    @overload
    @classmethod
    def get_version(cls: Type[SELF_VERSIONED_MODEL], entity_id: str,
        version_number: int) ->SELF_VERSIONED_MODEL:
        ...

    @overload
    @classmethod
    def get_version(cls: Type[SELF_VERSIONED_MODEL], entity_id: str,
        version_number: int, *, strict: Literal[True]) ->SELF_VERSIONED_MODEL:
        ...

    @overload
    @classmethod
    def get_version(cls: Type[SELF_VERSIONED_MODEL], entity_id: str,
        version_number: int, *, strict: Literal[False]) ->Optional[
        SELF_VERSIONED_MODEL]:
        ...
    ...
    @classmethod
    def get_version(cls: Type[SELF_VERSIONED_MODEL], entity_id: str,
        version_number: int, *, strict: bool=...) ->Optional[
        SELF_VERSIONED_MODEL]:
        ...

    @classmethod
    def get_version(cls: Type[SELF_VERSIONED_MODEL], entity_id: str,
        version_number: int, strict: bool=True) ->Optional[SELF_VERSIONED_MODEL
        ]:
        ...
        try:
            return cls(id=entity_id, version=version_number
    ...
    @classmethod
    def get_multi_versions(cls: Type[SELF_VERSIONED_MODEL], entity_id: str,
        ...
        if max_version > current_version:
            raise ValueError(
                'Requested version number %s cannot be higher than the current version number %s.'
        ...
        for snapshot_model in snapshot_models:
            ...
            snapshot_dict = snapshot_model.content
            reconstituted_model = cls(id=entity_id)._reconstitute(snapshot_dict
                )
            reconstituted_model.created_on = snapshot_model.created_on
            reconstituted_model.last_updated = snapshot_model.last_updated
    ...
    @overload
    @classmethod
    def get(cls: Type[SELF_VERSIONED_MODEL], entity_id: str
        ) ->SELF_VERSIONED_MODEL:
        ...

    @overload
    @classmethod
    def get(cls: Type[SELF_VERSIONED_MODEL], entity_id: str, *, strict:
        Literal[True], version: Optional[int]=None) ->SELF_VERSIONED_MODEL:
        ...

    @overload
    @classmethod
    def get(cls: Type[SELF_VERSIONED_MODEL], entity_id: str, *, strict:
        Literal[False], version: Optional[int]=None) ->Optional[
        SELF_VERSIONED_MODEL]:
        ...
    ...
    @classmethod
    def get(cls: Type[SELF_VERSIONED_MODEL], entity_id: str, *, strict:
        bool=..., version: Optional[int]=None) ->Optional[SELF_VERSIONED_MODEL
        ]:
        ...

    @classmethod
    def get(cls: Type[SELF_VERSIONED_MODEL], entity_id: str, strict: bool=
        True, version: Optional[int]=None) ->Optional[SELF_VERSIONED_MODEL]:
        """Gets model instance.
    ...
    @classmethod
    def get_snapshots_metadata(cls, model_instance_id: str, version_numbers:
        List[int], allow_deleted: bool=False) ->List[SnapshotsMetadataDict]:
        ...
        if not allow_deleted:
            ...
            model._require_not_marked_deleted()
        snapshot_ids = [cls.get_snapshot_id(model_instance_id,
            version_number) for version_number in version_numbers]
        metadata_keys = [datastore_services.Key(cls.SNAPSHOT_METADATA_CLASS,
        ...
        returned_models_without_none = cast(List[BaseSnapshotMetadataModel],
            returned_models)
        return [{'committer_id': model.committer_id, 'commit_message':
            model.commit_message, 'commit_cmds': model.commit_cmds,
            'commit_type': model.commit_type, 'version_number':
            version_numbers[ind], 'created_on_ms': utils.
            get_time_in_millisecs(model.created_on)} for ind, model in
            enumerate(returned_models_without_none)]
    ...
    def get_export_policy(cls) ->Dict[str, EXPORT_POLICY]:
        ...
        Takeout, since commits do not contain any personal user data.
        """
        return dict(BaseModel.get_export_policy(), **{'version':
...
class BaseSnapshotMetadataModel(BaseModel):
    ...
    committer_id = datastore_services.StringProperty(required=True, indexed
        =True)
    commit_type = datastore_services.StringProperty(required=True, choices=
        VersionedModel.COMMIT_TYPE_CHOICES)
    commit_message = datastore_services.StringProperty(indexed=True)
    commit_cmds = datastore_services.JsonProperty(indexed=False)
    commit_cmds_user_ids = datastore_services.StringProperty(repeated=True,
        indexed=True)
    content_user_ids = datastore_services.StringProperty(repeated=True,
        indexed=True)
    ...
    def get_export_policy(cls) ->Dict[str, EXPORT_POLICY]:
        ...
        return dict(BaseModel.get_export_policy(), **{'committer_id':
            EXPORT_POLICY.NOT_APPLICABLE, 'commit_type': EXPORT_POLICY.
            EXPORTED, 'commit_message': EXPORT_POLICY.EXPORTED,
            'commit_cmds': EXPORT_POLICY.NOT_APPLICABLE,
    ...
    def has_reference_to_user_id(cls, user_id: str) ->bool:
        ...
        Returns:
            bool. Whether any models refer to the given user ID.
        """
        return cls.query(datastore_services.any_of(cls.committer_id ==
            user_id, cls.commit_cmds_user_ids == user_id, cls.
            content_user_ids == user_id)).get(keys_only=True) is not None

    @classmethod
    def create(cls: Type[SELF_BASE_SNAPSHOT_METADATA_MODEL], snapshot_id:
        str, committer_id: str, commit_type: str, commit_message: Optional[
        str], commit_cmds: AllowedCommitCmdsListType
        ...
        Returns:
            ...
            is called.
        """
        return cls(id=snapshot_id, committer_id=committer_id, commit_type=
            commit_type, commit_message=commit_message, commit_cmds=commit_cmds
            )
    ...
    def export_data(cls, user_id: str) ->Dict[str, Dict[str, str]]:
        metadata_models: Sequence[BaseSnapshotMetadataModel] = cls.query(
            cls.committer_id == user_id).fetch(projection=[cls.commit_type,
            cls.commit_message])
        user_data = {}
        for metadata_model in metadata_models:
            message_without_user_ids = None
            if metadata_model.commit_message is not None:
                message_without_user_ids = re.sub(feconf.USER_ID_REGEX,
                    '<user ID>', metadata_model.commit_message)
            user_data[metadata_model.id] = {'commit_type': metadata_model.
                commit_type, 'commit_message': message_without_user_ids}
        return user_data
...
class BaseSnapshotContentModel(BaseModel):
    ...
    def get_export_policy(cls) ->Dict[str, EXPORT_POLICY]:
        ...
        Takeout.
        """
        return dict(BaseModel.get_export_policy(), **{'content':
            EXPORT_POLICY.NOT_APPLICABLE})

    @classmethod
    def create(cls: Type[SELF_BASE_SNAPSHOT_CONTENT_MODEL], snapshot_id:
        str, content: Dict[str, Any]) ->SELF_BASE_SNAPSHOT_CONTENT_MODEL:
