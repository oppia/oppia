...
class BaseModel(datastore_services.Model):
    ...
    def get_multi(
        ...
        entities: List[Optional[SELF_BASE_MODEL]] = datastore_services.get_multi(
            entity_keys
        )
    ...
    def update_timestamps_multi(
        cls, entities: List[SELF_BASE_MODEL], update_last_updated_time: bool = True
    ) -> None:
    ...
    def get_all(
        ...
        """
        return (
            cls.query() if include_deleted else cls.query().filter(cls.deleted == False)
...
class VersionedModel(BaseModel):
    ...
    def delete(  # type: ignore[override]
        ...
        if force_deletion:
            ...
            if self.COMMIT_LOG_ENTRY_CLASS is not None:
                commit_log_ids = (
                    self.COMMIT_LOG_ENTRY_CLASS.get_instance_id(self.id, version_number)
                    for version_number in version_numbers
                )
                commit_log_keys = [
    ...
    def get_export_policy(cls) -> Dict[str, EXPORT_POLICY]:
        ...
        """
        return dict(
            BaseModel.get_export_policy(), **{'version': EXPORT_POLICY.NOT_APPLICABLE}
        )
...
class BaseSnapshotContentModel(BaseModel):
    ...
    def get_export_policy(cls) -> Dict[str, EXPORT_POLICY]:
        ...
        """
        return dict(
            BaseModel.get_export_policy(), **{'content': EXPORT_POLICY.NOT_APPLICABLE}
        )
