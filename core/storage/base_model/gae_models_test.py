...
class BaseModelUnitTests(test_utils.GenericTestBase):
    ...
    def test_put_multi(self) -> None:
        ...
        models_2_without_none = cast(
            List[base_models.BaseModel], base_models.BaseModel.get_multi(model_ids)
        )
        base_models.BaseModel.update_timestamps_multi(
...
class BaseHumanMaintainedModelTests(test_utils.GenericTestBase):
    ...
    def test_put_multi(self) -> None:
        with self.assertRaisesRegex(
            NotImplementedError, 'Use put_multi_for_human or put_multi_for_bot instead'
        ):
...
class BaseSnapshotMetadataModelTests(test_utils.GenericTestBase):
    ...
    def test_has_reference_to_user_id(self) -> None:
        ...
        self.assertFalse(
            base_models.BaseSnapshotMetadataModel.has_reference_to_user_id('x_id')
        )
...
class VersionedModelTests(test_utils.GenericTestBase):
    ...
    def test_revert_raises_error_when_not_allowed(self) -> None:
        model1 = TestVersionedModel(id='model_id1')
        with self.assertRaisesRegex(
            Exception, 'Reverting objects of type TestVersionedModel is not allowed.'
        ):
    ...
    def test_get_snapshots_metadata_with_invalid_model_raises_error(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception, 'Invalid version number 10 for model TestVersionedModel with id '
            'model_id1'
        ):
    ...
    def test_get_multi_versions_errors(self) -> None:
        ...
        with self.assertRaisesRegex(
            ValueError, 'Requested version number 3 cannot be higher than the current '
            'version number 2.'
        ):
