...
class CollectionModel(base_models.VersionedModel):
    ...
    def compute_models_to_commit(  # type: ignore[override]
        ...
        collection_commit_log = CollectionCommitLogEntryModel.create(
            ...
            commit_cmds, collection_rights_model.status,
    ...
    def delete_multi( # type: ignore[override]
        ...
        super(CollectionModel, cls).delete_multi(
            entity_ids, committer_id, commit_message, force_deletion=force_deletion
        )
        ...
        if not force_deletion:
            ...
            for model, rights_model in zip(versioned_models, collection_rights_models):
                ...
                collection_commit_log = CollectionCommitLogEntryModel.create(
                    model.id, model.version, committer_id, feconf.COMMIT_TYPE_DELETE,
                    commit_message, [{
...
class CollectionRightsModel(base_models.VersionedModel):
    ...
    status = datastore_services.StringProperty(
        ...
        indexed=True,
        choices=[constants.ACTIVITY_STATUS_PRIVATE, constants.ACTIVITY_STATUS_PUBLIC]
    )
    ...
    def save(
        self, committer_id: str, commit_message: str,
        commit_cmds: base_models.AllowedCommitCmdsListType
    ) -> None:
    ...
    def convert_to_valid_dict(model_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # user IDs.
        for field_name in ('owner_ids', 'editor_ids', 'voice_artist_ids', 'viewer_ids'):
...
class CollectionSummaryModel(base_models.BaseModel):
    ...
    def has_reference_to_user_id(cls, user_id: str) -> bool:
        ...
        return cls.query(
            datastore_services.any_of(
                cls.owner_ids == user_id, cls.editor_ids == user_id,
                cls.viewer_ids == user_id, cls.contributor_ids == user_id
            )
        ).get(keys_only=True) is not None
    ...
    def get_non_private(cls) -> Sequence[CollectionSummaryModel]:
        ...
        """
        return cls.get_all().filter(cls.status != constants.ACTIVITY_STATUS_PRIVATE
                                   ).fetch(feconf.DEFAULT_QUERY_LIMIT)

    @classmethod
    def get_private_at_least_viewable(cls,
                                      user_id: str) -> Sequence[CollectionSummaryModel]:
