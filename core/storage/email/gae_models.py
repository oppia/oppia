...
class SentEmailModel(base_models.BaseModel):
    ...
    """
    recipient_id = datastore_services.StringProperty(required=True, indexed
        =True)
    ...
    intent = datastore_services.StringProperty(required=True, indexed=True,
        choices=[feconf.EMAIL_INTENT_SIGNUP, feconf.EMAIL_INTENT_MARKETING,
        feconf.EMAIL_INTENT_DAILY_BATCH, feconf.
        EMAIL_INTENT_EDITOR_ROLE_NOTIFICATION, feconf.
        EMAIL_INTENT_FEEDBACK_MESSAGE_NOTIFICATION, feconf.
        EMAIL_INTENT_SUBSCRIPTION_NOTIFICATION, feconf.
        EMAIL_INTENT_SUGGESTION_NOTIFICATION, feconf.
        EMAIL_INTENT_UNPUBLISH_EXPLORATION, feconf.
        EMAIL_INTENT_DELETE_EXPLORATION, feconf.
        EMAIL_INTENT_REPORT_BAD_CONTENT, feconf.
        EMAIL_INTENT_QUERY_STATUS_NOTIFICATION, feconf.
        EMAIL_INTENT_ONBOARD_CD_USER, feconf.EMAIL_INTENT_REMOVE_CD_USER,
        feconf.EMAIL_INTENT_ADDRESS_CONTRIBUTOR_DASHBOARD_SUGGESTIONS,
        feconf.EMAIL_INTENT_REVIEW_CREATOR_DASHBOARD_SUGGESTIONS, feconf.
        EMAIL_INTENT_REVIEW_CONTRIBUTOR_DASHBOARD_SUGGESTIONS, feconf.
        EMAIL_INTENT_ADD_CONTRIBUTOR_DASHBOARD_REVIEWERS, feconf.
        EMAIL_INTENT_ACCOUNT_DELETED, feconf.BULK_EMAIL_INTENT_TEST, feconf
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        """
        return dict(super(cls, cls).get_export_policy(), **{'recipient_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'recipient_email':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'sender_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'sender_email':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'intent': base_models
            .EXPORT_POLICY.NOT_APPLICABLE, 'subject': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'html_body': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'sent_datetime': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'email_hash': base_models.
    ...
    def apply_deletion_policy(cls, user_id: str) ->None:
        ...
        """
        keys = cls.query(datastore_services.any_of(cls.recipient_id ==
            user_id, cls.sender_id == user_id)).fetch(keys_only=True)
    ...
    def has_reference_to_user_id(cls, user_id: str) ->bool:
        ...
        """
        return cls.query(datastore_services.any_of(cls.recipient_id ==
            user_id, cls.sender_id == user_id)).get(keys_only=True) is not None
    ...
    def _generate_id(cls, intent: str) ->str:
        ...
        for _ in range(base_models.MAX_RETRIES):
            new_id = '%s.%s' % (id_prefix, utils.convert_to_hash(str(utils.
                get_random_int(base_models.RAND_RANGE)), base_models.ID_LENGTH)
                )
            if not cls.get_by_id(new_id):
                return new_id
        raise Exception(
            'The id generator for SentEmailModel is producing too many collisions.'
            )
    ...
    def create(cls, recipient_id: str, recipient_email: str, sender_id: str,
        ...
        email_model_instance = cls(id=instance_id, recipient_id=
            recipient_id, recipient_email=recipient_email, sender_id=
            sender_id, sender_email=sender_email, intent=intent, subject=
            subject, html_body=html_body, sent_datetime=sent_datetime)
    ...
    def _pre_put_hook(self) ->None:
        ...
        super()._pre_put_hook()
        self.email_hash = self._generate_hash(self.recipient_id, self.
            subject, self.html_body)

    @classmethod
    def get_by_hash(cls, email_hash: str, sent_datetime_lower_bound:
        Optional[datetime.datetime]=None) ->Sequence[SentEmailModel]:
        ...
        if sent_datetime_lower_bound is not None:
            if not isinstance(sent_datetime_lower_bound, datetime.datetime):
                raise Exception('Expected datetime, received %s of type %s' %
                    (sent_datetime_lower_bound, type(
    ...
    def _generate_hash(cls, recipient_id: str, email_subject: str,
        ...
        """
        hash_value = utils.convert_to_hash(recipient_id + email_subject +
            email_body, 100)
        return hash_value
    ...
    def check_duplicate_message(cls, recipient_id: str, email_subject: str,
        ...
        time_interval = datetime.timedelta(minutes=feconf.
            DUPLICATE_EMAIL_INTERVAL_MINS)
        sent_datetime_lower_bound = datetime_now - time_interval
        messages = cls.get_by_hash(email_hash, sent_datetime_lower_bound=
            sent_datetime_lower_bound)
        for message in messages:
            if (message.recipient_id == recipient_id and message.subject ==
                email_subject and message.html_body == email_body):
                return True
        return False
...
class BulkEmailModel(base_models.BaseModel):
    ...
    sender_email = datastore_services.StringProperty(required=True)
    intent = datastore_services.StringProperty(required=True, indexed=True,
        choices=[feconf.BULK_EMAIL_INTENT_MARKETING, feconf.
        BULK_EMAIL_INTENT_IMPROVE_EXPLORATION, feconf.
        BULK_EMAIL_INTENT_CREATE_EXPLORATION, feconf.
        BULK_EMAIL_INTENT_CREATOR_REENGAGEMENT, feconf.
        BULK_EMAIL_INTENT_LEARNER_REENGAGEMENT, feconf.
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        (since emails were sent to them).
        """
        return dict(super(cls, cls).get_export_policy(), **{'sender_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'sender_email':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'recipient_ids':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'intent': base_models
            .EXPORT_POLICY.NOT_APPLICABLE, 'subject': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'html_body': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'sent_datetime': base_models.
    ...
    def apply_deletion_policy(cls, user_id: str) ->None:
        ...
        Args:
            user_id: str. The ID of the user whose data should be deleted.
        """
        keys = cls.query(datastore_services.any_of(cls.sender_id == user_id)
    ...
    @classmethod
    def create(cls, instance_id: str, sender_id: str, sender_email: str,
        intent: str, subject: str, html_body: str, sent_datetime: datetime.
        datetime) ->None:
        ...
        Args:
            ...
            sent_datetime: datetime.datetime. The date and time the email
                was sent, in UTC.
        """
        email_model_instance = cls(id=instance_id, sender_id=sender_id,
