...
class ClassroomModelUnitTest(test_utils.GenericTestBase):
    ...
    def test_create_new_model(self) -> None:
        classroom_id = (classroom_models.ClassroomModel.generate_new_classroom_id())
        classroom_model_instance = (
            classroom_models.ClassroomModel.create(
                classroom_id, 'physics', 'physics', 'Curated physics course.',
                'Learn physics through fun stories!', 'Start from the basic physics.',
                {}, False, 'thumbnail.svg', 'transparent', 1000, 'banner.png',
                'transparent', 1000, 0
            )
        )
        ...
        self.assertEqual(
            classroom_model_instance.teaser_text, 'Learn physics through fun stories!'
        )
        self.assertEqual(
            classroom_model_instance.topic_list_intro, 'Start from the basic physics.'
        )
        self.assertEqual(classroom_model_instance.is_published, False)
        self.assertEqual(classroom_model_instance.thumbnail_filename, 'thumbnail.svg')
        self.assertEqual(classroom_model_instance.thumbnail_bg_color, 'transparent')
        self.assertEqual(classroom_model_instance.thumbnail_size_in_bytes, 1000)
        self.assertEqual(classroom_model_instance.banner_filename, 'banner.png')
        self.assertEqual(classroom_model_instance.banner_bg_color, 'transparent')
    ...
    def test_get_model_association_to_user_not_corresponding_to_user(self) -> None:
        self.assertEqual(
    ...
    def test_get_model_by_url_fragment(self) -> None:
        ...
        self.assertEqual(
            ...
            get_by_url_fragment('incorrect_url_fragment'), None
        )

    def test_get_model_by_name(self) -> None:
        self.assertEqual(
            classroom_models.ClassroomModel.get_by_name('math'), self.classroom_model
        )
        self.assertEqual(
            classroom_models.ClassroomModel.get_by_name('incorrect_name'), None
        )

    def test_get_model_by_id(self) -> None:
        self.assertEqual(
            classroom_models.ClassroomModel.get_by_id('id'), self.classroom_model
        )
        self.assertEqual(
            classroom_models.ClassroomModel.get_by_id('incorrect_id'), None
        )
        ...
        with self.assertRaisesRegex(
            ...
            with self.swap(
                ...
                classroom_model_cls.create(
                    ...
                    'Learn math through fun stories!', 'Start from the basic math.', {},
                    True, 'thumbnail.svg', 'transparent', 1000, 'banner.png',
                    'transparent', 1000, 0
                )

        # Test generate_new_classroom_id method.
        with self.assertRaisesRegex(
            Exception, 'New classroom id generator is producing too many collisions.'
        ):
            # Swap dependent method get_by_id to simulate collision every time.
            with self.swap(
                classroom_model_cls, 'get_by_id',
