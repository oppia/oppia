...
class ExplorationStatsTaskEntryModel(base_models.BaseModel):
    ...
    composite_entity_id = datastore_services.StringProperty(required=True, indexed=True)

    # The type of entity a task entry refers to.
    entity_type = datastore_services.StringProperty(required=True, indexed=True, choices=TASK_ENTITY_TYPES)
    ...
    task_type = datastore_services.StringProperty(required=True, indexed=True, choices=TASK_TYPES)
    ...
    target_type = datastore_services.StringProperty(required=True, indexed=True, choices=TASK_TARGET_TYPES)
    ...
    issue_description = datastore_services.StringProperty(default=None, required=False, indexed=True)
    # Tracks the state/progress of a task entry.
    status = datastore_services.StringProperty(required=True, indexed=True, choices=TASK_STATUS_CHOICES)
    # ID of the user who closed the task, if any.
    resolver_id = datastore_services.StringProperty(default=None, required=False, indexed=True)
    # The date and time at which a task was closed or deprecated.
    resolved_on = datastore_services.DateTimeProperty(default=None, required=False, indexed=True)
    ...
    def apply_deletion_policy(cls, user_id: str) -> None:
        ...
        """
        task_entry_keys = cls.query(cls.resolver_id == user_id).fetch(keys_only=True)
    ...
    def export_data(user_id: str) -> Dict[str, List[str]]:
        ...
        """
        task_ids_resolved_by_user = ExplorationStatsTaskEntryModel.query(ExplorationStatsTaskEntryModel.resolver_id == user_id)
        return {
            'task_ids_resolved_by_user': ([t.id for t in task_ids_resolved_by_user]),
            'issue_descriptions': ([t.issue_description for t in task_ids_resolved_by_user]),
            'statuses': ([t.status for t in task_ids_resolved_by_user]),
            'resolution_msecs': ([t.resolved_on for t in task_ids_resolved_by_user]),
        }
    ...
    @classmethod
    def generate_composite_entity_id(cls, entity_type: str, entity_id: str, entity_version: int) -> str:
    ...
    def create(
        ...
        composite_entity_id = cls.generate_composite_entity_id(entity_type, entity_id, entity_version)
        task_entry = cls(
            id=task_id,
