...
class GeneralFeedbackThreadModel(base_models.BaseModel):
    ...
    has_suggestion = datastore_services.BooleanProperty(indexed=True, default=False, required=True)
    ...
    def export_data(cls, user_id: str) -> Dict[str, Dict[str, Union[str, bool, None]]]:
        ...
        user_data = {}
        feedback_models: Sequence[GeneralFeedbackThreadModel] = cls.get_all().filter(cls.original_author_id == user_id).fetch()
        ...
        for feedback_model in feedback_models:
            user_data[feedback_model.id] = {
                ...
                'last_updated_msec': utils.get_time_in_millisecs(feedback_model.last_updated),
            }
    ...
    def get_threads(
        ...
        """
        return cls.get_all().filter(cls.entity_type == entity_type).filter(cls.entity_id == entity_id).order(-cls.last_updated).fetch(limit)
...
class GeneralFeedbackMessageModel(base_models.BaseModel):
    ...
    updated_status = datastore_services.StringProperty(choices=STATUS_CHOICES, indexed=True)
    ...
    # the web).
    received_via_email = datastore_services.BooleanProperty(default=False, indexed=True, required=True)
    ...
    @classmethod
    def export_data(cls, user_id: str) -> Dict[str, Dict[str, Union[str, int, bool, None]]]:
        ...
        user_data = {}
        feedback_models: Sequence[GeneralFeedbackMessageModel] = cls.get_all().filter(cls.author_id == user_id).fetch()
    ...
    @classmethod
    def create(cls, message_identifier: feedback_domain.FullyQualifiedMessageIdentifier) -> GeneralFeedbackMessageModel:
    ...
    def create_multi(
        ...
        instance_ids = [cls._generate_id(thread_id, message_id) for thread_id, message_id in zip(thread_ids, message_ids)]

        # Check if the new ids are valid.
        current_instances = cls.get_multi(instance_ids)
        conflict_ids = [current_instance.id for current_instance in current_instances if current_instance is not None]
        if len(conflict_ids) > 0:
            raise Exception('The following feedback message ID(s) conflicted on create: %s' % (' '.join(conflict_ids)))
    ...
    @overload
    @classmethod
    def get(cls, thread_id: str, message_id: int, *, strict: Literal[True]) -> GeneralFeedbackMessageModel: ...

    @overload
    @classmethod
    def get(cls, thread_id: str, message_id: int, *, strict: Literal[False]) -> Optional[GeneralFeedbackMessageModel]: ...

    @overload
    @classmethod
    def get(cls, thread_id: str, message_id: int, *, strict: bool = ...) -> Optional[GeneralFeedbackMessageModel]: ...

    @classmethod
    def get(cls, thread_id: str, message_id: int, strict: bool = True) -> Optional[GeneralFeedbackMessageModel]:
    ...
    def get_messages(cls, thread_id: str) -> Sequence[GeneralFeedbackMessageModel]:
        ...
        Returns:
            ...
            messages.
        """
        return cls.get_all().filter(cls.thread_id == thread_id).fetch(feconf.DEFAULT_QUERY_LIMIT)
    ...
    def get_message_counts(cls, thread_ids: List[str]) -> List[int]:
        ...
        return [thread_model.message_count if thread_model else None for thread_model in thread_models]

    # TODO(#13523): Change the return value of the function below from
    # tuple(list, str|None, bool) to a domain object.
    @classmethod
    def get_all_messages(cls, page_size: int, urlsafe_start_cursor: Optional[str]) -> Tuple[Sequence[GeneralFeedbackMessageModel], Optional[str], bool]:
...
class GeneralFeedbackThreadUserModel(base_models.BaseModel):
    ...
    thread_id = datastore_services.StringProperty(required=True, indexed=True)
    message_ids_read_by_user = datastore_services.IntegerProperty(repeated=True, indexed=True)
    ...
    @classmethod
    def create_multi(cls, user_id: str, thread_ids: List[str]) -> List[GeneralFeedbackThreadUserModel]:
    ...
    def get_multi(  # type: ignore[override]
        ...
        """
        instance_ids = [cls.generate_full_id(user_id, thread_id) for thread_id in thread_ids]
    ...
    def export_data(cls, user_id: str) -> Dict[str, Dict[str, List[str]]]:
        ...
        for user_model in found_models:
            user_data[user_model.thread_id] = {'message_ids_read_by_user': user_model.message_ids_read_by_user}
        return user_data
