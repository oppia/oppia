...
class FeedbackThreadModelTest(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        user_models.UserSettingsModel(id=self.NEW_USER_1_ID,
                                      email='some@email.com').put()
    ...
    def test_export_data_nontrivial(self) -> None:
        user_data = (
            feedback_models.GeneralFeedbackThreadModel.export_data(self.USER_ID)
        )
...
class GeneralFeedbackMessageModelTests(test_utils.GenericTestBase):
    ...
    def test_get_all_messages(self) -> None:
        ...
        all_messages = (
            feedback_models.GeneralFeedbackMessageModel.get_all_messages(2, None)
        )
    ...
    def test_export_data_nontrivial(self) -> None:
        ...
        thread_id = feedback_services.create_thread(
            test_export_thread_type, test_export_thread_id, test_export_author_id,
            test_export_updated_subject, test_export_text
        )

        feedback_services.create_message(
            thread_id, test_export_author_id, test_export_updated_status,
            test_export_updated_subject, test_export_text
        )
