...
class AppFeedbackReportModel(base_models.BaseModel):
    ...
    @classmethod
    def create(
        cls, entity_id: str, platform: str, submitted_on: datetime.datetime,
        local_timezone_offset_hrs: int, report_type: str, category: str,
        platform_version: str, android_device_country_locale_code: Optional[str],
        android_sdk_version: Optional[int], android_device_model: Optional[str],
        entry_point: str, entry_point_topic_id: Optional[str],
        entry_point_story_id: Optional[str], entry_point_exploration_id: Optional[str],
        entry_point_subtopic_id: Optional[str], text_language_code: str,
        audio_language_code: str, android_report_info: Optional[ReportInfoDict],
    ...
    def generate_id(
        ...
        for _ in range(base_models.MAX_RETRIES):
            random_hash = utils.convert_to_hash(
                str(utils.get_random_int(base_models.RAND_RANGE)), base_models.ID_LENGTH
            )
            new_id = '%s.%s.%s' % (
    ...
    def get_all_unscrubbed_expiring_report_models(
        ...
        datetime_before_which_to_scrub = datetime_now - (
            feconf.APP_FEEDBACK_REPORT_MAXIMUM_LIFESPAN + datetime.timedelta(days=1)
        )
...
class AppFeedbackReportTicketModel(base_models.BaseModel):
    ...
    @classmethod
    def create(
        cls, entity_id: str, ticket_name: str, platform: str,
        github_issue_repo_name: Optional[str], github_issue_number: Optional[int],
        newest_report_timestamp: datetime.datetime, report_ids: List[str]
    ) -> str:
...
class AppFeedbackReportStatsModel(base_models.BaseModel):
    ...
    @classmethod
    def create(
        cls, entity_id: str, platform: str, ticket_id: str,
    ...
    @classmethod
    def calculate_id(
        cls, platform: str, ticket_id: Optional[str], stats_tracking_date: datetime.date
    ) -> str:
    ...
    def get_stats_for_ticket(cls,
                             ticket_id: str) -> Sequence[AppFeedbackReportStatsModel]:
