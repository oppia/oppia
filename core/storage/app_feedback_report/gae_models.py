...
from typing import Dict, Final, List, Literal, Optional, Sequence, TypedDict, TypeVar
SELF_REPORT_MODEL = TypeVar('SELF_REPORT_MODEL', bound='AppFeedbackReportModel'
    )
MYPY = False
if MYPY:
...
base_models, user_models = models.Registry.import_models([models.Names.
    BASE_MODEL, models.Names.USER])
...
class AppFeedbackReportModel(base_models.BaseModel):
    ...
    platform = datastore_services.StringProperty(required=True, indexed=
        True, choices=PLATFORM_CHOICES)
    scrubbed_by = datastore_services.StringProperty(required=False, indexed
        =True)
    ...
    submitted_on = datastore_services.DateTimeProperty(required=True,
        indexed=True)
    local_timezone_offset_hrs = datastore_services.IntegerProperty(required
        =False, indexed=True)
    ...
    entry_point_story_id = datastore_services.StringProperty(required=False,
        indexed=True)
    entry_point_exploration_id = datastore_services.StringProperty(required
        =False, indexed=True)
    entry_point_subtopic_id = datastore_services.IntegerProperty(required=
        False, indexed=True)
    ...
    @classmethod
    def create(cls, entity_id: str, platform: str, submitted_on: datetime.
        datetime, local_timezone_offset_hrs: int, report_type: str,
        category: str, platform_version: str,
        android_device_country_locale_code: Optional[str],
        android_sdk_version: Optional[int], android_device_model: Optional[
        str], entry_point: str, entry_point_topic_id: Optional[str],
        entry_point_story_id: Optional[str], entry_point_exploration_id:
        Optional[str], entry_point_subtopic_id: Optional[str],
        text_language_code: str, audio_language_code: str,
        android_report_info: Optional[ReportInfoDict], web_report_info:
        Optional[ReportInfoDict]) ->str:
        ...
        if platform == PLATFORM_CHOICE_ANDROID:
            android_schema_version = (feconf.
                CURRENT_ANDROID_REPORT_SCHEMA_VERSION)
        else:
            web_schema_version = feconf.CURRENT_WEB_REPORT_SCHEMA_VERSION
        report_entity = cls(id=entity_id, platform=platform, submitted_on=
            submitted_on, local_timezone_offset_hrs=
            local_timezone_offset_hrs, report_type=report_type, category=
            category, platform_version=platform_version,
            android_device_country_locale_code=
            android_device_country_locale_code, android_sdk_version=
            android_sdk_version, android_device_model=android_device_model,
            entry_point=entry_point, entry_point_topic_id=
            entry_point_topic_id, entry_point_exploration_id=
            entry_point_exploration_id, entry_point_story_id=
            entry_point_story_id, entry_point_subtopic_id=
            entry_point_subtopic_id, text_language_code=text_language_code,
            audio_language_code=audio_language_code, android_report_info=
            android_report_info, android_report_info_schema_version=
            android_schema_version, web_report_info=web_report_info,
    ...
    @classmethod
    def generate_id(cls, platform: str, submitted_on_datetime: datetime.
        datetime) ->str:
        ...
        for _ in range(base_models.MAX_RETRIES):
            random_hash = utils.convert_to_hash(str(utils.get_random_int(
                base_models.RAND_RANGE)), base_models.ID_LENGTH)
            new_id = '%s.%s.%s' % (platform, int(submitted_datetime_in_msec
                ), random_hash)
            if not cls.get_by_id(new_id):
                return new_id
        raise Exception(
            'The id generator for AppFeedbackReportModel is producing too many collisions.'
            )

    @classmethod
    def get_all_unscrubbed_expiring_report_models(cls) ->Sequence[
        ...
        datetime_before_which_to_scrub = datetime_now - (feconf.
            APP_FEEDBACK_REPORT_MAXIMUM_LIFESPAN + datetime.timedelta(days=1))
        report_models: Sequence[AppFeedbackReportModel] = cls.query(cls.
            created_on < datetime_before_which_to_scrub, cls.scrubbed_by ==
            None).fetch()
        return report_models

    @classmethod
    def get_filter_options_for_field(cls, filter_field: FilterFieldNames
        ) ->List[str]:
        ...
        if filter_field == FilterFieldNames.REPORT_TYPE:
            ...
            filter_values = [model.android_device_country_locale_code for
                model in query]
        else:
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{'platform':
            base_models.EXPORT_POLICY.EXPORTED, 'scrubbed_by': base_models.
            EXPORT_POLICY.EXPORTED, 'ticket_id': base_models.EXPORT_POLICY.
            EXPORTED, 'submitted_on': base_models.EXPORT_POLICY.EXPORTED,
            'local_timezone_offset_hrs': base_models.EXPORT_POLICY.EXPORTED,
            'report_type': base_models.EXPORT_POLICY.EXPORTED, 'category':
            base_models.EXPORT_POLICY.EXPORTED, 'platform_version':
            base_models.EXPORT_POLICY.EXPORTED,
            'android_device_country_locale_code': base_models.EXPORT_POLICY
            .NOT_APPLICABLE, 'android_device_model': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'android_sdk_version':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'entry_point':
            base_models.EXPORT_POLICY.NOT_APPLICABLE,
            'entry_point_topic_id': base_models.EXPORT_POLICY.
            NOT_APPLICABLE, 'entry_point_story_id': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'entry_point_exploration_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE,
            'entry_point_subtopic_id': base_models.EXPORT_POLICY.
            NOT_APPLICABLE, 'text_language_code': base_models.EXPORT_POLICY
            .NOT_APPLICABLE, 'audio_language_code': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'android_report_info':
            base_models.EXPORT_POLICY.NOT_APPLICABLE,
            'android_report_info_schema_version': base_models.EXPORT_POLICY
            .NOT_APPLICABLE, 'web_report_info': base_models.EXPORT_POLICY.
            NOT_APPLICABLE, 'web_report_info_schema_version': base_models.
    ...
    def export_data(cls, user_id: str) ->Dict[str, Dict[str, str]]:
        ...
        """
        user_data = {}
        report_models: Sequence[AppFeedbackReportModel] = cls.get_all().filter(
            cls.scrubbed_by == user_id).fetch()
        for report_model in report_models:
            submitted_on_msec = utils.get_time_in_millisecs(report_model.
                submitted_on)
            ...
            user_data[report_model.id] = {'scrubbed_by':
                scrubbed_by_username, 'platform': report_model.platform,
                'ticket_id': report_model.ticket_id, 'submitted_on': utils.
                get_human_readable_time_string(submitted_on_msec),
                'local_timezone_offset_hrs': report_model.
                local_timezone_offset_hrs, 'report_type': report_model.
                report_type, 'category': report_model.category,
    ...
    def has_reference_to_user_id(cls, user_id: str) ->bool:
        ...
        """
        return cls.query(cls.scrubbed_by == user_id).get(keys_only=True
            ) is not None
...
class AppFeedbackReportTicketModel(base_models.BaseModel):
    ...
    platform = datastore_services.StringProperty(required=True, indexed=
        True, choices=PLATFORM_CHOICES)
    github_issue_repo_name = datastore_services.StringProperty(required=
        False, indexed=True, choices=GITHUB_REPO_CHOICES)
    ...
    newest_report_timestamp = datastore_services.DateTimeProperty(required=
        False, indexed=True)
    ...
    def create(cls, entity_id: str, ticket_name: str, platform: str,
        ...
        Optional[int], newest_report_timestamp: datetime.datetime,
        ...
        Returns:
            ...
            instance.
        """
        ticket_entity = cls(id=entity_id, ticket_name=ticket_name, platform
            =platform, github_issue_repo_name=github_issue_repo_name,
            github_issue_number=github_issue_number, archived=False,
            newest_report_timestamp=newest_report_timestamp, report_ids=
            report_ids)
    ...
    def generate_id(cls, ticket_name: str) ->str:
        ...
        Raises:
            Exception. If the id generator is producing too many collisions.
        """
        current_datetime_in_msec = utils.get_time_in_millisecs(datetime.
            datetime.utcnow())
        for _ in range(base_models.MAX_RETRIES):
            name_hash = utils.convert_to_hash(ticket_name, base_models.
                ID_LENGTH)
            random_hash = utils.convert_to_hash(str(utils.get_random_int(
                base_models.RAND_RANGE)), base_models.ID_LENGTH)
            ...
            if not cls.get_by_id(new_id):
                return new_id
        raise Exception(
            'The id generator for AppFeedbackReportTicketModel is producing toomany collisions.'
            )
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        ...
        return dict(super(cls, cls).get_export_policy(), **{'ticket_name':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'platform':
            base_models.EXPORT_POLICY.NOT_APPLICABLE,
            'github_issue_repo_name': base_models.EXPORT_POLICY.
            NOT_APPLICABLE, 'github_issue_number': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'archived': base_models.
            EXPORT_POLICY.NOT_APPLICABLE, 'newest_report_timestamp':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'report_ids':
            base_models.EXPORT_POLICY.NOT_APPLICABLE})
...
class AppFeedbackReportStatsModel(base_models.BaseModel):
    ...
    stats_tracking_date = datastore_services.DateProperty(required=True,
        indexed=True)
    total_reports_submitted = datastore_services.IntegerProperty(required=
        True, indexed=True)
    ...
    def create(cls, entity_id: str, platform: str, ticket_id: str,
        stats_tracking_date: datetime.date, total_reports_submitted: int,
        ...
        """
        stats_entity = cls(id=entity_id, ticket_id=ticket_id, platform=
            platform, stats_tracking_date=stats_tracking_date,
            total_reports_submitted=total_reports_submitted,
            daily_param_stats=daily_param_stats,
            daily_param_stats_schema_version=feconf.
            CURRENT_FEEDBACK_REPORT_STATS_SCHEMA_VERSION)
    ...
    def calculate_id(cls, platform: str, ticket_id: Optional[str],
        ...
        return '%s:%s:%s' % (platform, ticket_id, stats_tracking_date.
            isoformat())

    @classmethod
    def get_stats_for_ticket(cls, ticket_id: str) ->Sequence[
    ...
    def get_export_policy(cls) ->Dict[str, base_models.EXPORT_POLICY]:
        """Model doesn't contain any data directly corresponding to a user."""
        return dict(super(cls, cls).get_export_policy(), **{'ticket_id':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'platform':
            base_models.EXPORT_POLICY.NOT_APPLICABLE, 'stats_tracking_date':
            base_models.EXPORT_POLICY.NOT_APPLICABLE,
            'total_reports_submitted': base_models.EXPORT_POLICY.
            NOT_APPLICABLE, 'daily_param_stats_schema_version': base_models
            .EXPORT_POLICY.NOT_APPLICABLE, 'daily_param_stats': base_models
            .EXPORT_POLICY.NOT_APPLICABLE})
