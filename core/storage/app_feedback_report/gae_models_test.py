...
class AppFeedbackReportModelTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.feedback_report_model = (
            app_feedback_report_models.AppFeedbackReportModel(
                id='%s.%s.%s' % (
                    self.PLATFORM_ANDROID, int(self.REPORT_SUBMITTED_TIMESTAMP_1_MSEC
                                              ), 'randomInteger123'
                ),
                platform=self.PLATFORM_ANDROID,
                scrubbed_by=self.user_id,
                ticket_id='%s.%s.%s' % (
                    'random_hash', int(self.TICKET_CREATION_TIMESTAMP_MSEC
                                      ), '16CharString1234'
                ),
    ...
    def test_create_and_get_android_report_model(self) -> None:
        ...
        app_feedback_report_models.AppFeedbackReportModel.create(
            report_id, self.PLATFORM_ANDROID, self.REPORT_SUBMITTED_TIMESTAMP_2, 0,
            self.REPORT_TYPE_SUGGESTION, self.CATEGORY_OTHER, self.PLATFORM_VERSION,
            self.DEVICE_COUNTRY_LOCALE_CODE_INDIA, self.ANDROID_SDK_VERSION,
            self.ANDROID_DEVICE_MODEL, self.ENTRY_POINT_NAVIGATION_DRAWER, None, None,
            None, None, self.TEXT_LANGUAGE_CODE_ENGLISH,
    ...
    def test_create_raises_exception_by_mocking_collision(self) -> None:
        ...
        with self.assertRaisesRegex(
            ...
            with self.swap(
                ...
                types.MethodType(
                    lambda x, y: True, app_feedback_report_models.AppFeedbackReportModel
                )
            ):
                ...
                model_class.create(
                    report_id, self.PLATFORM_ANDROID, self.REPORT_SUBMITTED_TIMESTAMP_1,
    ...
    def test_export_data_with_scrubber(self) -> None:
        exported_data = (
            app_feedback_report_models.AppFeedbackReportModel.export_data(self.user_id)
        )
    ...
    def test_get_all_unscrubbed_expiring_report_models(self) -> None:
        ...
        ) - (feconf.APP_FEEDBACK_REPORT_MAXIMUM_LIFESPAN + datetime.timedelta(days=10))
        expired_model = app_feedback_report_models.AppFeedbackReportModel(
            id='%s.%s.%s' % (
                self.PLATFORM_ANDROID,
                int(utils.get_time_in_millisecs(expired_timestamp)), 'randomInteger123'
            ),
            platform=self.PLATFORM_ANDROID,
            scrubbed_by=None,
            ticket_id='%s.%s.%s' % (
    ...
    def test_get_filter_options_with_invalid_field_throws_exception(self) -> None:
        model_class = app_feedback_report_models.AppFeedbackReportModel
        ...
        with self.assertRaisesRegex(
            ...
            with self.swap(
                model_class, 'query', self._mock_query_filters_returns_empty_list
            ):
...
class AppFeedbackReportTicketModelTests(test_utils.GenericTestBase):
    ...
    REPORT_IDS: Final = [
        '%s.%s.%s' %
        (PLATFORM, int(REPORT_SUBMITTED_TIMESTAMP_MSEC), 'randomInteger123')
    ]
    ...
    def test_create_and_get_ticket_model(self) -> None:
        ...
        ticket_model = (
            app_feedback_report_models.AppFeedbackReportTicketModel.get(ticket_id)
        )
    ...
    def test_get_deletion_policy(self) -> None:
        ...
        self.assertEqual(
            model.get_deletion_policy(), base_models.DELETION_POLICY.NOT_APPLICABLE
        )
...
class AppFeedbackReportStatsModelTests(test_utils.GenericTestBase):
    ...
    def test_create_and_get_stats_model(self) -> None:
        ...
        stats_model = (
            app_feedback_report_models.AppFeedbackReportStatsModel.get_by_id(entity_id)
        )
