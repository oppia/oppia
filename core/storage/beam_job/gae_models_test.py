...
class BeamJobRunModelTest(test_utils.GenericTestBase):
    ...
    def test_get_new_id_raises_error_after_too_many_failed_attempts(self) -> None:
        ...
        with collision_context:
            with self.assertRaisesRegex(
                RuntimeError, r'Failed to generate a unique ID after \d+ attempts'
            ):
    ...
    def test_get_export_policy(self) -> None:
        ...
        self.assertEqual(
            export_policy['dataflow_job_id'], base_models.EXPORT_POLICY.NOT_APPLICABLE
        )
        self.assertEqual(
            export_policy['job_name'], base_models.EXPORT_POLICY.NOT_APPLICABLE
        )
        self.assertEqual(
            export_policy['latest_job_state'], base_models.EXPORT_POLICY.NOT_APPLICABLE
        )
...
class BeamJobRunResultModelTest(test_utils.GenericTestBase):
    ...
    def test_get_model_association_to_user(self) -> None:
        self.assertEqual(
            beam_job_models.BeamJobRunResultModel.get_model_association_to_user(),
            base_models.MODEL_ASSOCIATION_TO_USER.NOT_CORRESPONDING_TO_USER,
        )

    def test_get_export_policy(self) -> None:
        export_policy = beam_job_models.BeamJobRunResultModel.get_export_policy()
        self.assertEqual(
