...
class DateRangeUnitTests(test_utils.GenericTestBase):
    """Tests for the DateRange class."""

    start_date = datetime.date.today()
    end_date = datetime.date.today() - datetime.timedelta(days=90)

    def test_is_valid_date_range(self) -> None:
        model = utils.DateRange(
            self.start_date,
            self.end_date)
        self.assertEqual(
            model.is_valid_date_range(),
            True)
    ...
    def test_is_date_in_the_date_range(
            self) -> None:
        model = utils.DateRange(
            self.start_date,
            self.end_date)
        ...
        self.assertEqual(
            model.is_date_in_the_date_range(
