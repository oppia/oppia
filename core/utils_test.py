...
class UtilsTests(test_utils.GenericTestBase):
    ...
    def test_unescape_encoded_uri_component(self) -> None:
        self.assertEqual(
            utils.unescape_encoded_uri_component('/El%20Ni%C3%B1o/'), '/El Niño/')  # pylint: disable=line-too-long
    ...
    def test_compress_and_decompress_zlib(self) -> None:
        ...
        self.assertEqual(
            len(utils.decompress_from_zlib(string_compressed)), len(string_instance))  # pylint: disable=line-too-long

    def test_compute_list_difference(self) -> None:
        self.assertEqual(
            utils.compute_list_difference(['-5', '-1', '0', '7', '-4'], ['2', '0', '-5', '-2', '7']), ['-1', '-4'])  # pylint: disable=line-too-long
