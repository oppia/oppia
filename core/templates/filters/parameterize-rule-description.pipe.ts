...
export class ParameterizeRuleDescriptionPipe
   implements PipeTransform {
  POSITION_OF_TERMS_MAPPING = AppConstants.POSITION_OF_TERMS_MAPPING;
  ...
  transform(
    ...
    return (
        rule: Rule,
        interactionId: string, choices: AnswerChoice[]): string => {
      if (!rule) {
        return '';
      }
      ...
      while (true) {
        ...
        varType === 'NumericExpression'
          ...
          for (var i = 0; i < this.POSITION_OF_TERMS_MAPPING.length; i++) {
            if (this.POSITION_OF_TERMS_MAPPING[i].name === inputs[varName]) {
              replacementText =
              this.POSITION_OF_TERMS_MAPPING[i].humanReadableName;
            }
          }
        } else if (varType === 'ListOfCodeEvaluation') {
          replacementText = '[';
          for (var i = 0; i < Object.keys(
            inputs[varName]).length; i++) {
            if (i !== 0) {
              replacementText += ', ';
            }
