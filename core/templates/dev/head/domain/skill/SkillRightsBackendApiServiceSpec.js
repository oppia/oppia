...
describe('Skill rights backend API service', function() {
  ...
  it('should successfully set a skill to be public', function() {
    ...
    $httpBackend.expect(
      'PUT', '/skill_editor_handler/publish_skill/0')
      .respond(200);
    SkillRightsBackendApiService.setSkillPublic('0', 1)
      .then(
  ...
  it('should report a cached skill rights after caching it', function() {
    ...
    expect(SkillRightsBackendApiService.isCached('0'))
      .toBe(false);
    // Cache a skill.
    ...
    SkillRightsBackendApiService.loadSkillRights('0')
      .then(
        successHandler, failHandler);
    $rootScope.$digest();
