...
describe('URL Interpolation Service', function() {
  ...
  it('should add hash to url if hash is set', function() {
    expect(uis._getUrlWithSlug('/hash_test.html'))
      .toBe(
        '/hash_test.' + hashes['/hash_test.html'] + '.html'
      );
    expect(uis._getUrlWithSlug('/path_test/hash_test.html'))
      .toBe(
        '/path_test/hash_test.' + hashes['/path_test/hash_test.html'] + '.html'
      );
    expect(uis._getUrlWithSlug('/hash_test.min.js'))
      .toBe(
  ...
  it('should build complete URL with prefixes and hash', function() {
    expect(uis._getCompleteUrl('/test_folder', '/hash_test.html'))
      .toBe(
        '/build/test_folder/hash_test.' + hashes['/hash_test.html'] + '.html'
      );
    expect(
      uis._getCompleteUrl('/test_folder', '/path_test/hash_test.html'))
      .toBe(
        '/build/test_folder/path_test/hash_test.' +
      hashes['/path_test/hash_test.html'] + '.html'
      );
    expect(uis._getCompleteUrl('/test_folder', '/hash_test.min.js'))
      .toBe(
        '/build/test_folder/hash_test.min.' + hashes['/hash_test.min.js'] +
        '.js'
      );
    expect(uis._getCompleteUrl('', '/hash_test.html'))
      .toBe(
  ...
  it('should throw an error for erroneous URLs', function() {
    ...
    expect(uis.interpolateUrl.bind(null, null, {}))
      .toThrow(
        new Error('Invalid or empty URL template passed in: \'null\''));
    expect(uis.interpolateUrl.bind(null, undefined, {}))
      .toThrow(
        new Error('Invalid or empty URL template passed in: \'undefined\''));
    expect(uis.interpolateUrl.bind(null, '', {}))
      .toThrow(
  ...
  it('should throw an error for erroneous interpolation values', function() {
    expect(uis.interpolateUrl.bind(null, 'url', null))
      .toThrow(
        new Error('Expected an object of interpolation values to be passed ' +
        'into interpolateUrl.'));
    expect(uis.interpolateUrl.bind(null, 'url', undefined))
      .toThrow(
        new Error('Expected an object of interpolation values to be passed ' +
        'into interpolateUrl.'));
    expect(
  ...
  it('should interpolate URLs not requiring parameters', function() {
    expect(uis.interpolateUrl('/test_url/', {}))
      .toBe('/test_url/');
    expect(uis.interpolateUrl('/test_url/', {param: 'value'}))
      .toBe('/test_url/');
  });

  it('should interpolate URLs when parameters have parentheses', function() {
    expect(uis.interpolateUrl('/test_url/<param>', {param: 'value (1'}))
  ...
  it('should interpolate URLs requiring one or more parameters', function() {
    expect(uis.interpolateUrl('/test_url/<fparam>', {fparam: 'value'}))
      .toBe('/test_url/value');
    expect(uis.interpolateUrl(
  ...
  it('should interpolate parameters within words or adjacent to other ' +
    ...
    expect(uis.interpolateUrl('word<with_param>', {with_param: '_with_value'}))
  ...
  it('should escape the "=" symbol correctly', function() {
    expect(uis.interpolateUrl(
      '/test_url/<first_param>', {first_param: 'first=param', }))
  ...
  it('should not interpolate bad parameter names and values', function() {
    ...
    expect(uis.interpolateUrl.bind(null, '/test_url/<>', {'': 'value'}))
    ...
    // parameter name.
    expect('/test_url/<b@d!#$%^&*() p@r@m n@m3`~[]{}>', {})
      .toBe(
    ...
    expect(uis.interpolateUrl.bind(null, '/test_url/<<name>>', {name: 'value'}))
      .toThrow(new Error(
        'Invalid URL template received: \'/test_url/<<name>>\''));

    expect(uis.interpolateUrl('/test_url/<name>', {name: '<value>'}))
      .toEqual('/test_url/%3Cvalue%3E');

    expect(uis.interpolateUrl('/test_url/<name>', {name: '<<value>>'}))
      .toEqual('/test_url/%3C%3Cvalue%3E%3E');

    expect(uis.interpolateUrl('/test_url/<name>', {name: '<>'}))
  ...
  it('should throw an error for missing parameters', function() {
    ...
    expect(uis.interpolateUrl.bind(null, '/test_url/<page1>', {page2: 'v'}))
      .toThrow(new Error(
        'Expected variable \'page1\' when interpolating URL.'));
  });

  it('should throw an error for non-string parameters', function() {
    expect(uis.interpolateUrl.bind(null, '/test_url/<page>', {page: 0}))
      .toThrow(new Error(
        'Parameters passed into interpolateUrl must be strings.'));
    expect(uis.interpolateUrl.bind(null, '/test_url/<page>', {page: {}}))
      .toThrow(new Error(
        'Parameters passed into interpolateUrl must be strings.'));
    expect(uis.interpolateUrl.bind(null, '/test_url/<page>', {page: []}))
      .toThrow(new Error(
        'Parameters passed into interpolateUrl must be strings.'));
    expect(uis.interpolateUrl.bind(null, '/test_url/<page>', {page: /abc/}))
      .toThrow(new Error(
        'Parameters passed into interpolateUrl must be strings.'));
  });

  it('should interpolate correct path', function() {
    expect(uis.getStaticImageUrl('/test.png'))
      .toBe(
        '/build/assets/images/test.png');
    expect(uis.getStaticImageUrl('/test_url/test.png'))
      .toBe(
        '/build/assets/images/test_url/test.png');
    expect(uis.getStaticImageUrl('/hash_test.png'))
      .toBe(
        '/build/assets/images/hash_test.' + hashes['/images/hash_test.png'] +
      '.png');

    expect(uis.getStaticVideoUrl('/test.mp4'))
      .toBe(
        '/build/assets/videos/test.mp4');
    expect(uis.getStaticVideoUrl('/test_url/test.mp4'))
      .toBe(
        '/build/assets/videos/test_url/test.mp4');
    expect(uis.getStaticVideoUrl('/hash_test.mp4'))
      .toBe(
        '/build/assets/videos/hash_test.' + hashes['/videos/hash_test.mp4'] +
      '.mp4');

    expect(uis.getInteractionThumbnailImageUrl('LogicProof'))
      .toBe(
        '/build/extensions/interactions/LogicProof/static/LogicProof.png');
    expect(uis.getInteractionThumbnailImageUrl('interTest'))
      .toBe(
        '/build/extensions/interactions/interTest/static/interTest.' +
      hashes['/interactions/interTest/static/interTest.png'] + '.png');

    expect(uis.getDirectiveTemplateUrl('/test.html'))
      .toBe(
        '/build/templates/head/test.html');
    expect(uis.getDirectiveTemplateUrl('/test_url/test.html'))
      .toBe(
        '/build/templates/head/test_url/test.html');
    expect(uis.getDirectiveTemplateUrl('/pages_test/hash_test.html'))
      .toBe(
        '/build/templates/head/pages_test/hash_test.' +
      hashes['/pages_test/hash_test.html'] + '.html');

    expect(uis.getStaticAssetUrl('/test.json'))
      .toBe(
        '/build/assets/test.json');
    expect(uis.getStaticAssetUrl('/test_url/test.json'))
      .toBe(
        '/build/assets/test_url/test.json');
    expect(uis.getStaticAssetUrl('/assets_test/hash_test.json'))
      .toBe(
        '/build/assets/assets_test/hash_test.' +
      hashes['/assets_test/hash_test.json'] + '.json');

    expect(uis.getExtensionResourceUrl('/test.html'))
      .toBe(
        '/build/extensions/test.html');
    expect(uis.getExtensionResourceUrl('/test_url/test.html'))
      .toBe(
        '/build/extensions/test_url/test.html');
    expect(uis.getExtensionResourceUrl('/path_test/hash_test.html'))
      .toBe(
  ...
  it('should interpolate URLs not requiring parameters', function() {
    expect(uis.getStoryUrl('/storyId', {}))
      .toBe('/story/storyId');
    expect(uis.getStoryUrl('/storyId123', {}))
      .toBe('/story/storyId123');
    expect(uis.getStoryUrl('/story&Id', {}))
      .toBe('/story/story&Id');
    expect(function() {
      uis.getStoryUrl('', {});
    })
  ...
  it('should throw an error for empty path', function() {
    expect(uis.getStaticImageUrl.bind(null, null))
      .toThrow(
        new Error(
          'Empty path passed in method.'));
    expect(uis.getStaticImageUrl.bind(null, ''))
      .toThrow(
        new Error(
          'Empty path passed in method.'));

    expect(uis.getStaticVideoUrl.bind(null, null))
      .toThrow(
        new Error(
          'Empty path passed in method.'));
    expect(uis.getStaticVideoUrl.bind(null, ''))
      .toThrow(
        new Error(
          'Empty path passed in method.'));

    expect(uis.getInteractionThumbnailImageUrl.bind(null, null))
      .toThrow(
        new Error(
          'Empty interactionId passed in getInteractionThumbnailImageUrl.'));
    expect(uis.getInteractionThumbnailImageUrl.bind(null, ''))
      .toThrow(
        new Error(
          'Empty interactionId passed in getInteractionThumbnailImageUrl.'));

    expect(uis.getDirectiveTemplateUrl.bind(null, null))
      .toThrow(
        new Error('Empty path passed in method.'));
    expect(uis.getDirectiveTemplateUrl.bind(null, ''))
      .toThrow(
        new Error('Empty path passed in method.'));

    expect(uis.getStaticAssetUrl.bind(null, null))
      .toThrow(
        new Error('Empty path passed in method.'));
    expect(uis.getStaticAssetUrl.bind(null, ''))
      .toThrow(
        new Error('Empty path passed in method.'));

    expect(uis.getExtensionResourceUrl.bind(null, null))
      .toThrow(
        new Error('Empty path passed in method.'));
    expect(uis.getExtensionResourceUrl.bind(null, ''))
      .toThrow(
        new Error('Empty path passed in method.'));
  });
  ...
  it('should throw an error for path not beginning with forward slash',
    function() {
      expect(uis.getStaticImageUrl.bind(null, 'test_fail.png'))
        .toThrow(
          new Error(
            'Path must start with \'\/\': \'' + 'test_fail.png' + '\'.'));
      expect(uis.getStaticImageUrl.bind(null, 'test_url/fail.png'))
        .toThrow(
          new Error(
            'Path must start with \'\/\': \'' + 'test_url/fail.png' + '\'.'));

      expect(uis.getStaticVideoUrl.bind(null, 'test_fail.png'))
        .toThrow(
          new Error(
            'Path must start with \'\/\': \'' + 'test_fail.png' + '\'.'));
      expect(uis.getStaticVideoUrl.bind(null, 'test_url/fail.png'))
        .toThrow(
          new Error(
            'Path must start with \'\/\': \'' + 'test_url/fail.png' + '\'.'));

      expect(uis.getDirectiveTemplateUrl.bind(null, 'test_fail.html'))
        .toThrow(
          new Error(
            'Path must start with \'\/\': \'' + 'test_fail.html' + '\'.'));
      expect(
        uis.getDirectiveTemplateUrl.bind(null, 'test_url/fail.html'))
        .toThrow(
          new Error(
            'Path must start with \'\/\': \'' + 'test_url/fail.html' + '\'.'));

      expect(uis.getStaticAssetUrl.bind(null, 'test_fail.html'))
        .toThrow(
          new Error(
            'Path must start with \'\/\': \'' + 'test_fail.html' + '\'.'));
      expect(uis.getStaticAssetUrl.bind(null, 'test_url/fail.html'))
        .toThrow(
          new Error(
            'Path must start with \'\/\': \'' + 'test_url/fail.html' + '\'.'));

      expect(uis.getExtensionResourceUrl.bind(null, 'test_fail.html'))
        .toThrow(
          new Error(
            'Path must start with \'\/\': \'' + 'test_fail.html' + '\'.'));
      expect(
        uis.getExtensionResourceUrl.bind(null, 'test_url/fail.html'))
        .toThrow(
          new Error(
            'Path must start with \'\/\': \'' + 'test_url/fail.html' + '\'.'));
    });
