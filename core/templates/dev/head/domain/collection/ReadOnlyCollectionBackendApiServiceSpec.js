...
describe('Read only collection backend API service', function() {
  ...
  it('should successfully fetch an existing collection from the backend',
    function() {
      ...
      $httpBackend.expect('GET', '/collection_handler/data/0')
        .respond(
          sampleDataResults);
      ReadOnlyCollectionBackendApiService.fetchCollection('0')
        .then(
  ...
  it('should load a cached collection after fetching it from the backend',
    function() {
      ...
      $httpBackend.expect('GET', '/collection_handler/data/0')
        .respond(
          sampleDataResults);
      ReadOnlyCollectionBackendApiService.loadCollection('0')
        .then(
          successHandler, failHandler);
      $httpBackend.flush();
  ...
  it('should use the rejection handler if the backend request failed',
    function() {
      ...
      expect(failHandler)
        .toHaveBeenCalledWith('Error loading collection 0.');
    }
  );
  ...
  it('should report caching and support clearing the cache', function() {
    ...
    expect(ReadOnlyCollectionBackendApiService.isCached('0'))
      .toBeFalsy();
