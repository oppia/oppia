...
describe('Editable question backend API service', function() {
  ...
  it('should successfully fetch an existing question from the backend',
    function() {
      ...
      $httpBackend.expect('GET', '/question_editor_handler/data/0')
        .respond(
          sampleDataResults);
      EditableQuestionBackendApiService.fetchQuestion('0')
        .then(
  ...
  it('should use the rejection handler if the backend request failed',
    function() {
      ...
      $httpBackend.expect('GET', '/question_editor_handler/data/1')
        .respond(
          500, 'Error loading question 1.');
      EditableQuestionBackendApiService.fetchQuestion('1')
        .then(
          successHandler, failHandler);
      $httpBackend.flush();
      ...
      expect(failHandler)
        .toHaveBeenCalledWith('Error loading question 1.');
    }
  );
  ...
  it('should update a question after fetching it from the backend',
    function() {
      ...
      question.version = '2';
      var questionWrapper = {question_dict: question};

      $httpBackend.expect('PUT', '/question_editor_handler/data/0')
        .respond(
          questionWrapper);
  ...
  it('should use the rejection handler if the question to update ' +
    ...
    // Loading a question the first time should fetch it from the backend.
    $httpBackend.expect('PUT', '/question_editor_handler/data/1')
      .respond(
