...
describe('Story contents object factory', function() {
  ...
  it('should correctly return index of node (or -1, if not present) ' +
    ...
    expect(_sampleStoryContents.getNodeIndex('node_1'))
      .toEqual(0);
    expect(_sampleStoryContents.getNodeIndex('node_10'))
      .toEqual(-1);
  });
  ...
  it('should correctly correctly validate valid story contents', function() {
    expect(_sampleStoryContents.validate())
      .toEqual([]);
  });
  ...
  it('should correctly set initial node id when first node is ' +
    ...
    expect(storyContents.getInitialNodeId())
      .toEqual('node_1');
    expect(storyContents.getNodes()[0].getTitle())
      .toEqual('Title 1');
  });
  ...
  it('should correctly correctly validate case where prerequisite skills ' +
    ...
    expect(_sampleStoryContents.validate())
      .toEqual([
        'The prerequisite skill with id skill_3 was not completed before ' +
      'node with id node_3 was unlocked'
      ]);
  });
  ...
  it('should correctly correctly validate the case where the story graph ' +
    ...
    expect(_sampleStoryContents.validate())
      .toEqual(['Loops are not allowed in the node graph']);
  });
