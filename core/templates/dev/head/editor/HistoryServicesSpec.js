...
describe('Versions tree service', function() {
  ...
  describe('versions tree service', function() {
    ...
    it('should get correct list of parents', function() {
      vts.generateVersionTree(snapshots);
      var expectedParents = {
        1: -1,
        2: 1,
        3: 2,
    ...
    it('should get correct change list', function() {
      ...
      expect(function() {
        vts.getChangeList(1);
          ...
          'Tried to retrieve change list of version 1'));
      ...
      expect(vts.getChangeList(2)).toEqual(expectedChangeList);
      expectedChangeList = [{
        'version_number': 2,
        'cmd': 'AUTO_revert_version_number'
      }];
      expect(vts.getChangeList(4)).toEqual(expectedChangeList);
      expectedChangeList = [
        {
          'cmd': 'delete_state',
          'state_name': 'B'
      ...
      expect(vts.getChangeList(5)).toEqual(expectedChangeList);
      expectedChangeList = [{
        'cmd': 'edit_state_property',
        'state_name': 'D',
        'new_value': {'type': 'text', 'value': 'Some text'},
      ...
      expect(vts.getChangeList(8)).toEqual(expectedChangeList);
    });
  });
});
...
describe('Compare versions service', function() {
  ...
  describe('compare versions service', function() {
    ...
    var testExplorationData1 = [
      {
        'states': {
          'A': 'dummy_exploration_data'
        }
      },
      {
        'states': {
          'A': 'new_dummy_exploration_data',
        }
      },
      {
        'states': {
          'B': 'new_dummy_exploration_data',
        }
      },
      {
        'states': {
      ...
      },
      {
        'states': {
          'A': 'new_dummy_exploration_data',
          'B': 'dummy_exploration_data'
        }
      },
      {
        'states': {
          'A': 'new_dummy_exploration_data',
          'C': 'dummy_exploration_data'
        }
      },
      {
        'states': {
          'A': 'new_dummy_exploration_data',
          'C': 'another_dummy_exploration_data'
        }
      },
      {
        'states': {
          'A': 'new_dummy_exploration_data',
          'B': 'some_dummy_exploration_data',
          'C': 'another_dummy_exploration_data'
        }
      },
      {
    ...
    // Tests for getStatesData on linear commits
    it('should detect changed, renamed and added states', function() {
      ...
      vts.generateVersionTree(testSnapshots1);
      cvs.getStatesData(1, 7).then(function(data) {
        expect(data).toEqual({
          1: {
            'newestStateName': 'A',
    ...
    it('should add new state with same name as old name of renamed state', function() {
      ...
      vts.generateVersionTree(testSnapshots1);
      cvs.getStatesData(5, 8).then(function(data) {
        expect(data).toEqual({
          1: {
            'newestStateName': 'A',
    ...
    it('should mark added, then deleted state as changed', function() {
      ...
      vts.generateVersionTree(testSnapshots1);
      cvs.getStatesData(7, 9).then(function(data) {
        expect(data).toEqual({
          1: {
            'newestStateName': 'A',
            'stateProperty': 'unchanged',
    ...
    it('should mark deleted then added states as changed', function() {
      ...
      vts.generateVersionTree(testSnapshots1);
      cvs.getStatesData(8, 10).then(function(data) {
        expect(data).toEqual({
          1: {
            'newestStateName': 'A',
            'stateProperty': 'unchanged',
    ...
    it('should mark renamed then deleted states as deleted', function() {
      ...
      vts.generateVersionTree(testSnapshots1);
      cvs.getStatesData(11, 13).then(function(data) {
        expect(data).toEqual({
          1: {
            'newestStateName': 'D',
    ...
    it('should mark changed state as unchanged when name and content is same' +
      ...
      $httpBackend.expect('GET', '/createhandler/data/0?v=11')
          .respond(testExplorationData1[10]);
      vts.generateVersionTree(testSnapshots1);
      cvs.getStatesData(1, 11).then(function(data) {
        expect(data).toEqual({
          1: {
            'newestStateName': 'A',
            'stateProperty': 'unchanged',
    ...
    it('should mark renamed state as not renamed when name is same on both versions',
      ...
      vts.generateVersionTree(testSnapshots1);
      cvs.getStatesData(2, 4).then(function(data) {
        expect(data).toEqual({
          1: {
            'newestStateName': 'A',
            'stateProperty': 'unchanged',
    ...
    it('should mark states correctly when a series of changes are applied',
      ...
      $httpBackend.expect('GET', '/createhandler/data/0?v=13')
          .respond(testExplorationData1[12]);
      vts.generateVersionTree(testSnapshots1);
      cvs.getStatesData(1, 13).then(function(data) {
        expect(data).toEqual({
          1: {
            'newestStateName': 'D',
            'stateProperty': 'deleted',
    ...
    var testExplorationData2 = [
      ...
      },
      {
        'states': {
          'A': 'dummy_exploration_data',
          'C': 'dummy_exploration_data',
          'D': 'new_dummy_exploration_data'
        }
      }
    ];

    // Tests for getStatesData with reversions
    it('should mark states correctly when there is 1 reversion', function() {
      ...
      vts.generateVersionTree(testSnapshots2);
      cvs.getStatesData(1, 5).then(function(data) {
        expect(data).toEqual({
          1: {
            'newestStateName': 'A',
            'stateProperty': 'unchanged',
    ...
    it('should mark states correctly when there is 1 reversion to before v1', function() {
      ...
      $httpBackend.expect('GET', '/createhandler/data/0?v=5')
          .respond(testExplorationData2[4]);
      vts.generateVersionTree(testSnapshots2);
      cvs.getStatesData(3, 5).then(function(data) {
        expect(data).toEqual({
          1: {
            'newestStateName': 'A',
            'stateProperty': 'unchanged',
    ...
    it('should mark states correctly when compared version is a reversion', function() {
      ...
      $httpBackend.expect('GET', '/createhandler/data/0?v=5')
          .respond(testExplorationData2[4]);
      vts.generateVersionTree(testSnapshots2);
      cvs.getStatesData(4, 5).then(function(data) {
        expect(data).toEqual({
          1: {
            'newestStateName': 'A',
            'stateProperty': 'unchanged',
