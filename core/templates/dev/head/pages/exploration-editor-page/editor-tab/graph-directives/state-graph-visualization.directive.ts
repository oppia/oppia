...
oppia.directive(
  'stateGraphVisualization', [
    'UrlInterpolationService', function(UrlInterpolationService) {
      return {
        // Note: This directive is used as attribute because pannability does
        //    not work when directive is used as element. (Convention in the
        //    codebase is to use directive as element.)
        restrict: 'A',
        scope: {
          allowPanning: '@',
          ...
          //  - 'finalStateIds': The list of ids corresponding to terminal
          //             states (i.e., those whose interactions are terminal).
          graphData: '&',
          // Object whose keys are ids of nodes to display a warning tooltip
          // over
          highlightStates: '=',
          ...
          // values are secondary labels. If this is undefined, it means no
          // nodes have secondary labels.
          nodeSecondaryLabels: '=',
            ...
            var redrawGraph = function() {
              if ($scope.graphData()) {
                ...
                $scope.drawGraph(
                  ...
                  $scope.graphData().initStateId,
                  $scope.graphData().finalStateIds
                );
            ...
            $scope.drawGraph = function(
              ...
              $scope.getNodeTitle = function(node) {
                ...
                if (node.reachable === false) {
                  ...
                  warning = (
                    'Warning: there is no path from this state to the END' +
                    ' state.'
                  );
                }
              ...
              if ($scope.allowPanning) {
                // Without the timeout, $element.find fails to find the
                // required rect in the state graph modal dialog.
                $timeout(function() {
                  ...
                  d3.select($element.find('rect.pannable-rect')[0])
                    .call(d3.behavior.zoom().scaleExtent([1, 1])
                      .on('zoom', function() {
                        if (
                          graphBounds.right - graphBounds.left < dimensions.w) {
                        ...
                        if (
                          graphBounds.bottom - graphBounds.top < dimensions.h) {
                        ...
                        $scope.innerTransformStr = (
                          'translate(' + (
                            <d3.ZoomEvent>d3.event).translate + ')'
                        );
                        $scope.$apply();
                      })
                    );
              ...
              if ($scope.centerAtCurrentState) {
                $timeout(function() {
                  ...
                  // Center the graph at the node representing the current
                  // state.
                  origTranslations[0] = (
