...
oppia.factory('TranslationRecordingService',['$q', '$window', function($q, $window){
      ...
      mp3Worker.onmessage = function(e) {
        switch(e.data.cmd) {
          case 'end':
            ...
            break;
          default :
            console.log("Unexpected command and data type from worker.");
            // console.warn(e);
        }
      }
  ...
  return {
    ...
    startRecord : function() {
      ...
      navigator.then(function(stream) {
        // set microphone stream will be used for stopping track stream in another function.
        microphoneStream = stream;
        _processMicAudio(stream);
      },function(error){
        console.log("Microphone was not started because of user denied permission.");
      });
