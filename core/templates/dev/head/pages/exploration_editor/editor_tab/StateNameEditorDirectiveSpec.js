...
describe('Sidebar state name controller', function() {
  describe('SidebarStateName', function() {
    ...
    it('should correctly normalize whitespace in a state name', function() {
      ...
      expect(scope._getNormalizedStateName('Fourth State'))
        .toEqual('Fourth State');
      expect(scope._getNormalizedStateName('    '))
        .toEqual('');
      expect(scope._getNormalizedStateName('Z    '))
        .toEqual('Z');
      expect(scope._getNormalizedStateName('    .'))
        .toEqual('.');
    });
    ...
    it('should not save duplicate state names', function() {
      expect(scope.saveStateName('Second State'))
        .toBe(false);
    });
    ...
    it('should allow state names to be variations of \'END\'', function() {
      ...
      expect(scope.saveStateName('END'))
        .toBe(true);
      expect(scope.saveStateName('enD'))
        .toBe(true);
      expect(scope.saveStateName('end'))
    ...
    it('should check that state name edits are independent', function() {
      ...
      expect(ecs.getActiveStateName())
        .toEqual('Fourth State');
      expect(ess.getState('Fourth State'))
        .toBeTruthy();
      expect(ess.getState('Third State'))
        .toBeFalsy();

      ecs.setActiveStateName('First State');
      scope.saveStateName('Fifth State');
      expect(ess.getState('Fifth State'))
        .toBeTruthy();
      expect(ess.getState('First State'))
        .toBeFalsy();
      expect(mockExplorationData.autosaveChangeList)
        .toHaveBeenCalled();
      $httpBackend.expectPUT(autosaveDraftUrl)
