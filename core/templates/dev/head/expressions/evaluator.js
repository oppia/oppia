...
oppia.factory('expressionEvaluatorService', [
  '$log', 'expressionParserService', function($log, expressionParserService) {
    ...
    ExprWrongNumArgsError.prototype.toString = function() {
      return (
        this.name + ': {' + this.args + '} not in range [' + this.expectedMin +
        ',' + this.expectedMax + ']');
    };
    ...
    /**
    * @param {*} parsed - Parse output from the parser. See parser.pegjs for the
    *   data structure.
    ...
    */
    var evaluate = function(parsed, envs) {
      ...
      // are JavaScript primitives (as described in the "Parser output" section
      // of parser.pegjs).
    ...
    * system or user defined functions and parameters, as well as system
    * operators.
    * @param {string} name - The name to look up.
    * @param {!Array.<!Object>} envs - Represents a nested name space
    *   environment to look up the name in. The first element is looked up first
    *   (i.e. has higher precedence).
    * @throws {ExprUndefinedVarError} The named variable was not found in the
    *   given environment.
    */
