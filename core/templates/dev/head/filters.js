...
oppia.filter('spacesToUnderscores', [
  function() {
...
oppia.filter('underscoresToCamelCase', [
  function() {
    return function(input) {
...
oppia.filter('camelCaseToHyphens', [
  function() {
    return function(input) {
...
oppia.filter('truncateAtFirstLine', [
  function() {
    return function(input) {
      if (!input) {
        return input;
...
oppia.filter('replaceInputsWithEllipses', [
  function() {
...
oppia.filter('truncateAtFirstEllipsis', [
  function() {
    var pattern = /\.\.\./g;
    return function(input) {
      if (!input) {
  ...
  '$filter', 'UtilsService', function($filter, UtilsService) {
    return function(input, characterCount) {
      if (UtilsService.isString(input)) {
        ...
        return input.substr(0, characterCount - 3)
          .trim() + '...';
      } else {
        return input;
      }
...
oppia.filter('normalizeWhitespace', [
...
oppia.filter('normalizeWhitespacePunctuationAndCase', [
  function() {
    return function(input) {
      if (typeof input === 'string' || input instanceof String) {
        ...
        for (var i = 0; i < inputLines.length; i++) {
          ...
          for (var j = 0; j < inputLine.length; j++) {
            var currentChar = inputLine.charAt(j)
              .toLowerCase();
            if (currentChar === ' ') {
...
oppia.filter('convertToPlainText', [
  function() {
    return function(input) {
...
oppia.filter('summarizeNonnegativeNumber', [
  function() {
    return function(input) {
      input = Number(input);
...
oppia.filter('truncateAndCapitalize', [
  function() {
  ...
  function() {
    return function(input) {
      ...
      return trimmedInput.charAt(0)
        .toUpperCase() + trimmedInput.slice(1);
    };
  }
]);

oppia.filter('removeDuplicatesInArray', [
  function() {
    return function(input) {
...
oppia.filter('stripFormatting', [
  function() {
    ...
    // must be removed.
      ...
      ').*".*>)(?!<b>|<\/b>|<i>|<\/i>|<p>|<\/p>|<br>)<[^>]+>', 'gm');
      var strippedText = html ? String(html)
        .replace(styleRegex, '') : '';
      strippedText = strippedText ? String(strippedText)
        .replace(
          tagRegex, '') : '';
      return strippedText;
    };
...
oppia.filter('getAbbreviatedText', [
  function() {
