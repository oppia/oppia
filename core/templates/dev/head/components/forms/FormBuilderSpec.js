  ...
  it('should convert HTML to and from raw text correctly', inject[
    function($filter) {
      htmlUnicodeHtmlPairings.forEach(function(pairing) {
        expect($filter('convertHtmlToUnicode')(pairing[0]))
          .toEqual(pairing[1]);
        expect($filter('convertUnicodeToHtml')(pairing[1]))
          .toEqual(pairing[2]);
      });
    }
  ]);
...
describe('Normalizer tests', function() {
  ...
  it('should impose minimum bounds', inject[function($filter) {
    ...
    expect(filter(1.23, args))
      .toBe(true);
    expect(filter(-1.23, args))
      .toBe(true);
    expect(filter(-1.99, args))
      .toBe(true);
    expect(filter(-2, args))
      .toBe(true);
    expect(filter(-2.01, args))
      .toBe(false);
    expect(filter(-3, args))
      .toBe(false);
  }]);
...
describe('Testing apply-validation directive', function() {
  ...
  it('should apply isAtLeast validation', inject[function($compile) {
    ...
    expect(testInput.$valid)
      .toEqual(true);
    expect(Object.keys(testInput.$error).length)
      .toEqual(0);
