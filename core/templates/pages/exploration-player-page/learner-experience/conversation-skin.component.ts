...
export class ConversationSkinComponent {
  ...
  ngOnInit(): void {
    this.conversationSkinService._editorPreviewMode =
    ...
    this.directiveSubscriptions.add(
      this.playerPositionService.onNewCardOpened.subscribe(
        (newCard: StateCard) => {
          ...
          this.conversationSkinService.nextCardIfStuck = null;
    ...
    this.directiveSubscriptions.add(
      this.conversationSkinService.onGiveFeedbackAndStayOnCurrentCard.subscribe(
        args => {
          this.giveFeedbackAndStayOnCurrentCard(
            args.feedbackHtml,
            args.missingPrerequisiteSkillId,
            args.refreshInteraction,
            args.refresherExplorationId
          );
        }
      )
    );
    ...
    this.directiveSubscriptions.add(
      this.conversationSkinService.onMoveToNewCard.subscribe(args => {
        this.moveToNewCard(
          args.feedbackHtml,
    ...
    this.directiveSubscriptions.add(
      this.conversationSkinService.onShowUpcomingCard.subscribe(() => {
        this.showUpcomingCard();
      })
    );

    this.directiveSubscriptions.add(
      this.conversationSkinService.onShowPendingCard.subscribe(() => {
    ...
    this.directiveSubscriptions.add(
      this.explorationPlayerStateService.onPlayerStateChange.subscribe(
        newStateName => {
          ...
          if (!this.conversationSkinService._editorPreviewMode) {
          ...
          if (
            !this.conversationSkinService._editorPreviewMode &&
            this.conversationSkinService.nextCard.isTerminal()
          ) {
            ...
            this.statsReportingService.recordExplorationCompleted(
              ...
              String(
                this.conversationSkinService.completedChaptersCount &&
                  this.conversationSkinService.completedChaptersCount + 1
              ),
            ...
            if (!this.conversationSkinService.explorationActuallyStarted) {
              this.statsReportingService.recordExplorationActuallyStarted(
                newStateName
              );
              this.conversationSkinService.explorationActuallyStarted = true;
            }
          }
        }
      )
    ...
    this.userService.getUserInfoAsync().then(async userInfo => {
      ...
      this.windowRef.nativeWindow.addEventListener('beforeunload', e => {
        ...
        if (
          ...
          !this.conversationSkinService.displayedCard.isTerminal() &&
  ...
  isCorrectnessFooterEnabled(): boolean {
    return (
      this.conversationSkinService.answerIsCorrect &&
  ...
  isLearnAgainButton(): boolean {
    ...
    let interaction =
      this.conversationSkinService.displayedCard.getInteraction();
    ...
    return (
      this.conversationSkinService.pendingCardWasSeenBefore &&
      !this.conversationSkinService.answerIsCorrect
    );
  }
  ...
  private _navigateToDisplayedCard(): void {
    ...
    this.conversationSkinService.displayedCard =
      this.playerTranscriptService.getCard(index);
    ...
    if (
      this.conversationSkinService._nextFocusLabel &&
      ...
      if (this.parentExplorationIds.length > 0) {
        ...
        recommendedExplorationIds =
          this.explorationEngineService.getAuthorRecommendedExpIdsByStateName(
            this.conversationSkinService.displayedCard.getStateName()
          );
  ...
  triggerIfLearnerStuckAction(): void {
    ...
    this.responseTimeout = setTimeout(() => {
      if (
        this.conversationSkinService.nextCardIfStuck &&
        this.conversationSkinService.nextCardIfStuck !==
          this.conversationSkinService.displayedCard
      ) {
  ...
  triggerRedirectionToStuckState(): void {
    ...
    this.conversationSkinService.nextCard =
      this.conversationSkinService.nextCardIfStuck;
  ...
  private _initializeDirectiveComponents(initialCard, focusLabel): void {
    ...
    this.conversationSkinService.nextCard = initialCard;
    if (!this.explorationPlayerStateService.isInDiagnosticTestPlayerMode()) {
      this.explorationPlayerStateService.onPlayerStateChange.emit(
        this.conversationSkinService.nextCard.getStateName()
      );
    }
  ...
  skipCurrentQuestion(): void {
    this.explorationPlayerStateService.skipCurrentQuestion(nextCard => {
      this.conversationSkinService.nextCard = nextCard;
      this.showPendingCard();
    });
  }
  ...
  submitAnswer(
    ...
    this.conversationSkinService.answerIsBeingProcessed = true;
    ...
    if (this.getCanAskLearnerForAnswerInfo()) {
      setTimeout(() => {
        ...
        this.conversationSkinService.answerIsBeingProcessed = false;
  ...
  giveFeedbackAndStayOnCurrentCard(
    ...
    if (refreshInteraction) {
      ...
      this.playerTranscriptService.updateLatestInteractionHtml(
        this.conversationSkinService.displayedCard.getInteractionHtml() +
    ...
    this.conversationSkinService.scrollToBottom();
  }
  ...
  moveToNewCard(
    ...
    this.upcomingInteractionInstructions =
      this.conversationSkinService.nextCard.getInteractionInstructions();
  ...
  showPendingCard(): void {
    ...
    setTimeout(
      () => {
        this._addNewCard(this.conversationSkinService.nextCard);

        this.conversationSkinService.upcomingInlineInteractionHtml = null;
    ...
    this.playerPositionService.onNewCardOpened.emit(
      this.conversationSkinService.nextCard
    );
  }
  ...
  showUpcomingCard(): void {
    ...
    if (this.conversationSkinService.questionSessionCompleted) {
    ...
    if (this.conversationSkinService.moveToExploration) {
      this.conversationSkinService.moveToExploration = false;
    ...
    if (
      ...
      this.conversationSkinService.nextCard.getStateName() ===
        this.conversationSkinService.displayedCard.getStateName() &&
      this.conversationSkinService.conceptCard
    ) {
      this.explorationPlayerStateService.recordNewCardAdded();
      this._addNewCard(
        StateCard.createNewCard(
          null,
          this.conversationSkinService.conceptCard.getExplanation().html,
          null,
          null,
          null,
          null,
    ...
    if (this.isLearnAgainButton()) {
      ...
      if (indexOfRevisionCard !== null) {
        this.conversationSkinService.displayedCard.markAsNotCompleted();
    ...
    if (this.conversationSkinService.answerIsCorrect) {
      this.conversationSkinService.pendingCardWasSeenBefore = false;
    }
    this.conversationSkinService.answerIsCorrect = false;
    this.showPendingCard();
  }
