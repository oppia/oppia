...
type AlgorithmIdPredictionServiceMap = Map<
  string,
  Map<number, TextInputPredictionService>
>;
...
export class PredictionAlgorithmRegistryService {
  ...
  constructor(private textInputPredictionService: TextInputPredictionService) {
    this.algorithmIdPredictionServiceMapping = new Map(
      Object.entries({
        TextClassifier: new Map([[1, this.textInputPredictionService]]),
      })
    );
  }
  ...
  testOnlySetPredictionService(
    algorithmId: string,
    dataSchemaVersion: number,
    service: TextInputPredictionService
  ): void {
