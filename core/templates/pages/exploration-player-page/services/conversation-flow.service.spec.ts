...
fdescribe('Conversation flow service', () => {
  ...
  it('should return false for isLearnAgainButton when concept card is shown', () => {
    const mockCard = new StateCard(
      null,
      null,
      null,
    ...
    // pendingCardWasSeenBefore & getAnswerIsCorrect impact the return.
    ...
    expect(result).toBeFalse(); // conceptCardIsBeingShown => false
  });

  it('should return true for isLearnAgainButton when all conditions met', () => {
    const mockCard = new StateCard(
      'State',
      null,
      null,
  ...
  it('should throw when response.version is null', fakeAsync(() => {
    spyOn(playerPositionService, 'getDisplayedCardIndex').and.returnValue(1);
    spyOn(urlService, 'isIframed').and.returnValue(false);
    spyOn(pageContextService, 'isInExplorationEditorPage').and.returnValue(
      false
