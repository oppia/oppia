...
export class QuestionPlayerEngineService {
  ...
  submitAnswer(
    ...
    const interactionIsInline =
      !interactionId ||
      (
        InteractionSpecsConstants.INTERACTION_SPECS as Record<
          string,
          {display_mode: string}
        >
      )[interactionId].display_mode ===
  ...
  private getRandomSuffix(): string {
    // Appending a random suffix forces Angular's change detection to re-render
    // the component when the question HTML would otherwise be identical.
    let randomSuffix = '';
