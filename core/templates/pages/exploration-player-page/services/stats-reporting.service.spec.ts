...
describe('Stats reporting service ', () => {
  ...
  beforeEach(() => {
    ...
    spyOn(siteAnalyticsService, 'registerStartExploration').and.callThrough();
    spyOn(siteAnalyticsService, 'registerFinishExploration').and.callThrough();
    spyOn(
      siteAnalyticsService,
      'registerCuratedLessonCompleted'
    ).and.callThrough();
    spyOn(siteAnalyticsService, 'registerAnswerSubmitted').and.callThrough();
  ...
  it(
    'should set session properties when calling ' + "'initSession'",
    fakeAsync(() => {
      spyOn(urlService, 'getUrlParams').and.returnValue({
        classroom_url_fragment: 'classroom',
      });
      ...
      expect(statsReportingService.explorationVersion).toBeUndefined();
      ...
      expect(statsReportingService.explorationVersion).toEqual(
        explorationVersion
      );
  ...
  it(
    "should not again record exploration's stats when " +
  ...
  it(
    'should not send request to backend when an exploration ' +
      'it is about to start and already recorded',
    () => {
      spyOn(urlService, 'getUrlParams').and.returnValue({
        classroom_url_fragment: 'classroom',
      });
      let postStatsSpy = spyOn(
        statsReportingBackendApiService,
        'postsStatsAsync'
      ).and.returnValue(Promise.resolve({}));
      spyOn(
  ...
  it('should record stats when an answer is actually submitted', () => {
    ...
    expect(siteAnalyticsService.registerAnswerSubmitted).toHaveBeenCalledWith(
      explorationId,
      true
    );
  });
