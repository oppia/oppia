...
describe('Checkpoint celebration utility service', () => {
  ...
  it('should get a random i18n key when message kind is specified', () => {
    ...
    expect(
      checkpointCelebrationUtilityService.getRandomI18nKey(
        'KEY_PREFIX',
        10,
        'KIND_A'
      )
    ).toEqual('KEY_PREFIX_KIND_A_5');
  });
  ...
  it('should get the checkpoint message', () => {
    ...
    expect(
      checkpointCelebrationUtilityService.getCheckpointMessage(2, 8)
    ).toEqual('DUMMY_CHECKPOINT_MESSAGE_KEY');
    expect(
  ...
  it('should get the checkpoint title', () => {
    spyOn(
      checkpointCelebrationUtilityService,
      'getCheckpointTitleI18nKey'
    ).and.returnValue('DUMMY_CHECKPOINT_TITLE_I18N_KEY');
    spyOn(translateService, 'instant').and.callThrough();

    expect(checkpointCelebrationUtilityService.getCheckpointTitle()).toEqual(
  ...
  it('should correctly set and retrieve isOnCheckpointedState value', () => {
    expect(checkpointCelebrationUtilityService.getIsOnCheckpointedState()).toBe(
      false
    );
