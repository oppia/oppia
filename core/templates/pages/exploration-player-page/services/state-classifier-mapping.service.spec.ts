...
describe('State classifier mapping service', () => {
  describe('Test that classifier data is fetched properly when ML is enabled', () => {
    ...
    beforeEach(() => {
      ...
      spyOn(
        appService,
        'isMachineLearningClassificationEnabled'
    ...
    it('should handle failure of fetching classifier data', waitForAsync(async () => {
      spyOn(
        classifierDataBackendApiService,
        'getClassifierDataAsync'
      ).and.callFake(() => {
    ...
    it('should return classifier data when it exists.', () => {
      ...
      let retrievedClassifier = mappingService.getClassifier(
        stateName
      ) as Classifier;
    ...
    it('should return undefined when classifier data does not exist.', () => {
      ...
      var nonExistentClassifier =
        mappingService.getClassifier(stateNameNonexistent);
    ...
    it('should return false when it does not have classifier data .', () => {
      mappingService.init(expId, expVersion);
      var stateNameNonexistent = 'stateName2';
      expect(mappingService.hasClassifierData(stateNameNonexistent)).toBe(
        false
      );
    });

    it(
      'should not return correct classifier details when init is not ' +
        'called',
      () => {
