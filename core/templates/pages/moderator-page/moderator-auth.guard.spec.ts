...
/**
 * @fileoverview Tests for ModeratorAuthGuard
 */
...
describe('ModeratorAuthGuard', () => {
  let userService: UserService;
  let router: Router;
  let guard: ModeratorAuthGuard;
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(ModeratorAuthGuard);
    userService = TestBed.inject(UserService);
    router = TestBed.inject(Router);
  });

  it('should redirect user to 401 page if user is not moderator', done => {
  ...
  it('should not redirect user to 401 page if user is moderator', done => {
    const getUserInfoAsyncSpy = spyOn(
      userService,
      'getUserInfoAsync'
    ).and.returnValue(
      Promise.resolve(
        new UserInfo([], true, false, false, false, false, '', '', '', true)
      )
    );
    const navigateSpy = spyOn(router, 'navigate').and.callThrough();
