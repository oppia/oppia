...
} from 'domain/blog/blog-post-editor-backend-api.service';
...
export class BlogPostEditorComponent implements OnInit {
  ...
  initEditor(): void {
    this.blogPostEditorBackendService
      .fetchBlogPostEditorData(this.blogPostId)
      .then(
        (editorData: BlogPostEditorData) => {
          ...
          if (this.blogPostData.publishedOn && this.blogPostData.lastUpdated) {
            if (
              this.blogPostData.lastUpdated.slice(0, -8) ===
  ...
  updateLocalTitleValue(): void {
    this.blogPostUpdateService.setBlogPostTitle(this.blogPostData, this.title);
    ...
    if (
      ...
      this.blogPostEditorBackendService
        .doesPostWithGivenTitleAlreadyExistAsync(this.blogPostId, this.title)
        .then(
          (response: boolean) => {
            if (!response) {
  ...
  updateBlogPostData(isBlogPostPublished: boolean): void {
    ...
    this.blogPostEditorBackendService
      .updateBlogPostDataAsync(this.blogPostId, isBlogPostPublished, changeDict)
      .then(
        () => {
    ...
    this.blogPostEditorBackendService
      ...
      .then(
        () => {
          ...
          this.alertsService.addSuccessMessage('Thumbnail Saved Successfully.');
        },
        errorResponse => {
          this.alertsService.addWarning(
