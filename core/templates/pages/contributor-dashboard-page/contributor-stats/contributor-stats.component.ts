...
import {
  ...
  QuestionContributionBackendDict,
  QuestionReviewBackendDict,
...
export class ContributorStatsComponent {
  ...
  options: Option[] = [this.translationContributionOption];
  ...
  async fetchStats(): Promise<void> {
    ...
    if (response.translation_contribution_stats.length > 0) {
      response.translation_contribution_stats.map(stat => {
        const translationContributionStatsData =
          this.statsData.translationContribution.get(stat.language_code);
    ...
    if (response.translation_review_stats.length > 0) {
      response.translation_review_stats.map(stat => {
        const translationReviewStatsData = this.statsData.translationReview.get(
          stat.language_code
        );
  ...
  goToNextPage(page: PageableStats): void {
    if (page.currentPageStartIndex + this.ITEMS_PER_PAGE >= page.data?.length) {
  ...
  openCertificateDownloadModal(
    ...
    const modalRef = this.modalService.open(CertificateDownloadModalComponent, {
      size: 'lg',
      backdrop: 'static',
...
angular
  ...
  .directive(
    ...
    downgradeComponent({component: ContributorStatsComponent})
  );
