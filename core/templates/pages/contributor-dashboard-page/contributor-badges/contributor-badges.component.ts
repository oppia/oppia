...
export class ContributorBadgesComponent {
    ...
    async ngOnInit(): Promise<void> {
        ...
        const allContributionStats =
            await this.contributionAndReviewStatsService.fetchAllStats(
                username
            );
        ...
        if (allContributionStats.translation_contribution_stats.length > 0) {
            allContributionStats.translation_contribution_stats.forEach(
                stat => {
    ...
    onDocumentClick(event: MouseEvent): void {
        ...
        if (
            targetElement &&
            !this.mobileBadgeTypeDropdownRef.nativeElement.contains(
                targetElement
            )
        ) {
            this.mobileBadgeTypeDropdownShown = false;
