...
export class TranslateTextService {
  ...
  /**
   * Submits a translated suggestion to the backend.
   *
   * When wasAutoGenerated is true, the auto-generation metadata is forwarded
   * to the backend so reviewers can distinguish AI-populated translations from
   * manually written ones and know whether the contributor edited the
   * AI-generated text before submission.
   *
   * @param translation - The translated HTML or string.
   * @param languageCode - BCP-47 target language code.
   * @param imagesData - Image blobs attached to the suggestion.
   * @param dataFormat - 'html', 'unicode', or a set-of-strings format.
   * @param wasAutoGenerated - True when the text was pre-populated by the
   *   AI auto-translate feature. Defaults to false.
   * @param autoGenerationProvider - Internal provider ID (e.g. 'azure').
   *   Required when wasAutoGenerated is true.
   * @param wasEdited - True when the contributor modified the AI suggestion
   *   before submission. Defaults to false.
   * @param successCallback - Called on successful submission.
   * @param errorCallback - Called with the error reason on failure.
   */
