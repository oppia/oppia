...
export class TranslationModalComponent {
  ...
  autoGeneratedTranslation: string | string[] = '';
  activeStatus!: Status;
  activeLanguageCode!: string;
  isGeneratingTranslation: boolean = false;
  isTranslationAutoGenerated: boolean = false;
  HTML_SCHEMA!: {
    type: 'html';
    ui_config: UiConfig;
  };
  ...
  isAutomaticTranslationSuggestionsEnabled(): boolean {
    return this.platformFeatureService.status
      .EnableAutomaticTranslationSuggestions.isEnabled;
  }

  generateTranslation(): void {
    if (this.isGeneratingTranslation) {
      return;
    }
    this.isGeneratingTranslation = true;

    // We only support html data format currently.
    if (
      this.activeDataFormat !== 'html' ||
      typeof this.textToTranslate !== 'string'
    ) {
      this.isGeneratingTranslation = false;
      return;
    }

    // Default target language code is the active language code.
    // Source language code is typically English.
    const sourceLanguageCode = 'en';
    const targetLanguageCode = this.activeLanguageCode;
    ...
    this.translateTextBackendApiService
      ...
      .then(response => {
        ...
        this.autoGeneratedTranslation = translatedText;
        this.isGeneratingTranslation = false;
        this.isTranslationAutoGenerated = true;
        this.updateHtml(translatedText);
      })
      .catch(error => {
        this.isGeneratingTranslation = false;
        this.alertsService.addWarning('Failed to generate translation.');
      });
  }
  ...
  isTranslationEdited(): boolean {
    if (!this.isTranslationAutoGenerated) {
      return false;
    }

    // Auto-translation is only supported for the html format (string values).
    // For set-of-strings formats the values are arrays; we cannot meaningfully
    // compare them with HTML normalisation, so we treat them as edited.
    if (
      typeof this.activeWrittenTranslation !== 'string' ||
      typeof this.autoGeneratedTranslation !== 'string'
    ) {
      return true;
    }

    const html1: string = this.activeWrittenTranslation;
    const html2: string = this.autoGeneratedTranslation;
    ...
    const normalize = (html: string): string => {
      ...
      // CKEditor often wraps raw text in <p> tags — strip them for comparison.
      if (str.startsWith('<p>') && str.endsWith('</p>')) {
        str = str.substring(3, str.length - 4).trim();
      }

      // Use DOMParser to decode HTML entities and extract plain text.
      const parsed = new DOMParser().parseFromString(str, 'text/html');
      const text = parsed.body.textContent ?? '';

      // Normalise all whitespace sequences to a single space.
      return text.replace(/\s+/g, ' ').trim();
    };

    if (normalize(html1) === normalize(html2)) {
      return false;
    }
  ...
  suggestTranslatedText(): void {
    ...
    const confirmUneditedThenProceed = () => {
      if (this.isTranslationAutoGenerated && !this.isTranslationEdited()) {
        const modalRef = this.ngbModal.open(
  ...
  updateTranslatedText(): void {
    ...
    const confirmUneditedThenProceed = () => {
      if (
        this.isTranslationAutoGenerated &&
        this.activeWrittenTranslation === this.autoGeneratedTranslation
      ) {
        const modalRef = this.ngbModal.open(
          TranslationModalUneditedConfirmationModalComponent,
          {
