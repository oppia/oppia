...
} from 'pages/contributor-dashboard-page/services/translate-text-backend-api.service';
...
export class TranslationModalComponent {
  ...
  // True while the auto-translate API call is in flight.
  isAutoTranslating: boolean = false;
  // Set to true once the editor is populated by the AI auto-translate feature.
  wasAutoGenerated: boolean = false;
  // Internal ID of the provider that produced the translation (e.g. 'azure').
  autoGenerationProvider: string = '';
  // Set to true when the contributor edits the AI-generated translation before
  // submission.
  ...
  constructor(
    ...
    private readonly translateTextBackendApiService: TranslateTextBackendApiService,
  ...
  updateHtml($event: SchemaDefaultValue): void {
    if ($event !== this.activeWrittenTranslation) {
      ...
      // Mark the translation as edited if the contributor modified an
      // AI-generated suggestion before submitting it.
  ...
  suggestTranslatedText(): void {
    ...
    if (!this.uploadingTranslation && !this.loadingData) {
      ...
      this.translateTextService.suggestTranslatedText(
        ...
        imagesData,
        this.activeDataFormat,
        this.wasAutoGenerated,
        this.autoGenerationProvider,
        this.wasEdited,
        () => {
  ...
  /**
   * Requests an AI-generated translation from the backend and populates the
   * translation editor with the result.
   *
   * Only works for html/unicode formats; silently returns for set-of-strings.
   * On success, sets wasAutoGenerated=true so the metadata is forwarded when
   * the contributor submits. On failure, shows a dismissible warning.
   */
  async requestAutoTranslation(): Promise<void> {
    // Auto-translate is not applicable for set-of-strings formats because
    // those require one translation entry per answer choice, which the AI
    // response does not produce.
    if (this.isAutoTranslating || this.isSetOfStringDataFormat()) {
      return;
    }
    this.isAutoTranslating = true;
    try {
      const response: MachineTranslationResponse =
        await this.translateTextBackendApiService.getMachineTranslationAsync(
          this.textToTranslateAsString,
          'en',
          this.activeLanguageCode
        );
      this.wasAutoGenerated = true;
      this.autoGenerationProvider = response.translation_provider;
      this.wasEdited = false;
      this.activeWrittenTranslation = response.translated_text;
      // Destroy and re-create the schema-based-editor so it picks up the
      // programmatically set value (see existing resetEditor() TODO comment).
      this.resetEditor();
      this.updateTranslationErrors();
    } catch (error: unknown) {
  ...
  private clearTranslation(): void {
    this.activeWrittenTranslation = '';
    // Reset auto-generation state so the next card starts with a clean slate.
    this.wasAutoGenerated = false;
    this.autoGenerationProvider = '';
    this.wasEdited = false;
    this.updateTranslationErrors();
  }
