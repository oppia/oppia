...
export class TranslationModalComponent {
  ...
  hasImageInAutoTranslation: boolean = false;
  hasAltTextModalBeenOpened: boolean = false;
  HTML_SCHEMA!: {
    type: 'html';
    ui_config: UiConfig;
  };
  ...
  generateTranslation(): void {
    ...
    this.translateTextBackendApiService
      ...
      .then(response => {
        ...
        this.hasImageInAutoTranslation = translatedText.includes(
          '<oppia-noninteractive-image'
        );
        this.hasAltTextModalBeenOpened = false;
  ...
  hasSubmitValidationErrors(): boolean {
    if (this.hasImageInAutoTranslation && !this.hasAltTextModalBeenOpened) {
      return true;
    }
    return (
