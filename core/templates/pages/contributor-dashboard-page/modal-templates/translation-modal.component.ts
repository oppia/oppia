...
export class TranslationModalComponent {
  ...
  isTranslationEdited(): boolean {
    ...
    const html1 = this.activeWrittenTranslation;
    const html2 = this.autoGeneratedTranslation;
    ...
    const normalize = (html: string) => {
      ...
      // CKEditor often wraps raw text in <p> tags.
      if (str.startsWith('<p>') && str.endsWith('</p>')) {
        str = str.substring(3, str.length - 4).trim();
      }

      // Parse HTML to normalize structure and decode entities
      const div = document.createElement('div');
      div.innerHTML = str;

      // CKEditor may also replace some spaces with &nbsp;
      return div.innerHTML.replace(/&nbsp;/g, ' ').replace(/\s+/g, ' ').trim();
    };
