...
interface availabilityStatusType {
  available: boolean;
  needsUpdate: boolean;
}
...
export class TranslationStatusService implements OnInit {
  ...
  _getContentAvailabilityStatus(
      stateName: string, contentId: string): availabilityStatusType {
    this.langCode = this.translationLanguageService.getActiveLanguageCode();
  ...
  _getActiveStateContentAvailabilityStatus(contentId: string):
    availabilityStatusType {
    if (this.translationTabActiveModeService.isTranslationModeActive()) {
...
angular.module('oppia').factory(
  'TranslationStatusService', downgradeInjectable(
