...
import { AngularNameService } from './angular-name.service';
...
import { StateEditorRefreshService } from './state-editor-refresh.service';
...
export class ExplorationStatesService {
  ...
  /** @private */
  refreshGraphEventEmitter = new EventEmitter();
  ...
  private _setState(stateName: string, stateData, refreshGraph: boolean): void {
    ...
    if (refreshGraph) {
      this.refreshGraphEventEmitter.emit();
    }
  }

  getStatePropertyMemento(stateName: string, backendName: string): State {
    let accessorList = this.PROPERTY_REF_DATA[backendName];
    let propertyRef = this._states.getState(stateName);
    try {
      accessorList.forEach((key: string | number) => {
  ...
  saveStateProperty(
    ...
    let oldValue = (
      this.getStatePropertyMemento(
        stateName, backendName) as unknown as string);
    ...
    if (this.BACKEND_CONVERSIONS.hasOwnProperty(backendName)) {
      ...
      oldBackendValue = (
        this.convertToBackendRepresentation(
          oldValue, backendName));
    }

    if (!angular.equals(oldValue, newValue)) {
      this.changeListService.editStateProperty(
        stateName, backendName, newBackendValue,
         oldBackendValue as unknown as string);
  ...
  convertToBackendRepresentation(
      frontendValue: string, backendName: string): string {
        ...
        let result = (
          this.answerClassificationService.getMatchingClassificationResult(
            ...
            this.angularNameService.getNameOfInteractionRulesService(
              this._states.getState(stateName).interaction.id
  ...
  getStateContentMemento(stateName: string): State {
    return this.getStatePropertyMemento(stateName, 'content');
  }

  saveStateContent(stateName: string, newContent: string): void {
    this.saveStateProperty(stateName, 'content', newContent);
  }

  getStateParamChangesMemento(stateName: string): State {
    return this.getStatePropertyMemento(stateName, 'param_changes');
  }

  saveStateParamChanges(stateName: string, newParamChanges: string): void {
    this.saveStateProperty(stateName, 'param_changes', newParamChanges);
  }

  getInteractionIdMemento(stateName: string): State {
  ...
  getInteractionAnswerGroupsMemento(stateName: string): State {
    return this.getStatePropertyMemento(stateName, 'answer_groups');
  }

  saveInteractionAnswerGroups(
      stateName: string, newAnswerGroups: string): void {
  ...
  saveWrittenTranslation(
      contentId: string, dataFormat: DataFormatToDefaultValuesKey,
      languageCode: string, stateName: string, translationHtml: string): void {
  ...
  markWrittenTranslationsAsNeedingUpdate(
      contentId: string, stateName: string): void {
  ...
  addState(newStateName: string, successCallback: (arg0) => void): void {
  ...
  get onRefreshGraph(): EventEmitter<unknown> {
    return this.refreshGraphEventEmitter;
  }
}
