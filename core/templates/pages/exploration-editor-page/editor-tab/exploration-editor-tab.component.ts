...
export class ExplorationEditorTabComponent
  ...
  saveButtonTutorialElement = {
    type: 'element',
    selector: this._ID_TUTORIAL_SAVE_BUTTON,
    heading: 'Save',
    text: (
  ...
  //     let ngJoyrideTemplate = $templateCache.get(
  ...
  EDITOR_TUTORIAL_DETAILS = [
    {
      id: 'ExplorationEditorTabTourTemplate0',
  ...
  EDITOR_TUTORIAL_OPTIONS = [{
    ...
    type: 'element',
    selector: this._ID_TUTORIAL_STATE_CONTENT,
    heading: 'Content',
    text: (
      '<p>An Oppia exploration is divided into several \'cards\'. ' +
      'The first part of a card is the <b>content</b>.</p>' +
    ...
    type: 'function',
    fn: function(isGoingForward: string): void {
      let idToScrollTo = (
        isGoingForward ? this._ID_TUTORIAL_STATE_INTERACTION :
        this._ID_TUTORIAL_STATE_CONTENT);
      $('html, body').animate({
    ...
    type: 'title',
    selector: this._ID_TUTORIAL_STATE_INTERACTION,
    heading: 'Interaction',
    text: (
      '<p>After you\'ve written the content of your conversation, ' +
      'choose an <b>interaction type</b>. ' +
    ...
    fn: function(isGoingForward: string): void {
      let idToScrollTo = (
        isGoingForward ? this._ID_TUTORIAL_PREVIEW_TAB :
    ...
    type: 'element',
    selector: this._ID_TUTORIAL_PREVIEW_TAB,
    heading: 'Preview',
    text: (
      'At any time, you can click the <b>preview</b> button to ' +
      'play through your exploration.'),
    placement: 'bottom'
  }, this.saveButtonTutorialElement, {
    type: 'title',
    heading: 'Tutorial Complete',
    text: (
      '<h2>Now for the fun part...</h2>' +
  ...
  startTutorial(): void {
    ...
    this.joyride.startTour(
      ...
      stepDefaultPosition: 'top', themeColor: '#212f23' }
    ).subscribe((value)=> {
      this.index = Number(value.number) - 1;
      let html = (
        '<h3>' + this.EDITOR_TUTORIAL_DETAILS[this.index].heading +
        '</h3>' + this.EDITOR_TUTORIAL_DETAILS[this.index].text);

      if (document.getElementsByClassName('defaultEditorTour')[0]) {
        document.getElementsByClassName(
          // eslint-disable-next-line oppia/no-inner-html
          'defaultEditorTour')[0].innerHTML = html;
      }
    });
  }
    ...
    this.userExplorationPermissionsService.getPermissionsAsync()
      .then((permissions) => {
        if (!permissions.canEdit) {
          let index = this.EDITOR_TUTORIAL_OPTIONS.indexOf(
            this.saveButtonTutorialElement);
          this.EDITOR_TUTORIAL_OPTIONS.splice(index, 1);
        }
      });
  }
  ...
  onSkipTutorial(): void {
    this.siteAnalyticsService.registerSkipTutorialEvent(
      this.explorationId);
    this.leaveTutorial();
  }
  ...
  saveInteractionId(displayedValue: string): void {
    ...
    this.stateEditorService.setInteractionId(angular.copy(displayedValue));
  }
  ...
  saveSolution(displayedValue: Solution | SubtitledHtml): void {
    this.explorationStatesService.saveSolution(
      this.stateEditorService.getActiveStateName(),
      angular.copy(displayedValue) as SubtitledHtml);

    this.stateEditorService.setInteractionSolution(
      angular.copy(displayedValue) as Solution);
  }
  ...
  saveLinkedSkillId(displayedValue: string): void {
    ...
    this.stateEditorService.setLinkedSkillId(angular.copy(displayedValue));
  }
