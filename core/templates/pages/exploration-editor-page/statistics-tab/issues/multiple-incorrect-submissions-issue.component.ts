...
angular.module('oppia').component('multipleIncorrectSubmissionsIssue', {
  template: require('./multiple-incorrect-submissions-issue.component.html'),
  controller: [
    ...
    function(
      ...
      ctrl.finalBlock = HtmlEscaperService.escapedJsonToObj(
        $attrs.finalBlock);
      ctrl.index = ctrl.finalBlock.length - 1;
      ctrl.actionStartIndex = parseInt($attrs.actionStartIndex);
      ctrl.stateName =
        ctrl.finalBlock[ctrl.index].actionCustomizationArgs.state_name.value;

      var _nonSubmitActions = [];
      for (
        var i = 0; ctrl.finalBlock[i] &&
        ctrl.finalBlock[i].actionType !== ACTION_TYPE_ANSWER_SUBMIT; i++) {
        _nonSubmitActions.push(ctrl.finalBlock[i]);
      }
