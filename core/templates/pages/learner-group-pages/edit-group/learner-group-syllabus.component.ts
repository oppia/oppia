...
export class LearnerGroupSyllabusComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.learnerGroup) {
      this.learnerGroupSyllabusBackendApiService
        .fetchLearnerGroupSyllabus(this.learnerGroup.id)
        .then(groupSyllabus => {
  ...
  saveNewSyllabusItems(): void {
    ...
    let modelRef = this.ngbModal.open(SyllabusAdditionSuccessModalComponent, {
      backdrop: true,
        ...
        this.learnerGroupBackendApiService
          ...
          .then(learnerGroup => {
            this.subtopicSummaries = this.subtopicSummaries.filter(
              subtopicSummary =>
                subtopicSummary.subtopicPageId !== subtopicPageId
            );
            this.learnerGroup = learnerGroup;
            this.setDisplayOrderOfSyllabusItems();
          });
