...
export class ContributorDashboardAdminStatsBackendApiService {
  ...
  async fetchCommunityStats(): Promise<CommunityContributionStatsBackendDict> {
    return new Promise((resolve, reject) => {
      this.http
        .get<CommunityContributionStatsBackendDict>(
  ...
  async fetchAssignedLanguageIds(username: string): Promise<string[]> {
    return new Promise((resolve, reject) => {
      this.http
        .get<UserRolesBackendResponse>(PageConstants.ADMIN_ROLE_HANDLER_URL, {
          params: {
        ...
        .toPromise()
        .then(response => resolve(response.coordinated_language_ids));
    });
  }
  ...
  > {
    ...
    this.params = {
      ...
      offset: nextOffset,
      topic_ids:
        filter.topicIds.length > 0 ? JSON.stringify(filter.topicIds) : [],
    ...
    if (contributionType === AppConstants.CONTRIBUTION_STATS_TYPE_TRANSLATION) {
      if (
        ...
        return new Promise((resolve, reject) => {
          this.http
            .get<TranslationSubmitterStatsBackendDict>(url, {
              params: this.params,
            } as Object)
            .toPromise()
            .then(
        ...
        return new Promise((resolve, reject) => {
          this.http
            .get<TranslationReviewerStatsBackendDict>(url, {
              params: this.params,
            } as Object)
            .toPromise()
            .then(
      ...
      if (
        ...
        return new Promise((resolve, reject) => {
          this.http
            .get<QuestionSubmitterStatsBackendDict>(url, {
              params: this.params,
            } as Object)
            .toPromise()
            .then(
        ...
        return new Promise((resolve, reject) => {
          this.http
            .get<QuestionReviewerStatsBackendDict>(url, {
              params: this.params,
            } as Object)
            .toPromise()
            .then(
              response => {
                resolve({
                  stats: response.stats.map(backendDict =>
                    QuestionReviewerStats.createFromBackendDict(backendDict)
                  ),
                  nextOffset: response.next_offset,
                  more: response.more,
                });
  ...
  async fetchTopics(classroomId: string): Promise<TopicChoice[]> {
    const classroomPromise = this.classroomBackendApiService
      ...
      .then(classResponse => {
        return this.classroomBackendApiService.fetchClassroomDataAsync(
          classResponse.classroomDict.urlFragment
        );
      });
    return (await classroomPromise)._topicSummaries.map(obj => ({
      id: obj.id,
      topic: obj.name,
    }));
  }
