...
import {
  CommunityContributionStatsBackendDict,
...
describe('Contributor dashboard Admin page', () => {
  ...
  let questionCoordinatorInfo = new UserInfo(
    ['USER_ROLE', 'QUESTION_COORDINATOR'],
    true,
    false,
    false,
    false,
  ...
  let fullAccessUserInfo = new UserInfo(
    ['USER_ROLE', 'QUESTION_COORDINATOR', 'TRANSLATION_COORDINATOR'],
    true,
    false,
    false,
    false,
  ...
  beforeEach(waitForAsync(() => {
    ...
    ContAdminStatsSpy = spyOn(ContributorAdminStatsTable.prototype, 'ngOnInit');
  }));
  ...
  describe('when user is not logged in', () => {
    ...
    it(
      'should not update translation and question coordinator view' +
        'if username is null',
      fakeAsync(() => {
        component.ngOnInit();
        tick();
  ...
  describe('when no languages are available', () => {
    ...
    it(
      'should default to English language if no language is populated ' +
  ...
  describe('when user is logged in', () => {
    ...
    it(
      'should fetch contributor admin stats only once when' +
      ...
      fakeAsync(() => {
        fetchContributorAdminStatsSpy = spyOn(
          ...
          Promise.resolve({stats: [], nextOffset: 0, more: false})
        );
        component.ngOnInit();
        fixture.detectChanges();
        tick();
    ...
    it(
      'should throw error if fetchAssignedLanguageIds returns invalid ' +
        'language',
      fakeAsync(() => {
        fetchAssignedLanguageIdsSpy.and.returnValue(
          Promise.resolve(['invalid_language'])
        );
    ...
    it('should select language from dropdown', fakeAsync(() => {
      ...
      expect(component.selectedLanguage.language).toBe(
    ...
    it(
      ...
      fakeAsync(() => {
        ...
        expect(component.filter.lastDate).toEqual(
    ...
    it(
      ...
      fakeAsync(() => {
        ...
        tick();
        fixture.detectChanges();
        const initialLastDate = component.filter.lastDate;

        component.changeLastDate(
          component.getDateThatIsDaysBeforeToday(numberOfDaysSinceEndDate)
        );

        expect(component.filter.lastDate).toEqual(initialLastDate);
      })
    );
    ...
    it(
      'should apply the topic filter to the topic dropdown' +
    ...
    it(
      'should throw error when topic filter has chosen a topic id ' +
      ...
      fakeAsync(() => {
        ...
        expect(() => {
          component.applyTopicFilter();
          tick();
        }).toThrowError("Selected Topic Id doesn't match any valid topic.");
      })
    );
    ...
    it(
      'should set the translation coordinator variable when the user' +
        ' is a translation coordinator',
      fakeAsync(() => {
    ...
    it(
      'should set the question coordinator variable when the user' +
        ' is a question coordinator',
      fakeAsync(() => {
        getUserInfoSpy.and.returnValue(
    ...
    it('should update translation and question coordinator view', fakeAsync(() => {
      component.ngOnInit();
      tick();
      fixture.detectChanges();
    ...
    it(
      'should open question role editor modal and return changed value' +
        ' question submitter',
      fakeAsync(() => {
        const addRightsSpy = spyOn(
          contributorDashboardAdminBackendApiService,
