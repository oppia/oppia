...
export class VoiceoverBackendApiService {
                    ...
                    response => {
                        resolve({
                            ...
                            languageCodesMapping:
                                response.language_codes_mapping,
                                ...
                                response.sentence_tokens_with_durations.map(
                                    tokenWithDuration => {
                                        return {
                                            ...
                                            audioOffsetMsecs:
                                                tokenWithDuration.audio_offset_msecs,
                                        };
                                    }
                                ),
                        });
    ...
    async fetchEntityVoiceoversByLanguageCodeAsync(
        ...
        return new Promise((resolve, reject) => {
            this.http
                .get<EntityVoiceoversBulkBackendDict>(
                    entityVoiceoversBulkHandlerUrl
                )
                .toPromise()
                .then(
                    response => {
                ...
                .then(
                    response => {
                        ...
                        if (response?.exploration_data) {
                            explorationData = {
                                explorationTitle:
                                    response.exploration_data.exploration_title,
                                autogeneratableLanguageAccentCodes:
