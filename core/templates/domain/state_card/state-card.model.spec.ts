...
describe('State card object factory', () => {
  ...
  it('should be able to get the various fields', () => {
    ...
    expect(_sampleCard1.getLastInputResponsePair()).toEqual(null);
    expect(_sampleCard1.getLastOppiaResponse()).toEqual(null);
    ...
    expect(_sampleCard1.isTerminal()).toEqual(false);
    expect(_sampleCard1.isInteractionInline()).toEqual(true);
    expect(_sampleCard2.isInteractionInline()).toEqual(true);
    expect(_sampleCard1.getInteractionInstructions()).toEqual(null);
    expect(_sampleCard2.getInteractionInstructions()).toEqual(null);
  ...
  it('should return the last answer when calling \'getLastAnswer\'' +
      'or null if there is no last answer',
  () => {
    expect(_sampleCard1.getLastAnswer()).toEqual(null);
  ...
  it('should find whether content audio translation is available ' +
      'when calling \'isContentAudioTranslationAvailable\'',
  () => {
    spyOn(
      audioTranslationLanguageService, 'isAutogeneratedAudioAllowed'
    ).and.returnValue(false);

    expect(_sampleCard1.isContentAudioTranslationAvailable()).toEqual(true);
    expect(_sampleCard2.isContentAudioTranslationAvailable()).toEqual(false);
  });

  it('should get all the hints from interaction when calling \'getHints\'',
    () => {
      let expectedResult = [
  ...
  it('should get interaction solution when calling \'getSolution\'', () => {
  ...
  it('should find whether interaction supports hints ' +
      'when calling \'doesInteractionSupportHints\'',
  () => {
    expect(_sampleCard1.doesInteractionSupportHints()).toEqual(true);
    expect(_sampleCard2.doesInteractionSupportHints()).toEqual(false);
  });

  it('should mark and unmark interaction as completed', () => {
    expect(_sampleCard1.isCompleted()).toEqual(false);

    _sampleCard1.markAsCompleted();
    expect(_sampleCard1.isCompleted()).toEqual(true);

    _sampleCard1.markAsNotCompleted();
    expect(_sampleCard1.isCompleted()).toEqual(false);
  });

  it('should get written translations when calling \'writtenTranslations\'',
    () => {
  ...
  it('should restore every property immutably ' +
      'when calling \'restoreImmutable\'',
  () => {
    expect(_sampleCard1.getStateName()).toEqual('State 1');
    expect(_sampleCard1.getInteractionHtml()).toEqual(
      '<interaction></interaction>');
    expect(_sampleCard1.getInteractionId()).not.toEqual(null);
