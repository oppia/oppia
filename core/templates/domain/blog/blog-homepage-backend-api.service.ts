...
export class BlogHomePageBackendApiService {
  ...
  async fetchBlogHomePageDataAsync(offset: string): Promise<BlogHomePageData> {
    return new Promise((resolve, reject) => {
      this.http
        .get<BlogHomePageBackendResponse>(
          BlogHomePageConstants.BLOG_HOMEPAGE_DATA_URL_TEMPLATE +
            '?offset=' +
            offset
        )
        .toPromise()
        .then(
          response => {
            resolve({
              ...
              blogPostSummaryDicts: response.blog_post_summary_dicts.map(
                blogPostSummary => {
  ...
  async fetchBlogAuthorProfilePageDataAsync(
    ...
    return new Promise((resolve, reject) => {
      const authorProfilePageUrl = this.urlInterpolationService.interpolateUrl(
        BlogAuthorProfilePageConstants.BLOG_AUTHOR_PROFILE_PAGE_DATA_URL_TEMPLATE,
        {
      ...
      this.http
        .get<BlogAuthorProfilePageBackendResponse>(
        ...
        .then(
          response => {
            resolve({
              ...
              blogPostSummaries: response.summary_dicts.map(blogPostSummary => {
                return BlogPostSummary.createFromBackendDict(blogPostSummary);
              }),
              displayedAuthorName:
  ...
  async fetchBlogPostPageDataAsync(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.http
        .get<BlogPostPageBackendResponse>(blogPostDataUrl)
        .toPromise()
        .then(
          response => {
            resolve({
              authorUsername: response.author_username,
              summaryDicts: response.summary_dicts.map(blogPostSummary => {
                return BlogPostSummary.createFromBackendDict(blogPostSummary);
              }),
