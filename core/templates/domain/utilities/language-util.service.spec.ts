...
describe('Language util service', function() {
  ...
  it('should throw error while autogenerating audio language with invalid code',
    () => {
      expect(function() {
        lus.getAutogeneratedAudioLanguages('hii');
      }).toThrowError();
    });

  it('should correctly get content language desription', () => {
    expect(Object.values(lus.getContentLanguageDescription('en'))).toEqual(
      Object.values(mockSupportedContentLanguages[0].description));
  });

  it('should correctly get all voiceover language codes', () => {
    expect(Object.values(lus.getAllVoiceoverLanguageCodes())).toEqual(
      Object.values(mockSupportedAudioLanguages.map(audioLanguage => {
