...
describe('URL Interpolation Service', () => {
  ...
  it('should build complete URL with prefixes and hash', () => {
    ...
    expect(
      ...
      '/build/test_folder/path_test/hash_test.' +
        hashes['/path_test/hash_test.html'] +
        '.html'
    );
  ...
  it('should throw an error for erroneous URLs', () => {
    expect(uis.interpolateUrl.bind(uis, '', {})).toThrowError(
  ...
  it('should interpolate URLs when parameters have parentheses', () => {
    expect(
      uis.interpolateUrl('/test_url/<param>', {
        param: 'value (1',
      })
  ...
  it('should interpolate URLs requiring one or more parameters', () => {
    expect(
      uis.interpolateUrl('/test_url/<fparam>', {
        fparam: 'value',
      })
    ).toBe('/test_url/value');
    expect(
  ...
  it(
    'should interpolate parameters within words or adjacent to other ' +
      'parameters',
    () => {
      // It also doesn't need to have '/' prefixing the URL.
      expect(
        uis.interpolateUrl('word<with_param>', {
          with_param: '_with_value',
        })
      ).toBe('word_with_value');
      expect(
        uis.interpolateUrl('Eating_<param1><param2>_with_syrup', {
          param1: 'pan',
          param2: 'cakes',
        })
  ...
  it(
    'should interpolate parameters beginning, ending, or composing ' +
      'the URL',
    () => {
      expect(
        uis.interpolateUrl('<prefix>_with_words', {
          prefix: 'Signs',
        })
      ).toBe('Signs_with_words');
      expect(
        uis.interpolateUrl('Neither_here_nor_<suffix>', {
          suffix: 'anywhere',
        })
      ).toBe('Neither_here_nor_anywhere');
      expect(
        uis.interpolateUrl('<param>', {
          param: 'value',
        })
      ).toBe('value');
    }
  ...
  it('should sanitize parameters but not URLs', () => {
    expect(uis.interpolateUrl('URL with a space', {})).toBe('URL with a space');
    expect(
      uis.interpolateUrl('/test_url/<first_param>?<query_name>=<query_value>', {
        first_param: 'SEARCH',
  ...
  it('should escape the "=" symbol correctly', () => {
    expect(
      uis.interpolateUrl('/test_url/<first_param>', {
  ...
  it('should not interpolate bad parameter names and values', () => {
    ...
    expect(
      uis.interpolateUrl.bind(uis, '/test_url/<>', {
        '': 'value',
      })
    ...
    // parameter name.
    expect(
      uis.interpolateUrl('/test_url/<b@d!#$%^&*() p@r@m n@m3`~[]{}>', {})
    ).toBe('/test_url/<b@d!#$%^&*() p@r@m n@m3`~[]{}>');

    // Parameter names cannot have spaces.
    expect(
      uis.interpolateUrl('/test_url/<parameter with spaces>', {
        'parameter with spaces': 'value',
      })
    ).toBe('/test_url/<parameter with spaces>');

    expect(
      uis.interpolateUrl.bind(uis, '/test_url/<<name>>', {
        name: 'value',
      })
    ).toThrowError("Invalid URL template received: '/test_url/<<name>>'");
  ...
  it('should throw an error for missing parameters', () => {
    ...
    expect(
      uis.interpolateUrl.bind(uis, '/test_url/<page1>', {
        page2: 'v',
      })
    ).toThrowError("Expected variable 'page1' when interpolating URL.");
  });
  ...
  it('should interpolate correct path', () => {
    ...
    expect(uis.getStaticImageUrl('/hash_test.png')).toBe(
      '/build/assets/images/hash_test.' +
        hashes['/images/hash_test.png'] +
        '.png'
    );
    ...
    expect(uis.getStaticAssetUrl('/test.json')).toBe('/build/assets/test.json');
  ...
  it('should throw an error for empty path', () => {
    ...
    expect(uis.getStaticAssetUrl.bind(uis, '')).toThrowError(
      'Empty path passed in method.'
    );

    expect(uis.getExtensionResourceUrl.bind(uis, '')).toThrowError(
      'Empty path passed in method.'
    );
  });
  ...
  it('should throw an error for path not beginning with forward slash', () => {
    ...
    expect(uis.getStaticAssetUrl.bind(uis, 'test_url/fail.html')).toThrowError(
      "Path must start with '/': '" + 'test_url/fail.html' + "'."
    );

    expect(
      uis.getExtensionResourceUrl.bind(uis, 'test_fail.html')
    ).toThrowError("Path must start with '/': '" + 'test_fail.html' + "'.");
    expect(
      uis.getExtensionResourceUrl.bind(uis, 'test_url/fail.html')
    ).toThrowError("Path must start with '/': '" + 'test_url/fail.html' + "'.");
  });
});
