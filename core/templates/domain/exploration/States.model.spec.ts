...
describe('States Object Factory', () => {
  ...
  it('should create a new state given a state name and set ' +
    ...
    expect(newStates.getState('new state')).toEqual(
      State.createFromBackendDict('new state', newState2));
    ...
    newStates.setState(
      'new state', State.createFromBackendDict('new state', newState));
    expect(newStates.getState('new state')).toEqual(
  ...
  it('should correctly retrieve the terminal states', () => {
    ...
    newStates.setState(
      'first state', State.createFromBackendDict('first state', newState));
    expect(newStates.getFinalStateNames()).toEqual['new state'];
  });

  it('should correctly delete a state', () => {
    let statesWithAudio = States.createFromBackendDict(statesWithAudioDict);
    statesWithAudio.deleteState('first state');
    expect(statesWithAudio).toEqual(States.createFromBackendDict({
      'second state': secondState
    }));
  });
  ...
  it('should correctly set any states\' interaction.defaultOutcomes that ' +
    ...
    expect(states).toEqual(States.createFromBackendDict({
      'third state': {
        content: {
          content_id: 'content',
          html: 'content'
  ...
  it('should correctly get all audio language codes in states', () => {
    const statesWithAudio = States.createFromBackendDict(statesWithAudioDict);
