...
interface ExplorationCache {
    [explorationId: string]: ReadOnlyExploration;
}
interface ReadOnlyExploration {
...
export class ReadOnlyExplorationBackendApiService {
  ...
  private _fetchExploration(
      explorationId: string,
      version: number,
      successCallback: (value: ReadOnlyExploration) => void,
      errorCallback: (reason?: string) => void)
    : Promise<ReadOnlyExploration> {
    return new Promise((resolve, reject) => {
      const explorationDataUrl = this._getExplorationUrl(
        explorationId, version);

      this.http.get<ReadOnlyExploration>(explorationDataUrl).toPromise().then(
        response => {
  ...
  fetchExploration(
      explorationId: string,
      version: number): Promise<ReadOnlyExploration> {
    return new Promise((resolve, reject) => {
  ...
  cacheExploration(
      explorationId: string,
      exploration: ReadOnlyExploration): void {
    this._explorationCache[explorationId] = exploration;
  }
