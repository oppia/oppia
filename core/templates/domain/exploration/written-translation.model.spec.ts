...
describe('WrittenTranslation object factory', () => {
  let writtenTranslationDict: WrittenTranslation;
  let writtenTranslation: WrittenTranslation;
  beforeEach(() => {
    writtenTranslation = writtenTranslationDict.createFromBackendDict({
      data_format: 'html',
  ...
  it('should set and get translation value correctly', () => {
    expect(writtenTranslation).toEqual(writtenTranslationDict.createFromBackendDict({
      data_format: 'html',
      translation: '<p>HTML</p>',
      needs_update: false
    }));
    ...
    const unicodeWrittenTranslation = writtenTranslationDict.createFromBackendDict({
      data_format: 'unicode',
      translation: 'unicode',
      needs_update: false
    });
    ...
    const setOfStringsWrittenTranslation = writtenTranslationDict.createFromBackendDict({
  ...
  it('should throw error for an invalid data format on creation', () => {
    expect(() => writtenTranslationDict.createNew('invalid')).toThrowError(
  ...
  it('should create a new written translation translation', () => {
    expect(writtenTranslationDict.createNew(TRANSLATION_DATA_FORMAT_HTML)).toEqual(
      writtenTranslationDict.createFromBackendDict({
        data_format: 'html',
        translation: '',
        needs_update: false
      })
    ...
    expect(writtenTranslationDict.createNew(TRANSLATION_DATA_FORMAT_UNICODE)).toEqual(
      writtenTranslationDict.createFromBackendDict({
        data_format: 'unicode',
        translation: '',
        needs_update: false
    ...
    expect(
      writtenTranslationDict.createNew(TRANSLATION_DATA_FORMAT_SET_OF_UNICODE_STRING)
    ).toEqual(
      writtenTranslationDict.createFromBackendDict({
    ...
    expect(
      writtenTranslationDict.createNew(TRANSLATION_DATA_FORMAT_SET_OF_NORMALIZED_STRING)
    ).toEqual(
      writtenTranslationDict.createFromBackendDict({
        data_format: 'set_of_normalized_string',
        translation: [],
