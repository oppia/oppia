...
describe('Skill backend API service', () => {
  ...
  it('should make a request to update the skill in the backend.', fakeAsync(() => {
    ...
    skillBackendApiService
      .updateSkillAsync('1', 1, 'commit message', [changeList])
      .then(response => {
  ...
  it('should make a request to check if skill description exists in backend.', fakeAsync(() => {
    const backendResponse = {
  ...
  it('should make a request to check if skill assigned for diagnostic test.', fakeAsync(() => {
    const backendResponse = {
      topic_names: [],
    };
    const successHandler = jasmine.createSpy('success');
    ...
    skillBackendApiService
      .getTopicNamesWithGivenSkillAssignedForDiagnosticTest(skillId)
      .then(successHandler, failHandler);
  ...
  it('should succesfully fetch multiple existing skills from the backend.', fakeAsync(() => {
    ...
    skillBackendApiService.fetchMultiSkillsAsync(['1', '2']).then(response => {
  ...
  it('should use the rejection handler if delete a existing skill fails.', fakeAsync(() => {
    const successHandler = jasmine.createSpy('success');
    const failHandler = jasmine.createSpy('fail');
