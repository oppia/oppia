...
describe('Skill rights backend API service', () => {
  ...
  beforeEach(() => {
    ...
    skillRightsObject = SkillRights.createFromBackendDict(sampleSkillRights);
  });
  afterEach(() => {
    httpTestingController.verify();
  });

  it('should successfully fetch a skill dict from backend', fakeAsync(() => {
    let successHandler = jasmine.createSpy('success');
    let failHandler = jasmine.createSpy('fail');

    skillRightsBackendApiService
      .fetchSkillRightsAsync('0')
      .then(successHandler, failHandler);
    let req = httpTestingController.expectOne('/skill_editor_handler/rights/0');
  ...
  it(
    'should use reject handler when fetching a skill dict from backend' +
      ' fails',
    fakeAsync(() => {
      let successHandler = jasmine.createSpy('success');
      let failHandler = jasmine.createSpy('fail');
  ...
  it(
    "should successfully fetch a skill dict from backend if it's not" +
      'cached',
    fakeAsync(() => {
      let successHandler = jasmine.createSpy('success');
      let failHandler = jasmine.createSpy('fail');
      skillRightsBackendApiService
        .loadSkillRightsAsync('0')
        .then(successHandler, failHandler);
      let req = httpTestingController.expectOne(
        '/skill_editor_handler/rights/0'
      );
