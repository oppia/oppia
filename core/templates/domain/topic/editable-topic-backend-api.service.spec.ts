...
describe('Editable topic backend API service', () => {
  ...
  it('should successfully fetch an existing subtopic page from the backend', fakeAsync(() => {
    let successHandler = jasmine.createSpy('success');
    let failHandler = jasmine.createSpy('fail');

    editableTopicBackendApiService
      .fetchSubtopicPageAsync('topicId', 1)
      .then(successHandler, failHandler);
  ...
  it('should update a topic after fetching it from the backend', fakeAsync(() => {
    ...
    editableTopicBackendApiService.fetchTopicAsync('0').then(function (data) {
    ...
    editableTopicBackendApiService
      .updateTopicAsync(topic.id, topic.version, 'Name is updated', [])
      .then(successHandler, failHandler);
  ...
  it("should use the rejection handler if the topic to update doesn't exist", fakeAsync(() => {
    let successHandler = jasmine.createSpy('success');
    let failHandler = jasmine.createSpy('fail');

    // Loading a topic the first time should fetch it from the backend.
    editableTopicBackendApiService
      .updateTopicAsync('1', 1, 'Update an invalid topic.', [])
      .then(successHandler, failHandler);
    let req = httpTestingController.expectOne('/topic_editor_handler/data/1');
    expect(req.request.method).toEqual('PUT');
  ...
  it('should check if a topic name already exists', fakeAsync(() => {
    editableTopicBackendApiService
      .doesTopicWithNameExistAsync('topic-name')
      .then(topicNameExists => {
        expect(topicNameExists).toBeTrue();
      });
    let req = httpTestingController.expectOne('/topic_name_handler/topic-name');
    expect(req.request.method).toEqual('GET');
    req.flush({
  ...
  it('should use the rejection handler if the topic name already exists', fakeAsync(() => {
    editableTopicBackendApiService
      .doesTopicWithNameExistAsync('topic-name')
      .then(
        () => {},
  ...
  it('should check if a topic url fragment already exists', fakeAsync(() => {
    editableTopicBackendApiService
      .doesTopicWithUrlFragmentExistAsync('topic-url-fragment')
      .then(topicUrlFragmentExists => {
  ...
  it(
    'should use the rejection handler when fetching stories from a' +
      ' topic fails',
    fakeAsync(() => {
      let successHandler = jasmine.createSpy('success');
      let failHandler = jasmine.createSpy('fail');
  ...
  it('should use the rejection handler when deleting a topic fails', fakeAsync(() => {
  ...
  it('should use the rejection handler for getting topic id to topic name', fakeAsync(() => {
    var successHandler = jasmine.createSpy('success');
    var failHandler = jasmine.createSpy('fail');
