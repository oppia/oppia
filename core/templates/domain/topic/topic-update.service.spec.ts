...
describe('Topic update service', function () {
  ...
  it("should set/unset changes to a topic's thumbnail filename", () => {
    expect(_sampleTopic.getThumbnailFilename()).toEqual(null);
  ...
  it("should set/unset changes to a topic's thumbnail bg color", () => {
    expect(_sampleTopic.getThumbnailBgColor()).toEqual(null);
  ...
  it('should rearrange a skill in a subtopic', () => {
    ...
    const subtopic = _sampleTopic.getSubtopicById(1);
    if (!subtopic) {
      throw new Error('Expected subtopic to exist');
    }
    let skills = subtopic.getSkillSummaries();
  ...
  it('should add and delete a study guide section', () => {
    ...
    sections = _sampleStudyGuide.getSections();
    expect(sections.length).toEqual(2);
    expect(sections[1].toBackendDict()).toEqual({
      heading: {
        content_id: 'section_heading_0',
        unicode_str: 'new heading',
      },
