...
export class UnitsObjectFactory {
  ...
  getDuplicatedUnit(units: string): string {
    ...
    for (let i = 0; i < normalizedUnits.length; i++) {
        ...
        if (seenUnits.has(normalizedUnit)) {
            // Return the first original unit that caused a duplicate
  ...
  getCorrectedFormat(units: { units: { unit: string; exponent: number }[] }): string {
    let unitDetails = units.units;
    let unitNormalizationMap: { [unit: string]: string } = {};
    unitDetails.forEach(({ unit }) => {
        unitNormalizationMap[unit] = ObjectsDomainConstants.UNIT_TO_NORMALIZED_UNIT_MAPPING[unit] || unit;
    });
    ...
    for (let unit in aggregatedExponents) {
        ...
        if (exponent > 0) {
            ...
            denominatorUnits.push(exponent === -1 ? `${unit}` : `${unit}^${-exponent}`);
        }
    }
