...
} from 'domain/exploration/exploration-search-result.model';
...
export class SearchExplorationsBackendApiService {
  ...
  private _fetchExplorations(
    ...
    this.http
      .get<SearchExplorationBackendResponse>(queryUrl)
      .toPromise()
      .then(
        response => {
          ...
          var explorationSearchResultList =
            explorationSearchResultBackendList.map(explorationSearchResult =>
  ...
  async fetchExplorationsAsync(
    searchQuery: string
  ): Promise<ExplorationSearchResult[]> {
