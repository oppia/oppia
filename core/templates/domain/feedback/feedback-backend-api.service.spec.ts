...
describe('Feedback backend api service', () => {
  ...
  it('should fetch platform feedback reports', fakeAsync(() => {
    const onSuccess = jasmine.createSpy('onSuccess');
    ...
    expect(req.request.params.get('status')).toEqual('open');
    expect(req.request.params.get('date_from_msecs')).toEqual('1000');
    expect(req.request.params.get('date_to_msecs')).toEqual('2000');
    req.flush({
      summaries: [
        {
          id: 'report_id',
          report_message_preview: 'The donate page is broken.',
          status: 'open',
          source: 'app',
          category: null,
        },
