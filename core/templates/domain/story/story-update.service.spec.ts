...
describe('Story update service', () => {
  ...
  beforeEach(() => {
    ...
    spyOn(
      storyUpdateService.onStoryChapterUpdateEventEmitter,
      'emit'
    ).and.stub();
  ...
  it('should add/remove a prerequisite skill id to/from a node in the story', () => {
    ...
    expect(
      storyUpdateService.onStoryChapterUpdateEventEmitter.emit
    ).toHaveBeenCalled();
    expect(storyNodeLastModifiedSpy).toHaveBeenCalled();
  ...
  it(
    'should create a proper backend change dict for adding a destination ' +
      'node id to a node',
    () => {
      storyUpdateService.addDestinationNodeIdToNode(
        _sampleStory,
  ...
  it(
    'should not remove a destination node id from a node in the story ' +
  ...
  it('should add/remove a story node', () => {
    ...
    expect(_sampleStory.getStoryContents().getNodes()[2].getId()).toEqual(
      'node_3'
    );
    expect(_sampleStory.getStoryContents().getNodes()[2].getTitle()).toEqual(
      'Title 2'
    );
  ...
  it(
    'should throw an error if we try to finalize a story ' +
  ...
  it(
    'should throw an error if we try to unfinalize a story ' +
  ...
  it(
    'should create a proper backend change dict for unfinalizing a node ' +
      'outline',
    () => {
      let storyNodeLastModifiedSpy = spyOn(
        storyUpdateService,
  ...
  it('should set a story node outline', () => {
    ...
    expect(_sampleStory.getStoryContents().getNodes()[0].getOutline()).toBe(
      'Outline'
    );
  ...
  it('should set a story node title', () => {
    ...
    expect(_sampleStory.getStoryContents().getNodes()[0].getTitle()).toBe(
      'Title 1'
    );
  ...
  it('should set a story node status', () => {
    ...
    expect(_sampleStory.getStoryContents().getNodes()[0].getStatus()).toBe(
      'Published'
    );
    storyUpdateService.setStoryNodeStatus(_sampleStory, 'node_1', 'Draft');
    expect(_sampleStory.getStoryContents().getNodes()[0].getStatus()).toBe(
      'Draft'
    );
    expect(storyNodeLastModifiedSpy).toHaveBeenCalled();
  ...
  it('should set a story node planned publication date', () => {
    ...
    expect(
      _sampleStory
        .getStoryContents()
        .getNodes()[0]
        .getPlannedPublicationDateMsecs()
    ).toBe(30);
  ...
  it('should set a story node description', () => {
    ...
    expect(_sampleStory.getStoryContents().getNodes()[0].getDescription()).toBe(
      'Description 1'
    );
  ...
  it(
    'should create a proper backend change dict for setting the exploration ' +
      'id of a node',
    () => {
      let storyNodeLastModifiedSpy = spyOn(
        storyUpdateService,
  ...
  it(
    'should rearrange node in story when calling ' + "'rearrangeNodeInStory'",
    () => {
      storyUpdateService.rearrangeNodeInStory(_sampleStory, 0, 1);
      expect(_sampleStory.getStoryContents().getNodes()[0].getId()).toBe(
        'node_2'
      );
  ...
  it(
    ...
    () => {
      ...
      expect(
        _sampleStory.getStoryContents().getNodes()[0].getThumbnailBgColor()
      ).toBe('red');
      expect(storyNodeLastModifiedSpy).toHaveBeenCalled();
