...
export class StoryUpdateService {
  ...
  private _updateStoryEditorBrowserTabsUnsavedChangesStatus(story: Story) {
    var storyEditorBrowserTabsInfo: EntityEditorBrowserTabsInfo | null =
  ...
  setInitialNodeId(story: Story, newInitialNodeId: string): void {
    let oldInitialNodeId = cloneDeep(
      story.getStoryContents().getInitialNodeId()
    );
    this._applyStoryContentsPropertyChange(
      story,
      StoryDomainConstants.INITIAL_NODE_ID,
      oldInitialNodeId,
      newInitialNodeId,
      (changeDict, story) => {
        // ---- Apply ----
  ...
  addStoryNode(story: Story, nodeTitle: string): void {
    ...
    this._applyChange(
      story,
      StoryDomainConstants.CMD_ADD_STORY_NODE,
      {
        node_id: nextNodeId,
        title: nodeTitle,
      },
  ...
  deleteStoryNode(story: Story, nodeId: string): void {
    this._applyChange(
      story,
      StoryDomainConstants.CMD_DELETE_STORY_NODE,
      {
        node_id: nodeId,
      },
      (changeDict, story) => {
  ...
  setStoryNodeTitle(story: Story, nodeId: string, newTitle: string): void {
    ...
    this._applyStoryNodePropertyChange(
      story,
      StoryDomainConstants.STORY_NODE_PROPERTY_TITLE,
      nodeId,
      oldTitle,
      newTitle,
      (changeDict, story) => {
  ...
  setStoryNodeStatus(story: Story, nodeId: string, newStatus: string): void {
    ...
    this._applyStoryNodePropertyChange(
      story,
      StoryDomainConstants.STORY_NODE_PROPERTY_STATUS,
      nodeId,
      oldStatus,
      newStatus,
      (changeDict, story) => {
  ...
  setStoryNodePlannedPublicationDateMsecs(
    ...
    this._applyStoryNodePropertyChange(
      ...
      nodeId,
      oldPlannedPublicationDateMsecs,
      newPlannedPublicationDateMsecs,
      (changeDict, story) => {
        // ---- Apply ----
        story
          .getStoryContents()
  ...
  setStoryNodeLastModifiedMsecs(
    story: Story,
    nodeId: string,
    newLastModifiedMsecs: number
  ): void {
    let storyNode = this._getStoryNode(story.getStoryContents(), nodeId);
    let oldLastModifiedMsecs = storyNode.getLastModifiedMsecs();

    if (
      this._platformFeatureService.status.SerialChapterLaunchCurriculumAdminView
        .isEnabled
    ) {
      this._applyStoryNodePropertyChange(
        story,
        StoryDomainConstants.STORY_NODE_PROPERTY_LAST_MODIFIED_MSECS,
        nodeId,
        oldLastModifiedMsecs,
        newLastModifiedMsecs,
        (changeDict, story) => {
          // ---- Apply ----
          story
            .getStoryContents()
            .setNodeLastModifiedMsecs(nodeId, newLastModifiedMsecs);
        },
        (changeDict, story) => {
          // ---- Undo ----
          story
            .getStoryContents()
            .setNodeLastModifiedMsecs(nodeId, oldLastModifiedMsecs);
        }
      );
    }
  }
  ...
  setStoryNodeFirstPublicationDateMsecs(
    ...
    let storyNode = this._getStoryNode(story.getStoryContents(), nodeId);
    let oldFirstPublicationDateMsecs = storyNode.getFirstPublicationDateMsecs();
    this.setStoryNodeLastModifiedMsecs(story, nodeId, Date.now());
    ...
    this._applyStoryNodePropertyChange(
      ...
      nodeId,
      oldFirstPublicationDateMsecs,
      newFirstPublicationDateMsecs,
      (changeDict, story) => {
        // ---- Apply ----
        story
          .getStoryContents()
  ...
  setStoryNodeUnpublishingReason(
    story: Story,
    nodeId: string,
    newUnpublishingReason: string | null
  ): void {
    let storyNode = this._getStoryNode(story.getStoryContents(), nodeId);
    ...
    this._applyStoryNodePropertyChange(
      story,
      StoryDomainConstants.STORY_NODE_PROPERTY_UNPUBLISHING_REASON,
      nodeId,
      oldUnpublishingReason,
      newUnpublishingReason,
      (changeDict, story) => {
        // ---- Apply ----
        story
          .getStoryContents()
          .setNodeUnpublishingReason(nodeId, newUnpublishingReason);
      },
      (changeDict, story) => {
        // ---- Undo ----
        story
          .getStoryContents()
          .setNodeUnpublishingReason(nodeId, oldUnpublishingReason);
      }
    );
  }
  ...
  addDestinationNodeIdToNode(
    ...
    let storyNode = this._getStoryNode(story.getStoryContents(), nodeId);
    let oldDestinationNodeIds = cloneDeep(storyNode.getDestinationNodeIds());
    ...
    this._applyStoryNodePropertyChange(
      ...
      nodeId,
      oldDestinationNodeIds,
      newDestinationNodeIds,
      (changeDict, story) => {
        // ---- Apply ----
        story
          .getStoryContents()
          .addDestinationNodeIdToNode(nodeId, destinationNodeId);
      },
      (changeDict, story) => {
        // ---- Undo ----
        story
          .getStoryContents()
          .removeDestinationNodeIdFromNode(nodeId, destinationNodeId);
      }
    );
  }
  ...
  addPrerequisiteSkillIdToNode(
    ...
    this._applyStoryNodePropertyChange(
      ...
      (changeDict, story) => {
        ...
        story
          .getStoryContents()
          .removePrerequisiteSkillIdFromNode(nodeId, skillId);
        this._storyChapterUpdateEventEmitter.emit();
      }
    );
  }
  ...
  addAcquiredSkillIdToNode(
    ...
    let storyNode = this._getStoryNode(story.getStoryContents(), nodeId);
    let oldAcquiredSkillIds = cloneDeep(storyNode.getAcquiredSkillIds());
