...
export class VersionDiffVisualization implements OnInit {
  ...
  getEarlierVersionHeader: any;
  // The header for the pane of the state comparison modal corresponding
  // to the later version of the exploration.
  getLaterVersionHeader: any;

  COLOR_ADDED: string;
  COLOR_DELETED: string;
  COLOR_CHANGED: string;
  COLOR_UNCHANGED: string;
  COLOR_RENAMED_UNCHANGED: string;

  // Constants for names in legend.
  NODE_TYPE_ADDED: string;
  NODE_TYPE_DELETED: string;
  NODE_TYPE_CHANGED: string;
  NODE_TYPE_CHANGED_RENAMED: string;
  NODE_TYPE_RENAMED: string;
  NODE_TYPE_UNCHANGED: string;

  STATE_PROPERTY_ADDED: string;
  STATE_PROPERTY_DELETED: string;
  STATE_PROPERTY_CHANGED: string;
  STATE_PROPERTY_UNCHANGED: string;
  ...
  diffGraphData: any;
  v1InitStateId: any;
  LEGEND_GRAPH_COLORS: any;
  diffGraphSecondaryLabels: any;
  diffGraphNodeColors: any;
  LEGEND_GRAPH_SECONDARY_LABELS: any;
  LEGEND_GRAPH_LINK_PROPERTY_MAPPING: any;
  DIFF_GRAPH_LINK_PROPERTY_MAPPING: any;
  ...
  onClickStateInDiffGraph(stateId: any): void {
  ...
  //   deleted.
  showStateDiffModal(
      newStateName: any, oldStateName: any, stateProperty: any): void {
    ...
    if (stateProperty !== this.STATE_PROPERTY_DELETED &&
        this.getDiffData().v2States.hasOwnProperty(newStateName)) {
      newState = this.getDiffData().v2States[newStateName];
    }

    let oldState = null;
    let stateNameToRetrieve = oldStateName || newStateName;
    if (stateProperty !== this.STATE_PROPERTY_ADDED &&
        this.getDiffData().v1States.hasOwnProperty(stateNameToRetrieve)) {
      oldState = this.getDiffData().v1States[stateNameToRetrieve];
    }
    ...
    modalRef.componentInstance.headers = {
      leftPane: this.getEarlierVersionHeader(),
      rightPane: this.getLaterVersionHeader()
    };
  ...
  ngOnInit(): void {
    ...
    this.COLOR_ADDED = '#4EA24E';
    this.COLOR_DELETED = '#DC143C';
    this.COLOR_CHANGED = '#1E90FF';
    this.COLOR_UNCHANGED = 'beige';
    this.COLOR_RENAMED_UNCHANGED = '#FFD700';

    // Constants for names in legend.
    this.NODE_TYPE_ADDED = 'Added';
    this.NODE_TYPE_DELETED = 'Deleted';
    this.NODE_TYPE_CHANGED = 'Changed';
    this.NODE_TYPE_CHANGED_RENAMED = 'Changed/renamed';
    this.NODE_TYPE_RENAMED = 'Renamed';
    this.NODE_TYPE_UNCHANGED = 'Unchanged';

    this.STATE_PROPERTY_ADDED = 'added';
    this.STATE_PROPERTY_DELETED = 'deleted';
    this.STATE_PROPERTY_CHANGED = 'changed';
    this.STATE_PROPERTY_UNCHANGED = 'unchanged';

    this.nodesData = this.getDiffData().nodes;
    ...
    this.v1InitStateId = this.getDiffData().v1InitStateId;

    this.diffGraphData = {
      nodes: this.diffGraphNodes,
      links: this.getDiffData().links,
      initStateId: this.getDiffData().v2InitStateId,
      finalStateIds: this.getDiffData().finalStateIds
    };
