  ...
  for (const validatorSpec of validators) {
    ...
    const filterArgs = Object.fromEntries(
      Object.entries(validatorSpec)
        .filter(([key, _]) => key !== 'id')
        .map(([key, value]) => {
          return [underscoresToCamelCasePipe.transform(key), cloneDeep(value)];
        })
    ) as FilterArgsType;
