...
export class StateResponsesComponent implements OnInit, OnDestroy {
    ...
    isSelfLoopThatIsMarkedCorrect(outcome: Outcome): boolean {
        if (!outcome) {
            ...
            return (
                outcome.dest === currentStateName && outcome.labelledAsCorrect
            );
        }
    }
    ...
    isCreatingNewState(outcome: Outcome): boolean {
        return (
            outcome && outcome.dest === AppConstants.PLACEHOLDER_OUTCOME_DEST
        );
    }
    ...
    isLinearWithNoFeedback(outcome: Outcome): boolean | void {
        ...
        if (
            ...
            return (
                this.isCurrentInteractionLinear() &&
            ...
            if (misconceptions) {
                misconceptions.forEach(misconception => {
                    if (misconception.isMandatory()) {
                        ...
                        if (
                            !taggedSkillMisconceptionIds.hasOwnProperty(
                                skillMisconceptionId
                            )
                        ) {
                ...
                newInteractionId => {
                    ...
                    this.responsesService.onInteractionIdChanged(
                        ...
                        (newAnswerGroups, newDefaultOutcome) => {
                            ...
                            this.refreshWarnings.emit();
                            this.answerGroups =
                                this.responsesService.getAnswerGroups();
                            this.defaultOutcome =
