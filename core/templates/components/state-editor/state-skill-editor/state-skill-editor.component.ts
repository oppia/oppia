...
export class StateSkillEditorComponent implements OnInit {
      ...
      () => {
        if (this.stateLinkedSkillIdService.displayed) {
          this.skillBackendApiService
            .fetchSkillAsync(this.stateLinkedSkillIdService.displayed)
            .then(skill => {
  ...
  addSkill(): void {
    let sortedSkillSummaries =
      this.storyEditorStateService.getSkillSummaries() as SkillSummaryBackendDict[];
  ...
  deleteSkill(): void {
    ...
    this.ngbModal
      .open(DeleteStateSkillModalComponent, {
        backdrop: true,
      })
      .result.then(
        () => {
