...
describe('PlaythroughIssuesBackendApiService', () => {
  ...
  describe('.fetch', () => {
    it('should return the issues data provided by the backend', fakeAsync(() => {
      ...
      expect(successHandler).toHaveBeenCalledWith(
        backendIssues.map(PlaythroughIssue.createFromBackendDict)
      );
    ...
    it('should not fetch an issue when another issue was already fetched', fakeAsync(() => {
      let successHandler = jasmine.createSpy('success');
      let failureHandler = jasmine.createSpy('failure');
    ...
    it('should return the playthrough data provided by the backend', fakeAsync(() => {
      ...
      playthroughIssuesBackendApiService
        .fetchPlaythroughAsync('7', '1')
        .then(successHandler, failureHandler);
      let req = httpTestingController.expectOne('/playthroughdatahandler/7/1');
      expect(req.request.method).toEqual('GET');
      req.flush(backendPlaythrough);
      flushMicrotasks();

      expect(successHandler).toHaveBeenCalledWith(
        PlaythroughIssue.createFromBackendDict(backendPlaythrough)
      );
      expect(failureHandler).not.toHaveBeenCalled();
    }));
  });
  ...
  it('should use the rejection handler if the backend request failed.', fakeAsync(() => {
    ...
    var req = httpTestingController.expectOne('/playthroughdatahandler/7/1');
    expect(req.request.method).toEqual('GET');
    req.flush(
      {
        error: 'Some error in the backend.',
  ...
  describe('.resolve', () => {
    it('should resolve an issue', fakeAsync(() => {
      ...
      let failureHandler = jasmine.createSpy('failure');
      let explorationId = '7';
      let playthroughIssue = PlaythroughIssue.createFromBackendDict(
        backendIssues[0]
      );
      ...
      req = httpTestingController.expectOne('/resolveissuehandler/7');
    ...
    it('should use the rejection handler when try to get non fetched issue', fakeAsync(() => {
      let successHandler = jasmine.createSpy('success');
      ...
      playthroughIssuesBackendApiService
        .resolveIssueAsync(playthroughIssue, explorationId, 1)
        .then(successHandler, failHandler);
      let req = httpTestingController.expectOne(
