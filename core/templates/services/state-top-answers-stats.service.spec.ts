...
describe('StateTopAnswersStatsService', () => {
  ...
  const spyOnBackendApiFetchStatsAsync = (
    stateName: string,
    answersStatsBackendDicts: AnswerStatsBackendDict[]
  ): jasmine.Spy => {
    const answersStats = answersStatsBackendDicts.map(a =>
      AnswerStats.createFromBackendDict(a)
    );
    return spyOn(
      stateTopAnswersStatsBackendApiService,
      'fetchStatsAsync'
  ...
  it('should have stats for state provided by backend', fakeAsync(async () => {
    const states = makeStates();
    spyOnBackendApiFetchStatsAsync('Hola', [
      {answer: 'hola', frequency: 3, is_addressed: false},
    ]);
    stateTopAnswersStatsService.initAsync(expId, states);
    flushMicrotasks();
    await stateTopAnswersStatsService.getInitPromiseAsync();
  ...
  it('should only returns state names with stats', fakeAsync(async () => {
    ...
    expect(stateTopAnswersStatsService.getStateNamesWithStats()).toEqual([
      'Hola',
    ]);
  }));
  ...
  it('should return empty stats for a newly added state', fakeAsync(async () => {
    ...
    expect(stateTopAnswersStatsService.getStateStats('Me Llamo')).toEqual([]);
  }));
  ...
  it('should recognize newly resolved answers', fakeAsync(async () => {
    ...
    expect(
      stateTopAnswersStatsService.getUnresolvedStateStats('Hola')
  ...
  it("should add new answer when Interaction Id's are not equal", fakeAsync(async () => {
    const states = makeStates();
    spyOnBackendApiFetchStatsAsync('Hola', [
      {answer: 'adios', frequency: 3, is_addressed: false},
    ]);
    ...
    // Pre-checks.
    expect(stateTopAnswersStatsService.getStateStats('Hola')).toEqual([
      new AnswerStats('adios', 'adios', 3, false),
    ]);
  ...
  it('should getTopAnswersByStateNameAsync', fakeAsync(() => {
    ...
    stateTopAnswersStatsService
      .getTopAnswersByStateNameAsync(expId, states)
      .then(data => {
