...
export class PlaythroughService {
  ...
  private expStopwatch: Stopwatch = null;
  private isLearnerInSamplePopulation: boolean = null;
  private multipleIncorrectStateName: MultipleIncorrectStateNames = {
    state_name: null,
    num_times_incorrect: null
  };
  private cycleIdentifier: CycleIdentifier = {
    cycle: null,
    num_cycles: null
  };
  private visitedStates: string[] = [];
  private misTracker: boolean = false;
  private cstTracker: boolean = false;
  ...
  private removeOldQuitAction(): void {
    ...
    this.playthrough.actions = this.playthrough.actions.filter(
      a => a.actionType !== AppConstants.ACTION_TYPE_EXPLORATION_QUIT);
    this.playthrough.actions.push(quitAction);
  }

  private determineIfLearnerIsInSamplePopulation(probability: number): boolean {
  ...
  private recordStateTransitionInCyclicIssueTracker(
    ...
    if (this.cycleIdentifier.num_cycles <
      ...
      if (this.visitedStates.indexOf(destStateName) !== -1) {
        // Cycle identified.
        let cycleStartIndex = this.visitedStates.indexOf(destStateName);
        this.visitedStates.push(destStateName);
        let cycleString = this.visitedStates.slice(
          cycleStartIndex, this.visitedStates.length).toString();
  ...
  private isMultipleIncorrectSubmissionsIssue(): boolean {
    return (
      this.multipleIncorrectStateName.num_times_incorrect >=
      ServicesConstants.NUM_INCORRECT_ANSWERS_THRESHOLD);
  }

  private isCyclicStateTransitionsIssue(): boolean {
    return (
      this.cycleIdentifier.num_cycles >=
      ServicesConstants.NUM_REPEATED_CYCLES_THRESHOLD);
  }

  private isEarlyQuitIssue(timeSpentInExpInSecs: number): boolean {
    return (
      timeSpentInExpInSecs < ServicesConstants.EARLY_QUIT_THRESHOLD_IN_SECS);
  }
  ...
  private analyzePlaythrough(): void {
    ...
    let timeSpentInExpInSecs = this.expStopwatch.getTimeInSecs();
  ...
  initSession(
    ...
    this.visitedStates = [];
    this.misTracker = false;
    this.cstTracker = false;
  }
...
angular.module('oppia').factory(
  'PlaythroughService', downgradeInjectable(PlaythroughService));
