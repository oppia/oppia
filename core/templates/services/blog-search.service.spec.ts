...
describe('Blog Post Search Service', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [BlogPostSearchService, BlogHomePageBackendApiService],
    });
  ...
  describe('updateSearchFieldsBasedOnUrlQuery', () => {
    ...
    it('should find as many keywords as provided in search query', () => {
      urlSearchQuery =
        '?q=protractor%20blog%20test&tags=("News"%20OR%20' + '"Mathematics")';
      response =
        searchService.updateSearchFieldsBasedOnUrlQuery(urlSearchQuery);
      expect(response.selectedTags.sort()).toEqual(['Mathematics', 'News']);
    ...
    it('should only use correct fields when ampersand is not escaped anywhere', () => {
      ...
      expect(response.searchQuery).toEqual('protractor');
    });
    ...
    it('should omit url component if it is malformed', () => {
      ...
      urlSearchQuery = '?q=protractor%20test&tags="Mathematics"';
      response =
        searchService.updateSearchFieldsBasedOnUrlQuery(urlSearchQuery);
      expect(response.selectedTags.length).toBe(0);
  ...
  describe('executeSearchQuery', () => {
    ...
    it('should use reject handler if fetching data fails while search exec', fakeAsync(() => {
      spyOn(
        blogHomePageBackendApiService,
        'fetchBlogPostSearchResultAsync'
      ).and.returnValue(
      ...
      expect(errorHandler).toHaveBeenCalledWith('Some error in the backend.');
    }));
