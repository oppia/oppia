...
describe('Exploration Improvements Service', () => {
  ...
  let explorationImprovementsTaskRegistryService: ExplorationImprovementsTaskRegistryService;
  ...
  beforeEach(() => {
    ...
    explorationImprovementsTaskRegistryService = TestBed.inject(
    ...
    essGetExplorationStatsSpy = spyOn(
      explorationStatsService,
      'getExplorationStatsAsync'
    );
    pibasFetchIssuesSpy = spyOn(
      playthroughIssuesBackendApiService,
      'fetchIssuesAsync'
    );
    stassGetTopAnswersByStateNameAsyncSpy = spyOn(
      stateTopAnswersStatsService,
      'getTopAnswersByStateNameAsync'
    );

    eibasGetTasksAsyncSpy.and.returnValue(
      Promise.resolve(new ExplorationImprovementsResponse([], new Map()))
    );
  ...
  it('should enable improvements tab based on back-end response', fakeAsync(async () => {
  ...
  it('should propagate errors from the back-end', fakeAsync(async () => {
    ...
    const promise = explorationImprovementsService
      .initAsync()
  ...
  describe('Flushing updated tasks', () => {
    ...
    beforeEach(() => {
      eibasPostTasksAsyncSpy = spyOn(
        explorationImprovementsBackendApiService,
        'postTasksAsync'
      );
      eibasGetConfigAsyncSpy = spyOn(
        explorationImprovementsBackendApiService,
        'getConfigAsync'
      );
      ...
      eibasGetConfigAsyncSpy.and.returnValue(
        Promise.resolve(newExpImprovementsConfig(true))
      );
    });

    it(
      'should do nothing when flush is attempted while the improvements ' +
        'tab is disabled',
      fakeAsync(() => {
        eibasGetConfigAsyncSpy.and.returnValue(
          Promise.resolve(newExpImprovementsConfig(false))
        );
    ...
    it('should post new high bounce rate tasks', fakeAsync(async () => {
      ...
      const eqPlaythrough = Playthrough.createNewEarlyQuitPlaythrough(
        expId,
        expVersion,
        {
      ...
      pibasFetchIssuesSpy.and.returnValue(Promise.resolve([eqPlaythrough]));
    ...
    it('should post new NGR tasks after they are resolved', fakeAsync(async () => {
      ...
      const [ngrTask] =
        explorationImprovementsTaskRegistryService.getOpenNeedsGuidingResponsesTasks();
