...
describe('Entity voiceovers service', () => {
  ...
  it('should toggle manual voiceover needs update by language accent', () => {
    entityVoiceoversService.addEntityVoiceovers('en-US', entityVoiceovers);
    expect(
      entityVoiceovers.voiceoversMapping.content0.manual.needsUpdate
    ).toBeFalse();
  ...
  it('should get all voiceovers across all language accent codes', () => {
    let manualVoiceover2: VoiceoverBackendDict = {
      filename: 'b.mp3',
      file_size_bytes: 200000,
      needs_update: false,
    ...
    let allVoiceovers = entityVoiceoversService.getAllVoiceovers();

    expect(allVoiceovers.length).toBe(3);
    expect(allVoiceovers[0].filename).toBe('a.mp3');
    expect(allVoiceovers[1].filename).toBe('b.mp3');
    expect(allVoiceovers[2].filename).toBe('c.mp3');
  });

  it('should get active entity voiceovers when activeLanguageAccentCode is undefined', () => {
    entityVoiceoversService.addEntityVoiceovers('en-US', entityVoiceovers);
    entityVoiceoversService.activeLanguageAccentCode = undefined as any;

    let activeVoiceovers = entityVoiceoversService.getActiveEntityVoiceovers();

    expect(activeVoiceovers).toEqual(entityVoiceovers);
    expect(entityVoiceoversService.activeLanguageAccentCode).toBe('en-US');
  });

  it('should emit voiceoverLoad event after fetching entity voiceovers', fakeAsync(() => {
    const onVoiceoverLoadSpy = jasmine.createSpy('onVoiceoverLoad');
    entityVoiceoversService.onVoiceoverLoad.subscribe(onVoiceoverLoadSpy);

    entityVoiceoversService.fetchEntityVoiceovers();
    tick();
    flushMicrotasks();

    expect(onVoiceoverLoadSpy).toHaveBeenCalled();
  }));

  it('should get onLanguageAccentCodeChange event emitter', () => {
    const emitter = entityVoiceoversService.onLanguageAccentCodeChange;
    expect(emitter).toBeDefined();
    expect(emitter.observers.length).toBe(0);

    const spy = jasmine.createSpy('languageAccentCodeChange');
  ...
  it('should reset languageAccentCodeToEntityVoiceovers when fetching entity voiceovers', fakeAsync(() => {
    entityVoiceoversService.addEntityVoiceovers('en-US', entityVoiceovers);
    expect(
      Object.keys(entityVoiceoversService.languageAccentCodeToEntityVoiceovers)
        .length
    ).toBe(1);
    ...
    // After fetching, old data is cleared and replaced with new data from API
    expect(
      Object.keys(entityVoiceoversService.languageAccentCodeToEntityVoiceovers)
        .length
    ).toBeGreaterThan(0);
  ...
  it('should mark voiceovers as needing update only for voiceovers that exist', () => {
    entityVoiceoversService.addEntityVoiceovers('en-US', entityVoiceovers);

    // Try to mark a non-existent content ID
    entityVoiceoversService.markManualVoiceoverAsNeedingUpdate(
      'nonexistent_content'
    );
  ...
  it('should remove voiceovers only for content that exists', () => {
    entityVoiceoversService.addEntityVoiceovers('en-US', entityVoiceovers);

    // Try to remove a non-existent content ID
    entityVoiceoversService.removeAllVoiceoversForContent(
      'nonexistent_content'
    );

    // Should not throw error, original content should remain
    expect(
      Object.keys(entityVoiceovers.voiceoversMapping).includes('content0')
    ).toBeTrue();
  });
