...
const getModuleFromRouteObjectNode = (
    ...
    const loadChildrenCallArgument = loadChildrenInitializer
        ...
        .getArguments()[0];
    return getValueFromLiteralStringOrBinaryExpression(
        loadChildrenCallArgument
    );
};
...
const convertRouteNodeToMap = (
    ...
    if (module) {
        ...
        const childRouteToModuleMapping =
            getRouteToModuleMappingFromRoutingModule(
                resolvedModule,
                routePath,
                parentRoutingModuleFilePath || resolvedModule
            );
...
const getRouteNodesFromRoutingModuleSourceFile = (
    ...
    if (
        !angularModuleObjectArgument.isKind(
