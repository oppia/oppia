    ...
    document.querySelectorAll('[src], [href]').forEach(element => {
        if (element.tagName === 'script' || element.tagName === 'style') {
            ...
            if (module) {
                ...
                try {
                    const modulePath = resolveModuleRelativeToRoot(
                        module,
                        file
                    );
...
const getDependenciesFromHtmlFile = (
    ...
    return Array.from(
        new Set([
            ...getAngularDependenciesFromHtmlFile(
                file,
...
class RootFilesMappingGenerator {
    ...
    private getTestSuiteModules(): string[] {
        ...
        for (const testSuiteConfig of testSuiteConfigFiles) {
            const config = JSON.parse(
                fs.readFileSync(testSuiteConfig, 'utf-8')
            );
    ...
    public generateRootFilesMapping(): Record<string, string[]> {
        ...
        for (const file of this.files) {
            rootFilesMapping[file] = this.getRootFilesOfFile(
                file,
                rootFilesMapping
            );
        }
        ...
        for (const [file, rootFiles] of Object.entries(rootFilesMapping)) {
            ...
            for (const rootFile of rootFiles) {
                modulizedRootFiles.push(
                    ...this.getRootFilesOfFile(
                        rootFile,
                        modulizedRootFilesMapping,
                        false
                    )
                );
            }
            modulizedRootFilesMapping[file] = Array.from(
                new Set(modulizedRootFiles)
            );
        }
