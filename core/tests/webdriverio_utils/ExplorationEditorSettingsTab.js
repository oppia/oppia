...
var ExplorationEditorSettingsTab = function () {
  ...
  this.expectAvailableFirstStatesToBe = async function (names) {
    ...
    var options = await $$('.e2e-test-initial-state-select-element').map(
      async function (elem) {
        await waitFor.visibilityOf(
          elem,
    ...
    await action.click(
      'State name option',
      initialStateSelectOption(selectedStateName)
    );
    await action.click('Neutral element', neutralElement);
  };
  ...
  this.setLanguage = async function (language) {
    await waitFor.presenceOf(
      ...
      'Language input takes too long to be visible.'
    );
  ...
  this.expectCategoryToBe = async function (category) {
    ...
    expect(await explorationCategoryDropdown.getText()).toEqual(category);
  };
  ...
  this.expectLanguageToBe = async function (language) {
    ...
    expect(await explorationLanguageInput.getText()).toEqual(language);
    await action.click('Neutral element', neutralElement);
  };
  ...
  this.expectObjectiveToBe = async function (objective) {
    ...
    expect(await explorationObjectiveInput.getValue()).toEqual(objective);
  };

  this.expectTitleToBe = async function (title) {
    await waitFor.presenceOf(
      explorationTitleInput,
      'Title input takes too long to be visible.'
    );
    expect(await explorationTitleInput.getValue()).toEqual(title);
  };
  ...
  this.expectWarningsColorToBe = async function (color) {
    ...
    expect(
      (await explorationObjectiveWarning.getCSSProperty('color')).value
    ).toEqual(color);
  };
};
