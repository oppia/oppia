...
var ExplorationEditorTranslationTab = function () {
  ...
  var audioUploadErrorMessageElement = $('.e2e-test-upload-error-message');
  ...
  this.expectToBeInVoiceoverMode = async function () {
    ...
    ).toBe('Voiceovers for language:');
    expect(
      await action.getText('Progress bar element', progressBarLabelElement)
    ).toBe('Exploration voiceover progress:');
  };

  this.expectContentTabContentToMatch = async function (content) {
    await action.click('Content tab button', contentTabButton);
    expect(await action.getText('Content tab text', contentTabText)).toMatch(
      content
    );
  };
  ...
  this.expectHintsTabContentsToMatch = async function (contents) {
    ...
    for (var index in contents) {
      await action.click(`Translation hint ${index}`, translationHint(index));
      expect(
        await action.getText(
          `Translation hint ${index}`,
          translationHintText(index)
        )
      ).toMatch(contents[index]);
    }
  };
  ...
  this.expectSolutionTabContentToMatch = async function (content) {
    ...
    expect(
      await action.getText('Solution tab button', solutionTabText)
    ).toMatch(content);
  };

  this.expectNumericalStatusToMatch = async function (content) {
    expect(await action.getText('Numerical status', numericalStatus)).toMatch(
      content
    );
  };
  ...
  this.expectSelectedLanguageToBe = async function (language) {
    var selectedLanguageElement = $(
      '.e2e-test-translation-language-selector'
    ).$('option:checked');
    expect(
      await action.getText('Selected language element', selectedLanguageElement)
    ).toMatch(language);
  };
  ...
  this.moveToState = async function (targetName) {
    ...
    if (!matched) {
      throw new Error(
        'State ' +
          targetName +
          ' not found by editorTranslationTab.' +
          'moveToState.'
      );
    }
  };
  ...
  this.expectCorrectStatusColor = async function (stateName, expectedColor) {
    ...
    for (var i = 0; i < listOfNames.length; i++) {
      if (listOfNames[i] === stateName) {
        ...
        expect(
          (await stateBackgroundNodes[i].getCSSProperty('fill')).value
        ).toBe(expectedColor);
        matched = true;
      }
    }
