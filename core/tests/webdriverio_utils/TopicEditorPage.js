...
var TopicEditorPage = function () {
  ...
  this.publishTopic = async function () {
    ...
    await waitFor.invisibilityOf(
      publishTopicButton,
      'Topic is taking too long to publish.'
    );
  };
  ...
  this.saveQuestion = async function () {
    ...
    await waitFor.invisibilityOf(
      saveQuestionButton,
      'Question modal takes too long to disappear'
    );
  };
  ...
  this.changeSubtopicPageContents = async function (content) {
    ...
    await action.click(
      'Save Subtopic Page Content Button',
  ...
  this.addSubtopic = async function (title, urlFragment, imgPath, htmlContent) {
    ...
    await action.setValue(
      'New subtopic title field',
      newSubtopicTitlefield,
      title
    );
    ...
    await action.click('Show schema editor button', showSchemaEditorElement);
    ...
    await action.click(
      'Confirm subtopic creation button',
  ...
  this.replacementDragSkillToSubtopic = async function (subtopicIndex = 0) {
    await action.click(
      'Save rearranged skills modal',
      $('.e2e-test-skill-item-edit-btn')
    );
    await action.click(
  ...
  this.expectSubtopicWithIndexToHaveSkills = async function (
    ...
    var subtopicColumns = await subtopicColumnsSelector();
    var assignedSkillDescriptions = await subtopicColumns[subtopicIndex].$$(
  ...
  this.addDiagnosticTestSkill = async function (skillId) {
    await action.click(
      ...
      addNewDiagnosticTestSkillButton
    );
    await action.select(
      'New skill selector',
  ...
  this.getTargetMoveSkill = async function (subtopicIndex, skillDescription) {
  ...
  this.expectNumberOfStoriesToBe = async function (count) {
    if (count) {
      await waitFor.visibilityOf(
        storyListTable,
        'Story list table takes too long to appear.'
      );
    }
  ...
  this.expectStoryTitleToBe = async function (title, index) {
    ...
    var text = await action.getText('Story List Text', storyListText);
    expect(text).toEqual(title);
  };
  ...
  this.expectStoryPublicationStatusToBe = async function (status, index) {
    ...
    var text = await action.getText('Story List Text', storyListText[0]);
  ...
  this.navigateToStoryWithIndex = async function (index) {
    ...
    await waitFor.invisibilityOf(
      storyListTable,
      'Story list table too long to disappear.'
    );
  };
  ...
  this.updatePageTitleFragment = async function (newPageTitleFragment) {
    await action.setValue(
      ...
      newPageTitleFragment
    );
    await action.click(
      'Page Title Fragment label',
      topicPageTitleFragmentLabel
    );
  };
  ...
  this.expectTopicNameToBe = async function (name) {
    ...
    let desc = await browser.execute(() => {
      return document.getElementsByClassName('e2e-test-topic-name-field')[0]
        .value;
    });
  ...
  this.saveTopic = async function (commitMessage) {
    ...
    await waitFor.visibilityOf(
      commitMessageField,
      'Commit Message field taking too long to appear.'
    );
    await action.setValue(
