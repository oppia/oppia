...
var LibraryPage = function () {
  ...
  var allExplorationsTitled = async function (explorationName) {
    const tiles = await $$('.e2e-test-exp-summary-tile-title');
  ...
  var _getExplorationElements = async function (name, isHidden = false) {
    ...
    return await allExplorationSummaryTiles.filter(async function (tile) {
      var tileTitle = await action.getText(
        'Exp Summary Title',
        tile.$(expSummaryTileTitleLocator)
      );
  ...
  var _submitSearchQuery = async function (searchQuery) {
    ...
    await waitFor.visibilityOf(
      searchInputElement,
      'Search input takes too long to appear'
    );

    // Function get is a zero-based index.
    var searchInputs = await searchInputsSelector();
    var searchInput = browser.isMobile ? searchInputs[1] : searchInputs[0];
  ...
  this.playExploration = async function (explorationName) {
    ...
    const explorationCardElement = await (
