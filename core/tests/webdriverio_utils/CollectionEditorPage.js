...
var CollectionEditorPage = function () {
  ...
  this.addExistingExploration = async function (explorationId) {
    await waitFor.visibilityOf(
      addExplorationInput,
      'Add Exploration Input is not visible'
    );
    await action.setValue(
      'Add Exploration Input',
      addExplorationInput,
      explorationId
    );
  ...
  this.setCommitMessage = async function (message) {
    await waitFor.visibilityOf(
      saveModal,
      'Save Modal takes too long to appear'
    );
    await waitFor.elementToBeClickable(
      commitMessageInput,
    ...
    await action.setValue('Commit Message Input', commitMessageInput, message);
  };
  ...
  this.saveDraft = async function () {
    await waitFor.elementToBeClickable(
      saveDraftButton,
      'Collection Save Draft button is not clickable'
    );
  ...
  this.closeSaveModal = async function () {
    await waitFor.elementToBeClickable(
      ...
      'Publish Changes button is not clickable'
    );
    await action.click('Close Save Modal Button', closeSaveModalButton);
    await waitFor.invisibilityOf(
      closeSaveModalButton,
      'Save Modal takes too long to close'
    );
  };
  ...
  this.setObjective = async function (objective) {
    await action.setValue(
      ...
      collectionEditorObjectiveInput,
      objective
    );
  };
  ...
  this.setCategory = async function (category) {
    ...
    await action.click('category option: ' + category, dropdownOption);
  };
  ...
  this.saveChanges = async function () {
    ...
    await waitFor.invisibilityOf(
      saveChangesButton,
      'Save Changes Modal takes too long to close'
    );
    await waitFor.invisibilityOf(
      saveInProgressLabel,
      'Collection is taking too long to save.'
    );
  };
};
