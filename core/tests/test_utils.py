...
def get_filepath_from_filename(filename: str, rootdir: str) -> Optional[str]:
    ...
    """
    matches = list(itertools.chain.from_iterable((os.path.join(subdir, f) for f in filenames if f == filename) for subdir, _, filenames in os.walk(rootdir)))
    if len(matches) > 1:
...
def mock_load_template(filename: str, template_is_aot_compiled: bool = False) -> str:
    ...
    """
    filepath = get_filepath_from_filename(filename, os.path.join('core', 'templates', 'pages'))
    ...
    for module_name in get_storage_model_module_names():
        ...
        for member_name, member_obj in inspect.getmembers(module):
            if inspect.isclass(member_obj):
                clazz: Type[base_models.BaseModel] = getattr(module, member_name)
...
def swap_is_feature_flag_enabled_function(
    ...
    def mock_is_feature_flag_enabled(
        ...
        """
        return any(expected_feature_flag_name.value == feature_flag_name for expected_feature_flag_name in feature_flag_names)

    original_is_feature_flag_enabled = getattr(feature_flag_services, 'is_feature_flag_enabled')
    setattr(
        feature_flag_services,
...
def enable_feature_flags(
    ...
    def decorator(
        ...
        def wrapper(*args: Any, **kwargs: Any) -> _GenericHandlerFunctionReturnType:
    ...
    def mock_get_platform_parameter_value(
        ...
        Raises:
            Exception. The parameter_name is not present in the
                platform_parameter_names list.
        """
        platform_parameter_name_value_dict = dict((x.value, y) for x, y in platform_parameter_name_value_tuples)
        if parameter_name not in platform_parameter_name_value_dict:
            raise Exception('The value for the platform parameter %s was needed in this test, but not specified in the set_platform_parameters decorator. Please use this information in the decorator.' % parameter_name)
        return platform_parameter_name_value_dict[parameter_name]

    original_get_platform_parameter_value = getattr(platform_parameter_services, 'get_platform_parameter_value')
    setattr(
        platform_parameter_services,
...
def set_platform_parameters(
    ...
    def decorator(
        ...
        # of arguments with different types.
        def wrapper(*args: Any, **kwargs: Any) -> _GenericHandlerFunctionReturnType:
            with swap_get_platform_parameter_value_function(platform_parameter_name_value_tuples):
                return func(*args, **kwargs)
...
class ElasticSearchStub:
    ...
    def _generate_index_not_found_error(self, index: str) -> None:
        ...
        raise elasticsearch.NotFoundError('index_not_found_exception: no such index [%s]' % index, meta, body)
    ...
    def mock_delete(
        ...
        meta = type('Meta', (), {'status': 404})()
        raise elasticsearch.NotFoundError(f'document not found: [{index}][{id}]', meta, body)

    def mock_delete_by_query(self, index: str, body: Dict[str, Dict[str, Dict[str, str]]]) -> DeletedDocumentDict:
    ...
    def mock_search(
        ...
        for doc in self._DB[index]:
            if doc['id'] not in result_doc_ids:
        ...
        return {
            ...
            'max_score': max([0.0] + [d['_score'] for d in formatted_result_docs]),
        }
...
class AuthServicesStub:
    ...
    @classmethod
    def establish_auth_session(cls, _: webapp2.Request, __: webapp2.Response) -> None:
    ...
    @classmethod
    def get_auth_claims_from_request(cls, _: webapp2.Request) -> Optional[auth_domain.AuthClaims]:
    ...
    def verify_external_auth_associations_are_deleted(self, user_id: str) -> bool:
    ...
    def get_auth_id_from_user_id(self, user_id: str) -> Optional[str]:
        ...
        """
        return next(
            (auth_id for auth_id, user in self._user_id_by_auth_id.items() if user.id == user_id and not user.deleted),
            None,
        )

    def get_user_id_from_auth_id(self, auth_id: str, include_deleted: bool = False) -> Optional[str]:
    ...
    def get_multi_user_ids_from_auth_ids(self, auth_ids: List[str]) -> List[Optional[str]]:
    ...
    def get_multi_auth_ids_from_user_ids(self, user_ids: List[str]) -> List[Optional[str]]:
        ...
        Returns:
            ...
            IDs, or None for associations which don't exist.
        """
        auth_id_by_user_id = {user.id: auth_id for auth_id, user in self._user_id_by_auth_id.items()}
        return [auth_id_by_user_id.get(user_id, None) for user_id in user_ids]

    def associate_auth_id_with_user_id(self, auth_id_user_id_pair: auth_domain.AuthIdUserIdPair) -> None:
        ...
        if auth_id in self._user_id_by_auth_id:
            raise Exception('auth_id=%r is already associated with user_id=%r' % (auth_id, self._user_id_by_auth_id[auth_id].id))
        auth_models.UserAuthDetailsModel(id=user_id, firebase_auth_id=auth_id).put()
        self._external_user_id_associations.add(user_id)
        self._user_id_by_auth_id[auth_id] = AuthServicesStub.AuthUser(user_id)

    def associate_multi_auth_ids_with_user_ids(self, auth_id_user_id_pairs: List[auth_domain.AuthIdUserIdPair]) -> None:
        ...
        """
        collisions = ', '.join('{auth_id=%r: user_id=%r}' % (a, self._user_id_by_auth_id[a].id) for a, _ in auth_id_user_id_pairs if a in self._user_id_by_auth_id)
        if collisions:
            raise Exception('already associated: %s' % collisions)
        datastore_services.put_multi([auth_models.UserAuthDetailsModel(id=user_id, firebase_auth_id=auth_id) for auth_id, user_id in auth_id_user_id_pairs])
        external_user_ids: Set[str] = {u for _, u in auth_id_user_id_pairs}
        self._external_user_id_associations.update(external_user_ids)
        auth_id_user_id_pairs_with_deletion = {auth_id: AuthServicesStub.AuthUser(user_id) for auth_id, user_id in auth_id_user_id_pairs}
...
class TaskqueueServicesStub:
    ...
    def __init__(self, test_base: GenericTestBase) -> None:
        ...
        self._client = cloud_tasks_emulator.Emulator(task_handler=self._task_handler, automatic_task_handling=False)
    ...
    def _task_handler(
        ...
        headers = {
            ...
            'X-Appengine-TaskName': (task_name.encode('utf-8') if task_name else b'None'),
        }
    ...
    def get_pending_tasks(self, queue_name: Optional[str] = None) -> List[cloud_tasks_emulator.Task]:
...
class TestBase(unittest.TestCase):
    ...
    def get_updated_param_dict(
        ...
        for param_change in param_changes:
            try:
                ...
                raise Exception('Parameter %s not found' % param_change.name) from e

            raw_value = param_change.get_value(new_param_dict)
            new_param_dict[param_change.name] = object_registry.Registry.get_object_class_by_type(obj_type).normalize(raw_value)
    ...
    @contextlib.contextmanager
    def capture_logging(self, min_level: int = logging.NOTSET) -> Iterator[List[str]]:
    ...
    # value.
    @contextlib.contextmanager
    def swap_to_always_return(self, obj: Any, attr: str, value: Optional[Any] = None) -> Iterator[None]:
    ...
    def swap_with_checks(
        ...
        def new_function_with_checks(*args: Any, **kwargs: Any) -> Any:
            ...
            if expected_args:
                ...
                self.assertEqual(
                    args,
                    next_args,
                    msg='*args to call #%d of %s' % (new_function_with_checks.call_num, msg),  # type: ignore[attr-defined]
                )
            ...
            if expected_kwargs:
                ...
                self.assertEqual(
                    kwargs,
                    next_kwargs,
                    msg='**kwargs to call #%d of %s' % (new_function_with_checks.call_num, msg),  # type: ignore[attr-defined]
                )
        ...
        try:
            ...
            # arguments of arbitrary and mixed types from different functions.
            kwargs: Any
            for args, kwargs in itertools.zip_longest(expected_args_iter, expected_kwargs_iter, fillvalue={}):
            ...
            if pretty_unused_args:
                ...
                missing_call_summary = '\n'.join(
                    '\tCall %d of %d: %s(%s)' % (i, num_expected_calls, attr, call_args)
    ...
    def assertRaises(self, *args: Any, **kwargs: Any) -> None:  # type: ignore[override]
        raise NotImplementedError('self.assertRaises should not be used in these tests. Please use self.assertRaisesRegex instead.')
    ...
    def assertRaisesRegex(  # type: ignore[override]
        ...
        if not expected_regex:
            raise Exception('Please provide a sufficiently strong regexp string to validate that the correct error is being raised.')
    ...
    def assert_matches_regexps(
        ...
        differences = ['~ [i=%d]:\t%r does not match: %r' % (i, item, regexp) for i, (regexp, item) in enumerate(zip(regexps, items)) if get_match(regexp, item, flags=re.DOTALL) is None]
        if len(items) < len(regexps):
            extra_regexps = regexps[len(items) :]
            differences.extend('- [i=%d]:\tmissing item expected to match: %r' % (i, regexp) for i, regexp in enumerate(extra_regexps, start=len(items)))
        if len(regexps) < len(items):
            extra_items = items[len(regexps) :]
            differences.extend('+ [i=%d]:\textra item %r' % (i, item) for i, item in enumerate(extra_items, start=len(regexps)))
...
class AppEngineTestBase(TestBase):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._platform_taskqueue_services_stub = TaskqueueServicesStub(cast(GenericTestBase, self))
    ...
    def tearDown(self) -> None:
        datastore_services.delete_multi(list(datastore_services.query_everything().iter(keys_only=True)))
    ...
    def count_jobs_in_taskqueue(self, queue_name: Optional[str]) -> int:
        ...
        Returns:
            ...
            taskqueue.
        """
        return self._platform_taskqueue_services_stub.count_jobs_in_taskqueue(queue_name=queue_name)

    def process_and_flush_pending_tasks(self, queue_name: Optional[str] = None) -> None:
        """Executes all of the tasks in a single queue if a queue name is
        specified or all of the tasks in the taskqueue if no queue name is
        specified.
        ...
        Args:
            queue_name: str|None. Name of the queue. Pass in None if no specific
                queue is designated.
        """
        self._platform_taskqueue_services_stub.process_and_flush_tasks(queue_name=queue_name)
    ...
    def get_pending_tasks(self, queue_name: Optional[str] = None) -> List[cloud_tasks_emulator.Task]:
        ...
        Returns:
            ...
            taskqueue.
        """
        return self._platform_taskqueue_services_stub.get_pending_tasks(queue_name=queue_name)
    ...
    def mock_set_constants_to_default(self) -> None:
        ...
        """
        raise Exception('Tests should mock common.set_constants_to_default() to avoid modifying the constants file during tests.')

    @contextlib.contextmanager
    def mock_datetime_utcnow(self, mocked_now: datetime.datetime) -> Iterator[None]:
...
class GenericTestBase(AppEngineTestBase):
    ...
    VERSION_1_STORY_CONTENTS_DICT: Final = {
        'nodes': [
            {
                'outline': ('<p>Value</p><oppia-noninteractive-math raw_latex-with-value="&amp;quot;+,-,-,+&amp;quot;"></oppia-noninteractive-math>'),
                'exploration_id': None,
    ...
    VERSION_4_STORY_CONTENTS_DICT: Final = {
        'nodes': [
            {
                'outline': ('<p>Value</p><oppia-noninteractive-math math_content-with-value="{&amp;quot;raw_latex&amp;quot;: &amp;quot;+,-,-,+&amp;quot;, &amp;quot;svg_filename&amp;quot;: &amp;quot;&amp;quot;}"></oppia-noninteractive-math>'),
                'exploration_id': None,
                'destination_node_ids': [],
                'outline_is_finalized': False,
                'acquired_skill_ids': [],
...
"""
    ...
    def run(self, result: Optional[unittest.TestResult] = None) -> None:
        ...
        with contextlib.ExitStack() as stack:
            ...
            stack.enter_context(self.swap(es_client.indices, 'create', es_stub.mock_create_index))
            stack.enter_context(self.swap(es_client, 'index', es_stub.mock_index))
            stack.enter_context(self.swap(es_client, 'exists', es_stub.mock_exists))
            stack.enter_context(self.swap(es_client, 'delete', es_stub.mock_delete))
            stack.enter_context(self.swap(es_client, 'delete_by_query', es_stub.mock_delete_by_query))
            stack.enter_context(self.swap(es_client, 'search', es_stub.mock_search))
            stack.enter_context(
                self.swap(
                    memory_cache_services,
                    'flush_caches',
    ...
    @contextlib.contextmanager
    def login_context(self, email: str, is_super_admin: bool = False) -> Iterator[Optional[str]]:
    ...
    def signup(self, email: str, username: str, is_super_admin: bool = False) -> None:
    ...
    def set_curriculum_admins(self, curriculum_admin_usernames: List[str]) -> None:
    ...
    def set_topic_managers(self, topic_manager_usernames: List[str], topic_id: str) -> None:
    ...
    def set_translation_coordinators(self, translation_coordinator_usernames: List[str], language_id: str) -> None:
    ...
    def set_collection_editors(self, collection_editor_usernames: List[str]) -> None:
    ...
    @overload
    def get_user_id_from_email(self, email: str, *, strict: Literal[True]) -> str: ...

    @overload
    def get_user_id_from_email(self, email: str, *, strict: Literal[False]) -> Optional[str]: ...

    def get_user_id_from_email(self, email: str, strict: bool = True) -> Optional[str]:
        ...
        """
        user_settings = user_services.get_user_settings_by_auth_id(self.get_auth_id_from_email(email))
        if user_settings is None:
            if not strict:
                return None
            raise Exception('No user_id found for the given email address: %s' % email)
    ...
    def get_all_python_files(self) -> List[str]:
        ...
        for _dir, _, files in os.walk(current_dir):
            for file_name in files:
                filepath = os.path.relpath(os.path.join(_dir, file_name), start=current_dir)
                if filepath.endswith('.py') and filepath.startswith(('core/', 'extensions/')) and not filepath.startswith('core/tests'):
    ...
    def get_custom_response(
        ...
        Returns:
            webtest.TestResponse. The test response.
        """
        self.assertNotIn(expected_content_type, ['text/html', 'application/json'])
    ...
    def get_response_without_checking_for_errors(
        ...
        if http_method == 'POST':
            ...
            response = self.testapp.delete(url, params=params, expect_errors=True)
    ...
    def _parse_json_response(self, json_response: webtest.TestResponse, expect_errors: bool) -> Any:
    ...
    def post_json(
        ...
        response: Dict[str, Any] = self._parse_json_response(json_response, expect_errors)
        return response
    ...
    def _send_post_request(
        ...
        expect_errors: bool,
        expected_status_int: int = 200,
        upload_files: Optional[Union[List[Tuple[str, str, bytes]], Tuple[Tuple[bytes, ...], ...]]] = None,
        headers: Optional[Dict[str, str]] = None,
    ) -> webtest.TestResponse:
        ...
        if upload_files is not None:
            encoded_upload_files = tuple(tuple(f.encode('utf-8') if isinstance(f, str) else f for f in upload_file) for upload_file in upload_files)
    ...
    def put_json(
        ...
        json_response = self.testapp.put(url, params=params, expect_errors=expect_errors)
    ...
    def save_new_default_exploration(self, exploration_id: str, owner_id: str, title: str = 'A title') -> exp_domain.Exploration:
        ...
        """
        exploration = exp_domain.Exploration.create_default_exploration(exploration_id, title=title, category='Algebra')
    ...
    def set_interaction_for_state(
        ...
        def traverse_schema_and_assign_content_ids(value: Any, schema: Dict[str, Any], ca_name: str) -> None:
            ...
            if is_subtitled_html_spec or is_subtitled_unicode_spec:
                ...
                for x in value:
                    traverse_schema_and_assign_content_ids(x, schema['items'], ca_name)
            elif schema['type'] in (
            ...
            # schemas in the future, this utility function will fail noisily.
                ...
                raise NotImplementedError(
                    (  # pragma: no cover
                        'GenericTestBase.set_interaction_for_state() received an interaction schema with an unsupported type: %s. If you need to test such an interaction, please update GenericTestBase to add support.'
                    )
                    % schema['type']
                )
        ...
        for ca_spec in ca_specs:
            ...
            traverse_schema_and_assign_content_ids(ca_value, ca_spec.schema, ca_name)
            # Here we use cast because these ca_values are fetched dynamically
            # and contain only default types.
            customization_args_value = cast(state_domain.UnionOfCustomizationArgsDictValues, ca_value)
    ...
    def save_new_valid_exploration(
        ...
        self.set_interaction_for_state(init_state, interaction_id, content_id_generator)
    ...
    def save_new_linear_exp_with_state_names_and_interactions(
        ...
        for from_state_name, dest_state_name in zip(state_names[:-1], state_names[1:]):
            from_state = exploration.states[from_state_name]
            self.set_interaction_for_state(from_state, next(iterable_interaction_ids), content_id_generator)
    ...
    def add_explorations_to_story(self, topic_id: str, story_id: str, exp_ids: List[str]) -> None:
    ...
    def save_new_story(
        ...
        """
        story = story_domain.Story.create_default_story(story_id, title, description, corresponding_topic_id, url_fragment)
        story.title = title
    ...
    def save_new_subtopic(self, subtopic_id: int, owner_id: str, topic_id: str) -> subtopic_page_domain.SubtopicPage:
        ...
        """
        subtopic_page = subtopic_page_domain.SubtopicPage.create_default_subtopic_page(subtopic_id, topic_id)
        subtopic_changes = [
        ...
        subtopic_page_services.save_subtopic_page(owner_id, subtopic_page, 'Create new subtopic', subtopic_changes)
        return subtopic_page

    def save_new_study_guide(self, subtopic_id: int, owner_id: str, topic_id: str) -> study_guide_domain.StudyGuide:
    ...
    def save_new_topic(
        ...
        thumbnail_bg_color: Optional[str] = (constants.ALLOWED_THUMBNAIL_BG_COLORS['topic'][0]),
        ...
        canonical_story_references = [topic_domain.StoryReference.create_default_story_reference(story_id) for story_id in canonical_story_ids]
        additional_story_references = [topic_domain.StoryReference.create_default_story_reference(story_id) for story_id in additional_story_ids]
    ...
    def save_new_question_with_state_data_schema_v27(
        ...
        question_model = question_models.QuestionModel(
            ...
            inapplicable_skill_misconception_ids=(inapplicable_skill_misconception_ids or []),
        )
    ...
    def save_new_question_suggestion_with_state_data_schema_v27(
        ...
        if suggestion_id is None:
            suggestion_id = feedback_models.GeneralFeedbackThreadModel.generate_new_thread_id(feconf.ENTITY_TYPE_SKILL, skill_id)
    ...
    def save_new_valid_classroom(
        ...
        topic_id_to_prerequisite_topic_ids: Optional[Dict[str, List[str]]] = None,
        ...
        """
        dummy_thumbnail_data = classroom_config_domain.ImageData('thumbnail.svg', 'transparent', 1000)
        ...
        classroom = classroom_config_domain.Classroom(
            ...
            topic_id_to_prerequisite_topic_ids=(topic_id_to_prerequisite_topic_ids if topic_id_to_prerequisite_topic_ids is not None else {}),
            is_published=is_published,
            diagnostic_test_is_enabled=(diagnostic_test_is_enabled),
            thumbnail_data=(thumbnail_data if thumbnail_data is not None else dummy_thumbnail_data),
            banner_data=(banner_data if banner_data is not None else dummy_banner_data),
            index=0,
        )
...
class LinterTestBase(GenericTestBase):
    ...
    def assert_same_list_elements(self, phrases: List[str], stdout: List[str]) -> None:
        ...
        Args:
            ...
            stdout: list(str). A list of the output results from the method's
                execution.
        """
        self.assertTrue(any(all(p in output for p in phrases) for output in stdout))

    def assert_failed_messages_count(self, stdout: List[str], expected_failed_count: int) -> None:
...
class EmailMessageMock:
    ...
    def __init__(
        ...
        recipient_variables: Optional[Dict[str, Dict[str, Union[str, int]]]] = None,
...
class GenericEmailTestBase(GenericTestBase):
    ...
    emails_dict: Dict[str, List[EmailMessageMock]] = collections.defaultdict(list)
    ...
    def _send_email_to_recipients(
        ...
        new_email = EmailMessageMock(
            ...
            recipient_variables=(recipient_variables if recipient_variables else None),
...
class FailingFunction(FunctionWrapper):
    ...
    def __init__(
        ...
        if not self._always_fail and self._num_tries_before_success < 0:
            raise ValueError('num_tries_before_success should either be an integer greater than or equal to 0, or FailingFunction.INFINITY')
    ...
    def pre_call_hook(self, args: OrderedDict[str, Any]) -> None:
        ...
        """
        self._times_called += 1
        call_should_fail = self._always_fail or self._num_tries_before_success >= self._times_called
        if call_should_fail:
