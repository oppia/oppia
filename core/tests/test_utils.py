    ...
    def swap_with_checks(
        ...
        try:
            ...
            if pretty_unused_args:
                num_expected_calls = new_function_with_checks.call_num + len( # type: ignore[attr-defined]
                    pretty_unused_args
                )
...
"""
    ...
    def run(self, result: Optional[unittest.TestResult] = None) -> None:
        ...
        with contextlib.ExitStack() as stack:
            ...
            stack.enter_context(
                self.swap(
                    elastic_search_services.ES, 'index', es_stub.mock_index
                )
            )
            stack.enter_context(
                self.swap(
                    elastic_search_services.ES, 'exists', es_stub.mock_exists
                )
            )
            stack.enter_context(
                self.swap(
                    elastic_search_services.ES, 'delete', es_stub.mock_delete
                )
            )
            stack.enter_context(
                self.swap(
            ...
            stack.enter_context(
                self.swap(
                    elastic_search_services.ES, 'search', es_stub.mock_search
                )
            )
            stack.enter_context(
                self.swap(
