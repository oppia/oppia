...
/**
 * @fileoverview Acceptance tests for viewing statistics and past explorations
 * on the creator dashboard.
 * User Journey: Create explorations, have another user play and rate one,
 * then verify stats as the creator.
 */
...
test.describe('Exploration Editor', function () {
  ...
  test('should display created explorations and their statistics on the creator dashboard after creating, playing, and rating as a logged-in user', async function () {
    ...
    await explorationEditor.expectExplorationNameToAppearNTimes('Real Numbers');
    await explorationEditor.expectExplorationNameToAppearNTimes('Fractions');
    await explorationEditor.expectExplorationNameToAppearNTimes('Untitled', 2);
  });
