...
export class Contributor extends ExplorationEditor {
  ...
  async navigateToTabInMyContributions(
    ...
    if (this.isViewportAtMobileWidth()) {
      ...
      const optionElement = await this.page.waitForSelector(`xpath=${xpath}`);
      ...
      await this.page.waitForSelector(`xpath=${xpathActive}`);
    }
  }
  ...
  async selectBadgeTypeInMobileView(
    ...
    const badgeOption = await this.page.waitForSelector(`xpath=${buttonXPath}`);
    if (!badgeOption) {
  ...
  async fillReviewComment(comment: string): Promise<void> {
    ...
    await expect(this.page.locator(reviewCommentTextareaSelector)).toHaveValue(
      comment
    );
  }
