...
interface FeedbackMessageProperties {
  timestamp: string;
...
export class Moderator extends BaseUser {
   ...
   */
  private async getPropertiesOfCommit(
    commitIndex: number
  ): Promise<CommitProperties> {
    await this.page.waitForSelector(commitRowSelector);
    const commitRows = await this.page.$$(commitRowSelector);
  ...
  async expectCommitPropertyToBe(
    commitIndex: number,
    property: keyof CommitProperties,
    value: string
  ): Promise<void> {
    const commit = await this.getPropertiesOfCommit(commitIndex);
  ...
  async clickOnExplorationLinkInRecentCommitsTable(
    ...
    await this.clickOnElement(explorationElement as ElementHandle<Element>);
   ...
   */
  private async fetchFeedbackMessage(
    messageIndex: number
  ): Promise<FeedbackMessageProperties> {
    ...
    return {
      ...
      explorationId: explorationId as string,
  ...
  async openFeedbackTabFromLinkInExplorationId(
    ...
    await this.page.waitForFunction((explorationID: string) => {
    ...
    await this.clickOnElementWithText(explorationID as string);
  ...
  async featureActivity(
    ...
    const explorationIdFieldElements = await this.page.$$(explorationIDField);
    if (explorationIdFieldElements.length < activityIndex + 1) {
      throw new Error(
        `Invalid activity index: ${activityIndex}\n.` +
          `Found only ${explorationIdFieldElements.length} activities.`
      );
    }
    const explorationIdFieldElement = explorationIdFieldElements[activityIndex];
    await this.typeInInputField(
      explorationIdFieldElement as ElementHandle<Element>,
      explorationId as string
    );
  ...
  async unfeatureActivityAtIndex(index: number): Promise<void> {
    // Subtracting 1 from index to make it 0-based.
    index -= 1;
    ...
    await this.clickOnElement(deleteButton as ElementHandle<Element>);
...
export let ModeratorFactory = (page: Page): Moderator => {
  return new Moderator(page);
};
