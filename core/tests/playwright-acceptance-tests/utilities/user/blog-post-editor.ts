...
// Selectors used by the RTE inline within updateBlogBodyUsingAllRTEFeatures.
...
export class BlogPostEditor extends BaseUser {
  ...
  async uploadBlogPostThumbnailImage(
    ...
    if (this.isViewportAtMobileWidth()) {
      ...
      await this.clickOnElementWithSelector(addThumbnailImageButton);
      await this.expectElementToBeVisible('body.modal-open', false);
    }
  }
      ...
      if (tagText === tag) {
        ...
        await this.page.waitForFunction(
          ...
          },
          {
            ...
            state: shouldBePresent ? 'true' : 'false',
          }
        );
  ...
  async updateBlogBodyUsingAllRTEFeatures(): Promise<void> {
    ...
    // Change format to heading (adapted from rteEditor.changeFormatTo('heading')).
    await this.page.waitForSelector(paragraphFormatOptionSelector);
    const formatOptionElement = await this.page.$(
      paragraphFormatOptionSelector
    );
    if (!formatOptionElement) {
      throw new Error('Format option not found.');
    }
    await formatOptionElement.click();
    ...
    const headingSelector = 'a[title*="heading"]';
    const headingElement = await iframe.waitForSelector(headingSelector);
    if (!headingElement) {
      throw new Error('Format "heading" not found.');
    }
    ...
    await this.page.waitForFunction(
      ...
      {selector: paragraphFormatOptionSelector, value: 'heading'}
    );

    // Heading Paragraph.
    await this.page.keyboard.type('Test Heading\n');

    // Change format to normal (adapted from rteEditor.changeFormatTo('normal')).
    const formatOptionElement2 = await this.page.$(
      paragraphFormatOptionSelector
    );
    if (!formatOptionElement2) {
      throw new Error('Format option not found.');
    }
    ...
    const normalSelector = 'a[title*="normal"]';
    const normalElement = await iframe2.waitForSelector(normalSelector);
    if (!normalElement) {
      throw new Error('Format "normal" not found.');
    }
    await normalElement.click();
    ...
    await this.page.waitForFunction(
      ...
      {selector: paragraphFormatOptionSelector, value: 'Normal'}
    );
