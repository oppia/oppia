...
export class TranslationSubmitter extends BaseUser {
  ...
  async clickOnPaginationButtonInTranslationSubmitterPage(
    ...
    await this.page.waitForFunction(
      ({
        ...
        selector: string;
        preClickFirstHeading: string | null;
      }) => {
        const element = document.querySelector(selector);
        return element && element.textContent !== preClickFirstHeading;
      },
      {selector: opportunityItemHeadingSelector, preClickFirstHeading}
    );
  }
  ...
  async clickOnSkipTranslationButton(): Promise<void> {
    ...
    const preClickContent = await this.page.evaluate((selector: string) => {
  ...
  async typeTextForRTE(text: string): Promise<void> {
    ...
    await this.page.waitForFunction(
      ...
      {selector: rteEditorBodySelector, initialHTMLContent}
    );
  }
...
export const TranslationSubmitterFactory = (page: Page): TranslationSubmitter =>
  new TranslationSubmitter(page);
