...
const translationSubmittedToastMessage = 'Submitted translation for review.';
...
export class TranslationSubmitter extends BaseUser {
  /** Verifies the V2 submission confirmation toast. */
  async expectTranslationSubmittedToast(): Promise<void> {
    const submittedToast = this.page
      .locator(toastMessageSelector)
      .filter({hasText: translationSubmittedToastMessage})
      .last();
