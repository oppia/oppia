...
describe('Topic and Story viewer functionality', function () {
  ...
  it('should play through story and save progress on login.', async function () {
    ...
    await users.createAndLoginUser(
      'newStoryViewer@storyviewer.com',
      'newStoryViewer',
      false
    );
  ...
  it(
    'should dismiss sign-up section and load practice session page upon' +
      'clicking the practice session card on the last state.',
    async function () {
      await topicAndStoryViewerPage.get(
        'math',
        'topic-tasv-one',
  ...
  it(
    'should load the next chapter upon clicking the next chapter ' +
  ...
  it('should check for topic description, stories and revision cards', async function () {
    await users.createAndLoginCurriculumAdminUser(
      'creator1@storyViewer.com',
      'creatorStoryViewer1'
    );
