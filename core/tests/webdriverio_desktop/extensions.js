...
describe('Interactions', function () {
    ...
    for (var interactionId in interactions.INTERACTIONS) {
      ...
      for (var i = 0; i < interaction.testSuite.length; i++) {
        ...
        await explorationEditorMainTab.setInteraction.apply(
          null,
          [interactionId].concat(test.interactionArguments)
        );
        ...
        await explorationEditorMainTab.addResponse.apply(
          ...
          [interactionId, await forms.toRichText('yes'), null, false].concat(
            test.ruleArguments
          )
        );
        ...
        await explorationPlayerPage.expectInteractionToMatch.apply(
          null,
          [interactionId].concat(test.expectedInteractionDetails)
        );
        ...
        var clearHelpcardButton = $('.e2e-test-close-help-card-button');
        ...
        if (isExisting) {
          ...
          await waitFor.invisibilityOf(
            helpCard,
            'Help Card takes too long to disappear.'
          );
        }
  ...
  it('should publish and play exploration successfully', async function () {
    ...
    await users.createAndLoginUser(
      'explorationEditor@interactions.com',
      'explorationEditor'
    );
    await workflow.createExploration(true);
    await explorationEditorMainTab.setStateName('Graph');
    await explorationEditorMainTab.setContent(
      await forms.toRichText('Draw a complete graph with the given vertices.'),
      true
    );
  ...
  it(
    'should successfully play through exploration containing ' +
      'two supplemental card interactions',
    async () => {
      await users.createAndLoginUser(
        'supplementalCardEditor@interactions.com',
        'supplementalCardEditor'
      );
      await workflow.createExploration(true);
      await explorationEditorMainTab.setStateName('Graph');
      await explorationEditorMainTab.setContent(
      ...
      await users.createAndLoginUser(
        'supplementalLearner@interactions.com',
        'supplementalLearner'
      );
      await libraryPage.get();
