...
describe('Blog Pages functionality', function () {
  ...
  it('should show published blog posts on blog page and use pagination,', async function () {
    // Publishing 7 blog posts by user with username 'blog'.
    await blogDashboardPage.get();
    await blogDashboardPage.updateAuthorDetails(
      'blog',
      'Oppia Blog Author with name blog'
    );
    await blogPages.publishNewBlogPostFromBlogDashboard(
      'Blog post Title Two',
      'Hi there, I’m Oppia! I’m an online personal tutor for everybody!' +
  ...
  it(
    'should navigate to blog post pages, show correct blog post title and' +
      ' show correct blog post recommendations',
    async function () {
      ...
      await blogPages.expectBlogPostPageTitleToBe(
        'Eight Article by blog author'
      );
      await blogPages.expectBlogAuthorDetailsToBeVisible('blog');
      await blogPages.expectNumberOfRecommendationPostsToBe(2);
      ...
      await blogPages.expectRecommendationsToContainPostWithTitle('Sixth Blog');
  ...
  it(
    'should load correct search results on performing search using query and' +
      ' on applying tag filters',
    async function () {
      await blogPages.get();
