...
describe('Full exploration editor', function () {
  ...
  it('should reflect skills and goal in exploration editor settings', async function () {
  ...
  it('should report an exploration to moderators', async function () {
    ...
    await users.createUser(
      'creator@editorAndPlayer.com',
      'creatorEditorAndPlayer'
    );
  ...
  it('should prevent going back when help card is shown', async function () {
    ...
    await explorationEditorMainTab.setContent(
      await forms.toRichText('this is card 2 with non-inline interaction'),
      true
    );
    await explorationEditorMainTab.setInteraction('CodeRepl');
    await explorationEditorMainTab.addResponse(
      'CodeRepl',
      await forms.toRichText('Nice. Press continue button'),
      'final card',
      true,
      'CodeDoesNotContain',
      'test'
    ...
    await waitFor.visibilityOf(
      nextCardButton,
      'Next Card button taking too long to show up.'
    );
    await waitFor.invisibilityOf(
      backButton,
      'Back button takes too long to disappear.'
    );
  ...
  it(
    'should redirect back to parent exploration correctly when parent id is' +
      ' given as query parameter',
    async function () {
      await users.createUser(
        'user1@editorAndPlayer.com',
        'user1EditorAndPlayer'
      );
  ...
  it('should navigate multiple states correctly, with parameters', async function () {
    await users.createUser('user4@editorAndPlayer.com', 'user4EditorAndPlayer');
    ...
    await explorationEditorMainTab.moveToState('card 2');
    await explorationEditorMainTab.setContent(
      await forms.toRichText('this is card 2 with previous answer {{answer}}'),
      true
    );
    ...
    await explorationPlayerPage.submitAnswer('MultipleChoiceInput', 'complete');
    await explorationPlayerPage.expectExplorationToBeOver();
    await users.logout();
  });
