...
describe('Logged-out User', function () {
  ...
  it(
    'should only see welcome message when no blogs are available',
    async function () {
      // Navigate to "About" > "Blog" from top navbar.
      await interestParent.navigateToBlogPageViaNavbar();
      // Verify welcome message and no results message.
  ...
  it(
    'should only see published blogs (drafts are hidden)',
    async function () {
  ...
  it(
    'should display correct number of blogs per page with pagination',
    async function () {
  ...
  it(
    ...
    async function () {
      ...
      await blogPostWriter.openBlogEditorPage();
      await blogPostWriter.uploadBlogPostThumbnailImage();
      await blogPostWriter.updateBlogPostTitle('Education Blog Post');
      await blogPostWriter.updateBodyTextTo('This is about education');
      await blogPostWriter.selectTag('Educators');
      await blogPostWriter.saveBlogBodyChanges();
      await blogPostWriter.publishTheBlogPost();

      await interestParent.navigateToBlogPage();
      await interestParent.filterBlogPostsByTag('Educators');
      await interestParent.expectBlogSearchResultsToHaveTag('Educators');

      await interestParent.navigateToBlogPage();
      await interestParent.filterBlogPostsByKeyword('Education');
      await interestParent.expectBlogSearchResultsToContain('Education');
