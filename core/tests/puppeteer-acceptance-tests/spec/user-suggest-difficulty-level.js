//Users can suggest questions by selecting difficulty level to a lesson in a Topic
...
module.exports = class e2eBlogPostAdmin extends baseUser {
    ...
    async hardDifficultyAndContinue(){
        //3. Select difficulty level as Medium and click on Continue button
        await this.clickOn(LABEL_FOR_CONTINUE_BUTTON);
    }

    async typeYourQuestion(){
        //4. In 'Type your question here' field enter 'Question 1: Test Question', click on Save Question button. Click on Add 
        //   interaction button and select Drag and drop sort type under Common tab
        await this.type(LABEL_FOR_NEW_TOPIC_NAME, "Question 1: Test Question"); //TODO::need to find CSS selector of question button
        await this.clickOn(LABEL_FOR_SAVE_QUESTION)
        await this.clickOn(LABEL_FOR_ADD_INTERACTION)
        //TODO:: select Drag and drop sort type under Common tab
    }

    async customizeInteraction(){
        //5. In Customize Interaction dialogbox enter '1' in the textbox, click on Add a new Item enter '2', click on Add a new 
        //   item and enter '3'. Click on 'Save Interaction' button
        await this.clickOn(LABEL_FOR_SAVE_INTERACTION)
    }

    async selectLearnersResponse(){
        //6. In the Add Response dialogbox, "If the learners response 'is equal to the ordering'" and enter "Correct" in the 
        //   field "Oppia tells the learner". Then click on Save and Add Another button and enter, 'is equal to ordering with one 
        //   item at incorrect position', click on 'Tag with misconception' and select the misconception and click on "Done" button. 
        //   Click on "Save Response" button. Click on "Add Hint" button
        //7. In Hint #1 enter 'Sort by number'
    ...
    async solutionSubmit(){
        //8. Select "'The only' solution is.. '1,2,3'", then click on the Submit button. In the explanation dialogbox enter 'Solution' 
        //   and click on 'Check and Save Solution'.
        await this.clickOn(LABEL_FOR_CHECK_AND_SAVE_SOLUTION)
    }

    async clickOnAnswers(){
        //9. Click on "All other answers 'wrong'", enter 'Incorrect answer' in "Oppia tells the learner", 
        //and then click on "Save Feedback"
...
//5. In Customize Interaction dialogbox enter '1' in the textbox, click on Add a new Item enter '2', click on Add a new 
//   item and enter '3'. Click on 'Save Interaction' button
//6. In the Add Response dialogbox, select "If the learners response 'is equal to the ordering'" and enter "Correct" in the 
//   field "Oppia tells the learner". Then click on Save and Add Another button and enter, 'is equal to ordering with one 
//   item at incorrect position', click on 'Tag with misconception' and select the misconception and click on "Done" button. 
//   Click on "Save Response" button. Click on "Add Hint" button
//7. In Hint #1 enter 'Sort by number', then click on "Save Hint" button. Next, click on "Add Solution" button
//8. Select "'The only' solution is.. '1,2,3'", then click on the Submit button. In the explanation dialogbox enter 'Solution' 
//   and click on 'Check and Save Solution'.
//9. Click on "All other answers 'wrong'", enter 'Incorrect answer' in "Oppia tells the learner", and then click on "Save Feedback"
...
//3. "Selected Difficulty", "Notes from rubrics", "Your Practice Question", "Type your question here label with edit icon" should be 
//    shown
//4. Question text must be added and Customize interaction (Drag and Drop sort) dialogbox should open with textbox 'Items for drag and 
//   drop' and warning message 'Please enter at least two choices'
