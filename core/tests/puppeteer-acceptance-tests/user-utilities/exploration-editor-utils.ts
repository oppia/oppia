...
export class ExplorationEditor extends BaseUser {
  ...
  async expectRevisionsToBeOrderedBy(property: string): Promise<void> {
    ...
    for (let i = 0; i < revisions.length - 1; i++) {
      let value1 = await revisions[i].$eval(property, el => el.textContent);
      let value2 = await revisions[i + 1].$eval(property, el => el.textContent);

      if (!value1 || !value2) {
        throw new Error(`Failed to retrieve property ${property}`);
      }
