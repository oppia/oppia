...
export class e2eSuperAdmin extends baseUser {
  /**
  ...
  async assignRoleToUser(username: string, role: string): Promise<void> {
    ...
    await this.page.evaluate(async(role) => {
      ...
      for (let i = 0; i < allRoles.length; i++) {
        if ((allRoles[i] as HTMLElement).innerText.toLowerCase() === role) {
          (allRoles[i] as HTMLButtonElement).click();  // TODO: check if flake does not occur by removing {waitUntil: 'networkidle0'} within the click function.
          return;
        }
      }
  ...
  async expectUserToHaveRole(username: string, role: string): Promise<void> {
    ...
    await this.page.evaluate((role) => {
      ...
      for (let i = 0; i < userRoles.length; i++) {
        if ((userRoles[i] as HTMLElement).innerText.toLowerCase() === role) {
          return;
        }
      }
