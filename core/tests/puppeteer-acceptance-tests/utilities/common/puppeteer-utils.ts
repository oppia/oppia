...
export class BaseUser {
        ...
        await puppeteer
            ...
            .then(async browser => {
                ...
                if (process.env.VIDEO_RECORDING_IS_ENABLED === '1') {
                    ...
                    const fullScreenRecordingPath = path.join(
                        outputDir,
                        outputFileName
                    );
                    showMessage(
                        `Saving screen recording to ${fullScreenRecordingPath}`
                    );
    ...
    async captureScreenshotsForFailedTest(): Promise<void> {
        ...
        const outputFileName =
            `${specName}-${new Date().toISOString()}`.replace(
                /[^a-z0-9.-]/gi,
                '_'
            );
        ...
        const screenshotPromises = BaseUser.instances.map(
            async (instance, i) => {
                if (instance.page) {
                ...
                const clickabilityDiagnostics = await this.page.evaluate(
                    (targetElement: Element) => {
                        ...
                        const isAriaDisabled =
                            targetElement.getAttribute('aria-disabled') ===
                                'true' ||
                            targetElement.closest('[aria-disabled="true"]') !==
                                null;
                        ...
                        const firstRectTopElement = firstClientRect
                            ? document.elementFromPoint(
                                  firstClientRect.left +
                                      firstClientRect.width / 2,
                                  firstClientRect.top +
                                      firstClientRect.height / 2
                              )
                            : null;
                        ...
                        if (isCoveredByOtherElement) {
                            reasons.push(
                                `Element is blocked by ${blockingElement}.`
                            );
                        }
    ...
    async selectMatOption(value: string): Promise<void> {
        ...
        for (const matOptionElement of matOptionElements) {
            if (
                (await matOptionElement.evaluate(el =>
                    el.textContent?.trim()
                )) === value
            ) {
    ...
    async uploadFile(filePath: string): Promise<void> {
        ...
        if (inputUploadHandle === null) {
            throw new Error(
                'No file input found while attempting to upload a file.'
            );
        }
    ...
    async openExternalLink(
        ...
        if (href !== expectedUrl) {
            throw new Error(
                `Actual URL differs from expected. It opens: ${href}.`
            );
        }
    }
                ...
                await Promise.race([
                    ...
                    new Promise((_, reject) =>
                        setTimeout(
                            () =>
                                reject(
                                    new Error('Screen recorder stop timed out')
                                ),
    ...
    async expectExplorationToBeAccessibleByUrl(
        ...
        try {
            ...
            showMessage(
                'Exploration is accessible with the URL, i.e. published.'
            );
        } catch (error) {
    ...
    async verifyAnchorTagIsPresent(
        ...
        const anchor = await context.waitForSelector(
    ...
    async expectElementToBeVisible(
        ...
        showMessage(
            `Element ${selector} is ${visibility ? 'visible' : 'hidden'}.`
        );
    }
        ...
        try {
            await this.page.waitForFunction(
                (
                    ...
                    return (
                        element && element.textContent?.trim() === text.trim()
                    );
                },
                {},
                selector,
        ...
        await this.page.waitForFunction(
            (selector: string) => {
                const el: HTMLInputElement | null =
                    document.querySelector(selector);
                return (
                    el?.click !== undefined || el?.addEventListener || el?.click
                );
            },
            {},
            selector
    ...
    async clickAndVerifyAnchorWithInnerText(
        ...
        if (!element) {
            throw new Error(
                `Anchor with inner text ${anchorInnerText} not found.`
            );
        }
    ...
    async findChildElementInParent(
        ...
        for (const h of allElements || []) {
            ...
            const targetHeadingText = await targetHeadingElement?.evaluate(
                ele => ele.textContent?.trim()
            );
            lastHeadingText = targetHeadingText || undefined;
            showMessage(
                `gettingText: ${targetHeadingElement} ${targetHeadingText}`
            );
