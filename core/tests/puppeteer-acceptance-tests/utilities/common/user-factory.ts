...
export class UserFactory {
    ...
    >(user: TUser, roles: TRoles): TUser & UnionToIntersection<TRoles[number]> {
        for (const role of roles) {
            ...
            Object.getOwnPropertyNames(rolePrototype).forEach(
                (name: string) => {
    ...
    >(
        ...
        for (const role of roles) {
            if (superAdminInstance === null) {
                superAdminInstance =
                    await UserFactory.createNewSuperAdmin('superAdm');
            }
            ...
            switch (role) {
                ...
                case ROLES.TOPIC_MANAGER:
                    if (typeof args !== 'string') {
                        throw new Error(
                            'Expected additional argument to be string.'
                        );
                    }
    ...
    static enableVoiceoverAutogenerationUsingCloudService =
        async function (): Promise<void> {
            ...
            showMessage(
                'Enabled text to speech synthesis using cloud service.'
            );
        };
