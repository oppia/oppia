...
export class ExplorationEditor extends BaseUser {
        ...
        await this.page.waitForFunction(
            (
                ...
                return (
                    ...
                    document
                        .querySelector(formErrorContainer)
    ...
    async selectItemSelectionOptions(options: string[]): Promise<void> {
        ...
        for (const optionElement of optionElements) {
            ...
            const checkboxIsCurrentlyChecked = await checkboxElement.evaluate(
                el => el.classList.contains('mat-checkbox-checked')
            );
    ...
    async confirmDeleteExplorationButton(): Promise<void> {
        ...
        await this.clickOnElementWithSelector(
            confirmExplorationDeletetionButton
        );
        await this.page.waitForSelector(confirmExplorationDeletetionButton, {
            visible: false,
        });
    ...
    async changePaginationInHistoryTabTo(numberOfPages: number): Promise<void> {
        ...
        if (index === -1) {
            throw new Error(
                `Could not find option with value ${numberOfPages}`
            );
        }
    ...
    async compareExplorationVersionsInHistoryTab(
        ...
        const versionSelectElements = await this.page.$$(
            versionComparasionSelect
        );
        ...
        expect(
            await versionSelectElements[0].evaluate(el => el.textContent)
        ).toBe(version1);
        expect(
            await versionSelectElements[1].evaluate(el => el.textContent)
        ).toBe(version2);
    }

    async startAFeedbackThread(
        subject: string,
        feedback: string
    ): Promise<void> {
    ...
    async addResponseDetailsInResponseModal(
        ...
        if (responseIsCorrect) {
            await this.clickOnElementWithSelector(
                correctAnswerInTheGroupSelector
            );
        }
        if (isLastResponse) {
     ...
     */
    async customizeDragAndDropSortInteraction(
        options: string[]
    ): Promise<void> {
    ...
    async customizeEndExplorationInteraction(
        ...
        for (const explorationId of explorationIds) {
            ...
            const lastTextInputField =
                textInputFields[textInputFields.length - 1];
    ...
    async customizeCodeEditorInteraction(
        ...
        showMessage(
            'Customized Code Editor / Preview Code Editor successfully.'
        );
    }
    ...
    async customizeAlgebricExpressionInputInteraction(
        ...
        try {
            await this.page.waitForFunction(
                (element: Element, value: string) => {
                    return (
                        (element as HTMLInputElement).textContent?.trim() ===
                        value
                    );
                },
                {},
    ...
    async customizeItemSelectionInteraction(
        ...
        showMessage(
            'Item Selection interaction has been customized successfully.'
        );
    }
    ...
    async getRuleEditorModal(): Promise<puppeteer.ElementHandle<Element>> {
        ...
        const ruleEditor = await responseBox.$(
    ...
    async updateDragAndDropSortLearnersAnswerInResponseModal(
        ...
        const ruleType =
            typeof optionsSelections[0] === 'number'
                ? 'ordering'
                : 'comparasion';
    ...
    async addMusicNotesInputSolutionToState(
        ...
        for (let i = 0; i < musicNodes.length; i++) {
            const addNoteButton = await solutionModal.$(
                addResponseOptionButton
            );
            await addNoteButton?.click();
    ...
    async updateSetInputLearnerAnswerInResponseModal(
        ...
        for (let i = 0; i < values.length; i++) {
            const addNewElementButton = await responseBox.$(
                addResponseOptionButton
            );
    ...
    async navigateToExplorationEditorFromCreatorDashboard(): Promise<void> {
        ...
        await this.clickAndWaitForNavigation(
            createExplorationButtonSelector,
            true
        );
    ...
    async publishExplorationWithMetadata(
        ...
        const confirmPublish = async (): Promise<string> => {
            ...
            const explorationId = explorationIdUrl.replace(
                /^.*\/explore\//,
                ''
            );
    ...
    async addInteraction(
        ...
        showMessage(
            `${interactionToAdd} interaction has been added successfully.`
        );
    }
    ...
    async expectTagsToMatch(expectedTags: string[]): Promise<void> {
        ...
        const lowercaseExpectedTags = expectedTags.map(tag =>
            tag.toLowerCase()
        );
        await this.page.waitForSelector('mat-chip-list');
        const observedTags = await this.page.evaluate(() => {
            const tagElements = Array.from(
                document.querySelectorAll('mat-chip')
            );
            return tagElements
                .map(tag => {
                    const textContent =
                        tag.querySelector('.mat-chip-remove')?.previousSibling
                            ?.textContent;
    ...
    async navigateToCard(
        ...
        try {
            ...
            if (retry) {
                showMessage(
                    `Unable to navigate to the card ${cardName}. Retrying...`
                );
    ...
    async updateSolutionExplanation(explanation: string): Promise<void> {
        ...
        await this.clickOnElementWithSelector(
            editStateSolutionExplanationSelector
        );
        await this.typeInInputField(stateContentInputField, explanation);
    ...
    async getExistingVersionFiles(
        ...
        return files.filter(file =>
            file.match(
                new RegExp(`^${filePrefix}${version}( \\(\\d+\\))?\\.zip$`)
            )
        );
    }
    ...
    async downloadExploration(
        ...
        for (const historyItem of historyItems) {
            ...
            if (parseInt(versionText, 10) === explorationVersion) {
                ...
                if (downloadedFile) {
                    showMessage(
                        `${downloadedFile} file is successfully downloaded`
                    );
                    return;
                } else {
                    throw new Error(
        ...
        try {
            await this.page.waitForFunction(
                (element: HTMLElement, value: string, matchCase: boolean) => {
                    return (
                        (element.innerText.trim() === value.trim()) ===
                        matchCase
                    );
                },
                {},
     ...
     */
    async expectNumberOfSubscribersToBe(
        ...
        if (
            ...
            showMessage(
                `Number of subscribers is equal to ${subscriberCount}.`
            );
        } else {
            throw new Error(
    ...
    async editTranslationOfContent(
        ...
        const activeContentType = await this.page.$eval(
    ...
    async verifyTranslationInModifyTranslationsModal(
        ...
        const translationElementText = await this.page.evaluate(
            languageCode => {
    ...
    async updateTranslationFromModal(
        ...
        await this.clickOnElementWithSelector(
            modifyTranslationsModalDoneButton
        );
    ...
    async regenerateVoiceoverForContent(
        ...
        await this.clickOnElementWithSelector(
            regenerateAutomaticVoiceoverButton
        );
    ...
    async addDragAndDropSortSolution(
        ...
        await this.page.waitForSelector(dragAndDropItemSelector, {
            visible: true,
        });
        const solutionModal = await this.getSolutionModal();
        ...
        for (let i = 0; i < sortedOptions.length - 1; i++) {
            ...
            const optionElements = await solutionModal.$$(
                dragAndDropItemSelector
            );
    ...
    async expectFeedbackStatusInList(
        ...
        await this.page.waitForFunction(
            (selector: string, elementNumber: number, expectedText: string) => {
                ...
                return (
                    elements[elementNumber - 1].textContent?.trim() ===
                    expectedText
                );
            },
            {},
    ...
    async clickOnSaveDraftButton(): Promise<void> {
        ...
        await this.expectElementToBeVisible(
            saveExplorationModalContainerSelector
        );
    }
    ...
    async expectAddResponseModalHeaderToBe(
        ...
        const header = await this.page.$eval(
            addResponseModalHeaderSelector,
            el => el.textContent?.trim()
        );
    ...
    async expectCustomizeInteractionTitleToBe(title: string): Promise<void> {
        ...
        await this.expectTextContentToBe(
            customizeInteractionHeaderSelector,
            title
        );
    }
    ...
    async expectHintsToConatin(expectedHint: string): Promise<void> {
        ...
        const hints = await this.page.$$eval(
            currentHintSummarySelector,
            elements => elements.map(el => el.textContent?.trim())
        );
    ...
    async expectInteractionPreviewCardToBeVisible(): Promise<void> {
        const visible = await this.isElementVisible(
            interactionPreviewCardSelector
        );
     ...
     */
    async expectJoyrideContentToContain(
    ...
    async submitCodeEditorAnswer(answer: string): Promise<void> {
        const codeEditor = await this.getElementInParent(
            codeEditorInSolutionModal
        );
        if (!codeEditor) {
            throw new Error('Code editor not found.');
        }
    ...
    async expectGraphNodeCanBeMoved(
        ...
        const initalCoordinates = await graphHelper.getNodeCoordinates(
            nodes[0]
        );
    ...
    async addImageRTE(
        ...
        // Get Fields.
        const imageDescriptionInput = await helperModel?.$(
        ...
        if (imageDescriptionInput) {
            ...
            throw new Error(
                'Image description input not found in the helper modal'
            );
        }
    ...
    async expectExplorationHistoryDateHasProperFormat(): Promise<void> {
        const explorationHistoryDateSelector =
            '.e2e-test-history-tab-commit-date';
        ...
        return await this.page.evaluate(
            (name: string, selectorToClick: string, nodeSelector: string) => {
                const nodes = Array.from(
                    document.querySelectorAll(nodeSelector)
                );
    ...
    async expectSaveRecommendationModalToBeVisible(): Promise<void> {
        await this.expectElementToBeVisible(
            saveRecommendationModalSelector,
            true
        );
    ...
    async expectFeedbackPageTobeVisible(): Promise<void> {
        await this.expectElementToBeVisible(
            explorationFeedbackTabContentSelector
        );
    }
}
