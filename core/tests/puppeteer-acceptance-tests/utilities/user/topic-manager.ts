...
export class TopicManager extends BaseUser {
    ...
    async resetTopicFilter(): Promise<void> {
        ...
        if (
            await this.isElementVisible(topicStatusDropdownSelector, true, 5000)
        ) {
            await this.expectTextContentToBe(
    ...
    async clickOnTopicAndSkillsOptionInProfileMenu(): Promise<void> {
        ...
        await this.clickOnElementWithSelector(
            topicAndSkillsOptionInProfileMenu
        );
        await this.expectElementToBeVisible(
            topicAndSkillsDashboardPageSelector
        );
    }
     ...
     */
    async expectToBeInSkillEditorPage(
    ...
    async removeSkillFromState(): Promise<void> {
        ...
        await this.expectElementToBeVisible(
            removeSkillModalHeaderSelector,
            false
        );
    }
    ...
    async expectError401Unauthorized(): Promise<void> {
        try {
            ...
            showMessage(
                'Verified: Error 401 Unauthorized is displayed as expected.'
            );
        } catch (error) {
    ...
    async editTopicDetails(
        ...
        if (urlFragment) {
            ...
            const closeToastMessageButton =
                'button.e2e-test-close-toast-warning';
    ...
    async togglePracticeTabCheckbox(): Promise<void> {
        ...
        try {
            ...
            const practiceTabToggleElement =
                await this.page.$(practiceTabToggle);
    ...
    async expectCreateTopicButtonNotPresent(): Promise<void> {
        ...
        try {
            ...
            throw new Error(
                'Create topic button is present, which is not expected.'
            );
        } catch (error) {
    ...
    async unassignSkillFromTopic(
        ...
        for (const topicLabel of topicLabels) {
            ...
            if (labelTopicName === topicName) {
                ...
                if (!checkbox) {
                    throw new Error(
                        `Checkbox not found for topic "${topicName}"`
                    );
                }
    ...
    async getSkillElementFromSelection(
        ...
        for (const skillItem of skillItems) {
            ...
            const descriptionElement = await skillItem.$(
    ...
    async selectSkillAndClickOnDoneInSkillSelectionModal(
        ...
        if (!skillElement) {
            throw new Error(
                `Skill ${skillName} not found in Skill Selection Modal`
            );
        }
        const radioInnerCircleSelectorElement =
            await skillElement.waitForSelector(
     ...
     * @param {string} skillName - The name of the skill to select.
     */
    async filterAndSelectSkillInSkillSelector(
        skillName: string
    ): Promise<void> {
    ...
    async expectPreviewInteractionType(expectedType: string): Promise<void> {
        try {
            ...
            switch (expectedType) {
                ...
                default:
                    throw new Error(
                        `Unsupported interaction type: ${expectedType}`
                    );
            }
            ...
            console.error(
                `Error in expectPreviewInteractionType: ${error.message}`
            );
            throw error;
        }
    }
     ...
     */
    async adjustPaginatorToShowItemsPerPage(
        itemsPerPage: number
    ): Promise<void> {
        try {
        ...
        showMessage(
            `Paginator adjusted to show ${itemsPerPage} items per page.`
        );
    }
    ...
    async expectFilteredSkillsInOrder(expectedOrder: string[]): Promise<void> {
        ...
        try {
            ...
            if (
                !topicNames.every(
                    (name, index) => name === expectedOrder[index]
                )
            ) {
    ...
    async expectCreateSkillButtonNotPresent(): Promise<void> {
        ...
        try {
            ...
            throw new Error(
                'Create skill button is present, which is not expected.'
            );
        } catch (error) {
            if (error instanceof puppeteer.errors.TimeoutError) {
    ...
    async addMisconception(
        ...
        if (this.isViewportAtMobileWidth()) {
            ...
            if (!element) {
                ...
                const misconceptionHeader = await this.page.$(
                    misconceptionCardHeader
                );
    ...
    async verifyMisconceptionPresence(
        ...
        try {
            ...
            const misconceptions = await this.page.$$(
                misconceptionTitleSelector
            );
    ...
    async deleteMisconception(misconceptionName: string): Promise<void> {
        await this.waitForStaticAssetsToLoad();
        await this.page.waitForSelector(misconceptionListSelector, {
            visible: true,
        });
        const misconceptionLists = await this.page.$$(
            misconceptionListSelector
        );
        ...
        for (const misconceptionList of misconceptionLists) {
            ...
            const titleElement = await misconceptionList.$(
                misconceptionTitleElement
            );
            if (titleElement) {
                const title = await this.page.evaluate(
                    el => el.textContent,
                ...
                if (title.trim() === misconceptionName) {
                    ...
                    if (deleteButton) {
                        ...
                        throw new Error(
                            'Misconception delete button not found.'
                        );
                    }
                }
            }
        ...
        if (!misconceptionFound) {
            throw new Error(
                `The misconception ${misconceptionName} was not found`
            );
        }
    }
    ...
    async addPrerequisiteSkillInSkillEditor(skillName: string): Promise<void> {
        try {
            if (this.isViewportAtMobileWidth()) {
                await this.page.waitForSelector(
                    togglePrerequisiteSkillsDropdown
                );
                await this.clickOnElementWithSelector(
                    togglePrerequisiteSkillsDropdown
                );
            ...
            const confirmSkillSelectionButtonSelectorElement =
                await this.page.$(confirmSkillSelectionButtonSelector);
            if (!confirmSkillSelectionButtonSelectorElement) {
                throw new Error(
                    'Confirm skill selection button selector not found'
                );
            }
            await this.clickOnElementWithSelector(
                confirmSkillSelectionButtonSelector
    ...
    async removePrerequisiteSkill(skillName: string): Promise<void> {
        try {
            ...
            for (const skillCard of skillCards) {
                const skillLink = await skillCard.$(
                    skillPrerequisiteLinkSelector
                );
                ...
                if (skillText === skillName) {
                    ...
                    const removeIcon = await skillCard.$(
                        removeSkillIconSelector
                    );
                    if (removeIcon) {
            ...
            await this.page.waitForFunction(
                (selector: string, skillName: string) => {
                    ...
                    for (const skillElement of Array.from(skillElements)) {
                        ...
                        if (
                            skillNameElement?.textContent?.trim() === skillName
                        ) {
                            return false;
                        }
                    }
    ...
    async verifyPrerequisiteSkillPresent(
        ...
        try {
            ...
            const skillCards = await this.page.$$(
                skillPrerequisiteTitleSelector
            );
    ...
    async publishUpdatedSkill(updateMessage: string): Promise<void> {
        if (this.isViewportAtMobileWidth()) {
            ...
            await this.clickOnElementWithSelector(
                mobileSaveOrPublishSkillSelector
            );
        } else {
            await this.waitForStaticAssetsToLoad();
    ...
    async expectConceptCardPreviewToHave(
        ...
        const contentElement = await this.page.$(
            skillPreviewModalContentSelector
        );
    ...
    private async openAllMobileDropdownsInSkillEditor(): Promise<void> {
        if (!this.isViewportAtMobileWidth()) {
            showMessage(
                'Skipping opening dropdowns since we are in desktop view'
            );
        }
    ...
    async openSubtopicEditor(
        ...
        try {
            ...
            const newError = new Error(
                `Failed to open subtopic editor: ${error}`
            );
            newError.stack = error.stack;
            throw newError;
        }
            ...
            for (const subtopic of subtopics) {
                ...
                if (subtopicTitle === subtopicName) {
                    ...
                    if (optionsButton) {
                        ...
                        await subtopic.waitForSelector(
                            deleteSubtopicButtonSelector
                        );
                        const deleteButton = await subtopic.$(
                            deleteSubtopicButtonSelector
                        );
                        if (deleteButton) {
    ...
    async expectSubtopicPreviewToHave(
        ...
        const isExplanationPresent =
            await this.isTextPresentOnPage(explanation);
        ...
        try {
            ...
            for (const storyListItem of storyListItems) {
                const storyTitleElement =
                    await storyListItem.$(storyTitleSelector);
            ...
            throw new Error(
                `Story ${storyName} not found in topic ${topicName}.`
            );
        } catch (error) {
            const newError = new Error(
                `Failed to delete story from topic: ${error}`
            );
            newError.stack = error.stack;
            throw newError;
        }
    ...
    async expectNewChapterErrorSpan(errorSpan: string): Promise<void> {
        ...
        const errorSpanElement = await this.page.$(
            newChapterErrorMessageSelector
        );
        ...
        try {
            ...
            for (const titleElement of chapterTitles) {
                ...
                if (title === chapterName) {
                    ...
                    showMessage(
                        `Chapter ${chapterName} opened in chapter editor.`
                    );
                    ...
                    if (this.isViewportAtMobileWidth()) {
                        ...
                        if (elements.length < 5) {
                            throw new Error(
                                'Not enough elements collapsible headers found,'
                            );
                        }
    ...
    async deleteChapterFromStory(
        ...
        try {
            ...
            const storyEditorNodes = await this.page.$$(
                storyEditorNodeSelector
            );

            for (const storyEditorNode of storyEditorNodes) {
                const chapter = await storyEditorNode.$eval(
                ...
                if (chapter === chapterName) {
                    ...
                    if (editOptionsButton) {
                        ...
                        await storyEditorNode.waitForSelector(
            ...
            await this.page.waitForFunction(
                (selector: string, enabled: boolean) => {
                    ...
                    return (
                        ...
                        !(element?.textContent?.trim() === 'Save Changes') ===
                        enabled
                    );
                },
                {},
    ...
    async expectSaveChangesInSkillEditorToBe(
        ...
        if (this.isViewportAtMobileWidth()) {
            if (
                !this.isElementVisible(navigationContainerSelector, true, 5000)
            ) {
                await this.expectElementToBeVisible(mobileOptionsSelector);
                await this.clickOnElementWithSelector(mobileOptionsSelector);
            }
            await this.page.waitForFunction(
                (selector: string, enabled: boolean) => {
                    ...
                    return (
                        ...
                        // there are no changes to be saved and we treat it as disabled.
                        !(
                            element?.textContent?.trim() === 'Discard Changes'
                        ) === enabled
                    );
                },
                {},
    ...
    async expectSkillAssignedToTopic(
        ...
        await this.page.waitForFunction(
            (
                ...
                const elements =
                    document.querySelectorAll(skillElementSelector);
    ...
    async editStoryDetails(
        ...
        await this.expectElementValueToBe(
            storyTitleInStoryEditorSelector,
            title
        );
    ...
    async expandHeaderInMobile(header: 'Prerequisite Skills'): Promise<void> {
        ...
        if (await this.isElementVisible(bodySelector, true, 10000)) {
            showMessage(
                `Skipping test as the ${header} section is already expanded`
            );
            return;
        }
