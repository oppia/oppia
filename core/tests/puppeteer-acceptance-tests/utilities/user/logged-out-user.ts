...
export class LoggedOutUser extends BaseUser {
    ...
    async navigateToDonationThanksModalOnAboutPage(): Promise<void> {
        ...
        await this.page.waitForSelector(thanksForDonatingClass, {
            visible: true,
        });
    }
    ...
    async navigateToCommunityLibraryOnNavbar(): Promise<void> {
        ...
        if (
            (await this.isElementVisible(communityLibraryContainerSelector)) !==
            true
        ) {
    ...
    async expectBlogPostsToHaveAtLeastOneTag(): Promise<void> {
        ...
        const allPostsHaveTags = await this.page.$$eval(
            ...
            (posts, tagSelector) =>
                posts.every(
                    post => post.querySelector(tagSelector as string) !== null
                ),
            blogPostTagSelector
        );
        if (!allPostsHaveTags) {
    ...
    async clickTeachButtonInAboutMenuOnNavbar(): Promise<void> {
        if (this.isViewportAtMobileWidth()) {
            ...
            await this.clickOnElementWithSelector(
                mobileSidebarExpandAboutMenuButton
            );
            await this.clickButtonToNavigateToNewPage(
    ...
    async expectExplorationToBePresentInProfilePageWithTitle(
        ...
        if (explorations.length === 0) {
            throw new Error(
                'There are no explorations authored by the creator.'
            );
        }
    ...
    async dismissDonationThanksModalOnDonatePage(): Promise<void> {
        ...
        const thanksForDonatingHeader = await this.page.$(
            thanksForDonatingClass
        );
    ...
    async expectTestimonailsCarouselToBeFunctionalInTeachPage(): Promise<void> {
        ...
        showMessage(
            'The testimonial carousel in teach page is working correctly.'
        );
    }
    ...
    async expectLessonCreationStepsAccordionToBeFunctionalInTeachPage(): Promise<void> {
        ...
        if (!lessonCreationSection) {
            ...
            showMessage(
                'The lesson creation section is visible on the teach page.'
            );
        }
        await this.clickOnElementWithSelector(
        ...
        showMessage(
            'Lesson Creation accordion expand button is working correctly'
        );
        await this.clickOnElementWithSelector(
        ...
        showMessage(
            'Lesson Creation accordion close button is working correctly'
        );
    }
    ...
    async clickOnCreateAccountButtonInSaveProgressModal(): Promise<void> {
        ...
        await this.expectElementToBeVisible(
            lessonInfoSignUpButtonSelector,
            false
        );
    }
    ...
    async clickReadBlogPostLinkInPartnershipsPage(): Promise<void> {
        ...
        if (this.isViewportAtMobileWidth()) {
            ...
            await this.page.waitForSelector(carouselSlideSelector, {
                visible: true,
            });
    ...
    async expectVolunteerExpectationsTabsToBeFunctionalInVolunteerPage(): Promise<void> {
        ...
        if (!tabsSection) {
            throw new Error(
                'The tabs section is not visible in Volunteer page'
            );
        }
        ...
        if (!this.isViewportAtMobileWidth()) {
            ...
            await this.clickOnElementWithSelector(
                tabsPreviousButtonInVolunteerPage
            );
        }
    ...
    async clickLicenseLinkInTermsPage(): Promise<void> {
        await this.page.waitForSelector(ccLicenseLinkInTermsPage, {
            visible: true,
        });
        await this.clickButtonToNavigateToNewPage(
    ...
    async verifyAdminEmailLinkInContactUsPage(): Promise<void> {
        ...
        if (
            href !==
            'https://mail.google.com/mail/?view=cm&fs=1&to=admin@oppia.org'
        ) {
            throw new Error(
    ...
    async verifyPressEmailLinkInContactUsPage(): Promise<void> {
        ...
        if (
            href !==
            'https://mail.google.com/mail/?view=cm&fs=1&to=press@oppia.org'
        ) {
            throw new Error(
    ...
    async expectToViewFeaturedActivities(
        ...
        for (const expectedActivity of expectedActivityTitles) {
            ...
            showMessage(
                `Activity with title ${expectedActivity} found as expected.`
            );
        }
    }
    ...
    async expectFeaturesAccordionToBeFunctionalInAboutPage(): Promise<void> {
        ...
        if (!featuresSection) {
            throw new Error(
                'The features section is not visible on the About page.'
            );
        }
    ...
    async expectVolunteerCarouselToBeFunctionalInAboutPage(): Promise<void> {
        const volunteerCarouselSelectorInAboutPage =
            !this.isViewportAtMobileWidth()
        ...
        if (!volunteerCarousel) {
            throw new Error(
                'The volunteer carousel in About page is not visible.'
            );
        }
    ...
    async clickVolunteerWithOppiaButtonInAboutPage(): Promise<void> {
        const volunteerWithOppiaButtonInAboutPage =
            this.isViewportAtMobileWidth()
    ...
    async clickVolunteerLearnMoreButtonInAboutPage(): Promise<void> {
        const volunteerLearnMoreButtonInAboutPage =
            this.isViewportAtMobileWidth()
    ...
    async clickDonateButtonOnDonatePageInMobileMode(): Promise<void> {
        if (this.isViewportAtMobileWidth()) {
            try {
                await this.page.waitForSelector(
                    mobileDonateButtonOnDonatePage,
                    {
                        visible: true,
                    }
                );
                const donateButton = await this.page.$(
                    mobileDonateButtonOnDonatePage
                );
    ...
    async expectToBeOnErrorPage(statusCode: number): Promise<void> {
        ...
        if (!errorText) {
            throw new Error(
                `Error text is not visible. URL: ${this.page.url()}`
            );
        }
    ...
    async submitAnswerInTextArea(answer: string): Promise<void> {
        await this.waitForElementToBeClickable(
            submitResponseToInteractionTextArea
        );
    ...
    async expectNewsletterSubscriptionThanksMessage(): Promise<void> {
        ...
        if (
            !thanksMessage ||
            !thanksMessage.includes('Thanks for subscribing!')
        ) {
    ...
    async filterLessonsByCategories(categoryNames: string[]): Promise<void> {
        ...
        const filterOptions = await this.page.$$(
    ...
    async filterLessonsByLanguage(languageNames: string[]): Promise<void> {
        ...
        const selectedElements = await this.page.$$(
        ...
        try {
            ...
            for (const card of cards) {
                ...
                if (titleText === expectedExplorationName) {
                    ...
                    const ratingElement = await card.$(
                        explorationRatingSelector
                    );
                    if (ratingElement) {
                        const ratingSpan =
                            await ratingElement.$('span:nth-child(2)');
        ...
        try {
            ...
            for (const expectedName of expectedTopicNames) {
                if (!topicNameTexts.includes(expectedName.trim())) {
                    throw new Error(
                        `Topic "${expectedName}" not found in topic names.`
                    );
                }
            }
        } catch (error) {
    ...
    async selectAndOpenTopic(topicName: string): Promise<void> {
        try {
            ...
            const newError = new Error(
                `Failed to select and open topic: ${error}`
            );
            newError.stack = error.stack;
            throw newError;
        }
    ...
    async expectToBeInClassroomPage(
        ...
        try {
            ...
            for (const title of storyTitles) {
                ...
                if (titleText.trim() === storyName.trim()) {
                    ...
                    await this.page.waitForSelector(chapterTitleSelector);
                    const chapterTitles =
                        await this.page.$$(chapterTitleSelector);
                    for (const chapter of chapterTitles) {
                        const chapterText = await this.page.evaluate(
                            el => el.textContent.trim(),
                            chapter
                        ...
                        if (chapterText.trim().includes(chapterName.trim())) {
                            ...
                            showMessage(
                                `Chapter ${chapterName} is opened successfully.`
                            );
                            return;
                        }
                    }
     ...
     */
    async navigateToPracticeTabUsingURL(
    ...
    async selectReviewCardToLearn(subtopicName: string): Promise<void> {
        try {
            ...
            const newError = new Error(
                `Failed to select review card: ${error}`
            );
            newError.stack = error.stack;
            throw newError;
        }
     ...
     */
    async expectAttributionInHtmlSectionToBe(
        htmlString: string
    ): Promise<void> {
    ...
    async shareExplorationAndVerifyRedirect(
        ...
        switch (platform) {
            ...
            case 'Twitter':
                expectedUrl =
                    testConstants.SocialsShare.Twitter.Domain + explorationId;
                break;
            case 'Classroom':
                expectedUrl =
     ...
     */
    async expectSignInButtonToBePresent(
        present: boolean = true
    ): Promise<void> {
        await this.waitForStaticAssetsToLoad();
        try {
        ...
        if (!present) {
            throw new Error(
                'Sign-in button is present, expected to be absent.'
            );
        }
    }
    ...
    async openLessonInfoModal(): Promise<void> {
        ...
        await this.page.waitForSelector(lessonInfoCardSelector, {
            visible: true,
        });
    }
    ...
    async expectProgressReminder(shouldBeFound: boolean): Promise<void> {
        ...
        try {
            ...
            if (!shouldBeFound) {
                throw new Error(
                    'Progress remainder is found, which is not expected.'
                );
            }
    ...
    async checkProgressUrlValidityInfo(expectedText: string): Promise<void> {
        await this.page.waitForSelector(validityInfoTextSelector, {
            visible: true,
        });
    ...
    async copyProgressUrl(): Promise<string> {
        try {
            ...
            await this.waitForPageToFullyLoad();
            await this.page.waitForSelector(copyProgressUrlButton, {
                visible: true,
            });
    ...
    async clickOnProfileIconInLessonInfoModel(): Promise<void> {
        ...
        await this.waitForElementToStabilize(
            contributorIconInLessonInfoSelctor
        );
        await this.clickOnElementWithSelector(
            contributorIconInLessonInfoSelctor
        );
     ...
     */
    async expectLessonInfoToShowNoOfViews(
    ...
    async expectLessonInfoToShowLastUpdated(): Promise<void> {
        await this.waitForStaticAssetsToLoad();
        await this.page.waitForSelector(lastUpdatedInfoSelector, {
            timeout: 3000,
        });
    ...
    async expectNoSaveProgressBeforeCheckpointInfo(): Promise<void> {
        try {
            ...
            throw new Error(
                '"Save Progress" button found, which is not expected.'
            );
        } catch (error) {
            if (error instanceof puppeteer.errors.TimeoutError) {
    ...
    async changeLessonLanguage(languageCode: string): Promise<void> {
        await this.page.waitForSelector(lessonLanguageSelector, {
            visible: true,
        });
    ...
    async startVoiceover(): Promise<void> {
        ...
        const isDropdownVisible =
            await this.isElementVisible(voiceoverDropdown);
    ...
    async expectVoiceoverIsSkippable(): Promise<void> {
        ...
        await this.page.waitForFunction(
            (selector: string, value: number) => {
                const element = document.querySelector(selector);
                return (
                    parseInt(element?.textContent?.trim() ?? '', 10) >= value
                );
            },
            {},
            audioSliderSelector,
    ...
    async expectStayAnonymousCheckboxToBePresent(
        ...
        if (status) {
            await this.page.waitForSelector(stayAnonymousCheckbox, {
                visible: true,
            });
            ...
            try {
                ...
                if (error instanceof puppeteer.errors.TimeoutError) {
                    showMessage(
                        'Stay anonymous checkbox is not present, as expected.'
                    );
                } else {
                    throw error;
                }
    ...
    async expectContinueToNextCardButtonToBePresent(
        ...
        if (status) {
            ...
            try {
                ...
                throw new Error(
                    'Continue button is present, but it should not be.'
                );
            } catch (error) {
                if (error instanceof puppeteer.errors.TimeoutError) {
     ...
     */
    async expectLessonInfoTextToBePresent(
        status: boolean = true
    ): Promise<void> {
        if (status) {
            ...
            try {
                ...
                throw new Error(
                    'Lesson info text is present, but it should not be.'
                );
            } catch (error) {
                if (error instanceof puppeteer.errors.TimeoutError) {
                    showMessage(
                        'Lesson info text is not present, as expected.'
                    );
                } else {
                    throw error;
                }
     ...
     */
    async expectVoiceoverBarToBePresent(
    ...
    async expectContactUsPageToContainContentCardsWithHeading(
        ...
        for (let heading of headings) {
            await this.expectContactUsPageToContainContentCardWithHeading(
                heading
            );
        }
    }
    ...
    async verifyLearnerStoriesCarouselInPartnershipPageWorksProperly(): Promise<void> {
        ...
        const newSlideId = await this.page.$eval(
            activeItemSelector,
            el => el.id
        );
     ...
     */
    async expectDevModeLabelToBeVisible(
        visible: boolean = true
    ): Promise<void> {
        try {
     ...
     */
    async clickOnClassroomTileInLearnPage(
        classroomName: string
    ): Promise<void> {
        await this.expectElementToBeVisible(classroomTileContainerSelector);

        const classroomTiles = await this.page.$$(
     ...
     */
    async expectTabTitleInTopicPageToBe(
    ...
    async expectToBeInPracticeSession(): Promise<void> {
        expect(
            await this.isElementVisible(practiceSessionContainerSelector)
        ).toBe(true);
    }
    ...
    async expectBlogPageLayoutToBeCorrect(): Promise<void> {
        ...
        if (blogPosts.length === 0) {
            showMessage(
                'No blog posts found, but layout elements are present.'
            );
        }
        await this.expectElementToBeVisible(blogPostTitleSelector);
    ...
    async expectSuggestedBlogPostsSectionToBeVisible(): Promise<void> {
        ...
        if (suggestedSection) {
            await this.expectElementToBeVisible(
                blogSuggestedForYouHeadingSelector
            );
        }
    }
}
