...
class PopulateStoryNodeJob(base_jobs.JobBase):
    ...
    def _update_story_node(
        self, topic_story_list: Tuple[List[topic_models.TopicModel],
            ...
            try:
                ...
                for story_model in story_model_list:
                    ...
                    story_reference = next(
                        story_ref
                        for story_ref in (topic_model.canonical_story_references)
                    ...
                    for node in nodes:
                        ...
                        for version in range(current_story_version, 0, -1):
                            ...
                            for cmd in story_metadata.commit_cmds:
                                if (
                                    cmd['cmd'] == 'update_story_node_property' and
                        ...
                        node_published_on = story_published_on if (
                            ...
                            story_published_on > node_created_on
                        ) else (node_created_on)
                        node['first_publication_date_msecs'] = (
                            node_published_on if node['status'] == 'Published' else None
                        )
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        topic_story_pairs = ((fetched_topic_models, fetched_story_models) |
        ...
        updated_story_models_job_results = (
            updated_story_models_results | job_result_transforms.
            ResultsToJobRunResults('TOPIC MODELS WHOSE STORIES ARE UPDATED')
        )

        filtered_migrated_stories = (
            updated_story_models_results | 'Filter migration results' >>
            (results_transforms.DrainResultsOnError())
        )

        if self.DATASTORE_UPDATES_ALLOWED:
            unused_put_results = (
                filtered_migrated_stories | 'Unwrap story models lists' >>
