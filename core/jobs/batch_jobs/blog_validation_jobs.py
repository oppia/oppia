...
class FindDuplicateBlogPostTitlesJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[blog_validation_errors.DuplicateBlogTitleError]:
        return (
            self.pipeline | 'Get every Blog Model' >>
...
class FindDuplicateBlogPostUrlsJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[blog_validation_errors.DuplicateBlogUrlError]:
        return (
            self.pipeline | 'Get every Blog Post Model' >>
            (ndb_io.GetModels(blog_models.BlogPostModel.query())) |
...
class FindDuplicateBlogPostSummaryTitlesJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[blog_validation_errors.DuplicateBlogTitleError]:
        return (
            self.pipeline | 'Get every Blog Summary Model' >>
...
class FindDuplicateBlogPostSummaryUrlsJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[blog_validation_errors.DuplicateBlogUrlError]:
        return (
            self.pipeline | 'Get every Blog Post Summary Model' >>
            (ndb_io.GetModels(blog_models.BlogPostSummaryModel.query())) |
            GetModelsWithDuplicatePropertyValues('url_fragment') |
            'Flatten models into a list of errors' >> beam.FlatMap(
                lambda models: [
...
class FindDuplicateBlogAuthorDetailsModelForAuthorJob(base_jobs.JobBase):
    ...
    def run(
        ...
        return (
            self.pipeline | 'Get every Blog Author Details Model' >>
...
class GetModelsWithDuplicatePropertyValues(beam.PTransform):  # type: ignore[misc]
    ...
    def expand(
        self,
        blog_model_pcoll: beam.PCollection[Union[blog_models.BlogPostModel,
                                                 blog_models.BlogPostSummaryModel,
                                                 blog_models.BlogAuthorDetailsModel]]
    ) -> beam.PCollection[
        Union[blog_models.BlogPostModel, blog_models.BlogPostSummaryModel,
              blog_models.BlogAuthorDetailsModel]]:
        return (
            blog_model_pcoll | 'Discard models with empty property value' >>
    ...
    def get_property_value(
        ...
        property_value: Union[str, bool,
                              datetime.datetime] = job_utils.get_model_property(
