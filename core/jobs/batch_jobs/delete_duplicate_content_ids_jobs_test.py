        ...
        self.assert_job_output_is(
            [
                job_run_result.JobRunResult.as_stdout(
                    'Exploration exp_id (version 1) has duplicate content IDs: {\'content_2\': [\'Introduction\', \'State2\']}'
                )
            ]
        )
...
class FixExplorationsWithDuplicateContentIdsJobTests(
    ...
    def test_fix_job_with_duplicates(self) -> None:
        ...
        self.assert_job_output_is(
            [
                job_run_result.JobRunResult.as_stdout(
                    f'Fixed exploration exp_id (version 1) - regenerated content IDs: [\'{original_content_id} -> content_3 in State2\']'
                )
            ]
        )
