...
class IdentifyExplorationsWithDuplicateContentIdsJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        explorations_with_duplicates = (
            self.pipeline
            | 'Get all exploration models' >> ndb_io.GetModels(exp_models.ExplorationModel.get_all(include_deleted=False))
            | 'Transform to exploration domain objects' >> beam.Map(exp_fetchers.get_exploration_from_model)
            | 'Check for duplicate content IDs' >> beam.Map(self._check_for_duplicate_content_ids)
            | 'Filter explorations with duplicates' >> beam.Filter(lambda result: result is not None)
        )

        return explorations_with_duplicates | 'Create job run results' >> beam.Map(lambda result: job_run_result.JobRunResult.as_stdout(f'Exploration {result["exp_id"]} (version {result["version"]}) has duplicate content IDs: {result["duplicates"]}'))
    ...
    def _check_for_duplicate_content_ids(
        ...
        if duplicate_content_ids:
            ...
            for duplicate_id in duplicate_content_ids:
                states_with_duplicate = [state_name for state_name, content_ids in state_to_content_ids.items() if duplicate_id in content_ids]
...
class FixExplorationsWithDuplicateContentIdsJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        fixed_explorations = (
            ...
            | 'Get all exploration models' >> ndb_io.GetModels(exp_models.ExplorationModel.get_all(include_deleted=False))
            | 'Transform to exploration domain objects' >> beam.Map(exp_fetchers.get_exploration_from_model)
            | 'Check for duplicate content IDs' >> beam.Map(self._check_and_fix_duplicate_content_ids)
            | 'Filter fixed explorations' >> beam.Filter(lambda result: result is not None)
        )

        if self.DATASTORE_UPDATES_ALLOWED:
            (fixed_explorations | 'Extract fixed exploration models' >> beam.Map(lambda result: result['fixed_model']) | 'Put fixed models' >> ndb_io.PutModels())

        return fixed_explorations | 'Create job run results' >> beam.Map(lambda result: job_run_result.JobRunResult.as_stdout(f'Fixed exploration {result["exp_id"]} (version {result["version"]}) - regenerated content IDs: {result["fixed_content_ids"]}'))
    ...
    def _check_and_fix_duplicate_content_ids(
        ...
        for duplicate_id in duplicate_content_ids:
            ...
            for state_name in states_with_duplicate[1:]:
                ...
                new_content_id = content_id_generator.generate(translation_domain.ContentType.CONTENT)
...
def _replace_content_id_in_state(state: state_domain.State, old_content_id: str, new_content_id: str) -> None:
    ...
    if state.interaction:
        ...
        if state.interaction.default_outcome and hasattr(state.interaction.default_outcome, 'feedback') and hasattr(state.interaction.default_outcome.feedback, 'content_id'):
        ...
        if state.interaction.solution and hasattr(state.interaction.solution, 'explanation') and hasattr(state.interaction.solution.explanation, 'content_id'):
...
def _replace_content_id_in_value(value: Any, old_content_id: str, new_content_id: str) -> None:
...
class AuditFixExplorationsWithDuplicateContentIdsJob(FixExplorationsWithDuplicateContentIdsJob):
