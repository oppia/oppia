        ...
        return (
            ...
            >> beam.Map(
                lambda result: job_run_result.JobRunResult.as_stdout(
                    f'Exploration {result["exp_id"]} (version {result["version"]}) has duplicate content IDs: {result["duplicates"]}'
                )
            )
        )
...
class FixExplorationsWithDuplicateContentIdsJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        return fixed_explorations | 'Create job run results' >> beam.Map(
            lambda result: job_run_result.JobRunResult.as_stdout(
                f'Fixed exploration {result["exp_id"]} (version {result["version"]}) - regenerated content IDs: {result["fixed_content_ids"]}'
            )
        )
