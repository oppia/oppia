...
class VoiceArtistMetadataModelsTestsBaseClass(
    ...
    def _create_curated_explorations(self) -> None:
        ...
        new_voiceovers_dict = {
            'voiceovers_mapping': {
                'content_0': {'en': self.voiceover_dict_5, 'hi': self.voiceover_dict_6},
                'ca_placeholder_2': {
    ...
    def _create_exp_voice_artists_link(self) -> None:
        ...
        exp_voice_artist_link_model_1 = (
            voiceover_services.create_exploration_voice_artists_link_model_instance(
                self.CURATED_EXPLORATION_ID_1, content_id_to_voiceovers_mapping_1
            )
        )
        ...
        exp_voice_artist_link_model_2 = (
            voiceover_services.create_exploration_voice_artists_link_model_instance(
                self.CURATED_EXPLORATION_ID_2, content_id_to_voiceovers_mapping_2
            )
        )
...
class CreateExplorationVoiceArtistLinkModelsJobTests(
    ...
    JOB_CLASS: Type[
        voiceover_migration_job.PopulateManualVoiceoversToEntityVoiceoversModelJob
    ] = voiceover_migration_job.PopulateManualVoiceoversToEntityVoiceoversModelJob
    ...
    def test_version_is_added_after_running_job(self) -> None:
        ...
        job_result_template = (
            'Migrated %s voiceovers for exploration: %s, in language ' 'accent code %s.'
        )
        ...
        self.assert_job_output_is(
            [
                job_run_result.JobRunResult(
                    ...
                    % (str(2), self.CURATED_EXPLORATION_ID_1, 'en-US'),
                    stderr='',
                ),
                job_run_result.JobRunResult(
                    stdout=job_result_template
                    % (str(1), self.CURATED_EXPLORATION_ID_1, 'hi-IN'),
                    stderr='',
                ),
                job_run_result.JobRunResult(
                    stdout=job_result_template
                    % (str(2), self.CURATED_EXPLORATION_ID_2, 'en-US'),
                    stderr='',
                ),
                job_run_result.JobRunResult(
                    stdout=job_result_template
                    % (str(2), self.CURATED_EXPLORATION_ID_2, 'hi-IN'),
                    stderr='',
                ),
            ]
        )

        entity_voiceovers_models: Sequence[voiceover_models.EntityVoiceoversModel] = (
        ...
        expected_entity_voiceover_id_to_model = {
            'exploration-exploration_id_1-5-en-US': {
                'content_0': {'manual': self.voiceover_dict_1, 'auto': None},
                'ca_placeholder_2': {'manual': self.voiceover_dict_3, 'auto': None},
            },
            'exploration-exploration_id_1-5-hi-IN': {
                'content_0': {'manual': self.voiceover_dict_2, 'auto': None}
            },
            'exploration-exploration_id_2-4-en-US': {
                'content_0': {'manual': self.voiceover_dict_5, 'auto': None},
                'ca_placeholder_2': {'manual': self.voiceover_dict_4, 'auto': None},
            },
            'exploration-exploration_id_2-4-hi-IN': {
                'content_0': {'manual': self.voiceover_dict_6, 'auto': None},
                'ca_placeholder_2': {'manual': self.voiceover_dict_7, 'auto': None},
            },
        }
    ...
    def test_should_raise_exception_for_empty_accent_code(self) -> None:
        ...
        error_message = (
            'Please assign all the accents for voice artists in ' 'language code hi.'
        )
...
class AuditExplorationVoiceArtistLinkModelsJobTests(
    ...
    JOB_CLASS: Type[voiceover_migration_job.AuditEntityVoiceoverModelJob] = (
