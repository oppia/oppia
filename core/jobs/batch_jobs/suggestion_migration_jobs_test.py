...
class MigrateSuggestionJobTests(job_test_utils.JobTestBase):
    ...
    def test_unmigrated_suggestion_is_migrated(self) -> None:
        ...
        unmigrated_suggestion_model = suggestion_models.GeneralSuggestionModel.get(
            suggestion_1_model.id
        )
        self.assertEqual(
        ...
        migrated_suggestion_model = suggestion_models.GeneralSuggestionModel.get(
            suggestion_1_model.id
        )
        self.assertEqual(
...
class MigrateQuestionSuggestionsJobTests(
    ...
    def test_unmigrated_question_suggestion_is_migrated(self) -> None:
        skill_id = skill_services.get_new_skill_id()
        self.save_new_skill(skill_id, self.author_id, description='description')
        suggestion_id = self.save_new_question_suggestion_with_state_data_schema_v27(
