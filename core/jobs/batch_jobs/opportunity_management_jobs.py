...
(
    exp_models, opportunity_models, question_models, skill_models, story_models,
    topic_models
...
class DeleteSkillOpportunityModelJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        skill_opportunity_model = (
            self.pipeline | 'Get all non-deleted skill models' >> ndb_io.GetModels(
                opportunity_models.SkillOpportunityModel.get_all(include_deleted=False)
            )
        )

        unused_delete_result = (
            skill_opportunity_model | beam.Map(lambda model: model.key) |
            'Delete all models' >> ndb_io.DeleteModels()
        )

        return (
            skill_opportunity_model | 'Create job run result' >>
...
class GenerateSkillOpportunityModelJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        question_skill_link_models = (
            self.pipeline | 'Get all non-deleted QuestionSkillLinkModels' >> (
                ndb_io.GetModels(
        ...
        skills = (
            self.pipeline | 'Get all non-deleted SkillModels' >>
            (ndb_io.GetModels(skill_models.SkillModel.get_all(include_deleted=False))) |
        ...
        skills_with_question_counts = (
            ...
            | 'Flatten skill and question_skill_links' >> beam.Map(
                lambda skill_and_question_skill_links_object: {
                    'skill':
                        list(skill_and_question_skill_links_object['skill'][0])[0],
        ...
        opportunities_results = (
            skills_with_question_counts | beam.Map(
                lambda skills_with_question_counts_object: self.
                _create_skill_opportunity_model(
        ...
        unused_put_result = (
            opportunities_results | 'Filter the results with OK status' >>
            beam.Filter(lambda result: result.is_ok()) |
            'Fetch the models to be put' >> beam.Map(lambda result: result.unwrap()) |
...
class DeleteExplorationOpportunitySummariesJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        unused_delete_result = (
            exp_opportunity_summary_model | beam.Map(lambda model: model.key) |
            'Delete all models' >> ndb_io.DeleteModels()
        )

        return (
            exp_opportunity_summary_model | 'Create job run result' >>
            (job_result_transforms.CountObjectsToJobRunResult())
        )
...
class GenerateExplorationOpportunitySummariesJob(base_jobs.JobBase):
    ...
    def _generate_opportunities_related_to_topic(
        topic: topic_domain.Topic, stories_dict: Dict[str, story_domain.Story],
        exps_dict: Dict[str, exp_domain.Exploration]
    ) -> result.Result[List[opportunity_models.ExplorationOpportunitySummaryModel],
                       Exception]:
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        topics = (
            self.pipeline | 'Get all non-deleted topic models' >>
            (ndb_io.GetModels(topic_models.TopicModel.get_all(include_deleted=False))) |
            'Get topic from model' >> beam.Map(topic_fetchers.get_topic_from_model)
        )

        story_ids_to_story = (
            self.pipeline | 'Get all non-deleted story models' >>
            ndb_io.GetModels(story_models.StoryModel.get_all(include_deleted=False)) |
            'Get story from model' >> beam.Map(story_fetchers.get_story_from_model) |
            'Combine stories and ids' >> beam.Map(lambda story: (story.id, story))
        )

        exp_ids_to_exp = (
            self.pipeline | 'Get all non-deleted exp models' >> ndb_io.GetModels(
        ...
        unused_put_result = (
            opportunities_results | 'Filter the results with SUCCESS status' >>
            beam.Filter(lambda result: result.is_ok()) | 'Fetch the models to be put' >>
        ...
        return (
            opportunities_results | 'Count the output' >>
            (job_result_transforms.ResultsToJobRunResults())
        )
