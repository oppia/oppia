...
class MigrateStoryModels(beam.PTransform):  # type: ignore[misc]
    ...
    def expand(self, pipeline: beam.Pipeline) -> Tuple[
        ...
        migrated_stories = filtered_migrated_stories | 'Unwrap ok' >> beam.Map(
            lambda result_item: result_item.unwrap()
        )

        story_changes = (
            unmigrated_story_models
            | 'Generate story changes'
            >> beam.FlatMapTuple(self._generate_story_changes)
        )
...
class MigrateStoryJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        transformed_story_objects_list, job_run_results = (
            self.pipeline
            | 'Perform migration and filter migration results' >> (MigrateStoryModels())
        )
