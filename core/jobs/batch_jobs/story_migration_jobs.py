...
class MigrateStoryModels(beam.PTransform):  # type: ignore[misc]
    ...
    def expand(
        ...
        unmigrated_story_models = (
            pipeline
            | 'Get all non-deleted story models' >> (ndb_io.GetModels(story_models.StoryModel.get_all()))
        ...
        story_summary_models = (
            pipeline
            | 'Get all non-deleted story summary models' >> (ndb_io.GetModels(story_models.StorySummaryModel.get_all()))
            # Pylint disable is needed because pylint is not able to correctly
            # detect that the value is passed through the pipe.
            | 'Add story summary keys'
            >> beam.WithKeys(  # pylint: disable=no-value-for-parameter
        ...
        topics = (
            ...
            | 'Transform model into domain object' >> beam.Map(topic_fetchers.get_topic_from_model)
            # Pylint disable is needed because pylint is not able to correctly
            # detect that the value is passed through the pipe.
            | 'Add topic keys'
            >> beam.WithKeys(  # pylint: disable=no-value-for-parameter
        ...
        transformed_story_objects_list = (
            story_objects_list
            | 'Remove unmigrated stories' >> beam.Filter(lambda x: len(x['story_change']) > 0 and len(x['story']) > 0)
            | 'Reorganize the story objects'
            >> beam.Map(
                lambda objects: {
...
class MigrateStoryJob(base_jobs.JobBase):
    ...
    def _update_story(
        ...
        with datastore_services.get_ndb_context():
            models_to_put = updated_story_model.compute_models_to_commit(
                ...
                'Update story contents schema version to %d.' % (feconf.CURRENT_STORY_CONTENTS_SCHEMA_VERSION),
                change_dicts,
                additional_models={},
            )
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        story_models_to_put = transformed_story_objects_list | 'Generate story models to put' >> beam.FlatMap(
        ...
        ((story_models_to_put, story_summary_models_to_put) | 'Merge models' >> beam.Flatten() | 'Put models into the datastore' >> ndb_io.PutModels())
