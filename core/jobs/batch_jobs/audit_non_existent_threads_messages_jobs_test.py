        ...
        self.assert_job_output_is(
            [
                job_run_result.JobRunResult.as_stdout(
                    (
                        f'GeneralFeedbackMessageModel with non-existent thread: id={message.id}, thread_id={message.thread_id}, message_id={message.message_id}'
                    )
                ),
                job_run_result.JobRunResult.as_stdout(
    ...
    def test_mixed_valid_and_invalid_messages(self) -> None:
        ...
        self.assert_job_output_is(
            [
                job_run_result.JobRunResult.as_stdout(
                    (
                        f'GeneralFeedbackMessageModel with non-existent thread: id={invalid_message.id}, thread_id={invalid_message.thread_id}, message_id={invalid_message.message_id}'
                    )
                ),
                job_run_result.JobRunResult.as_stdout(
                    'invalid_feedback_message_models_count: 1'
    ...
    def test_cleanup_removes_invalid_messages_and_user_models(self) -> None:
        ...
        self.assert_job_output_is(
            [
                job_run_result.JobRunResult.as_stdout(
                    (
                        f'Deleted GeneralFeedbackMessageModel: id={invalid_message.id}, thread_id={invalid_message.thread_id}, message_id={invalid_message.message_id}'
                    )
                ),
                job_run_result.JobRunResult.as_stdout(
                    (
                        f'Deleted GeneralFeedbackThreadUserModel: id={invalid_thread_user.id}, thread_id={invalid_thread_user.thread_id}, user_id={invalid_thread_user.user_id} '
                    )
                ),
                job_run_result.JobRunResult.as_stdout(
