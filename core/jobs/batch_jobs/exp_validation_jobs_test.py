...
class GetNumberOfInvalidExplorationsJobTests(job_test_utils.JobTestBase):
    ...
    def setUp(self):
        ...
        # Exploration model with invalid state classifier model id.
        self.exp_6 = self.create_model(
            ...
            states={
                feconf.DEFAULT_INIT_STATE_NAME: self.STATE_2,
                'second state': self.STATE_3
            }
        )

        # Exploration model with invalid param changes.
        ...
        # Exploration model with invalid param specs.
        ...
        # Exploration model with invalid training data.
        self.exp_9 = self.create_model(
            exp_models.ExplorationModel,
            id=self.EXPLORATION_ID_9,
            title='title 9',
            init_state_name=feconf.DEFAULT_INIT_STATE_NAME,
            category=feconf.DEFAULT_EXPLORATION_CATEGORY,
            objective=feconf.DEFAULT_EXPLORATION_OBJECTIVE,
        ...
        # Valid opportunity model.
        self.opportunity_4 = self.create_model(
            opportunity_models.ExplorationOpportunitySummaryModel,
            id=self.EXPLORATION_ID_6,
            topic_id='topic_id6',
            topic_name='a_topic name',
        ...
        # Valid opportunity model.
        self.opportunity_5 = self.create_model(
            opportunity_models.ExplorationOpportunitySummaryModel,
            id=self.EXPLORATION_ID_7,
            topic_id='topic_id7',
            topic_name='a_topic name',
        ...
        # Valid opportunity model.
        self.opportunity_6 = self.create_model(
            opportunity_models.ExplorationOpportunitySummaryModel,
            id=self.EXPLORATION_ID_8,
            topic_id='topic_id8',
            topic_name='a_topic name',
        ...
        # Valid opportunity model.
        self.opportunity_7 = self.create_model(
            opportunity_models.ExplorationOpportunitySummaryModel,
            id=self.EXPLORATION_ID_9,
            topic_id='topic_id9',
            topic_name='a_topic name',
    ...
    def test_run_with_invalid_state_classifier_id(self) -> None:
        self.put_multi([self.exp_6, self.opportunity_4])
        self.assert_job_output_is([
            job_run_result.JobRunResult.as_stdout('EXPS SUCCESS: 1'),
            job_run_result.JobRunResult.as_stdout('CURATED EXPS SUCCESS: 1'),
            job_run_result.JobRunResult.as_stdout(
            ...
            job_run_result.JobRunResult.as_stderr(
                'The id of exp is %s and the states having not None '
                'classifier model id are %s' % (
                    self.EXPLORATION_ID_6,
                    [feconf.DEFAULT_INIT_STATE_NAME, 'second state'])
            )
        ])

    def test_run_with_invalid_param_changes(self) -> None:
        self.put_multi([self.exp_7, self.opportunity_5])
        self.assert_job_output_is([
            job_run_result.JobRunResult.as_stdout('EXPS SUCCESS: 1'),
            job_run_result.JobRunResult.as_stdout('CURATED EXPS SUCCESS: 1'),
            job_run_result.JobRunResult.as_stdout(
            ...
            job_run_result.JobRunResult.as_stderr(
                'The id of exp is %s and the length of '
                'its param changes is %s' % (
                    self.EXPLORATION_ID_7, len(self.PARAM_CHANGES_1)
                )
            )
        ])

    def test_run_with_invalid_param_specs(self) -> None:
        self.put_multi([self.exp_8, self.opportunity_6])
        self.assert_job_output_is([
            job_run_result.JobRunResult.as_stdout('EXPS SUCCESS: 1'),
            job_run_result.JobRunResult.as_stdout('CURATED EXPS SUCCESS: 1'),
            job_run_result.JobRunResult.as_stdout(
            ...
            job_run_result.JobRunResult.as_stderr(
                'The id of exp is %s and the length of '
                'its param specs is %s' % (self.EXPLORATION_ID_8, 2)
            )
        ])

    def test_run_with_invalid_training_data(self) -> None:
        self.put_multi([self.exp_9, self.opportunity_7])
        self.assert_job_output_is([
            job_run_result.JobRunResult.as_stdout('EXPS SUCCESS: 1'),
            job_run_result.JobRunResult.as_stdout('CURATED EXPS SUCCESS: 1'),
            job_run_result.JobRunResult.as_stdout(
    ...
    def test_run_with_non_curated_explorations(self) -> None:
