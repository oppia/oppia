...
class CreateExplorationVoiceArtistLinkModelsJob(base_jobs.JobBase):
    ...
    def extract_added_voiceovers_between_successive_snapshots(
        ...
        for state in new_snapshot_model.content['states'].values():
            voiceovers_mapping = state['recorded_voiceovers'][
                'voiceovers_mapping'
            ]
    ...
    def get_exploration_voice_artists_link_model(
        ...
        for state in exploration_model.states.values():
            ...
            ) in voiceovers_mapping.items():
                for (
                    lang_code,
                    voiceover_dict,
                ) in lang_code_to_voiceovers.items():
        ...
        for (
            language_code,
            voiceovers_count,
        ) in language_code_to_voiceovers_count.items():
            debug_logs += 'Language code: %s, voiceovers count: %s.\n' % (
                language_code,
                voiceovers_count,
            )
        ...
        for version in range(current_version, 0, -1):
            ...
            debug_logs += 'Iteration for snapshots: %s and %s\n' % (
                old_snapshot_id,
                new_snapshot_id,
            )
            logging.info(
            ...
            voice_artist_id = metadata_models_dict[
            ...
            try:
                with datastore_services.get_ndb_context():
                    voice_artist_username = user_services.get_username(
                        voice_artist_id
                    )
            except Exception as e:
                logging.info(e)
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        voice_artist_link_models_and_logs = (
            ...
            | 'Filter None objects'
            >> beam.Filter(lambda model: model[0] is not None)
        )
