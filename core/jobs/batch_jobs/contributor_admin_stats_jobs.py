...
class GenerateContributorAdminStatsJob(base_jobs.JobBase):
    ...
    def transform_translation_contribution_stats(
        ...
        try:
            ...
            return result.Err(
                'Unable to create total translation contribution stats for contributor id(%s) and language code(%s): %s'
    ...
    def transform_question_contribution_stats(
        ...
        try:
            ...
            return result.Err(
                'Unable to create total question contribution stats for contributor id(%s): %s'
                % (contributor_user_id, e)
            )
...
class ValidateTotalContributionStatsJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        translation_groups_with_no_total_count_results = (
            ...
            >> beam.Map(
                lambda count: job_run_result.JobRunResult.as_stdout(
                    f'Missing Total Translation Submitter Models FAILED: {count}'
                )
            )
        )
        ...
        error_translation_validaition_count_results = (
            ...
            >> beam.Map(
                lambda count: job_run_result.JobRunResult.as_stdout(
                    f'Invalid Total Translation Submitter Models FAILED: {count}'
                )
            )
        )
