...
(opportunity_models, suggestion_models, topic_models) = models.Registry.import_models([models.Names.OPPORTUNITY, models.Names.SUGGESTION, models.Names.TOPIC])
...
class GenerateContributorAdminStatsJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        translation_suggestions = general_suggestions_models | 'Filter translation suggestions' >> beam.Filter(lambda m: (m.suggestion_type == feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT)) | 'Group submitted translation suggestions by target' >> (beam.GroupBy(lambda m: m.target_id))

        exp_opportunities = (
            self.pipeline
            | 'Get all non-deleted exp opportunity models' >> ndb_io.GetModels(opportunity_models.ExplorationOpportunitySummaryModel.get_all(include_deleted=False))
            | 'Transform to exp opportunity domain object' >> beam.Map(opportunity_services.get_exploration_opportunity_summary_from_model)
        ...
        shortlisted_translation_suggestions = (
            exp_opportunity_to_submitted_suggestions
            | 'Filter valid translation suggestions' >> beam.Filter(lambda grouped_data: (len(grouped_data['opportunity']) > 0 and len(grouped_data['suggestion']) > 0))
            | 'Extract translation suggestions only' >> beam.Map(lambda grouped_data: grouped_data['suggestion'])
            | 'Extract and fully flatten t suggestions' >> beam.FlatMap(lambda grouped_data: list(grouped_data[0]))
        )

        translation_general_suggestions_stats = shortlisted_translation_suggestions | 'Group by language and user' >> beam.Map(lambda stats: ((stats.language_code, stats.author_id), stats))

        question_suggestions = general_suggestions_models | 'Filter question suggestions' >> beam.Filter(lambda m: (m.suggestion_type == feconf.SUGGESTION_TYPE_ADD_QUESTION)) | 'Group submitted question suggestions by target' >> (beam.GroupBy(lambda m: m.target_id))
        ...
        shortlisted_question_suggestions = (
            skill_opportunity_to_submitted_suggestions
            | 'Filter valid question suggestions' >> beam.Filter(lambda grouped_data: (len(grouped_data['opportunity']) > 0 and len(grouped_data['suggestion']) > 0))
            | 'Extract question suggestions only' >> beam.Map(lambda grouped_data: grouped_data['suggestion'])
            | 'Extract and fully flatten suggestions' >> beam.FlatMap(lambda grouped_data: list(grouped_data[0]))
        )

        question_general_suggestions_stats = shortlisted_question_suggestions | 'Group by user' >> beam.Map(lambda stats: (stats.author_id, stats))

        translation_contribution_stats = (
            self.pipeline
            | 'Get all non-deleted TranslationContributionStatsModel models' >> ndb_io.GetModels(suggestion_models.TranslationContributionStatsModel.get_all(include_deleted=False))
            | 'Filter translation contribution with no topic' >> beam.Filter(lambda m: m.topic_id != '')
        ...
        translation_reviewer_stats = (
            self.pipeline
            | 'Get all non-deleted TranslationReviewStatsModel models' >> ndb_io.GetModels(suggestion_models.TranslationReviewStatsModel.get_all(include_deleted=False))
        ...
        question_contribution_stats = (
            self.pipeline | 'Get all non-deleted QuestionContributionStatsModel models' >> ndb_io.GetModels(suggestion_models.QuestionContributionStatsModel.get_all(include_deleted=False)) | 'Group QuestionContributionStatsModel by contributor' >> beam.Map(lambda stats: (stats.contributor_user_id, stats))
        )
        ...
        translation_submitter_total_stats_model_results = (
            ...
            | 'Filter valid translation cases' >> beam.Filter(lambda grouped_data: (len(grouped_data[1]['translation_contribution_stats']) > 0))
            ...
            | 'Filter total translation contribution stats' >> beam.Filter(lambda res: res is not None)
        )

        translation_submitter_total_stats_models = translation_submitter_total_stats_model_results | 'Filter translation contribution ok results' >> beam.Filter(lambda res: res.is_ok()) | 'Unpack translation contribution result' >> beam.Map(lambda res: res.unwrap())

        translation_submitter_error_job_run_results = translation_submitter_total_stats_model_results | 'Filter translation contribution err results' >> beam.Filter(lambda res: res.is_err()) | 'Transform translation contribution error to job run result' >> (job_result_transforms.ResultsToJobRunResults())
        ...
        question_submitter_total_stats_model_results = (
            ...
            | 'Filter valid question cases' >> beam.Filter(lambda grouped_data: (len(grouped_data[1]['question_contribution_stats']) > 0))
            ...
            | 'Filter total question contribution stats' >> beam.Filter(lambda res: res is not None)
        )

        question_submitter_total_stats_models = question_submitter_total_stats_model_results | 'Filter question contribution ok result' >> beam.Filter(lambda res: res.is_ok()) | 'Unpack question contribution result' >> beam.Map(lambda res: res.unwrap())

        question_submitter_error_job_run_results = question_submitter_total_stats_model_results | 'Filter question contribution err results' >> beam.Filter(lambda res: res.is_err()) | 'Transform question contribution error to job run result' >> (job_result_transforms.ResultsToJobRunResults())
        ...
        if self.DATASTORE_UPDATES_ALLOWED:
            (translation_submitter_total_stats_models | 'Put TranslationSubmitterTotalContributionStatsModel models' >> ndb_io.PutModels())

            (translation_reviewer_total_stats_models | 'Put TranslationReviewerTotalContributionStatsModel models' >> ndb_io.PutModels())

            (question_submitter_total_stats_models | 'Put QuestionSubmitterTotalContributionStatsModel models' >> ndb_io.PutModels())

            (question_reviewer_total_stats_models | 'Put QuestionReviewerTotalContributionStatsModel models' >> ndb_io.PutModels())
    ...
    def transform_translation_contribution_stats(
        keys: Tuple[str, str],
        translation_contribution_stats: Iterable[suggestion_models.TranslationContributionStatsModel],
        translation_general_suggestions_stats: Iterable[suggestion_models.GeneralSuggestionModel],
    ) -> Optional[
        result.Result[
        ...
        try:
            ...
            return result.Err('Unable to create total translation contribution stats for contributor id(%s) and language code(%s): %s' % (contributor_user_id, language_code, e))

    @staticmethod
    def transform_translation_review_stats(
        keys: Tuple[str, str],
        translation_reviewer_stats: Iterable[suggestion_models.TranslationReviewStatsModel],
        ...
        with datastore_services.get_ndb_context():
            translation_review_stats_models = suggestion_models.TranslationReviewerTotalContributionStatsModel(  # pylint: disable=line-too-long
                ...
                accepted_translations_count=accepted_translations_count,
                accepted_translations_with_reviewer_edits_count=(accepted_translations_with_reviewer_edits_count),
                accepted_translation_word_count=(accepted_translation_word_count),
                rejected_translations_count=rejected_translations_count,
                first_contribution_date=first_contribution_date,
                last_contribution_date=last_contribution_date,
    ...
    def transform_question_contribution_stats(
        ...
        question_contribution_stats: Iterable[suggestion_models.QuestionContributionStatsModel],
        question_general_suggestions_stats: Iterable[suggestion_models.GeneralSuggestionModel],
    ) -> Optional[result.Result[suggestion_models.QuestionSubmitterTotalContributionStatsModel, str]]:
        ...
        try:
            ...
            with datastore_services.get_ndb_context():
                question_submit_stats_models = suggestion_models.QuestionSubmitterTotalContributionStatsModel(  # pylint: disable=line-too-long
                    ...
                    accepted_questions_without_reviewer_edits_count=(accepted_questions_without_reviewer_edits_count),
            ...
            return result.Err('Unable to create total question contribution stats for contributor id(%s): %s' % (contributor_user_id, e))
    ...
    def transform_question_review_stats(
        ...
        with datastore_services.get_ndb_context():
            question_review_stats_models = suggestion_models.QuestionReviewerTotalContributionStatsModel(  # pylint: disable=line-too-long
                ...
                accepted_questions_count=accepted_questions_count,
                accepted_questions_with_reviewer_edits_count=(accepted_questions_with_reviewer_edits_count),
                rejected_questions_count=rejected_questions_count,
                first_contribution_date=first_contribution_date,
                last_contribution_date=last_contribution_date,
            )
...
class AuditAndLogIncorretDataInContributorAdminStatsJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        translation_suggestion_counts_and_logs = (
            ...
            | 'Filter out None values from translation suggestion' >> beam.Filter(lambda x: x is not None)
        )

        translation_suggestion_count_result = (
            translation_suggestion_counts_and_logs
            | 'Unpack translation suggestion counts' >> beam.Map(lambda element: element[0])
            | 'Total translation suggestion count' >> beam.CombineGlobally(sum)
            | 'Report translation suggestion count' >> beam.Map(lambda result: (job_run_result.JobRunResult.as_stdout(f'LOGGED TRANSLATION SUGGESTION COUNT SUCCESS: {result}')))
        )

        translation_suggestion_logs = translation_suggestion_counts_and_logs | 'Unpack translation suggestion logs' >> beam.Map(lambda element: (job_run_result.JobRunResult.as_stdout(element[1])))
        ...
        question_suggestion_counts_and_logs = (
            ...
            | 'Filter out None values from question suggestion' >> beam.Filter(lambda x: x is not None)
        )

        question_suggestion_count_result = (
            question_suggestion_counts_and_logs
            | 'Unpack question suggestion counts' >> beam.Map(lambda element: element[0])
            | 'Total question suggestion count' >> beam.CombineGlobally(sum)
            | 'Report question suggestion count' >> beam.Map(lambda result: (job_run_result.JobRunResult.as_stdout(f'LOGGED QUESTION SUGGESTION COUNT SUCCESS: {result}')))
        )

        question_suggestion_logs = question_suggestion_counts_and_logs | 'Unpack question suggestion logs' >> beam.Map(lambda element: (job_run_result.JobRunResult.as_stdout(element[1])))
    ...
    def log_translation_contribution(
        ...
        with datastore_services.get_ndb_context():
            for s in general_suggestion_models:
                story_id = exp_services.get_story_id_linked_to_exploration(s.target_id)
                if story_id is None:
                    ...
                    opportunity_model_exists = opportunity_models.ExplorationOpportunitySummaryModel.get_by_id(s.target_id) is not (None)
                    debug_logs += f'exp_opportunity_model_exists: {opportunity_model_exists},\n}},\n'
                else:
                    ...
                    if story is None:
                        ...
                        if topic_id not in (valid_topic_ids_with_contribution_stats):
    ...
    def log_question_contribution(
        ...
        with datastore_services.get_ndb_context():
            for s in general_suggestion_stats:
                topic_assignments = list(skill_services.get_all_topic_assignments_for_skill(s.target_id))
                for t in topic_assignments:
                    if t.topic_id not in (valid_topic_ids_with_contribution_stats):
                        # Valid stats model does not exists.
                        logged_suggestions_count += 1
                        debug_logs += (
                            '{\n'
                        ...
                        # Check if xploration opportunity model exists.
                        opportunity_model_exists = opportunity_models.SkillOpportunityModel.get_by_id(s.target_id) is not None

                        debug_logs += f'skill_opportunity_model_exists: {opportunity_model_exists},\n}},\n'
...
class ValidateTotalContributionStatsJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        # Fetch all the data.
        general_suggestions_models = self.pipeline | 'Get General Suggestions' >> ndb_io.GetModels(suggestion_models.GeneralSuggestionModel.get_all(include_deleted=False))

        translation_totals = self.pipeline | 'Get Translation Totals' >> ndb_io.GetModels(suggestion_models.TranslationSubmitterTotalContributionStatsModel.get_all(include_deleted=False))

        translation_contribs = self.pipeline | 'Get Translation Contributions' >> ndb_io.GetModels(suggestion_models.TranslationContributionStatsModel.get_all(include_deleted=False))

        all_translation_suggestions_grouped_by_topic_id = general_suggestions_models | 'Filter translation suggestions' >> beam.Filter(lambda m: (m.suggestion_type == feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT)) | 'Group submitted translation suggestions by target' >> (beam.GroupBy(lambda m: m.target_id))
        ...
        question_totals = self.pipeline | 'Get Question Totals' >> ndb_io.GetModels(suggestion_models.QuestionSubmitterTotalContributionStatsModel.get_all(include_deleted=False))

        question_contribs = self.pipeline | 'Get Question Contributions' >> ndb_io.GetModels(suggestion_models.QuestionContributionStatsModel.get_all(include_deleted=False))

        all_question_suggestions_grouped_by_skill_id = general_suggestions_models | 'Filter question suggestions' >> beam.Filter(lambda m: (m.suggestion_type == feconf.SUGGESTION_TYPE_ADD_QUESTION)) | 'Group submitted question suggestions by target' >> (beam.GroupBy(lambda m: m.target_id))
        ...
        topics_set = self.pipeline | 'Get Topics' >> ndb_io.GetModels(topic_models.TopicModel.get_all(include_deleted=False)) | 'Topic IDs to Set' >> beam.Map(lambda t: t.id) | 'Combine Topic IDs' >> beam.combiners.ToSet()

        # Key datasets for joins.
        trans_totals_kv = translation_totals | beam.Map(lambda t: ((t.contributor_id, t.language_code), t))
        trans_contribs_kv = translation_contribs | beam.Map(lambda c: ((c.contributor_user_id, c.language_code), c))
        trans_suggestions_kv = translation_suggestions | beam.Map(lambda s: ((s.author_id, s.language_code), s))

        quest_totals_kv = question_totals | beam.Map(lambda t: (t.contributor_id, t))
        quest_contribs_kv = question_contribs | beam.Map(lambda c: (c.contributor_user_id, c))
        quest_suggestions_kv = question_suggestions | beam.Map(lambda s: (s.author_id, s))
        ...
        filtered_translation_groups = (
            ...
            >> beam.Map(
                self._make_check_missing_total_fn(
                    total_model_name=('TranslationSubmitterTotalContributionStatsModel'),
                    contrib_model_name='TranslationContributionStatsModel',
                    suggestion_model_name='Translation GeneralSuggestionModel',
                )
            )
            | 'Filter non-none translation groups' >> beam.Filter(lambda r: r is not None)
        )

        valid_translation_groups = filtered_translation_groups | 'Filter translation Ok' >> beam.Filter(lambda r: r.is_ok()) | 'Unwrap translation Ok (kv)' >> beam.Map(lambda r: r.unwrap())

        translation_validaion_results = valid_translation_groups | 'Validate Translation Stats' >> beam.Map(
        ...
        filtered_question_groups = (
            ...
            >> beam.Map(
                self._make_check_missing_total_fn(
                    total_model_name=('QuestionSubmitterTotalContributionStatsModel'),
                    contrib_model_name='QuestionContributionStatsModel',
                    suggestion_model_name='Question GeneralSuggestionModel',
                )
            )
            | 'Filter non-none question groups' >> beam.Filter(lambda r: r is not None)
        )

        valid_question_groups = filtered_question_groups | 'Filter questions Ok' >> beam.Filter(lambda r: r.is_ok()) | 'Unwrap questions Ok (kv)' >> beam.Map(lambda r: r.unwrap())

        question_validaion_results = valid_question_groups | 'Validate Question Stats' >> beam.Map(
        ...
        success_translation_validaition_count_results = (
            translation_validaion_results | 'Filter OK translations' >> beam.Filter(lambda res: res.is_ok()) | 'Unwrap OK translations' >> beam.Map(lambda res: res.unwrap()) | 'Count OK translations' >> (job_result_transforms.CountObjectsToJobRunResult('Valid Translation Submitter Models'))
        )

        translation_groups_with_no_total = filtered_translation_groups | 'Filter translation Err' >> beam.Filter(lambda r: r.is_err())

        translation_groups_with_no_total_count_results = (
            translation_groups_with_no_total
            | 'Count missing total translations' >> (beam.combiners.Count.Globally())
            | 'Filter missing total translations count > 0' >> beam.Filter(lambda x: x > 0)
            | 'Map missing total translations count' >> beam.Map(lambda count: job_run_result.JobRunResult.as_stdout(f'Missing Total Translation Submitter Models FAILED: {count}'))
        )

        translation_groups_with_no_total_logs = translation_groups_with_no_total | 'Map missing total translations to logs' >> (job_result_transforms.ResultsToJobRunResults())

        error_translation_validaition = translation_validaion_results | 'Filter ERR translations' >> beam.Filter(lambda res: res.is_err())
        ...
        error_translation_validaition_count_results = (
            ...
            | 'Filter ERR translations count > 0' >> beam.Filter(lambda x: x > 0)
            | 'Map ERR translations count' >> beam.Map(lambda count: job_run_result.JobRunResult.as_stdout(f'Invalid Total Translation Submitter Models FAILED: {count}'))
        )

        error_translation_validaition_logs = error_translation_validaition | 'Map ERR translations to logs' >> (job_result_transforms.ResultsToJobRunResults())

        success_question_validaition_count_results = question_validaion_results | 'Filter OK questions' >> beam.Filter(lambda res: res.is_ok()) | 'Unwrap OK questions' >> beam.Map(lambda res: res.unwrap()) | 'Count OK questions' >> (job_result_transforms.CountObjectsToJobRunResult('Valid Question Submitter Models'))

        question_groups_with_no_total = filtered_question_groups | 'Filter question Err' >> beam.Filter(lambda r: r.is_err())

        question_groups_with_no_total_count_results = (
            question_groups_with_no_total
            | 'Count missing total questions' >> (beam.combiners.Count.Globally())
            | 'Filter missing total questions count > 0' >> beam.Filter(lambda x: x > 0)
            | 'Map missing total questions count' >> beam.Map(lambda count: job_run_result.JobRunResult.as_stdout(f'Missing Total Question Submitter Models FAILED: {count}'))
        )

        question_groups_with_no_total_logs = question_groups_with_no_total | 'Map missing total questions to logs' >> (job_result_transforms.ResultsToJobRunResults())

        error_question_validaition = question_validaion_results | 'Filter ERR questions' >> beam.Filter(lambda res: res.is_err())

        error_question_validaition_count_results = (
            error_question_validaition | 'Count ERR questions' >> beam.combiners.Count.Globally() | 'Filter ERR questions count > 0' >> beam.Filter(lambda x: x > 0) | 'Map ERR questions count' >> beam.Map(lambda count: job_run_result.JobRunResult.as_stdout(f'Invalid Total Question Submitter Models FAILED: {count}'))
        )

        error_question_validaition_logs = error_question_validaition | 'Map ERR questions to logs' >> (job_result_transforms.ResultsToJobRunResults())
        ...
        [
            Tuple[
                ...
                Dict[
                    str,
                    Union[
                        List[suggestion_models.TranslationSubmitterTotalContributionStatsModel],
    ...
    def _validate_translation(
        total: (suggestion_models.TranslationSubmitterTotalContributionStatsModel),
        contributions: List[suggestion_models.TranslationContributionStatsModel],
        ...
        valid_contributions = [c for c in contributions if (c.topic_id in valid_topic_ids_set)]
        ...
        if not contribution_topic_ids.issubset(set(total.topic_ids_with_translation_submissions)):
            missing = contribution_topic_ids - set(total.topic_ids_with_translation_submissions)
        ...
        for field in fields:
            ...
            if aggregated != total_value:
                error_logs += f'-> field {field} aggregated {aggregated} != total {total_value}\n'
        ...
        if dates:
            ...
            if first_date != total.first_contribution_date:
                error_logs += f'-> first contribution {first_date} != {total.first_contribution_date}\n'
            if last_date != total.last_contribution_date:
                error_logs += f'-> last contribution {last_date} != {total.last_contribution_date}\n'
        ...
        if recent_review_outcomes != total.recent_review_outcomes:
            error_logs += f'-> recent outcomes {recent_review_outcomes} != {total.recent_review_outcomes}\n'

        recent_performance = (counts['accepted'] + counts['accepted_with_edits']) - (2 * counts['rejected'])
        if recent_performance != total.recent_performance:
            error_logs += f'-> recent performance {recent_performance} != {total.recent_performance}\n'

        # Validate overall accuracy.
        if total.submitted_translations_count:
            accuracy = (total.accepted_translations_count / (total.submitted_translations_count)) * 100
            if round(accuracy, 2) != total.overall_accuracy:
                error_logs += f'-> accuracy {round(accuracy, 2)} != {total.overall_accuracy}\n'

        if not error_logs:
            return result.Ok(total.id)
        return result.Err(f'\nValidation failed for TranslationSubmitterTotalContributionStatsModel {total.id}:\n{error_logs}')
    ...
    def _validate_question(
        ...
        # Validate topic_ids list.
        contribution_topic_ids = {c.topic_id for c in valid_contributions}
        if not contribution_topic_ids.issubset(set(total.topic_ids_with_question_submissions)):
            missing = contribution_topic_ids - set(total.topic_ids_with_question_submissions)
            error_logs += f'-> missing topic_ids {missing} in total stats\n'
        ...
        rejected_questions_count = [s.status for s in suggestions if s.status == 'rejected']
        if len(rejected_questions_count) != total.rejected_questions_count:
            error_logs += f'-> field rejected_questions_count {len(rejected_questions_count)} != total {total.rejected_questions_count}\n'

        # Validate first and last contribution dates.
        if valid_contributions:
            first_date = min(c.first_contribution_date for c in valid_contributions)
            last_date = max(c.last_contribution_date for c in valid_contributions)
            if first_date != total.first_contribution_date:
                error_logs += f'-> first contribution {first_date} != {total.first_contribution_date}\n'
            if last_date != total.last_contribution_date:
                error_logs += f'-> last contribution {last_date} != {total.last_contribution_date}\n'
        ...
        if total.submitted_questions_count:
            accuracy = (total.accepted_questions_count / (total.submitted_questions_count)) * 100
            if round(accuracy, 2) != total.overall_accuracy:
                error_logs += f'-> accuracy {round(accuracy, 2)} != {total.overall_accuracy}\n'

        if not error_logs:
            return result.Ok(total.id)
        return result.Err(f'\nValidation failed for QuestionSubmitterTotalContributionStatsModel {total.id}:\n{error_logs}')
