...
from core.domain import exp_domain, exp_fetchers, exp_services, state_domain
...
class VerifyVersionHistoryModelsJob(base_jobs.JobBase):
    ...
    def convert_to_formatted_model_group(self, model_group: UnformattedModelGroupForVerificationJobDict) -> Optional[FormattedModelGroupForVerificationJobDict]:
        ...
        if model_group_is_valid:  # pragma: no cover
            ...
            for exp_model in all_exp_models:
                if exp_model is not None and exp_model.version >= 1 and exp_model.version <= exp_model_vlatest.version:  # pragma: no cover
            ...
            if model_group_is_valid:
                ...
                if model_group_is_valid:  # pragma: no cover
                    ...
                    for version_history in version_history_models:
                        if version_history is not None and version_history.exploration_version is not None and version_history.exploration_version >= 1 and version_history.exploration_version <= exp_model_vlatest.version:  # pragma: no cover
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        all_explorations = (
            self.pipeline
            | 'Get all the exploration snapshot models' >> ndb_io.GetModels(exp_models.ExplorationSnapshotContentModel.get_all(include_deleted=False))
            | 'Filter the exploration snapshot models without None' >> beam.Filter(lambda model: model is not None)
            | 'Get reconstituted exploration models' >> beam.Map(self.generate_exploration_from_snapshot)
            | 'Get Exploration objects from models' >> beam.Map(self.get_exploration_from_model)
            | 'Filter explorations without None' >> beam.Filter(lambda x: x is not None)
            | 'Get id-model pair for exploration models' >> beam.Map(lambda exploration: (exploration.id, exploration))
        )

        all_explorations_vlatest = (
            self.pipeline
            | 'Get all the exploration models at latest version' >> ndb_io.GetModels(exp_models.ExplorationModel.get_all(include_deleted=False))
            | 'Get Exploration objects from exp models vlatest' >> beam.Map(self.get_exploration_from_model)
            | 'Filter the explorations without None' >> beam.Filter(lambda x: x is not None)
            | 'Get id-model pair for exploration models at vlatest' >> beam.Map(lambda exploration: (exploration.id, exploration))
        )
        ...
        verification_results = (
            ...
            | 'Get formatted model groups' >> beam.Map(self.convert_to_formatted_model_group)
            | 'Filter valid model groups' >> beam.Filter(lambda x: x is not None)
            | 'Get the verification result for each model group' >> beam.Map(self.verify_version_history_models)
        )
...
class DeleteExplorationVersionHistoryModelsJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        (version_history_models | beam.Map(lambda model: model.key) | 'Delete all models' >> ndb_io.DeleteModels())
...
class ComputeExplorationVersionHistoryJob(base_jobs.JobBase):
    ...
    def get_updated_version_history_model(
        ...
        if vh_model is None:
            vh_model = exp_models.ExplorationVersionHistoryModel(
                id=exp_models.ExplorationVersionHistoryModel.get_instance_id(exp_id, current_version),
    ...
    def get_reverted_version_history_model(
        ...
        if current_vh_model is None:
            current_vh_model = exp_models.ExplorationVersionHistoryModel(
                ...
                metadata_last_edited_version_number=(revert_to_vh_model.metadata_last_edited_version_number),
                metadata_last_edited_committer_id=(revert_to_vh_model.metadata_last_edited_committer_id),
                ...
                if revert_to_version is not None:
                    if revert_to_version <= 0 or revert_to_version >= version:
                        return (
                            exp_id,
                            [],
                            'Reverting to the version %d which is out of the range [1, %d]' % (revert_to_version, version - 1),
                            version,
                        )
                    ...
                    old_metadata_dict: exp_domain.ExplorationMetadataDict = {
                        ...
                        'states_schema_version': (old_exploration.states_schema_version),
                    ...
                    new_metadata_dict: exp_domain.ExplorationMetadataDict = {
                        ...
                        'states_schema_version': (new_exploration.states_schema_version),
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        (flattened_vh_models | 'Save the models to the datastore' >> ndb_io.PutModels())
