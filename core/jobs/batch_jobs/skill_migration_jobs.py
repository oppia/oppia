...
class MigrateSkillModels(beam.PTransform):  # type: ignore[misc]
    ...
    def expand(
        ...
        unmigrated_skill_models = (
            pipeline
            | 'Get all non-deleted skill models' >> (ndb_io.GetModels(skill_models.SkillModel.get_all()))
        ...
        skill_summary_models = (
            pipeline
            | 'Get all non-deleted skill summary models' >> (ndb_io.GetModels(skill_models.SkillSummaryModel.get_all()))
            # Pylint disable is needed because pylint is not able to correctly
            # detect that the value is passed through the pipe.
            | 'Add skill summary ID'
            >> beam.WithKeys(  # pylint: disable=no-value-for-parameter
        ...
        transformed_skill_objects_list = (
            skill_objects_list
            | 'Remove unmigrated skills' >> beam.Filter(lambda x: len(x['skill_changes']) > 0 and len(x['skill']) > 0)
            | 'Reorganize the skill objects'
            >> beam.Map(
                lambda objects: {
...
class MigrateSkillJob(base_jobs.JobBase):
    ...
    def _update_skill(
        ...
        commit_message = ('Update skill content schema version to %d and skill misconceptions schema version to %d and skill rubrics schema version to %d.') % (
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        skill_models_to_put = transformed_skill_objects_list | 'Generate skill models to put' >> beam.FlatMap(
        ...
        ((skill_models_to_put, skill_summary_models_to_put) | 'Merge models' >> beam.Flatten() | 'Put models into the datastore' >> ndb_io.PutModels())
