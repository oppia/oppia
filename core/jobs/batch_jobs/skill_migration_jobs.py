...
class MigrateSkillModels(beam.PTransform):  # type: ignore[misc]
    ...
    def expand(
        ...
        migrated_skills = filtered_migrated_skills | 'Unwrap ok' >> beam.Map(
            lambda result_item: result_item.unwrap()
        )

        skill_changes = (
            unmigrated_skill_models
            | 'Generate skill changes'
            >> beam.FlatMapTuple(self._generate_skill_changes)
        )
...
class MigrateSkillJob(base_jobs.JobBase):
    ...
    def _update_skill(
        ...
        commit_message = (
            'Update skill content schema version to %d and skill misconceptions schema version to %d and skill rubrics schema version to %d.'
        ) % (
