...
class GenerateContributionStatsJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        non_deleted_suggestion_models = (
            self.pipeline | 'Get all non-deleted suggestion models' >> ndb_io.GetModels(
        ...
        reviewed_translation_suggestions_grouped_by_target = (
            ...
            'Filter reviewed translate suggestions' >> beam.Filter(
                lambda m: (
                    m.suggestion_type == feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT and m.
                    status in [
                        suggestion_models.STATUS_ACCEPTED, suggestion_models.
                        STATUS_REJECTED
                    ]
                )
        ...
        reviewed_question_suggestions_grouped_by_target = (
            non_deleted_suggestion_models | 'Filter question reviews' >> beam.Filter(
                lambda m: (
                    m.suggestion_type == feconf.SUGGESTION_TYPE_ADD_QUESTION and m.
                    status in [
                        suggestion_models.STATUS_ACCEPTED, suggestion_models.
                        STATUS_REJECTED
                    ]
        ...
        skill_opportunities_by_id = (
            self.pipeline | 'Get all non-deleted skill opportunity models' >> (
                ndb_io.GetModels(
        ...
        user_review_stats_models = (
            translation_review_stats_keys_and_results | 'Filter ok review results' >>
            beam.Filter(lambda key_and_result: key_and_result[1].is_ok()) |
        ...
        user_question_review_stats_models = (
            question_review_stats_keys_and_results | 'Filter ok question review results'
            >> beam.Filter(lambda key_and_result: key_and_result[1].is_ok()) |
        ...
        user_review_stats_error_job_run_results = (
            translation_review_stats_keys_and_results | 'Filter review err results' >>
            beam.Filter(lambda key_and_result: key_and_result[1].is_err())
            # Pylint disable is needed because pylint is not able to correctly
            # detect that the value is passed through the pipe.
        ...
        user_stats_models_job_run_results = (
            user_contribution_stats_models | 'Create contribution job run result' >> (
        ...
        user_review_stats_models_job_run_results = (
            user_review_stats_models | 'Create review job run result' >> (
                job_result_transforms.CountObjectsToJobRunResult(
        ...
        return ((
            user_stats_error_job_run_results, user_stats_models_job_run_results,
            user_review_stats_error_job_run_results,
            user_review_stats_models_job_run_results,
            user_question_contribution_stats_error_job_run_results,
            user_question_contribution_stats_models_job_run_results,
    ...
    def _generate_translation_stats(
        ...
        model: Union[Type[suggestion_models.TranslationContributionStatsModel],
        ...
        if opportunity is not None:
            ...
            for suggestion in suggestions:
                user_id = (
                    ...
                    if model == suggestion_models.TranslationContributionStatsModel else
                ...
                try:
                    ...
                    translation_contribution_stats_dict = {
                        ...
                        'last_updated_date': (suggestion.last_updated.date()),
                        'created_on_date': (suggestion.created_on.date())
                    }
    ...
    def _generate_question_stats(
        suggestions: Iterable[suggestion_registry.SuggestionAddQuestion], skill_id: str,
        model: Union[Type[suggestion_models.QuestionContributionStatsModel],
        ...
        with datastore_services.get_ndb_context():
            for topic in skill_services.get_all_topic_assignments_for_skill(skill_id):
                ...
                for suggestion in suggestions:
                    user_id = (
                        suggestion.author_id
                        if model == suggestion_models.QuestionContributionStatsModel
    ...
    def _combine_translation_review_stats_objects(
        ...
        return suggestion_registry.TranslationReviewStats(
            ...
            accepted_translations_with_reviewer_edits_count=sum(
                stat.accepted_translations_with_reviewer_edits_count for stat in stats
            ),
    ...
    def _combine_question_contribution_stats_objects(
        ...
        return suggestion_registry.QuestionContributionStats(
            ...
            accepted_questions_without_reviewer_edits_count=sum(
                stat.accepted_questions_without_reviewer_edits_count for stat in stats
            ),
            first_contribution_date=all_first_contributed_dates[0],
            last_contribution_date=all_last_contributed_dates[-1]
        )
    ...
    def _combine_question_review_stats_objects(
        ...
        return suggestion_registry.QuestionReviewStats(
            ...
            accepted_questions_with_reviewer_edits_count=sum(
                stat.accepted_questions_with_reviewer_edits_count for stat in stats
            ),
            first_contribution_date=all_first_contributed_dates[0],
            last_contribution_date=all_last_contributed_dates[-1]
        )
    ...
    def _generate_translation_contribution_model(
        entity_id: str, translation: suggestion_registry.TranslationContributionStats
    ) -> suggestion_models.TranslationContributionStatsModel:
