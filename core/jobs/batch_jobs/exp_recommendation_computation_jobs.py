...
class ComputeExplorationRecommendationsJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        exp_recommendations_models = (
            ...
            | 'Sort and slice similarities'
            >> beam.MapTuple(
                lambda exp_id, similarities: (
                    exp_id,
                    self._sort_and_slice_similarities(similarities),
                )
            )
            | 'Create recommendation models' >> beam.MapTuple(self._create_recommendation)
        )

        (exp_recommendations_models | 'Put models into the datastore' >> ndb_io.PutModels())
