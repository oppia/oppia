...
class RemoveNonExistentThreadsMessagesJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        invalid_message_logs = (
            ...
            >> beam.Map(
                lambda model: job_run_result.JobRunResult.as_stdout(
                    (
                        f'GeneralFeedbackMessageModel with non-existent thread: id={model.id}, thread_id={model.thread_id}, message_id={model.message_id}'
                    )
                )
            )
        )
        ...
        invalid_user_thread_logs = (
            invalid_user_threads
            | 'Log invalid user threads'
            >> beam.Map(
                lambda model: job_run_result.JobRunResult.as_stdout(
                    (
                        f'GeneralFeedbackThreadUserModel with non-existent thread: id={model.id}, thread_id={model.thread_id}, user_id={model.user_id}'
                    )
                )
            )
        )
        ...
        if self.DATASTORE_UPDATES_ALLOWED:
            deleted_message_logs = (
                ...
                >> beam.Map(
                    lambda model: job_run_result.JobRunResult.as_stdout(
                        (
                            f'Deleted GeneralFeedbackMessageModel: id={model.id}, thread_id={model.thread_id}, message_id={model.message_id}'
                        )
                    )
                )
            )
            ...
            deleted_user_thread_logs = (
                ...
                >> beam.Map(
                    lambda model: job_run_result.JobRunResult.as_stdout(
                        (
                            f'Deleted GeneralFeedbackThreadUserModel: id={model.id}, thread_id={model.thread_id}, user_id={model.user_id} '
                        )
                    )
                )
            )
