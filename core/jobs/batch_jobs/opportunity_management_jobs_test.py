...
class AuditPopulateSkillOpportunityModelPropertiesJobTests(
    job_test_utils.JobTestBase
):

    JOB_CLASS: Type[
        opportunity_management_jobs.AuditPopulateSkillOpportunityModelPropertiesJob
    ] = (
        opportunity_management_jobs.AuditPopulateSkillOpportunityModelPropertiesJob
    )
    ...
    def test_job_audits_properties(self) -> None:
        topic_rights_model = self.create_model(
            topic_models.TopicRightsModel,
            id='topic_id',
            topic_is_published=True,
        ...
        self.assert_job_output_is(
            [
                job_run_result.JobRunResult(
                    stdout='SKILL OPPORTUNITY MODEL AUDITED SUCCESS: 3'
                )
            ]
        )
