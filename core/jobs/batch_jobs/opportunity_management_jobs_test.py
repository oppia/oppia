            ...
            self.assert_job_output_is(
                [
                    job_run_result.JobRunResult(
                        stdout='',
                        stderr='ERROR: "Expected question_count to be a non-negative integer, received -1": 2',
                    )
                ]
            )
    ...
    def test_generation_job_fails_when_story_id_is_not_available(self) -> None:
        ...
        self.assert_job_output_is(
            [
                job_run_result.JobRunResult(
                    stderr=(
                        'ERROR: "Failed to regenerate opportunities for topic id: topic_1_id, missing_exp_with_ids: [], missing_story_with_ids: [\'missing_id\']": 1'
                    )
                )
            ]
        )
    ...
    def test_generation_job_fails_when_exp_id_is_not_available(self) -> None:
        ...
        self.assert_job_output_is(
            [
                job_run_result.JobRunResult(
                    stderr=(
                        'ERROR: "Failed to regenerate opportunities for topic id: topic_1_id, missing_exp_with_ids: [\'missing_id\'], missing_story_with_ids: []": 1'
                    )
                )
            ]
        )
