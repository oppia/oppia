...
# ignore here.
class GetDeprecatedSuggestionEditStateContentModels(
    beam.PTransform
):  # type: ignore[misc]
    ...
    def expand(
    ...
    ) -> Tuple[beam.PCollection[suggestion_models.GeneralSuggestionModel],
               beam.PCollection[job_run_result.JobRunResult]]:
        suggestion_edit_state_content_model_to_delete = (
            pipeline | 'Get all general suggestion models' >>
        ...
        suggestion_edit_state_content_model_to_delete_count = (
            ...
            'Count edit state content suggestion to be deleted' >> (
                ...
                CountObjectsToJobRunResult('EDIT STATE CONTENT SUGGESTION')
            )
        )
...
class AuditDeprecatedSuggestionEditStateContentModelsDeletionJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        job_run_results = (
            self.pipeline | 'Perform fetching and deletion of edit state content'
            ' suggestion results' >> (GetDeprecatedSuggestionEditStateContentModels())
        )[1]
