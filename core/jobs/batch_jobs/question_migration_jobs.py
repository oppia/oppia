...
class PopulateQuestionSummaryVersionOneOffJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        all_question_models = (
            self.pipeline
            | 'Get all non-deleted question models' >> (ndb_io.GetModels(question_models.QuestionModel.get_all()))
        ...
        question_summary_models_to_put = (
            question_summary_models | 'Filter oks' >> beam.Filter(lambda result_item: result_item.is_ok()) | 'Unwrap ok' >> beam.Map(lambda result_item: result_item.unwrap()) | 'Get rid of ID' >> beam.Values()  # pylint: disable=no-value-for-parameter
        )

        (question_summary_models_to_put | 'Put models into datastore' >> ndb_io.PutModels())
...
class MigrateQuestionModels(beam.PTransform):  # type: ignore[misc]
    ...
    def expand(
        ...
        question_summary_models = (
            self.pipeline
            | 'Get all non-deleted question summary models' >> (ndb_io.GetModels(question_models.QuestionSummaryModel.get_all()))
            # Pylint disable is needed because pylint is not able to correctly
            # detect that the value is passed through the pipe.
            | 'Add question summary keys'
            >> beam.WithKeys(  # pylint: disable=no-value-for-parameter
        ...
        transformed_question_objects_list = (
            question_objects_list
            | 'Remove unmigrated questions' >> beam.Filter(lambda x: len(x['question_changes']) > 0 and len(x['question']) > 0)
            | 'Reorganize the question objects'
            >> beam.Map(
                lambda objects: {
        ...
        already_migrated_job_run_results = (
            question_objects_list | 'Remove migrated questions' >> beam.Filter(lambda x: (len(x['question_changes']) == 0 and len(x['question']) > 0)) | 'Transform already migrated question into job run results' >> (job_result_transforms.CountObjectsToJobRunResult('QUESTION PREVIOUSLY MIGRATED'))
        )
...
class MigrateQuestionJob(base_jobs.JobBase):
    ...
    def _update_question(
        ...
        with datastore_services.get_ndb_context():
            models_to_put = updated_question_model.compute_models_to_commit(
                ...
                'Update state data contents schema version to %d.' % (feconf.CURRENT_STATE_SCHEMA_VERSION),
                change_dicts,
                additional_models={},
            )
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        question_models_to_put = transformed_question_objects_list | 'Generate question models to put' >> beam.FlatMap(
        ...
        question_summary_model_to_put = transformed_question_objects_list | 'Generate question summary to put' >> beam.Map(
        ...
        ((question_models_to_put, question_summary_model_to_put) | 'Merge models' >> beam.Flatten() | 'Put models into datastore' >> ndb_io.PutModels())
