...
class RemoveProfilePictureFieldJobTests(job_test_utils.JobTestBase):
    ...
    def test_model_without_profile_picture_field_works(self) -> None:
        ...
        migrated_setting_model = user_models.UserSettingsModel.get_by_id(
            user_1.id
        )
    ...
    def test_removal_of_profile_field(self) -> None:
        with self.swap(
            ...
            self.assertNotIn(
                'profile_picture_data_url',
                migrated_setting_model_user_1._values,
            )  # pylint: disable=protected-access
            self.assertNotIn(
                'profile_picture_data_url',
                migrated_setting_model_user_1._properties,
            )  # pylint: disable=protected-access
            ...
            self.assertNotIn(
                'profile_picture_data_url',
                migrated_setting_model_user_2._values,
            )  # pylint: disable=protected-access
            self.assertNotIn(
                'profile_picture_data_url',
                migrated_setting_model_user_2._properties,
            )  # pylint: disable=protected-access
