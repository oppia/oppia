...
class GetUsersWithInvalidBioJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        user_usernames_and_bios = (
            ...
            'Extract username and bio from model' >> beam.
            Map(lambda user_settings: (user_settings.username, user_settings.user_bio))
        )
        ...
        number_of_users_queried_report = (
            user_usernames_and_bios | 'Report count of user models' >>
            (job_result_transforms.CountObjectsToJobRunResult('CountTotalUsers'))
        )

        number_of_users_with_invalid_bio_report = (
            users_with_invalid_bios | 'Report count of invalid user models' >>
