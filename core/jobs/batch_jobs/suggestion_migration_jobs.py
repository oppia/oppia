...
class RegenerateContentIdForTranslationSuggestionsInReviewJob(base_jobs.JobBase):
    ...
    ) -> List[result.Result[suggestion_models.GeneralSuggestionModel, Tuple[
        str, Exception]]]:
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        exploration_models = (
            self.pipeline | 'Get all exploration models' >>
            ndb_io.GetModels(exp_models.ExplorationModel.get_all()) |
            'Add exploration id as key' >> beam.
        ...
        migrated_suggestion_results = (
            ...
            beam.Filter(lambda objects: len(objects['suggestion_models']) != 0) |
            'Transform and migrate model' >> beam.Map(
                lambda objects: (
                    self._update_content_id_in_translation_suggestions(
                        objects['suggestion_models'][0], objects['exploration_model'][0]
                    )
                )
        ...
        migrated_suggestion_job_run_results = (
            migrated_suggestion_results | 'Generate results for migration' >> (
                job_result_transforms.
                ResultsToJobRunResults('SUGGESTION TARGET PROCESSED')
            )
        )
        ...
        migrated_suggestions_count_job_run_results = (
            ...
            (job_result_transforms.CountObjectsToJobRunResult('SUGGESTION MIGRATED'))
        )
...
class MigrateQuestionSuggestionsJob(base_jobs.JobBase):
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        question_suggestions = (
            self.pipeline | 'Get all GeneralSuggestionModels' >> ndb_io.GetModels(
                suggestion_models.GeneralSuggestionModel.get_all(include_deleted=False)
            ) | 'Filter question suggestions' >> (
                beam.Filter(
                    lambda model: (
                        model.suggestion_type == feconf.SUGGESTION_TYPE_ADD_QUESTION and
        ...
        models_count_job_run_results = (
            question_suggestions | 'Transform suggestions into job run results' >>
            (job_result_transforms.CountObjectsToJobRunResult('QUESTION MODELS COUNT'))
        )
        ...
        migrate_suggestion_results = (
            ...
            ) | 'Migrate question_dict in change field' >>
            beam.Map(self._migrate_question_dict)
        )
        ...
        migrated_exp_job_run_results = (
            migrate_suggestion_results | 'Generate results for migration' >>
            (job_result_transforms.ResultsToJobRunResults('SUGGESTION MIGRATED'))
        )
        ...
        return ((models_count_job_run_results, migrated_exp_job_run_results) |
                beam.Flatten())
