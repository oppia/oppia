...
class MigrateExplorationJob(base_jobs.JobBase):
    ...
    def _migrate_exploration(
        ...
        Returns:
            ...
            or Exception. Exploration object s returned when the migration was
    ...
    def _generate_exp_changes(
        ...
        """
        states_schema_version = exp_model.states_schema_version
        if states_schema_version < feconf.CURRENT_STATE_SCHEMA_VERSION:
            exp_change = exp_domain.ExplorationChange({
                'cmd': (
                    exp_domain.CMD_MIGRATE_STATES_SCHEMA_TO_LATEST_VERSION),
                'from_version': states_schema_version,
                'to_version': feconf.CURRENT_STATE_SCHEMA_VERSION})
            yield (exp_id, exp_change)

    @staticmethod
    def _delete_exp_from_cache(
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        exp_changes = (
            ...
            | 'Generate exploration changes' >> beam.FlatMapTuple(
                self._generate_exp_changes)
        )

        exp_objects_list = (
            {
                'exp_model': unmigrated_exp_models,
            ...
            | 'Remove unmigrated explorations' >> beam.Filter(
                lambda x: len(x['exp_changes']) > 0 and
                          len(x['exploration']) > 0)
        ...
        cache_deletion_job_run_results = (
            ...
            | 'Delete exploration from cache' >> beam.Map(
                lambda exp_object: self._delete_exp_from_cache(
