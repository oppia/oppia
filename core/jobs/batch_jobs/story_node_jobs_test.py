    ...
    def test_topic_with_no_story_reference_raises_error(self) -> None:
        ...
        self.assert_job_output_is(
            [
                job_run_result.JobRunResult(
                    stderr=(
                        'TOPIC MODELS WHOSE STORIES ARE UPDATED ERROR: "(\'story_1_id\', StopIteration())": 1'
                    )
                )
            ]
        )
    ...
    def test_story_with_wrong_commit_history_raises_error(self) -> None:
        ...
        self.assert_job_output_is(
            [
                job_run_result.JobRunResult(
                    stderr=(
                        'TOPIC MODELS WHOSE STORIES ARE UPDATED ERROR: "(\'story_1_id\', Exception(\'Node was not created.\'))": 1'
                    )
                )
            ]
        )
