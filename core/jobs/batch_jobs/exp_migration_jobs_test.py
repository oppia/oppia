...
class MigrateExplorationJobTests(
    ...
    def test_unmigrated_exp_with_unsupported_content_is_migrated(self) -> None:
        ...
        self.assertEqual(exp_model.states_schema_version, 41)
        validate_exploration_for_story_swap = self.swap_to_always_raise(
            exp_services, 'validate_exploration_for_story',
            exp_services.UnsupportedRTEContentValidationError()
        )
        with validate_exploration_for_story_swap:
            self.assert_job_output_is([
                job_run_result.JobRunResult(stdout='EXP PROCESSED SUCCESS: 1'),
                job_run_result.JobRunResult(
                    stdout='CACHE DELETION SUCCESS: 1', stderr=''),
