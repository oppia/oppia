...
class GetUsersWithInvalidBioJobTests(job_test_utils.JobTestBase):
    ...
    def test_user_with_null_bio(self) -> None:
        ...
        self.assert_job_output_is([
            job_run_result.JobRunResult(
                stderr=f'The id of user is "{self.USER_ID_1}" and its bio '
                + f'is "{user.user_bio}"'
            ),
            job_run_result.JobRunResult(
                stdout='INVALID SUCCESS: 1'),
            job_run_result.JobRunResult(
                stdout='USERS SUCCESS: 1')
        ])
    ...
    def test_user_with_too_long_bio(self) -> None:
        ...
        self.assert_job_output_is([
            job_run_result.JobRunResult(
                stderr=f'The id of user is "{self.USER_ID_3}" and its bio '
                + f'is "{user.user_bio}"'
            ),
            job_run_result.JobRunResult(
                stdout='INVALID SUCCESS: 1'),
