...
class PopulateManualVoiceoversToEntityVoiceoversModelJob(base_jobs.JobBase):
    ...
    def create_entity_voiceovers_instances(
        cls, exploration_model: exp_models.ExplorationModel,
    ...
    def run(self) -> beam.PCollection[job_run_result.JobRunResult]:
        ...
        """
        exploration_models = (
            self.pipeline | 'Get exploration models' >>
            ndb_io.GetModels(exp_models.ExplorationModel.get_all())
        )

        exploration_voice_artists_link_models = (
            self.pipeline | 'Get exploration voice artists link models' >> (
                ndb_io.GetModels(
        ...
        voice_artist_metadata_models = (
            self.pipeline | 'Get voice artist metadata model' >>
        ...
        paired_exploration_models = (
            exploration_models |
            'Pair Exploration ID to model' >> beam.Map(lambda model: (model.id, model))
        )
            ...
            'Filter invalid exploration and voice artist link models' >> (
                beam.Filter(
                    lambda element: (
                        ...
                        len(element[1]['exploration_voice_artists_link_model']) > 0
                    )
                )
        ...
        entity_voiceovers_models_result = (
            ...
            'Get result data from migrated models' >> beam.Map(
                lambda model: job_run_result.JobRunResult.as_stdout(
                    ...
                    'accent code %s.' % (
                        len(list(model.voiceovers_mapping.keys())), model.entity_id,
