...
_MODEL_TYPES_BY_BASE_CLASS: Dict[Type[base_models.BaseModel],
                                 FrozenSet[Type[base_models.BaseModel]]] = {
                                     base_class:
                                         frozenset({base_class}).union(
                                             t for t in _ALL_MODEL_TYPES
                                             if issubclass(t, base_class)
...
ModelRelationshipsType = Callable[..., Iterator[Tuple[
    model_property.PropertyType, Sequence[Type[base_models.BaseModel]]]]]
...
class AuditsExisting:
    ...
    def __call__(self, do_fn_type: Type[beam.DoFn]) -> Type[beam.DoFn]:
        ...
        with_input_types, with_output_types = (
            typehints.with_input_types(typehints.Union[self._targeted_model_types]),
...
class RelationshipsOf:
    ...
    _ID_REFERENCING_PROPERTIES: Dict[model_property.ModelProperty,
    ...
    def __call__(
        ...
        for property_instance, referenced_models in yielded_items:
            ...
            self._ID_REFERENCING_PROPERTIES[property_of_model].update(
                self._get_model_kind(m) for m in referenced_models
                if m is not self._model_class
            )
    ...
    def get_id_referencing_properties_by_kind_of_possessor(
        ...
        """
        by_kind: Callable[[model_property.ModelProperty],
                          str] = lambda model_property: model_property.model_kind
        id_referencing_properties_by_kind_of_possessor = itertools.groupby(
            sorted(cls._ID_REFERENCING_PROPERTIES.keys(), key=by_kind), key=by_kind
        )
        references_of: Callable[[model_property.ModelProperty],
                                Set[str]] = lambda p: cls._ID_REFERENCING_PROPERTIES[p]
        return {
    ...
    def get_all_model_kinds_referenced_by_properties(cls) -> Set[str]:
        ...
        """
        return set(
            itertools.chain.from_iterable(cls._ID_REFERENCING_PROPERTIES.values())
        )
    ...
    def _validate_name_of_model_relationships(
        ...
        if actual_name != expected_name:
            raise ValueError(
                'Please rename the function from "%s" to "%s"' %
