...
class ErrorMessageTests(core_test_utils.TestBase):
    ...
    def test_error_message_with_wrong_input(self) -> None:
        error = base_validation_errors.BaseAuditError(
            'testing string', 'non-existing model', None
        )
        self.assertEqual(
            error.stderr, 'BaseAuditError in non-existing model: testing string'
        )
...
class BaseAuditErrorTests(AuditErrorsTestBase):
    ...
    def test_equality_between_same_types_and_different_values(self) -> None:
        self.assertNotEqual(
            FooError(self.model), FooError(job_utils.clone_model(self.model, id='987'))
        )
...
class InconsistentTimestampsErrorTests(AuditErrorsTestBase):
    ...
    def test_message(self) -> None:
        ...
        self.assertEqual(
            error.stderr, 'InconsistentTimestampsError in BaseModel(id="123"): '
            'created_on=%r is later than last_updated=%r' % (self.NOW, self.YEAR_AGO)
        )
...
class ModelExpiredErrorTests(AuditErrorsTestBase):
    ...
    def test_message(self) -> None:
        ...
        self.assertEqual(
            error.stderr, 'ModelExpiredError in BaseModel(id="123"): deleted=True when '
            'older than %d days' %
...
class ModelRelationshipErrorTests(AuditErrorsTestBase):
    ...
    def test_message(self) -> None:
        error = base_validation_errors.ModelRelationshipError(
            model_property.ModelProperty(FooModel, FooModel.bar_id), '123', 'BarModel',
            '123'
        )
