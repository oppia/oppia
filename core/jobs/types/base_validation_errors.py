    ...
    def __init__(
        ...
        if model_id:
            error_message = '%s in %s(id=%s): %s' % (
                self.__class__.__name__, model_kind, utils.quoted(model_id), message
            )
        else:
            error_message = '%s in %s: %s' % (
                self.__class__.__name__, model_kind, message
            )
...
class InvalidCommitTypeError(BaseAuditError):
    ...
    def __init__(
        self, model: Union[base_models.BaseCommitLogEntryModel,
...
class ModelRelationshipError(BaseAuditError):
    ...
    def __init__(
        self, id_property: model_property.ModelProperty, model_id: Optional[str],
...
class CommitCmdsValidateError(BaseAuditError):
    ...
    def __init__(
        self, model: Union[base_models.BaseCommitLogEntryModel,
                           base_models.BaseSnapshotMetadataModel],
        commit_cmd_dict: Mapping[str, change_domain.AcceptableChangeDictTypes], e: str
    ) -> None:
        message = (
