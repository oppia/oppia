...
class DuplicateBlogAuthorModelErrorTests(
    ...
    def test_message(self) -> None:
        author_details_model = blog_models.BlogAuthorDetailsModel(
            id='id1', displayed_author_name='user one', author_id='user', author_bio=''
        )
        ...
        self.assertEqual(
            ...
            ' author id=%s is not unique' %
            utils.quoted(author_details_model.author_id)
        )
...
class InconsistentLastUpdatedTimestampsErrorTests(
    ...
    def test_message(self) -> None:
        ...
        self.assertEqual(
            error.stderr, 'InconsistentLastUpdatedTimestampsError in BlogPostModel'
...
class InconsistentPublishLastUpdatedTimestampsErrorTests(
    ...
    def test_message(self) -> None:
        ...
        error = (
            blog_validation_errors.InconsistentPublishLastUpdatedTimestampsError(model)
        )
...
class ModelMutatedDuringJobErrorTests(base_validation_errors_test.AuditErrorsTestBase):
    ...
    def test_message_for_published_on(self) -> None:
        ...
        self.assertEqual(
            error.stderr, 'ModelMutatedDuringJobErrorForPublishedOn in BlogPostModel('
    ...
    def test_message_for_last_updated(self) -> None:
        ...
        self.assertEqual(
            error.stderr, 'ModelMutatedDuringJobErrorForLastUpdated in BlogPostModel('
