...
(auth_models, collection_models, email_models, exp_models, feedback_models,
    skill_models, story_models, user_models) = (models.Registry.
    import_models([models.Names.AUTH, models.Names.COLLECTION, models.Names
    .EMAIL, models.Names.EXPLORATION, models.Names.FEEDBACK, models.Names.
    SKILL, models.Names.STORY, models.Names.USER]))
datastore_services = models.Registry.import_datastore_services()


@validation_decorators.AuditsExisting(auth_models.UserAuthDetailsModel,
    user_models.UserEmailPreferencesModel, user_models.UserSettingsModel)
...
class ValidateActivityMappingOnlyAllowedKeys(beam.DoFn):
    ...
    def process(self, input_model: user_models.PendingDeletionRequestModel
        ...
        allowed_keys = [name.value for name in models.
            MODULES_WITH_PSEUDONYMIZABLE_CLASSES]
        incorrect_keys = [key for key in model.
            pseudonymizable_entity_mappings.keys() if key not in allowed_keys]
        if incorrect_keys:
            yield user_validation_errors.ModelIncorrectKeyError(model,
                incorrect_keys)


@validation_decorators.RelationshipsOf(user_models.CompletedActivitiesModel)
def completed_activities_model_relationships(model: Type[user_models.
    CompletedActivitiesModel]) ->Iterator[Tuple[datastore_services.Property,
    List[Type[Union[exp_models.ExplorationModel, collection_models.
    CollectionModel]]]]]:
...
def incomplete_activities_model_relationships(model: Type[user_models.
    IncompleteActivitiesModel]) ->Iterator[Tuple[datastore_services.
    Property, List[Type[Union[exp_models.ExplorationModel,
...
def exp_user_last_playthrough_model_relationships(model: Type[user_models.
    ExpUserLastPlaythroughModel]) ->Iterator[Tuple[datastore_services.
    Property, List[Type[exp_models.ExplorationModel]]]]:
    """Yields how the properties of the model relates to the ID of others."""
...
def learner_playlist_model_relationships(model: Type[user_models.
    LearnerPlaylistModel]) ->Iterator[Tuple[datastore_services.Property,
    List[Type[Union[exp_models.ExplorationModel, collection_models.
    CollectionModel]]]]]:
    """Yields how the properties of the model relates to the ID of others."""
    yield model.exploration_ids, [exp_models.ExplorationModel]
...
def user_contributions_model_relationships(model: Type[user_models.
    UserContributionsModel]) ->Iterator[Tuple[datastore_services.Property,
...
def user_email_preferences_model_relationships(model: Type[user_models.
    UserEmailPreferencesModel]) ->Iterator[Tuple[model_property.
    PropertyType, List[Type[user_models.UserSettingsModel]]]]:
    """Yields how the properties of the model relates to the ID of others."""
    yield model.id, [user_models.UserSettingsModel]


@validation_decorators.RelationshipsOf(user_models.UserSubscriptionsModel)
def user_subscriptions_model_relationships(model: Type[user_models.
    UserSubscriptionsModel]) ->Iterator[Tuple[datastore_services.Property,
    List[Type[Union[exp_models.ExplorationModel, collection_models.
    CollectionModel, feedback_models.GeneralFeedbackThreadModel,
    user_models.UserSubscribersModel]]]]]:
    """Yields how the properties of the model relates to the ID of others."""
    yield model.exploration_ids, [exp_models.ExplorationModel]
    yield model.collection_ids, [collection_models.CollectionModel]
    yield model.general_feedback_thread_ids, [feedback_models.
...
def user_subscribers_model_relationships(model: Type[user_models.
    UserSubscribersModel]) ->Iterator[Tuple[datastore_services.Property,
...
def user_recent_changes_batch_model_relationships(model: Type[user_models.
    UserRecentChangesBatchModel]) ->Iterator[Tuple[model_property.
    PropertyType, List[Type[user_models.UserSettingsModel]]]]:
    """Yields how the properties of the model relates to the ID of others."""
    yield model.id, [user_models.UserSettingsModel]
...
def user_stats_model_relationships(model: Type[user_models.UserStatsModel]
    ) ->Iterator[Tuple[model_property.PropertyType, List[Type[user_models.
    UserSettingsModel]]]]:
    """Yields how the properties of the model relates to the ID of others."""
    yield model.id, [user_models.UserSettingsModel]
...
def exploration_user_data_model_relationships(model: Type[user_models.
    ExplorationUserDataModel]) ->Iterator[Tuple[datastore_services.Property,
    List[Type[exp_models.ExplorationModel]]]]:
    """Yields how the properties of the model relates to the ID of others."""
    yield model.exploration_id, [exp_models.ExplorationModel]
...
def collection_progress_model_relationships(model: Type[user_models.
    CollectionProgressModel]) ->Iterator[Tuple[datastore_services.Property,
    List[Type[Union[collection_models.CollectionModel, exp_models.
    ExplorationModel, user_models.CompletedActivitiesModel]]]]]:
    """Yields how the properties of the model relates to the ID of others."""
...
def story_progress_model_relationships(model: Type[user_models.
    StoryProgressModel]) ->Iterator[Tuple[datastore_services.Property, List
...
def user_query_model_relationships(model: Type[user_models.UserQueryModel]
    ) ->Iterator[Tuple[datastore_services.Property, List[Type[email_models.
    BulkEmailModel]]]]:
    """Yields how the properties of the model relates to the ID of others."""
...
def user_bulk_emails_model_relationships(model: Type[user_models.
    UserBulkEmailsModel]) ->Iterator[Tuple[datastore_services.Property,
...
def user_skill_mastery_model_relationships(model: Type[user_models.
    UserSkillMasteryModel]) ->Iterator[Tuple[datastore_services.Property,
    List[Type[skill_models.SkillModel]]]]:
    """Yields how the properties of the model relates to the ID of others."""
    yield model.skill_id, [skill_models.SkillModel]


@validation_decorators.RelationshipsOf(user_models.
    UserContributionProficiencyModel)
def user_contribution_proficiency_model_relationships(model: Type[
    user_models.UserContributionProficiencyModel]) ->Iterator[Tuple[
...
def user_contribution_rights_model_relationships(model: Type[user_models.
    UserContributionRightsModel]) ->Iterator[Tuple[model_property.
    PropertyType, List[Type[user_models.UserSettingsModel]]]]:
    """Yields how the properties of the model relates to the ID of others."""
    yield model.id, [user_models.UserSettingsModel]
...
class ValidateDraftChangeListLastUpdated(beam.DoFn):
    ...
    def process(self, input_model: user_models.ExplorationUserDataModel
        ) ->Iterator[Union[user_validation_errors.
        DraftChangeListLastUpdatedNoneError, user_validation_errors.
        DraftChangeListLastUpdatedInvalidError]]:
        ...
        """
        model = job_utils.clone_model(input_model)
        if (model.draft_change_list and not model.
        ...
        if (model.draft_change_list_last_updated and model.
            draft_change_list_last_updated > current_time):
...
class ValidateArchivedModelsMarkedDeleted(beam.DoFn):
    ...
    def process(self, input_model: user_models.UserQueryModel) ->Iterator[
        user_validation_errors.ArchivedModelNotMarkedDeletedError]:
