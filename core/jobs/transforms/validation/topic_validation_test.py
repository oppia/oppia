...
class ValidateCanonicalNameMatchesNameInLowercaseTests(
    ...
    def test_process_for_not_matching_canonical_name(self) -> None:
        ...
        output = (
            self.pipeline | beam.Create([model_with_different_name]) |
            beam.ParDo(topic_validation.ValidateCanonicalNameMatchesNameInLowercase())
        )
        self.assert_pcoll_equal(
            output, [
                topic_validation_errors.
                ModelCanonicalNameMismatchError(model_with_different_name)
            ]
        )
    ...
    def test_process_for_matching_canonical_name(self) -> None:
        ...
        output = (
            self.pipeline | beam.Create([model_with_same_name]) |
            beam.ParDo(topic_validation.ValidateCanonicalNameMatchesNameInLowercase())
        )
...
class ValidateTopicRightsSnapshotMetadataModelTests(job_test_utils.PipelinedTestBase):
    ...
    def test_topic_rights_change_object_with_missing_cmd(self) -> None:
        ...
        output = (
            self.pipeline | beam.Create([invalid_commit_cmd_model]) |
            beam.ParDo(topic_validation.ValidateTopicRightsSnapshotMetadataModel())
        )
