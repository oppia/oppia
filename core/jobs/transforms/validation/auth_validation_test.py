...
class ValidateUserIdByFirebaseAuthIdModelIdTests(job_test_utils.PipelinedTestBase):
    def test_reports_error_for_invalid_id(self) -> None:
        ...
        self.assert_pcoll_equal(
            output,
            [
                base_validation_errors.ModelIdRegexError(model_with_invalid_id, feconf.FIREBASE_AUTH_ID_REGEX),
            ],
        )
...
class RelationshipsOfTests(test_utils.TestBase):
    def test_user_auth_details_model_relationships(self) -> None:
        self.assertItemsEqual(
            validation_decorators.RelationshipsOf.get_model_kind_references('UserAuthDetailsModel', 'firebase_auth_id'),
            ['UserIdByFirebaseAuthIdModel'],
        )
        self.assertItemsEqual(
            validation_decorators.RelationshipsOf.get_model_kind_references('UserAuthDetailsModel', 'gae_id'),
            ['UserIdentifiersModel'],
        )

    def test_user_id_by_firebase_auth_id_model_relationships(self) -> None:
        self.assertItemsEqual(
            validation_decorators.RelationshipsOf.get_model_kind_references('UserIdByFirebaseAuthIdModel', 'user_id'),
            ['UserAuthDetailsModel'],
        )

    def test_user_identifiers_model_relationships(self) -> None:
        self.assertItemsEqual(
            validation_decorators.RelationshipsOf.get_model_kind_references('UserIdentifiersModel', 'user_id'),
            ['UserAuthDetailsModel'],
        )
