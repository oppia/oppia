...
class BaseMapReduceJobManagerMetaClass(type):
    ...
    Ideally, the methods Would be implemented as class methods, but the map
    reduce job pipelines does not support class methods. Thus, we are forced to
    fall back to using static methods which can only be generated by a meta
    class.
    """
