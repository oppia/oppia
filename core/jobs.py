...
class BaseMapReduceJobManagerMetaClass(type):
    """The base type class of map reduce jobs.

    Provides classes with a "_real_map" and "_real_reduce" static method, which
    converts all of the strings generated by the class's "map" and "reduce"
    static methods into bytes. This is necessary because the map reduce job
    pipelines only accept bytes.

    Ideally, these two methods would be implemented using class methods.
    Unfortunately, map reduce job pipelines can _only_ be implemented using
    static methods.
    Since we can not take advantage of inheritance from a static method, we're
    forced to patch in the "_real_map" and "_real_reduce" methods onto the class
    by using this meta class.
    """
    ...
    def __new__(mcs, name, bases, dct):
        ...
        def _real_map(item):
            ...
            values it produces recursively.
        ...
        def _real_reduce(key, values):
            ...
            Args:
                key: bytes. A key value as emitted from the map() function,
                    above.
                values: list(bytes). A list of all values from all mappers that
                    were grouped with the given key.
