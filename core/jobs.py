...
class BaseMapReduceJobManagerMetaClass(type):
    ...
    converts all strings the class's "map" and "reduce" methods produce as
    bytes. This is necessary because the map reduce job pipelines expect bytes.

    Ideally, the methods would be implemented using class methods. Unfortunately
    however, map reduce job pipelines can only be implemented using static
    methods. As a consequence, we need to fall back to using custom static
    methods to wrap around the implementations, and that can only be
    accomplished using meta classes.
    """
