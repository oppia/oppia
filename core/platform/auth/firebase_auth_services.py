    ...
    try:
        ...
        if 'initialize_app' in str(error):
            firebase_admin.initialize_app(
                options={
                    'projectId': feconf.OPPIA_PROJECT_ID
                }
            )
        else:
            raise
...
def mark_user_for_deletion(user_id: str) -> None:
    ...
    ) = auth_models.UserAuthDetailsModel.get_multi([user_id], include_deleted=True)
    ...
    assoc_by_auth_id_model = (
        ...
        get_multi([assoc_by_user_id_model.firebase_auth_id], include_deleted=True)[0]
    )
...
def get_auth_id_from_user_id(user_id: str,
    ...
    """
    (assoc_by_user_id_model,
    ) = auth_models.UserAuthDetailsModel.get_multi([user_id],
                                                   include_deleted=include_deleted)
    return (
        None
        if assoc_by_user_id_model is None else assoc_by_user_id_model.firebase_auth_id
    )
...
def get_user_id_from_auth_id(auth_id: str,
                             include_deleted: bool = False) -> Optional[str]:
...
def associate_auth_id_with_user_id(
    ...
    if (
        assoc_by_user_id_model is None or
...
def associate_multi_auth_ids_with_user_ids(
    ...
    assoc_by_user_id_models = [
        ...
        for auth_id, user_id, assoc_by_user_id_model in
        zip(auth_ids, user_ids, auth_models.UserAuthDetailsModel.get_multi(user_ids))
        if (
            assoc_by_user_id_model is None or
            assoc_by_user_id_model.firebase_auth_id is None
        )
