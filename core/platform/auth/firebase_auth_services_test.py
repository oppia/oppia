...
UidsZipPartitionTupleType = Tuple[List[Tuple[int, Tuple[str, str]]],
                                  List[Tuple[int, Tuple[str, str]]]]

RecordsPartitionTupleType = Tuple[List[Tuple[int, Tuple[firebase_auth.ImportUserRecord,
                                                        str]]],
                                  List[Tuple[int, Tuple[firebase_auth.ImportUserRecord,
                                                        str]]]]
...
class FirebaseAdminSdkStub:
    ...
    def install(self, test: test_utils.TestBase) -> None:
        ...
        with contextlib.ExitStack() as swap_stack:
            ...
            for name in self._UNIMPLEMENTED_SDK_FUNCTION_NAMES:
                swap_stack.enter_context(
                    test.swap_to_always_raise(firebase_auth, name, NotImplementedError)
                )
    ...
    def delete_users(
        ...
        if force_delete:
            ...
            errors = [
                (i, 'uid=%r must be disabled first' % uid) for i, uid in enabled_uids
            ]
    ...
    def list_users(
        ...
        page_list = [[user for user in user_group if user is not None]
    ...
    def assert_is_user(self, uid: str) -> None:
        ...
        self._test.assertIn(
            uid, self._users_by_uid, msg='Firebase account not found: uid=%r' % uid
        )
    ...
    def assert_is_user_multi(self, uids: List[str]) -> None:
        ...
        assert self._test is not None
        self._test.assertEqual(
            not_found, [], msg='Firebase accounts not found: uids=%r' % (not_found,)
        )
    ...
    def assert_is_not_user_multi(self, uids: List[str]) -> None:
        ...
        assert self._test is not None
        self._test.assertEqual(
            found, [], msg='Unexpected Firebase accounts exists: uids=%r' % (found,)
        )
    ...
    def _decode_user_claims(
        ...
        try:
            ...
            return cast(
                Dict[str, Optional[Union[str, bool]]], json.loads(encoded_claims)
            )
        except ValueError:
            return None

    def _set_user_fragile(
        self, uid: str, email: Optional[str], disabled: bool,
    ...
    def _create_list_users_page_fragile(
        ...
        if page_index < len(page_list):
            ...
            page.get_next_page = lambda: (
                None if not page.has_next_page else self.
                _create_list_users_page_fragile(page_list, page_index + 1)
            )
...
class EstablishFirebaseConnectionTests(test_utils.TestBase):
    ...
    def test_returns_existing_connection(self) -> None:
        ...
        init_app_swap = self.swap_with_call_counter(
            firebase_admin, 'initialize_app', raises=Exception('unexpected call')
        )
...
class FirebaseAuthServicesTestBase(test_utils.AppEngineTestBase):
    ...
    def create_response(self, session_cookie: Optional[str] = None) -> webapp2.Response:
        ...
        if session_cookie:
            res.set_cookie(
                constants.FIREBASE_AUTH_SESSION_COOKIE_NAME, value=session_cookie
            )
        return res
...
class DestroyAuthSessionTests(FirebaseAuthServicesTestBase):
    ...
    def test_deletes_cookie_from_response(self) -> None:
        ...
        self.assert_matches_regexps(
            res.headers.get_all('Set-Cookie'), ['session=abc;', 'session=; Max-Age=0;']
        )
...
class GetAuthClaimsFromRequestTests(FirebaseAuthServicesTestBase):
    ...
    def test_returns_none_when_cookie_is_missing(self) -> None:
        ...
        self.assertIsNone(
            firebase_auth_services.get_auth_claims_from_request(self.create_request())
        )
        self.assertIsNone(
    ...
    def test_feconf_admin_email_address_is_super_admin(self) -> None:
        ...
        self.assertEqual(
            firebase_auth_services.get_auth_claims_from_request(
                self.create_request(session_cookie=cookie)
            ), auth_domain.AuthClaims(self.AUTH_ID, feconf.ADMIN_EMAIL_ADDRESS, True)
        )
...
class GenericAssociationTests(FirebaseAuthServicesTestBase):
    ...
    def test_present_association_is_not_considered_to_be_deleted(self) -> None:
        ...
        self.assertFalse(
            firebase_auth_services.verify_external_auth_associations_are_deleted('uid')
        )
...
class DeleteAuthAssociationsTests(FirebaseAuthServicesTestBase):
    ...
    def swap_get_users_to_return_non_empty_users_result(self) -> ContextManager[None]:
        ...
        return self.swap_to_always_return(
            firebase_auth, 'get_users',
            firebase_auth.GetUsersResult([firebase_auth.UserRecord({
                'localId': 'id'
            })], [])
        )
