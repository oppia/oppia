...
class CloudDatastoreServicesTests(test_utils.GenericTestBase):
    ...
    def test_update_timestamps_multi(self) -> None:
        ...
        self.assertEqual(
            self.user_query_model.get_by_id('query_id').last_updated, self.curr_time
        )
    ...
    def test_fetch_multiple_entities_throws_error_on_duplicate_parameters(self) -> None:
        ...
        with self.assertRaisesRegex(Exception, error_msg):
            cloud_datastore_services.fetch_multiple_entities_by_ids_and_models([
                ('UserQueryModel', ['query_id']), ('UserQueryModel', ['query_id'])
            ])
    ...
    def test_get_multi_throws_error_on_failure(self) -> None:
        ...
        with self.swap_to_always_raise(ndb, 'get_multi',
                                       Exception('Mock key error')), self.swap(
        ...
        self.assertEqual(
            observed_log_messages, [
                'Exception raised: Mock key error', 'Exception raised: Mock key error',
    ...
    def test_ndb_query_with_filters(self) -> None:
        ...
        results: Tuple[
            Sequence[cloud_datastore_services.Model], cloud_datastore_services.Cursor,
            bool] = user_models.UserQueryModel.query(
                user_models.UserQueryModel.submitter_id == self.admin_user_id,
