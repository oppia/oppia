...
class EmailTests(test_utils.GenericTestBase):
    ...
    def test_send_mail_logs_to_terminal(self) -> None:
        ...
        self.assertEqual(
            observed_log_messages, [logging_info_email_body, logging_info_notification]
        )
    ...
    def test_send_mail_to_multiple_recipients_logs_to_terminal(self) -> None:
        ...
        msg_body = (
            ...
            Attachments: %s
            """ % (
                feconf.SYSTEM_EMAIL_ADDRESS, recipient_email_list_str, 'subject', 4, 4,
                bcc_email_list_str, '123', len(recipient_variables), 'attachment.txt'
            )
        )
        logging_info_email_body = textwrap.dedent(msg_body)
        logging_info_notification = (
