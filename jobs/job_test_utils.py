...
class PipelinedTestBase(test_utils.TestBase):
    ...
    def flush_pipeline(self, strict=True):
        """Flushes the pipeline wrapping the test to end immediately.

        Args:
            strict: bool. Whether to raise an exception when the pipeline has
                been flushed twice.

        Raises:
            RuntimeError. The pipeline has been closed twice and strict is True.
        """
        if self._close_stack:
            self._close_stack()
            self._close_stack = None
        elif strict:
            raise RuntimeError('flush_pipeline() must be called exactly once')

    def assert_pcoll_equal(self, actual, expected):
        """Asserts that the given PCollections are equal.

        NOTE: Only one assert_pcoll_*() method may be called in a test.
    ...
    def new_model(self, model_class, **properties):
