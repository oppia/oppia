...
TYPESCRIPT_CHECKS_CMDS: Final = [PYTHON_CMD, '-m', 'scripts.run_typescript_checks']
...
class ChangedBranch:
    ...
    def __init__(self, new_branch: str) -> None:
        ...
        self.old_branch = subprocess.check_output(get_branch_cmd,
    ...
    for branch, (modified_files, files_to_lint) in collected_files.items():
        with ChangedBranch(branch):
            ...
            backend_associated_test_file_check_status = (
                run_script_and_get_returncode(BACKEND_ASSOCIATED_TEST_FILE_CHECK_CMD)
            )
            ...
            if ci_check_status != 0:
                print('Push aborted due to failing tests are captured '
                      'in ci check.')
                sys.exit(1)
            python_test_files = (
                git_changes_utils.get_python_dot_test_files_from_diff(files_to_lint)
            )
