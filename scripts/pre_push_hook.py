...
ACTIONS_CI_PROTRACTOR_CHECK_CMDS = [
...
def does_diff_include_actions_config_or_js_files(diff_files):
    """Returns true if diff includes Actions config or Javascript files.

    Args:
        diff_files: list(str). List of files changed.

    Returns:
        bool. Whether the diff contains changes in Actions config or
        Javascript files.
    """
    ...
    for branch, (modified_files, files_to_lint) in collected_files.items():
        with ChangedBranch(branch):
            ...
            if frontend_status != 0:
                ...
                sys.exit(1)
            if does_diff_include_actions_config_or_js_files(files_to_lint):
                actions_ci_check_status = run_script_and_get_returncode(
                    ACTIONS_CI_PROTRACTOR_CHECK_CMDS)
            if actions_ci_check_status != 0:
                python_utils.PRINT(
