...
class PreCommitLinterTests(test_utils.LinterTestBase):
    ...
    def test_main_with_no_args(self) -> None:
        ...
        get_changed_filepaths_swap = self.swap(
            run_lint_checks, '_get_changed_filepaths', mock_get_changed_filepaths
        )
    ...
    def test_main_with_path_arg(self) -> None:
        ...
        self.assertFalse(all_checks_passed(self.linter_stdout))
        self.assert_same_list_elements(['19:16', 'Unexpected whitespace before \":\"'],
                                       self.linter_stdout)
    ...
    def test_main_with_invalid_filepath_with_path_arg(self) -> None:
        ...
        self.assert_same_list_elements(['Could not locate file or directory'],
                                       self.linter_stdout)

    def test_main_with_invalid_filepath_with_file_arg(self) -> None:
        with self.print_swap, self.assertRaisesRegex(SystemExit, '1'):
            run_lint_checks.main(args=['--files=invalid_file.py'])
        self.assert_same_list_elements(['The following file(s) do not exist'],
                                       self.linter_stdout)
        ...
        with self.print_swap, self.sys_swap:
            with self.install_swap, self.oppia_is_dockerized_swap:
                run_lint_checks.main(
                    args=[
                        '--path=%s' %
                        VALID_TS_FILEPATH, '--only-check-file-extensions=ts'
                    ]
                )
        self.assertFalse(all_checks_passed(self.linter_stdout))
    ...
    def test_main_with_only_check_file_extensions_arg_with_js_ts_options(self) -> None:
        with self.print_swap, self.assertRaisesRegex(SystemExit, '1'):
            run_lint_checks.main(
                args=[
                    '--path=%s' %
                    VALID_TS_FILEPATH, '--only-check-file-extensions', 'ts', 'js'
                ]
            )
    ...
    def test_get_all_files_in_directory(self) -> None:
        with self.print_swap, self.sys_swap:
            with self.install_swap, self.oppia_is_dockerized_swap:
                run_lint_checks.main(
                    args=['--path=scripts/linters/', '--only-check-file-extensions=ts']
                )
    ...
    def test_html_file(self) -> None:
        ...
        self.assert_same_list_elements(['All Linter Checks Passed.'],
                                       self.linter_stdout)
