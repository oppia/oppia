...
class CustomHTMLParser(html.parser.HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: List[Tuple[str, Optional[str]]]) -> None:
        ...
        if tag_line.startswith(opening_tag) and (column_number != expected_indentation):
            error_message = (
                ...
                'of %s, found indentation of %s '
                'for %s tag on line %s ' %
                (self.filepath, expected_indentation, column_number, tag, line_number)
            )
            self.error_messages.append(error_message)
            self.failed = True
        ...
        for attr, value in attrs:
            ...
            if value:
                ...
                self._check_space_between_attributes_and_values(
                    tag, attr, value, rendered_text, value_in_quotes, attr_pos_mapping
                )
        ...
        if leading_spaces_count != last_starttag_col_num and (
            ...
            error_message = (
                ...
                'start tag %s on line %s ' % (
                    self.filepath, tag, line_number, last_starttag,
    ...
    def _check_space_between_attributes_and_values(
        self, tag: str, attr: str, value: str, rendered_text: str,
        value_in_quotes: bool, attr_pos_mapping: Dict[str, List[int]]
    ) -> None:
        ...
        if attr not in attr_pos_mapping:
            ...
            for match in re.finditer(re.escape(attr), rendered_text.lower()):
                ...
                if (
                    rendered_text[start - 1] in [' ', '"'] and
                    rendered_text[end] in [' ', '=']
                ):
