...
CODEOWNER_IMPORTANT_PATHS: Final = [
    '/dependencies.json', '/package.json', '/yarn.lock',
    '/scripts/install_third_party_libs.py', '/.github/CODEOWNERS', '/.github/stale.yml'
]
...
class CodeownerLinterTests(test_utils.LinterTestBase):
    ...
    def setUp(self) -> None:
        ...
        def mock_listdir(unused_arg: str) -> List[str]:
            return [
                'dependencies.json', 'package.json', 'yarn.lock',
                'scripts/install_third_party_libs.py', '.github/CODEOWNERS',
                '.github/stale.yml'
            ]
        ...
        def mock_listdir_with_blanket_codeowner_only_file(unused_arg: str) -> List[str]:
            return [
                'dependencies.json', 'package.json', 'yarn.lock',
                'scripts/install_third_party_libs.py', '.github/CODEOWNERS',
                '.github/stale.yml', 'core/domain/new_file.py'
            ]

        self.listdir_swap = self.swap(os, 'listdir', mock_listdir)
        self.listdir_swap_file_with_blanket_codeowner_only = (
            self.swap(os, 'listdir', mock_listdir_with_blanket_codeowner_only_file)
        )
    ...
    def test_duplicate_important_patterns_at_the_bottom_of_codeowners(self) -> None:
        codeowner_path_swap = self.swap(
            codeowner_linter, 'CODEOWNER_FILEPATH', INVALID_DUPLICATE_CODEOWNER_FILEPATH
        )
    ...
    def test_check_codeowner_file_with_success_message(self) -> None:
        ...
        codeowner_important_paths_swap = self.swap(
            codeowner_linter, 'CODEOWNER_IMPORTANT_PATHS', CODEOWNER_IMPORTANT_PATHS
        )
        ...
        with self.listdir_swap, codeowner_important_paths_swap:
            with codeowner_path_swap:
                linter = codeowner_linter.CodeownerLintChecksManager(FILE_CACHE)
                lint_task_report = linter.check_codeowner_file()
        self.assertEqual(['SUCCESS  CODEOWNERS check passed'],
    ...
    def test_check_file_with_only_blanket_codeowner_defined_fails(self) -> None:
        ...
        self.assert_same_list_elements([error_message],
                                       lint_task_report.trimmed_messages)
        self.assertEqual('CODEOWNERS', lint_task_report.name)
        self.assertTrue(lint_task_report.failed)
    ...
    def test_check_invalid_inline_comment_codeowner_filepath(self) -> None:
        ...
        with self.listdir_swap, codeowner_swap:
            linter = codeowner_linter.CodeownerLintChecksManager(FILE_CACHE)
            lint_task_report = linter.check_codeowner_file()
        self.assert_same_list_elements(['Please remove inline comment from line 17'],
                                       lint_task_report.trimmed_messages)
        self.assertEqual('CODEOWNERS', lint_task_report.name)
        self.assertTrue(lint_task_report.failed)
    ...
    def test_check_codeowner_file_without_codeowner_name(self) -> None:
        ...
        with self.listdir_swap, codeowner_swap:
            linter = codeowner_linter.CodeownerLintChecksManager(FILE_CACHE)
            lint_task_report = linter.check_codeowner_file()
        self.assert_same_list_elements(['Pattern on line 18 doesn\'t have codeowner'],
                                       lint_task_report.trimmed_messages)
        self.assertEqual('CODEOWNERS', lint_task_report.name)
        self.assertTrue(lint_task_report.failed)
    ...
    def test_check_codeowner_file_with_wildcard(self) -> None:
        codeowner_swap = self.swap(
            codeowner_linter, 'CODEOWNER_FILEPATH', INVALID_WILDCARD_IN_FILEPATH
        )
    ...
    def test_get_linters(self) -> None:
        ...
        self.assertTrue(
            isinstance(custom_linter, codeowner_linter.CodeownerLintChecksManager)
        )
