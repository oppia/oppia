...
class JsTsLintTests(test_utils.LinterTestBase):
    ...
    def validate(
        self, lint_task_report: List[concurrent_task_utils.TaskResult],
        expected_messages: List[str], failed_count: int
    ) -> None:
    ...
    def test_compile_all_ts_files_with_error(self) -> None:
        ...
        def mock_popen_error_call(
            ...
            *args: str,
            **kwargs: str  # pylint: disable=unused-argument
        ) -> Ret:
            return Ret()
    ...
    def test_validate_and_parse_js_and_ts_files_with_exception(self) -> None:
        ...
        with esprima_swap, compile_all_ts_files_swap, self.assertRaisesRegex(
            ...
            js_ts_linter.JsTsLintChecksManager([], [VALID_JS_FILEPATH],
    ...
    def test_check_constants_declaration(self) -> None:
        ...
        def mock_compile_all_ts_files() -> None:
            cmd = (
                ...
                js_ts_linter.COMPILED_TYPESCRIPT_TMP_PATH, 'true', 'es2017,dom', 'true',
                'true', 'es5', './node_modules/@types', INVALID_CONSTANT_AJS_FILEPATH,
    ...
    def test_check_duplicate_constant_declaration_in_separate_files(self) -> None:
        ...
        def mock_compile_all_ts_files() -> None:
            cmd = (
                ...
                js_ts_linter.COMPILED_TYPESCRIPT_TMP_PATH, 'true', 'es2017,dom', 'true',
                'true', 'es5', './node_modules/@types', INVALID_CONSTANT_IN_TS_FILEPATH
            )
            subprocess.call(cmd, shell=True, stdout=subprocess.PIPE)
        ...
        with compile_all_ts_files_swap:
            lint_task_report = js_ts_linter.JsTsLintChecksManager([], [
                INVALID_CONSTANT_IN_TS_FILEPATH, INVALID_CONSTANT_IN_TS_FILEPATH
            ], FILE_CACHE).perform_all_lint_checks()
        shutil.rmtree(js_ts_linter.COMPILED_TYPESCRIPT_TMP_PATH, ignore_errors=True)
        expected_messages = [
    ...
    def test_third_party_linter_with_invalid_eslint_path(self) -> None:
        ...
        with exists_swap, self.assertRaisesRegex(
            Exception, 'ERROR    Please run start.py first to install node-eslint and '
            'its dependencies.'
        ):
            js_ts_linter.ThirdPartyJsTsLintChecksManager([
                INVALID_SORTED_DEPENDENCIES_FILEPATH
    ...
    def test_custom_linter_with_no_files(self) -> None:
        lint_task_report = js_ts_linter.JsTsLintChecksManager([], [], FILE_CACHE
                                                             ).perform_all_lint_checks()
        self.assertEqual([
    ...
    def test_angular_services_index_error(self) -> None:
        ...
        def mock_compile_all_ts_files() -> None:
            cmd = (
                ...
                js_ts_linter.COMPILED_TYPESCRIPT_TMP_PATH, 'true', 'es2017,dom', 'true',
                'true', 'es5', './node_modules/@types', VALID_UNLISTED_SERVICE_PATH
            )
            subprocess.call(cmd, shell=True, stdout=subprocess.PIPE)
        ...
        expected_messages = [
            ...
            'from %s' %
            (class_name, angular_services_index_path, VALID_UNLISTED_SERVICE_PATH),
    ...
    def test_angular_services_index_success(self) -> None:
        ...
        def mock_compile_all_ts_files() -> None:
            cmd = (
                ...
                js_ts_linter.COMPILED_TYPESCRIPT_TMP_PATH, 'true', 'es2017,dom', 'true',
                'true', 'es5', './node_modules/@types', VALID_IGNORED_SERVICE_PATH
            )
            subprocess.call(cmd, shell=True, stdout=subprocess.PIPE)
    ...
    def test_get_linters_with_success(self) -> None:
        custom_linter, third_party = js_ts_linter.get_linters([VALID_JS_FILEPATH],
                                                              [VALID_TS_FILEPATH],
                                                              FILE_CACHE)
        self.assertTrue(isinstance(custom_linter, js_ts_linter.JsTsLintChecksManager))
        self.assertTrue(
            isinstance(third_party, js_ts_linter.ThirdPartyJsTsLintChecksManager)
        )
