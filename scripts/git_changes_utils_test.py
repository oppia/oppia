...
class GitChangesUtilsTests(test_utils.GenericTestBase):
    ...
    def test_get_upstream_remote_name_without_errors(self) -> None:
        ...
        with popen_swap:
            self.assertEqual(
                git_changes_utils.get_upstream_git_repository_remote_name(), 'upstream'
            )

    def test_get_upstream_remote_name_with_error_in_obtaining_remote(self) -> None:

        def mock_communicate() -> Tuple[bytes, bytes]:
            return (b'test', b'test_oppia_error')

        process = subprocess.Popen([b'echo', b'test'],
                                   stdout=subprocess.PIPE,
                                   stderr=subprocess.PIPE)
        ...
        popen_swap = self.swap_with_checks(
            subprocess, 'Popen', mock_popen, expected_args=[(['git', 'remote'],)]
        )
    ...
    def test_get_upstream_remote_name_with_error_in_obtaining_remote_url(self) -> None:
    ...
    def test_get_upstream_remote_name_with_no_remote_set(self) -> None:
        process_for_remote = subprocess.Popen([b'echo', b''],
                                              stdout=subprocess.PIPE,
                                              stderr=subprocess.PIPE)

        def mock_popen(
            cmd_tokens: List[bytes],
            stdout: int,
            stderr: int  # pylint: disable=unused-argument
        ) -> subprocess.Popen[bytes]:  # pylint: disable=unsubscriptable-object
            return process_for_remote
        ...
        with popen_swap, self.assertRaisesRegex(
            Exception, 'Error: Please set the git \'upstream\' repository.\n'
    ...
    def test_get_upstream_remote_name_with_multiple_remotes(self) -> None:
        ...
        with popen_swap, self.assertRaisesRegex(
            Exception, 'Error: Please keep only one remote branch for oppia:develop.\n'
            'To do that follow these steps:\n'
            '1. Run the command \'git remote -v\'\n'
    ...
    def test_get_local_remote_name_without_errors(self) -> None:
        ...
        with popen_swap:
            self.assertEqual(
                git_changes_utils.get_local_git_repository_remote_name(), 'origin'
            )
    ...
    def test_get_local_remote_name_with_error_in_obtaining_remote_url(self) -> None:
    ...
    def test_get_local_remote_name_with_no_remote_set(self) -> None:
        ...
        with popen_swap, self.assertRaisesRegex(
            Exception, 'Error: Please set the git \'origin\' repository.\n'
            'To do that follow these steps:\n'
            '1. Run the command \'git remote -v\'\n'
    ...
    def test_get_local_remote_name_with_multiple_remotes(self) -> None:
        ...
        with popen_swap, self.assertRaisesRegex(
            Exception, 'Error: Please keep only one remote branch for your Oppia fork.'
            '\nTo do that follow these steps:\n'
            '1. Run the command \'git remote -v\'\n'
            '2. This command will list the remote references. There will be '
    ...
    def test_git_diff_name_status_with_diff_filter(self) -> None:
        ...
        subprocess_swap = self.swap_with_checks(
            ...
            mock_start_subprocess_for_result,
            expected_args=[([
                'git', 'diff', '--name-status', '--diff-filter=filter', 'left', 'right',
                '--'
            ],)]
        )
    ...
    def test_compare_to_remote_with_file_not_in_oppia_directory_should_error(
        ...
        with subprocess_swap, git_diff_swap, get_merge_base_swap:
            with curr_dir_swap, self.assertRaisesRegex(
                ValueError, 'Error: The file /usr/directory/file2.py is not inside the '
                'oppia directory.'
            ):
                git_changes_utils.compare_to_remote(
                    'remote', 'local branch', 'remote/local branch'
    ...
    def test_get_parent_branch_name_for_diff_with_hotfix_branch(self) -> None:
        ...
        with get_branch_swap:
            self.assertEqual(
                git_changes_utils.get_parent_branch_name_for_diff(), 'release-1.2.3'
            )
    ...
    def test_get_changed_files_with_non_empty_ref_list(self) -> None:
        ...
        def mock_compare_to_remote(
            unused_remote: str, unused_local_branch: str, unused_remote_branch: str
        ) -> List[str]:
    ...
    def test_get_changed_files_with_no_remote_branch(self) -> None:
        ...
        get_parent_branch_name_for_diff_swap = self.swap(
            git_changes_utils, 'get_parent_branch_name_for_diff', lambda: 'develop'
        )
    ...
    def test_get_refs_with_empty_remote_sha(self) -> None:
        with tempfile.NamedTemporaryFile() as temp_stdin_file:
            with utils.open_file(temp_stdin_file.name, 'w') as f:
                f.write(
                    'local_ref local_sha1 remote_ref %s' % git_changes_utils.EMPTY_SHA1
                )
            with utils.open_file(temp_stdin_file.name, 'r') as f:
                with self.swap(sys, 'stdin', f):
                    self.assertEqual(
    ...
    def test_get_python_dot_test_files_from_diff_with_test_file(self) -> None:
        ...
        os_path_exists_swap = self.swap_with_checks(
            ...
            'exists',
            mock_os_path_exists,
            expected_args=[('test/file1_test.py',), ('test/file2_test.py',),
                           ('test/file3_test.py',)]
        )
    ...
    def test_get_python_dot_test_files_from_diff_with_no_test_file(self) -> None:
    ...
    def test_get_python_dot_test_files_from_diff_with_existing_test_file(self) -> None:
    ...
    def test_get_changed_python_test_files(self) -> None:
        ...
        def mock_get_staged_acmrt_files() -> List[bytes]:
            return [b'test/file1.py', b'file2.ts', b'test/file3.py', b'test/file4.py']
        ...
        def mock_get_python_dot_test_files_from_diff(
            ...
            if diff_files == [b'test/file1.py', b'file2.ts', b'test/file3.py']:
                ...
                return {
                    'test.file1_test.py', 'test.file3_test.py', 'test.file4_test.py'
                }
            return set()
        ...
        get_staged_acmrt_files_swap = self.swap(
            git_changes_utils, 'get_staged_acmrt_files', mock_get_staged_acmrt_files
        )
                    ...
                    with get_parent_branch_name_for_diff_swap:
                        self.assertEqual(
                            git_changes_utils.get_changed_python_test_files(), {
                                'test.file1_test.py', 'test.file3_test.py',
                                'test.file4_test.py'
                            }
                        )
