...
class CheckCITestSuitesToRunTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
            ...
            f.write(
                json.dumps({
                    'RUN_NO_TESTS_ROOT_FILES': [
                        'README.md', 'assets/README.md', 'CODEOWNERS'
                    ],
        ...
        with open(
            os.path.join(
                acceptance_test_modules_mapping_directory, 'blog-admin/assign-roles.txt'
            ),
            'w',
            encoding='utf-8'
        ) as f:
        ...
        with open(
            os.path.join(
                acceptance_test_modules_mapping_directory, 'blog-editor/publish.txt'
            ),
            'w',
            encoding='utf-8'
        ) as f:
    ...
    def get_test_suites_to_run_from_github_output(
        ...
        with open(self.github_output_file_path, 'r', encoding='utf-8') as f:
            ...
            test_suites: Sequence[check_ci_test_suites_to_run.GenericTestSuiteDict
        ...
        with swap_popen:
            ...
            self.assertEqual(
                git_diff_name_status_files, [
                    ...
                    'core/constants.py', 'core/utils.py', 'core/templates/pages/Base.ts'
                ]
            )
    ...
    def test_partition_lighthouse_pages_into_test_suites_one_shard_output(self) -> None:
        lighthouse_pages: List[check_ci_test_suites_to_run.LighthousePageDict] = [{
            'name': 'splash',
            'url': 'http://localhost:8181/',
            'page_module': 'splash-page.module.ts'
        ...
        self.assertEqual(
            check_ci_test_suites_to_run.partition_lighthouse_pages_into_test_suites(
                ...
                'environment':
                    'python',
                'pages_to_run': [
                    'splash', 'about', 'terms', 'privacy-policy', 'exploration-player'
                ]
            }]
        )
            ...
            with self.ci_test_suite_configs_directory_swap, self.test_modules_mapping_directory_swap:  # pylint: disable=line-too-long
                with self.root_files_config_file_path_swap, self.generate_root_files_mapping_swap:  # pylint: disable=line-too-long
                    check_ci_test_suites_to_run.main([
                        '--github_base_ref', 'base', '--github_head_ref', 'head',
        ...
        with self.root_files_mapping_file_path_swap, self.lighthouse_pages_config_file_path_swap:  # pylint: disable=line-too-long
            with self.ci_test_suite_configs_directory_swap, self.test_modules_mapping_directory_swap:  # pylint: disable=line-too-long
                with self.root_files_config_file_path_swap, self.generate_root_files_mapping_swap:  # pylint: disable=line-too-long
                    with self.swap(
                        check_ci_test_suites_to_run, 'get_git_diff_name_status_files',
                        lambda *args: ['README.md', 'assets/README.md', 'CODEOWNERS']
                    ):
                        check_ci_test_suites_to_run.main([
                            '--github_base_ref', 'base', '--github_head_ref', 'head'
                        ])
            ...
            with self.ci_test_suite_configs_directory_swap, self.test_modules_mapping_directory_swap:  # pylint: disable=line-too-long
                with self.root_files_config_file_path_swap, self.generate_root_files_mapping_swap:  # pylint: disable=line-too-long
                    with self.swap(
                        check_ci_test_suites_to_run, 'get_git_diff_name_status_files',
                        lambda *args: [
                            'splash-banner.component.html', 'about-component.ts',
                            'terms.component.html', 'exploration-player.component.html',
                            'exploration-player-banners.component.html'
                        ]
                    ):
                        check_ci_test_suites_to_run.main([
                            ...
                            self.get_test_suites_to_run_from_github_output(),
                            {
                                ...
                                'acceptance': {
                                    'docker': {
                                        ...
                                        'suites': [{
                                            'name':
                                                'exploration-player/view-exploration',  # pylint: disable=line-too-long
                                            'module':
                                                'exploration-player/view-exploration.spec.ts',  # pylint: disable=line-too-long
                                            'environment':
                                                'docker'
                                        }]
                                    },
                                ...
                                'lighthouse_performance': {
                                    'suites': [{
                                        'name':
                                            '1',
                                        'module':
                                            '.lighthouserc-performance.js',  # pylint: disable=line-too-long
                                        'environment':
                                            'python',
                                        'pages_to_run': [
                                            'about', 'exploration-player', 'splash',
                                            'terms'
                                        ]
                                    }],
                                    'count': 1
                                ...
                                'lighthouse_accessibility': {
                                    'suites': [{
                                        'name':
                                            '1',
                                        'module':
                                            '.lighthouserc-accessibility.js',  # pylint: disable=line-too-long
                                        'environment':
                                            'python',
                                        'pages_to_run': [
                                            'about', 'exploration-player', 'splash',
                            ...
                            self.get_test_suites_to_run_from_github_output(),
                            {
                                ...
                                'acceptance': {
                                    ...
                                    'python': {
                                        'suites': [{
                                            'name':
                                                'blog-admin/create-blog-post',  # pylint: disable=line-too-long
                                            'module':
                                                'blog-admin/create-blog-post.spec.ts'  # pylint: disable=line-too-long
                                        }],
                                        'count': 1
                                    }
                                },
