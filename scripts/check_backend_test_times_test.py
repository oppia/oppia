...
class CheckBackendTestTimesTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        super().setUp()
        self.backend_test_time_reports_directory = (tempfile.TemporaryDirectory())
    ...
    def test_get_sorted_backend_test_times_from_reports_no_reports(self) -> None:
        ...
        with backend_test_time_reports_swap:
            with self.assertRaisesRegex(
                RuntimeError, 'No backend test time reports found in %s. Please run '
    ...
    def test_get_sorted_backend_test_times_from_reports(self) -> None:
        ...
        self.assertEqual(sorted_backend_test_times, self.sorted_backend_test_times)
        self.assertEqual(
    ...
    def test_check_backend_test_times_creates_correct_file(self) -> None:
        ...
        for line in backend_test_times_temp_file.readlines():
            ...
            sorted_backend_test_times_from_file.append({
                ...
                'test_time_by_average_test_case': float(test_time_by_average_test_case)
            })
        self.assertEqual(
            sorted_backend_test_times_from_file, self.sorted_backend_test_times
        )
        sorted_backend_test_times_message = [
            '\033[1mBACKEND TEST TIMES SORTED BY TIME:\033[0m',
            'test_six: 1.1 SECONDS.', 'test_five: 1.2 SECONDS.',
            'test_seven: 1.3 SECONDS.', 'test_ten: 1.4 SECONDS.',
            'test_two: 1.4 SECONDS.', 'test_eight: 1.5 SECONDS.',
            'test_nine: 1.6 SECONDS.', 'test_three: 1.7 SECONDS.',
            'test_one: 1.8 SECONDS.', 'test_four: 2.3 SECONDS.',
