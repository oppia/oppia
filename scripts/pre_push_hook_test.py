...
class PrePushHookTests(test_utils.GenericTestBase):
    ...
    def test_get_remote_name_with_error_in_obtaining_remote(self) -> None:
        ...
        with subprocess.Popen(
            [b'echo', b'test'], stdout=subprocess.PIPE,
            stderr=subprocess.PIPE) as process:
            # Here we use MyPy ignore because here we are assigning a value to
            # the 'communicate' method, and according to MyPy, assignment to a
    ...
    def test_get_refs(self) -> None:
        with tempfile.NamedTemporaryFile() as temp_stdin_file:
            temp_stdin_file_name = temp_stdin_file.name
            with utils.open_file(temp_stdin_file_name, 'w') as f:
                f.write('local_ref local_sha1 remote_ref remote_sha1')
            with utils.open_file(temp_stdin_file_name, 'r') as f:
