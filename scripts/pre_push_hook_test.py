...
class PrePushHookTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.ts_swap = self.swap(
            pre_push_hook, 'does_diff_include_ts_files', mock_does_diff_include_ts_files
        )
    ...
    def test_install_hook_with_existing_symlink(self) -> None:
        ...
        subprocess_swap = self.swap(
            common, 'start_subprocess_for_result', mock_start_subprocess_for_result
        )
    ...
    def test_does_diff_include_ci_config_or_test_files(self) -> None:
        ...
        self.assertTrue(
            pre_push_hook.does_diff_include_ci_config_or_test_files([
                b'core/tests/puppeteer-acceptance-tests/specs/test.spec.ts', b'test.ts'
            ])
        )
    ...
    def test_error_while_branch_change(self) -> None:
        ...
        self.assertIn(
            ...
            'that is being linted or stash your changes.', self.print_arr
        )
                    ...
                    with self.execute_mypy_checks_swap:
                        with self.assertRaisesRegex(SystemExit, '1'):
                            with self.swap_check_backend_python_libs:
                                pre_push_hook.main(args=[])
        self.assertTrue(
            'Push failed, please correct the linting issues above.' in self.print_arr
        )
                            ...
                            with self.execute_mypy_checks_swap:
                                with self.assertRaisesRegex(SystemExit, '1'):
                                    with self.swap_check_backend_python_libs:
                                        pre_push_hook.main(args=[])
        self.assertTrue(
            'Push aborted due to failing tests are captured in ci check.' in
            self.print_arr
        )
    ...
    def test_main_with_install_arg(self) -> None:
        ...
        with self.swap(pre_push_hook, 'install_hook',
                       mock_install_hook), (self.swap_check_backend_python_libs):
