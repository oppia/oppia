...
class PrePushHookTests(test_utils.GenericTestBase):
            ...
            with self.get_changed_files_swap, self.uncommitted_files_swap:
                with check_output_swap, self.assertRaisesRegex(SystemExit, '1'):
                    with self.swap_check_backend_python_libs:
                        pre_push_hook.main(args=[])
        self.assertIn(
            '\nCould not change branch to branch1. This is most probably because you are in a dirty state. Change manually to the branch that is being linted or stash your changes.',
            self.print_arr,
        )
                    ...
                    with self.execute_mypy_checks_swap:
                        with self.assertRaisesRegex(SystemExit, '1'):
                            with self.swap_check_backend_python_libs:
                                pre_push_hook.main(args=[])
        self.assertIn(
            'Push failed, please correct the mypy type annotation issues above.',
            self.print_arr,
        )
    ...
    def test_main_skips_linter_when_files_to_lint_is_empty(self) -> None:
        ...
        start_linter_mock = mock.Mock(return_value=0)
        run_script_mock = mock.Mock(return_value=0)
        exit_mock = mock.Mock()
