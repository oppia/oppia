...
class PrePushHookTests(test_utils.GenericTestBase):
    ...
    def setUp(self):
        ...
        self.does_diff_include_actions_config_or_js_files = False
        def mock_does_diff_include_actions_config_or_js_files(
                unused_diff_files):
            return self.does_diff_include_actions_config_or_js_files
        ...
        self.actions_config_or_js_files_swap = self.swap(
            pre_push_hook,
            'does_diff_include_actions_config_or_js_files',
            mock_does_diff_include_actions_config_or_js_files)
    ...
    def test_does_diff_include_actions_config_or_js_files(self):
        self.assertTrue(
            pre_push_hook.does_diff_include_actions_config_or_js_files(
                ['file1.js', 'protractor.conf.js', 'e2e_dummy.yml']))

    def test_does_diff_include_actions_config_or_js_files_fail(self):
        self.assertFalse(
            pre_push_hook.does_diff_include_actions_config_or_js_files(
    ...
    def test_invalid_actions_e2e_test_suites_failure(self):
        self.does_diff_include_actions_config_or_js_files = True
            ...
            with self.collect_files_swap, self.uncommitted_files_swap:
                with self.check_output_swap, self.start_linter_swap:
                    with run_script_and_get_returncode_swap:
                        with self.actions_config_or_js_files_swap:
                            with self.assertRaisesRegexp(SystemExit, '1'):
                                with self.swap_check_backend_python_libs:
                                    pre_push_hook.main(args=[])
        self.assertTrue(
