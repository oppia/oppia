...
def _normalize_directory_name(directory_name):
    ...
    different case won't treated as different in code. For example,
    `GoogleAppEnginePipeline-1.9.22.1.dist-info` and
    `googleappenginepipeline-1.9.22.1.dist-info` are equivalent, generated by
    the same python installation. To make sure that the directory names are not
    ambiguous, we use this method to enforce that all directories are lowercase.

    Args:
        directory_name: str. The directory name to be normalized.

    Returns:
        str. A normalized directory name that is all lowercase.
    """
...
def _get_requirements_file_contents():
    ...
    with python_utils.open_file(
        ...
        for l in lines:
            ...
            # Libraries with different case are equivalent libraries: e.g
            # 'Flask' is the same library as 'flask'. Therefore, we normalize
            # all library names in order to compare libraries without
            # ambiguities.
            library_name = (
...
def _remove_metadata(library_name, version_string):
    ...
    directory_names = [
        name for name in os.listdir(common.THIRD_PARTY_PYTHON_LIBS_DIR)
        if os.path.isdir(
    ...
    for directory_name in directory_names:
        ...
        if (_normalize_directory_name(
                directory_name) in possible_directory_names):
            path_to_delete = os.path.join(
                common.THIRD_PARTY_PYTHON_LIBS_DIR, directory_name)
