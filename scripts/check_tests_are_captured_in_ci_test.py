...
class CheckTestsAreCapturedInCiTest(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.dummy_e2e_directory = os.path.join(self.temp_directory.name, 'webdriverio')
        os.mkdir(self.dummy_e2e_directory)
        self.dummy_e2e_webdriverio_config_file = os.path.join(self.dummy_e2e_directory, 'wdio.conf.js')
        with open(self.dummy_e2e_webdriverio_config_file, 'w+', encoding='utf-8') as f:
            f.write(E2E_TEST_CONFIG_FILE_CONTENT)
        self.dummy_acceptance_directory = os.path.join(self.temp_directory.name, 'acceptance')
        os.mkdir(self.dummy_acceptance_directory)
        self.dummy_acceptance_specs_directory = os.path.join(self.dummy_acceptance_directory, 'specs')
        self.dummy_acceptance_old_directory = os.path.join(self.dummy_acceptance_directory, 'specs-old')
        ...
        for suite in ACCEPTANCE_TEST_SUITES:
            ...
            os.mknod(suite_file)
        self.ci_test_suite_configs_directory = os.path.join(self.temp_directory.name, 'ci-test-suite-configs')
        os.mkdir(self.ci_test_suite_configs_directory)
        e2e_ci_test_suite_config_file = os.path.join(self.ci_test_suite_configs_directory, 'e2e.json')
        with open(e2e_ci_test_suite_config_file, 'w+', encoding='utf-8') as f:
            f.write(json.dumps({'suites': E2E_TEST_SUITES}))
        acceptance_ci_test_suite_config_file = os.path.join(self.ci_test_suite_configs_directory, 'acceptance.json')
        with open(acceptance_ci_test_suite_config_file, 'w+', encoding='utf-8') as f:
    ...
    def test_compute_test_suites_difference(self) -> None:
        ...
        test_suites_difference = check_tests_are_captured_in_ci.compute_test_suites_difference(test_suites_one, test_suites_two)
        self.assertEqual(
    ...
    def test_read_webdriverio_config_file(self) -> None:
        with self.e2e_webdriverio_config_file_swap:
            webdriverio_config_file_content = check_tests_are_captured_in_ci.read_webdriverio_config_file()
            self.assertEqual(webdriverio_config_file_content, E2E_TEST_CONFIG_FILE_CONTENT)

    def test_get_e2e_test_modules_from_webdriverio_directory(self) -> None:
        webdriverio_files_path = os.path.join(os.getcwd(), 'core', 'tests', 'webdriverio')
        webdriverio_desktop_files_path = os.path.join(os.getcwd(), 'core', 'tests', 'webdriverio_desktop')
        ...
        with get_acceptance_test_suites_from_ci_config_file_swap:
            with get_acceptance_test_suites_from_acceptance_directory_swap:
                with self.assertRaisesRegex(
                    Exception,
                    re.escape('Acceptance test suites and CI test suites are not in sync. The following suites are not in sync: %s. Please update the CI config file for acceptance tests at core/tests/ci-test-suite-configs/acceptance.json with the suites listed above.' % (json.dumps(ACCEPTANCE_TEST_SUITES[1:3]))),
                ):
    ...
    def test_check_tests_are_captured_in_ci_with_wdio_error(self) -> None:
        ...
        with get_e2e_test_modules_from_webdriverio_directory_swap:
            with get_e2e_test_modules_from_webdriverio_config_file_swap:
                with self.assertRaisesRegex(
                    Exception,
                    'One or more test module from webdriverio or webdriverio_desktop directory is missing from wdio.conf.js',
                ):
                    check_tests_are_captured_in_ci.main()
    ...
    def test_check_tests_are_captured_in_ci_with_e2e_error(self) -> None:
        ...
        with get_e2e_test_suites_from_ci_config_file_swap:
            with get_e2e_test_suites_from_webdriverio_config_file_swap:
                with self.assertRaisesRegex(
                    Exception,
                    re.escape('E2E test suites and CI test suites are not in sync. The following suites are not in sync: %s. Please update the CI config file for e2e tests at core/tests/ci-test-suite-configs/e2e.json with the suites listed above.' % (json.dumps(E2E_TEST_SUITES[1:2]))),
                ):
                    check_tests_are_captured_in_ci.main()
