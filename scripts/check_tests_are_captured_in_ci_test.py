        ...
        with get_acceptance_test_suites_from_ci_config_file_swap:
            with get_acceptance_test_suites_from_acceptance_directory_swap:
                with self.assertRaisesRegex(
                    Exception,
                    re.escape(
                        'Acceptance test suites and CI test suites are not in sync. The following suites are not in sync: %s. Please update the CI config file for acceptance tests at core/tests/ci-test-suite-configs/acceptance.json with the suites listed above.'
        ...
        with get_e2e_test_suites_from_ci_config_file_swap:
            with get_e2e_test_suites_from_webdriverio_config_file_swap:
                with self.assertRaisesRegex(
                    Exception,
                    re.escape(
                        'E2E test suites and CI test suites are not in sync. The following suites are not in sync: %s. Please update the CI config file for e2e tests at core/tests/ci-test-suite-configs/e2e.json with the suites listed above.'
