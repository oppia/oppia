...
class CheckFrontendCoverageTests(test_utils.GenericTestBase):
    ...
    def test_get_stanzas_from_lcov_file(self) -> None:
        ...
        with self.open_patch:
            ...
            self.assertEqual(stanzas[1].file_path, '/opensource/oppia/file2.ts')
            ...
            self.assertEqual(stanzas[2].file_path, '/opensource/oppia/file3.ts')
    ...
    def test_check_coverage_changes(self) -> None:
        ...
        sys_exit_patch = mock.patch.object(sys, 'exit', mock_sys_exit)
        with (
            sys_exit_patch
        ), self.exists_patch, self.open_patch, self.print_patch:  # pylint: disable=line-too-long
            with not_fully_covered_files_patch:
        ...
        with self.exists_patch, self.open_patch, self.print_patch:
            with not_fully_covered_files_patch, self.capture_logging() as logs:
                ...
                self.assertEqual(
                    ...
                    [
                        '\033[1m/opensource/oppia/file.ts\033[0m seems to be '
                        'fully covered! Before removing it manually from the '
                        'denylist in the file '
        ...
        with self.exists_patch, self.open_patch, self.print_patch:
            with not_fully_covered_files_patch, self.capture_logging() as logs:
                ...
                self.assertEqual(
                    logs,
                    [
                        '\033[1m/opensource/oppia/newfilename.ts\033[0m seems '
                        'to be not completely tested. Make sure it\'s fully '
                        'covered.\n\033[1m/opensource/oppia/file.ts\033[0m is '
                        'in the frontend test coverage denylist but it doesn\'t'
                        ' exist anymore. If you have renamed it, please make '
                        'sure to remove the old file path and add the new file '
                        'path in the denylist in the file scripts/'
    ...
    def test_fully_covered_filenames_is_sorted(self) -> None:
        ...
        not_fully_covered_files_patch = mock.patch.object(
            check_frontend_test_coverage,
            'NOT_FULLY_COVERED_FILENAMES',
            ['/opensource/oppia/anotherfile.tsfile.ts'],
        )
        ...
        sys_exit_patch = mock.patch.object(sys, 'exit', mock_sys_exit)
        with sys_exit_patch, self.exists_patch, self.open_patch:
            with self.print_patch, not_fully_covered_files_patch:
                (
    ...
    def test_check_frontend_test_coverage_with_files_to_check(self) -> None:
        ...
        with self.check_call_patch, self.exists_patch, self.open_patch:
            with not_fully_covered_files_patch, self.capture_logging() as logs:
                with self.assertRaisesRegex(SystemExit, '1'):
