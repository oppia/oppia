...
def is_file_excluded(file_path: str) -> bool:
    ...
    """
    exclude_criteria = [
        file_path.startswith(exclude_directory)
...
def get_search_files(repository_path: str) -> List[str]:
    ...
    for root, _, files in os.walk(repository_path):
        for file in files:
            ...
            if not is_file_excluded(relative_file_path) and os.path.isfile(
                file_path
            ):
    ...
    for file_path in search_files:
        with open(file_path, 'r', encoding='utf-8', errors='replace') as file:
            for line_index, line_content in enumerate(file, start=1):
                todo: Optional[TodoDict] = get_todo_in_line(
                    file_path, line_content, line_index
                )
                if todo:
                    todos.append(todo)
    return todos
