...
class MockUrlOpenResponse:
    ...
    def read(self) -> 'MockUrlOpenResponse.MockBody':
...
class GithubApiTests(test_utils.GenericTestBase):
    ...
    def test_deep_get(self) -> None:
        self.assertEqual(
            github_api.deep_get({'a': {'b': {'c': 1}}}, ['a', 'b', 'c']), 1
        )
        self.assertEqual(
            github_api.deep_get({'a': {'b': {'c': 'string'}}}, ['a', 'b', 'c']),
            'string',
        )
        self.assertEqual(
            github_api.deep_get({'a': {'b': {'c': 1}}}, ['a', 'b']), {'c': 1}
        ...
        self.assertEqual(
            github_api.deep_get({'a': {'b': {'c': 1}}}, ['a', 'd']), None
        )

    @contextlib.contextmanager
    def swap_to_successful_gh_subprocess_run(self) -> Iterator[None]:
    ...
    def swap_to_graphql_urlopen(
        ...
        def mock_urlopen(request: urlrequest.Request) -> MockUrlOpenResponse:
            self.assertEqual(
            ...
            self.assertEqual(
                request.data,
                json.dumps({'query': expected_constructed_query}).encode(
                    'utf-8'
                ),
            )
    ...
    def test_bad_github_auth_token_throws_error(self) -> None:
        ...
        swap_subprocess_run = self.swap(subprocess, 'run', mock_subprocess_run)
        error_message = 'Failed to get GitHub auth token from the GitHub CLI.'
        with swap_subprocess_run, self.assertRaisesRegex(
            RuntimeError, error_message
        ):
            github_api.get_github_auth_token()
    ...
    def test_run_graphql_query_unauthorized_throws_error(self) -> None:
        ...
        swap_urlopen = self.swap(urlrequest, 'urlopen', mock_urlopen)
        error_message = (
    ...
    def test_fetch_latest_comment_for_issue_success(self) -> None:
        ...
        body = {
            'issue': {
                'comments': {'nodes': [{'body': 'Comment Body Number 1'}]}
            }
        }
        with self.swap_to_successful_gh_subprocess_run():
            with self.swap_to_graphql_urlopen(body, 200, query):
