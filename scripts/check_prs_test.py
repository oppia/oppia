...
class PullRequestMonitorTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        super().setUp()
        # Here we use type Any because the mock headers may include various
        # types of authentication tokens and accept headers.
        self.mock_headers: Dict[str, Any] = {
            'Authorization': 'Bearer test_token',
            'Accept': 'application/vnd.github.v3+json'
        }
        # Here we use type Any to mock complex PR response structures.
        ...
        # Here we use type Any to mock commit data structures.
        ...
        # Here we use type Any to mock issue response data.
    ...
    def test_main_workflow(self, mock_datetime: Any) -> None:
        # Here we use type Any because the datetime mock can have various
        # attributes and methods that are dynamically set.
        ...
        with (
            mock.patch(
                'scripts.check_prs.get_prs',
                return_value=[active_pr, warning_pr, stale_pr]
            ),
            mock.patch(
                'scripts.check_prs.get_pr_commits',
                return_value=[]
            ),
    ...
    def test_empty_repository_state(self, mock_get_prs: Any) -> None:
        # Here we use type Any because the mock can return various values.
