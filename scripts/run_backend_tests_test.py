...
class RunBackendTestsTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.coverage_exc_list = [
            sys.executable, '-m', 'coverage', 'run', '--branch', TEST_RUNNER_PATH,
            test_target_flag
        ]
    ...
    def test_run_shell_command_successfully(self) -> None:
        ...
        def mock_popen(
            cmd_tokens: list[str],
            **unsued_kwargs: str  # pylint: disable=unused-argument
        ) -> MockProcess:
            return MockProcess()

        swap_popen = self.swap_with_checks(
            subprocess, 'Popen', mock_popen, expected_args=((self.coverage_exc_list,),)
        )
    ...
    def test_module_in_shards_not_found_throws_error(self) -> None:
        ...
        self.assertEqual(
            ...
            'filesystem. See %s.' % ({'scripts.new_script_test'}, SHARDS_WIKI_LINK),
            returned_error_msg
        )
    ...
    def test_module_not_in_shards_throws_error(self) -> None:
        ...
        self.assertEqual(
            ...
            ({'scripts.new_script_test'}, SHARDS_WIKI_LINK), returned_error_msg
        )
    ...
    def test_subprocess_error_while_execution_throws_error(self) -> None:
        ...
        task_to_taskspec[
            tasks[0]
        ] = run_backend_tests.TestingTaskSpec('scripts.new_script.py', False)
    ...
    def test_empty_test_files_show_no_tests_were_run(self) -> None:
        ...
        task_to_taskspec[tasks[0]
                        ] = run_backend_tests.TestingTaskSpec(test_target, False)
    ...
    def test_failed_test_suite_throws_error(self) -> None:
        ...
        self.assertIn(
            'FAILED    %s: %s errors, %s failures' % (test_target, 0, 2), self.print_arr
        )
    ...
    def test_successfull_test_run_message_is_printed_correctly(self) -> None:
        ...
        task_to_taskspec[tasks[0]
                        ] = run_backend_tests.TestingTaskSpec(test1_target, True)
        task_to_taskspec[tasks[1]
                        ] = run_backend_tests.TestingTaskSpec(task2_target, True)

        with self.print_swap, self.swap(run_backend_tests, 'AVERAGE_TEST_CASE_TIME', 1):
    ...
    def test_successful_test_run_with_generate_time_report_flag(self) -> None:
        ...
        swap_check_results = self.swap(
            ...
            lambda *unused_args, **unused_kwargs: (100, 0, 0, expected_time_report)
        )
        swap_check_coverage = self.swap(
            run_backend_tests, 'check_coverage', lambda *unused_args, **unused_kwargs:
    ...
    def test_test_failed_due_to_error_in_parsing_coverage_report(self) -> None:
        ...
        task_to_taskspec[tasks[0]
                        ] = run_backend_tests.TestingTaskSpec(test_target, True)
    ...
    def test_invalid_delimiter_in_test_targets_argument_throws_error(self) -> None:
        with self.swap_install_third_party_libs:
            from scripts import run_backend_tests

        with self.swap_redis_server, self.swap_cloud_datastore_emulator:
            with self.assertRaisesRegex(
                Exception, r'The delimiter in each test_target should be a dot \(\.\)'
            ):
    ...
    def test_invalid_test_targets_message_is_displayed_correctly(self) -> None:
        ...
        self.assertIn(
            'WARNING : each test_target should point to the test file.', self.print_arr
        )
        ...
        with swapcheck_coverage, self.swap_cloud_datastore_emulator:
            with self.swap_redis_server, swap_check_results:
                with self.swap_execute_task, self.assertRaisesRegex(
                    ...
                    run_backend_tests.main(
                        args=['--test_targets', 'scripts.run_backend_tests_test']
                    )
    ...
    def test_multiple_tests_in_test_targets_argument_is_run_successfully(self) -> None:
        with self.swap_install_third_party_libs:
            from scripts import run_backend_tests

        executed_tasks = []
        test_targets = ('scripts.test_file_test,'
    ...
    def test_successful_test_run_with_run_on_changed_files_in_branch_flag(self) -> None:
        with self.swap_install_third_party_libs:
            from scripts import run_backend_tests
    ...
    def test_backend_tests_with_run_on_changed_files_in_branch_no_remote(self) -> None:
        with self.swap_install_third_party_libs:
            from scripts import run_backend_tests
