...
class RunBackendTestsTests(test_utils.GenericTestBase):
    ...
    def test_module_in_shards_not_found_throws_error(self) -> None:
        ...
        self.assertEqual(
            'Modules %s are in the backend test shards but missing from the filesystem. See %s.'
    ...
    def test_module_not_in_shards_throws_error(self) -> None:
        ...
        self.assertEqual(
            'Modules %s are present on the filesystem but are not listed in the backend test shards. See %s.'
            % ({'scripts.new_script_test'}, SHARDS_WIKI_LINK),
            returned_error_msg,
        )
