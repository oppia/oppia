...
_PARSER.add_argument(
    '--repository_path', type=str, help='The path to the repository.'
)
_PARSER.add_argument(
    '--issue',
    type=int,
...
_PARSER.add_argument('--new_comment_file', type=str, help='The new comment.')
...
def main(args: Optional[List[str]] = None) -> None:
    ...
    with open(
        repository_path + parsed_args.new_comment_file, 'r', encoding='utf-8'
    ) as new_comment_file:
        new_comment_lines = new_comment_file.read().splitlines()

    if (
        len(latest_comment_lines) != len(new_comment_lines)
        or latest_comment_lines[0] != new_comment_lines[0]
    ):
        raise Exception(NEW_COMMENT_SHOULD_BE_POSTED)

    for index in range(1, len(latest_comment_lines)):
        latest_comment_line_content = latest_comment_lines[index][
            compare_start_index:
        ]
        new_comment_line_content = new_comment_lines[index][
            compare_start_index:
        ]
