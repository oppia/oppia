...
from scripts import (
    common,
)  # isort:skip pylint: disable=wrong-import-position, unused-import
...
_PARSER.add_argument(
    "--chrome_driver_version", help="Uses the specified version of the chrome driver"
)
_PARSER.add_argument(
    "--debug_mode",
...
def run_tests(args: argparse.Namespace) -> Tuple[List[bytes], int]:
    ...
    with contextlib.ExitStack() as stack:
        ...
        if (args.mobile) and (args.suite not in MOBILE_SUITES):
            print(
                f"The {args.suite} suite should not be run " + "in the mobile viewport"
            )
            sys.exit(1)
...
def main(args: Optional[List[str]] = None) -> None:
    ...
    with servers.managed_portserver():
        for attempt_num in range(1, MAX_RETRY_COUNT + 1):
            ...
            rerun = flake_checker.check_test_flakiness(output, parsed_args.suite)
            if rerun:
                print("Rerunning.")
            else:
                print("Not rerunning.")
