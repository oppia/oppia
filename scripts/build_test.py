...
class BuildTests(test_utils.GenericTestBase):
    ...
    def test_generate_app_yaml_with_prod_mode(self):
        mock_prod_yaml_filepath = 'mock_app_prod.yaml'
        mock_yaml_filepath = 'mock_app.yaml'
        app_prod_yaml_filepath_swap = self.swap(
            build, 'APP_PROD_YAML_FILEPATH', mock_prod_yaml_filepath)
        app_yaml_filepath_swap = self.swap(
            build, 'APP_YAML_FILEPATH', mock_yaml_filepath)

        app_prod_yaml_temp_file = tempfile.NamedTemporaryFile()
        app_prod_yaml_temp_file.name = mock_prod_yaml_filepath
        with python_utils.open_file(mock_prod_yaml_filepath, 'w') as tmp:
            tmp.write(u'Some content in mock_app_prod.yaml')
        ...
        with app_prod_yaml_filepath_swap, app_yaml_filepath_swap:
            build.generate_app_yaml(is_prod_mode=True)
        ...
        self.assertEqual(
            content,
            '# THIS FILE IS AUTOGENERATED, DO NOT MODIFY\n'
            'Some content in mock_app_prod.yaml')

        app_yaml_temp_file.close()
        app_prod_yaml_temp_file.close()
