...
class BuildTests(test_utils.GenericTestBase):
    ...
    def test_generate_app_yaml(self):
        app_dev_yaml_file_swap = self.swap(
            build, 'APP_DEV_YAML_FILE', 'core/tests/data/mock_app_dev.yaml')
        app_yaml_file_swap = self.swap(
            build, 'APP_YAML_FILE', 'core/tests/data/mock_app.yaml')

        self.assertFalse(os.path.isfile('core/tests/data/mock_app.yaml'))

        with app_dev_yaml_file_swap, app_yaml_file_swap:
            build.generate_app_yaml()

        self.assertTrue(os.path.isfile('core/tests/data/mock_app.yaml'))

        with open('core/tests/data/mock_app.yaml', 'r') as yaml_file:
        ...
        build.safe_delete_file('core/tests/data/mock_app.yaml')
