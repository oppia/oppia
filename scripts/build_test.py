...
class BuildTests(test_utils.GenericTestBase):
    ...
    def test_insert_hash(self) -> None:
        ...
        self.assertEqual(build._insert_hash('file.js', '123456'), 'file.123456.js')  # pylint: disable=protected-access
        self.assertEqual(
        ...
        for _, _, files in os.walk(MOCK_EXTENSIONS_DEV_DIR):
            for filename in files:
                if not build.should_file_be_built(filename) or any(
                    filename.endswith(p) for p in build.FILE_EXTENSIONS_TO_IGNORE
                ):
    ...
    def test_compare_file_count(self) -> None:
        ...
        with self.assertRaisesRegex(
            ValueError, ('%s files in first dir list != %s files in second dir list') %
    ...
    def test_verify_filepath_hash(self) -> None:
        ...
        hashed_base_filename = build._insert_hash(  # pylint: disable=protected-access
            base_filename, test_utils.generate_random_hexa_str())
        with self.assertRaisesRegex(
            KeyError, 'Hash from file named %s does not match hash dict values' %
    ...
    def test_hash_should_be_inserted(self) -> None:
        ...
        with self.swap(
            ...
            self.assertFalse(
                build.hash_should_be_inserted('path/to/fonts/fontawesome-webfont.svg')
            )
            self.assertFalse(
            ...
            self.assertTrue(
                build.
                hash_should_be_inserted('rich_text_components/Video/protractor.js')
            )
    ...
    def test_filter_hashes(self) -> None:
        ...
        with self.swap(
            build, 'FILEPATHS_PROVIDED_TO_FRONTEND', ('test_path/*', 'path/to/file.js')
        ):
            hashes = {
                'path/to/file.js': '123456',
    ...
    def test_generate_build_tasks_to_build_directory(self) -> None:
        ...
        build_all_files_tasks = (
            build.generate_build_tasks_to_build_all_files_in_directory(
                MOCK_EXTENSIONS_DEV_DIR, extensions_dirnames_to_dirpaths['out_dir']
            )
        )
    ...
    def test_get_recently_changed_filenames(self) -> None:
        ...
        with self.swap(build, 'FILE_EXTENSIONS_TO_IGNORE', (
            '.html',
            '.py',
        )):
            self.assertEqual(
    ...
    def test_generate_app_yaml_with_deploy_mode(self) -> None:
        ...
        env_vars_to_remove_from_deployed_app_yaml_swap = self.swap(
            build, 'ENV_VARS_TO_REMOVE_FROM_DEPLOYED_APP_YAML',
    ...
    def test_generate_app_yaml_with_deploy_mode_with_nonexistent_var_raises(
        ...
        env_vars_to_remove_from_deployed_app_yaml_swap = self.swap(
            build, 'ENV_VARS_TO_REMOVE_FROM_DEPLOYED_APP_YAML', ['DATASTORE_HOST']
        )
        ...
        with app_dev_yaml_filepath_swap, app_yaml_filepath_swap:
            with env_vars_to_remove_from_deployed_app_yaml_swap:
                with self.assertRaisesRegex(
                    Exception, 'Environment variable \'DATASTORE_HOST\' to be '
    ...
    def test_minify_third_party_libs(self) -> None:
        ...
        self.assertFalse(
            os.path.isfile('core/tests/data/third_party/css/third_party.min.css')
        )
        self.assertFalse(
            os.path.isfile('core/tests/data/third_party/js/third_party.min.js')
        )
        self.assertFalse(
            os.path.isfile('core/tests/data/third_party/js/third_party.min.js.map')
        )
        ...
        self.assertLess(
            os.path.getsize('core/tests/data/third_party/js/third_party.min.js'),
    ...
    def test_cannot_minify_third_party_libs_in_dev_mode(self) -> None:
        ...
        clean_swap = self.swap(build, 'clean', mock_clean)
        assert_raises_regexp_context_manager = self.assertRaisesRegex(
            Exception, 'minify_third_party_libs_only should not be set in non-prod env.'
        )
    ...
    def test_build_using_ng_command(self) -> None:
        ...
        def mock_managed_ng_build(
            use_prod_env: bool,
            watch_mode: bool  # pylint: disable=unused-argument
        ) -> Iterator[scripts_test_utils.PopenStub]:
            yield scripts_test_utils.PopenStub()
    ...
    def test_build_using_ng_command_with_incorrect_filecount_fails(self) -> None:
...
class E2EAndAcceptanceBuildTests(test_utils.GenericTestBase):
    ...
    def test_build_js_files_in_dev_mode_with_hash_file_exists(self) -> None:
        ...
        self.exit_stack.enter_context(
            self.swap_with_checks(
                build, 'main', lambda *_, **__: None, expected_kwargs=[{
                    'args': []
                }]
            )
        )
    ...
    def test_build_js_files_in_prod_mode(self) -> None:
        self.exit_stack.enter_context(
            self.swap_with_checks(common, 'run_cmd', lambda *_: None, called=False)
        )
        self.exit_stack.enter_context(
            self.swap_with_checks(
                build,
