...
class InstallThirdPartyTests(test_utils.GenericTestBase):
    ...
    def test_dependencies_syntax_with_extra_optional_key(self) -> None:
        ...
        self.assertTrue(
            'Only one of these keys pair must be used: "rootDir, rootDirPrefix".'
            in print_arr
        )
    ...
    def test_dependencies_syntax_with_invalid_url(self) -> None:
        ...
        with print_swap, self.assertRaisesRegex(SystemExit, '1'):
            install_dependencies_json_packages.test_dependencies_syntax(
                'zip',
                {
                    ...
                    'downloadFormat': 'zip',
                    'url': (
                        'https://python.org/packages/beautifulsoup4-4.7.1.tar#md5=321d'
                    ),
        ...
        self.assertTrue(
            'This url https://python.org/packages/beautifulsoup4-4.7.1.tar is invalid for zip file format.'
            in print_arr
        )
    ...
    def test_validate_dependencies_with_missing_download_format(self) -> None:
        ...
        with (
            ...
            self.assertRaisesRegex(
                Exception,
                re.escape(
                    'downloadFormat not specified in {\'version\': \'2.7.5\', \'url\': \'https://github.com/mathjax/2.7.5.zip\', \'targetDirPrefix\': \'MathJax-\'}'
                ),
            ),
        ):
            install_dependencies_json_packages.validate_dependencies('filepath')
