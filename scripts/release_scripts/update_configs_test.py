...
class UpdateConfigsTests(test_utils.GenericTestBase):
    ...
    def test_invalid_config(self) -> None:
        with self.assertRaisesRegex(
            Exception,
            'Invalid line in invalid_feconf_updates.config config file: INVALID_KEY: \'invalid\'',
        ):
    ...
    def test_missing_line_in_local_config(self) -> None:
        with self.assertRaisesRegex(
            Exception,
            'Could not find correct number of lines in feconf.txt matching: EXTRA_KEY = \'extra\'',
        ):
            update_configs.apply_changes_based_on_config(
                MOCK_LOCAL_FECONF_PATH,
