...
INVALID_FECONF_CONFIG_PATH: Final = os.path.join(
    os.getcwd(), 'core', 'tests', 'release_sources', 'invalid_feconf_updates.config'
)
VALID_FECONF_CONFIG_PATH: Final = os.path.join(
    os.getcwd(), 'core', 'tests', 'release_sources', 'valid_feconf_updates.config'
)
...
class UpdateConfigsTests(test_utils.GenericTestBase):
    ...
    def test_app_yaml_verification_with_wildcard_header_present(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception, r'\'Access-Control-Allow-Origin: "\*"\' must be updated to '
    ...
    def test_changes_are_applied_to_config(self) -> None:
        ...
        expected_text = original_text.replace(
            'ADMIN_EMAIL_ADDRESS = \'\'', 'ADMIN_EMAIL_ADDRESS = \'adm@example.com\''
        )
        try:
            update_configs.apply_changes_based_on_config(
    ...
    def test_function_calls_without_prompt_for_feconf_and_terms_update(self) -> None:
        ...
        def mock_apply_changes(
            unused_local_filepath: str, unused_config_filepath: str,
        ...
        def mock_update_app_yaml(
            unused_release_app_dev_yaml_path: str, unused_feconf_config_path: str
        ) -> None:
