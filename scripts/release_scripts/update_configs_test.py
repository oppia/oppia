...
class UpdateConfigsTests(test_utils.GenericTestBase):
    ...
    def test_feconf_verification_with_correct_config(self) -> None:
        ...
        with tempfile.NamedTemporaryFile() as temp_app_yaml:
    ...
    def test_app_yaml_verification_with_wildcard_header_present(self) -> None:
        ...
        with tempfile.NamedTemporaryFile() as temp_feconf:
            ...
            with tempfile.NamedTemporaryFile() as temp_app_yaml:
                temp_app_yaml_path = temp_app_yaml.name
                feconf_text = (
                    'MAILGUN_API_KEY = \'%s\'\n'
                    'MAILCHIMP_API_KEY = \'%s\'\n'
                    'REDISHOST = \'192.13.2.1\'\n'
    ...
    def test_update_app_yaml_correctly_updates(self) -> None:
        with tempfile.NamedTemporaryFile() as temp_feconf_config:
            temp_feconf_config_path = temp_feconf_config.name
            with tempfile.NamedTemporaryFile() as temp_app_yaml:
                temp_app_yaml_path = temp_app_yaml.name
    ...
    def test_update_analytics_ids(self) -> None:
        with tempfile.NamedTemporaryFile() as temp_file:
            temp_analytics_constants_config_path = temp_file.name
            with tempfile.NamedTemporaryFile() as temp_constants:
                temp_constants_path = temp_constants.name
                constants_text = (
                    '  "UA_ANALYTICS_ID": "456"\n'
                    '  "GA_ANALYTICS_ID": "123"\n'
                ...
                with utils.open_file(
                    temp_analytics_constants_config_path, 'w') as f:
                ...
                with utils.open_file(
                    temp_analytics_constants_config_path, 'r') as f:
                    self.assertEqual(
                        f.read(),
                        expected_analytics_constants_config_text)

    def test_raises_error_with_invalid_update_analytics_ids(self) -> None:
        with tempfile.NamedTemporaryFile() as temp_constants_file:
            temp_constants_path = temp_constants_file.name
            with tempfile.NamedTemporaryFile() as temp_app_yaml:
                temp_analytics_constants_config_path = temp_app_yaml.name
                analytics_constants_config_text = (
                        '  "GA_ANALYTICS_ID": ""\n'
                        '  "UA_ANALYTICS_ID": ""\n'
                        '  "SITE_NAME_FOR_ANALYTICS": ""\n'
