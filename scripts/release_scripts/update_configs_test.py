...
class UpdateConfigsTests(test_utils.GenericTestBase):
        ...
        with tempfile.NamedTemporaryFile() as temp_feconf:
            ...
            with tempfile.NamedTemporaryFile() as temp_app_yaml:
                ...
                update_configs.update_app_yaml(
                    temp_app_yaml_path, temp_feconf_path)
    ...
    def test_update_analytics_ids(self) -> None:
        with tempfile.NamedTemporaryFile() as temp_constants:
            ...
            with tempfile.NamedTemporaryFile() as temp_analytics_constants:
                temp_analytics_constants_path = temp_analytics_constants.name
                constants_text = (
                    '  "GA_ANALYTICS_ID": "123"\n'
                ...
                with utils.open_file(temp_analytics_constants_path, 'w') as f:
                    f.write(analytics_constants_config_text)

                update_configs.update_analytics_constants_based_on_config(
                    temp_analytics_constants_path,
                    temp_constants_path
                )
                with utils.open_file(temp_analytics_constants_path, 'r') as f:
                    self.assertEqual(
                        f.read(), expected_analytics_constants_config_text)
