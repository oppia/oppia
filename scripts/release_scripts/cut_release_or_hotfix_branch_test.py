...
class CutReleaseOrHotfixBranchTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.check_function_calls = {
            'verify_local_repo_is_clean_is_called':
                False,
            'verify_current_branch_name_is_called':
                False,
            'get_remote_alias_is_called':
                False,
            'check_call_is_called':
                False,
            'verify_target_branch_does_not_already_exist_is_called':
                False,
            (
                'verify_target_version_compatible_with_latest_'
                'released_version_is_called'
            ):
                False,
            (
                'verify_hotfix_number_is_one_ahead_of_'
                'previous_hotfix_number_is_called'
            ):
                False,
            'open_new_tab_in_browser_if_possible_is_called':
                False
        }
        ...
        def mock_verify_target_version(unused_target_version: str) -> None:
            self.check_function_calls['verify_target_version_compatible_with_'
                                      'latest_released_version_is_called'] = True

        def mock_verify_hotfix_number(
            unused_remote_alias: str, unused_target_version: str,
            unused_hotfix_number: int
        ) -> None:
            self.check_function_calls['verify_hotfix_number_is_one_ahead_of_'
                                      'previous_hotfix_number_is_called'] = True

        def mock_open_tab(unused_url: str) -> None:
            self.check_function_calls['open_new_tab_in_browser_if_possible_is_called'
                                     ] = True
        ...
        self.verify_local_repo_swap = self.swap(
            common, 'verify_local_repo_is_clean', mock_verify_local_repo_is_clean
        )
        self.verify_branch_name_swap = self.swap(
            common, 'verify_current_branch_name', mock_verify_current_branch_name
        )
    ...
    def test_exception_is_raised_if_target_branch_exists(self) -> None:
        ...
        with check_output_swap, self.assertRaisesRegex(
            Exception, 'ERROR: The target branch name already exists locally. '
            'Run "git branch -D new-branch" to delete it.'
        ):
            (
                cut_release_or_hotfix_branch.
                verify_target_branch_does_not_already_exist('upstream', 'new-branch')
            )
    ...
    def test_failure_to_fetch_release_info(self) -> None:
        ...
        with self.url_open_swap, self.assertRaisesRegex(
            ...
            (
                cut_release_or_hotfix_branch.
                verify_target_version_compatible_with_latest_release('1.2.3')
            )
    ...
    def test_invalid_last_version_tag(self) -> None:
        ...
        with self.url_open_swap, load_swap, self.assertRaisesRegex(
            Exception, 'ERROR: Could not parse version number of latest GitHub release.'
        ):
            (
                cut_release_or_hotfix_branch.
                verify_target_version_compatible_with_latest_release('1.2.3')
    ...
    def test_invalid_target_version_raises_error(self) -> None:
        ...
        with self.url_open_swap, load_swap, self.assertRaisesRegex(
            ...
            (
                cut_release_or_hotfix_branch.
                verify_target_version_compatible_with_latest_release('123')
            )
    ...
    def test_invalid_difference_between_patch_versions(self) -> None:
        ...
        load_swap = self.swap(json, 'load', mock_load)
        with self.url_open_swap, load_swap, self.assertRaisesRegex(
            AssertionError, 'The current patch version is not equal to previous '
    ...
    def test_invalid_difference_between_minor_versions(self) -> None:
        ...
        load_swap = self.swap(json, 'load', mock_load)
        with self.url_open_swap, load_swap, self.assertRaisesRegex(
            AssertionError, 'The current minor version is not equal to previous minor '
            'version plus one.'
        ):
            (
                cut_release_or_hotfix_branch.
                verify_target_version_compatible_with_latest_release('1.2.0')
            )
    ...
    def test_invalid_patch_version_with_valid_difference_between_minor_versions(
        ...
        with self.url_open_swap, load_swap, self.assertRaisesRegex(
            ...
            (
                cut_release_or_hotfix_branch.
                verify_target_version_compatible_with_latest_release('1.2.1')
            )
    ...
    def test_main_with_valid_args(self) -> None:
        ...
        expected_check_function_calls: Dict[str, Optional[Union[str, bool, int]]] = {
        ...
        branch_cut_swap = self.swap(
            cut_release_or_hotfix_branch, 'execute_branch_cut', mock_execute_branch_cut
        )
    ...
    def test_exception_is_raised_if_actions_ci_is_failing(self) -> None:
                ...
                with self.verify_target_version_swap, self.open_tab_swap:
                    with self.get_remote_alias_swap, self.check_call_swap:
                        with input_swap, self.assertRaisesRegex(
                            Exception, 'Tests should pass on develop before this '
                            'script is run.'
                        ):
        ...
        self.expected_check_function_calls['verify_target_version_compatible_with_'
    ...
    def test_function_calls_for_hotfix_branch_with_hotfix_number_more_than_one(
        ...
        expected_cmd_tokens = [
            ...
            'release-1.2.3-hotfix-2', 'git', 'pull', 'upstream',
            'release-1.2.3-hotfix-2', 'git', 'checkout', '-b', 'release-1.2.3-hotfix-3',
    ...
    def test_function_calls_for_hotfix_branch_with_hotfix_number_equal_to_one(
        ...
        self.assertEqual(self.check_function_calls, self.expected_check_function_calls)
        expected_cmd_tokens = [
            'git', 'pull', 'upstream', 'develop', 'git', 'checkout', 'release-1.2.3',
            'git', 'pull', 'upstream', 'release-1.2.3', 'git', 'checkout', '-b',
            'release-1.2.3-hotfix-1', 'release-1.2.3'
        ]
        self.assertEqual(self.all_cmd_tokens, expected_cmd_tokens)
