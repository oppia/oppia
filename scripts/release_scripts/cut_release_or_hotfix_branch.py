...
def verify_hotfix_number_is_one_ahead_of_previous_hotfix_number(
    ...
    Raises:
        Exception. The difference between two continuous hotfix numbers
            is not one.
    """
    all_branches = subprocess.check_output(['git', 'branch', '-a'],
                                           encoding='utf-8').split('\n')
...
def _get_hotfix_branch_type_and_name(target_version: str,
                                     hotfix_number: int) -> Tuple[str, str]:
    ...
    """
    return (
        BRANCH_TYPE_HOTFIX, 'release-%s-hotfix-%s' % (target_version, hotfix_number)
    )
...
def execute_branch_cut(target_version: str, hotfix_number: int) -> None:
    ...
    common.open_new_tab_in_browser_if_possible(
        'https://github.com/oppia/oppia/actions?query=branch:%s' % branch_to_check
    )
    print(
        'Please confirm: are Actions checks passing on %s? (y/n) ' % (branch_to_check)
    )
    answer = input().lower()
    if answer not in common.AFFIRMATIVE_CONFIRMATIONS:
        raise Exception(
            'Tests should pass on %s before this script is run.' % (branch_to_check)
        )
