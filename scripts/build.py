...
def generate_app_yaml(deploy_mode: bool = False) -> None:
    ...
    with common.open_file(APP_DEV_YAML_FILEPATH, 'r') as yaml_file:
    ...
    with common.open_file(APP_YAML_FILEPATH, 'w+') as prod_yaml_file:
...
def _minify_css(source_path: str, target_path: str) -> None:
    ...
    with common.open_file(source_path, 'r') as source_file:
        with common.open_file(target_path, 'w') as target_file:
...
def get_dependencies_filepaths() -> Dict[str, List[str]]:
    ...
    with common.open_file(DEPENDENCIES_FILE_PATH, 'r') as json_file:
...
def generate_md5_hash(filepath: str) -> str:
    ...
    """
    m = hashlib.md5()
    with common.open_file(filepath, 'rb', encoding=None) as f:
        while True:
...
def minify_func(source_path: str, target_path: str, filename: str) -> None:
    ...
    if filename.endswith('.html'):
        ...
        with common.open_file(source_path, 'r+') as source_html_file:
            with common.open_file(target_path, 'w+') as minified_html_file:
