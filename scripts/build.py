...
def inject_angular_css_hashes() -> None:
    """Inject Angular CLI generated CSS hashes into webpack HTML templates.

    This function scans the dist/oppia-angular-prod directory for the hashed
    styles.{hash}.css file generated by Angular CLI, then updates all HTML
    files in backend_prod_files/webpack_bundles/ to reference the correct
    hashed filename. It also updates assets/hashes.json to include the
    Angular CSS hash for frontend cache invalidation.

    This runs after build_using_ng() to ensure the Angular CLI build has
    completed and the hashed CSS file exists.
    """
    print('Injecting Angular CSS hashes into webpack templates')

    # Find the hashed styles file generated by Angular CLI.
    dist_dir = 'dist/oppia-angular-prod'
    if not os.path.exists(dist_dir):
        print('Warning: %s does not exist. Skipping CSS hash injection.' % dist_dir)
        return
    ...
    hashed_styles_filename = styles_files[0]
    print('Found Angular styles file: %s' % hashed_styles_filename)
    ...
    if not os.path.exists(webpack_bundles_dir):
        print(
            'Warning: %s does not exist. Skipping CSS hash injection.' % webpack_bundles_dir
        )
        return
    ...
    for html_file in html_files:
        ...
        # Replace the placeholder styles.css reference with the hashed version.
    ...
    print('Updated %d HTML files with Angular CSS hashes' % updated_count)

    # Update assets/hashes.json to include the Angular CSS hash.
    # Extract the hash from the filename (e.g., styles.abc123.css -> abc123).
    hash_match = re.match(r'styles\.([a-f0-9]+)\.css', hashed_styles_filename)
    if hash_match:
        ...
        print('Warning: Could not extract hash from %s' % hashed_styles_filename)
    ...
    for root, dirnames, filenames in os.walk(os.path.join(os.getcwd(), source)):
        ...
        for filename in filenames:
            ...
            if not any(
                ...
                if (
                    hash_should_be_inserted(source + relative_path)
                    and relative_path in file_hashes
                ):
                    relative_path = _insert_hash(
