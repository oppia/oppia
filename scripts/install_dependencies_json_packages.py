...
import certifi
from typing import (
    BinaryIO, Dict, Final, List, Literal, TextIO, TypedDict, Union, cast, overload
)
...
DOWNLOAD_FORMATS_TO_DEPENDENCIES_KEYS: Dict[DownloadFormatType,
                                            DownloadFormatToDependenciesKeysDict] = {
                                                'zip': {
                                                    'mandatory_keys': [
                                                        'version', 'url',
                                                        'downloadFormat'
    ...
    for _, dependency in dependencies.items():
        ...
        if download_format == _DOWNLOAD_FORMAT_FILES:
            ...
            dependency_zip_root_name = (
                dependency['rootDir'] if 'rootDir' in dependency else
                dependency['rootDirPrefix'] + dependency['version']
            )
            dependency_target_root_name = (
                dependency['targetDir'] if 'targetDir' in dependency else
                dependency['targetDirPrefix'] + dependency['version']
            )
            download_and_unzip_files(
                dependency['url'], THIRD_PARTY_STATIC_DIR, dependency_zip_root_name,
                dependency_target_root_name
            )
