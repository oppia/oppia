...
class LintChecksManager(object):
    ...
    def _check_todo_comments_have_user(self):
        """Checks that a todo comment has an assigned user
        in the format TODO(@username): message.

        Returns:
            summary_messages: list(str). Summary of messages
            generated by the check.
        """
        if self.verbose_mode_enabled:
            print 'Starting todo comments check'
            print '----------------------------------------'
            ...
            for filepath in self.all_filepaths:
                ...
                for line_num, line in enumerate(FileCache.readlines(
                    ...
                    if stripped_line.startswith('#') or (
                        ...
                        # Use re.search method to match the regex anywhere.
                        if 'TODO' in stripped_line and (
                                todo_regex.search(stripped_line) is None):
                            failed = True
                            print '%s --> Line %s: %s' % (
                                filepath, line_num + 1, message
                            )
                if failed:
                    summary_message = (
                        '%s   TODO check failed' % _MESSAGE_TYPE_FAILED
                    )
                else:
                    summary_message = (
                        '%s    TODO check passed' % _MESSAGE_TYPE_SUCCESS
                    )
    ...
    def perform_all_lint_checks(self):
        ...
        todo_check_messages = self._check_todo_comments_have_user()
        all_messages = (
