...
def _check_docstrings(all_files):
    ...
    order in the function definition matches the order int the doc string.

    Args:
        all_files: list. Names of files to consider during docstring check.

    Returns:
        summary_messages: list. Summary of messages generated by the check.
    """
    ...
    arg_order_result = True
    for filename in files_to_check:
        if not _check_docstrings_arg_order(filename):
            arg_order_result = False

    if not arg_order_result:
        failed = False
