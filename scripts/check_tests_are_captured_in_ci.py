...
def get_acceptance_test_suites_from_ci_config_file() -> List[TestSuiteDict]:
    ...
    Returns:
        ...
        config file.
    """
    with open(ACCEPTANCE_CI_TEST_SUITE_CONFIG_FILE_PATH, 'r', encoding='utf-8') as f:
...
def get_e2e_test_suites_from_webdriverio_config_file() -> List[TestSuiteDict]:
    ...
    suite_object_string = re.sub(
        r'[\n\t\s]*',
        '',
        re.compile(r'suites = {([^}]+)}').findall(webdriverio_config_file_content)[0],
    )
    ...
    for test_suite_match in test_suite_regex.findall(suite_object_string):
        ...
        test_suite_modules = [
            test_suite for test_suite in test_suite_match[1].split(',') if test_suite
        ]
        test_suite_modules = [
            module.strip()[1:-1].replace('./', '') for module in test_suite_modules
        ]
...
def get_acceptance_test_suites_from_acceptance_directory() -> (
    ...
    for module in acceptance_test_files:
        acceptance_test_suite_name = os.path.relpath(
            module, ACCEPTANCE_TEST_SPECS_DIRECTORY
        ).replace('.spec.ts', '')
        if acceptance_test_suite_name in ACCEPTANCE_TEST_SUITES_THAT_ARE_NOT_RUN_IN_CI:
            continue
...
def main() -> None:
    ...
    print('Checking all acceptance test suites are captured in CI ' 'config files...')
    ...
    acceptance_test_suites_difference = compute_test_suites_difference(
        acceptance_test_suites_from_config, acceptance_test_suites_from_directory
    )
    ...
    webdriverio_test_suite_modules = get_e2e_test_modules_from_webdriverio_directory()
    webdriverio_conf_test_modules = get_e2e_test_modules_from_webdriverio_config_file()

    if not (
        sorted(webdriverio_test_suite_modules) == sorted(webdriverio_conf_test_modules)
    ):
        raise Exception(
    ...
    e2e_test_suites_from_directory = get_e2e_test_suites_from_webdriverio_config_file()
