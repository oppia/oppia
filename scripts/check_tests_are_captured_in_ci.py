...
def get_e2e_test_modules_from_webdriverio_config_file() -> List[str]:
    ...
    suite_object_string = re.compile(r'suites = {([^}]+)}'
                                    ).findall(webdriverio_config_file_content)[0]
...
def get_acceptance_test_suites_from_acceptance_directory() -> List[TestSuiteDict]:  # pylint: disable=line-too-long
    ...
    for module in acceptance_test_files:
        ...
        if (
            acceptance_test_suite_name in ACCEPTANCE_TEST_SUITES_THAT_ARE_NOT_RUN_IN_CI
        ):
            continue
...
def compute_test_suites_difference(
    ...
    test_suites_difference = [
        ...
        if test_suite not in test_suites_from_config or
...
def main() -> None:
    ...
    print('Checking all acceptance test suites are captured in CI '
          'config files...')
