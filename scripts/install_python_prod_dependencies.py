...
def _remove_metadata(library_name: str, version_string: str) -> None:
    ...
    possible_normalized_directory_names = (
        _get_possible_normalized_metadata_directory_names(library_name, version_string)
    )
    ...
    for normalized_dirname, original_dirname in normalized_to_original_dirnames.items():
...
def _reinstall_all_dependencies() -> None:
    ...
    _pip_install_requirements(
        common.THIRD_PARTY_PYTHON_LIBS_DIR, common.COMPILED_REQUIREMENTS_FILE_PATH
    )
...
def _get_possible_normalized_metadata_directory_names(
    ...
    # underscores.
    return {
        normalize_directory_name('%s-%s.dist-info' % (library_name, version_string)),
        normalize_directory_name(
            '%s-%s.dist-info' % (library_name.replace('-', '_'), version_string)
        ),
        normalize_directory_name('%s-%s.egg-info' % (library_name, version_string)),
        normalize_directory_name(
        ...
        normalize_directory_name(
            '%s-%s-py3.9.egg-info' % (library_name.replace('-', '_'), version_string)
        ),
    }
...
def verify_pip_is_installed() -> None:
    ...
    try:
        ...
        if common.is_mac_os():
            print(
                'https://github.com/oppia/oppia/wiki/Installing-Oppia-%28Mac-' 'OS%29'
            )
        elif common.is_linux_os():
            print('https://github.com/oppia/oppia/wiki/Installing-Oppia-%28Linux' '%29')
        else:
            print(
                'https://github.com/oppia/oppia/wiki/Installing-Oppia-%28' 'Windows%29'
            )
