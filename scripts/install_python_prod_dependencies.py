...
# instead.
LIBRARY_NAMES_WITH_INITIAL_PERIOD: Final = [
    'jaraco-classes',
    'jaraco-context',
...
def normalize_python_library_name(library_name: str) -> str:
    ...
    if any(
            library_name == name
            for name in LIBRARY_NAMES_WITH_INITIAL_PERIOD):
...
def _get_third_party_python_libs_directory_contents() -> Dict[str, str]:
    ...
    Returns:
        ...
        value.
    """
    direct_url_packages, standard_packages = common.partition(
    ...
    return {
        ...
        normalize_directory_name(
            '%s-%s.egg-info' % (
                library_name.replace('-', '_'), version_string)),
        normalize_directory_name(
            '%s-%s-py3.12.egg-info' % (library_name, version_string)),
        normalize_directory_name(
            '%s-%s-py3.12.egg-info' % (
                library_name.replace('-', '_'), version_string))
    }
