...
class Ret:
    ...
    def __init__(
        self, returncode: int = 0, communicate_val: Tuple[bytes, bytes] = (b'', b'')
    ) -> None:
...
class InstallThirdPartyLibsTests(test_utils.GenericTestBase):
    ...
    def test_install_third_party_main_also_installs_hooks(self) -> None:
        ...
        def mock_main_for_pre_commit_hook(
            args: List[str],
        ) -> None:  # pylint: disable=unused-argument
            check_function_calls['pre_commit_hook_main_is_called'] = True

        def mock_main_for_pre_push_hook(
            args: List[str],
        ) -> None:  # pylint: disable=unused-argument
        ...
        swap_install_gcloud_sdk = self.swap(
            install_third_party_libs, 'install_gcloud_sdk', mock_install_gcloud_sdk
        )
        swap_install_redis_cli = self.swap(
            install_third_party_libs, 'install_redis_cli', mock_install_redis_cli
        )
        ...
        swap_install_json_deps_main = self.swap(
            install_dependencies_json_packages, 'main', mock_external_script_call
        )
...
class InstallRedisAndElasticSearchTests(test_utils.GenericTestBase):
    ...
    def test_install_redis_cli_function_calls(self) -> None:
        ...
        ) -> Ret:  # pylint: disable=unused-argument
...
class SetupTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.test_py_swap = self.swap(
            install_third_party_libs, 'test_python_version', mock_test_python_version
        )
...
class GoogleCloudSdkInstallationTests(test_utils.GenericTestBase):
    ...
    def test_gcloud_install_without_errors(self) -> None:
        ...
        self.assertTrue(
            'Download complete. Installing Google Cloud SDK...' in self.print_arr
        )
    ...
    def test_gcloud_install_with_errors(self) -> None:
        ...
        self.assertEqual(self.check_function_calls, self.expected_check_function_calls)
        self.assertTrue(
            'Error downloading Google Cloud SDK. Exiting.' in self.print_arr
        )
    ...
    def test_directory_copying(self) -> None:
        ...
        self.assertEqual(
            initialized_directories, [common.GOOGLE_CLOUD_SDK_HOME, correct_google_path]
        )
