...
class InstallThirdPartyLibsTests(test_utils.GenericTestBase):
    ...
    def test_function_calls_on_windows(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(prefix='py') as temp_py_config_file:
            with utils.open_file(temp_py_config_file.name, 'w') as f:
                f.write(py_actual_text)

        pq_actual_text = (
            'ConverterMapping,\n"ConverterMapping",\nOther Line\n')
        with tempfile.NamedTemporaryFile(prefix='pq') as temp_pq_config_file:
            with utils.open_file(temp_pq_config_file.name, 'w') as f:
