...
class InstallPythonDevDependenciesTests(test_utils.GenericTestBase):
    ...
    def test_main_passes_with_no_assert_and_no_change(self) -> None:
        ...
        assert_swap = self.swap_with_checks(
            install_python_dev_dependencies, 'check_python_env_is_suitable', mock_func
        )
        install_tools_swap = self.swap_with_checks(
            install_python_dev_dependencies, 'install_installation_tools', mock_func
        )
        compile_swap = self.swap_with_checks(
            install_python_dev_dependencies, 'compile_pip_requirements', mock_compile
        )
        install_dependencies_swap = self.swap_with_checks(
            install_python_dev_dependencies, 'install_dev_dependencies', mock_func
        )
    ...
    def test_main_passes_with_uninstall(self) -> None:
        ...
        install_tools_swap = self.swap_with_checks(
            install_python_dev_dependencies, 'install_installation_tools', lambda: None
        )
        compile_swap = self.swap_with_checks(
            install_python_dev_dependencies, 'compile_pip_requirements', mock_compile
        )
        uninstall_dependencies_swap = self.swap_with_checks(
            install_python_dev_dependencies, 'uninstall_dev_dependencies', lambda: None
        )
