...
TIME_REPORT_PATH: Final = os.path.join(os.getcwd(), 'backend_test_time_report.json')
...
_EXCLUSIVE_GROUP.add_argument(
    '--test_shard', help='optional name of shard to run', type=str
)
_EXCLUSIVE_GROUP.add_argument(
...
class TestingTaskSpec:
    ...
    def run(self) -> List[concurrent_task_utils.TaskResult]:
        ...
        if self.generate_coverage_report:
            exc_list = [
                sys.executable, '-m', 'coverage', 'run', '-p', '--branch',
...
def get_all_test_targets_from_path(
    test_path: Optional[str] = None, include_load_tests: bool = True
) -> List[str]:
    ...
    excluded_dirs = [
        ...
        'core/tests/build_sources', '.direnv'
    ]
...
def check_shards_match_tests(include_load_tests: bool = True) -> str:
    ...
    if len(set(shard_modules)) != len(shard_modules):
        ...
        raise Exception(
            'Failed to find  module duplicated in shards.'
        )  # pragma: no cover
    ...
    for task in tasks:
        ...
        if not task.finished:
            ...
            try:
                ...
                test_time_by_average_test_case = (test_count * AVERAGE_TEST_CASE_TIME)
                time_report[spec.test_target
                           ] = (test_time, test_time_by_average_test_case)
                print(
...
def main(args: Optional[List[str]] = None) -> None:
    ...
    with contextlib.ExitStack() as stack:
        ...
        if parsed_args.test_targets:
            ...
            for test_target in test_targets:
                ...
                if (test_target.endswith('_test') or '_test.' in test_target):
            ...
            if feconf.OPPIA_IS_DOCKERIZED:  # docker: no cover
                ...
                docker_exclude_tests = [
                    ...
                    'scripts.build_test', 'scripts.run_acceptance_tests_test'
                ]
                all_test_targets = [
            ...
            all_test_targets = list(git_changes_utils.get_changed_python_test_files())
        else:
            include_load_tests = not parsed_args.exclude_load_tests
            all_test_targets = get_all_test_targets_from_path(
                test_path=parsed_args.test_path, include_load_tests=include_load_tests
            )
    ...
    print(
        'Ran %s test%s in %s test class%s.' % (
            total_count, '' if total_count == 1 else 's', len(tasks),
    ...
    if parsed_args.generate_coverage_report:
        ...
        if coverage != 100:
            ...
            print('For more information, please see our '
...
def check_coverage(
    ...
    """
    if combine:
        combine_process = subprocess.run([sys.executable, '-m', 'coverage', 'combine'],
                                         capture_output=True,
                                         encoding='utf-8',
                                         check=False)
    ...
    cmd = [
        ...
        '--omit="%s*","third_party/*","/usr/share/*"' % common.OPPIA_TOOLS_DIR,
        '--show-missing', '--skip-covered'
    ]
    if include:
    ...
    if process.stdout.strip() == 'No data to report.':
        ...
        raise RuntimeError(
            'Failed to calculate coverage because subprocess failed. %s' % process
        )
    else:
