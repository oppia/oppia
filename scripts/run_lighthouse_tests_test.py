...
class RunLighthouseTestsTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.lighthouse_check_bash_command = [
            ...
            lhci_path,
            'autorun',
            '--config=%s' % (LIGHTHOUSE_CONFIG_FILENAMES[LIGHTHOUSE_MODE_PERFORMANCE]),
    ...
    def test_run_lighthouse_puppeteer_script_failed(self) -> None:
        ...
        self.assertIn(
            'Puppeteer script failed. More details can be found above.', self.print_arr
        )
    ...
    def test_puppeteer_script_succeeds_when_recording_succeeds(self) -> None:
        ...
        self.assertIn(
            'Resulting puppeteer video saved at %s' % self.extra_args[1], self.print_arr
        )
    ...
    def test_run_lighthouse_checks_succesfully(self) -> None:
        ...
        with self.print_swap, swap_popen:
            run_lighthouse_tests.run_lighthouse_checks(LIGHTHOUSE_MODE_PERFORMANCE)
    ...
    def test_run_lighthouse_checks_failed(self) -> None:
        ...
        self.assertIn('ABC error.', self.print_arr)
        self.assertIn(
            'Lighthouse checks failed. More details can be found above.', self.print_arr
        )
    ...
    def test_run_lighthouse_tests_in_performance_mode(self) -> None:
        ...
        swap_isdir = self.swap(os.path, 'isdir', lambda _: True)
        swap_build = self.swap_with_checks(
            build, 'main', lambda args: None, expected_kwargs=[{'args': ['--prod_env']}]
        )
    ...
    def test_run_lighthouse_tests_skipping_webpack_build_in_performance_mode(
        ...
        self.assertIn(
            'Building files in production mode skipping webpack build.', self.print_arr
        )
        self.assertIn('Puppeteer script completed successfully.', self.print_arr)
