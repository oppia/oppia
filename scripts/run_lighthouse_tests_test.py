...
class RunLighthouseTestsTests(test_utils.GenericTestBase):
    ...
    def setUp(self) -> None:
        ...
        self.swap_dev_appserver = mock.patch.object(
            servers,
            'managed_dev_appserver',
            side_effect=lambda *unused_args, **unused_kwargs: MockCompilerContextManager(),
        )
        with open('dummy-lighthouse-pages.json', 'w', encoding='utf-8') as f:
            f.write(
                json.dumps(
                    {
                        'splash': {'url': 'http://localhost:8181/'},
                        'about': {'url': 'http://localhost:8181/about'},
                        'contact': {'url': 'http://localhost:8181/contact'},
                    }
                )
            )
    ...
    def test_run_lighthouse_puppeteer_script_successfully(self) -> None:
        ...
        swap_popen = mock.patch.object(
            subprocess,
            'Popen',
            side_effect=mock_popen,
        )

        with self.print_patch, swap_popen as popen_mock:
    ...
    def test_puppeteer_script_succeeds_when_recording_succeeds(self) -> None:
        ...
        popen_mock.assert_called_once_with(
            self.puppeteer_bash_command + self.extra_args,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
            ...
            with self.swap_elasticsearch_dev_server, self.swap_dev_appserver:
                with self.swap_ng_build, self.print_patch:
                    with self.swap_redis_server, swap_run_lighthouse_tests:
                        with self.swap_firebase_auth_emulator, self.swap_cloud_datastore_emulator:
                            with self.lighthouse_pages_json_filepath_patch:
            ...
            with self.swap_elasticsearch_dev_server, self.swap_dev_appserver:
                with self.swap_redis_server, self.swap_cloud_datastore_emulator:
                    with self.swap_firebase_auth_emulator, swap_build:
                        with (
                            swap_popen
                        ), swap_run_lighthouse_tests as run_checks_mock:
                            with self.lighthouse_pages_json_filepath_patch:
                                run_lighthouse_tests.main(
    ...
    def test_run_lighthouse_tests_skipping_webpack_build_in_performance_mode(
        ...
        with self.swap_redis_server, swap_run_lighthouse_tests as run_checks_mock:
            with self.swap_ng_build, self.swap_elasticsearch_dev_server, self.print_patch, self.swap_webpack_compiler:
                with self.swap_firebase_auth_emulator, self.swap_cloud_datastore_emulator:
                    with self.swap_dev_appserver, self.lighthouse_pages_json_filepath_patch:
                        with swap_isdir, swap_popen:
                            run_lighthouse_tests.main(
                                args=[
                                    '--mode',
                                    'performance',
    ...
    def test_main_function_calls_puppeteer_record(self) -> None:
        ...
        for path in common.CHROME_PATHS:
            if os.path.isfile(path):
                env['CHROME_BIN'] = path
                break
        swap_dev_appserver = mock.patch.object(
            servers,
            'managed_dev_appserver',
            side_effect=lambda *unused_args, **unused_kwargs: MockCompilerContextManager(),
        )
        swap_run_puppeteer_script = mock.patch.object(
            run_lighthouse_tests,
            'run_lighthouse_puppeteer_script',
            side_effect=mock_run_puppeteer_script,
        )
        swap_run_lighthouse_tests = mock.patch.object(
            run_lighthouse_tests,
            'run_lighthouse_checks',
