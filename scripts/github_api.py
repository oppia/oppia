...
class GithubApi:
    ...
    def deep_get(
        ...
        if not keys or data is None:
            return data
        return self.deep_get(data.get(keys[0]), keys[1:])

    def get_github_auth_token(self) -> str:
    ...
    def get_authorization_bearer(self) -> str:
        """Formats the Github auth token to be used as a bearer token."""
        return f'Bearer {self.get_github_auth_token()}'
    ...
    def fetch_linked_issues_for_pull_request(
        ...
        query = (
            ...
            """) % (pull_request)
        response = self.run_graphql_query(query)
        issues: List[GithubIssueDict] = (
            self.deep_get(response, [
                'repository',
                'pullRequest',
    ...
    def fetch_latest_comment_from_issue(self, issue: int) -> GithubCommentDict:
