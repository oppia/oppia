...
class ManagedProcessTests(test_utils.TestBase):
    ...
    def swap_managed_cloud_datastore_emulator_io_operations(
        ...
        is_data_dir: Callable[
            [str], bool] = (lambda p: p == common.CLOUD_DATASTORE_EMULATOR_DATA_DIR)

        new_exists = (lambda p: data_dir_exists if is_data_dir(p) else old_exists(p))
    ...
    def test_concats_command_args_when_shell_is_true(self) -> None:
        ...
        self.assertEqual(popen_calls, [self.POPEN_CALL('a 1', {
            'shell': True
        })])

    def test_passes_command_args_as_list_of_strings_when_shell_is_false(self) -> None:
        popen_calls = self.exit_stack.enter_context(self.swap_popen())
        logs = self.exit_stack.enter_context(self.capture_logging())
    ...
    def test_filters_empty_strings_from_command_args_when_shell_is_true(self) -> None:
        popen_calls = self.exit_stack.enter_context(self.swap_popen())
        logs = self.exit_stack.enter_context(self.capture_logging())
    ...
    def test_filters_empty_strings_from_command_args_when_shell_is_false(self) -> None:
        popen_calls = self.exit_stack.enter_context(self.swap_popen())
        logs = self.exit_stack.enter_context(self.capture_logging())
    ...
    def test_killing_process_raises_no_exception_if_disabled(self) -> None:
        ...
        proc = self.exit_stack.enter_context(
            servers.managed_process(['a'], timeout_secs=10, raise_on_nonzero_exit=False)
        )
    ...
    def test_raise_when_process_errors(self) -> None:
        self.exit_stack.enter_context(self.swap_popen())
        self.exit_stack.enter_context(
            self.swap_to_always_raise(psutil, 'wait_procs', error=Exception('uh-oh'))
        )
    ...
    def test_managed_cloud_datastore_emulator_creates_missing_data_dir(self) -> None:
        self.exit_stack.enter_context(self.swap_popen())
    ...
    def test_managed_cloud_datastore_emulator_acknowledges_data_dir(self) -> None:
        popen_calls = self.exit_stack.enter_context(self.swap_popen())
    ...
    def test_managed_elasticsearch_dev_server(self) -> None:
        ...
        self.assertEqual(
            popen_calls[0].program_args, '%s/bin/elasticsearch -q' % common.ES_PATH
        )
        self.assertEqual(
    ...
    def test_managed_redis_server_deletes_redis_dump_when_it_exists(self) -> None:
        original_os_remove = os.remove
        original_os_path_exists = os.path.exists
    ...
    def test_managed_portserver(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception, 'Process Portserver.* exited unexpectedly with exit code 1'
        ):
            self.exit_stack.close()
        ...
        self.assertEqual(
            ...
            '--portserver_unix_socket_address %s' % (common.PORTSERVER_SOCKET_FILEPATH),
        )
    ...
    def test_managed_webpack_compiler_in_watch_mode_when_build_succeeds(self) -> None:
        popen_calls = self.exit_stack.enter_context(
    ...
    def test_managed_webpack_compiler_in_watch_mode_raises_when_not_built(self) -> None:
    ...
    def test_managed_webpack_compiler_uses_prod_source_maps_config(self) -> None:
        popen_calls = self.exit_stack.enter_context(
            self.swap_popen(outputs=[b'Built at: 123'])
        )

        self.exit_stack.enter_context(
            servers.managed_webpack_compiler(use_prod_env=True, use_source_maps=True)
        )
        self.exit_stack.close()
    ...
    def test_managed_webpack_compiler_uses_prod_config(self) -> None:
        ...
        self.exit_stack.enter_context(
            servers.managed_webpack_compiler(use_prod_env=True, use_source_maps=False)
        )
        self.exit_stack.close()
        ...
        self.assertEqual(
            popen_calls[0].program_args, '%s %s --config %s' %
            (common.NODE_BIN_PATH, common.WEBPACK_BIN_PATH, common.WEBPACK_PROD_CONFIG)
        )
    ...
    def test_managed_webpack_compiler_uses_dev_source_maps_config(self) -> None:
        ...
        self.exit_stack.enter_context(
            servers.managed_webpack_compiler(use_prod_env=False, use_source_maps=True)
        )
        self.exit_stack.close()
    ...
    def test_managed_webpack_compiler_uses_dev_config(self) -> None:
        ...
        self.exit_stack.enter_context(
            servers.managed_webpack_compiler(use_prod_env=False, use_source_maps=False)
        )
        self.exit_stack.close()
        ...
        self.assertEqual(
            popen_calls[0].program_args, '%s %s --config %s' %
            (common.NODE_BIN_PATH, common.WEBPACK_BIN_PATH, common.WEBPACK_DEV_CONFIG)
        )
    ...
    def test_managed_webdriverio_server_fails_to_get_chrome_version(self) -> None:
        popen_calls = self.exit_stack.enter_context(self.swap_popen())
        self.exit_stack.enter_context(self.swap(common, 'OS_NAME', 'Linux'))
        self.exit_stack.enter_context(
    ...
    def test_managed_acceptance_test_server_with_explicit_args(self) -> None:
        popen_calls = self.exit_stack.enter_context(self.swap_popen())
        test_file_path = ('blog-admin/assign-roles-to-users-and-change-tag-properties')
    ...
    def test_managed_acceptance_test_server_headless(self) -> None:
        popen_calls = self.exit_stack.enter_context(self.swap_popen())
        suite_name = ('blog-admin/assign-roles-to-users-and-change-tag-properties')
