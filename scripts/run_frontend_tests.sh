...
if [ "$RUN_MINIFIED_TESTS" = "true" ]; then
  ...
  echo ""
  $PYTHON_CMD scripts/build.py --compile --prod_env --minify_third_party_libs_only
  $XVFB_PREFIX $NODE_MODULE_DIR/karma/bin/karma start core/tests/karma.conf.ts --prod_env=True
fi
