...
find . -type f \( -name "*.spec.ts" ! -name "state-name-editor.directive.spec.ts" ! -name "combined-tests.spec.ts" -o -name "*Spec.ts" \) -exec cat {} \; > ./core/templates/dev/head/combined-tests.spec.ts
