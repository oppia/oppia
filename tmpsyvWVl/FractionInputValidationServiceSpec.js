...
describe('FractionInputValidationService', function () {
    ...
    var denominatorEqualsFiveRule, equalsOneAndHalfRule, equalsOneRule, equalsThreeByTwoRule, equivalentToOneAndSimplestFormRule, equivalentToOneRule, exactlyEqualToOneAndNotInSimplestFormRule, HasFractionalPartExactlyEqualToOneAndHalfRule, HasFractionalPartExactlyEqualToNegativeValue, HasFractionalPartExactlyEqualToThreeHalfs, HasFractionalPartExactlyEqualToTwoFifthsRule, greaterThanMinusOneRule, integerPartEqualsOne, integerPartEqualsZero, lessThanTwoRule, nonIntegerRule, numeratorEqualsFiveRule, zeroDenominatorRule;
    ...
    beforeEach(angular.mock.inject(function ($injector) {
        ...
        createFractionDict = function (isNegative, wholeNumber, numerator, denominator) {
            return {
        ...
        answerGroups = [agof.createNew([equalsOneRule, lessThanTwoRule], goodDefaultOutcome, false, null)];
    }));
    ...
    it('should catch redundant rules caused by greater/less than range', function () {
    ...
    it('should catch non integer inputs in the numerator', function () {
        ...
        expect(warnings).toEqual([{
                type: WARNING_TYPES.ERROR,
                message: ('Rule ' + 1 + ' from answer group ' +
    ...
    it('should catch not allowImproperFraction and rule has improper fraction', function () {
    ...
    it('should catch not allowNonzeroIntegerPart and rule has integer part', function () {
    ...
    it('should correctly check validity of HasFractionalPartExactlyEqualTo rule', function () {
        customizationArgs.requireSimplestForm = false;
        answerGroups[0].rules = [HasFractionalPartExactlyEqualToOneAndHalfRule];
        var warnings = validatorService.getAllWarnings(currentState, customizationArgs, answerGroups, goodDefaultOutcome);
        expect(warnings).toEqual([{
                type: WARNING_TYPES.ERROR,
                message: ('Rule 1 from answer group 1 is invalid as ' +
