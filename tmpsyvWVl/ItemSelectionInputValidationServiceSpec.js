...
require('interactions/ItemSelectionInput/directives/' +
...
describe('ItemSelectionInputValidationService', function () {
    ...
    var goodAnswerGroups = null, goodDefaultOutcome = null;
    var customizationArguments = null;
    var IsProperSubsetValidOption = null;
    var oof = null, agof = null, rof = null;
    var badAnswerGroup = null;
    var ThreeInputsAnswerGroups = null, OneInputAnswerGroups = null, NoInputAnswerGroups = null;
    beforeEach(function () {
    ...
    beforeEach(angular.mock.inject(function ($injector) {
        ...
        goodAnswerGroups = [agof.createNew([rof.createFromBackendDict({
                    rule_type: 'Equals',
                    inputs: {
        ...
        ThreeInputsAnswerGroups = [agof.createNew([rof.createFromBackendDict({
                    rule_type: 'Equals',
                    inputs: {
        ...
        OneInputAnswerGroups = [agof.createNew([rof.createFromBackendDict({
                    rule_type: 'Equals',
                    inputs: {
                        x: ['Selection 1']
                    }
        ...
        NoInputAnswerGroups = [agof.createNew([rof.createFromBackendDict({
        ...
        IsProperSubsetValidOption = [agof.createNew([rof.createFromBackendDict({
    ...
    it('should expect the minAllowableSelectionCount to be less than or ' +
        ...
        var warnings = validatorService.getAllWarnings(currentState, customizationArguments, ThreeInputsAnswerGroups, goodDefaultOutcome);
    ...
    it('should expect more that 1 element to be in the rule input, if the ' +
        '"proper subset" rule is used.', function () {
        var warnings = validatorService.getAllWarnings(currentState, customizationArguments, IsProperSubsetValidOption, goodDefaultOutcome);
        expect(warnings).toEqual([{
                type: WARNING_TYPES.ERROR,
                message: ('In answer group 1, ' +
                    'rule 1, the "proper subset" rule must include at least 2 options.')
            }]);
    });
    it('should expect number of correct options to be in between the maximum ' +
        'and minimum allowed selections when the "Equals" rule is used.', function () {
        // Make min allowed selections greater than correct answers.
        customizationArguments.minAllowableSelectionCount.value = 2;
        var warnings = validatorService.getAllWarnings(currentState, customizationArguments, OneInputAnswerGroups, goodDefaultOutcome);
        expect(warnings).toEqual([{
                type: WARNING_TYPES.ERROR,
                message: ('In answer group 1, rule 1, the number of correct options in ' +
                    'the "Equals" rule should be between 2 and 2 (the ' +
                    'minimum and maximum allowed selection counts).')
            }]);
    });
    it('should expect at least one option when ' +
        '"ContainsAtLeastOneOf" rule is used.', function () {
        var warnings = validatorService.getAllWarnings(currentState, customizationArguments, NoInputAnswerGroups, goodDefaultOutcome);
        expect(warnings).toEqual([{
                type: WARNING_TYPES.ERROR,
                message: ('In answer group 1, rule 1, the "ContainsAtLeastOneOf" rule ' +
