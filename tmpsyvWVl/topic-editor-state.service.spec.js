...
var TopicRightsObjectFactory_ts_1 = require("domain/topic/TopicRightsObjectFactory.ts");
...
describe('Topic editor state service', function () {
    ...
    beforeEach(function () {
        angular.mock.module(function ($provide) {
            $provide.value('TopicRightsObjectFactory', new TopicRightsObjectFactory_ts_1.TopicRightsObjectFactory());
        });
    });
    beforeEach(angular.mock.module('oppia', GLOBALS.TRANSLATOR_PROVIDER_FOR_TESTS));
    beforeEach(angular.mock.module('oppia', function ($provide) {
        fakeEditableTopicBackendApiService = (FakeEditableTopicBackendApiService());
        $provide.value('EditableTopicBackendApiService', [fakeEditableTopicBackendApiService][0]);
        fakeTopicRightsBackendApiService = (FakeTopicRightsBackendApiService());
        $provide.value('TopicRightsBackendApiService', [fakeTopicRightsBackendApiService][0]);
    }));
    beforeEach(angular.mock.inject(function ($injector) {
        ...
        fakeTopicRightsBackendApiService.backendTopicRightsObject = (topicRightsObject);
        ...
        fakeEditableTopicBackendApiService.newBackendSubtopicPageObject = (subtopicPageObject);
    ...
    it('should request to load the subtopic page from the backend', function () {
        spyOn(fakeEditableTopicBackendApiService, 'fetchSubtopicPage').and.callThrough();
        TopicEditorStateService.loadSubtopicPage('validTopicId', 1);
        expect(fakeEditableTopicBackendApiService.fetchSubtopicPage).toHaveBeenCalled();
    });
    it('should not request to load the subtopic page from the backend after ' +
        'loading it once', function () {
        spyOn(fakeEditableTopicBackendApiService, 'fetchSubtopicPage').and.callThrough();
        var subtopicPage = SubtopicPageObjectFactory.createFromBackendDict(secondSubtopicPageObject);
        TopicEditorStateService.setSubtopicPage(subtopicPage);
        TopicEditorStateService.loadSubtopicPage('validTopicId', 0);
        expect(fakeEditableTopicBackendApiService.fetchSubtopicPage).not.toHaveBeenCalled();
    });
    it('should not add duplicate subtopic pages to the local cache', function () {
        ...
        TopicEditorStateService.setSubtopicPage(subtopicPage);
        expect(TopicEditorStateService.getCachedSubtopicPages().length).toEqual(1);
        expect(TopicEditorStateService.getSubtopicPage().getPageContents().getHtml()).toEqual('<p>New Data</p>');
    });
    it('should correctly delete newly created subtopic pages from the ' +
        ...
        expect(TopicEditorStateService.getCachedSubtopicPages()[0].getId()).toEqual('validTopicId-0');
        expect(TopicEditorStateService.getCachedSubtopicPages()[0].getPageContents()
            .getHtml()).toEqual('<p>Data</p>');
        expect(TopicEditorStateService.getCachedSubtopicPages()[1].getId()).toEqual('validTopicId-1');
        expect(TopicEditorStateService.getCachedSubtopicPages()[1].getPageContents()
            .getHtml()).toEqual('<p>Data 2</p>');
    });
    ...
    it('should correctly delete already existing subtopic pages without ' +
        ...
        expect(TopicEditorStateService.getCachedSubtopicPages().length).toEqual(1);
        expect(TopicEditorStateService.getCachedSubtopicPages()[0].getId()).toEqual('validTopicId-1');
        expect(TopicEditorStateService.getCachedSubtopicPages()[0].getPageContents()
            .getHtml()).toEqual('<p>Data 1</p>');
    });
    it('should request to load the topic rights from the backend', function () {
    ...
    it('should fire an init event after loading the first topic', function () {
        spyOn($rootScope, '$broadcast').and.callThrough();
        TopicEditorStateService.loadTopic(5);
        $rootScope.$apply();
        expect($rootScope.$broadcast).toHaveBeenCalledWith('topicInitialized');
    });
    it('should fire a loaded event after loading a new subtopic page', function () {
        spyOn($rootScope, '$broadcast').and.callThrough();
        TopicEditorStateService.loadSubtopicPage('validTopicId', 1);
        $rootScope.$apply();
        expect($rootScope.$broadcast).toHaveBeenCalledWith('subtopicPageLoaded');
    ...
    it('should indicate a topic is no longer loading after an error', function () {
        expect(TopicEditorStateService.isLoadingTopic()).toBe(false);
    ...
    it('it should report that a topic has loaded through loadTopic()', function () {
    ...
    it('it should report that a topic has loaded through setTopic()', function () {
        expect(TopicEditorStateService.hasLoadedTopic()).toBe(false);
        var newTopic = TopicObjectFactory.create(secondBackendTopicObject.topicDict, secondBackendTopicObject.skillIdToDescriptionDict);
    ...
    it('should be able to set a new topic with an in-place copy', function () {
        var previousTopic = TopicEditorStateService.getTopic();
        var expectedTopic = TopicObjectFactory.create(secondBackendTopicObject.topicDict, secondBackendTopicObject.skillIdToDescriptionDict);
    ...
    it('should be able to set a new topic rights with an in-place copy', function () {
        var previousTopicRights = TopicEditorStateService.getTopicRights();
        var expectedTopicRights = topicRightsObjectFactory.createFromBackendDict(secondTopicRightsObject);
    ...
    it('should fail to save the topic without first loading one', function () {
        expect(function () {
            TopicEditorStateService.saveTopic('Commit message');
        }).toThrow();
    });
    it('should not save the topic if there are no pending changes', function () {
        TopicEditorStateService.loadTopic(5);
        $rootScope.$apply();
        spyOn($rootScope, '$broadcast').and.callThrough();
        expect(TopicEditorStateService.saveTopic('Commit message')).toBe(false);
    ...
    it('should be able to save the topic and pending changes', function () {
        ...
        TopicUpdateService.setTopicName(TopicEditorStateService.getTopic(), 'New name');
        $rootScope.$apply();
        expect(TopicEditorStateService.saveTopic('Commit message')).toBe(true);
        $rootScope.$apply();
        var expectedId = '0';
        var expectedVersion = '1';
        var expectedCommitMessage = 'Commit message';
        var updateTopicSpy = (fakeEditableTopicBackendApiService.updateTopic);
        expect(updateTopicSpy).toHaveBeenCalledWith(expectedId, expectedVersion, expectedCommitMessage, jasmine.any(Object));
    });
    ...
    it('should indicate a topic is no longer saving after an error', function () {
        TopicEditorStateService.loadTopic(5);
        TopicUpdateService.setTopicName(TopicEditorStateService.getTopic(), 'New name');
        $rootScope.$apply();
        expect(TopicEditorStateService.isSavingTopic()).toBe(false);
