...
var ExplorationFeaturesService_ts_1 = require("services/ExplorationFeaturesService.ts");
...
describe('Learner Action Render Service', function () {
    ...
    beforeEach(angular.mock.module(function ($provide) {
        ...
        $provide.value('ExplorationFeaturesService', new ExplorationFeaturesService_ts_1.ExplorationFeaturesService());
    ...
    describe('Test learner action render service functions', function () {
        beforeEach(angular.mock.inject(function ($injector) {
            ...
            spyOn(this.ExplorationStatesService, 'getState')
                .withArgs('stateName1').and.returnValue({ interaction: { id: 'Continue' } })
                .withArgs('stateName2').and.returnValue({ interaction: { id: 'TextInput' } })
                .withArgs('stateName3').and.returnValue({ interaction: {
        ...
        it('should render tables for MultipleIncorrectSubmissions issue block.', function () {
            ...
            expect(this.$sce.getTrustedHtml(finalBlockHTML)).toEqual('1. Started exploration at card "stateName1".' +
        ...
        it('should render HTML for learner action display blocks.', function () {
            this.PlaythroughService.recordExplorationStartAction('stateName1');
            this.PlaythroughService.recordAnswerSubmitAction('stateName1', 'stateName2', 'Continue', '', 'Welcome', 30);
            this.PlaythroughService.recordAnswerSubmitAction('stateName2', 'stateName3', 'TextInput', 'Hello', 'Go ahead', 30);
            this.PlaythroughService.recordAnswerSubmitAction('stateName3', 'stateName3', 'MultipleChoiceInput', 'Choice1', 'Go ahead', 30);
            this.PlaythroughService.recordExplorationQuitAction('stateName2', 120);
            var learnerActions = this.PlaythroughService.getPlaythrough().actions;
            var displayBlocks = this.LearnerActionRenderService.getDisplayBlocks(learnerActions);
            expect(displayBlocks.length).toEqual(1);
            ...
            for (var i = 0; i < displayBlocks[0].length; i++) {
                actionHtmlList.push(this.LearnerActionRenderService.renderLearnerAction(displayBlocks[0][i], 0, i + 1));
            }
            expect(actionHtmlList[0]).toEqual('1. Started exploration at card "stateName1".');
            expect(actionHtmlList[1]).toEqual('2. Pressed "Continue" to move to card "stateName2" after 30 seconds.');
            expect(actionHtmlList[2]).toEqual('<answer-submit-action answer="&amp;quot;Hello&amp;quot;" ' +
            ...
            expect(actionHtmlList[3]).toEqual('<answer-submit-action answer="&amp;quot;Choice1&amp;quot;" ' +
                'dest-state-name="stateName3" time-spent-in-state-secs="30" ' +
            ...
            expect(actionHtmlList[4]).toEqual('5. Left the exploration after spending a total of 120 seconds on ' +
                'card "stateName2".');
        });
    });
});
