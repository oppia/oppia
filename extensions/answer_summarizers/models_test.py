...
class AnswerFrequenciesUnitTests(CalculationUnitTestBase):
    ...
    def test_answer_frequencies_are_not_calculated_for_linear_interactions(
        ...
        with self.assertRaisesRegex(
            Exception,
            'Linear interaction \'Continue\' is not allowed for the calculation of answers\' frequencies.',
        ):
...
class Top5AnswerFrequenciesUnitTests(CalculationUnitTestBase):
    ...
    def test_top_5_answers_are_not_calculated_for_linear_interactions(
        ...
        with self.assertRaisesRegex(
            Exception,
            'Linear interaction \'Continue\' is not allowed for the calculation of top 5 answers, by frequency.',
        ):
            self._perform_calculation(state_answers_dict)
...
class Top10AnswerFrequenciesUnitTests(CalculationUnitTestBase):
    ...
    def test_top_10_answers_are_not_calculated_for_linear_interactions(
        ...
        with self.assertRaisesRegex(
            Exception,
            'Linear interaction \'Continue\' is not allowed for the calculation of top 10 answers, by frequency.',
        ):
            self._perform_calculation(state_answers_dict)
...
class FrequencyCommonlySubmittedElementsUnitTests(CalculationUnitTestBase):
    ...
    def test_common_answers_are_not_calculated_for_linear_interactions(
        ...
        with self.assertRaisesRegex(
            Exception,
            'Linear interaction \'Continue\' is not allowed for the calculation of commonly submitted answers\' frequencies.',
        ):
            self._perform_calculation(state_answers_dict)
    ...
    def test_raises_error_if_non_iterable_answer_provided(self) -> None:
        ...
        with self.assertRaisesRegex(
            Exception,
            'To calculate commonly submitted answers\' frequencies, answers must be provided in an iterable form, like: SetOfUnicodeString.',
        ):
            self._perform_calculation(state_answers_dict)
...
class TopAnswersByCategorizationUnitTests(CalculationUnitTestBase):
    ...
    def test_top_answers_are_not_calculated_for_linear_interactions(
        ...
        with self.assertRaisesRegex(
            Exception,
            'Linear interaction \'Continue\' is not allowed for the calculation of top submitted answers, by frequency.',
        ):
            self._perform_calculation(state_answers_dict)
