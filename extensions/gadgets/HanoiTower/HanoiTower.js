...
oppia.directive('oppiaGadgetHanoiTower', [
  'oppiaHtmlEscaper', 'learnerParamsService', function(oppiaHtmlEscaper, learnerParamsService) {
    return {
      restrict: 'E',
      templateUrl: 'gadget/HanoiTower',
      controller: ['$scope', '$element', function ($scope, $element) {
        $scope.numberOfDisks = 3;

        $scope.$watch(function() {
          return learnerParamsService.getValue('HanoiTower0numDisks');
        }, function(newValue, oldValue) {
          if (newValue) {
            $scope.numberOfDisks = parseInt(newValue);
            $scope.resetGame();
          }
        });
