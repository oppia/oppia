...
export class SvgEditorComponent implements OnInit {
    ...
    setSavedSvgFilename(filename: string, setData: boolean): void {
        ...
        this.data = {
            ...
            savedSvgUrl:
                this.getTrustedResourceUrlForSvgFileName(filename).safeUrl,
        };
        this.value = filename;
                ...
                this.postSvgToServer(dimensions, resampledFile).then(
                    data => {
                        ...
                        img.onload = () => {
                            ...
                            var dimensions =
                                this.imagePreloaderService.getDimensionsOfImage(
                                    data.filename
                                );
                            this.svgContainerStyle = {
                                height: dimensions.height + 'px',
    ...
    private createBezierControlPoints(
        left: number,
        top: number
    ): fabric.Circle {
        ...
        this.canvas.on('mouse:down', options => {
            ...
            if (this.drawMode === this.DRAW_MODE_POLY) {
                ...
                if (
                    ...
                    this.setPolyStartingPoint(options);
                    this.polyOptions.lines[
                        this.polyOptions.lineCounter - 1
                    ].set({
