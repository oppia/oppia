...
describe('SvgEditor', () => {
    ...
    it(
        ...
        waitForAsync(
            fakeAsync(() => {
                spyOnProperty(document, 'readyState').and.returnValue(
                    'loading'
                );
                spyOn(document, 'addEventListener').and.callFake(
                    (eventName, handler) => {
...
describe('SvgEditor initialized with value attribute', () => {
    ...
    var mockAssetsBackendApiService = {
        getImageUrlForPreview: (contentType, contentId, filepath) => {
            return (
                '/imageurl_' + contentType + '_' + contentId + '_' + filepath
            );
        },
    };
    var mockImagePreloaderService = {
    ...
    beforeEach(waitForAsync(() => {
        ...
        component =
            TestBed.createComponent(SvgEditorComponent).componentInstance;
