...
class GraphRuleUnitTests(test_utils.GenericTestBase):
    ...
    def test_is_isomorphic_rule(self):
        ...
        # Testing that isomorphism is only checked up to the vertex limit
        self.assertTrue(graph.IsIsomorphicTo(
            _nullGraph(graph.IsIsomorphicTo.ISOMORPHISM_VERTEX_LIMIT)
        ).eval(
            _nullGraph(graph.IsIsomorphicTo.ISOMORPHISM_VERTEX_LIMIT)
        ))
        self.assertFalse(graph.IsIsomorphicTo(
            _nullGraph(graph.IsIsomorphicTo.ISOMORPHISM_VERTEX_LIMIT + 1)
        ).eval(
            _nullGraph(graph.IsIsomorphicTo.ISOMORPHISM_VERTEX_LIMIT + 1)
        ))
