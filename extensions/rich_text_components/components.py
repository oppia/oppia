...
class BaseRteComponent:
    ...
    rich_text_component_specs = constants.parse_json_from_ts(constants.get_package_file_contents(package, filepath))
    ...
    def validate(cls, value_dict: Any) -> None:
        ...
        customization_arg_specs = cls.rich_text_component_specs[cls.__name__]['customization_arg_specs']
        ...
        if set(attr_names) != set(required_attr_names):
            ...
            raise utils.ValidationError('Missing attributes: %s, Extra attributes: %s' % (', '.join(missing_attr_names), ', '.join(extra_attr_names)))
...
class Collapsible(BaseRteComponent):
    ...
    def validate(cls, value_dict: Dict[str, str]) -> None:
        ...
        collapsible_components = inner_soup.findAll(name='oppia-noninteractive-collapsible')
...
class Math(BaseRteComponent):
    ...
    def validate(cls, value_dict: Dict[str, Dict[str, str]]) -> None:
        ...
        if not re.match(filename_pattern_regex, filename):
            raise utils.ValidationError('Invalid svg_filename attribute in math component: %s' % (filename))
