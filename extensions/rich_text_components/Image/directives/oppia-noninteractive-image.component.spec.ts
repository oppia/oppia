...
describe('NoninteractiveImage', () => {
  ...
  it("should preload image when the user click 'Reload Image'", fakeAsync(() => {
    spyOn(imagePreloaderService, 'getImageUrlAsync').and.resolveTo(dataUrlSvg);
  ...
  it('should throw error if image url is empty', fakeAsync(() => {
    spyOn(imagePreloaderService, 'getImageUrlAsync').and.resolveTo(null);
    expect(() => {
      component.loadImage();
      tick();
    }).toThrowError();
  ...
  it("should display 'Reload Image' when the image cannot be loaded", fakeAsync(() => {
