...
describe('RTE display component', () => {
  ...
  describe('Null safety for strict mode compliance', () => {
    ...
    it('should return empty string when math content lacks raw_latex property', () => {
      const mathNode = document.createElement('oppia-noninteractive-math');
    ...
    it('should handle null element when highlighting during voiceover', fakeAsync(() => {
      ...
      spyOn(
        automaticVoiceoverHighlightService,
        'getCurrentSentenceIdToHighlight'
      ).and.returnValue('nonexistent-id');
    ...
    it('should handle null previous element when clearing highlight', fakeAsync(() => {
      ...
      component.previousHighlightedElementId = 'nonexistent-id';
      spyOn(document, 'getElementById').and.returnValue(null);
    ...
    it('should handle text node with null parentElement in ngOnChanges', fakeAsync(() => {
      const changes: SimpleChanges = {
        rteString: {
          previousValue: '',
          currentValue: '<p>Test content</p>',
          firstChange: true,
          isFirstChange: () => true,
        },
      };

      const orphanTextNode = document.createTextNode('orphan text');
      Object.defineProperty(orphanTextNode, 'parentElement', {
        value: null,
        writable: false,
      });
      ...
      component.rteString = '<p>Test content</p>';
      fixture.detectChanges();
    ...
    it('should clear highlight interval on ngOnDestroy', fakeAsync(() => {
    ...
    it('should normalize spacing between inline elements when rendering', fakeAsync(() => {
      // Test the core fix: spacing normalization prevents words from running together.
      spyOn(
        component,
        'isAutomaticVoiceoverRegenerationFromExpFeatureEnabled'
      ).and.returnValue(false);

      component.rteString = '<p>time.<em>Incidentally</em></p>';
      fixture.detectChanges();

      const changes: SimpleChanges = {
        rteString: {
          previousValue: '',
          currentValue: '<p>time.<em>Incidentally</em></p>',
          firstChange: false,
          isFirstChange: () => false,
        },
      };
      ...
      // After normalization, there should be a space between the period and <em> tag.
      const content = fixture.nativeElement.textContent || '';
      expect(content).toContain('time. Incidentally');
      expect(content).not.toContain('time.Incidentally');
      discardPeriodicTasks();
    }));
  });
});
