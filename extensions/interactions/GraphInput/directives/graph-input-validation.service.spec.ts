...
describe('GraphInputValidationService', () => {
    ...
    expect(warnings).toEqual([
      {
        ...
        message:
          'The graph used in customization exceeds supported maximum ' +
  ...
  it(
    'should validate the maximum number of vertices in the graph for an ' +
      'isomorphism check',
    () => {
      (answerGroups[0].rules[0].inputs.g as GraphAnswer).vertices = new Array(
        11
      );
      (answerGroups[0].rules[1].inputs.g as GraphAnswer).vertices = new Array(
        11
      );
      (answerGroups[1].rules[0].inputs.g as GraphAnswer).vertices = new Array(
        11
      );
      var warnings = validatorService.getAllWarnings(
        currentState,
      ...
      expect(warnings).toEqual([
        {
          type: WARNING_TYPES.CRITICAL,
          message:
            'The graph used in the learner answer 1 in Oppia response ' +
        ...
        },
        {
          type: WARNING_TYPES.CRITICAL,
          message:
            'The graph used in the learner answer 2 in Oppia response ' +
            '1 exceeds supported maximum number of vertices of ' +
            '10 for isomorphism check.',
        },
        {
  ...
  it('should throw error when rule is invalid', () => {
    ...
    expect(warnings).toEqual([
      {
        type: AppConstants.WARNING_TYPES.CRITICAL,
        message: 'Learner answer 1 in Oppia response 1 is invalid.',
      },
      {
        type: AppConstants.WARNING_TYPES.CRITICAL,
        message: 'Learner answer 1 in Oppia response 2 is invalid.',
      },
    ]);
  });
});
