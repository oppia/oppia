...
var customizeInteraction = function(elem, assumption, target, defaultText) {
  elem.element(by.id('logicQuestionAssumptions'))
    .sendKeys(assumption);
  elem.element(by.id('logicQuestionTarget'))
    .sendKeys(target);
  elem.element(by.id('logicQuestionProof'))
    .sendKeys(defaultText);
};

var expectInteractionDetailsToMatch = function(elem) {
  expect(
    elem.element(by.tagName('oppia-interactive-logic-proof'))
      .isPresent()
  )
    .toBe(true);
};
...
var submitAnswer = function() {
  ...
  element(by.css('.oppia-learner-confirm-button'))
    .click();
};
