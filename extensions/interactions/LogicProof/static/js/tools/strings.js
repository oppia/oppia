...
var DEFAULT_LINE_TEMPLATE_STRINGS = [
  ...
  // from commit 2d9335019521 when speed issues are fixed.
    ...
    variables: '',
    error: [
      'If you want to reach a contradiction from {{~R}} and so conclude' +
    ' {{R}} then you\'ll need to start from the law of the excluded middle' +
    ' \'We know {{R\u2228~R}}\.'
    ]
  }
];
...
var DEFAULT_VARIABLE_MISTAKE_STRINGS = [
  {
    ...
    occurs: (
      '~is_initializer(n) \u2227 \u2203x\u2208variables(n).' +
      '~is_initialized(x,n)'),
    message: [
    ...
    message: [
      'We originally took {{variable(scoper(n-1))}} as our arbitrary ' +
    'variable so this, rather than {{variable(n)}}, needs to be the one that' +
    ' we quantify out over.'
    ]
  }, {
...
var DEFAULT_TARGET_MISTAKE_STRINGS = [
  {
    ...
    message: [
      'We are trying to prove {{target()}} so it should be given by ' +
    'the final line of the proof.'
    ]
  }
];
...
var DEFAULT_CONTROL_FUNCTION_STRINGS = [
  {
    ...
    description: 'The most recent line (not including n) in ' +
    'whose scope line n is'
  }, {
    LHS: 'scoper2(n)',
    ...
    'is_in_scope(k,n))'),
    description: 'Whether variable x is initialized and still ' +
    'available by line n'
  }, {
    ...
    description: (
      'Whether there is a line before n that proves R (again, it may ' +
    'still be out of scope and thus unusable).')
  }, {
    LHS: 'prover(R,n)',
    ...
    LHS: 'yields_false(n)',
    RHS: 'template(n)=\'not_eliminate\'\u2228template(n)=\'or_' +
    'eliminate_false\'',
    ...
    description: (
      'Whether line n is a proof of S under the assumption of R, ' +
      'made on line m')
  }, {
