  ...
  '$scope', function($scope) {
    ...
    $scope.displayMessage = function(message, line) {
      ...
      while (pointer < message.length) {
        var nextSpace = message.slice(pointer, pointer + 70)
          .lastIndexOf(' ');
        var breakPoint = (
    ...
    $scope.doLocalCheck = function() {
      ...
      proof = logicProofStudent.buildProof($scope.proofString,
        questionInstance);
    ...
    $scope.replaceUnicode = function(input) {
      ...
      // We use this as .replace() only replaces one instance.
        output = output.split($scope.REPLACEMENT_PAIRS[i].old)
          .join(
