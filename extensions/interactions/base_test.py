...
_INTERACTION_CONFIG_SCHEMA: Final = [('name', str), ('display_mode', str),
                                     ('description', str),
                                     ('_customization_arg_specs', list),
                                     ('is_terminal', bool), ('needs_summary', bool),
                                     ('show_generic_submit_button', bool)]

AnswerVisualizationsDictKeys = Literal['id', 'options', 'calculation_id',
...
class InteractionAnswerUnitTests(test_utils.GenericTestBase):
    ...
    def test_get_rule_description_with_invalid_rule_name_raises_error(self) -> None:
    ...
    def test_get_rule_param_type_with_invalid_rule_param_name_raises_error(
        ...
        interaction = interaction_registry.Registry.get_interaction_by_id('CodeRepl')
        with self.assertRaisesRegex(
            Exception, 'Rule CodeEquals has no param called invalid_rule_param_name'
        ):
...
class InteractionUnitTests(test_utils.GenericTestBase):
        ...
        for ca_spec in customization_args:
            self.assertTrue(
                all(
                    ...
                    for attr in ['name', 'description', 'schema', 'default_value']
                )
            )
            ...
            if ca_spec.schema['type'] == 'custom':
                ...
                self.assertEqual(
                    ca_spec.default_value, obj_class.normalize(ca_spec.default_value)
                )
            self._set_expect_invalid_default_value(ca_spec.schema, False)

    def _validate_answer_visualization_specs(
        self, answer_visualization_specs: List[base.AnswerVisualizationSpecsDict]
    ) -> None:
        ...
        _answer_visualizations_specs_schema: List[
            Tuple[AnswerVisualizationsDictKeys,
                  Type[object]]] = [('id', str), ('options', dict),
                                    ('calculation_id', str),
    ...
    def _get_linear_interaction_ids(self) -> List[str]:
        ...
        return [
            ...
            if interaction_registry.Registry.get_interaction_by_id(interaction_id
                                                                  ).is_linear
        ]
    ...
    def test_interaction_properties(self) -> None:
        ...
        self.assertItemsEqual(
            list(interaction_dict.keys()), [
                'id', 'name', 'description', 'display_mode', 'customization_arg_specs',
                'is_trainable', 'is_terminal', 'is_linear', 'rule_descriptions',
                'instructions', 'narrow_instructions', 'needs_summary',
    ...
    def test_html_field_types_to_rule_specs_mapping_are_valid(self) -> None:
        ...
        generated_html_field_types_dict: Dict[str, Dict[str, Dict[str, Dict[str, Dict[
            str, Set[str]]]]]] = (
                collections.defaultdict(
                    lambda: collections.defaultdict(
                        lambda: collections.defaultdict(
                            lambda: collections.
                            defaultdict(lambda: collections.defaultdict(set))
                        )
                    )
                )
            )
        ...
        for html_type, html_type_dict in (html_field_types_to_rule_specs_dict.items()):
        ...
        for interaction_id, interaction_rules in (rule_descriptions_dict.items()):
            for rule_type, rule_description in interaction_rules.items():
                ...
                for html_type, input_variables in (
                    ...
                    html_type_dict['ruleTypes'][rule_type]['htmlInputVariables'
    ...
    def test_default_interactions_are_valid(self) -> None:
        ...
        for interaction_id in all_interaction_ids:
            ...
            try:
                self.assertTrue(
                    os.path.isfile(os.path.join(interaction_dir, 'webdriverio.js'))
                )
            ...
            if interaction_id in INTERACTIONS_THAT_USE_COMPONENTS:
                ...
                rules_service_ts_file = os.path.join(
                    directives_dir, '%s-rules.service.ts' % hyphenated_interaction_id
                )
            ...
            self.assertIn(
                'oppiaInteractive%s' % interaction_id, interaction_ts_file_content
            )
            self.assertIn('oppiaResponse%s' % interaction_id, response_ts_file_content)
            self.assertIn(
                'oppiaShortResponse%s' % interaction_id, short_response_ts_file_content
            )
            self.assertIn(
            ...
            self.assertIn(
                '%s-rules.service.ts' % hyphenated_interaction_id, ts_file_content
            )
            self.assertIn(
                '%s-validation.service.ts' % hyphenated_interaction_id, ts_file_content
            )
            ...
            for ind, visualization in enumerate(answer_visualizations):
                ...
                self.assertEqual(
                    visualization.options, answer_visualization_specs[ind]['options']
                )
            ...
            if interaction.is_linear:
                self.assertTrue(
                    isinstance(interaction.default_outcome_heading, str) and
            ...
            for rule_name in list(interaction.rules_dict.keys()):
                ...
                for (_, param_obj_cls) in param_list:
                    ...
                    if param_obj_cls.__name__ in [
                        'NonnegativeInt', 'ListOfCodeEvaluation', 'ListOfCoordTwoDim',
                        'ListOfGraph', 'SetOfNormalizedString'
                    ]:
                        continue
    ...
    def test_trainable_interactions_have_more_than_just_a_classifier(self) -> None:
    ...
    def test_get_interaction_dependency_ids_correctly(self) -> None:
        ...
        for interaction_id in all_interaction_ids:
            ...
            # test.
            self.assertEqual(interaction.dependency_ids, interaction._dependency_ids)  # pylint: disable=protected-access
    ...
    def test_linear_interaction_ids_list_matches_linear_interactions(self) -> None:
