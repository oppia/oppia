...
describe('MultipleChoiceInputValidationService', () => {
    ...
    goodAnswerGroups = [
      agof.createNew(
        [
          ...
          Rule.createFromBackendDict(ruleDict, 'MultipleChoiceInput')
        ),
        goodDefaultOutcome,
        [],
        null
  ...
  it('should expect at least two choices', () => {
    customizationArguments.choices.value = [new SubtitledHtml('1', '')];
    ...
    expect(warnings).toEqual([
      {
        type: WARNING_TYPES.CRITICAL,
        message:
          'Please ensure learner answer 1 in Oppia response 1 refers ' +
          'to a valid choice.',
      },
    ]);
