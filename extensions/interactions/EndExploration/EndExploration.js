    ...
    return {
      ...
      controller: [
        ...
        function(
          ...
          var authorRecommendationsPromise = (
            authorRecommendationsDeferred.promise);
            ...
            $http({
              ...
              data.summaries.map(function(explorationSummary, index) {
                if (explorationSummary) {
                  authorRecommendedExplorationSummaries.push(
                    explorationSummary);
                } else {
                  if ($scope.isInEditorPage) {
                    $scope.invalidExpIds.push(
                      authorRecommendedExplorationIds[index]);
                  }
              ...
              $scope.recommendedExplorationIds = (
                authorRecommendedExplorationIds);
              $scope.recommendedExplorationSummaries = (
                authorRecommendedExplorationSummaries);
            ...
            authorRecommendationsPromise.then(function() {
              ...
              $http.get('/explorehandler/recommendations/' + explorationId, {
                ...
                var filteredRecommendationExplorationIds =
                  ...
                  return (
                    $scope.recommendedExplorationIds.indexOf(value) == -1);
                });
                ...
                var filteredRecommendationsSize = (
                  filteredRecommendationExplorationIds.length);
                for (var i = 0;
                     i < Math.min(
                       filteredRecommendationsSize, maxSystemRecommendations);
                     i++) {
                ...
                if (systemRecommendedExplorationIds.length > 0) {
                  $http({
                    ...
                    params: {
                      stringified_exp_ids: JSON.stringify(
                        systemRecommendedExplorationIds)
                    }
                  }).success(function(data) {
                    $scope.recommendedExplorationIds = (
                      $scope.recommendedExplorationIds.concat(
                        systemRecommendedExplorationIds));
                    $scope.recommendedExplorationSummaries = (
                      $scope.recommendedExplorationSummaries.concat(
                        data.summaries));
                  });
                }
              });
