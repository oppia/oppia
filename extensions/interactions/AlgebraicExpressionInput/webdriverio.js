...
var customizeInteraction = async function(elem, customLetters) {
  await waitFor.presenceOf(await elem.$(
    '.protractor-test-custom-letters-div'),
  'The custom letters div took too long to load.');
  for (let letter of customLetters) {
    if (letter.match(/[a-z]/)) {
      await action.click('Math OSK Tab', await elem.$('button=abc'));
    } else {
      await action.click('Math OSK Tab', await elem.$('button=αβγ'));
    }
    await action.click('Math OSK Letter', await elem.$(`button=${letter}`));
  }
};
...
var submitAnswer = async function(elem, answer) {
  ...
  var submitBtn = await $('.protractor-test-submit-answer-button');
