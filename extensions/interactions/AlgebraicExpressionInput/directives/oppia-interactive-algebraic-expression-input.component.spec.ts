...
/**
 * @fileoverview Unit tests for the AlgebraicExpressionInput component.
 */

fdescribe('AlgebraicExpressionInput', function() {
    var AlgebraicExpressionInputCtrl = null;
    class MockGuppy {
    ...
    beforeEach(angular.mock.inject(function($componentController) {
      AlgebraicExpressionInputCtrl = $componentController(
    ...
    it('should assign a random id to the guppy divs', function() {
      ...
      AlgebraicExpressionInputCtrl.$onInit();
    ...
    it('should initialize ctrl.value with an empty string', function() {
      AlgebraicExpressionInputCtrl.value = null;
      AlgebraicExpressionInputCtrl.$onInit();
      expect(AlgebraicExpressionInputCtrl.value).not.toBeNull();
    });
  
    it('should correctly validate current answer', function() {
      
      // This should be validated as true if the editor hasn't been touched.
      AlgebraicExpressionInputCtrl.value = '';
      expect(AlgebraicExpressionInputCtrl.isCurrentAnswerValid()).toBeTrue();
      expect(AlgebraicExpressionInputCtrl.warningText).toBe('');
  
  
      AlgebraicExpressionInputCtrl.hasBeenTouched = true;
  
      AlgebraicExpressionInputCtrl.value = '';
      expect(AlgebraicExpressionInputCtrl.isCurrentAnswerValid()).toBeFalse();
      expect(AlgebraicExpressionInputCtrl.warningText).toBe(
        'Please enter a non-empty answer.');
  
      AlgebraicExpressionInputCtrl.value = 'a/';
      expect(AlgebraicExpressionInputCtrl.isCurrentAnswerValid()).toBeFalse();
      expect(AlgebraicExpressionInputCtrl.warningText).toBe(
        '/ is not a valid postfix operator.');
  
      AlgebraicExpressionInputCtrl.value = '12+sqrt(4)';
      expect(AlgebraicExpressionInputCtrl.isCurrentAnswerValid()).toBeFalse();
      expect(AlgebraicExpressionInputCtrl.warningText).toBe(
        'It looks like you have entered only numbers. Make sure to include' +
        ' the necessary variables mentioned in the question.');
  
      AlgebraicExpressionInputCtrl.value = 'x-y=0';
      expect(AlgebraicExpressionInputCtrl.isCurrentAnswerValid()).toBeFalse();
      expect(AlgebraicExpressionInputCtrl.warningText).toBe(
