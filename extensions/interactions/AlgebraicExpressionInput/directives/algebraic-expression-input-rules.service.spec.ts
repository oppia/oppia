...
describe('Algebraic expression input rules service', () => {
  ...
  it('should have a correct ContainsSomeOf rule', () => {
    ...
    expect(algebraicRulesService.ContainsSomeOf('((x)^(2)-x)/(z)-4*y',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.ContainsSomeOf('((x)^(2)-x)/(z)',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.ContainsSomeOf('-4*y',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.ContainsSomeOf('((x)^(2)-x)*(z^(-1))',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.ContainsSomeOf('-8y/2',
      {x: inputString})).toBeTrue();

    expect(algebraicRulesService.ContainsSomeOf('(x-(x)^(2))/(z)+4*y',
      {x: inputString})).toBeFalse();
    expect(algebraicRulesService.ContainsSomeOf('x(x-1)/z',
      {x: inputString})).toBeFalse();
    expect(algebraicRulesService.ContainsSomeOf('(x^2)/z - x/z',
      {x: inputString})).toBeFalse();
    ...
    expect(algebraicRulesService.ContainsSomeOf(
      '(a)^(2)+(b)^(2)+(c)^(2)+2*a*b+2*b*c+2*a*c',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.ContainsSomeOf('a^2 + b^2 + 2ab',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.ContainsSomeOf('(a + b)^2 + c^2 + 2ac + 2bc',
      {x: inputString})).toBeTrue();

    expect(algebraicRulesService.ContainsSomeOf('(a + b + c)^2',
      {x: inputString})).toBeFalse();
    ...
    expect(algebraicRulesService.ContainsSomeOf('x^(2t+3) + x^4',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.ContainsSomeOf('x^(2t+3)',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.ContainsSomeOf('x^4',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.ContainsSomeOf('(x^2)^2',
      {x: inputString})).toBeTrue();

    expect(algebraicRulesService.ContainsSomeOf('(x^3)(x^(2t) + x)',
      {x: inputString})).toBeFalse();
    expect(algebraicRulesService.ContainsSomeOf('(x^(2t+4) + x^5)/x',
      {x: inputString})).toBeFalse();


    inputString = '9x^2 - 6x + 1';

    expect(algebraicRulesService.ContainsSomeOf('9x^2 - 6x + 1',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.ContainsSomeOf('9x^2 - 6x',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.ContainsSomeOf('9x^2 + 1',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.ContainsSomeOf('- 6x + 1',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.ContainsSomeOf('9x^2 - 6x - 1',
      {x: inputString})).toBeTrue();

    expect(algebraicRulesService.ContainsSomeOf('sqrt((3x-1)^(2))',
      {x: inputString})).toBeFalse();
    expect(algebraicRulesService.ContainsSomeOf('3*(3x^2 - 2x) - 1',
      {x: inputString})).toBeFalse();
    expect(algebraicRulesService.ContainsSomeOf('((3*x-1))^(4)',
      {x: inputString})).toBeFalse();
  });
  ...
  it('should have a correct OmitsSomeOf rule', () => {
    ...
    expect(algebraicRulesService.OmitsSomeOf('((x)^(2)-x)/(z)',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.OmitsSomeOf('-4*y',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.OmitsSomeOf('4*y',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.OmitsSomeOf('(x^2)/z - x/z - 4y',
      {x: inputString})).toBeTrue();

    expect(algebraicRulesService.OmitsSomeOf('((x)^(2)-x)/(z)-4*y',
      {x: inputString})).toBeFalse();
    expect(algebraicRulesService.OmitsSomeOf('((x)^(2)-x)*(z^(-1))-4*y',
      {x: inputString})).toBeFalse();


    inputString = '(a)^(2)+(b)^(2)+(c)^(2)+2*a*b+2*b*c+2*a*c';

    expect(algebraicRulesService.OmitsSomeOf('a^2 + c^2 + 2ab + 2bc + 2ac',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.OmitsSomeOf('a^2 + b^2 + c^2',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.OmitsSomeOf('(a + b)^2 + c^2 + 2ac + 2bc',
      {x: inputString})).toBeTrue();
    ...
    expect(algebraicRulesService.OmitsSomeOf('(x)^(2*t+3)',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.OmitsSomeOf('(x)^(4)',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.OmitsSomeOf('(x^2)^2',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.OmitsSomeOf('(x^(2t))*x^3',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.OmitsSomeOf('(x^(2t))*x*x*x',
      {x: inputString})).toBeTrue();

    expect(algebraicRulesService.OmitsSomeOf('x^(2t+3) + x^4',
      {x: inputString})).toBeFalse();
    expect(algebraicRulesService.OmitsSomeOf('x^(2t+4-1) + x^(8-4)',
      {x: inputString})).toBeFalse();


    inputString = '9x^2 - 6x + 1';

    expect(algebraicRulesService.OmitsSomeOf('9x^2 - 6x',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.OmitsSomeOf('9x^2 + 1',
      {x: inputString})).toBeTrue();
    expect(algebraicRulesService.OmitsSomeOf('- 6x + 1',
      {x: inputString})).toBeTrue();
