...
describe('Drag and Drop Sort Input rules service', function() {
  ...
  it('should have a correct \'is equal to ordering\' rule', function() {
    var RULE_INPUT = {x: [['a', 'b'], ['c'], ['de', 'f']]};
    expect(ddsrs.IsEqualToOrdering(
      [['a', 'b'], ['c'], ['de', 'f']], RULE_INPUT))
      .toBe(true);
    expect(ddsrs.IsEqualToOrdering(
      [['b', 'a'], ['c'], ['de', 'f']], RULE_INPUT))
      .toBe(true);
    expect(ddsrs.IsEqualToOrdering(
      [['abbb', 'b'], ['c'], ['d', 'f']], RULE_INPUT))
      .toBe(false);
    expect(ddsrs.IsEqualToOrdering(
      [['a', 'b'], ['c', 'f'], ['de']], RULE_INPUT))
      .toBe(false);
    expect(ddsrs.IsEqualToOrdering(
      [['a', 'b', 'g'], ['c'], ['de', 'f']], RULE_INPUT))
      .toBe(false);
    expect(ddsrs.IsEqualToOrdering(
      [['c'], ['a', 'b'], ['de', 'f']], RULE_INPUT))
      .toBe(false);
    expect(ddsrs.IsEqualToOrdering(
      [['a', 'b'], ['c']], RULE_INPUT))
      .toBe(false);
