...
(function () {
  ...
  function isRemoveEmpty(node) {
    ...
    return (
      (node.name == 'a' && node.attributes.href) ||
   ...
   */
  CKEDITOR.htmlParser.fragment.fromHtml = function (
    fragmentHtml,
    parent,
    fixingBlock
  ) {
    ...
    var root =
      parent instanceof CKEDITOR.htmlParser.element
        ? parent
        : typeof parent == 'string'
          ? new CKEDITOR.htmlParser.element(parent)
    ...
    function checkAutoParagraphing(parent, node) {
      ...
      if (
        ...
        if (
          node.attributes &&
          (realName = node.attributes['data-cke-real-element-type'])
        )
          name = realName;
        else name = node.name;
        ...
        if (
          ...
          // Fixing malformed nested lists by moving it into a previous list item. (#3828)
            ...
            if (!(lastChild && lastChild.name == 'li'))
              addElement(
                (lastChild = new CKEDITOR.htmlParser.element('li')),
                currentNode
              );
    ...
    parser.onText = function (text) {
      ...
      if (
        (!currentNode._.hasInlineStarted || pendingBRs.length) &&
        !inPre &&
        !inTextarea
      ) {
    ...
    add: function (node, index) {
      ...
      if (!this._.hasInlineStarted)
        this._.hasInlineStarted =
          node.type == CKEDITOR.NODE_TEXT ||
          (node.type == CKEDITOR.NODE_ELEMENT && !node._.isBlockLike);
    },
