...
(function () {
  ...
  CKEDITOR.editable = CKEDITOR.tools.createClass({
    ...
    proto: {
      ...
      clearListeners: function () {
        ...
        try {
          while (listeners.length) listeners.pop().removeListener();
        } catch (e) {}
      },
      ...
      insertText: function (text) {
        ...
        html = html
          .replace(/(>|\s) /g, function (match, before) {
            return before + '&nbsp;';
          })
          .replace(/ (?=<)/g, '&nbsp;');
      ...
      insertElement: function (element, range) {
        if (!range) this.insertElementIntoSelection(element);
        else this.insertElementIntoRange(element, range);
      },
      ...
      insertElementIntoRange: function (element, range) {
        ...
        if (range.checkReadOnly()) return false;
        ...
        if (isBlock) {
          while (
            (current = range.getCommonAncestor(0, 1)) &&
            ...
            // simply replace it with the inserting block.(#3664)
              ...
              range.splitBlock(
                enterMode == CKEDITOR.ENTER_DIV ? 'div' : 'p',
                editor.editable()
              );
          }
        }
        ...
        if (this.insertElementIntoRange(element, range)) {
          ...
          if (isBlock) {
            ...
            if (
              ...
              if (next.getDtd()['#']) range.moveToElementEditStart(next);
              // Otherwise move cursor to the before end of the last element.
              else range.moveToElementEditEnd(element);
            }
      ...
      setData: function (data, isSnapshot) {
        if (!isSnapshot) data = this.editor.dataProcessor.toHtml(data);
      ...
      getData: function (isSnapshot) {
        ...
        if (!isSnapshot) data = this.editor.dataProcessor.toDataFormat(data);
      ...
      setup: function () {
        ...
        this.on(
          ...
          function (evt) {
            ...
            if (CKEDITOR.env.opera) {
              ...
              if (
                active.equals(
                  this.isInline() ? this : this.getWindow().getFrame()
                )
              ) {
                evt.cancel();
                return;
        ...
        if (styles) {
          ...
          if (!head.getCustomData('stylesheet')) {
            ...
            sheet = new CKEDITOR.dom.element(
        ...
        this.setCustomData(
          'cke_includeReadonly',
          !editor.config.disableReadonlyStyling
        );
          ...
          if (keyCode in backspaceOrDelete) {
            ...
            if (
              ...
              if (
                (block = path.block) &&
                (next =
                  block[rtl ? 'getPrevious' : 'getNext'](isNotWhitespace)) &&
                next.type == CKEDITOR.NODE_ELEMENT &&
                next.is('table') &&
                ...
                path.blockLimit &&
                path.blockLimit.is('td') &&
                (parent = path.blockLimit.getAscendant('table')) &&
                range.checkBoundaryOfElement(
                  parent,
                  rtl ? CKEDITOR.START : CKEDITOR.END
                ) &&
                (next =
                  parent[rtl ? 'getPrevious' : 'getNext'](isNotWhitespace))
              ) {
                editor.fire('saveSnapshot');
              ...
              // BACKSPACE/DEL pressed at the start/end of table cell.
                ...
                (parent = path.contains(['td', 'th', 'caption'])) &&
                range.checkBoundaryOfElement(
                  parent,
                  rtl ? CKEDITOR.START : CKEDITOR.END
                )
        ...
        if (editor.blockless && CKEDITOR.env.ie && CKEDITOR.env.needsBrFiller) {
          this.attachListener(this, 'keyup', function (evt) {
            if (
              evt.data.getKeystroke() in backspaceOrDelete &&
    ...
    _: {
      detach: function () {
        ...
        if ((classes = this.removeCustomData('classes'))) {
          while (classes.length) this.removeClass(classes.pop());
        }
  ...
  CKEDITOR.editor.prototype.editable = function (element) {
    ...
    if (arguments.length) {
      editable = this._.editable = element
        ? element instanceof CKEDITOR.editable
          ? element
          : new CKEDITOR.editable(this, element)
  ...
  function fixDom(evt) {
    ...
    // content of editable.
    if (
      shouldAutoParagraph(editor, path.block, blockLimit) &&
      range.collapsed &&
      !range.getCommonAncestor().isReadOnly()
    ) {
      ...
      if (
        ...
        var fixedBlock = range.fixBlock(
          true,
          editor.activeEnterMode == CKEDITOR.ENTER_DIV ? 'div' : 'p'
        );
    ...
    if (selectionUpdateNeeded) range.select();
  }
  ...
  function needsBrFiller(selection, path) {
    ...
    if (
      ...
      !(
        lastNode &&
        lastNode.type == CKEDITOR.NODE_ELEMENT &&
        lastNode.isBlockBoundary()
      ) &&
      !pathBlock.is('pre') &&
      !pathBlock.getBogus()
    )
  ...
  function isNotBubbling(fn, src) {
    return function (evt) {
      var other = CKEDITOR.dom.element.get(
        evt.data.$.toElement ||
          evt.data.$.fromElement ||
          evt.data.$.relatedTarget
      );
  ...
  function shouldAutoParagraph(editor, pathBlock, pathBlockLimit) {
    return (
      ...
      editor.editable().equals(pathBlockLimit) &&
      !pathBlock
    );
  }
    ...
    editor.on('insertElement', function (evt) {
      ...
      if (
        ...
        if (element.getAttribute('contentEditable') != 'false')
          element.data(
            'cke-editable',
            element.hasAttribute('contenteditable') ? 'true' : '1'
          );
  ...
  CKEDITOR.on('instanceCreated', function (evt) {
    ...
    editor.on('mode', function () {
      ...
      if (editable && editable.isInline()) {
        ...
        if (ariaLabel) editable.changeAttr('title', ariaLabel);

        // Put the voice label in different spaces, depending on element mode, so
        // the DOM element get auto detached on mode reload or editor destroy.
        var ct = this.ui.space(
  ...
  var insert = (function () {
    ...
    function prepareRangeToDataInsertion(that) {
      ...
      if (!range.collapsed) {
        ...
        if (
          node &&
          !(node.equals(ancestor) || node.contains(ancestor)) &&
      ...
      while (
        (previous = getRangePrevious(range)) &&
        checkIfElement(previous) &&
        previous.isBlockBoundary() &&
    ...
    function processDataForInsertion(that, data) {
      ...
      if (
        block && // Apply when there exists path block after deleting selection's content...
        !(block.getChildCount() == 1 && block.getBogus())
      ) {
        // ... and the only content of this block isn't a bogus.
    ...
    function insertDataIntoRange(that) {
      var range = that.range,
        ...
        separateEndContainer =
          !!endContainer.getCommonAncestor(startContainer) && // endC is not detached.
      ...
      for (; nodeIndex < nodesData.length; nodeIndex++) {
        ...
        // Ignore trailing <brs>
        if (
          nodeData.isLineBreak &&
          splitOnLineBreak(range, blockLimit, nodeData)
        ) {
        ...
        if (
          !nodeData.isBlock &&
          shouldAutoParagraph(that.editor, path.block, path.blockLimit) &&
        ...
        if (splittingContainer && nodeData.isElement) {
          ...
          while (
            insertionContainer &&
            !DTD[insertionContainer.getName()][nodeData.name]
          ) {
          ...
          // Unable to make the insertion happen in place, resort to the content filter.
            ...
            filteredNodes = filterElement(
              nodeData.node,
              blockLimit.getName(),
              !nodeIndex,
              nodeIndex == nodesData.length - 1
            );
          }
        }

        if (filteredNodes) {
          while ((node = filteredNodes.pop())) range.insertNode(node);
          filteredNodes = 0;
        }
    ...
    function cleanupAfterInsertion(that) {
      ...
      if (bogusNeededBlocks) {
        ...
        while ((node = bogusNeededBlocks.pop())) {
          if (CKEDITOR.env.needsBrFiller) node.appendBogus();
          else node.append(range.document.createText('\u00a0'));
        }
      }
    ...
    function autoParagraphTag(editor) {
      return editor.activeEnterMode != CKEDITOR.ENTER_BR &&
        editor.config.autoParagraph !== false
      ...
      for (; nodeIndex < nodesCount; ++nodeIndex) {
        ...
        if (checkIfElement(node)) {
          ...
          if (
            nodeName == 'br' &&
            node.data('cke-eol') &&
            (!nodeIndex || nodeIndex == nodesCount - 1)
          ) {
            sibling = nodeIndex
              ? nodesData[nodeIndex - 1].node
              : nodesList.getItem(nodeIndex + 1);

            // Line break has to have sibling which is not an <br>.
            lineBreak =
              sibling && (!checkIfElement(sibling) || !sibling.is('br'));
            // Line break has block element as a sibling.
            blockSibling =
              sibling &&
              checkIfElement(sibling) &&
              DTD.$block[sibling.getName()];
          }
    ...
    function getRangePrevious(range) {
      return (
        checkIfElement(range.startContainer) &&
        range.startContainer.getChild(range.startOffset - 1)
      );
    }
    ...
    function isInline(node) {
      return (
        node &&
        checkIfElement(node) &&
        (node.is(DTD.$removeEmpty) || (node.is('a') && !node.isBlockBoundary()))
      );
    }
    ...
    function mergeAncestorElementsOfSelectionEnds(
      ...
      if (
        (nextNode = walker.next()) && // Find next source node
    ...
    function removeBrsAdjacentToPastedBlocks(nodesData, range) {
      ...
      function remove(maybeBr, maybeBlockData) {
        if (
          maybeBlockData.isBlock &&
          maybeBlockData.isElement &&
          !maybeBlockData.node.is('br') &&
          checkIfElement(maybeBr) &&
          maybeBr.is('br')
        ) {
          maybeBr.remove();
          return 1;
    ...
    function splitOnLineBreak(range, blockLimit, nodeData) {
      ...
      if (
        pos == CKEDITOR.POSITION_IDENTICAL ||
        pos == CKEDITOR.POSITION_CONTAINS
      )
        return 0;
    ...
    function stripBlockTagIfSingleLine(dataWrapper) {
      ...
      if (
        dataWrapper.getChildCount() == 1 && // Only one node bein inserted.
  ...
  var fixTableAfterContentsDeletion = (function () {
    ...
    return function (range) {
      ...
      if (!deeperSibling) deeperSibling = container;
      ...
      if (deeperSibling.is('tr'))
        deeperSibling = fixTableStructure(
          deeperSibling,
          deeperSibling.getParent().is('thead') ? 'th' : 'td',
          appendToStart
        );
