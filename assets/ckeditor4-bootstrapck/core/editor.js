...
(function () {
  ...
  function Editor(instanceConfig, element, mode) {
    ...
    if (element !== undefined) {
      ...
      if (
        CKEDITOR.env.ie &&
        CKEDITOR.env.quirks &&
        mode == CKEDITOR.ELEMENT_MODE_INLINE
      )
  ...
  function updateCommand(editor, cmd) {
    cmd[
      cmd.startDisabled
        ? 'disable'
        : editor.readOnly && !cmd.readOnly
          ? 'disable'
          : cmd.modes[editor.mode]
            ? 'enable'
            : 'disable'
    ]();
  }
  ...
  function loadConfig(editor) {
    ...
    var loadedConfig =
      loadConfigLoaded[customConfig] || (loadConfigLoaded[customConfig] = {});
    ...
    if (loadedConfig.fn) {
      ...
      // "configLoaded" event.
      if (
        CKEDITOR.getUrl(editor.config.customConfig) == customConfig ||
        !loadConfig(editor)
      )
      ...
      CKEDITOR.scriptLoader.queue(customConfig, function () {
        ...
        if (CKEDITOR.editorConfig) loadedConfig.fn = CKEDITOR.editorConfig;
  ...
  function onConfigLoaded(editor) {
    ...
    editor.blockless =
      editor.elementMode == CKEDITOR.ELEMENT_MODE_INLINE
        ? !(
            editor.element.is('textarea') ||
            CKEDITOR.dtd[editor.element.getName()]['p']
          )
        : false;
    ...
    editor.tabIndex =
      config.tabIndex ||
      (editor.element && editor.element.getAttribute('tabindex')) ||
      0;

    editor.activeEnterMode = editor.enterMode = validateEnterMode(
      editor,
      config.enterMode
    );
    editor.activeShiftEnterMode = editor.shiftEnterMode = validateEnterMode(
      editor,
  ...
  function loadLang(editor) {
    CKEDITOR.lang.load(
      ...
      function (languageCode, lang) {
         ...
         */
        editor.title =
          typeof configTitle == 'string' || configTitle === false
            ? configTitle
            : [editor.lang.editor, editor.name].join(', ');
    ...
    CKEDITOR.plugins.load(plugins.split(','), function (plugins) {
      ...
      for (var pluginName in plugins) {
        ...
        if (pluginLangs && !editor.lang[pluginName]) {
          ...
          if (CKEDITOR.tools.indexOf(pluginLangs, editor.langCode) >= 0)
            ...
            if (
              langPart != editor.langCode &&
              CKEDITOR.tools.indexOf(pluginLangs, langPart) >= 0
            )
              lang = langPart;
          ...
          if (!plugin.langEntries || !plugin.langEntries[lang]) {
            ...
            // get downloaded.
            languageFiles.push(
              CKEDITOR.getUrl(plugin.path + 'lang/' + lang + '.js')
            );
          } else {
      ...
      CKEDITOR.scriptLoader.load(languageFiles, function () {
        ...
        for (i = 0; i < editor.config.blockedKeystrokes.length; i++)
          editor.keystrokeHandler.blockedKeystrokes[
            editor.config.blockedKeystrokes[i]
          ] = 1;
  ...
  CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {
      ...
      if (element.is('textarea')) {
        if (form) {
          ...
          if (isFunction(form.$.submit)) {
            ...
            // "submit" event.
            form.$.submit = CKEDITOR.tools.override(
              form.$.submit,
              function (originalSubmit) {
                return function () {
                  ...
                  if (originalSubmit.apply) originalSubmit.apply(this);
                  else originalSubmit();
                };
              }
            );
    ...
    elementPath: function (startNode) {
      ...
      return startNode
        ? new CKEDITOR.dom.elementPath(startNode, this.editable())
        : null;
    },
    ...
    execCommand: function (commandName, data) {
      ...
      if (command && command.state != CKEDITOR.TRISTATE_DISABLED) {
        if (this.fire('beforeCommandExec', eventData) !== true) {
          ...
          if (
            !command.async &&
            this.fire('afterCommandExec', eventData) !== true
          )
    ...
    setKeystroke: function () {
      var keystrokes = this.keystrokeHandler.keystrokes,
        newKeystrokes = CKEDITOR.tools.isArray(arguments[0])
          ? arguments[0]
    ...
    setActiveFilter: function (filter) {
      ...
      if (this.activeFilter !== filter) {
        ...
        if (filter === this.filter) this.setActiveEnterMode(null, null);
        else
