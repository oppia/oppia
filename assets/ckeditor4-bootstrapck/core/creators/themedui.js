...
(function () {
   ...
   */
  CKEDITOR.editor.prototype.resize = function (
    width,
    height,
    isContentHeight,
    resizeInner
    ...
    var delta = isContentHeight
      ? 0
      : (outer.$.offsetHeight || 0) - (contents.$.clientHeight || 0);
  ...
  function createInstance(element, config, data, mode) {
    ...
    editor.on('loaded', function () {
      ...
      if (
        ...
        editor.config.autoUpdateElement &&
        element.$.form
      )
  ...
  function loadTheme(editor) {
    ...
    if (!themedTpl) {
      themedTpl = CKEDITOR.addTemplate(
        'maincontainer',
        '<{outerEl}' +
          ' id="cke_{name}"' +
          ' class="{id} cke cke_reset cke_chrome cke_editor_{name} cke_{langDir} ' +
    ...
    if (height)
      editor.ui
        .space('contents')
        .setStyle('height', CKEDITOR.tools.cssLength(height));
