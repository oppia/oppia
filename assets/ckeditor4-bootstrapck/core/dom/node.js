...
CKEDITOR.dom.node = function (domNode) {
  if (domNode) {
    var type =
      domNode.nodeType == CKEDITOR.NODE_DOCUMENT
        ? 'document'
        : domNode.nodeType == CKEDITOR.NODE_ELEMENT
          ? 'element'
          : domNode.nodeType == CKEDITOR.NODE_TEXT
            ? 'text'
            : domNode.nodeType == CKEDITOR.NODE_COMMENT
              ? 'comment'
              : domNode.nodeType == CKEDITOR.NODE_DOCUMENT_FRAGMENT
                ? 'documentFragment'
...
CKEDITOR.tools.extend(CKEDITOR.dom.node.prototype, {
  ...
  getIndex: function (normalized) {
    ...
    do {
      ...
      if (
        normalized &&
        current != this.$ &&
        current.nodeType == CKEDITOR.NODE_TEXT &&
        (isNormalizing || !current.nodeValue)
      )
        continue;
  ...
  getPrevious: function (evaluator) {
    ...
    do {
      ...
      retval =
        previous && previous.nodeType != 10 && new CKEDITOR.dom.node(previous);
  ...
  getParent: function (allowFragmentParent) {
    ...
    return parent &&
      (parent.nodeType == CKEDITOR.NODE_ELEMENT ||
        (allowFragmentParent &&
          parent.nodeType == CKEDITOR.NODE_DOCUMENT_FRAGMENT))
      ? new CKEDITOR.dom.node(parent)
      : null;
  },
  ...
  getAscendant: function (reference, includeSelf) {
    ...
    while ($) {
      if (
        ...
        typeof reference == 'string' ? name == reference : name in reference)
      )
  ...
  isReadOnly: function () {
    ...
    if (element && typeof element.$.isContentEditable != 'undefined')
      ...
      while (element) {
        if (element.data('cke-editable')) break;
