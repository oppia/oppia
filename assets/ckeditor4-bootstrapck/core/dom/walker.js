...
(function () {
  ...
  function iterate(rtl, breakOnFalse) {
    ...
    if (!rtl && !this._.guardLTR) {
      ...
      var limitLTR =
        endCt.type == CKEDITOR.NODE_ELEMENT ? endCt : endCt.getParent();

      // The node that stops the walker from going to next.
      var blockerLTR =
        endCt.type == CKEDITOR.NODE_ELEMENT
    ...
    if (rtl && !this._.guardRTL) {
      // The node that stops walker from moving up.
      var limitRTL =
        startCt.type == CKEDITOR.NODE_ELEMENT ? startCt : startCt.getParent();

      // The node that stops the walker from going to next.
      var blockerRTL =
        startCt.type == CKEDITOR.NODE_ELEMENT
          ? startOffset
            ? startCt.getChild(startOffset - 1)
            : null
          : startCt.getPrevious();

      this._.guardRTL = function (node, movingOut) {
        return (
          (!movingOut || !limitRTL.equals(node)) &&
    ...
    if (this.current) node = this.current[getSourceNodeFn](false, type, guard);
      ...
      if (rtl) {
        ...
        if (node.type == CKEDITOR.NODE_ELEMENT) {
          if (endOffset > 0) node = node.getChild(endOffset - 1);
          else
            node =
              guard(node, true) === false
                ? null
                : node.getPreviousSourceNode(true, type, guard);
        }
      } else {
        node = startCt;
        ...
        if (node.type == CKEDITOR.NODE_ELEMENT) {
          if (!(node = node.getChild(startOffset)))
            node =
              guard(startCt, true) === false
                ? null
                : startCt.getNextSourceNode(true, type, guard);
        }
      }
  ...
  CKEDITOR.dom.element.prototype.isBlockBoundary = function (customNodeNames) {
    ...
    var inPageFlow =
      this.getComputedStyle('float') == 'none' &&
      !(this.getComputedStyle('position') in outOfFlowPositions);
    ...
    return !!(
      this.is(CKEDITOR.dtd.$block) ||
      (customNodeNames && this.is(customNodeNames))
    );
  };
  ...
  CKEDITOR.dom.walker.bogus = function (isReject) {
    ...
    return function (node) {
      var isBogus = CKEDITOR.env.needsBrFiller
        ? node.is && node.is('br')
      ...
      if (isBogus) {
        ...
        isBogus =
          parent.isBlockBoundary() &&
          (!next ||
            (next.type == CKEDITOR.NODE_ELEMENT && next.isBlockBoundary()));
      }
  ...
  var tailNbspRegex = /^[\t\r\n ]*(?:&nbsp;|\xa0)$/,
    ...
    toSkip = function (node) {
      return (
        ...
        isWhitespaces(node) ||
        (node.type == CKEDITOR.NODE_ELEMENT &&
          node.is(CKEDITOR.dtd.$inline) &&
          !node.is(CKEDITOR.dtd.$empty))
      );
    };
  ...
  function isEmpty(node) {
    ...
    for (; i < l; ++i) {
      if (!isIgnored(node.getChild(i))) return false;
    }
    return true;
  }
