...
(function () {
  ...
  iterator.prototype = {
    ...
    getNextParagraph: function (blockTag) {
      ...
      if (this._.nestedEditable) {
        ...
        } else this._.nestedEditable = null;
      }
      ...
      if (!block) {
        ...
        if (
          ...
          !startBlockLimit.equals(range.root)
        )
          block = startBlockLimit;
        else if (
          !block ||
          (this.enforceRealBlocks && block.getName() == 'li')
        ) {
          ...
          this._.nextNode = block.equals(lastNode)
            ? null
            : getNextSourceNode(range.getBoundaryNodes().endNode, 1, lastNode);
        }
      }
      ...
      if (removePreviousBr) {
        ...
        if (previousSibling && previousSibling.type == CKEDITOR.NODE_ELEMENT) {
          if (previousSibling.getName() == 'br') previousSibling.remove();
          else if (
            previousSibling.getLast() &&
            previousSibling.getLast().$.nodeName.toLowerCase() == 'br'
          )
      ...
      if (!this._.nextNode)
        this._.nextNode =
          isLast || block.equals(lastNode) || !lastNode
            ? null
  ...
  function startIterator() {
    ...
    touchPre =
      range.endContainer.hasAscendant('pre', true) ||
      range.startContainer.hasAscendant('pre', true);

    range.enlarge(
      (this.forceBrBreak && !touchPre) || !this.enlargeBr
    ...
    if (!range.collapsed) {
      ...
      if (
        ...
        if (testRange.checkEndOfBlock()) {
          var path = new CKEDITOR.dom.elementPath(
              testRange.endContainer,
              testRange.root
            ),
  ...
  function startNestedEditableIterator(
    parentIterator,
    blockTag,
    editablesContainer,
    remainingEditables
    ...
    if (filter && !filter.check(blockTag))
      return startNestedEditableIterator(
        parentIterator,
        blockTag,
        editablesContainer,
        editable.remaining
