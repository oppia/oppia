...
CKEDITOR.dom.element = function (element, ownerDocument) {
  if (typeof element == 'string')
    element = (ownerDocument ? ownerDocument.$ : document).createElement(
      element
    );
 ...
 * @todo
 */
CKEDITOR.dom.element.clearMarkers = function (
  database,
  element,
  removeFromDatabase
) {
...
(function () {
  CKEDITOR.tools.extend(CKEDITOR.dom.element.prototype, {
    ...
    appendHtml: function (html) {
      if (!this.$.childNodes.length) this.setHtml(html);
      else {
    ...
    appendText: function (text) {
      if (this.$.text != undefined) this.$.text += text;
    ...
    appendBogus: function (force) {
      if (!force && !(CKEDITOR.env.needsBrFiller || CKEDITOR.env.opera)) return;
      ...
      while (
        lastChild &&
        lastChild.type == CKEDITOR.NODE_TEXT &&
    ...
    focus: (function () {
      ...
      return function (defer) {
        if (defer) CKEDITOR.tools.setTimeout(exec, 100, this);
        else exec.call(this);
      };
    })(),
        ...
        ? // old IEs throws error on HTML manipulation (through the "innerHTML" property)
          ...
          function (html) {
            try {
              ...
              if (this.getParent()) return ($.innerHTML = html);
              else {
          ...
          switch (name) {
            ...
            case 'checked': {
              var attr = this.$.attributes.getNamedItem(name),
                attrValue = attr.specified
                  ? attr.nodeValue // For value given by parser.
    ...
    getComputedStyle: CKEDITOR.env.ie
      ? function (propertyName) {
          return this.$.currentStyle[
            CKEDITOR.tools.cssStyleToDomStyle(propertyName)
          ];
        }
      : function (propertyName) {
    ...
    isIdentical: function (otherElement) {
      ...
      // Remove distractions.
      thisEl.removeAttributes([
        '_moz_dirty',
        'data-cke-expando',
        'data-cke-saved-href',
        'data-cke-saved-name',
      ]);
      otherEl.removeAttributes([
        '_moz_dirty',
        'data-cke-expando',
        'data-cke-saved-href',
        'data-cke-saved-name',
      ...
      if (thisEl.$.isEqualNode) {
        ...
        thisEl.$.style.cssText = CKEDITOR.tools.normalizeCssText(
          thisEl.$.style.cssText
        );
        otherEl.$.style.cssText = CKEDITOR.tools.normalizeCssText(
          otherEl.$.style.cssText
        );
      ...
      for (var i = 0, count = children.count(); i < count; i++) {
        ...
        if (
          ...
          (child.type == CKEDITOR.NODE_TEXT &&
            CKEDITOR.tools.trim(child.getText()))
        )
          return false;
      }
      return true;
    ...
    removeStyle: function (name) {
      ...
      if (
        !$.removeProperty &&
        (name == 'border' || name == 'margin' || name == 'padding')
      ) {
    ...
    setOpacity: function (opacity) {
      ...
      } else this.setStyle('opacity', opacity);
    },
    ...
    getDocumentPosition: function (refDocument) {
      ...
      if (document.documentElement['getBoundingClientRect']) {
        ...
        if (needAdjustScrollAndBorders) {
          x =
            box.left + ((!quirks && $docElem.scrollLeft) || body.$.scrollLeft);
          x -= clientLeft;
        ...
        while (
          current &&
          !(current.getName() == 'body' || current.getName() == 'html')
        ) {
      ...
      if (refDocument) {
        ...
        if (!currentWindow.equals(refWindow) && currentWindow.$.frameElement) {
          var iframePosition = new CKEDITOR.dom.element(
    ...
    scrollIntoParent: function (parent, alignToTop, hscroll) {
      ...
      function screenPos(element, refWin) {
        ...
        if (!win.equals(refWin)) {
          var outerPos = screenPos(
            CKEDITOR.dom.element.get(win.$.frameElement),
            refWin
          );
      ...
      function margin(element, side) {
        return (
          parseInt(element.getComputedStyle('margin-' + side) || 0, 10) || 0
        );
      }
    ...
    getEditor: function () {
      ...
      for (name in instances) {
        ...
        if (
          instance.element.equals(this) &&
          instance.elementMode != CKEDITOR.ELEMENT_MODE_APPENDTO
        )
          return instance;
      }
  ...
  function getContextualizedSelector(element, selector) {
    return (
      ...
      element.$.id +
      ' ' +
      selector.split(/,\s*/).join(', #' + element.$.id + ' ')
    );
  }
  ...
  function marginAndPaddingSize(type) {
    ...
    for (var i = 0, len = sides[type].length; i < len; i++)
      adjustment +=
        parseInt(this.getComputedStyle(sides[type][i]) || 0, 10) || 0;
    return adjustment;
  }
  ...
  CKEDITOR.dom.element.prototype.getSize = function (type, isBorderBox) {
    var size =
      Math.max(
        this.$['offset' + CKEDITOR.tools.capitalize(type)],
