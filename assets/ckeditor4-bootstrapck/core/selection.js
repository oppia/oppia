...
(function () {
  ...
  function checkSelectionChangeTimeout() {
    ...
    checkSelectionChangeTimer = CKEDITOR.tools.setTimeout(
      checkSelectionChangeTimeoutExec,
      200,
      this
    );
  }
  ...
  function checkFillingChar(element) {
    ...
    if (fillingChar) {
      ...
      // creating it.
      if (fillingChar.getCustomData('ready')) removeFillingChar(element);
  ...
  function removeFillingChar(element, keepSelection) {
    ...
    if (fillingChar) {
      ...
      if (keepSelection !== false) {
        ...
        if (
          fillingChar.getLength() > 1 &&
          range &&
          range.intersectsNode(fillingChar.$)
        ) {
          bm = [sel.anchorOffset, sel.focusOffset];

          // Anticipate the offset change brought by the removed char.
          var startAffected =
              sel.anchorNode == fillingChar.$ && sel.anchorOffset > 0,
  ...
  function fixInitialSelection(root, nativeSel, doFocus) {
    ...
    if (!CKEDITOR.env.ie) {
      ...
      var listener2 = root.getDocument().on(
        'selectionchange',
        function (evt) {
          evt.cancel();
        },
  ...
  function getOnKeyDownListener(editor) {
    ...
    return function (evt) {
      ...
      var next =
        range[
          keystroke < 38 ? 'getPreviousEditableNode' : 'getNextEditableNode'
        ]();
  ...
  function getNonEditableFakeSelectionReceiver(ranges) {
    ...
    if (
      ...
      if (
        (shrinkedNode = clone.getEnclosedNode()) &&
        shrinkedNode.type == CKEDITOR.NODE_ELEMENT
      )
  ...
  CKEDITOR.on('instanceCreated', function (ev) {
    ...
    editor.on('contentDom', function () {
        ...
        function () {
          ...
          if (restoreSel && CKEDITOR.env.webkit)
            restoreSel =
              ...
              editor._.previousActive.equals(doc.getActive());
        ...
        if (doc.$.compatMode != 'BackCompat') {
          ...
          if (CKEDITOR.env.version > 7 && CKEDITOR.env.version < 11) {
            ...
            function onSelectEnd() {
              ...
              if (
                sel.type != 'None' &&
                range.parentElement().ownerDocument == doc.$
              )
                range.select();
            }
          }
      ...
      // of the editor (#9851).
        ...
        editable.attachListener(
          CKEDITOR.env.ie ? editable : doc.getDocumentElement(),
          'mouseup',
          checkSelectionChangeTimeout,
          editor
        );
    ...
    editor.on(
      ...
      function () {
        ...
        if (el && el.hasAttribute('data-cke-hidden-sel')) el.remove();
      },
      null,
      null,
      100
    ...
    if (CKEDITOR.env.webkit) {
      ...
      function afterData() {
        ...
        if (fillingChar) {
          ...
          if (resetSelection) {
            editor.document.$.defaultView
              .getSelection()
              .setPosition(fillingChar.$, fillingChar.getLength());
            resetSelection = 0;
          }
        }
      }
  ...
  CKEDITOR.editor.prototype.getSelection = function (forceRealSelection) {
    ...
    return editable && this.mode == 'wysiwyg'
      ? new CKEDITOR.dom.selection(editable)
      : null;
  };
  ...
  CKEDITOR.dom.selection = function (target) {
    ...
    this.document =
      target instanceof CKEDITOR.dom.document ? target : target.getDocument();
    ...
    var sel = isMSSelection
      ? this.document.$.selection
      : this.document.getWindow().$.getSelection();

    if (CKEDITOR.env.webkit) {
      if (
        (sel.type == 'None' && this.document.getActive().equals(root)) ||
        (sel.type == 'Caret' &&
          sel.anchorNode.nodeType == CKEDITOR.NODE_DOCUMENT)
      )
        fixInitialSelection(root, sel);
    } else if (CKEDITOR.env.gecko) {
      if (
        sel &&
        this.document.getActive().equals(root) &&
        sel.anchorNode &&
        sel.anchorNode.nodeType == CKEDITOR.NODE_DOCUMENT
      )
      ...
      if (!isMSSelection) {
        ...
        if (anchorNode) anchorNode = new CKEDITOR.dom.node(anchorNode);

        if (
          active &&
          active.equals(this.document.getDocumentElement()) &&
          anchorNode &&
          (root.equals(anchorNode) || root.contains(anchorNode))
        )
    ...
    if (nativeSel) {
      if (nativeSel.getRangeAt) {
        ...
        rangeParent =
          range && new CKEDITOR.dom.node(range.commonAncestorContainer);
      }
      // For old IEs.
      else {
        ...
        rangeParent =
          ...
          CKEDITOR.dom.element.get(
            (range.item && range.item(0)) || range.parentElement()
          );
      }
    }
    ...
    if (
      !(
        rangeParent &&
        (rangeParent.type == CKEDITOR.NODE_ELEMENT ||
          rangeParent.type == CKEDITOR.NODE_TEXT) &&
  ...
  CKEDITOR.dom.selection.prototype = {
    ...
    getNative: function () {
      ...
      return (this._.cache.nativeSel = isMSSelection
        ? this.document.$.selection
        : this.document.getWindow().$.getSelection());
    },
      ...
      var func = isMSSelection
        ? (function () {
            ...
            var getBoundaryInformation = function (range, start) {
              ...
              if (
                index == -1 ||
                (index == siblings.length - 1 && position < 0)
              ) {
              ...
              // Test range was one offset beyond OR behind the anchored text node.
                ...
                return {
                  ...
                  offset:
                    position > 0
                      ? -distance
                      : container.nodeValue.length + distance,
                };
              }
            };
            ...
            return function () {
              ...
              if (type == CKEDITOR.SELECTION_TEXT) {
                ...
                range.setStart(
                  new CKEDITOR.dom.node(boundaryInfo.container),
                  boundaryInfo.offset
                );
                ...
                if (
                  range.endContainer.getPosition(range.startContainer) &
        ...
        : function () {
            ...
            for (var i = 0; i < sel.rangeCount; i++) {
              ...
              range.setStart(
                new CKEDITOR.dom.node(nativeRange.startContainer),
                nativeRange.startOffset
              );
              range.setEnd(
                new CKEDITOR.dom.node(nativeRange.endContainer),
          ...
          for (var i = 0; i < ranges.length; i++) {
            ...
            walker.evaluator = function (node) {
              if (node.type == CKEDITOR.NODE_ELEMENT && node.isReadOnly()) {
                ...
                if (
                  !(
                    node.getPosition(walkerRange.endContainer) &
    ...
    unlock: function (restore) {
      ...
      if (restore) {
        ...
        var common =
          selectedElement || (ranges[0] && ranges[0].getCommonAncestor());
        if (!(common && common.getAscendant('body', 1))) return;

        if (faked) this.fake(selectedElement);
        else if (selectedElement) this.selectElement(selectedElement);
    ...
    reset: function () {
      ...
      if (editor && editor._.fakeSelection) {
        ...
        // %REMOVE_LINE%
          ...
          window.console &&
            console.log('Wrong selection instance resets fake selection.'); // %REMOVE_LINE%
        // else // %REMOVE_LINE%
    ...
    selectRanges: function (ranges) {
      var editor = this.root.editor,
        hadHiddenSelectionContainer =
          editor && editor._.hiddenSelectionContainer;
      ...
      if (isMSSelection) {
        ...
        if (
          selected &&
          selected.type == CKEDITOR.NODE_ELEMENT &&
          selected.getName() in styleObjectElements &&
        ...
        if (
          (range.startContainer.type == CKEDITOR.NODE_ELEMENT &&
            range.startContainer.getName() in nonCells) ||
          (range.endContainer.type == CKEDITOR.NODE_ELEMENT &&
            range.endContainer.getName() in nonCells)
        )
        ...
        if (endNode) {
          ...
          isStartMarkerAlone =
            !(next && next.getText && next.getText().match(fillerTextRegex)) && // already a filler there?
            (inPre ||
              !startNode.hasPrevious() ||
        ...
        for (var i = 0; i < ranges.length; i++) {
          ...
          if (
            ...
            if (
              (!leftSib &&
                !rightSib &&
                startContainer.is(CKEDITOR.dtd.$removeEmpty)) ||
    ...
    selectBookmarks: function (bookmarks) {
      ...
      if (bookmarks.isFake) this.fake(ranges[0].getEnclosedNode());
      else this.selectRanges(ranges);
