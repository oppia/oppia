...
(function () {
  var functions = [],
    cssVendorPrefix = CKEDITOR.env.gecko
      ? '-moz-'
      : CKEDITOR.env.webkit
        ? '-webkit-'
        : CKEDITOR.env.opera
  ...
  CKEDITOR.tools = {
    ...
    capitalize: function (str, keepCase) {
      return (
        ...
        (keepCase ? str.slice(1) : str.slice(1).toLowerCase())
      );
    },
    ...
    cssStyleToDomStyle: (function () {
      ...
      var cssFloat =
        typeof test.cssFloat != 'undefined'
          ? 'cssFloat'
          : typeof test.styleFloat != 'undefined'
            ? 'styleFloat'
            : 'float';

      return function (cssName) {
        if (cssName == 'float') return cssFloat;
        else {
    ...
    setTimeout: function (func, milliseconds, scope, args, ownerWindow) {
      ...
      return ownerWindow.setTimeout(function () {
        if (args) func.apply(scope, [].concat(args));
      ...
      if (privates) {
        ...
        $ = function () {
          ...
          for (var privateName in privates) {
            ...
            _[privateName] =
              typeof priv == 'function'
                ? CKEDITOR.tools.bind(priv, this)
                : priv;
          }
    ...
    normalizeCssText: function (styleText, nativeNormalize) {
      var props = [],
        name,
        parsedProps = CKEDITOR.tools.parseCssText(
          styleText,
          true,
          nativeNormalize
        );
    ...
    parseCssText: function (styleText, normalize, nativeNormalize) {
      ...
      if (nativeNormalize) {
        ...
        styleText = CKEDITOR.tools.convertRgbToHex(
          temp.getAttribute('style') || ''
        );
      }
