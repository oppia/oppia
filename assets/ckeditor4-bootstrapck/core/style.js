...
(function () {
  ...
  CKEDITOR.style = function (styleDefinition, variablesValues) {
    ...
    if (attrs && attrs.style) {
      styleDefinition.styles = CKEDITOR.tools.extend(
        ...
        CKEDITOR.tools.parseCssText(attrs.style)
      );
      delete attrs.style;
    }
  ...
  CKEDITOR.style.prototype = {
      ...
      switch (this.type) {
        ...
        case CKEDITOR.STYLE_INLINE:
          ...
          for (var i = 0, element; i < elements.length; i++) {
            ...
            if (
              this.type == CKEDITOR.STYLE_INLINE &&
              (element == elementPath.block ||
                element == elementPath.blockLimit)
            )
              continue;
    ...
    checkElementRemovable: function (element, fullMatch) {
      ...
      if (this.checkElementMatch(element, fullMatch)) return true;
  ...
  CKEDITOR.style.getStyleText = function (styleDefinition) {
    ...
    var stylesText =
        (styleDefinition.attributes && styleDefinition.attributes['style']) ||
        '',
      specialStylesText = '';
  ...
  function checkIfNodeCanBeChildOfStyle(
    def,
    currentNode,
    lastNode,
    nodeName,
  ...
  function checkIfStyleCanBeChildOf(
    def,
    currentParent,
    elementName,
    isUnknownElement
    ...
    return (
      currentParent &&
      ((currentParent.getDtd() || CKEDITOR.dtd.span)[elementName] ||
        isUnknownElement) &&
  ...
  function checkIfStartsRange(nodeName, currentNode, lastNode) {
    return (
      !nodeName ||
      !CKEDITOR.dtd.$removeEmpty[nodeName] ||
      (currentNode.getPosition(lastNode) | posPrecedingIdenticalContained) ==
        posPrecedingIdenticalContained
    );
  }
  ...
  function checkIfTextOrReadonlyOrEmptyElement(currentNode, nodeIsReadonly) {
    ...
    return (
      ...
      nodeIsReadonly ||
      (nodeType == CKEDITOR.NODE_ELEMENT && !currentNode.getChildCount())
    );
  }
  ...
  function checkPositionAndRule(nodeA, nodeB, def, posBitFlags) {
    return (
      (nodeA.getPosition(nodeB) | posBitFlags) == posBitFlags &&
  ...
  function applyInlineStyle(range) {
    ...
    while (currentNode) {
      ...
      if (currentNode.equals(lastNode)) {
        ...
        var nodeName =
            currentNode.type == CKEDITOR.NODE_ELEMENT
              ? currentNode.getName()
              : null,
          nodeIsReadonly =
            nodeName && currentNode.getAttribute('contentEditable') == 'false',
        ...
        if (
          ...
          if (
            ...
            if (
              !styleRange &&
              checkIfStartsRange(nodeName, currentNode, lastNode)
            ) {
            ...
            if (
              checkIfTextOrReadonlyOrEmptyElement(currentNode, nodeIsReadonly)
            ) {
          ...
          if (parent.getName() == elementName) {
            for (attName in def.attributes) {
              if (
                ...
                !(value = parent.getAttribute(styleName))
              )
                continue;
            ...
            for (styleName in def.styles) {
              if (
                ...
                !(value = parent.getStyle(styleName))
              )
                continue;
  ...
  function removeInlineStyle(range) {
    ...
    if (range.collapsed) {
      var startPath = new CKEDITOR.dom.elementPath(
          startNode.getParent(),
          range.root
        ),
      ...
      for (
        ...
        if (this.checkElementRemovable(element)) {
          ...
          if (
            ...
            if (element.is(this.element)) removeFromElement.call(this, element);
            else
      ...
      if (boundaryElement) {
        ...
        clonedElement[
          boundaryElement.match == 'start' ? 'insertBefore' : 'insertAfter'
        ](boundaryElement);
      }
    } else {
      ...
      while (!currentNode.equals(endNode)) {
        ...
        if (
          ...
          if (currentNode.getName() == this.element)
            ...
            removeOverrides(
              currentNode,
              getOverrides(this)[currentNode.getName()]
            );
          ...
          if (
            nextNode.type == CKEDITOR.NODE_ELEMENT &&
  ...
  function applyObjectStyle(range) {
    ...
    var start = range.getEnclosedNode() || range.getCommonAncestor(false, true),
      element = new CKEDITOR.dom.elementPath(start, range.root).contains(
        this.element,
        1
      );
  ...
  function removeObjectStyle(range) {
    var parent = range.getCommonAncestor(true, true),
      element = new CKEDITOR.dom.elementPath(parent, range.root).contains(
        this.element,
        1
      );
  ...
  function replaceBlock(block, newBlock) {
    ...
    if (isToPre) newBlock = toPre(block, newBlock);
      ...
      newBlock = fromPres(
        removeBlock ? [block.getHtml()] : splitIntoPres(block),
        newBlock
      );
  ...
  function mergePre(preBlock) {
    ...
    if (
      !(
        (previousBlock = preBlock.getPrevious(nonWhitespaces)) &&
        previousBlock.type == CKEDITOR.NODE_ELEMENT &&
  ...
  function removeFromElement(element, keepDataAttrs) {
    var def = this._.definition,
      ...
      removeEmpty =
        CKEDITOR.tools.isEmpty(attributes) && CKEDITOR.tools.isEmpty(styles);

    // Remove definition attributes/style from the elemnt.
    for (var attName in attributes) {
      // The 'class' element value must match (#1318).
      if (
        (attName == 'class' || this._.definition.fullMatch) &&
        element.getAttribute(attName) !=
          normalizeProperty(attName, attributes[attName])
      )
        continue;
    ...
    for (var styleName in styles) {
      ...
      if (
        this._.definition.fullMatch &&
        element.getStyle(styleName) !=
    ...
    if (removeEmpty) {
      if (this._.definition.alwaysRemoveElement)
        ...
        if (
          !CKEDITOR.dtd.$block[element.getName()] ||
      ...
      if (CKEDITOR.dtd.$block[element.getName()]) {
        ...
        if (
          ...
          (previous.type == CKEDITOR.NODE_TEXT ||
            !previous.isBlockBoundary({br: 1}))
        )
          element.append('br', 1);
        if (
          next &&
          (next.type == CKEDITOR.NODE_TEXT || !next.isBlockBoundary({br: 1}))
        )
          element.append('br');
  ...
  function getElement(style, targetDocument, element) {
    ...
    if (
      targetDocument.getCustomData('doc_processing_style') &&
      el.hasAttribute('id')
    )
...
CKEDITOR.editor.prototype.getStylesSet = function (callback) {
  if (!this._.stylesDefinitions) {
    var editor = this,
      ...
      configStyleSet =
        editor.config.stylesCombo_stylesSet || editor.config.stylesSet;
