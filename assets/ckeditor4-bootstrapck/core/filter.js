...
(function () {
  ...
  CKEDITOR.filter = function (editorOrRules) {
    ...
    if (editorOrRules instanceof CKEDITOR.editor) {
      ...
      this.allow(
        enterModeTags[editor.enterMode] +
          ' ' +
          enterModeTags[editor.shiftEnterMode],
        'default',
        1
      );
    }
  ...
  CKEDITOR.filter.prototype = {
    ...
    applyTo: function (fragment, toHtml, transformOnly, enterMode) {
      ...
      fragment.forEach(
        function (el) {
          if (el.type == CKEDITOR.NODE_ELEMENT) {
            ...
            el.type == CKEDITOR.NODE_COMMENT &&
            el.value.match(/^\{cke_protected\}(?!\{C\})/)
          ) {
            if (
              !filterProtectedElement(
                el,
                protectedRegexs,
                filterFn,
                rules,
      ...
      var node,
        ...
        enterTag =
          enterModeTags[
            enterMode ||
              (this.editor ? this.editor.enterMode : CKEDITOR.ENTER_P)
          ];
    ...
    addContentForms: function (forms) {
      ...
      for (i = 0; i < forms.length && !preferredForm; ++i) {
        ...
        if (
          (typeof form == 'string' || form instanceof CKEDITOR.style) &&
          this.check(form)
        )
          preferredForm = form;
      }
      ...
      for (i = 0; i < forms.length; ++i)
        transfGroups.push(
          getContentFormTransformationGroup(forms[i], preferredForm)
        );
    ...
    check: function (test, applyTransformations, strictCheck) {
      ...
      if (typeof test == 'string') {
        cacheKey =
          test +
          '<' +
          (applyTransformations === false ? '0' : '1') +
          (strictCheck ? '1' : '0') +
          '>';
      ...
      if (
        ...
        (transformations = this._.transformations[element.name])
      ) {
        for (i = 0; i < transformations.length; ++i)
      ...
      getFilterFunction(this)(
        clone,
        this._.rules,
        applyTransformations === false ? false : this._.transformations,
        toBeRemoved,
        false,
        !strictCheck,
        !strictCheck
  ...
  function applyRule(rule, element, status, isSpecific, skipRequired) {
    ...
    if (!status.allAttributes)
      status.allAttributes = applyRuleToHash(
        rule.attributes,
        element.attributes,
        status.validAttributes
      );

    if (!status.allStyles)
      status.allStyles = applyRuleToHash(
        rule.styles,
        element.styles,
        status.validStyles
      );
    ...
    if (!status.allClasses)
      status.allClasses = applyRuleToArray(
        rule.classes,
        element.classes,
        status.validClasses
      );
  ...
  function applyRuleToHash(itemsRule, items, validItems) {
    ...
    for (var name in items) {
      if (!validItems[name]) validItems[name] = itemsRule(name, items[name]);
    }
  ...
  function convertStyleToRules(style) {
    ...
    rules[styleDef.element] = rule = {
      ...
      requiredStyles:
        styleDef.styles && CKEDITOR.tools.objectKeys(styleDef.styles),
    };
  ...
  function filterProtectedElement(
    comment,
    protectedRegexs,
    filterFn,
    rules,
    ...
    var source = decodeURIComponent(
        comment.value.replace(/^\{cke_protected\}/, '')
      ),
      protectedFrag,
      toBeRemoved = [],
      node,
    ...
    if (protectedRegexs) {
      for (i = 0; i < protectedRegexs.length; ++i) {
        if (
          (match = source.match(protectedRegexs[i])) &&
    ...
    if (
      protectedFrag.children.length == 1 &&
      (node = protectedFrag.children[0]).type == CKEDITOR.NODE_ELEMENT
    )
  ...
  function getFilterFunction(that) {
    ...
    if (that._.filterFunction) return that._.filterFunction;
    ...
    return (that._.filterFunction = function (
      element,
      optimizedRules,
      transformations,
      toBeRemoved,
  ...
  function hasAllRequired(rule, element) {
    ...
    if ((reqs = rule.requiredClasses)) {
      ...
      for (i = 0; i < reqs.length; ++i) {
        if (CKEDITOR.tools.indexOf(existing, reqs[i]) == -1) return false;
      }
    }

    return (
      hasAllRequiredInHash(element.styles, rule.requiredStyles) &&
  ...
  function hasAllRequiredInHash(existing, required) {
    ...
    for (var i = 0; i < required.length; ++i) {
      if (!(required[i] in existing)) return false;
    }
    ...
    for (i = 0, l = rules.length; i < l; ++i) {
      ...
      // add this rule for every defined element.
        ...
        for (element in elements) {
          if (!elementsRules[element]) elementsRules[element] = [rule];
  ...
  function populateProperties(element) {
    ...
    if (!element.styles)
      element.styles = CKEDITOR.tools.parseCssText(
        element.attributes.style || '',
        1
      );
    if (!element.classes)
      element.classes = element.attributes['class']
  ...
  function standardizeRule(rule) {
    ...
    for (i in validators) {
      ...
      rule[validatorsRequired[i]] =
        extractRequired(
          convertValidatorToHash(rule[validatorsRequired[i]], delim),
          rule[i]
        ) || null;
    }
  ...
  function updateElement(element, status) {
      ...
      for (name in attrs) {
        ...
        if (!validAttrs[name]) {
          ...
          if (internalAttr.test(name)) {
            ...
            if (
              name != (origName = name.replace(/^data-cke-saved-/, '')) &&
              !validAttrs[origName]
            ) {
              delete attrs[name];
              isModified = true;
    ...
    if (!status.allStyles) {
      for (name in styles) {
        if (validStyles[name]) stylesArr.push(name + ':' + styles[name]);
    ...
    if (!status.allClasses) {
      for (name in validClasses) {
        if (validClasses[name]) classesArr.push(name);
      }
  ...
  function validateElement(element) {
    ...
    switch (element.name) {
      case 'a':
        ...
        if (!(element.children.length || element.attributes.name)) return false;
        break;
      case 'img':
  ...
  function allowedIn(node, parentDtd) {
    if (node.type == CKEDITOR.NODE_ELEMENT) return parentDtd[node.name];
  ...
  function removeElement(element, enterTag, toBeChecked) {
    ...
    if (DTD.$empty[name] || !element.children.length) {
      ...
      if (name == 'hr' && enterTag == 'br') element.replaceWith(createBr());
      else {
      ...
      if (enterTag == 'br') stripBlockBr(element, toBeChecked);
  ...
  function stripBlock(element, enterTag, toBeChecked) {
    ...
    var parent = element.parent,
      shouldAutoP =
        parent.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || parent.name == 'body',
      i,
      j,
      child,
      p,
    ...
    for (i = children.length; i > 0; ) {
      ...
      // Child which doesn't need to be auto paragraphed.
        ...
        if (
          parent.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT &&
  ...
  function applyTransformationsGroup(filter, element, group) {
    ...
    for (i = 0; i < group.length; ++i) {
      ...
      if (
        (!rule.check || filter.check(rule.check, false)) &&
