...
(function () {
  var floatSpaceTpl = CKEDITOR.addTemplate(
      'floatcontainer',
      '<div' +
        ' id="cke_{name}"' +
  ...
  function attach(editor) {
    var config = editor.config,
      ...
      layout = (function () {
          ...
          switch (newMode) {
            case 'top':
              updatePos(
                'absolute',
                'top',
                editorPos.y - spaceHeight - dockedOffsetY
              );
              break;
            case 'pin':
        ...
        return function (evt) {
          ...
          if (!(editable = editor.editable())) return;
          ...
          // |   |                           |   |
          //
          if (spaceHeight + dockedOffsetY <= editorRect.top) changeMode('top');
          //     +- - - - - - - - -  Editor -+
          //     |                           |
          // +------------------------ Viewport -+ \
          ...
          //
            ...
            spaceHeight + dockedOffsetY >
            viewRect.height - editorRect.bottom
          )
            changeMode('pin');
          //     +- - - - - - - - -  Editor -+
          //     |                           |
          ...
          if (spaceRect.width > viewRect.width) {
            ...
            if (alignSide == 'left') {
              ...
              // |   |                           |   |
              //
              if (editorRect.left > 0) offset = editorRect.left;
          ...
          floatSpace.setStyle(
            alignSide,
            pixelate(
              (mode == 'pin' ? pinnedOffsetX : dockedOffsetX) + offset + scroll
            )
          );
        };
      })();
    ...
    if (topHtml) {
      var floatSpace = CKEDITOR.document.getBody().append(
          CKEDITOR.dom.element.createFromHtml(
            floatSpaceTpl.output({
              content: topHtml,
              id: editor.id,
