...
(function () {
  var template =
    ...
    (CKEDITOR.env.gecko && CKEDITOR.env.version >= 10900 && !CKEDITOR.env.hc
      ? ''
  ...
  CKEDITOR.ui.button.prototype = {
    ...
    render: function (editor, output) {
      ...
      var focusFn = CKEDITOR.tools.addFunction(function (ev) {
        ...
        if (instance.onfocus)
          retVal =
            instance.onfocus(instance, new CKEDITOR.dom.event(ev)) !== false;
      ...
      if (this.modes) {
        ...
        function updateState() {
          ...
          if (mode) {
            ...
            var state = this.modes[mode]
              ? modeStates[mode] != undefined
                ? modeStates[mode]
        ...
        if (command) {
          ...
          stateName +=
            command.state == CKEDITOR.TRISTATE_ON
              ? 'on'
      ...
      var params = {
        ...
        title: this.title,
        titleJs:
          env.gecko && env.version >= 10900 && !env.hc
            ? ''
            : (this.title || '').replace("'", ''),
