      ...
      exec: function (editor) {
        if (editor.editable().hasFocus) {
          var sel = editor.getSelection(),
            path = new CKEDITOR.dom.elementPath(
          ...
          if ((cell = path.contains({td: 1, th: 1}, 1))) {
              ...
              next = CKEDITOR.tools.tryThese(
                ...
                function () {
                  var row = cell.getParent(),
                    ...
                    nextRow =
                      table.$.rows[row.$.rowIndex + (backward ? -1 : 1)];
            ...
            if (!(next || backward)) {
              ...
              var newRow = new CKEDITOR.dom.element(
                table.insertRow(-1),
                editor.document
              );

              for (var i = 0, count = cells.length; i < count; i++) {
                var newCell = newRow.append(
                  new CKEDITOR.dom.element(cells[i], editor.document).clone(
                    false,
                    false
                  )
                );
              ...
              if (
                !(
                  resultRange.checkStartOfBlock() &&
                  resultRange.checkEndOfBlock()
                )
              )
  ...
  CKEDITOR.plugins.add('tab', {
    init: function (editor) {
      ...
      if (tabTools) {
        editor.on('key', function (ev) {
          if (
            (ev.data.keyCode == 9 && editor.execCommand('selectNextCell')) || // TAB
            (ev.data.keyCode == CKEDITOR.SHIFT + 9 &&
              editor.execCommand('selectPreviousCell'))
          )
            // SHIFT+TAB
            ev.cancel();
        });
      ...
      editor.addCommand(
        'blurBack',
        CKEDITOR.tools.extend(blurBackCommand, meta)
      );
    ...
    while (
      ...
      if (!passedCurrent) {
        if (!enteredCurrent && element.equals(this)) {
          ...
          if (ignoreChildren) {
            if (
              !(element = element.getNextSourceNode(
                true,
      ...
      if (
        elementTabIndex > curTabIndex &&
        (!elected || !electedTabIndex || elementTabIndex < electedTabIndex)
      ) {
        elected = element;
...
CKEDITOR.dom.element.prototype.focusPrevious = function (
  ...
  while (
    (element = element.getPreviousSourceNode(false, CKEDITOR.NODE_ELEMENT))
  ) {
    if (!passedCurrent) {
      if (!enteredCurrent && element.equals(this)) {
        enteredCurrent = true;
        ...
        if (ignoreChildren) {
          if (
            !(element = element.getPreviousSourceNode(
              true,
              CKEDITOR.NODE_ELEMENT
            ))
          )
    ...
    if (curTabIndex <= 0) {
      ...
      if (
        elementTabIndex < curTabIndex &&
        (!elected || elementTabIndex > electedTabIndex)
      ) {
        elected = element;
        electedTabIndex = elementTabIndex;
      }
