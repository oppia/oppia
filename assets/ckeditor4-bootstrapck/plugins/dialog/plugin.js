...
(function () {
  ...
  CKEDITOR.dialog = function (editor, dialogName) {
    ...
    if (
      (buttonsOrder == 'OS' && CKEDITOR.env.mac) || // The buttons in MacOS Apps are in reverse order (#4750)
    ...
    function changeFocus(offset) {
      ...
      while (offset && !focusList[currentIndex].isFocusable()) {
        currentIndex =
          (currentIndex + offset + focusList.length) % focusList.length;
  ...
  CKEDITOR.dialog.prototype = {
    ...
    show: function () {
      ...
      if (
        !(
          element.getParent() &&
          element.getParent().equals(CKEDITOR.document.getBody())
        )
      )
      ...
      if (CKEDITOR.dialog._.currentTop === null) {
        ...
        parentElement.$.style.zIndex -= Math.floor(
      ...
      for (var i in definition.contents) {
        if (!definition.contents[i]) continue;
        ...
        else tab[0].removeClass('cke_dialog_tab_disabled');
      }
    ...
    layout: function () {
      ...
      if (!CKEDITOR.env.ie6Compat) {
        if (
          dialogSize.height + (posY > 0 ? posY : 0) > viewSize.height ||
      ...
      this.move(
        this._.moved ? this._.position.x : posX,
    ...
    foreach: function (fn) {
      for (var i in this._.contents) {
        for (var j in this._.contents[i]) fn.call(this, this._.contents[i][j]);
      }
      return this;
    },
    ...
    hide: function () {
      ...
      if (!this._.parentDialog) {
        ...
        element.removeListener(
          CKEDITOR.env.opera ? 'keypress' : 'keyup',
          accessKeyUpHandler
        );
  ...
  function initDragAndDrop(dialog) {
    ...
    function mouseMoveHandler(evt) {
      ...
      if (abstractDialogCoords.x + margins[3] < magnetDistance)
        ...
        viewPaneSize.width - dialogSize.width - magnetDistance
      )
        realX =
          viewPaneSize.width -
          dialogSize.width +
      ...
      if (abstractDialogCoords.y + margins[0] < magnetDistance)
        ...
        viewPaneSize.height - dialogSize.height - magnetDistance
      )
  ...
  function initResizeHandles(dialog) {
    ...
    var mouseDownFn = CKEDITOR.tools.addFunction(function ($event) {
      ...
      wrapperWidth =
        startSize.width - dialog.parts.contents.getSize('width', 1);
  ...
  function hideCover(editor) {
    ...
    if (CKEDITOR.env.ie6Compat) {
      win.$.setTimeout(function () {
        var prevScrollHandler =
          window.onscroll && window.onscroll.prevScrollHandler;
      ...
      hbox: function (
        ...
        var innerHTML = function () {
          ...
          for (i = 0; i < childHtmlList.length; i++) {
            ...
            if (widths) {
              if (widths[i]) styles.push('width:' + cssLength(widths[i]));
            } else
              styles.push(
