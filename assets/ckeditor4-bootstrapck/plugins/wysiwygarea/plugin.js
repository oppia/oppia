    ...
    init: function (editor) {
      ...
      editor.addMode('wysiwyg', function (callback) {
        ...
        var labelId = CKEDITOR.tools.getNextId(),
          desc = CKEDITOR.dom.element.createFromHtml(
            '<span id="' +
              labelId +
        ...
        function onLoad(evt) {
          ...
          editor.editable(
            new framedWysiwyg(editor, iframe.$.contentWindow.document.body)
          );
  ...
  function onDomReady(win) {
    ...
    try {
      editor.document.$.execCommand(
        ...
        false,
        !editor.config.disableNativeTableHandles
      );
    } catch (e) {}
    ...
    if (
      CKEDITOR.env.gecko ||
      (CKEDITOR.env.ie && editor.document.$.compatMode == 'CSS1Compat')
    ) {
      this.attachListener(this, 'keydown', function (evt) {
        ...
        if (keyCode == 33 || keyCode == 34) {
          ...
          // when editor contents are not overflowed. (#7955)
            ...
            range[
              keyCode == 33 ? 'moveToElementEditStart' : 'moveToElementEditEnd'
            ](this);
            range.select();
            evt.data.preventDefault();
          }
  ...
  var framedWysiwyg = CKEDITOR.tools.createClass({
    ...
    proto: {
      setData: function (data, isSnapshot) {
        ...
        if (isSnapshot) {
          ...
          var headExtra = CKEDITOR.tools
            .buildStyleHtml(iframeCssFixes())
            .replace(/<style>/, '<style data-cke-temp="1">');

          if (!fullPage)
            headExtra += CKEDITOR.tools.buildStyleHtml(
              editor.config.contentsCss
            );

          var baseTag = config.baseHref
            ? '<base href="' + config.baseHref + '" data-cke-temp="1" />'
            : '';
          ...
          if (fullPage) {
            ...
            if (!/<body[\s|>]/.test(data)) data = '<body>' + data;
            ...
            data =
              ...
              '"' +
              ' lang="' +
              (config.contentsLanguage || editor.langCode) +
              '">' +
              '<head>' +
              '<title>' +
              this._.docTitle +
      ...
      getData: function (isSnapshot) {
        if (isSnapshot) return this.getHtml();
        else {
          var editor = this.editor,
      ...
      focus: function () {
        if (this._.isLoadingData) this._.isPendingFocus = true;
