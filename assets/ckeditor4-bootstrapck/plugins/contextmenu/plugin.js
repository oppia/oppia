...
CKEDITOR.plugins.add('contextmenu', {
        ...
        addTarget: function (element, nativeContextMenuOnCtrl) {
          element.on(
            ...
            function (event) {
              ...
              if (nativeContextMenuOnCtrl && isCtrlKeyDown) return;

              // Cancel the browser context menu.
              domEvent.preventDefault();

              var doc = domEvent.getTarget().getDocument(),
                offsetParent = domEvent
                  .getTarget()
                  .getDocument()
                  .getDocumentElement(),
                ...
                offsetX = fromFrame
                  ? domEvent.$.clientX
                  : domEvent.$.pageX || scroll.x + domEvent.$.clientX,
                offsetY = fromFrame
                  ? domEvent.$.clientY
                  : domEvent.$.pageY || scroll.y + domEvent.$.clientY;
  ...
  beforeInit: function (editor) {
     ...
     */
    var contextMenu = (editor.contextMenu = new CKEDITOR.plugins.contextMenu(
      editor
    ));

    editor.on('contentDom', function () {
      contextMenu.addTarget(
        editor.editable(),
        editor.config.browserContextMenuOnCtrl !== false
      );
    });
    ...
    editor.setKeystroke(
      CKEDITOR.CTRL + CKEDITOR.SHIFT + 121 /*F10*/,
      'contextMenu'
    );
  },
});
