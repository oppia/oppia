        ...
        exec: function (editor) {
          ...
          editor.getClipboardData(
            ...
            function (data) {
              ...
              data &&
                editor.fire('paste', {type: 'html', dataValue: data.dataValue});
      ...
      editor.on(
        ...
        function (evt) {
          ...
          if (
            mswordHtml &&
            (forceFromWord ||
              /(class=\"?Mso|style=\"[^\"]*\bmso\-|w:WordDocument)/.test(
                mswordHtml
              ))
          ) {
            ...
            var isLazyLoad = loadFilterRules(editor, path, function () {
              ...
              if (isLazyLoad) editor.fire('paste', data);
              else if (
  ...
  function loadFilterRules(editor, path, callback) {
    ...
    if (isLoaded) callback();
      ...
      var filterFilePath = CKEDITOR.getUrl(
        editor.config.pasteFromWordCleanupFile || path + 'filter/default.js'
      );
