...
(function () {
  ...
  elementPrototype.addStyle = function (name, value, isPrepend) {
    ...
    styleText = (
      isPrepend ? [addingStyleText, styleText] : [styleText, addingStyleText]
    ).join(';');
  ...
  function postProcessList(list) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      if (match) {
        if (match[1] == mergeStyle || !mergeStyle) mergeStyle = match[1];
        else {
          mergeStyle = null;
          break;
        }
    ...
    if (mergeStyle) {
      for (i = 0; i < count; i++) {
        ...
        attrs.style &&
          (attrs.style =
            stylesFilter([['list-style-type']])(attrs.style) || '');
      }
  ...
  var plugin = (CKEDITOR.plugins.pastefromword = {
    utils: {
      ...
      isContainingOnlySpaces: function (element) {
        var text;
        return (
          (text = element.onlyChild()) && /^(:?\s|&nbsp;)+$/.test(text.value)
        );
      },
              ...
              plugin.filters.stylesFilter([
                ...
                [
                  ...
                  function (margin) {
                    ...
                    margin = CKEDITOR.tools.convertToPx(
                    ...
                    attrs['cke:indent'] =
                      (listBaseIndent &&
                        Math.ceil(margin / listBaseIndent) + 1) ||
                      1;
                  },
                ],
    ...
    filters: {
            ...
            plugin.filters.stylesFilter([
              [
                ...
                function (val) {
                  ...
                  margin &&
                    (previousListItemMargin = CKEDITOR.tools.convertToPx(
                      margin[0]
                    ));
                },
              ],
        ...
        for (var i = 0; i < children.length; i++) {
          ...
          if ('cke:li' == child.name) {
            ...
            if (!bullet) {
              ...
              if (
                previousListType &&
                listMarkerPatterns[previousListType][
                  previousListStyleType
                ].test(bullet[1])
              ) {
        ...
        return styleDefinition
          ? function (element) {
              ...
              CKEDITOR.tools.extend(
                element.attributes,
                CKEDITOR.tools.clone(styleDef.attributes)
              );
      ...
      bogusAttrFilter: function (value, element) {
        if (element.name.indexOf('cke:') == -1) return false;
      },
    ...
    getRules: function (editor, filter) {
      var dtd = CKEDITOR.dtd,
        blockLike = CKEDITOR.tools.extend(
          {},
          dtd.$block,
          dtd.$listItem,
          dtd.$tableContent
        ...
        styleMigrateFilter = CKEDITOR.tools.bind(
        ...
        elements: {
          ...
          style: function (element) {
            if (CKEDITOR.env.gecko) {
              ...
              var styleDefSection = element
                  .onlyChild()
                  .value.match(/\/\* Style Definitions \*\/([\s\S]*?)\/\*/),
          ...
          p: function (element) {
            ...
            if (
              /MsoListParagraph/i.exec(element.attributes['class']) ||
          ...
          td: function (element) {
            ...
            if (element.getAncestor('thead')) element.name = 'th';
          },
          ...
          span: function (element) {
            ...
            if (isListBulletIndicator(element.parent)) return false;
              ...
              attrs.style =
                stylesFilter([
                  ...
                  [
                    ...
                    null,
                    !removeFontStyles
                      ? styleMigrateFilter(config['font_style'], 'family')
                      : null,
                  ],
                  [
                    /^font-size$/,
              ...
              ? // Provide a white-list of styles that we preserve, those should
                ...
                [
                  ...
                  [
                    ...
                    null,
                    function (value, element) {
                      if (element.name == 'img') return value;
                    },
                  ],
        ...
        comment: !CKEDITOR.env.ie
          ? function (value, node) {
              ...
              if (CKEDITOR.env.gecko && imageInfo) {
                var img = CKEDITOR.htmlParser.fragment.fromHtml(imageInfo[0])
                    .children[0],
  ...
  CKEDITOR.cleanWord = function (data, editor) {
    ...
    dataFilter.addRules(
      CKEDITOR.plugins.pastefromword.getRules(editor, dataFilter)
    );
