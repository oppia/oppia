...
CKEDITOR.dialog.add('select', function (editor) {
  ...
  function addOption(combo, optionText, optionValue, documentObject, index) {
    ...
    if (combo && oOption && oOption.getName() == 'option') {
      if (CKEDITOR.env.ie) {
        if (!isNaN(parseInt(index, 10))) combo.$.options.add(oOption.$, index);
  ...
  function removeSelectedOptions(combo) {
    ...
    for (var i = combo.getChildren().count() - 1; i >= 0; i--) {
      if (combo.getChild(i).$.selected) combo.getChild(i).remove();
    }
  ...
  function changeOptionPosition(combo, steps, documentObject) {
    ...
    if (iActualIndex < 0) return false;

    var iFinalIndex = iActualIndex + steps;
    iFinalIndex = iFinalIndex < 0 ? 0 : iFinalIndex;
    iFinalIndex =
      iFinalIndex >= combo.getChildCount()
        ? combo.getChildCount() - 1
        : iFinalIndex;

    if (iActualIndex == iFinalIndex) return false;
  ...
  return {
    ...
    onOk: function () {
      ...
      if (isInsertMode) element = editor.document.createElement('select');
        ...
        elements: [
          {
            ...
            setup: function (name, element) {
              if (name == 'clear') this.setValue(this['default'] || '');
              else if (name == 'select')
                this.setValue(
          ...
          },
          {
            ...
            children: [
              ...
              },
              {
                ...
                html:
                  '<span>' +
                  CKEDITOR.tools.htmlEncode(editor.lang.forms.select.lines) +
                  '</span>',
              },
            ],
          },
          {
            ...
            html:
              '<span>' +
              CKEDITOR.tools.htmlEncode(editor.lang.forms.select.opAvail) +
              '</span>',
          },
          {
            type: 'hbox',
                  ...
                  },
                  {
                    ...
                    onChange: function () {
                      var dialog = this.getDialog(),
                        ...
                        optName = dialog.getContentElement(
                          'info',
                          'txtOptName'
                        ),
                        optValue = dialog.getContentElement(
                          'info',
                          'txtOptValue'
                        ),
                    ...
                    setup: function (name, element) {
                      if (name == 'clear') removeAllOptions(this);
                      else if (name == 'option')
                        addOption(
                          this,
                          element.getText(),
                    ...
                    commit: function (element) {
                      ...
                      for (var i = 0; i < optionsNames.count(); i++) {
                        var oOption = addOption(
                          element,
                          optionsNames.getItem(i).getValue(),
                          optionsValues.getItem(i).getValue(),
                          dialog.getParentEditor().document
                        );
                        if (
                          optionsValues.getItem(i).getValue() == selectValue
                        ) {
