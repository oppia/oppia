...
CKEDITOR.dialog.add('link', function (editor) {
  ...
  var parseLink = function (editor, element) {
    ...
    if ((javascriptMatch = href.match(javascriptProtocolRegex))) {
      ...
      // Protected email link as function call.
        ...
        href.replace(
          ...
          function (match, funcName, funcArgs) {
    ...
    if (element) {
      ...
      var advAttr = function (inputName, attrName) {
        ...
        if (value !== null) retval.adv[inputName] = value || '';
      };
      advAttr('advId', 'id');
      ...
      retval.adv.advName =
        element.data('cke-saved-name') || element.getAttribute('name') || '';
    ...
    if (CKEDITOR.plugins.link.fakeAnchor) {
      ...
      for (i = 0, count = imgs.count(); i < count; i++) {
        if (
          (item = CKEDITOR.plugins.link.tryRestoreFakeAnchor(
            editor,
            imgs.getItem(i)
          ))
        )
  ...
  function getLinkClass(ele) {
    ...
    return className
      ? className.replace(/\s*(?:cke_anchor_empty|cke_anchor)(?:\s*$)?/g, '')
      : '';
  }
  ...
  return {
        ...
        elements: [
          {
            ...
            setup: function (data) {
              if (data.type) this.setValue(data.type);
            },
                ...
                children: [
                  {
                    ...
                    setup: function (data) {
                      if (data.url) this.setValue(data.url.protocol || '');
                    },
                    commit: function (data) {
            ...
            children: [
              {
                ...
                setup: function (data) {
                  if (data.anchors.length > 0) this.getElement().show();
              ...
              },
              {
                ...
                focus: true,
                setup: function (data) {
                  if (data.anchors.length < 1) this.getElement().show();
                  else this.getElement().hide();
                },
              },
            ],
            ...
            children: [
              {
                ...
                setup: function (data) {
                  ...
                  if (linkType && linkType.getValue() == 'email') this.select();
                },
                commit: function (data) {
            ...
            children: [
              {
                ...
                setup: function (data) {
                  if (data.target) this.setValue(data.target.type || 'notSet');
    ...
    onOk: function () {
      ...
      switch (data.type || 'url') {
        case 'url':
          var protocol =
              data.url && data.url.protocol != undefined
                ? data.url.protocol
                : 'http://',
            url = (data.url && CKEDITOR.tools.trim(data.url.url)) || '';
          attributes['data-cke-saved-href'] =
            url.indexOf('/') === 0 ? url : protocol + url;
          break;
        case 'anchor':
      ...
      if (data.adv) {
        ...
        if (data.adv['advName'])
          attributes['name'] = attributes['data-cke-saved-name'] =
            data.adv['advName'];
        else
      ...
      if (!this._.selectedElement) {
        ...
        if (
          ...
          element.addClass(
            element.getChildCount() ? 'cke_anchor' : 'cke_anchor_empty'
          );
