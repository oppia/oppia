...
(function () {
  ...
  NS.setHeightBannerFrame = function () {
    var height = '90px',
      bannerPlaceSpellTab = NS.dialog
        .getContentElement('SpellTab', 'banner')
        .getElement(),
      bannerPlaceGrammTab = NS.dialog
        .getContentElement('GrammTab', 'banner')
        .getElement(),
      bannerPlaceThesaurus = NS.dialog
  ...
  NS.sendData = function (scope) {
    ...
    scope.parts.tabs.on('click', function (event) {
      ...
      if (currentTab == scope._.currentTabId) {
        return;
      }
  ...
  NS.buildOptionSynonyms = function (key) {
    ...
    NS.textNode['Thesaurus'].setValue(
      NS.selectNode['synonyms'].getInputElement().getValue()
    );
  };

  var setBannerInPlace = function (htmlBanner) {
    var findBannerPlace = NS.dialog
      .getContentElement(NS.dialog._.currentTabId, 'banner')
      .getElement();
  ...
  var statusGrammarTab = function (aState) {
    //#19221
    aState = aState || 'true';
  ...
  var langConstructor = function (lang) {
    ...
    NS.buildOptionLang(
      langSelectBox.setLangList,
      NS.dialog.getParentEditor().name
    );
  ...
  var disableButtonSuggest = function (word) {
    if (word == 'no_any_suggestions') {
      ...
      var styleDisable = function (instanceButton) {
        ...
        button.getElement().hasClass('cke_disabled')
          ? button.getElement().setStyle('color', '#a0a0a0')
          : button.disable();
      };
      ...
      NS.LocalizationButton['ChangeTo'].instance
        .getElement()
        .setStyle('color', '#333');
  ...
  var handlerId = {
    ...
    grammerSuggest: function (response) {
      ...
      for (var i = 0; i < len; i++) {
        NS.grammerSuggest
          .getElement()
          .append(buildRadioInputs(arr[i], arr[i], check));
        check = false;
      }
    ...
    thesaurusSuggest: function (response) {
      ...
      var synKey = NS.selectNode['categories'].getInputElement().getChildren()
        .$[0].value;
      NS.selectNode['categories']
        .getInputElement()
        .getChildren().$[0].selected = true;
  ...
  var showCurrentTabs = function () {
    NS.dialog
      .getContentElement(NS.dialog._.currentTabId, 'bottomGroup')
      .getElement()
      .show();
  };
  ...
  var showCurrentFinishChecking = function () {
    NS.dialog
      .getContentElement(NS.dialog._.currentTabId, 'BlockFinishChecking')
      .getElement()
      .show();
  };
  ...
  CKEDITOR.dialog.add('checkspell', function (editor) {
    var handlerButtons = function (event) {
      ...
      if (cmd == 'FinishChecking') {
        editor.config.wsc_onFinish.call(
    ...
    return {
      ...
      onShow: function () {
        ...
        CKEDITOR.scriptLoader.load(wscCoreUrl, function (success) {
          ...
          NS.currentLang =
            editor.config.wsc_lang ||
            NS.LangComparer.getSPLangCode(editor.langCode);
          ...
          if (!dialogPartsTab.getElementsByTag('DIV').count()) {
            dialogPartsTab.append(
              NS.buildSelectLang(NS.dialog.getParentEditor().name)
            );
          }
          ...
          NS.div_overlay_no_check = new overlayBlock({
            ...
            id: 'no_check_over',
            background:
              '#fff url(' + NS.loadIconEmptyEditor + ') no-repeat 50% 50%',
            target: NS.OverlayPlace,
          });
                              ...
                              children: [
                                {
                                  ...
                                  id: 'labelSuggestions',
                                  label:
                                    NS.LocalizationLabel['Suggestions'].text +
                                    ':',
                                  onShow: function () {
                                    NS.LocalizationLabel[
                                      'Suggestions'
                      ...
                      children: [
                        ...
                        },
                        {
                          ...
                          onLoad: function () {
                            ...
                            NS.LocalizationButton['IgnoreAllWords'].instance =
                              this;
                          },
                          onClick: handlerButtons,
                        },
  ...
  CKEDITOR.dialog.add('options', function (editor) {
    ...
    return {
                  ...
                  children: [
                    {
                      ...
                      onShow: function () {
                        this.getInputElement().$.children[0].innerHTML =
                          NS.LocalizationComing['SpellCheckingOptions'];
                      },
                      children: [
                        {
                          type: 'vbox',
                          ...
                          children: [
                            {
                              ...
                              default: '',
                              onClick: function () {
                                checkboxState[this.id] = !this.getValue()
                                  ? 0
                                  : 1;
                              },
                            },
                      ...
                      children: [
                        {
                          ...
                          onLoad: function () {
                            nameNode = this;
                            var udn = NS.userDictionaryName
                              ? NS.userDictionaryName
                              : appTools.cookie.get('udn') && undefined
                                ? ' '
                                : this.getValue();
                            this.setValue(udn);
                          },
                          onShow: function () {
                            nameNode = this;
                            var udn = !appTools.cookie.get('udn')
                              ? this.getValue()
                              : appTools.cookie.get('udn');
                            this.setValue(udn);
                            this.setLabel(
                              NS.LocalizationComing['DictionaryName']
                            );
                          },
                          onHide: function () {
                            this.reset();
      ...
      onLoad: function () {
        ...
        linkOnCheckbox['IgnoreAllCapsWords'] = dialog.getContentElement(
          'OptionsTab',
          'IgnoreAllCapsWords'
        );
        linkOnCheckbox['IgnoreWordsNumbers'] = dialog.getContentElement(
          'OptionsTab',
          'IgnoreWordsNumbers'
        );
        linkOnCheckbox['IgnoreMixedCaseWords'] = dialog.getContentElement(
          'OptionsTab',
          'IgnoreMixedCaseWords'
        );
        linkOnCheckbox['IgnoreDomainNames'] = dialog.getContentElement(
          'OptionsTab',
          'IgnoreDomainNames'
        );
      },
      onShow: function () {
        ...
        !parseInt(checkboxState['IgnoreAllCapsWords'], 10)
          ? linkOnCheckbox['IgnoreAllCapsWords'].setValue('', false)
          : linkOnCheckbox['IgnoreAllCapsWords'].setValue('checked', false);
        !parseInt(checkboxState['IgnoreWordsNumbers'], 10)
          ? linkOnCheckbox['IgnoreWordsNumbers'].setValue('', false)
          : linkOnCheckbox['IgnoreWordsNumbers'].setValue('checked', false);
        !parseInt(checkboxState['IgnoreMixedCaseWords'], 10)
          ? linkOnCheckbox['IgnoreMixedCaseWords'].setValue('', false)
          : linkOnCheckbox['IgnoreMixedCaseWords'].setValue('checked', false);
        !parseInt(checkboxState['IgnoreDomainNames'], 10)
          ? linkOnCheckbox['IgnoreDomainNames'].setValue('', false)
          : linkOnCheckbox['IgnoreDomainNames'].setValue('checked', false);

        checkboxState['IgnoreAllCapsWords'] = !linkOnCheckbox[
          'IgnoreAllCapsWords'
        ].getValue()
          ? 0
          : 1;
        checkboxState['IgnoreWordsNumbers'] = !linkOnCheckbox[
          'IgnoreWordsNumbers'
        ].getValue()
          ? 0
          : 1;
        checkboxState['IgnoreMixedCaseWords'] = !linkOnCheckbox[
          'IgnoreMixedCaseWords'
        ].getValue()
          ? 0
          : 1;
        checkboxState['IgnoreDomainNames'] = !linkOnCheckbox[
          'IgnoreDomainNames'
        ].getValue()
          ? 0
          : 1;
        ...
        linkOnCheckbox[
          'IgnoreAllCapsWords'
        ].getElement().$.lastChild.innerHTML =
          NS.LocalizationComing['IgnoreAllCapsWords'];
        linkOnCheckbox[
          'IgnoreWordsNumbers'
        ].getElement().$.lastChild.innerHTML =
        ...
        linkOnCheckbox[
          ...
          NS.LocalizationComing['IgnoreMixedCaseWords'];
        linkOnCheckbox['IgnoreDomainNames'].getElement().$.lastChild.innerHTML =
          NS.LocalizationComing['IgnoreDomainNames'];
      },
    };
  });
