...
CKEDITOR.dialog.add('checkspell', function (editor) {
  ...
  var wscCoreUrl =
    editor.config.wsc_customLoaderScript ||
    protocol +
      '//loader.webspellchecker.net/sproxy_fck/sproxy.php' +
      '?plugin=fck2' +
      '&customerid=' +
      editor.config.wsc_customerId +
  ...
  if (editor.config.wsc_customLoaderScript) {
    errorMsg +=
      ...
      editor.lang.wsc.errorLoading.replace(
        /%s/g,
        editor.config.wsc_customLoaderScript
      ) +
      '</p>';
  ...
  function initAndSpell(dialog) {
    ...
    window.doSpell({
      ...
      lang:
        editor.config.wsc_lang || LangComparer.getSPLangCode(editor.langCode),
      intLang:
        editor.config.wsc_uiLang || LangComparer.getSPLangCode(editor.langCode),
      ...
      customDictionaryName:
        ...
        editor.config.wsc_customDictionaryIds.split(','),
  ...
  return {
    ...
    onShow: function () {
      var contentArea = this.getContentElement(
        'general',
        'content'
      ).getElement();
      ...
      contentArea
        .getChild(2)
        .setStyle('height', this._.contentSize.height + 'px');
