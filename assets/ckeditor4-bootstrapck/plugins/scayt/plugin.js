...
(function () {
  ...
  var onEngineLoad = function () {
    ...
    var createInstance = function (ev) {
      // Create new instance every time Document is created.
      if (
        typeof plugin.instances[editor.name] != 'undefined' ||
        plugin.instances[editor.name] != null
      )
        return;
      ...
      if (typeof scayt_custom_params == 'object') {
        for (var k in scayt_custom_params) oParams[k] = scayt_custom_params[k];
      }
    ...
    editor.on('afterCommandExec', function (ev) {
      if (!plugin.isScaytEnabled(editor)) return;

      if (
        editor.mode == 'wysiwyg' &&
        (ev.data.name == 'undo' || ev.data.name == 'redo')
      ) {
    ...
    editor.on('scaytDialog', function (ev) {
      ...
      htmlFilter.addRules({
        elements: {
          span: function (element) {
            if (
              element.attributes['data-scayt_word'] &&
    ...
    var equalsContentMethodName =
      typeof undoImagePrototype.equalsContent == 'function'
        ? 'equalsContent'
        : 'equals';

    undoImagePrototype[equalsContentMethodName] = CKEDITOR.tools.override(
      undoImagePrototype[equalsContentMethodName],
      function (org) {
    ...
    editorPrototype.checkDirty = CKEDITOR.tools.override(
      editorPrototype.checkDirty,
      function (org) {
        return function () {
          var retval = null,
  ...
  CKEDITOR.plugins.scayt = {
    ...
    setControlInfo: function (editor, o) {
      if (
        editor &&
        editor.name &&
        typeof this.controlInfo[editor.name] != 'object'
      )
    ...
    getUiTabs: function (editor) {
      ...
      for (var i = 0; i < 4; i++) {
        uiTabs[i] =
          typeof window.scayt != 'undefined' &&
          typeof window.scayt.uiTags != 'undefined'
  ...
  var commandDefinition = {
    ...
    exec: function (editor) {
      ...
      } else if (!editor.config.scayt_autoStartup && plugin.engineLoaded >= 0) {
        // Load first time
        editor.focus();
  ...
  CKEDITOR.plugins.add('scayt', {
    ...
    beforeInit: function (editor) {
      ...
      editor.config.menu_groups =
        items_order_str + ',' + editor.config.menu_groups;
    },
      ...
      if (editor.contextMenu && editor.addMenuItems) {
        editor.contextMenu.addListener(function (element, selection) {
          ...
          var sLang = scayt_control.getLang(),
            _r = {},
            contextCommands = editor.config.scayt_contextCommands || 'all',
          ...
          if (!items_suggestion || !items_suggestion.length) {
            ...
            for (var i = 0, l = items_suggestion.length; i < l; i += 1) {
              var commandName =
                'scayt_suggestion_' + items_suggestion[i].replace(' ', '_');
    ...
    afterInit: function (editor) {
      ...
      var elementsPathFilters,
        scaytFilter = function (element) {
          if (element.hasAttribute('data-scaytid')) return false;
        };

      if (
        editor._.elementsPath &&
        (elementsPathFilters = editor._.elementsPath.filters)
      )
