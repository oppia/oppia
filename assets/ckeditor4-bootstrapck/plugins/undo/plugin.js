  ...
  CKEDITOR.plugins.add('undo', {
    ...
    init: function (editor) {
      ...
      undoManager.onChange = function () {
        undoCommand.setState(
          undoManager.undoable()
            ? CKEDITOR.TRISTATE_OFF
            : CKEDITOR.TRISTATE_DISABLED
        );
        redoCommand.setState(
          undoManager.redoable()
            ? CKEDITOR.TRISTATE_OFF
            : CKEDITOR.TRISTATE_DISABLED
        );
      };
      ...
      function toggleUndoManager() {
        undoManager.enabled = editor.readOnly
          ? false
          : editor.mode == 'wysiwyg';
      ...
      editor.on('updateSnapshot', function () {
        if (undoManager.currentImage) undoManager.update();
      });
      ...
      if (startedTyping || modifierSnapshot) {
        ...
        CKEDITOR.tools.setTimeout(
          function () {
            ...
            // In IE, we need to remove the expando attributes.
            if (CKEDITOR.env.ie)
              currentSnapshot = currentSnapshot.replace(
                /\s+data-cke-expando=".*?"/g,
                ''
              );
            ...
            if (
              beforeTypeImage.contents != currentSnapshot &&
              beforeTypeCount == this.snapshots.length
            ) {
    ...
    lock: function (dontUpdate) {
      if (!this.locked) {
        if (dontUpdate) this.locked = {level: 1};
          ...
          // during this period.
          var matchedTip =
            this.currentImage && this.currentImage.equalsContent(imageBefore);
