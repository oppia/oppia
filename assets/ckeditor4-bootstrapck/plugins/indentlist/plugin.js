...
(function () {
  ...
  CKEDITOR.plugins.add('indentlist', {
    ...
    init: function (editor) {
          ...
          function (evt) {
            ...
            if (evt.data.keyCode == this.indentKey) {
              ...
              if (list) {
                ...
                if (
                  this.isIndent &&
                  firstItemInPath.call(this, editor.elementPath(), list)
                )
                  return;
      ...
      CKEDITOR.tools.extend(
        ...
        globalHelpers.specificDefinition.prototype,
        {
  ...
  function indentList(editor) {
    ...
    function indent(listNode) {
      ...
      var indentOffset = that.isIndent ? 1 : -1,
        ...
        baseIndent =
          listArray[lastItem.getCustomData('listarray_index')].indent;
      ...
      for (
        ...
        if (indentOffset > 0) {
          ...
          listArray[i].parent = new CKEDITOR.dom.element(
            listRoot.getName(),
            listRoot.getDocument()
          );
        }
      }
      ...
      if (!that.isIndent) {
        var parentLiElement;
        if (
          (parentLiElement = listNode.getParent()) &&
          parentLiElement.is('li')
        ) {
        ...
        for (i = 0; i < pendingLis.length; i++) {
          ...
          while (
            ...
            if (
              ...
              !li.getFirst(neitherWhitespacesNorBookmark)
            )
    ...
    while ((range = iterator.getNextRange())) {
      ...
      while (
        nearestListBlock &&
        !(
          nearestListBlock.type == CKEDITOR.NODE_ELEMENT &&
      ...
      if (!nearestListBlock) {
        ...
        if (
          ...
          selectedNode.getName() in context
        ) {
  ...
  function firstItemInPath(path, list) {
    if (!list) list = path.contains(this.context);
