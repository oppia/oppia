...
(function () {
  ...
  function commitAdvParams() {
    ...
    if (element) {
      ...
      else element.removeAttribute(attrName, value);
    }
  }
  ...
  CKEDITOR.plugins.add('dialogadvtab', {
    ...
    allowedContent: function (tabConfig) {
      if (!tabConfig) tabConfig = defaultTabConfig;

      var allowedAttrs = [];
      if (tabConfig.id) allowedAttrs.push('id');
      if (tabConfig.dir) allowedAttrs.push('dir');

      var allowed = '';

      if (allowedAttrs.length) allowed += '[' + allowedAttrs.join(',') + ']';

      if (tabConfig.classes) allowed += '(*)';
      if (tabConfig.styles) allowed += '{*}';
        ...
        if (tabConfig.styles) {
          contents.push({
            ...
            validate: CKEDITOR.dialog.validate.inlineStyle(
              lang.invalidInlineStyle
            ),
            onChange: function () {},

            getStyle: function (name, defaultValue) {
              var match = this.getValue().match(
            ...
            updateStyle: function (name, value) {
              ...
              styles = CKEDITOR.tools.normalizeCssText(
                tmp.getAttribute('style')
              );
