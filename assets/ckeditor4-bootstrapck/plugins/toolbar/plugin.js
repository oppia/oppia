...
(function () {
  ...
  CKEDITOR.plugins.add('toolbar', {
    ...
    init: function (editor) {
      ...
      var itemKeystroke = function (item, keystroke) {
        ...
        toolbarGroupCycling =
          toolbarGroupCycling === undefined || toolbarGroupCycling;
        ...
        switch (keystroke) {
          ...
          case CKEDITOR.SHIFT + 9: // SHIFT + TAB
            ...
            while (!toolbar || !toolbar.items.length) {
              toolbar =
                keystroke == 9
                  ? (toolbar ? toolbar.next : item.toolbar.next) ||
                    editor.toolbox.toolbars[0]
                  : (toolbar ? toolbar.previous : item.toolbar.previous) ||
                    editor.toolbox.toolbars[editor.toolbox.toolbars.length - 1];
      ...
      editor.on('uiSpace', function (event) {
        if (event.data.space != editor.config.toolbarLocation) return;
          ...
          for (var i = 0; i < items.length; i++) {
            ...
            if (item) {
              ...
              if (!toolbarObj) {
                ...
                toolbarName =
                  row.name &&
                  (editor.lang.toolbar.toolbarGroups[row.name] || row.name);
                ...
                output.push(
                  ...
                  '" class="cke_toolbar"',
                  toolbarName
                    ? ' aria-labelledby="' + toolbarId + '_label"'
                    : '',
                  ' role="toolbar">'
                );
            ...
            exec: function (editor) {
              ...
              if (!collapsed) {
                ...
                collapser.setAttribute(
                  'title',
                  editor.lang.toolbar.toolbarExpand
                );
              } else {
                toolbox.show();
                ...
                collapser.setAttribute(
                  'title',
                  editor.lang.toolbar.toolbarCollapse
                );
              }
          ...
          output.push(
            '<a title="' +
              (expanded
                ...
                : editor.lang.toolbar.toolbarExpand) +
              '" id="' +
              editor.ui.spaceId('toolbar_collapser') +
  ...
  function getToolbarConfig(editor) {
    ...
    function buildToolbarConfig() {
      ...
      var toolbar =
        CKEDITOR.tools.clone(editor.config.toolbarGroups) ||
    ...
    function fillGroup(toolbarGroup, uiItems) {
      if (uiItems.length) {
        if (toolbarGroup.items) toolbarGroup.items.push(editor.ui.create('-'));
        ...
        while ((item = uiItems.shift())) {
          ...
          if (
            !removeButtons ||
            CKEDITOR.tools.indexOf(removeButtons, name) == -1
          ) {
   ...
   * @member CKEDITOR.ui
   */
  CKEDITOR.ui.prototype.addToolbarGroup = function (
    name,
    previous,
    subgroupOf
  ) {
