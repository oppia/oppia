...
CKEDITOR.plugins.add('menu', {
  ...
  beforeInit: function (editor) {
    ...
    for (var i = 0; i < groups.length; i++) groupsOrder[groups[i]] = i + 1;
...
(function () {
  ...
  CKEDITOR.menu = CKEDITOR.tools.createClass({
    ...
    $: function (editor, definition) {
      ...
      var attrs = (panelDefinition.block.attributes =
        panelDefinition.attributes || {});
    ...
    _: {
        ...
        for (var i = 0; i < listeners.length; i++) {
          ...
          if (listenerItems) {
            for (var itemName in listenerItems) {
              ...
              if (
                item &&
                (!item.command || this.editor.getCommand(item.command).state)
              ) {
      ...
      showSubMenu: function (index) {
        ...
        if (menu) menu.removeAll();
          ...
          menu = this._.subMenu = new CKEDITOR.menu(
            this.editor,
        ...
        var element = this._.panel
          .getBlock(this.id)
          .element.getDocument()
          .getById(this.id + String(index));
    ...
    proto: {
      ...
      add: function (item) {
        ...
        if (!item.order) item.order = this.items.length;
      ...
      show: function (offsetParent, corner, offsetX, offsetY) {
        ...
        if (!panel) {
          panel = this._.panel = new CKEDITOR.ui.floatPanel(
            this.editor,
          ...
          panel.onEscape = CKEDITOR.tools.bind(function (keystroke) {
            if (this._.onEscape(keystroke) === false) return false;
          }, this);
          ...
          keys[editor.lang.dir == 'rtl' ? 37 : 39] = CKEDITOR.env.ie
            ? 'mouseup'
            : 'click'; // ARROW-RIGHT/ARROW-LEFT(rtl)
          ...
          this._.itemOverFn = CKEDITOR.tools.addFunction(function (index) {
            ...
            this._.showSubTimeout = CKEDITOR.tools.setTimeout(
              this._.showSubMenu,
              editor.config.menu_subMenuDelay || 400,
              this,
              [index]
            );
          }, this);
        ...
        var path = editor.elementPath(),
          mixedDirCls =
            path && path.direction() != editor.lang.dir
              ? ' cke_mixed_dir_content'
              : '';

        // Build the HTML that composes the menu and its items.
        var output = [
          '<div class="cke_menu' + mixedDirCls + '" role="presentation">',
        ];
        ...
        for (var i = 0; i < length; i++) {
          ...
          if (lastGroup != item.group) {
            output.push(
              '<div class="cke_menuseparator" role="separator"></div>'
            );
        ...
        // Show the panel.
        if (this.parent)
          this.parent._.panel.showAsChild(
            panel,
            this.id,
            offsetParent,
            corner,
  ...
  function sortItems(items) {
    items.sort(function (itemA, itemB) {
      if (itemA.group < itemB.group) return -1;
    ...
    proto: {
      render: function (menu, index, output) {
        ...
        if (this.role in {menuitemcheckbox: 1, menuitemradio: 1})
          ariaChecked =
            ' aria-checked="' +
            (state == CKEDITOR.TRISTATE_ON ? 'true' : 'false') +
            '"';
        ...
        var arrowLabel =
          '&#' + (this.editor.lang.dir == 'rtl' ? '9668' : '9658') + ';';
        ...
        var params = {
          ...
          iconStyle: CKEDITOR.skin.getIconStyle(
            ...
            iconName == this.icon ? null : this.icon,
            this.iconOffset
          ),
