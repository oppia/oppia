...
CKEDITOR.dialog.add('a11yHelp', function (editor) {
  ...
  var replaceVariables = (function () {
    ...
    for (var i in keystrokesByCode) keystrokesByName[keystrokesByCode[i]] = i;

    return function (match, name) {
      // Return the keystroke representation or leave match untouched
      // if there's no keystroke for such command.
      return keystrokesByName[name]
        ? representKeyStroke(keystrokesByName[name])
        : match;
    };
  })();
  ...
  function buildHelpContents() {
    ...
    for (var i = 0; i < sectionLength; i++) {
      ...
      for (var j = 0; j < itemsLength; j++) {
        ...
        if (itemLegend.match(variablesPattern)) continue;
