...
(function () {
      ...
      editor.on(
        ...
        function (evt) {
          ...
          // Strip presentional markup & unify text markup.
            ...
            data = htmlTextification(
              editor.config,
              data,
              textificationFilter ||
                (textificationFilter = getTextificationFilter(editor))
            );
          }
  ...
  function initClipboard(editor) {
    ...
    function addListenersToEditable() {
      ...
      editable.on(mainPasteEvent, function (evt) {
        if (CKEDITOR.env.ie && preventBeforePasteEvent) return;
      ...
      if (CKEDITOR.env.ie) {
        ...
        editable.on(
          'beforepaste',
          function (evt) {
            if (evt.data && !evt.data.$.ctrlKey) preventBeforePasteEventNow();
          },
          null,
          null,
          0
    ...
    function createCutCopyCmd(type) {
      return {
        ...
        exec: function (data) {
          ...
          if (!success) alert(editor.lang.clipboard[this.type + 'Error']); // Show cutError or copyError.
      ...
      return {
        ...
        exec: function (editor, data) {
          var fire = function (data, withBeforePaste) {
              data &&
                firePasteEvents(data.type, data.dataValue, !!withBeforePaste);
    ...
    function execIECommand(command) {
      ...
      (CKEDITOR.env.version > 7 ? doc.$ : doc.$.selection.createRange())[
        'execCommand'
      ](command);
    ...
    function firePasteEvents(type, data, withBeforePaste) {
      ...
      if (withBeforePaste) {
        ...
        // by other plugins.
        if (!editor.fire('beforePaste', eventData)) return false; // Event canceled
      }
    ...
    function getClipboardDataByPastebin(evt, callback) {
      ...
      var pastebin = new CKEDITOR.dom.element(
        (CKEDITOR.env.webkit || editable.is('body')) &&
        !(CKEDITOR.env.ie || CKEDITOR.env.opera)
          ? 'body'
          : 'div',
        doc
      );
    ...
    function getClipboardDataDirectly() {
      if (CKEDITOR.env.ie) {
        ...
        try {
          if (
            editor.editable().fire(mainPasteEvent) &&
    ...
    function pasteDataFromClipboard(evt) {
      ...
      getClipboardDataByPastebin(evt, function (data) {
        // Clean up.
        data = data.replace(/<span[^>]+data-cke-bookmark[^<]*?<\/span>/gi, '');
  ...
  function getTextificationFilter(editor) {
    ...
    var replaceWithParaIf = {
      ...
      removeIf = CKEDITOR.tools.extend(
        {
          area: 1,
          basefont: 1,
          embed: 1,
  ...
  function htmlTextification(config, data, filter) {
    ...
    // Cleanup cke:brs.
    data = data
      .replace(/\s*(<\/?[a-z:]+ ?\/?>)\s*/g, '$1') // Remove spaces around tags.
    ...
    data = data
      .replace(/<\/?p>/g, function (match) {
        if (match == '<p>') {
          if (++nested > 1) return '</p><p>';
        } else {
          if (--nested > 0) return '</p><p>';
        }
