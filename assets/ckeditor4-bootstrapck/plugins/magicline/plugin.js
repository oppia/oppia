...
(function () {
  ...
  function initPlugin(editor) {
    ...
    function addListeners() {
      ...
      that.boundary = that.inInlineMode
        ? that.editable
        : that.doc.getDocumentElement();
      ...
      editable.attachListener(
        that.inInlineMode ? doc : doc.getWindow().getFrame(),
        'mouseout',
        function (event) {
      ...
      editable.attachListener(
        that.inInlineMode ? editable : doc,
        'mousemove',
        function (event) {
      ...
      editable.attachListener(win, 'scroll', function (event) {
        ...
        if (env.webkit) {
          ...
          scrollTimeout = setTimeout(function () {
            ...
            if (!that.mouseDown) that.hiddenMode = 0;
            that.debug.showHidden(that.hiddenMode); // %REMOVE_LINE%
          }, 50);
        ...
        for (var t in {p: 1, br: 1, div: 1}) {
          ...
          for (i = elements.count(); i--; ) {
            if ((element = elements.getItem(i)).data('cke-magicline-hot')) {
              ...
              that.lastCmdDirection =
                element.data('cke-magicline-dir') === 'true' ? true : false;
      ...
      function checkMouse(mouse) {
        ...
        if (
          ...
          (that.element = elementFromMouse(that, true))
        ) {
          // 	-> There must be valid element.
  ...
  var extend = CKEDITOR.tools.extend,
    ...
    CSS_COMMON =
      'width:0px;height:0px;padding:0px;margin:0px;display:block;' +
      'z-index:9999;color:#fff;position:absolute;font-size: 0px;line-height:0px;',
    CSS_TRIANGLE =
      CSS_COMMON + 'border-color:transparent;display:block;border-style:solid;',
  ...
  function getAscendantTrigger(that) {
    ...
    if (node && isHtml(node)) {
      ...
      if (trigger && that.editable.contains(trigger)) {
        ...
        if (limit.getAttribute('contenteditable') == 'true') return trigger;
        // Trigger in non-editable area.
        else if (limit.is(that.triggers)) return limit;
        else return null;
  ...
  function getNonEmptyNeighbour(that, node, goBack) {
    node = node[goBack ? 'getPrevious' : 'getNext'](function (node) {
      return (
        (isTextNode(node) && !isEmptyTextNode(node)) ||
  ...
  function getClosestEditableLimit(element, includeSelf) {
    if (element.data('cke-editable')) return null;

    if (!includeSelf) element = element.getParent();
  ...
  function initLine(that) {
    var doc = that.doc,
      ...
      iconPath =
        this.path + 'images/' + (env.hidpi ? 'hidpi/' : '') + 'icon.png';

    extend(line, {
      attach: function () {
        // Only if not already attached
        if (!this.wrap.getParent()) this.wrap.appendTo(that.editable, true);
      ...
      lineChildren: [
        ...
        extend(newElementFromHtml(TRIANGLE_HTML, doc), {
          base:
            CSS_TRIANGLE + 'left:0px;border-left-color:' + that.boxColor + ';',
          looks: [
      ...
      detach: function () {
        ...
        if (this.wrap.getParent()) this.wrap.remove();
      ...
      place: function () {
        ...
        if (parent.equals(editable)) {
          ...
          styleSet.left =
            ...
            view.scroll.x -
            (that.inInlineMode
              ? view.editable.left + view.editable.border.left
              : 0);
          styleSet.width =
            any.size.outerWidth +
            any.size.margin.left +
            any.size.margin.right +
            view.scroll.x;
          styleSet.right = '';
        }
        ...
        if (upper && lower) {
          ...
          if (upper.size.margin.bottom === lower.size.margin.top)
            styleSet.top =
              0 | (upper.size.bottom + upper.size.margin.bottom / 2);
          else {
            ...
            // Upper margin > lower margin. Place at upper margin - lower margin.
              ...
              styleSet.top =
                upper.size.bottom +
                upper.size.margin.bottom -
                lower.size.margin.top;
          }
        }
        ...
        if (
          trigger.is(LOOK_TOP) ||
          inBetween(styleSet.top, view.scroll.y - 15, view.scroll.y + 5)
        ) {
          ...
          styleSet.top = that.inInlineMode
            ? view.editable.height +
              view.editable.padding.top +
              view.editable.padding.bottom
            : view.pane.bottom - 1;
      ...
      look: function (look) {
        if (this.oldLook == look) return;

        for (var i = this.lineChildren.length, child; i--; )
          (child = this.lineChildren[i]).setAttribute(
            'style',
            child.base + child.looks[0 | (look / 2)]
          );
      ...
      exec: (function () {
        ...
        function doAccess(target) {
          ...
          var hotNodeChar = env.ie && env.version < 9 ? ' ' : WHITE_SPACE,
            removeOld =
              that.hotNode && // Old hotNode must exist.
          ...
          accessFocusSpace(that, function (accessNode) {
            if (removeOld && that.hotNode) that.hotNode.remove();
        ...
        return function (editor) {
          ...
          if (isInTabu(that, selected)) return;
          ...
          if (
            !selected ||
            selected.equals(that.editable) ||
            selected.contains(that.editable)
          )
            return;
          ...
          if (
            (limit = getClosestEditableLimit(selected)) &&
            limit.getAttribute('contenteditable') == 'false'
          )
            selected = limit;
          ...
          if (
            ...
            (!getNonEmptyNeighbour(that, neighbor, !insertAfter) ||
              ...
              ((neighborSibling = getNonEmptyNeighbour(
                that,
                neighbor,
                !insertAfter
              )) &&
  ...
  function isLine(that, node) {
    if (!(node && node.type == CKEDITOR.NODE_ELEMENT && node.$)) return false;
  ...
  function isFloated(element) {
    ...
    return !!(
      options[element.getComputedStyle('float')] ||
      options[element.getAttribute('align')]
    );
  }
  ...
  function isInTabu(that, element) {
    ...
    for (var i = parents.length; i--; ) {
      for (var j = that.tabuList.length; j--; ) {
        if (parents[i].hasAttribute(that.tabuList[j])) return true;
      }
    }
  ...
  function isChildBetweenPointerAndEdge(that, parent, edgeBottom) {
    var edgeChild = parent[edgeBottom ? 'getLast' : 'getFirst'](
      function (node) {
    ...
    return edgeBottom
      ? edgeChild.size.top > that.mouse.y
      : edgeChild.size.bottom < that.mouse.y;
  }
  ...
  function triggerEditable(that) {
    ...
    if (
      !bottomTrigger && // Top trigger case.
      edgeNode.size.top >= 0 && // Check if the first element is fully visible.
      inBetween(mouse.y, 0, edgeNode.size.top + triggerOffset)
    ) {
      // Check if mouse in [0, edgeNode.top + triggerOffset].

      // Determine trigger look.
      triggerLook =
        that.inInlineMode || view.scroll.y === 0 ? LOOK_TOP : LOOK_NORMAL;
    ...
    // \->	Bottom case.
      ...
      triggerLook =
        that.inInlineMode ||
        inBetween(
          edgeNode.size.bottom,
          view.pane.height - triggerOffset,
          view.pane.height
        )
          ? LOOK_BOTTOM
          : LOOK_NORMAL;
  ...
  function triggerEdge(that) {
    ...
    //	\-> Top trigger.
    if (
      inBetween(mouse.y, element.size.top - 1, element.size.top + fixedOffset)
    )
      bottomTrigger = false;
    //	\-> Bottom trigger.
    else if (
    ...
    if (!elementSibling) {
      ...
      if (
        element.equals(
          that.editable[bottomTrigger ? 'getLast' : 'getFirst'](that.isRelevant)
        )
      ) {
        updateEditableSize(that);
      ...
      triggerSetup = [null, element]
        [bottomTrigger ? 'reverse' : 'concat']()
        .concat([
          bottomTrigger ? EDGE_BOTTOM : EDGE_TOP,
          TYPE_EDGE,
          triggerLook,
          element.equals(
            that.editable[bottomTrigger ? 'getLast' : 'getFirst'](
              that.isRelevant
            )
          )
            ? bottomTrigger
    ...
    // If so, create an TYPE_EDGE, EDGE_MIDDLE trigger.
      ...
      // Prepare a trigger.
      triggerSetup = [elementSibling, element]
        [bottomTrigger ? 'reverse' : 'concat']()
        .concat([EDGE_MIDDLE, TYPE_EDGE]);
  ...
  var triggerExpand = (function () {
    ...
    function expandEngine(that) {
      ...
      if (startElement.isReadOnly()) return null;

      (trigger = verticalSearch(
        that,
      ...
      that.debug.logElements(
        [startElement, upper, lower], // %REMOVE_LINE%
        ['Start', 'Upper', 'Lower'],
        'Post-processing'
      ); // %REMOVE_LINE%
      ...
      if (!checkMouseBetweenElements(that, upper, lower)) {
        that.debug.logEnd(
          'ABORT. Mouse is already above upper or below lower.'
        ); // %REMOVE_LINE%
        return null;
      }
      ...
      while (lower && !lower.equals(upper)) {
        // 3.1.
        if (!(upperNext = upper.getNext(that.isRelevant))) break;

        // 3.2.
        currentDistance = Math.abs(
          getMidpoint(that, upper, upperNext) - that.mouse.y
        );
      ...
      that.debug.logElements(
        [minElement, minElementNext], // %REMOVE_LINE%
        ['Min', 'MinNext'],
      ...
      if (!checkMouseBetweenElements(that, minElement, minElementNext)) {
        that.debug.logEnd(
          'ABORT. Mouse is already above minElement or below minElementNext.'
        ); // %REMOVE_LINE%
        return null;
      }
    ...
    function expandFilter(that, trigger) {
      ...
      if (
        ...
        that.debug.logEnd(
          'REJECTED. No upper or no lower or they contain each other.'
        ); // %REMOVE_LINE%
      ...
      // YES: two trigger elements, pure siblings.
        ...
        isTrigger(that, upper) &&
        isTrigger(that, lower) &&
        areSiblings(that, upper, lower)
      ) {
        that.debug.logElementsEnd(
          [upper, lower], // %REMOVE_LINE%
          ['upper', 'lower'],
          'APPROVED EDGE_MIDDLE'
        ); // %REMOVE_LINE%
  ...
  function getSize(that, element, ignoreScroll, force) {
    var getStyle = (function () {
        ...
        var computed = env.ie
          ...
          : that.win.$.getComputedStyle(element.$, '');
    ...
    for (var i = sizePrefixes.length; i--; ) {
      border[sizePrefixes[i]] =
        parseInt(getStyle('border-' + sizePrefixes[i] + '-width'), 10) || 0;
      padding[sizePrefixes[i]] =
        parseInt(getStyle('padding-' + sizePrefixes[i]), 10) || 0;
      margin[sizePrefixes[i]] =
        parseInt(getStyle('margin-' + sizePrefixes[i]), 10) || 0;
    }

    // updateWindowSize if forced to do so OR NOT ignoring scroll.
    if (!ignoreScroll || force) updateWindowSize(that, force);

    (box.top = docPosition.y - (ignoreScroll ? 0 : that.view.scroll.y)),
      (box.left = docPosition.x - (ignoreScroll ? 0 : that.view.scroll.x)),
      // w/ borders and paddings.
      (box.outerWidth = element.$.offsetWidth),
      (box.outerHeight = element.$.offsetHeight),
      ...
      (box.bottom = box.top + box.outerHeight),
      (box.right = box.left + box.outerWidth);
  ...
  function updateSize(that, element, ignoreScroll) {
    ...
    // This kind of caching provides great performance improvement.
      ...
      element.size.ignoreScroll == ignoreScroll &&
      element.size.date > new Date() - CACHE_TIME
    ) {
  ...
  function verticalSearch(that, stopCondition, selectCriterion, startElement) {
    ...
    while (mouse.y + mouseStep < viewPaneHeight && mouse.y - mouseStep > 0) {
      if (!upperFound) upperFound = stopCondition(upper, startElement);
...
CKEDITOR.config.magicline_keystrokePrevious =
  CKEDITOR.CTRL + CKEDITOR.SHIFT + 51; // CTRL + SHIFT + 3
