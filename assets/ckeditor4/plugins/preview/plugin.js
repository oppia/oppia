...
(function () {
  function h(a) {
    var e = CKEDITOR.plugins.getPath('preview'),
      ...
      f = (function () {
        ...
        if (d.baseHref)
          return '\x3cbase href\x3d"{HREF}"\x3e'.replace('{HREF}', d.baseHref);
        b = b.split('/');
        b.pop();
        b = b.join('/');
    ...
    return d.fullPage
      ...
      : d.docType +
          ...
          g +
          '\x3c/title\x3e' +
          CKEDITOR.tools.buildStyleHtml(d.contentsCss) +
          ...
          (function () {
            ...
            b.getAttribute('id') &&
              (c = c.replace(
                '\x3e',
                ' id\x3d"' + b.getAttribute('id') + '"\x3e'
              ));
            b.getAttribute('class') &&
              (c = c.replace(
                '\x3e',
                ' class\x3d"' + b.getAttribute('class') + '"\x3e'
              ));
            return c;
          })() +
          a.getData() +
  ...
  CKEDITOR.plugins.preview = {
    createPreview: function (a) {
      ...
      d = Math.round(0.7 * c.height);
      g = Math.round(0.1 * c.width);
      c = CKEDITOR.env.ie
      ...
      window._cke_htmlToLoad ||
        ((e = a.document), e.open(), e.write(f.dataValue), e.close());
