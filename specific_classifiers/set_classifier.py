...
class SetClassifier(object):
  ...
  def Classify(self, response, categories, params=None):
    ...
    Args:
      ...
      categories: a list of strings, each taking one of the following forms:
    ...
    for index, category in enumerate(categories):
      ...
      if category.startswith('S has elements not in '):
        category_set = self.ParseSetCategory('S has elements not in ', category)
        if response_set.difference(category_set):
          # TODO(sll): For this and the next case, find a way to return a random
          # element from this set difference.
          return index
