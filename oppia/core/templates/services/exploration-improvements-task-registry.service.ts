...
export class StateTasks implements Iterable<ExplorationTask> {
  ...
  public refresh(
    ...
    this.hbrTask.refreshStatus(
      expStats,
      this.supportingStats.eqPlaythroughIssues.length,
      config
    );
    this.iflTask.refreshStatus(
      this.supportingStats.cstPlaythroughIssues.length
    );
    this.ngrTask.refreshStatus(this.supportingStats.answerStats);
...
export class ExplorationImprovementsTaskRegistryService {
  ...
  initialize(
    ...
    resolvedTaskTypesByStateName: ReadonlyMap<
      string,
      readonly ExplorationTaskType[]
    >,
    topAnswersByStateName: ReadonlyMap<string, readonly AnswerStats[]>,
    playthroughIssues: readonly PlaythroughIssue[]
  ): void {
    ...
    const newStateTasks = new StateTasks(
      ...
      new Map(
        ImprovementsConstants.TASK_TYPES.map(taskType => [
          taskType,
          ExplorationTaskModel.createNewObsoleteTask(
  ...
  onStateRenamed(oldStateName: string, newStateName: string): void {
    ...
    const newStateTasks = new StateTasks(
      newStateName,
      new Map(
        (oldStateTasks as StateTasks).map(oldTask => [
          oldTask.taskType,
  ...
  private validateInitializationArgs(
    ...
    if (expStats.expVersion !== config.explorationVersion) {
      throw new Error(
        'Expected stats for exploration version ' +
          config.explorationVersion +
          ', but got stats for exploration version ' +
          expStats.expVersion
      );
    }
    ...
    for (const task of openTasks) {
      if (task.entityId !== config.explorationId) {
        throw new Error(
          'Expected task for exploration "' +
            config.explorationId +
            '", but ' +
            'got task for exploration "' +
            task.entityId +
            '"'
        );
      }
    ...
    for (const task of openTasks) {
      const stateNameReferences = stateNameReferencesByTaskType.get(
        task.taskType
      );
      if ((stateNameReferences as Set<string>).has(task.targetId)) {
        throw new Error(
          'Found duplicate task of type "' +
            task.taskType +
            '" targeting ' +
            'state "' +
            task.targetId +
  ...
  private registerNewStateTasks(
    ...
    const tasksByType = new Map([
      ...
      ...openTasks.map(task => [task.taskType, task]),
      ...resolvedTaskTypes.map(taskType => [
        taskType,
        ExplorationTaskModel.createNewResolvedTask(
          this.config.explorationId,
          this.config.explorationVersion,
          taskType,
          stateName
  ...
  private refreshStateTasks(stateName: string): void {
    ...
    const tasksWithOldStatus: [ExplorationTask, string][] = (
      stateTasks as StateTasks
    ).map(task => [task, task.getStatus()]);
  ...
  private popOpenTask(task: ExplorationTask): void {
    ...
    (arrayWithTask as ExplorationTask[]).splice(
      (arrayWithTask as ExplorationTask[]).indexOf(task),
      1
    );
  }
}
