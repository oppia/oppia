...
describe('Speech Synthesis Chunker Service', () => {
  ...
  describe('formatLatexToSpeakableText', () => {
    ...
    it(
      'should properly convert trigonometric functions in LaTeX to ' +
        'speakable text',
      () => {
        var latex1 = '\\\\sin{90}';
        var latex2 = '\\\\cos{0}';
  ...
  describe('convertToSpeakableText', () => {
    it(
      'should properly convert the raw_latex-with-value attribute to' +
        ' speakable text',
      () => {
        const html =
        ...
        expect(speechSynthesisChunkerService.convertToSpeakableText(html)).toBe(
          '5 minus 1 Speech. Text. '
        );
      }
    );

    it(
      'should properly convert the text-with-value attribute to' +
        ' speakable text',
      () => {
        const html =
  ...
  describe('speak', function () {
    const MockSpeechSynthesisUtteranceConstructor = SpeechSynthesisUtterance;
    ...
    it('should speak only one phrase when cancel is requested', fakeAsync(() => {
      const speakSpy = spyOn(window.speechSynthesis, 'speak').and.callFake(
        function (utterance: SpeechSynthesisUtterance) {
          // This throws "Argument of type '{ speak: () => void; onend:
          // () => void; ...}' is not assignable to parameter of type
