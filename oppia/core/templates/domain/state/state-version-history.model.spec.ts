...
describe('State version history model', () => {
  ...
  it('should correctly set the value of state name in previous version', () => {
    ...
    expect(stateVersionHistory.stateNameInPreviousVersion).toEqual('state_2');
  });
  ...
  it('should convert into backend dict', () => {
    ...
    expect(stateVersionHistory.toBackendDict()).toEqual(
  ...
  it('should correctly be created from backend dict', () => {
    ...
    expect(anotherStateVersionHistory.previouslyEditedInVersion).toEqual(
      stateVersionHistory.previouslyEditedInVersion
    );
    expect(anotherStateVersionHistory.stateNameInPreviousVersion).toEqual(
      stateVersionHistory.stateNameInPreviousVersion
    );
    expect(anotherStateVersionHistory.committerUsername).toEqual(
      stateVersionHistory.committerUsername
    );
  });
});
