...
class StoryDomainUnitTests(test_utils.GenericTestBase):
    ...
    def test_story_node_update_thumbnail_filename(self) -> None:
        ...
        fs.commit(
            '%s/new_image.svg' % (constants.ASSET_TYPE_THUMBNAIL),
            raw_image,
    ...
    def test_get_published_node_count_counts_only_published_nodes(self) -> None:
        self.story.story_contents.next_node_id = 'node_4'
        node_1: story_domain.StoryNodeDict = {
            'id': 'node_1',
            'thumbnail_filename': 'image.svg',
    ...
    def test_rearrange_node_in_story(self) -> None:
        ...
        self.assertEqual(nodes[0].destination_node_ids, ['node_2'])
        self.assertEqual(nodes[1].destination_node_ids, ['node_3'])
        ...
        self.assertEqual(nodes[2].id, 'node_3')
        self.assertEqual(nodes[0].destination_node_ids, ['node_1'])
        self.assertEqual(nodes[1].destination_node_ids, ['node_3'])
        self.assertEqual(len(nodes[2].destination_node_ids), 0)
        ...
        self.assertEqual(nodes[0].destination_node_ids, ['node_3'])
        self.assertEqual(nodes[1].destination_node_ids, ['node_1'])
        self.assertEqual(len(nodes[2].destination_node_ids), 0)
    ...
    def test_validate_exploration_id_whose_value_is_none(self) -> None:
        ...
        self._assert_validation_error('Expected exploration ID to not be None')
