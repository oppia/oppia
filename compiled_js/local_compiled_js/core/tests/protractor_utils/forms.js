...
var richTextComponents = require('../../../extensions/rich_text_components/protractor.js');
...
var GraphEditor = function (graphInputContainer) {
    ...
    var vertexElement = function (index) {
        ...
        return graphInputContainer.element(by.css('.protractor-test-graph-vertex-' + index));
    };
    var createVertex = function (xOffset, yOffset) {
        var addNodeButton = graphInputContainer.element(by.css('.protractor-test-Add-Node-button'));
        addNodeButton.click();
    ...
    var createEdge = function (vertexIndex1, vertexIndex2) {
        var addEdgeButton = graphInputContainer.element(by.css('.protractor-test-Add-Edge-button'));
        addEdgeButton.click();
        browser.actions()
    ...
    return {
        ...
        clearDefaultGraph: function () {
            var deleteButton = graphInputContainer.element(by.css('.protractor-test-Delete-button'));
            deleteButton.click();
...
var ListEditor = function (elem) {
    ...
    var addItem = function (objectType) {
        ...
        if (objectType !== null) {
            return getEditor(objectType)(elem.element(by.repeater('item in localValue track by $index').row(listLength)));
        }
    };
    var deleteItem = function (index) {
        elem.element(by.repeater('item in localValue track by $index').row(index)).element(by.css('.protractor-test-delete-list-entry')).click();
    };
    return {
        editItem: function (index, objectType) {
            var item = elem.element(by.repeater('item in localValue track by $index').row(index));
            var editor = getEditor(objectType);
            return editor(item);
        },
        addItem: addItem,
        ...
        setLength: function (desiredLength) {
            elem.all(by.repeater('item in localValue track by $index')).count().then(function (startingLength) {
...
var RichTextEditor = function (elem) {
    ...
    return {
        ...
        addRteComponent: function (componentName) {
            ...
            richTextComponents.getComponent(componentName).customizeComponent.apply(null, args);
            modal.element(by.css('.protractor-test-close-rich-text-component-editor')).click();
            ...
            elem.all(by.css('.oppia-rte')).first().sendKeys(protractor.Key.chord(protractor.Key.CONTROL, protractor.Key.END));
        }
    };
};
...
var AutocompleteDropdownEditor = function (elem) {
    return {
        setValue: function (text) {
            ...
            element(by.css('.select2-dropdown')).element(by.css('.select2-search input')).sendKeys(text + '\n');
        },
        expectOptionsToBe: function (expectedOptions) {
            elem.element(by.css('.select2-container')).click();
            element(by.css('.select2-dropdown')).all(by.tagName('li')).map(function (optionElem) {
                return optionElem.getText();
            }).then(function (actualOptions) {
                expect(actualOptions).toEqual(expectedOptions);
            });
            // Re-close the dropdown.
            element(by.css('.select2-dropdown')).element(by.css('.select2-search input')).sendKeys('\n');
        }
    };
};
    ...
    return {
        setValues: function (texts) {
            ...
            elem.element(by.css('.select2-selection__rendered'))
                ...
                return choiceElem.element(by.css('.select2-selection__choice__remove'));
            ...
            for (var i = 0; i < texts.length; i++) {
                elem.element(by.css('.select2-container')).click();
                elem.element(by.css('.select2-search__field')).sendKeys(texts[i] + '\n');
            }
        },
    ...
    var _toggleElementStatusesAndVerifyExpectedClass = function (texts, expectedClassBeforeToggle) {
        ...
        elem.element(by.css('.protractor-test-search-bar-dropdown-menu'))
            ...
            if (filteredElements.length !== texts.length) {
                throw ('Could not toggle element selection. Values requested: ' + texts +
                    '. Found ' + filteredElements.length + ' matching elements.');
            }
            for (var i = 0; i < filteredElements.length; i++) {
                // Check that, before toggling, the element is in the correct state.
                expect(filteredElements[i].getAttribute('class')).toMatch(expectedClassBeforeToggle);
        ...
        elem.all(by.xpath(XPATH_SELECTOR)).map(function (entry) {
            ...
            elem.all(by.xpath(XPATH_SELECTOR)).then(function (arrayOfElements) {
                elem.getText().then(function (fullText) {
                    var checker = RichTextChecker(arrayOfElements, arrayOfTexts, fullText);
...
var RichTextChecker = function (arrayOfElems, arrayOfTexts, fullText) {
    ...
    var _readFormattedText = function (text, tagName) {
        ...
        expect(arrayOfElems[arrayPointer].getAttribute('innerHTML')).toBe(text);
    ...
    return {
        readPlainText: function (text) {
            ...
            expect(fullText.substring(textPointer, textPointer + text.length)).toEqual(text);
            textPointer = textPointer + text.length;
            justPassedRteComponent = false;
        },
...
var CodeMirrorChecker = function (elem) {
     ...
     */
    var _compareTextAndHighlightingFromLine = function (currentLineNumber, scrollTo, compareDict) {
        ...
        // large.
        browser.executeScript("$('.CodeMirror-vscrollbar').first().scrollTop(" + String(scrollTo) +
            ');');
        elem.all(by.xpath('./div')).map(function (lineElement) {
            return lineElement.element(by.css('.CodeMirror-linenumber')).getText()
                ...
                expect(lineElement.element(by.css('.CodeMirror-linebackground')).isPresent())
            ...
            if (largestLineNumber !== currentLineNumber) {
                _compareTextAndHighlightingFromLine(largestLineNumber, scrollTo + CODEMIRROR_SCROLL_AMOUNT_IN_PIXELS, compareDict);
            }
            else {
     ...
     *  - 'checked': true or false, whether the line has been checked
     */
    var _compareTextFromLine = function (currentLineNumber, scrollTo, compareDict) {
        browser.executeScript("$('.CodeMirror-vscrollbar').first().scrollTop(" + String(scrollTo) +
            ');');
        elem.getText().then(function (text) {
            ...
            if (largestLineNumber !== currentLineNumber) {
                _compareTextFromLine(largestLineNumber, scrollTo + CODEMIRROR_SCROLL_AMOUNT_IN_PIXELS, compareDict);
            }
            else {
