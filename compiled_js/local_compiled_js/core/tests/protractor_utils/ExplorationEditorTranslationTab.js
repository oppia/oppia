...
var ExplorationEditorTranslationTab = function () {
    ...
    var saveUploadedAudioButton = element(by.css('.protractor-test-save-uploaded-audio-button'));
    ...
    var audioOverFiveMinutesErrorMessageElement = element(by.className('oppia-audio-file-upload-field-error-message'));
    var uploadAudioButton = element.all(by.className('protractor-test-upload-audio-button')).last();
    ...
    var audioUploadInputElem = element(by.className('protractor-test-upload-audio'));
    var translationGraph = element(by.css('.protractor-test-translation-graph'));
    var stateBackgroundNodes = translationGraph.all(by.css('.protractor-test-node-background'));
    ...
    this.uploadAudio = function (relativePathOfAudioToUpload) {
        ...
        waitFor.elementToBeClickable(saveUploadedAudioButton, 'Save button is not clickable');
        saveUploadedAudioButton.click();
        waitFor.invisibilityOf(saveUploadedAudioButton, 'Upload Audio modal takes too long to disappear');
    };
    ...
    this.expectAudioOverFiveMinutes = function (relativePathOfAudioToUpload) {
        ...
        waitFor.elementToBeClickable(saveUploadedAudioButton, 'Save button is not clickable');
        saveUploadedAudioButton.click();
        waitFor.visibilityOf(audioOverFiveMinutesErrorMessageElement, 'Error element is not visible');
        expect(audioOverFiveMinutesErrorMessageElement.getText()).toContain('Audio files must be under 300 seconds in length.');
    };
    ...
    this.openUploadAudioModal = function () {
        waitFor.elementToBeClickable(uploadAudioButton, 'Upload Audio button is not clickable');
    ...
    this.moveToState = function (targetName) {
        ...
        stateNodes.map(function (stateElement) {
            ...
            if (!matched) {
                throw Error('State ' + targetName + ' not found by editorTranslationTab.' +
                    'moveToState.');
            }
        });
    };
        ...
        stateNodes.map(function (stateElement) {
            ...
            for (var i = 0; i < listOfNames.length; i++) {
                if (listOfNames[i] === stateName) {
                    expect(stateBackgroundNodes.get(i).getCssValue('fill')).toBe(expectedColor);
                    matched = true;
                }
            }
            if (!matched) {
