...
var ExplorationEditorFeedbackTab = function () {
     ...
     */
    var explorationFeedbackSubject = element(by.css('.protractor-test-exploration-feedback-subject'));
    var feedbackTabRow = element(by.css('.protractor-test-oppia-feedback-tab-row'));
    var explorationFeedback = element(by.css('.protractor-test-exploration-feedback'));
    var feedbackBackButton = element(by.css('.protractor-test-oppia-feedback-back-button'));
    var feedbackResponseTextArea = element(by.css('.protractor-test-feedback-response-textarea'));
    var suggestionRowClassName = '.protractor-test-oppia-feedback-tab-row';
    var suggestionCommitMessageInput = element(by.css('.protractor-test-suggestion-commit-message'));
    var suggestionReviewMessageInput = element(by.css('.protractor-test-suggestion-review-message'));
    /*
     * Buttons
     */
    var acceptSuggestionButton = element(by.css('.protractor-test-exploration-accept-suggestion-btn'));
    var feedbackSendResponseButton = element(by.css('.protractor-test-oppia-feedback-response-send-btn'));
    var rejectSuggestionButton = element(by.css('.protractor-test-exploration-reject-suggestion-btn'));
    var viewSuggestionButton = element(by.css('.protractor-test-view-suggestion-btn'));
    /*
     * Workflows
     */
    ...
    this.getSuggestionThreads = function () {
        var threads = [];
        waitFor.visibilityOf(element.all(by.css(suggestionRowClassName)).first(), 'No suggestion threads are visible');
        return element.all(by.css(suggestionRowClassName)).then(function (rows) {
    ...
    this.readFeedbackMessages = function () {
        var messages = [];
        waitFor.visibilityOf(element.all(by.css(suggestionRowClassName)).first(), 'No feedback messages are visible.');
        return element.all(by.css(suggestionRowClassName)).then(function (rows) {
