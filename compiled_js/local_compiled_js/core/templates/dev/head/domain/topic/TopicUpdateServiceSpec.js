...
describe('Topic update service', function () {
    ...
    beforeEach(angular.mock.inject(function ($injector) {
        ContentIdsToAudioTranslationsObjectFactory = $injector.get('ContentIdsToAudioTranslationsObjectFactory');
        ...
        _firstSkillSummary = SkillSummaryObjectFactory.create('skill_1', 'Description 1');
        _secondSkillSummary = SkillSummaryObjectFactory.create('skill_2', 'Description 2');
        _thirdSkillSummary = SkillSummaryObjectFactory.create('skill_3', 'Description 3');
        _sampleSubtopicPage = SubtopicPageObjectFactory.createFromBackendDict(sampleSubtopicPageObject);
        _sampleTopic = TopicObjectFactory.create(sampleTopicBackendObject.topicDict, sampleTopicBackendObject.skillIdToDescriptionDict);
    }));
    it('should add/remove an additional story id to/from a topic', function () {
    ...
    it('should add/remove a canonical story id to/from a topic', function () {
    ...
    it('should add/remove an uncategorized skill id to/from a topic', function () {
        expect(_sampleTopic.getUncategorizedSkillSummaries()).toEqual([
            _firstSkillSummary
        ]);
        TopicUpdateService.addUncategorizedSkill(_sampleTopic, _thirdSkillSummary);
        expect(_sampleTopic.getUncategorizedSkillSummaries()).toEqual([
    ...
    it('should not create a backend change dict for adding an uncategorized ' +
        ...
        expect(function () {
            TopicUpdateService.addUncategorizedSkill(_sampleTopic, _firstSkillSummary);
        }).toThrow();
        expect(UndoRedoService.getCommittableChangeList()).toEqual([]);
    });
    it('should remove/add an uncategorized skill id from/to a topic', function () {
        ...
        TopicUpdateService.removeUncategorizedSkill(_sampleTopic, _firstSkillSummary);
        expect(_sampleTopic.getUncategorizedSkillSummaries()).toEqual([]);
        UndoRedoService.undoChange(_sampleTopic);
        expect(_sampleTopic.getUncategorizedSkillSummaries()).toEqual([
            _firstSkillSummary
    ...
    it('should create a proper backend change dict for changing names', function () {
        TopicUpdateService.setTopicName(_sampleTopic, 'new name');
        expect(UndoRedoService.getCommittableChangeList()).toEqual([{
                cmd: 'update_topic_property',
                property_name: 'name',
    ...
    it('should create a proper backend change dict for changing subtopic title', function () {
        TopicUpdateService.setSubtopicTitle(_sampleTopic, 1, 'new title');
        expect(UndoRedoService.getCommittableChangeList()).toEqual([{
    ...
    it('should create a proper backend change dict for adding a subtopic', function () {
        TopicUpdateService.addSubtopic(_sampleTopic, 'Title2');
        expect(UndoRedoService.getCommittableChangeList()).toEqual([{
                cmd: 'add_subtopic',
                subtopic_id: 2,
    ...
    it('should create a proper backend change dict for deleting a subtopic', function () {
        TopicUpdateService.deleteSubtopic(_sampleTopic, 1);
        expect(UndoRedoService.getCommittableChangeList()).toEqual([{
                cmd: 'delete_subtopic',
                subtopic_id: 1,
    ...
    it('should move a skill id to a subtopic', function () {
        ...
        TopicUpdateService.moveSkillToSubtopic(_sampleTopic, null, 1, _firstSkillSummary);
        expect(_sampleTopic.getUncategorizedSkillSummaries()).toEqual([]);
        expect(_sampleTopic.getSubtopics()[0].getSkillSummaries()).toEqual([
    ...
    it('should correctly create changelists when moving a skill to a newly ' +
        ...
        TopicUpdateService.moveSkillToSubtopic(_sampleTopic, null, 2, _firstSkillSummary);
        TopicUpdateService.removeSkillFromSubtopic(_sampleTopic, 2, _firstSkillSummary);
        TopicUpdateService.deleteSubtopic(_sampleTopic, 2);
        expect(UndoRedoService.getCommittableChangeList()).toEqual([]);
        TopicUpdateService.addSubtopic(_sampleTopic, 'Title 2');
        TopicUpdateService.moveSkillToSubtopic(_sampleTopic, 1, 2, _secondSkillSummary);
        TopicUpdateService.moveSkillToSubtopic(_sampleTopic, 2, 1, _secondSkillSummary);
        TopicUpdateService.deleteSubtopic(_sampleTopic, 2);
        expect(UndoRedoService.getCommittableChangeList()).toEqual([{
    ...
    it('should create properly decrement subtopic ids of later subtopics when ' +
        ...
        TopicUpdateService.moveSkillToSubtopic(_sampleTopic, 1, 3, _secondSkillSummary);
        TopicUpdateService.deleteSubtopic(_sampleTopic, 2);
        expect(UndoRedoService.getCommittableChangeList()).toEqual([{
                cmd: 'add_subtopic',
                title: 'Title 3',
    ...
    it('should not create a backend change dict for moving a skill id to a' +
        ...
        expect(function () {
            TopicUpdateService.moveSkillToSubtopic(_sampleTopic, null, 1, _secondSkillSummary);
        }).toThrow();
        expect(function () {
            TopicUpdateService.moveSkillToSubtopic(_sampleTopic, 1, 2, _secondSkillSummary);
        }).toThrow();
    ...
    it('should remove a skill id from a subtopic', function () {
        ...
        TopicUpdateService.removeSkillFromSubtopic(_sampleTopic, 1, _secondSkillSummary);
        expect(_sampleTopic.getUncategorizedSkillSummaries()).toEqual([
            _firstSkillSummary, _secondSkillSummary
        ]);
        expect(_sampleTopic.getSubtopics()[0].getSkillSummaries()).toEqual([]);
    ...
    it('should not create a backend change dict for removing a skill id from a' +
        'subtopic when an error is encountered', function () {
        expect(function () {
            TopicUpdateService.removeSkillFromSubtopic(_sampleTopic, 1, _firstSkillSummary);
        }).toThrow();
        expect(UndoRedoService.getCommittableChangeList()).toEqual([]);
    });
    ...
    it('should set/unset changes to a subtopic page\'s page content', function () {
        ...
        var newSampleSubtitledHtml = SubtitledHtmlObjectFactory.createFromBackendDict(newSampleSubtitledHtmlDict);
        ...
        TopicUpdateService.setSubtopicPageContentsHtml(_sampleSubtopicPage, 1, newSampleSubtitledHtml);
        expect(_sampleSubtopicPage.getPageContents().toBackendDict()).toEqual({
            subtitled_html: {
                html: 'new content',
                content_id: 'content'
    ...
    it('should create a proper backend change dict for changing html data', function () {
        var newSampleSubtitledHtmlDict = {
            html: 'new content',
            content_id: 'content'
        };
    ...
    it('should create a proper backend change dict for changing subtopic ' +
        ...
        var newSampleAudio = ContentIdsToAudioTranslationsObjectFactory
            .createFromBackendDict(newSampleAudioDict);
        TopicUpdateService.setSubtopicPageContentsAudio(_sampleSubtopicPage, 1, newSampleAudio);
        expect(UndoRedoService.getCommittableChangeList()).toEqual([{
                cmd: 'update_subtopic_page_property',
