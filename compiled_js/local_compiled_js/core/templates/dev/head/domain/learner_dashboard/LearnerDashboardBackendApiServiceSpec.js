...
describe('Learner Dashboard Backend API Service', function () {
    ...
    beforeEach(angular.mock.inject(function ($injector) {
        LearnerDashboardBackendApiService = $injector.get('LearnerDashboardBackendApiService');
    ...
    it('should successfully fetch learner dashboard data from the backend', function () {
        var successHandler = jasmine.createSpy('success');
        var failHandler = jasmine.createSpy('fail');
        $httpBackend.expect('GET', LEARNER_DASHBOARD_DATA_URL).respond(sampleDataResults);
        LearnerDashboardBackendApiService.fetchLearnerDashboardData().then(successHandler, failHandler);
        $httpBackend.flush();
    ...
    it('should use rejection handler if learner dashboard data ' +
        'backend request failed', function () {
        var successHandler = jasmine.createSpy('success');
        var failHandler = jasmine.createSpy('fail');
        $httpBackend.expect('GET', LEARNER_DASHBOARD_DATA_URL).respond(ERROR_STATUS_CODE, 'Error loading dashboard data.');
        LearnerDashboardBackendApiService.fetchLearnerDashboardData().then(successHandler, failHandler);
        $httpBackend.flush();
        expect(successHandler).not.toHaveBeenCalled();
        expect(failHandler).toHaveBeenCalledWith(jasmine.objectContaining({ data: 'Error loading dashboard data.' }));
    });
});
