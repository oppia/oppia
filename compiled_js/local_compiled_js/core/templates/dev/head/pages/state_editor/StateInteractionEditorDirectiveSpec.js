...
describe('State Interaction controller', function () {
    describe('StateInteraction', function () {
        ...
        beforeEach(angular.mock.inject(function ($compile, $controller, $injector, $rootScope, $templateCache) {
            scope = $rootScope.$new();
            ...
            var templateHtml = $templateCache.get('/pages/exploration_editor/editor_tab/' +
            ...
            var elem = angular.element('<state-interaction-editor on-save-state-content="saveStateContent" ' +
        ...
        it('should keep non-empty content when setting a terminal interaction', function () {
            ...
            siis.init('First State', state.interaction.id, state.interaction, 'widget_id');
            scas.init('First State', state.interaction.customizationArgs, state.interaction, 'widget_customization_args');
        ...
        it('should change to default text when adding a terminal interaction', function () {
            ...
            siis.init('End State', state.interaction.id, state.interaction, 'widget_id');
            scas.init('End State', state.interaction.customizationArgs, state.interaction, 'widget_customization_args');
            siis.displayed = 'TerminalInteraction';
            directiveScope.onCustomizationModalSavePostHook();
            expect(outerScope.saveInteractionId).toHaveBeenCalled();
            expect(outerScope.saveInteractionCustomizationArgs).toHaveBeenCalled();
