...
describe('State content editor directive', function () {
    ...
    beforeEach(angular.mock.inject(function ($compile, $injector, $rootScope, $templateCache) {
        ...
        outerScope.showMarkAllAudioAsNeedingUpdateModalIfRequired = (jasmine.createSpy(''));
        var elem = angular.element('<state-content-editor ' +
    ...
    it('should correctly handle no-op edits', function () {
        expect(ctrlScope.contentEditorIsOpen).toBe(false);
        expect(scs.savedMemento).toEqual(_getContent('content', 'This is some content.'));
    ...
    it('should check that content edits are saved correctly', function () {
        ...
        ctrlScope.openStateContentEditor();
        scs.displayed = _getContent('content', 'And now for something completely different.');
        ctrlScope.onSaveContentButtonClicked();
        expect(outerScope.saveStateContent).toHaveBeenCalled();
        expect(outerScope.showMarkAllAudioAsNeedingUpdateModalIfRequired)
            .toHaveBeenCalled();
    });
