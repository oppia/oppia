...
oppia.factory('ShowSuggestionModalForEditorViewService', [
    ...
    function ($log, $rootScope, $uibModal, ExplorationDataService, ExplorationStatesService, StateObjectFactory, SuggestionModalService, ThreadDataService, UrlInterpolationService) {
        var _templateUrl = UrlInterpolationService.getDirectiveTemplateUrl('/pages/suggestion_editor/' +
            'editor_view_suggestion_modal_directive.html');
        var _showEditStateContentSuggestionModal = function (activeThread, setActiveThread, isSuggestionHandled, hasUnsavedChanges, isSuggestionValid) {
            $uibModal.open({
                ...
                ThreadDataService.resolveSuggestion(activeThread.threadId, result.action, result.commitMessage, result.reviewMessage, result.audioUpdateRequired, function () {
                    ...
                    if (result.action === SuggestionModalService.ACTION_ACCEPT_SUGGESTION) {
                        ...
                        var state = StateObjectFactory.createFromBackendDict(stateName, stateDict);
                        state.content.setHtml(activeThread.getReplacementHtmlFromSuggestion());
                        if (result.audioUpdateRequired) {
                            state.contentIdsToAudioTranslations.markAllAudioAsNeedingUpdate(state.content.getContentId());
                        }
        ...
        return {
            showSuggestionModal: function (suggestionType, extraParams) {
                if (suggestionType === 'edit_exploration_state_content') {
                    _showEditStateContentSuggestionModal(extraParams.activeThread, extraParams.setActiveThread, extraParams.isSuggestionHandled, extraParams.hasUnsavedChanges, extraParams.isSuggestionValid);
                }
            }
        };
    }
