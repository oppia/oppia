...
oppia.controller('HistoryTab', [
    ...
    function ($http, $log, $rootScope, $scope, $uibModal, CompareVersionsService, DateTimeFormatService, ExplorationDataService, UrlInterpolationService, VersionTreeService) {
                ...
                $http.get($scope.explorationAllSnapshotsUrl).then(function (response) {
                    ...
                    for (var i = currentVersion - 1; i >= lowestVersionIndex; i--) {
                        ...
                        $scope.explorationVersionMetadata[versionNumber] = {
                            ...
                            createdOnStr: (DateTimeFormatService.getLocaleAbbreviatedDatetimeString(explorationSnapshots[i].created_on_ms)),
        ...
        var getVersionHeader = function (versionMetadata) {
            return ('Revision #' + versionMetadata.versionNumber +
        ...
        $scope.changeCompareVersion = function () {
            ...
            CompareVersionsService.getDiffGraphData(earlierComparedVersion, laterComparedVersion).then(function (response) {
                $log.info('Retrieved version comparison data');
                $log.info(response);
                $scope.diffData = response;
                $scope.earlierVersionHeader = getVersionHeader($scope.compareVersionMetadata.earlierVersion);
                $scope.laterVersionHeader = getVersionHeader($scope.compareVersionMetadata.laterVersion);
            });
        };
        // Check if valid versions were selected
        $scope.areCompareVersionsSelected = function () {
            return ($scope.compareVersions && $scope.selectedVersionsArray.length === 2);
        };
        ...
        $scope.showRevertExplorationModal = function (version) {
            $uibModal.open({
                templateUrl: UrlInterpolationService.getDirectiveTemplateUrl('/pages/exploration_editor/history_tab/' +
                ...
                controller: [
                    ...
                    function ($scope, $uibModalInstance, version, ExplorationDataService) {
                        ...
                        $scope.getExplorationUrl = function (version) {
                            return ('/explore/' + ExplorationDataService.explorationId +
                                '?v=' + version);
                        };
        ...
        $scope.computeVersionsToDisplay = function () {
            ...
            var end = Math.min(begin + $scope.VERSIONS_PER_PAGE, $scope.versionCheckboxArray.length);
            $scope.versionNumbersToDisplay = [];
