    ...
    'UrlInterpolationService', function (UrlInterpolationService) {
        return {
            ...
            controller: [
                ...
                'SUPPORTED_AUDIO_LANGUAGES', function ($rootScope, $scope, $timeout, $window, ExplorationLanguageCodeService, LanguageUtilService, TranslationLanguageService, TranslationStatusService, TranslationTabActiveModeService, DEFAULT_AUDIO_LANGUAGE, SUPPORTED_AUDIO_LANGUAGES) {
                    ...
                    $scope.canShowTabModeSwitcher = function () {
                        return ($scope.languageCode !== (ExplorationLanguageCodeService.displayed));
                    };
                    $scope.canShowModeSwitchHelperImage = function () {
                        var alreadyShown = $window.localStorage.getItem('mode_switch_helper_image_shown');
                        if (alreadyShown !== 'true') {
                            $timeout(function () {
                                $window.localStorage.setItem('mode_switch_helper_image_shown', true);
                            }, 15000);
                            return true;
                        }
                        else {
                    ...
                    var refreshDirectiveScope = function () {
                        $scope.inTranslationMode = (TranslationTabActiveModeService.isTranslationModeActive());
                        $scope.inVoiceoverMode = (TranslationTabActiveModeService.isVoiceoverModeActive());
                        allAudioLanguageCodes = (LanguageUtilService.getAllVoiceoverLanguageCodes());
                        if ($scope.inTranslationMode) {
                            var index = allAudioLanguageCodes.indexOf(ExplorationLanguageCodeService.displayed);
                            if (index !== -1) {
