...
oppia.factory('TranslationStatusService', [
    ...
    'TranslationTabActiveModeService', 'INTERACTION_SPECS', function (ExplorationStatesService, StateRecordedVoiceoversService, StateWrittenTranslationsService, TranslationLanguageService, TranslationTabActiveModeService, INTERACTION_SPECS) {
        ...
        var _getVoiceOverStatus = function (recordedVoiceovers, contentId) {
            ...
            var availableLanguages = recordedVoiceovers.getVoiceoverLanguageCodes(contentId);
            if (availableLanguages.indexOf(langCode) !== -1) {
                availabilityStatus.available = true;
                var audioTranslation = recordedVoiceovers.getVoiceover(contentId, langCode);
        ...
        var _getTranslationStatus = function (writtenTranslations, contentId) {
            ...
            var availableLanguages = (writtenTranslations.getTranslationsLanguageCodes(contentId));
            if (availableLanguages.indexOf(langCode) !== -1) {
                var writtenTranslation = (writtenTranslations.getWrittenTranslation(contentId, langCode));
        ...
        var _getContentAvailabilityStatus = function (stateName, contentId) {
            ...
            if (TranslationTabActiveModeService.isTranslationModeActive()) {
                var writtenTranslations = (ExplorationStatesService.getWrittenTranslationsMemento(stateName));
                return _getTranslationStatus(writtenTranslations, contentId);
            }
            else if (TranslationTabActiveModeService.isVoiceoverModeActive()) {
                var recordedVoiceovers = (ExplorationStatesService.getRecordedVoiceoversMemento(stateName));
        ...
        var _computeAllStatesStatus = function () {
            ...
            if (ExplorationStatesService.isInitialized()) {
                ExplorationStatesService.getStateNames().forEach(function (stateName) {
                    ...
                    var interactionId = ExplorationStatesService.getInteractionIdMemento(stateName);
                    ...
                    allContentId.forEach(function (contentId) {
                        var availabilityStatus = _getContentAvailabilityStatus(stateName, contentId);
                        if (!availabilityStatus.available) {
                            noTranslationCount++;
                        }
                        if (availabilityStatus.needsUpdate) {
                            if (TranslationTabActiveModeService.isTranslationModeActive()) {
                                stateNeedsUpdateWarnings[stateName] = (TRANSLATION_NEEDS_UPDATE_MESSAGE);
                            }
                            else {
                                stateNeedsUpdateWarnings[stateName] = (AUDIO_NEEDS_UPDATE_MESSAGE);
                            }
                        }
                    });
        ...
        var _getActiveStateComponentStatus = function (componentName) {
            ...
            if (contentIdList) {
                contentIdList.forEach(function (contentId) {
                    var availabilityStatus = _getActiveStateContentAvailabilityStatus(contentId);
                    if (availabilityStatus.available) {
                        availableAudioCount++;
                    }
                });
