...
oppia.factory('ExplorationRightsService', [
    ...
    function ($http, AlertsService, ExplorationDataService, ACTIVITY_STATUS_PRIVATE, ACTIVITY_STATUS_PUBLIC) {
        return {
            init: function (ownerNames, editorNames, voiceArtistNames, viewerNames, status, clonedFrom, isCommunityOwned, viewableIfPrivate) {
            ...
            makeCommunityOwned: function () {
                var that = this;
                var requestUrl = ('/createhandler/rights/' + ExplorationDataService.explorationId);
                return $http.put(requestUrl, {
                    ...
                    that.init(data.rights.owner_names, data.rights.editor_names, data.rights.voice_artist_names, data.rights.viewer_names, data.rights.status, data.rights.cloned_from, data.rights.community_owned, data.rights.viewable_if_private);
                });
            },
            ...
            publish: function () {
                var that = this;
                var requestUrl = ('/createhandler/status/' + ExplorationDataService.explorationId);
                return $http.put(requestUrl, {
                    make_public: true
                }).then(function (response) {
                    var data = response.data;
            ...
            saveModeratorChangeToBackend: function (emailBody) {
                var that = this;
                var explorationModeratorRightsUrl = ('/createhandler/moderatorrights/' +
