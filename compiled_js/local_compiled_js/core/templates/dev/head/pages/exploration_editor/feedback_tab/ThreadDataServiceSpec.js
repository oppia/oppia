...
describe('retrieving threads service', function () {
    ...
    it('should retrieve feedback threads', function () {
        ...
        httpBackend.whenGET('/generalsuggestionlisthandler?target_type=exploration' +
        ...
        ThreadDataService.fetchThreads(function () {
            for (var i = 0; i < mockFeedbackThreads.length; i++) {
                expect(ThreadDataService.data.feedbackThreads).toContain(mockFeedbackThreads[i]);
            }
            for (var i = 0; i < mockGeneralSuggestionThreads.length; i++) {
                expect(ThreadDataService.data.suggestionThreads).toContain(mockGeneralSuggestionThreads[i]);
            }
        });
        httpBackend.flush();
    });
