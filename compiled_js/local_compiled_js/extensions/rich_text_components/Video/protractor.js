...
var customizeComponent = function (modal, youtubeId, startTime, endTime, ifAutoplay) {
    objects.UnicodeStringEditor(modal.element(by.tagName('schema-based-unicode-editor'))).setValue(youtubeId);
    objects.IntEditor(modal.all(by.tagName('schema-based-int-editor')).first()).setValue(startTime);
    objects.IntEditor(modal.all(by.tagName('schema-based-int-editor')).last()).setValue(endTime);
    objects.BooleanEditor(modal.element(by.tagName('schema-based-bool-editor'))).setValue(ifAutoplay);
};
