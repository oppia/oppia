...
logicDemo.controller('LogicDemoTestController', ['$scope', function ($scope) {
        ...
        $scope.displayMessage = function (message, line) {
            ...
            while (pointer < message.length) {
                ...
                var breakPoint = ((nextSpace <= 0 || pointer + 70 >= message.length) ? 70 :
                    nextSpace + 1);
        ...
        $scope.submitProof = function () {
            var questionInstance = logicProofStudent.buildInstance($scope.questionData);
            try {
                var proof = logicProofStudent.buildProof($scope.proofString, questionInstance);
            ...
            for (var i = 0; i < questionInstance.mistake_table.length; i++) {
                for (var j = 0; j < questionInstance.mistake_table[i].entries.length; j++) {
                    ...
                    if (mistake.name === $scope.mistakeName) {
                        $scope.localCheck = logicProofStudent.evaluate(mistake.occurs, {
        ...
        $scope.submitMistakes = function (sectionNumber) {
            ...
            $scope.buildSuccess = false;
            try {
                $scope.questionData.mistake_table[sectionNumber] = (logicProofTeacher2.buildMistakeSection($scope.mistakeStrings[sectionNumber].name, $scope.mistakeStrings[sectionNumber].entries, $scope.questionData.control_functions));
        ...
        $scope.replaceUnicode = function (input) {
            ...
            for (var i = 0; i < $scope.REPLACEMENT_PAIRS.length; i++) {
                ...
                output = output.split($scope.REPLACEMENT_PAIRS[i].old).join($scope.REPLACEMENT_PAIRS[i]['new']);
            }
            return output;
        };
        ...
        $scope.requestJavascript = function () {
            if ($scope.questionSuccess && $scope.lineTemplateSuccess &&
                ...
                var docStart = 'LOGIC_PROOF_DEFAULT_QUESTION_DATA = {' +
                    ...
                    'language: logicProofData.BASE_STUDENT_LANGUAGE,' +
                    'general_messages: logicProofData.BASE_GENERAL_MESSAGES,';
                document.write(docStart + $scope.replaceUnicode(JSON.stringify({
