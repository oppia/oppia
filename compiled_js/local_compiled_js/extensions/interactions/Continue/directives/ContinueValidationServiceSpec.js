...
describe('ContinueValidationService', function () {
    ...
    it('should expect a non-empty button text customization argument', function () {
        var warnings = validatorService.getAllWarnings(currentState, customizationArguments, [], goodDefaultOutcome);
        expect(warnings).toEqual([]);
        customizationArguments.buttonText.value = '';
        warnings = validatorService.getAllWarnings(currentState, customizationArguments, [], goodDefaultOutcome);
        expect(warnings).toEqual([{
                type: WARNING_TYPES.CRITICAL,
                message: 'The button text should not be empty.'
            }]);
        expect(function () {
            validatorService.getAllWarnings(currentState, {}, [], goodDefaultOutcome);
        }).toThrow('Expected customization arguments to have property: buttonText');
    });
