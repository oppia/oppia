...
describe('Text Input Prediction Service', function () {
    ...
    describe('Test text prediction service', function () {
        ...
        it('should predict the same as oppia-ml', function () {
            var classifierData = window.__fixtures__['core/tests/data/text_input_classifier_data'];
            var trainingData = window.__fixtures__['core/tests/data/text_classifier_results'];
            var correctPredictions = 0, totalAnswers = 0;
            var predictedAnswerGroup = null;
            for (var i = 0; i < trainingData.length; i++) {
                for (var j = 0; j < trainingData[i].answers.length; j++) {
                    predictedAnswerGroup = predictionService.predict(classifierData, trainingData[i].answers[j]);
        ...
        it('should not have accuracy less than 85', function () {
            ...
            var classifierData = window.__fixtures__['core/tests/data/text_input_classifier_data'];
            var trainingData = window.__fixtures__['core/tests/data/text_input_training_data'];
            var correctPredictions = 0, totalAnswers = 0;
