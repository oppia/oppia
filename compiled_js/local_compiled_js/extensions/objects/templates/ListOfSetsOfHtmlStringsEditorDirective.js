    ...
    function (UrlInterpolationService, OBJECT_EDITOR_URL_PREFIX) {
        return {
            ...
            controller: ['$scope', function ($scope) {
                    ...
                    if ($scope.selectedRank !== '') {
                        $scope.maxPrevIndex = math.max(parseInt($scope.selectedRank), $scope.maxPrevIndex);
                    }
                    $scope.allowedChoices = function () {
                        var allowedList = [];
                        for (var i = 0; i <= math.min($scope.maxPrevIndex, $scope.choices.length - 1); i++) {
                    ...
                    $scope.selectedItem = function (choiceListIndex, selectedRankString) {
                        ...
                        $scope.maxPrevIndex = math.max(selectedRank + 1, $scope.maxPrevIndex);
