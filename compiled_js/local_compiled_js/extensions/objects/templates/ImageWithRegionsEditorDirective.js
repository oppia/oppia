            ...
            controller: [
                ...
                function ($scope, $element, $uibModal) {
                    ...
                    $scope.getRegionLabelEditorStyle = function () {
                        ...
                        var area = cornerAndDimensionsFromRegionArea($scope.value.labeledRegions[$scope.selectedRegion].region.area);
                    ...
                    var _calculateImageDimensions = function () {
                        ...
                        // zero.
                        var displayedImageHeight = ($scope.originalImageWidth === 0 ? 0.0 :
                    ...
                    $scope.$watch('value.imagePath', function (newVal) {
                        if (newVal !== '') {
                            ...
                            // width and height.
                            $('<img/>').attr('src', $scope.getPreviewUrl(newVal)).on('load', function () {
                    ...
                    $scope.regionLabelGetterSetter = function (index) {
                        return function (label) {
                            if (angular.isDefined(label)) {
                                ...
                                var labels = $scope.value.labeledRegions.map(function (region) {
                                    return region.label;
                                });
                    ...
                    var regionAreaFromCornerAndDimensions = function (x, y, width, height) {
                        return [
                            convertCoordsToFraction([x, y], [$scope.getImageWidth(), $scope.getImageHeight()]),
                            convertCoordsToFraction([x + width, y + height], [$scope.getImageWidth(), $scope.getImageHeight()])
                        ];
                    };
                    ...
                    var resizeRegion = function () {
                        ...
                        // rectangle.
                        var marginX = Math.abs($scope.originalRectArea.x - $scope.originalMouseX);
                        // The distance between where the mouse was first clicked to
                        // initiate the resize action and the top-most y co-ordinate of
                        // rectangle.
                        var marginY = Math.abs($scope.originalRectArea.y - $scope.originalMouseY);
                    ...
                    $scope.onSvgMouseMove = function (evt) {
                        ...
                        if ($scope.userIsCurrentlyDrawing) {
                            ...
                            draggedRegion.area = regionAreaFromCornerAndDimensions(newX1, newY1, $scope.originalRectArea.width, $scope.originalRectArea.height);
                        }
                            ...
                            if ($scope.rectWidth !== 0 && $scope.rectHeight !== 0) {
                                ...
                                var newRegion = {
                                    ...
                                    region: {
                                        ...
                                        area: regionAreaFromCornerAndDimensions($scope.rectX, $scope.rectY, $scope.rectWidth, $scope.rectHeight)
                                    }
                                };
                                $scope.value.labeledRegions.push(newRegion);
                                $scope.selectedRegion = ($scope.value.labeledRegions.length - 1);
                            }
                        }
                        $scope.userIsCurrentlyDrawing = false;
                        $scope.userIsCurrentlyDragging = false;
                    ...
                    $scope.onMouseMoveRegion = function () {
                        ...
                        var region = cornerAndDimensionsFromRegionArea($scope.value.labeledRegions[$scope.hoveredRegion].region.area);
                    ...
                    $scope.onMousedownRegion = function () {
                        ...
                        $scope.originalRectArea = cornerAndDimensionsFromRegionArea($scope.value.labeledRegions[$scope.hoveredRegion].region.area);
                    };
                    ...
                    $scope.getCursorStyle = function () {
                        ...
                        if ($scope.xDirection || $scope.yDirection) {
                            ...
                            else if (($scope.xDirection === -1 && !$scope.xDirectionToggled) ||
                                ($scope.xDirection === 1 && $scope.xDirectionToggled)) {
                                xDirectionCursor = 'e';
                            }
                            else {
                            ...
                            else if (($scope.yDirection === -1 && !$scope.yDirectionToggled) ||
                                ($scope.yDirection === 1 && $scope.yDirectionToggled)) {
                                yDirectionCursor = 's';
                            }
                            else {
