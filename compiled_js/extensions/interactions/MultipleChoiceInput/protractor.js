...
var expectInteractionDetailsToMatch = function (elem, richTextInstructionsArray) {
    elem.all(protractor_1.by.repeater('choice in choices track by $index'))
        .then(function (optionElements) {
        expect(optionElements.length).toEqual(richTextInstructionsArray.length);
        for (var i = 0; i < optionElements.length; i++) {
            forms.expectRichText(optionElements[i].element(protractor_1.by.css('.protractor-test-multiple-choice-option'))).toMatch(richTextInstructionsArray[i]);
        }
    });
};
...
var submitAnswer = function (elem, answer) {
    elem.element(protractor_1.by.tagName('oppia-interactive-multiple-choice-input')).
        element(protractor_1.by.buttonText(answer)).click();
};
