...
describe('Full system', function () {
    var completeCheck = function (assumptionsString, targetString, proofString) {
        var questionInstance = logicProofStudent.buildInstance(LOGIC_PROOF_DEFAULT_QUESTION_DATA);
        var question = logicProofTeacher.buildQuestion(assumptionsString, targetString, questionInstance.vocabulary);
    ...
    it('should accept fully correct proofs', function () {
        ...
        // issues are fixed.
        expect(completeCheck('∀x.(A(x)∧B(x))', '(∀x.A(x))∧(∀x.B(x))', [
            'given c',
    ...
    it('should reject proofs with any error', function () {
        ...
        }).toThrow(sharedErrorWrapper('The phrase starting \'we\' could not be identified; please ' +
        ...
        }).toThrow(sharedErrorWrapper('This line uses q, so you need to have an earlier line proving ' +
        ...
        }).toThrow(sharedErrorWrapper('We are trying to prove ∀x.A(x) so it should be given by the ' +
        ...
        }).toThrow(sharedErrorWrapper('You haven\'t said where d comes from; if you want it to be ' +
