...
var logicProofTeacher = (function () {
    ...
    var buildQuestion = function (assumptionsString, targetString, vocabulary) {
        if (assumptionsString.replace(/ /g, '') === '') {
            ...
            try {
                var assumptions = logicProofParser.parse(assumptionsString.replace(/ /g, ''), 'listOfExpressions');
            }
            catch (err) {
                ...
                throw {
                    message: logicProofShared.renderError(error, TEACHER_ERROR_MESSAGES, logicProofData.BASE_STUDENT_LANGUAGE)
                };
            }
        }
        try {
            var target = logicProofParser.parse(targetString.replace(/ /g, ''), 'expression');
        }
        catch (err) {
            var error = new logicProofShared.UserError('unparseable', {
                field: 'target'
        ...
        topTypes.push('boolean');
        try {
            var typing = logicProofShared.assignTypesToExpressionArray(expressions, topTypes, logicProofData.BASE_STUDENT_LANGUAGE, ['variable', 'constant', 'prefix_function']);
            if (typing.length > 1) {
                throw new logicProofShared.UserError('ambiguous_typing', {});
            }
            requireNoVocabularyWordsUsed(expressions, logicProofData.BASE_STUDENT_LANGUAGE.operators, vocabulary);
        }
        catch (err) {
            throw {
                message: logicProofShared.renderError(err, TEACHER_ERROR_MESSAGES, logicProofData.BASE_STUDENT_LANGUAGE)
            };
        }
        return {
    ...
    var requireNoVocabularyWordsUsed = function (expressionArray, knownOperators, vocabulary) {
        ...
        var operatorNamesToCheck = logicProofShared.getOperatorsFromExpressionArray(expressionArray);
