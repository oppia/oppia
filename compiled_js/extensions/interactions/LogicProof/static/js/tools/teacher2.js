...
var logicProofTeacher2 = (function () {
    ...
    var requireValidLineTemplate = function (lineTemplate, language, vocabulary) {
        ...
        // 2. Type checking
        var typeCheck = logicProofShared.assignTypesToExpressionArray(checkees, types, language, ['variable', 'atom'], true);
        if (typeCheck.length > 1) {
            throw new logicProofShared.UserError('ambiguous_typing', {});
        }
        // 3. Matchability checking
        var visibleOperators = logicProofShared.getOperatorsFromExpressionArray(visible);
        var hiddenOperators = logicProofShared.getOperatorsFromExpressionArray(invisible);
    ...
    var buildLineTemplate = function (nameString, readerViewString, antecedentsString, resultsString, variablesString, errorStrings, language, vocabulary) {
        var possibleReaderViews = logicProofShared.parseLineString(readerViewString, language.operators, vocabulary, true);
        ...
        try {
            var antecedents = logicProofParser.parse(antecedentsString.replace(/ /g, ''), 'listOfBooleanTemplates');
        }
        catch (err) {
        ...
        try {
            var results = logicProofParser.parse(resultsString.replace(/ /g, ''), 'listOfBooleanTemplates');
        }
        catch (err) {
            throw new logicProofShared.UserError('unparseable', {
                field: 'results'
        ...
        try {
            var variables = logicProofParser.parse(variablesString.replace(/ /g, ''), 'listOfVariables');
        }
        catch (err) {
            throw new logicProofShared.UserError('unparseable', {
                field: 'variables'
    ...
    var displayExpressionTemplate = function (template, operators) {
        var output = logicProofShared.displayExpression(template.expression, operators);
        for (var i = 0; i < template.substitutions.length; i++) {
            var newOutput = [];
            for (var key in template.substitutions[i]) {
                newOutput.push(key + '->' + logicProofShared.displayExpression(template.substitutions[i][key], operators));
            }
    ...
    var displayLineTemplate = function (template, operators, vocabulary) {
        ...
        return {
            ...
            variables: logicProofShared.displayExpressionArray(template.variables, operators),
    ...
    var buildLineTemplateTable = function (stringTable, vocabulary) {
        ...
        for (var i = 0; i < stringTable.length; i++) {
            try {
                table.push(buildLineTemplate(stringTable[i].name, stringTable[i].reader_view, stringTable[i].antecedents, stringTable[i].results, stringTable[i].variables, stringTable[i].error, logicProofData.BASE_STUDENT_LANGUAGE, vocabulary));
                failures.push('');
            }
            catch (err) {
                failed = true;
                failures.push(logicProofShared.renderError(err, logicProofTeacher.TEACHER_ERROR_MESSAGES, logicProofData.BASE_STUDENT_LANGUAGE));
            }
        }
        if (failed) {
            throw failures;
    ...
    var displayLineTemplateTable = function (table, vocabulary) {
        ...
        for (var i = 0; i < table.length; i++) {
            output.push(displayLineTemplate(table[i], logicProofData.BASE_STUDENT_LANGUAGE.operators, vocabulary));
        }
        return output;
    };
    // MISTAKE TABLE
    ...
    var validateAndTypeMistakeEntry = function (mistakeEntry, language) {
        ...
        var typeCheck = logicProofShared.assignTypesToExpression(mistakeEntry.occurs, ['boolean'], newLanguage, ['constant']);
        if (typeCheck.length > 1) {
            throw new logicProofShared.UserError('ambiguous_typing', {});
        }
        ...
        for (var i = 0; i < mistakeEntry.message.length; i++) {
            for (var j = 0; j < mistakeEntry.message[i].length; j++) {
                if (mistakeEntry.message[i][j].format === 'expression') {
                    ...
                    var typeCheck = logicProofShared.assignTypesToExpression(expression, ['integer', 'string', 'formula', 'set_of_formulas', 'boolean'], newLanguage, ['constant']);
                    if (typeCheck.length > 1) {
                        throw new logicProofShared.UserError('ambiguous_typing', {});
                    }
    ...
    var buildMistakeEntry = function (nameString, occursString, messageStrings, controlLanguage) {
        try {
            var occurs = logicProofParser.parse(occursString.replace(/ /g, ''), 'expression');
        }
        catch (err) {
            throw new logicProofShared.UserError('unparseable', {
    ...
    var displayMistakeEntry = function (mistakeEntry, operators) {
        ...
        return {
            ...
            occurs: logicProofShared.displayExpression(mistakeEntry.occurs, operators),
    ...
    var buildMistakeSection = function (name, entryStrings, controlFunctions) {
        ...
        for (var i = 0; i < entryStrings.length; i++) {
            try {
                ...
                failed = true;
                failures.push(logicProofShared.renderError(err, logicProofTeacher.TEACHER_ERROR_MESSAGES, controlLanguage));
            }
        }
        if (failed) {
            throw failures;
    ...
    var displayMistakeTable = function (mistakeTable) {
        ...
        for (var i = 0; i < mistakeTable.length; i++) {
            displayedSections.push(displayMistakeSection(mistakeTable[i], logicProofData.BASE_CONTROL_LANGUAGE.operators));
        }
    ...
    var validateAndTypeControlFunction = function (formulaLHS, formulaRHS, language) {
        ...
        for (var i = 0; i < formulaLHS.arguments.length; i++) {
            if (language.operators.hasOwnProperty(formulaLHS.arguments[i].top_operator_name)) {
                throw new logicProofShared.UserError('argument_is_function_name', {
                ...
                throw new logicProofShared.UserError('duplicate_argument', {
                    argument: formulaLHS.arguments[i].top_operator_name
                });
            }
            else if (logicProofShared.getOperatorsFromExpression(formulaRHS).indexOf(formulaLHS.arguments[i].top_operator_name) === -1) {
                throw new logicProofShared.UserError('unused_argument', {
                    argument: formulaLHS.arguments[i].top_operator_name
                });
            }
            else {
        ...
        for (var i = 0; i < formulaLHS.arguments.length; i++) {
            argumentTypes.push({
                type: logicProofShared.seekTypeInExpression(typeCheck[0].typedExpression, formulaLHS.arguments[i].top_operator_name),
    ...
    var buildControlFunction = function (LHSstring, RHSstring, descriptionString, controlLanguage) {
        try {
            var formulaLHS = logicProofParser.parse(LHSstring.replace(/ /g, ''), 'formulaLHS');
        }
        catch (err) {
            throw new logicProofShared.UserError('unparseable', {
        ...
        try {
            var formulaRHS = logicProofParser.parse(RHSstring.replace(/ /g, ''), 'expression');
        }
        catch (err) {
            throw new logicProofShared.UserError('unparseable', {
    ...
    var displayControlFunction = function (controlFunction, operators) {
        ...
        return {
            ...
            RHS: logicProofShared.displayExpression(controlFunction.definition, operators),
    ...
    var buildControlFunctionTable = function (controlFunctionStrings) {
        ...
        for (var i = 0; i < controlFunctionStrings.length; i++) {
            try {
                var built = buildControlFunction(controlFunctionStrings[i].LHS, controlFunctionStrings[i].RHS, controlFunctionStrings[i].description, controlLanguage);
            }
            catch (err) {
                throw {
                    message: logicProofShared.renderError(err, logicProofTeacher.TEACHER_ERROR_MESSAGES, logicProofData.BASE_CONTROL_LANGUAGE),
                    line: i
                };
            }
    ...
    var displayControlFunctionTable = function (table) {
        var displayedEntries = [];
        for (var i = 0; i < table.length; i++) {
            displayedEntries.push(displayControlFunction(table[i], logicProofData.BASE_CONTROL_LANGUAGE.operators));
        }
            ...
            try {
                return {
                    ...
                    content: (typeOfMessage === 'line') ?
                        logicProofParser.parse(fragmentString.replace(/ /g, ''), 'expressionTemplate2') :
                        logicProofParser.parse(fragmentString.slice(2, fragmentString.length - 2).replace(/ /g, ''), 'expression')
                };
            }
            catch (err) {
    ...
    var requireNoWordsUsed = function (expressionArray, knownOperators, vocabulary) {
        ...
        var operatorsToCheck = logicProofShared.getOperatorsFromExpressionArray(expressionArray);
