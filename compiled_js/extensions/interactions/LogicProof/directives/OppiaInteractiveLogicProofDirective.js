        ...
        return {
            ...
            controller: [
                ...
                function ($scope, $attrs, $uibModal, logicProofRulesService, WindowDimensionsService, UrlService, CurrentInteractionService) {
                    $scope.localQuestionData = HtmlEscaperService.escapedJsonToObj($attrs.questionWithValue);
                    ...
                    $scope.topTypes.push('boolean');
                    $scope.typing = logicProofShared.assignTypesToExpressionArray($scope.expressions, $scope.topTypes, logicProofData.BASE_STUDENT_LANGUAGE, ['variable', 'constant', 'prefix_function']);
                    $scope.questionData.language.operators = $scope.typing[0].operators;
                    if ($scope.questionData.assumptions.length <= 1) {
                        $scope.assumptionsString = logicProofShared.displayExpressionArray($scope.questionData.assumptions, $scope.questionData.language.operators);
                    }
                    else {
                        $scope.assumptionsString = logicProofShared.displayExpressionArray($scope.questionData.assumptions.slice(0, $scope.questionData.assumptions.length - 1), $scope.questionData.language.operators) + ' and ' + logicProofShared.displayExpression($scope.questionData.assumptions[$scope.questionData.assumptions.length - 1], $scope.questionData.language.operators);
                    }
                    $scope.targetString = logicProofShared.displayExpression($scope.questionData.results[0], $scope.questionData.language.operators);
                    ...
                    $scope.codeEditor = function (editor) {
                        ...
                        editor.on('beforeChange', function (instance, change) {
                            var convertedText = logicProofConversion.convertToLogicCharacters(change.text.join('\n'));
                    ...
                    $scope.showMessage = function (message, lineNum) {
                        $scope.errorMessage = $scope.constructDisplayedMessage(message, lineNum);
                    ...
                    CurrentInteractionService.registerCurrentInteraction($scope.submitProof, null);
