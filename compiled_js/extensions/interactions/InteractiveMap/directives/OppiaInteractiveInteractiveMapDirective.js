...
oppia.directive('oppiaInteractiveInteractiveMap', [
    ...
    function (HtmlEscaperService, UrlInterpolationService, interactiveMapRulesService, EVENT_NEW_CARD_AVAILABLE) {
        return {
            restrict: 'E',
            scope: {
            ...
            controller: [
                ...
                function ($scope, $attrs, $timeout, CurrentInteractionService) {
                        ...
                        $timeout(function () {
                            ...
                            if (!$scope.interactionIsActive) {
                                $scope.mapMarkers.push(new google.maps.Marker({
                                    map: $scope.map,
                                    position: new google.maps.LatLng($scope.getLastAnswer()[0], $scope.getLastAnswer()[1])
                                }));
                            }
                        }, 100);
                    };
                    ...
                    $scope.registerClick = function ($event, $params) {
                        ...
                        CurrentInteractionService.onSubmit([ll.lat(), ll.lng()], interactiveMapRulesService);
                    };
                    refreshMap();
                }
            ]
