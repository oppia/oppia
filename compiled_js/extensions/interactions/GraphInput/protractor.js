...
var customizeInteraction = function (interactionEditor, graphDict) {
    var graphInputContainer = interactionEditor.element(protractor_1.by.css('.protractor-test-graph-input-viz-container'));
    if (graphDict) {
...
var submitAnswer = function (conversationInput, graphDict) {
    ...
    // new nodes.
    var graphInputContainer = conversationInput.element(protractor_1.by.css('.protractor-test-graph-input-viz-container'));
    if (graphDict) {
        forms.GraphEditor(graphInputContainer).setValue(graphDict);
    }
    var submitAnswerButton = protractor_1.element(protractor_1.by.css('.protractor-test-submit-answer-button'));
