            ...
            controller: [
                ...
                function ($scope, $element, $attrs, $document, $timeout, FocusManagerService, graphDetailService, GRAPH_INPUT_LEFT_MARGIN, EVENT_NEW_CARD_AVAILABLE, DeviceInfoService) {
                    ...
                    var initViewboxSize = function () {
                        ...
                        $scope.svgViewBox = (0 + ' ' + 0 + ' ' + (boundingBox.width + boundingBox.x) +
                            ' ' + (viewBoxHeight));
                    };
                    $scope.graphOptions = [{
                            text: 'Labeled',
                    ...
                    $scope.onTouchFinalVertex = function (index) {
                        if ($scope.state.currentMode === _MODES.ADD_EDGE) {
                            tryAddEdge($scope.state.addEdgeVertex, index);
                            endAddEdge();
                            $scope.state.hoveredVertex = null;
                    ...
                    $scope.onMouseleaveVertex = function (index) {
                        ...
                        $scope.state.hoveredVertex = (index === $scope.state.hoveredVertex) ?
                    ...
                    var beginEditEdgeWeight = function (index) {
                        ...
                        $scope.selectedEdgeWeightValue = ($scope.graph.edges[$scope.state.selectedEdge].weight);
                        $scope.shouldShowWrongWeightWarning = false;
                    ...
                    $scope.onUpdateEdgeWeight = function () {
                        if (angular.isNumber($scope.selectedEdgeWeightValue)) {
                            $scope.graph.edges[$scope.state.selectedEdge].weight = ($scope.selectedEdgeWeightValue);
                        }
                        $scope.state.selectedEdge = null;
                    };
                    // Styling functions
