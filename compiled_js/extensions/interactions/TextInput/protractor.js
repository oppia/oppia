...
var customizeInteraction = function (elem, placeholderText, heightOfBox) {
    objects.UnicodeStringEditor(elem.element(by.tagName('schema-based-unicode-editor'))).setValue(placeholderText);
    objects.IntEditor(elem.element(by.tagName('schema-based-int-editor'))).setValue(heightOfBox);
};
var expectInteractionDetailsToMatch = function (elem, placeholderText, heightOfBox) {
    ...
    expect(elem.element(by.tagName('oppia-interactive-text-input')).isPresent()).toBe(true);
};
