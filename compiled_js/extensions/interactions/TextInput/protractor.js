...
var customizeInteraction = function (elem, placeholderText, heightOfBox) {
    objects.UnicodeStringEditor(elem.element(protractor_1.by.tagName('schema-based-unicode-editor'))).setValue(placeholderText);
    objects.IntEditor(elem.element(protractor_1.by.tagName('schema-based-int-editor'))).setValue(heightOfBox);
};
var expectInteractionDetailsToMatch = function (elem, placeholderText, heightOfBox) {
    if (placeholderText) {
        expect((protractor_1.element(protractor_1.by.tagName('oppia-interactive-text-input'))
    ...
    expect(elem.element(protractor_1.by.tagName('oppia-interactive-text-input')).isPresent()).toBe(true);
};
var submitAnswer = function (elem, answer) {
    ...
    var textInputElem = elem.element(protractor_1.by.tagName('oppia-interactive-text-input'));
    var textAreaElem = textInputElem.element(protractor_1.by.tagName('textarea'));
    var inputElem = textInputElem.element(protractor_1.by.tagName('input'));
    textAreaElem.isPresent().then(function (present) {
        if (present) {
            textAreaElem.sendKeys(answer);
            protractor_1.element(protractor_1.by.css('.protractor-test-submit-answer-button')).click();
        }
        else {
