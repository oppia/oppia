...
var customizeInteraction = function (elem, recommendedExplorationIdArray) {
    if (recommendedExplorationIdArray) {
        ...
        recommendedExplorationIdArray.forEach(function (explorationId) {
            var addExplorationIdButton = protractor_1.element(protractor_1.by.css('.protractor-test-add-list-entry'));
            addExplorationIdButton.click();
            objects.UnicodeStringEditor(elem.element(protractor_1.by.tagName('schema-based-unicode-editor'))).setValue(explorationId);
        });
    }
};
var expectInteractionDetailsToMatch = function (elem) {
    expect(elem.element(protractor_1.by.tagName('oppia-interactive-end-exploration')).isPresent()).toBe(true);
};
var testSuite = [];
