...
var BooleanEditor = function (elem) {
    return {
        setValue: function (value) {
            elem.element(protractor_1.by.tagName('input')).isSelected().then(function (currentValue) {
                if (value !== currentValue) {
                    elem.element(protractor_1.by.tagName('input')).click();
                }
            });
        }
    };
...
var CoordTwoDim = function (elem) {
    return {
        ...
        setValue: function (coordinates) {
            elem.all(protractor_1.by.tagName('input')).first().clear();
            elem.all(protractor_1.by.tagName('input')).first().sendKeys(coordinates[0]);
            elem.all(protractor_1.by.tagName('input')).last().clear();
            elem.all(protractor_1.by.tagName('input')).last().sendKeys(coordinates[1]);
        }
    };
};
var FilepathEditor = function (elem) {
    return {
        upload: function (filepath) {
            ...
            elem.element(protractor_1.by.css('.protractor-test-file-upload')).sendKeys(filepath);
        },
        setName: function (name) {
            elem.element(protractor_1.by.css('.protractor-test-file-name')).clear();
            elem.element(protractor_1.by.css('.protractor-test-file-name')).sendKeys(name);
        }
    };
};
var FractionEditor = function (elem) {
    return {
        setValue: function (value) {
            elem.element(protractor_1.by.tagName('input')).clear();
            elem.element(protractor_1.by.tagName('input')).sendKeys(value);
        },
        expectValueToBe: function (expectedValue) {
            elem.element(protractor_1.by.tagName('input')).getAttribute('value').then(function (value) {
...
var MathLatexStringEditor = function (elem) {
    return {
        setValue: function (rawLatex) {
            elem.element(protractor_1.by.tagName('textarea')).clear();
            elem.element(protractor_1.by.tagName('textarea')).sendKeys(rawLatex);
        }
    };
};
...
var ParameterNameEditor = function (elem) {
    return {
        setValue: function (text) {
            elem.element(protractor_1.by.cssContainingText('option', text)).click();
        }
    };
};
var SanitizedUrlEditor = function (elem) {
    return {
        setValue: function (text) {
            elem.element(protractor_1.by.tagName('input')).clear();
            elem.element(protractor_1.by.tagName('input')).sendKeys(text);
        },
        expectValueToBe: function (expectedValue) {
            elem.element(protractor_1.by.tagName('input')).getAttribute('value').then(function (value) {
                expect(value).toEqual(expectedValue);
