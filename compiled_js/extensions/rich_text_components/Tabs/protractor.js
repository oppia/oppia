...
var expectComponentDetailsToMatch = function (elem, tabArray) {
    elem.element(protractor_1.by.tagName('ul')).all(protractor_1.by.tagName('li')).then(function (titleElems) {
        expect(titleElems.length).toEqual(tabArray.length);
        elem.element(protractor_1.by.css('.tab-content')).all(protractor_1.by.xpath('./*'))
            ...
            for (var i = 0; i < tabArray.length; i++) {
                ...
                titleElems[i].click();
                forms.expectRichText(contentElems[i].element(protractor_1.by.css('.protractor-test-tab-content'))).toMatch(tabArray[i].content);
            }
        });
    });
};
