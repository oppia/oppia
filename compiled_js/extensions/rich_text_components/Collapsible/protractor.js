...
var customizeComponent = function (modal, heading, contentInstructions) {
    forms.UnicodeEditor(modal.element(by.tagName('schema-based-unicode-editor'))).setValue(heading);
    var richTextEditor = forms.RichTextEditor(modal.element(by.tagName('schema-based-html-editor')));
    richTextEditor.clear();
    contentInstructions(richTextEditor);
};
var expectComponentDetailsToMatch = function (elem, heading, contentInstructions) {
    var headerElement = elem.element(by.css('.protractor-test-collapsible-heading'));
    expect(headerElement.getText()).toMatch(heading);
    // Open the collapsible block so we can examine it.
    headerElement.click();
    forms.expectRichText(elem.element(by.css('.panel-body')).element(by.css('.protractor-test-collapsible-content'))).toMatch(contentInstructions);
};
