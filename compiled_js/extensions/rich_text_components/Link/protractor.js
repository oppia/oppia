...
var customizeComponent = function (modal, url) {
    objects.SanitizedUrlEditor(modal.element(protractor_1.by.tagName('sanitized-url-editor'))).setValue(url);
};
var expectComponentDetailsToMatch = function (elem, url) {
    expect(elem.element(protractor_1.by.tagName('a')).getAttribute('href')).toBe(url);
    expect(elem.element(protractor_1.by.tagName('a')).getAttribute('target')).toBe('_blank');
};
