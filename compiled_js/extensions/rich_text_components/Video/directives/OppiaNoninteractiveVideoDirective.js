    ...
    function ($sce, HtmlEscaperService, UrlInterpolationService) {
        return {
            ...
            controller: [
                ...
                function ($scope, $attrs, ContextService, $element, AutoplayedVideosService, PAGE_CONTEXT, $timeout, $window) {
                    ...
                    $scope.videoId = HtmlEscaperService.escapedJsonToObj($attrs.videoIdWithValue);
                    ...
                    $timeout(function () {
                        ...
                        if (ContextService.getPageContext() ===
                            ...
                            if (!AutoplayedVideosService.hasVideoBeenAutoplayed($scope.videoId) && isVisible) {
                                $scope.autoplaySuffix = '&autoplay=1';
                                AutoplayedVideosService.addAutoplayedVideo($scope.videoId);
                            }
                        }
                        $scope.videoUrl = $sce.trustAsResourceUrl('https://www.youtube.com/embed/' + $scope.videoId + '?rel=0' +
