...
describe('Guest collection progress service', function () {
    ...
    beforeEach(angular.mock.inject(function ($injector) {
        GuestCollectionProgressService = $injector.get('GuestCollectionProgressService');
        ...
        _collection0.addCollectionNode(CollectionNodeObjectFactory.createFromExplorationId(_expId0));
    }));
    afterEach(function () {
    ...
    describe('hasCompletedSomeExploration', function () {
        it('should initially not have any stored progress', function () {
            var hasProgress = (GuestCollectionProgressService.hasCompletedSomeExploration(_collectionId0));
            expect(hasProgress).toBe(false);
        });
        it('should have progress after recording an exploration', function () {
            GuestCollectionProgressService.recordExplorationCompletedInCollection(_collectionId0, _expId0);
            var hasProgress = (GuestCollectionProgressService.hasCompletedSomeExploration(_collectionId0));
            expect(hasProgress).toBe(true);
        });
        it('should not have progress after exp completed for another collection', function () {
            GuestCollectionProgressService.recordExplorationCompletedInCollection(_collectionId1, _expId0);
            var hasProgress = (GuestCollectionProgressService.hasCompletedSomeExploration(_collectionId0));
            expect(hasProgress).toBe(false);
        });
    });
    describe('getCompletedExplorationIds', function () {
        it('should initially provide no completed exploration ids', function () {
            var completedIds = (GuestCollectionProgressService.getCompletedExplorationIds(_collection0));
        ...
        it('should not provide completed ID for exp not in collection', function () {
            GuestCollectionProgressService.recordExplorationCompletedInCollection(_collectionId0, _expId1);
            var completedIds = (GuestCollectionProgressService.getCompletedExplorationIds(_collection0));
            expect(completedIds).toEqual([]);
        });
        it('should provide multiple completed exploration IDs', function () {
            ...
            GuestCollectionProgressService.recordExplorationCompletedInCollection(_collectionId1, _expId0);
            GuestCollectionProgressService.recordExplorationCompletedInCollection(_collectionId1, _expId2);
            var completedIds = (GuestCollectionProgressService.getCompletedExplorationIds(collection));
    ...
    describe('getNextExplorationId', function () {
        it('should provide the first exploration ID with no progress', function () {
            var collection = _createLinearCollection(_collectionId1);
            var nextExplorationId = (GuestCollectionProgressService.getNextExplorationId(collection, []));
            expect(nextExplorationId).toEqual(_expId0);
        });
        it('should provide the third exp ID with first two exps done', function () {
            var collection = _createLinearCollection(_collectionId1);
            var nextExplorationId = (GuestCollectionProgressService.getNextExplorationId(collection, [_expId0, _expId1]));
        ...
        it('should return null for fully completed collection', function () {
            var collection = _createLinearCollection(_collectionId1);
            var nextExplorationId = (GuestCollectionProgressService.getNextExplorationId(collection, [_expId0, _expId1, _expId2]));
