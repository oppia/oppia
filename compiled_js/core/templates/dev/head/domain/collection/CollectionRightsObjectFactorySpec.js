...
describe('Collection rights object factory', function () {
    ...
    it('should not be able to modify owner names', function () {
        ...
        var sampleCollectionRights = CollectionRightsObjectFactory.create(initialCollectionRightsBackendObject);
        var ownerNames = sampleCollectionRights.getOwnerNames();
        ownerNames.push('B');
        expect(sampleCollectionRights.getOwnerNames()).toEqual(['A']);
    });
    it('should accept accept changes to the bindable list of collection nodes', function () {
        var initialCollectionRightsBackendObject = {
            collection_id: 0,
            can_edit: true,
            can_unpublish: false,
    ...
    it('should throw error and not be able to set public when canEdit is false', function () {
        var initialCollectionRightsBackendObject = {
            collection_id: 0,
            can_edit: false,
            can_unpublish: false,
    ...
    it('should throw error when when canUnpublish is false during unpublishing', function () {
        ...
        var sampleCollectionRights = CollectionRightsObjectFactory.create(noUnpublishCollectionRightsBackendObject);
        expect(sampleCollectionRights.isPrivate()).toBe(false);
        expect(sampleCollectionRights.isPublic()).toBe(true);
        expect(function () {
            sampleCollectionRights.setPrivate();
    ...
    it('should create an empty collection rights object', function () {
        var emptyCollectionRightsBackendObject = (CollectionRightsObjectFactory.createEmptyCollectionRights());
        expect(emptyCollectionRightsBackendObject.getCollectionId()).toBeUndefined();
    ...
    it('should make a copy from another collection rights', function () {
        ...
        var sampleCollectionRights = CollectionRightsObjectFactory.create(noUnpublishCollectionRightsBackendObject);
        var emptyCollectionRightsBackendObject = (CollectionRightsObjectFactory.createEmptyCollectionRights());
        emptyCollectionRightsBackendObject.copyFromCollectionRights(sampleCollectionRights);
