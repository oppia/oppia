...
describe('Skill rights backend API service', function () {
    ...
    beforeEach(angular.mock.inject(function ($injector) {
        SkillRightsBackendApiService = $injector.get('SkillRightsBackendApiService');
    ...
    it('should successfully set a skill to be public', function () {
        ...
        $httpBackend.expect('PUT', '/skill_editor_handler/publish_skill/0').respond(200);
        SkillRightsBackendApiService.setSkillPublic('0', 1).then(successHandler, failHandler);
        $httpBackend.flush();
        $rootScope.$digest();
    ...
    it('should call the provided fail handler on HTTP failure', function () {
        var successHandler = jasmine.createSpy('success');
        var failHandler = jasmine.createSpy('fail');
        $httpBackend.expect('PUT', '/skill_editor_handler/publish_skill/0').respond(500, 'Error loading skill 0.');
        SkillRightsBackendApiService.setSkillPublic('0', 1).then(successHandler, failHandler);
        $httpBackend.flush();
        $rootScope.$digest();
    ...
    it('should report a cached skill rights after caching it', function () {
        ...
        SkillRightsBackendApiService.loadSkillRights('0').then(successHandler, failHandler);
        $rootScope.$digest();
