...
describe('PlaythroughImprovementCardObjectFactory', function () {
    ...
    describe('.createNew', function () {
        it('retrieves data from passed issue', function () {
            ...
            expect(card.getTitle()).toEqual(this.PlaythroughIssuesService.renderIssueStatement(issue));
            expect(card.getDirectiveData()).toEqual({
                suggestions: this.PlaythroughIssuesService.renderIssueSuggestions(issue),
                playthroughIds: ['1', '2'],
            });
            expect(card.getDirectiveType()).toEqual(this.PLAYTHROUGH_IMPROVEMENT_CARD_TYPE);
        });
    });
    describe('.fetchCards', function () {
        it('returns a card for each existing issue', function (done) {
            var earlyQuitIssue = this.PlaythroughIssueObjectFactory.createFromBackendDict({
                issue_type: 'EarlyQuit',
                issue_customization_args: {
                    state_name: { value: 'Hola' },
                    time_spent_in_exp_in_msecs: { value: 5000 },
            ...
            var earlyQuitCardTitle = this.PlaythroughIssuesService.renderIssueStatement(earlyQuitIssue);
            var multipleIncorrectSubmissionsIssue = this.PlaythroughIssueObjectFactory.createFromBackendDict({
            ...
            var multipleIncorrectSubmissionsCardTitle = this.PlaythroughIssuesService.renderIssueStatement(multipleIncorrectSubmissionsIssue);
            var cyclicTransitionsIssue = this.PlaythroughIssueObjectFactory.createFromBackendDict({
            ...
            var cyclicTransitionsCardTitle = this.PlaythroughIssuesService.renderIssueStatement(cyclicTransitionsIssue);
            spyOn(this.PlaythroughIssuesService, 'getIssues').and.returnValue(Promise.resolve([
                earlyQuitIssue,
    ...
    describe('PlaythroughImprovementCard', function () {
        ...
        describe('Discard Action Button', function () {
            ...
            it('marks the card as resolved after confirmation', function (done) {
                ...
                var resolveIssueSpy = spyOn(this.PlaythroughIssuesService, 'resolveIssue').and.stub();
