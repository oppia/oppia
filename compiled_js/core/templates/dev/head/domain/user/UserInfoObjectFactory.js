...
oppia.factory('UserInfoObjectFactory', [function () {
        var UserInfo = function (isModerator, isAdmin, isSuperAdmin, canCreateCollections, preferredSiteLanguageCode, username, isLoggedIn) {
        ...
        UserInfo.createFromBackendDict = function (data) {
            return new UserInfo(data.is_moderator, data.is_admin, data.is_super_admin, data.can_create_collections, data.preferred_site_language_code, data.username, data.user_is_logged_in);
        };
