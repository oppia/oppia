...
describe('Creator Dashboard backend API service', function () {
    ...
    it('should successfully fetch an creator dashboard data from the backend', function () {
        var successHandler = jasmine.createSpy('success');
        var failHandler = jasmine.createSpy('fail');
        $httpBackend.expect('GET', CREATOR_DASHBOARD_DATA_URL).respond(sampleDataResults);
        CreatorDashboardBackendApiService.fetchDashboardData().then(successHandler, failHandler);
        $httpBackend.flush();
    ...
    it('should use rejection handler if dashboard data backend request failed', function () {
        var successHandler = jasmine.createSpy('success');
        var failHandler = jasmine.createSpy('fail');
        $httpBackend.expect('GET', CREATOR_DASHBOARD_DATA_URL).respond(ERROR_STATUS_CODE, 'Error loading dashboard data.');
        CreatorDashboardBackendApiService.fetchDashboardData().then(successHandler, failHandler);
        $httpBackend.flush();
