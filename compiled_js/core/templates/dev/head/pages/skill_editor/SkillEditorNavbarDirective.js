...
oppia.directive('skillEditorNavbar', [
    'UrlInterpolationService', function (UrlInterpolationService) {
        return {
            restrict: 'E',
            templateUrl: UrlInterpolationService.getDirectiveTemplateUrl('/pages/skill_editor/skill_editor_navbar_directive.html'),
            controller: [
                ...
                function ($scope, $uibModal, AlertsService, UndoRedoService, SkillEditorStateService, SkillRightsBackendApiService, SkillEditorRoutingService, EVENT_SKILL_INITIALIZED, EVENT_SKILL_REINITIALIZED, EVENT_UNDO_REDO_SERVICE_CHANGE_APPLIED) {
                    ...
                    $scope.$on(EVENT_UNDO_REDO_SERVICE_CHANGE_APPLIED, _validateSkill);
                    ...
                    $scope.isSkillPublishable = function () {
                        return ($scope.skillRights.isPrivate() &&
                            $scope.validationIssues.length === 0 &&
                            $scope.getChangeListCount() === 0);
                    };
                    var _publishSkill = function () {
                        SkillRightsBackendApiService.setSkillPublic($scope.skill.getId(), $scope.skill.getVersion()).then(function () {
                            $scope.skillRights.setPublic();
                            SkillEditorStateService.setSkillRights($scope.skillRights);
                            AlertsService.addSuccessMessage('Skill Published.');
                        });
                    };
                    $scope.saveChanges = function () {
                        var modalInstance = $uibModal.open({
                            templateUrl: UrlInterpolationService.getDirectiveTemplateUrl('/pages/skill_editor/' +
