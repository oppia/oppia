...
describe('Signup controller', function () {
    describe('SignupCtrl', function () {
        ...
        it('should show warning if user has not agreed to terms', function () {
            ...
            expect(mockAlertsService.addWarning).toHaveBeenCalledWith('I18N_SIGNUP_ERROR_MUST_AGREE_TO_TERMS');
        });
        ...
        it('should show warning if username is too long', function () {
            scope.updateWarningText('abcdefghijklmnopqrstuvwxyzyxwvutsrqponmlkjihgfedcba');
            expect(scope.warningI18nCode).toEqual('I18N_SIGNUP_ERROR_USERNAME_MORE_50_CHARS');
        });
        it('should show warning if username has non-alphanumeric characters', function () {
            scope.updateWarningText('a-a');
            expect(scope.warningI18nCode).toEqual('I18N_SIGNUP_ERROR_USERNAME_ONLY_ALPHANUM');
        });
        it('should show warning if username has \'admin\' in it', function () {
            scope.updateWarningText('administrator');
            expect(scope.warningI18nCode).toEqual('I18N_SIGNUP_ERROR_USERNAME_WITH_ADMIN');
        });
        it('should show continue registration modal if user is logged ' +
            'out in new tab', function () {
            spyOn(scope, 'showRegistrationSessionExpiredModal');
            var errorResponseObject = {
                status_code: 401,
                error: ('Sorry, you have been logged out [probably in another ' +
                    'window]. Please log in again. You will be redirected ' +
                    'to main page in a while!')
            };
            $httpBackend.expectPOST('/signuphandler/data').respond(401, errorResponseObject);
