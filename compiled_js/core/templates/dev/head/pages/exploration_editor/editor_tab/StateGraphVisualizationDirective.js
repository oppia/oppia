    ...
    'UrlInterpolationService', function (UrlInterpolationService) {
        return {
            ...
            controller: [
                ...
                function ($element, $filter, $scope, $timeout, ExplorationWarningsService, StateGraphLayoutService, TranslationStatusService, MAX_NODES_PER_ROW, MAX_NODE_LABEL_LENGTH) {
                    ...
                    $scope.drawGraph = function (nodes, originalLinks, initStateId, finalStateIds) {
                        ...
                        var graphBounds = StateGraphLayoutService.getGraphBoundaries(nodeData);
                        ...
                        for (var i = 0; i < $scope.augmentedLinks.length; i++) {
                            ...
                            if (links[i].hasOwnProperty('linkProperty') &&
                                ...
                                if ($scope.linkPropertyMapping.hasOwnProperty(links[i].linkProperty)) {
                                    $scope.augmentedLinks[i].style = ($scope.linkPropertyMapping[links[i].linkProperty]);
                                }
                            }
                        }
                        var getNodeStrokeWidth = function (nodeId) {
                            var currentNodeIsTerminal = ($scope.finalStateIds.indexOf(nodeId) !== -1);
                        ...
                        for (var nodeId in nodeData) {
                            nodeData[nodeId].style = ('stroke-width: ' + getNodeStrokeWidth(nodeId) + '; ' +
                            ...
                            if (nodeColors) {
                                nodeData[nodeId].style += ('fill: ' + nodeColors[nodeId] + '; ');
                            }
                            ...
                            var currentNodeIsTerminal = ($scope.finalStateIds.indexOf(nodeId) !== -1);
                            nodeData[nodeId].nodeClass = (currentNodeIsTerminal ? 'terminal-node' :
                            ...
                            $timeout(function () {
                                ...
                                d3.select($element.find('rect.pannable-rect')[0])
                                    ...
                                    if (graphBounds.right - graphBounds.left < dimensions.w) {
                                        ...
                                        d3.event.translate[0] = clamp(d3.event.translate[0], dimensions.w - graphBounds.right -
                                            origTranslations[0], -graphBounds.left - origTranslations[0]);
                                    }
                                    if (graphBounds.bottom - graphBounds.top < dimensions.h) {
                                        d3.event.translate[1] = 0;
                                    }
                                    else {
                                        d3.event.translate[1] = clamp(d3.event.translate[1], dimensions.h - graphBounds.bottom -
                                            origTranslations[1], -graphBounds.top - origTranslations[1]);
                                    }
                        ...
                        if ($scope.centerAtCurrentState) {
                            $timeout(function () {
                                ...
                                origTranslations[0] = (dimensions.w / 2 - nodeData[$scope.currentStateId()].x0 -
                                    nodeData[$scope.currentStateId()].width / 2);
                                origTranslations[1] = (dimensions.h / 2 - nodeData[$scope.currentStateId()].y0 -
                                    nodeData[$scope.currentStateId()].height / 2);
                                if (graphBounds.right - graphBounds.left < dimensions.w) {
                                    origTranslations[0] = (dimensions.w / 2 -
                                        (graphBounds.right + graphBounds.left) / 2);
                                }
                                else {
                                    origTranslations[0] = clamp(origTranslations[0], dimensions.w - graphBounds.right, -graphBounds.left);
                                }
                                if (graphBounds.bottom - graphBounds.top < dimensions.h) {
                                    origTranslations[1] = (dimensions.h / 2 -
                                        (graphBounds.bottom + graphBounds.top) / 2);
                                }
                                else {
                                    origTranslations[1] = clamp(origTranslations[1], dimensions.h - graphBounds.bottom, -graphBounds.top);
                                }
                                $scope.overallTransformStr = ('translate(' + origTranslations + ')');
                                $scope.$apply();
                            }, 20);
                        }
                    };
