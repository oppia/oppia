...
oppia.factory('ExplorationDiffService', [
    'INTERACTION_SPECS', function (INTERACTION_SPECS) {
        ...
        var _postprocessStateIdsAndData = function (originalStateIds, stateIds, stateData, v1States, v2States) {
            ...
            for (var stateId in stateData) {
                if (stateData[stateId].stateProperty === STATE_PROPERTY_CHANGED &&
                    ...
                    angular.equals(v1States[stateData[stateId].originalStateName], v2States[stateData[stateId].newestStateName])) {
            ...
            for (var stateId in stateData) {
                ...
                if (oldState) {
                    oldStateIsTerminal = (oldState.interaction.id &&
                        INTERACTION_SPECS[oldState.interaction.id].is_terminal);
                }
                if (newState) {
                    newStateIsTerminal = (newState.interaction.id &&
        ...
        var _getDiffGraphData = function (v1States, v2States, changeListData) {
            ...
            return _postprocessStateIdsAndData(originalStateIds, stateIds, stateData, v1States, v2States);
        };
        /**
         ...
         */
        var _compareLinks = function (v1States, originalStateIds, v2States, newestStateIds) {
            var links = [];
                ...
                for (var j = 1; j <= _maxId; j++) {
                    if (i !== j && (adjMatrixV1[i][j] || adjMatrixV2[i][j])) {
                        links.push({
                            source: i,
                            target: j,
                            linkProperty: (adjMatrixV1[i][j] && adjMatrixV2[i][j] ? 'unchanged' :
