...
oppia.controller('FeedbackTab', [
    ...
    function ($log, $q, $rootScope, $scope, $uibModal, AlertsService, ChangeListService, DateTimeFormatService, ExplorationDataService, ExplorationStatesService, ShowSuggestionModalForEditorViewService, StateObjectFactory, ThreadDataService, ThreadStatusDisplayService, UrlInterpolationService, UserService, ACTION_ACCEPT_SUGGESTION, ACTION_REJECT_SUGGESTION) {
        ...
        $scope.showCreateThreadModal = function () {
            $uibModal.open({
                templateUrl: UrlInterpolationService.getDirectiveTemplateUrl('/pages/exploration_editor/feedback_tab/' +
                ...
                ThreadDataService.createNewThread(result.newThreadSubject, result.newThreadText, function () {
                    $scope.clearActiveThread();
        ...
        var _isSuggestionValid = function () {
            return ExplorationStatesService.hasState($scope.activeThread.getSuggestionStateName());
        };
        ...
        $scope.showSuggestionModal = function () {
            ShowSuggestionModalForEditorViewService.showSuggestionModal($scope.activeThread.suggestion.suggestionType, {
        ...
        $scope.setActiveThread = function (threadId) {
            ...
            var allThreads = [].concat($scope.threadData.feedbackThreads, $scope.threadData.suggestionThreads);
