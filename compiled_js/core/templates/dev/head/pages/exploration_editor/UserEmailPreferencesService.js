...
oppia.factory('UserEmailPreferencesService', [
    ...
    function ($http, $q, AlertsService, ExplorationDataService, UrlInterpolationService) {
        var MESSAGE_TYPE_SUGGESTION = 'suggestion';
        var MESSAGE_TYPE_FEEDBACK = 'feedback';
        return {
            init: function (feedbackNotificationsMuted, suggestionNotificationsMuted) {
            ...
            saveChangeToBackend: function (requestParams) {
                var that = this;
                var emailPreferencesUrl = UrlInterpolationService.interpolateUrl('/createhandler/notificationpreferences/<exploration_id>', {
                    exploration_id: ExplorationDataService.explorationId
                });
                return $http.put(emailPreferencesUrl, requestParams).then(function (response) {
                    var data = response.data;
                    AlertsService.clearWarnings();
                    that.init(data.email_preferences.mute_feedback_notifications, data.email_preferences.mute_suggestion_notifications);
                });
            }
        };
    }
