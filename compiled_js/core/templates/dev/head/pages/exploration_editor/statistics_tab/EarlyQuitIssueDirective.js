...
oppia.directive('earlyQuitIssueDirective', [
    'UrlInterpolationService', function (UrlInterpolationService) {
        return {
            ...
            templateUrl: UrlInterpolationService.getDirectiveTemplateUrl('/pages/exploration_editor/statistics_tab/' +
                ...
                function ($scope, $uibModal, AlertsService, PlaythroughIssuesService) {
                    ...
                    $scope.resolveIssue = function () {
                        if (!issueResolved) {
                            ...
                            AlertsService.addSuccessMessage('Issue resolved. Refresh the page to view changes.');
                            issueResolved = true;
                        }
                        else {
                            AlertsService.addSuccessMessage('Issue has already been resolved. No need to resolve again. ' +
                                'Refresh the page to view changes.');
                        }
                    };
                    $scope.showPlaythrough = function (playthroughId) {
                        PlaythroughIssuesService.getPlaythrough(playthroughId).then(function (playthrough) {
                            $uibModal.open({
                                templateUrl: UrlInterpolationService.getDirectiveTemplateUrl('/pages/exploration_editor/statistics_tab/' +
                                    ...
                                    function ($scope, $uibModalInstance, playthroughIndex, playthrough, AlertsService, LearnerActionRenderService) {
                                        ...
                                        $scope.showRemainingActions = function (pIdx) {
                                            ...
                                            if ($scope.maxHidden === 1) {
                                                getRemainingActionsElements(pIdx, $scope.maxHidden).style.display = 'block';
                                                ...
                                                for (i = $scope.maxHidden; i > 0; i--) {
                                                    if (getRemainingActionsElements(pIdx, i).style.display === 'block') {
                                                        currentShown = i;
                                                        break;
                                                    }
                                                }
                                                if (currentShown === 0) {
                                                    getRemainingActionsElements(pIdx, currentShown + 1).style.display = 'block';
                                                }
