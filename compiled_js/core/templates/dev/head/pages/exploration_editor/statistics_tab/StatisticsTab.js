...
oppia.controller('StatisticsTab', [
    ...
    function ($http, $scope, $uibModal, AlertsService, ComputeGraphService, DateTimeFormatService, ExplorationDataService, ExplorationFeaturesService, ExplorationStatesService, ReadOnlyExplorationBackendApiService, RouterService, StateImprovementSuggestionService, StateRulesStatsService, StatesObjectFactory, UrlInterpolationService, IMPROVE_TYPE_INCOMPLETE) {
        ...
        $scope.refreshExplorationStatistics = function (version) {
            $scope.explorationStatisticsUrl = ('/createhandler/statistics/' + ExplorationDataService.explorationId);
            $http.get($scope.explorationStatisticsUrl).then(function (statsResponse) {
                ...
                ReadOnlyExplorationBackendApiService.loadLatestExploration(ExplorationDataService.explorationId).then(function (response) {
                    ...
                    var improvements = (StateImprovementSuggestionService.getStateImprovements(states, $scope.stateStats));
        ...
        $scope.showStateStatsModal = function (stateName, improvementType) {
            ...
            StateRulesStatsService.computeStateRulesStats(ExplorationStatesService.getState(stateName)).then(function (stateRulesStats) {
                $uibModal.open({
                    ...
                    controller: [
                        ...
                        function ($scope, $uibModalInstance, $filter, $injector, stateName, stateStats, improvementType, visualizationsInfo, HtmlEscaperService, AngularNameService, AnswerClassificationService) {
                            ...
                            $scope.COMPLETION_RATE_PIE_CHART_OPTIONS1 = angular.copy(COMPLETION_RATE_PIE_CHART_OPTIONS);
                            $scope.COMPLETION_RATE_PIE_CHART_OPTIONS1.title = title1;
                            var title2 = 'Solution usage statistics';
                            $scope.COMPLETION_RATE_PIE_CHART_OPTIONS2 = angular.copy(COMPLETION_RATE_PIE_CHART_OPTIONS);
                            ...
                            var usefulFeedbackCount = ($scope.stateStats.useful_feedback_count);
                            var totalAnswersCount = ($scope.stateStats.total_answers_count);
                            ...
                            var numTimesSolutionViewed = ($scope.stateStats.num_times_solution_viewed);
                            $scope.pieChartData2 = [
                                ['Type', 'Number'],
                            ...
                            var _getVisualizationsHtml = function () {
                                var htmlSnippets = visualizationsInfo.map(function (vizInfo) {
                                    var escapedData = HtmlEscaperService.objToEscapedJson(vizInfo.data);
                                    var escapedOptions = HtmlEscaperService.objToEscapedJson(vizInfo.options);
                                    ...
                                    el.attr('addressed-info-is-supported', vizInfo.addressed_info_is_supported);
