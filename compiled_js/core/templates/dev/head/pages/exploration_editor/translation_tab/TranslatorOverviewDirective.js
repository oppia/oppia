    ...
    'UrlInterpolationService', function (UrlInterpolationService) {
        return {
            ...
            controller: [
                ...
                'TranslationStatusService', 'DEFAULT_AUDIO_LANGUAGE', function ($scope, $rootScope, $window, SUPPORTED_AUDIO_LANGUAGES, LanguageUtilService, TranslationLanguageService, TranslationStatusService, DEFAULT_AUDIO_LANGUAGE) {
                    var LAST_SELECTED_TRANSLATION_LANGUAGE = ('last_selected_translation_lang');
                    var prevLanguageCode = $window.localStorage.getItem(LAST_SELECTED_TRANSLATION_LANGUAGE);
                    ...
                    $scope.languageCodesAndDescriptions = (allAudioLanguageCodes.map(function (languageCode) {
                        return {
                            id: languageCode,
                    ...
                    $scope.getTranslationProgressStyle = function () {
                        ...
                        var progressPercent = (100 - ($scope.numberOfAudioNotAvailable /
                    ...
                    $scope.changeTranslationLanguage = function () {
                        if ($scope.isTranslationTabBusy) {
                            $scope.languageCode = $window.localStorage.getItem(LAST_SELECTED_TRANSLATION_LANGUAGE);
                            $rootScope.$broadcast('showTranslationTabBusyModal');
                            return;
                        }
                        TranslationLanguageService.setActiveLanguageCode($scope.languageCode);
                        $window.localStorage.setItem(LAST_SELECTED_TRANSLATION_LANGUAGE, $scope.languageCode);
                    };
                }
            ]
        };
