...
describe('PlaythroughIssuesBackendApiService', function () {
    ...
    describe('.fetch', function () {
        it('returns the issues data provided by the backend', function () {
            ...
            this.$httpBackend.expectGET('/issuesdatahandler/7?exp_version=1').respond(backendIssues);
            this.PlaythroughIssuesBackendApiService.fetchIssues('7', 1).then(successHandler, failureHandler);
            this.$httpBackend.flush();
            expect(successHandler).toHaveBeenCalledWith(backendIssues.map(this.piof.createFromBackendDict));
        ...
        it('returns the playthrough data provided by the backend', function () {
            ...
            var successHandler = jasmine.createSpy('success');
            var failureHandler = jasmine.createSpy('failure');
            this.$httpBackend.expectGET('/playthroughdatahandler/7/1').respond(backendPlaythrough);
            this.PlaythroughIssuesBackendApiService.fetchPlaythrough('7', '1').then(successHandler, failureHandler);
            this.$httpBackend.flush();
            expect(successHandler).toHaveBeenCalledWith(this.pof.createFromBackendDict(backendPlaythrough));
            expect(failureHandler).not.toHaveBeenCalled();
        });
    });
});
