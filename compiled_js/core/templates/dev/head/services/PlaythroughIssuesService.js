...
oppia.factory('PlaythroughIssuesService', [
    ...
    function ($sce, PlaythroughIssuesBackendApiService, ISSUE_TYPE_CYCLIC_STATE_TRANSITIONS, ISSUE_TYPE_EARLY_QUIT, ISSUE_TYPE_MULTIPLE_INCORRECT_SUBMISSIONS) {
        var issues = null;
        ...
        var renderMultipleIncorrectIssueStatement = function (stateName) {
            var statement = 'Several learners submitted answers to card "' + stateName +
        ...
        var renderEarlyQuitIssueSuggestions = function (issue) {
            var suggestions = [$sce.trustAsHtml('Review the cards up to and including <span class="state_link">' +
        ...
        var renderMultipleIncorrectIssueSuggestions = function (stateName) {
            var suggestions = [$sce.trustAsHtml('Check the wording of the card <span class="state_link">"' +
                    stateName + '</span> to ensure it is not confusing.'), $sce.trustAsHtml('Consider addressing the answers submitted in the sample playthroughs' +
        ...
        var renderCyclicTransitionsIssueSuggestions = function (issue) {
            ...
            var suggestions = [$sce.trustAsHtml('Check that the concept presented in <span class="state_link">"' +
        ...
        return {
            ...
            getIssues: function () {
                return PlaythroughIssuesBackendApiService.fetchIssues(explorationId, explorationVersion).then(function (issues) {
                    return issues;
                });
            },
            getPlaythrough: function (playthroughId) {
                return PlaythroughIssuesBackendApiService.fetchPlaythrough(explorationId, playthroughId).then(function (playthrough) {
                    return playthrough;
                });
            },
            ...
            resolveIssue: function (issue) {
                PlaythroughIssuesBackendApiService.resolveIssue(issue, explorationId, explorationVersion);
            }
        };
    }
]);
