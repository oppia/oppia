...
oppia.directive('hintEditor', [
    'UrlInterpolationService', function (UrlInterpolationService) {
        return {
            ...
            templateUrl: UrlInterpolationService.getDirectiveTemplateUrl('/components/hint_editor_directive.html'),
            controller: [
                ...
                function ($scope, EditabilityService, StateHintsService) {
                    ...
                    $scope.saveThisHint = function () {
                        $scope.hintEditorIsOpen = false;
                        var contentHasChanged = ($scope.hintMemento.hintContent.getHtml() !==
                            $scope.hint.hintContent.getHtml());
                        $scope.hintMemento = null;
                        if (contentHasChanged) {
                            var hintContentId = $scope.hint.hintContent.getContentId();
                            $scope.showMarkAllAudioAsNeedingUpdateModalIfRequired(hintContentId);
                        }
                        $scope.getOnSaveFn()();
                    };
