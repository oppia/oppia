...
oppia.directive('topNavigationBar', [
    'UrlInterpolationService', function (UrlInterpolationService) {
        return {
            restrict: 'E',
            scope: {},
            templateUrl: UrlInterpolationService.getDirectiveTemplateUrl('/components/top_navigation_bar/' +
            ...
            controller: [
                ...
                function ($scope, $http, $window, $timeout, $translate, SidebarStatusService, LABEL_FOR_CLEARING_FOCUS, UserService, SiteAnalyticsService, NavigationService, WindowDimensionsService, DebouncerService, DeviceInfoService) {
                    ...
                    UserService.getUserInfoAsync().then(function (userInfo) {
                        ...
                        if ($scope.username) {
                            $scope.profilePageUrl = UrlInterpolationService.interpolateUrl('/profile/<username>', {
                            ...
                            $http.get('/notificationshandler').then(function (response) {
                                ...
                                if ($window.location.pathname !== '/') {
                                    ...
                                    if ($scope.numUnseenNotifications > 0) {
                                        $window.document.title = ('(' + $scope.numUnseenNotifications + ') ' +
                    ...
                    $scope.standardNavIsShown = (NAV_MODES_WITH_CUSTOM_LOCAL_NAV.indexOf($scope.currentUrl) === -1);
                    $scope.onLoginButtonClicked = function () {
                        SiteAnalyticsService.registerStartLoginEvent('loginButton');
                        UserService.getLoginAndLogoutUrls().then(function (urlObject) {
                    ...
                    $scope.googleSignInIconUrl = (UrlInterpolationService.getStaticImageUrl('/google_signin_buttons/google_signin.svg'));
