...
var _selectLanguage = function (language) {
    protractor_1.element(protractor_1.by.css('.protractor-test-i18n-language-selector')).
...
describe('Basic user journeys', function () {
    describe('Account creation', function () {
        ...
        it('should create users', function () {
            users.createUser('ordinaryuser@userManagement.com', 'ordinaryUserManagement');
            users.login('ordinaryuser@userManagement.com');
            libraryPage.get();
            general.checkForConsoleErrors([]);
            protractor_1.browser.get(general.MODERATOR_URL_SUFFIX);
            general.checkForConsoleErrors([
...
describe('Site language', function () {
    ...
    it('should change after selecting a different language', function () {
        ...
        libraryPage.get();
        libraryPage.expectMainHeaderTextToBe('Imagina lo que podrías aprender hoy...');
    ...
    it('should set preferred audio language selected in the Preferences page.', function () {
    ...
    it('should save the language selected in the footer into the preferences.', function () {
    ...
    it('should not change in an exploration', function () {
        ...
        var placeholder = protractor_1.element(protractor_1.by.css('.protractor-test-float-form-input'))
            .getAttribute('placeholder');
        expect(placeholder).toEqual('Ingresa un número');
        general.ensurePageHasNoTranslationIds();
        users.logout();
    });
    it('should not change in exploration and collection player for guest users', function () {
        protractor_1.browser.get('/about');
        waitFor.pageToFullyLoad();
        _selectLanguage('Español');
        ...
        waitFor.pageToFullyLoad();
        expect(protractor_1.element(protractor_1.by.css('.oppia-share-collection-footer')).getText())
            .toEqual('COMPARTIR ESTA COLECCIÓN');
        general.ensurePageHasNoTranslationIds();
        // Checking exploration player page.
        protractor_1.browser.get('/explore/' + firstExplorationId);
        waitFor.pageToFullyLoad();
        expect(protractor_1.element(protractor_1.by.css('.author-profile-text')).getText())
