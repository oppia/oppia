...
describe('rich-text components', function () {
    ...
    it('should display correctly', function () {
        ...
        explorationEditorMainTab.setContent(function (richTextEditor) {
            ...
            // and click on them.
            richTextEditor.addRteComponent('Collapsible', 'title', forms.toRichText('inner'));
        ...
        explorationPlayerPage.expectContentToMatch(function (richTextChecker) {
            ...
            richTextChecker.readRteComponent('Collapsible', 'title', forms.toRichText('inner'));
...
describe('Interactions', function () {
    ...
    it('should pass their own test suites', function () {
        ...
        for (var interactionId in interactions.INTERACTIONS) {
            ...
            for (var i = 0; i < interaction.testSuite.length; i++) {
                ...
                explorationEditorMainTab.setInteraction.apply(null, [interactionId].concat(test.interactionArguments));
                ...
                explorationEditorPage.navigateToPreviewTab();
                explorationPlayerPage.expectInteractionToMatch.apply(null, [interactionId].concat(test.expectedInteractionDetails));
                ...
                var clearHelpcardButton = protractor_1.element(protractor_1.by.css('.protractor-test-close-help-card-button'));
    ...
    it('publish and play exploration successfully', function () {
         ...
         */
        users.createAndLoginUser('explorationEditor@interactions.com', 'explorationEditor');
        workflow.createExploration();
        explorationEditorMainTab.setStateName('Graph');
        explorationEditorMainTab.setContent(forms.toRichText('Draw a complete graph with the given vertices.'));
        var graphDict = {
        ...
        explorationEditorMainTab.addResponse('GraphInput', forms.toRichText('Good job!'), 'MathExp', true, 'IsIsomorphicTo', graphDict);
        var responseEditor = explorationEditorMainTab.getResponseEditor('default');
        responseEditor.setFeedback(forms.toRichText('A complete graph is a graph in which each pair of graph vertices is ' +
            'connected by an edge.'));
        explorationEditorMainTab.moveToState('MathExp');
        explorationEditorMainTab.setContent(function (richTextEditor) {
            richTextEditor.appendPlainText('Please simplify the following expression: ');
            // Some Latex styling is expected here.
            richTextEditor.addRteComponent('Math', '16x^{12}/4x^2');
        });
        explorationEditorMainTab.setInteraction('MathExpressionInput');
        // Proper Latex styling for rule spec is required.
        explorationEditorMainTab.addResponse('MathExpressionInput', forms.toRichText('Good job!'), 'End', true, 'IsMathematicallyEquivalentTo', '\\frac{16x^{12}}{4x^{2}}');
        // Expecting answer to be 4x^10
        var responseEditor = explorationEditorMainTab.getResponseEditor('default');
        responseEditor.setFeedback(forms.toRichText('A simplified expression should be smaller than the original.'));
        ...
        explorationEditorSettingsTab.setObjective('To publish and play this exploration');
        ...
        explorationPlayerPage.expectExplorationNameToBe('Regression Test Exploration');
        // Play Graph Input interaction.
        explorationPlayerPage.expectContentToMatch(forms.toRichText('Draw a complete graph with the given vertices.'));
        graphDict = {
            edges: [[1, 2], [1, 0], [0, 2]]
        };
        explorationPlayerPage.submitAnswer('GraphInput', graphDict);
        explorationPlayerPage.expectLatestFeedbackToMatch(forms.toRichText('Good job!'));
