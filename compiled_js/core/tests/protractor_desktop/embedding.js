...
describe('Embedding', function () {
    ...
    var createCountingExploration = function () {
        ...
        explorationEditorMainTab.setContent(forms.toRichText('Given three balls of different colors. How many ways are there ' +
            'to arrange them in a straight line?'));
        explorationEditorMainTab.setInteraction('NumericInput');
        explorationEditorMainTab.addResponse('NumericInput', null, 'correct but why', true, 'Equals', 6);
        explorationEditorMainTab.addResponse('NumericInput', forms.toRichText('Describe solution!!'), null, false, 'IsLessThanOrEqualTo', 0);
        var defaultResponseEditor = explorationEditorMainTab
            .getResponseEditor('default');
        defaultResponseEditor.setDestination('Not 6', true, null);
        // correct but why.
        explorationEditorMainTab.moveToState('correct but why');
        explorationEditorMainTab.setContent(forms.toRichText('Right! Why do you think it is 6?'));
        explorationEditorMainTab.setInteraction('TextInput', 'Type your answer here.', 5);
        explorationEditorMainTab.addResponse('TextInput', forms.toRichText('Yes, 3! = 3 x 2 x 1. That\'s 3 x 2 = 6 ways.'), 'END', true, 'Contains', 'permutation');
        explorationEditorMainTab.addResponse('TextInput', forms.toRichText('Yes, 3 factorial, or 3 x 2 x 1. That\'s 3 x 2 = 6 ways.'), 'END', false, 'Contains', 'factorial');
        explorationEditorMainTab.getResponseEditor('default').setFeedback(forms.toRichText('Figure out what the answer for 4 balls is!'));
        // Not 6.
        explorationEditorMainTab.moveToState('Not 6');
        explorationEditorMainTab.setContent(forms.toRichText('List the different ways?'));
    ...
    it('should display and play embedded explorations', function () {
        ...
        var playCountingExploration = function (version) {
            ...
            explorationPlayerPage.expectContentToMatch(forms.toRichText((version === 2) ?
            ...
            explorationPlayerPage.expectContentToMatch(forms.toRichText('Right! Why do you think it is 6?'));
        ...
        general.getExplorationIdFromEditor().then(function (expId) {
            ...
            for (var i = 0; i < TEST_PAGES.length; i++) {
                ...
                driver.get(general.SERVER_URL_PREFIX + general.SCRIPTS_URL_SLICE +
                    TEST_PAGES[i].filename);
                driver.findElement(protractor_1.by.css('.protractor-test-exploration-id-input-field')).sendKeys(explorationId);
                driver.findElement(protractor_1.by.css('.protractor-test-exploration-id-submit-button')).click();
                // Test of standard loading (new and old versions).
                protractor_1.browser.switchTo().frame(driver.findElement(protractor_1.by.css('.protractor-test-standard > iframe')));
                ...
                if (TEST_PAGES[i].isVersion1) {
                    ...
                    driver.findElement(protractor_1.by.css('.protractor-test-old-version > oppia > div > button')).click();
                }
                protractor_1.browser.switchTo().frame(driver.findElement(protractor_1.by.css('.protractor-test-old-version > iframe')));
                ...
                for (var i = 0; i < browserLogs.length; i++) {
                    ...
                    try {
                        ...
                        if (message.indexOf(EMBEDDING_PREFIX) !== -1) {
                            ...
                            embeddingLogs.push(message.substring(index + EMBEDDING_PREFIX.length, message.length - 1));
                        }
                    }
                    catch (err) { }
                }
    ...
    it('should use the exploration language as site language.', function () {
        ...
        var checkPlaceholder = function (expectedPlaceholder) {
            ...
            driver.findElement(protractor_1.by.css('.protractor-test-exploration-id-input-field')).sendKeys(explorationId);
            driver.findElement(protractor_1.by.css('.protractor-test-exploration-id-submit-button')).click();
            protractor_1.browser.switchTo().frame(driver.findElement(protractor_1.by.css('.protractor-test-embedded-exploration > iframe')));
            waitFor.pageToFullyLoad();
            expect(driver.findElement(protractor_1.by.css('.protractor-test-float-form-input'))
        ...
        general.getExplorationIdFromEditor().then(function (expId) {
            ...
            explorationEditorMainTab.setInteraction('NumericInput');
            explorationEditorMainTab.addResponse('NumericInput', forms.toRichText('Nice!!'), 'END', true, 'IsLessThanOrEqualTo', 0);
            explorationEditorMainTab.getResponseEditor('default').setFeedback(forms.toRichText('Ok!!'));
            explorationEditorMainTab.getResponseEditor('default').setDestination('(try again)', null, false);
            ...
            explorationEditorPage.saveChanges('Changing the language to a not supported one.');
            ...
            explorationEditorPage.saveChanges('Changing the language to a supported one.');
