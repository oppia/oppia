...
describe('Exploration translation', function () {
    ...
    beforeAll(function () {
        ...
        explorationEditorMainTab.setInteraction('NumericInput');
        explorationEditorMainTab.addResponse('NumericInput', forms.toRichText('This is feedback1.'), 'third', true, 'Equals', 6);
        explorationEditorMainTab.moveToState('third');
        explorationEditorMainTab.setContent(forms.toRichText('This is third card.'));
    ...
    it('should contain accessibility elements', function () {
        ...
        creatorDashboardPage.editExploration('tests');
        explorationEditorPage.navigateToTranslationTab();
        explorationEditorTranslationTab.expectNumericalStatusAccessibilityToMatch('0 items translated out of 11 items');
        explorationEditorTranslationTab.expectContentAccessibilityToMatch('Content of the card');
        explorationEditorTranslationTab.expectFeedbackAccessibilityToMatch('Feedback responses for answer groups');
        explorationEditorTranslationTab.expectHintAccessibilityToMatch('Hints for the state');
        explorationEditorTranslationTab.expectSolutionAccessibilityToMatch('Solutions for the state');
        explorationEditorTranslationTab.expectStartRecordingAccessibilityToMatch('Start recording');
        explorationEditorTranslationTab.expectUploadRecordingAccessibilityToMatch('Upload translated file');
        explorationEditorTranslationTab.expectPlayRecordingAccessibilityToMatch('Play recorded audio');
        users.logout();
        general.checkForConsoleErrors([]);
    });
    it('should have a correct numerical status', function () {
         ...
         * translation.
         */
        explorationEditorTranslationTab.expectWrongFileType(relativePathOfAudioToUpload);
        ...
        explorationEditorTranslationTab.expectAudioOverFiveMinutes(relativePathOfAudioToUpload);
        /**
        ...
        explorationEditorTranslationTab.expectNumericalStatusToMatch('(1/11)');
        users.logout();
        general.checkForConsoleErrors(['Failed to load resource: the server responded with a status of 400' +
                '(Bad Request)', { status_code: 400,
                error: 'Audio files must be under 300 seconds in length.' +
                    ' The uploaded file is 301.87 seconds long.' }]);
    });
    it('should provide correct status color for each state in the graph view', function () {
        ...
        explorationEditorTranslationTab.expectCorrectStatusColor('second', FEW_AUDIO_AVAILABLE_COLOR);
        explorationEditorTranslationTab.expectCorrectStatusColor('third', NO_AUDIO_AVAILABLE_COLOR);
        explorationEditorTranslationTab.expectCorrectStatusColor('final card', ALL_AUDIO_AVAILABLE_COLOR);
        general.checkForConsoleErrors([]);
    });
