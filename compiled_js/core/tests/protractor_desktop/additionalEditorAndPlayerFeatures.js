...
describe('Full exploration editor', function () {
    ...
    it('should walk through the tutorial when user repeatedly clicks Next', function () {
        users.createUser('userTutorial@stateEditor.com', 'userTutorialStateEditor');
    ...
    it('should prevent going back when help card is shown', function () {
        ...
        explorationEditorMainTab.getResponseEditor('default').setDestination('card 2', true, null);
        explorationEditorMainTab.moveToState('card 2');
        explorationEditorMainTab.setContent(forms.toRichText('this is card 2 with non-inline interaction'));
        explorationEditorMainTab.setInteraction('LogicProof', '', '', 'from p we have p');
        explorationEditorMainTab.addResponse('LogicProof', forms.toRichText('Great'), 'final card', true, 'Correct');
        ...
        protractor_1.element.all(protractor_1.by.css('.protractor-test-back-button')).then(function (buttons) {
    ...
    it('should redirect back to parent exploration correctly when parent id is' +
        ...
        workflow.createAndPublishExploration('Parent Exploration 1', 'Algebra', 'This is the topmost parent exploration.');
        general.getExplorationIdFromEditor().then(function (explorationId) {
            var parentId1 = explorationId;
            workflow.createAndPublishExploration('Parent Exploration 2', 'Algebra', 'This is the second parent exploration to which refresher ' +
                'exploration redirects.');
            general.getExplorationIdFromEditor().then(function (explorationId) {
                var parentId2 = explorationId;
                workflow.createAndPublishExploration('Refresher Exploration', 'Algebra', 'This is the most basic refresher exploration');
                general.getExplorationIdFromEditor().then(function (explorationId) {
                    var refresherExplorationId = explorationId;
                    protractor_1.browser.get('/explore/' + refresherExplorationId + '?parent=' +
    ...
    it('should give option for redirection when author has specified ' +
        ...
        explorationEditorSettingsTab.setTitle('Parent Exploration not in collection');
        ...
        explorationEditorMainTab.setContent(forms.toRichText('Parent Exploration Content'));
        explorationEditorMainTab.setInteraction('MultipleChoiceInput', [forms.toRichText('Correct'), forms.toRichText('Incorrect')]);
        explorationEditorMainTab.addResponse('MultipleChoiceInput', null, 'card 2', true, 'Equals', 'Correct');
        ...
        explorationEditorSettingsTab.setCategory('Algebra');
        explorationEditorSettingsTab.setObjective('This is the refresher exploration');
        explorationEditorPage.navigateToMainTab();
        explorationEditorMainTab.setContent(forms.toRichText('Refresher Exploration Content'));
        explorationEditorMainTab.setInteraction('EndExploration');
        explorationEditorPage.saveChanges();
        workflow.publishExploration();
        ...
        general.getExplorationIdFromEditor().then(function (refresherExplorationId) {
            ...
            creatorDashboardPage.get();
            creatorDashboardPage.editExploration('Parent Exploration not in collection');
            responseEditor = explorationEditorMainTab.getResponseEditor('default');
            responseEditor.setDestination(null, false, refresherExplorationId);
            explorationEditorPage.saveChanges('Add Refresher Exploration Id');
        });
        ...
        collectionEditorPage.searchForAndAddExistingExploration('Parent Exploration in collection');
        ...
        explorationPlayerPage.expectExplorationNameToBe('Refresher Exploration');
        explorationPlayerPage.clickOnReturnToParentButton();
        explorationPlayerPage.submitAnswer('MultipleChoiceInput', 'Incorrect');
        explorationPlayerPage.clickCancelRedirectionButton();
        explorationPlayerPage.expectContentToMatch(forms.toRichText('Parent Exploration Content'));
        ...
        protractor_1.element.all(protractor_1.by.css('.protractor-test-collection-exploration')).first().click();
        explorationPlayerPage.submitAnswer('MultipleChoiceInput', 'Incorrect');
        explorationPlayerPage.clickConfirmRedirectionButton();
        // Check the current url to see if collection_id is present in it.
        protractor_1.browser.getCurrentUrl().then(function (url) {
            var pathname = url.split('/');
            expect(pathname[4].split('?')[1].split('=')[0]).toEqual('collection_id');
            users.logout();
        });
    });
    it('should generate warning message if card height limit is exceeded ', function () {
        ...
        var content = 'line1\n\n\n\nline2\n\n\n\nline3\n\n\nline4';
        var heightMessage = protractor_1.element(protractor_1.by.css('.oppia-card-height-limit-warning'));
        richTextEditor.appendPlainText(content);
        expect(heightMessage.isPresent()).toBe(false);
        richTextEditor.appendPlainText('\n\n\nline5');
        waitFor.visibilityOf(heightMessage, 'Card height limit message not displayed');
        richTextEditor.appendPlainText('\b\b\b\b\b\b\b\b');
        expect(heightMessage.isPresent()).toBe(false);
        richTextEditor.appendPlainText('\n\n\nline5');
        waitFor.visibilityOf(heightMessage, 'Card height limit message not displayed');
        protractor_1.element(protractor_1.by.css('.oppia-hide-card-height-warning-icon')).click();
        expect(heightMessage.isPresent()).toBe(false);
        users.logout();
    });
    it('should navigate multiple states correctly, with parameters', function () {
        ...
        explorationEditorMainTab.addResponse('NumericInput', null, 'final card', true, 'Equals', 21);
        explorationEditorMainTab.getResponseEditor(0).setDestination('card 2', true, null);
        explorationEditorMainTab.moveToState('card 2');
        explorationEditorMainTab.setContent(forms.toRichText('this is card 2 with previous answer {{answer}}'));
        explorationEditorMainTab.setInteraction('MultipleChoiceInput', [forms.toRichText('return'), forms.toRichText('complete')]);
        explorationEditorMainTab.addResponse('MultipleChoiceInput', null, 'card 1', false, 'Equals', 'return');
        explorationEditorMainTab.getResponseEditor('default').setDestination('final card', false, null);
        // Setup a terminating state.
        explorationEditorMainTab.moveToState('final card');
        explorationEditorMainTab.setInteraction('EndExploration');
        explorationEditorPage.saveChanges();
        ...
        explorationPlayerPage.expectContentToMatch(forms.toRichText('this is card 2 with previous answer 21'));
    ...
    it('uses hints and solutions in an exploration', function () {
        var explorationPlayerPage = (new ExplorationPlayerPage.ExplorationPlayerPage());
        ...
        explorationEditorMainTab.setContent(forms.toRichText('What language is Oppia?'));
        explorationEditorMainTab.setInteraction('TextInput');
        explorationEditorMainTab.addResponse('TextInput', forms.toRichText('Good job'), 'End', true, 'Equals', 'Finnish');
        explorationEditorMainTab.getResponseEditor('default').setFeedback(forms.toRichText('Try again'));
    ...
    it('should handle discarding changes, navigation, deleting states, ' +
        ...
        explorationEditorMainTab.setInteraction('TextInput');
        explorationEditorMainTab.getResponseEditor('default').setDestination('final card', true, null);
        explorationEditorMainTab.getResponseEditor('default').setDestination('card2', true, null);
        ...
        explorationEditorMainTab.expectStateNamesToBe(['final card', 'card1', 'card2']);
        explorationEditorMainTab.setInteraction('EndExploration');
        // Check discarding of changes.
        explorationEditorPage.discardChanges();
        explorationEditorMainTab.expectCurrentStateToBe(general.FIRST_STATE_DEFAULT_NAME);
        ...
        explorationEditorMainTab.setInteraction('TextInput');
        explorationEditorMainTab.getResponseEditor('default').setDestination('final card', true, null);
        explorationEditorMainTab.getResponseEditor('default').setDestination('second', true, null);
        explorationEditorMainTab.moveToState('second');
        explorationEditorMainTab.expectStateNamesToBe(['final card', 'first', 'second']);
        explorationEditorMainTab.expectCurrentStateToBe('second');
        explorationEditorPage.navigateToSettingsTab();
        explorationEditorSettingsTab.expectAvailableFirstStatesToBe(['final card', 'first', 'second']);
        ...
        explorationEditorPage.navigateToMainTab();
        general.getExplorationIdFromEditor().then(function (explorationId) {
            expect(protractor_1.browser.getCurrentUrl()).toEqual(general.SERVER_URL_PREFIX + general.EDITOR_URL_SLICE +
        ...
        responseEditor.expectAvailableDestinationsToBe(['second', 'final card']);
        explorationEditorMainTab.addResponse('NumericInput', null, 'final card', false, 'IsGreaterThan', 2);
    ...
    it('should handle multiple rules in an answer group and also disallow ' +
        ...
        explorationEditorMainTab.setContent(forms.toRichText('How are you feeling?'));
        explorationEditorMainTab.setInteraction('TextInput');
        explorationEditorMainTab.addResponse('TextInput', forms.toRichText('You must be happy!'), null, false, 'Equals', 'happy');
        explorationEditorMainTab.addResponse('TextInput', forms.toRichText('No being sad!'), null, false, 'Contains', 'sad');
        var responseEditor = explorationEditorMainTab.getResponseEditor('default');
        responseEditor.setFeedback(forms.toRichText('Okay, now this is just becoming annoying.'));
        ...
        general.getExplorationIdFromPlayer().then(function (explorationId) {
            protractor_1.browser.get(general.SERVER_URL_PREFIX + general.EDITOR_URL_SLICE +
                explorationId);
        });
        explorationEditorMainTab.exitTutorial();
        ...
        explorationPlayerPage.expectLatestFeedbackToMatch(forms.toRichText('You must be happy!'));
        ...
        explorationPlayerPage.expectLatestFeedbackToMatch(forms.toRichText('Okay, now this is just becoming annoying.'));
        explorationPlayerPage.clickThroughToNextCard();
        explorationPlayerPage.expectExplorationToBeOver();
        users.logout();
    });
    it('should delete interactions cleanly', function () {
        ...
        explorationEditorPage.navigateToMainTab();
        explorationEditorMainTab.setInteraction('TextInput');
        explorationEditorMainTab.addResponse('TextInput', forms.toRichText('Happy!'), null, false, 'Equals', 'happy');
        explorationEditorMainTab.expectInteractionToMatch('TextInput');
        explorationEditorPage.saveChanges();
        explorationEditorMainTab.deleteInteraction();
        explorationEditorMainTab.setInteraction('EndExploration');
    ...
    it('should play the recommended exploration successfully', function () {
        ...
        workflow.createExploration();
        explorationEditorMainTab.setContent(forms.toRichText('You should recommend this exploration'));
        explorationEditorMainTab.setInteraction('EndExploration');
        ...
        general.getExplorationIdFromPlayer()
            ...
            workflow.createExploration();
            explorationEditorMainTab.setContent(forms.toRichText('I want to recommend an exploration at the end'));
            explorationEditorMainTab.setInteraction('EndExploration', [recommendedExplorationId]);
            explorationEditorPage.navigateToSettingsTab();
            explorationEditorSettingsTab.setTitle('Exploration with Recommendation');
            explorationEditorSettingsTab.setCategory('Algorithm');
            explorationEditorSettingsTab.setObjective('To display recommended exploration');
            explorationEditorPage.navigateToMainTab();
            explorationEditorPage.saveChanges();
            workflow.publishExploration();
        });
        ...
        var recommendedExplorationTile = protractor_1.element(protractor_1.by.css('.protractor-test-exp-summary-tile-title'));
        expect(recommendedExplorationTile.getText())
            .toEqual('Recommended Exploration 1');
        recommendedExplorationTile.click();
        explorationPlayerPage.expectExplorationNameToBe('Recommended Exploration 1');
        users.logout();
    });
    afterEach(function () {
