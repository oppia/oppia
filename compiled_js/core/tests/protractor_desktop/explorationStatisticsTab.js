...
describe('Issues visualization', function () {
    ...
    beforeAll(function () {
        ...
        users.createUser('user2@ExplorationIssues.com', 'learnerExplorationIssues');
        users.createAndLoginAdminUser('user1@ExplorationIssues.com', 'authorExplorationIssues');
        ...
        explorationEditorMainTab.setContent(forms.toRichText('Please write 1 in words.'));
        explorationEditorMainTab.setInteraction('TextInput');
        explorationEditorMainTab.addResponse('TextInput', forms.toRichText('Good job'), 'Two', true, 'Equals', 'One');
        ...
        explorationEditorMainTab.setContent(forms.toRichText('Please write 2 in words.'));
        explorationEditorMainTab.setInteraction('TextInput');
        explorationEditorMainTab.addResponse('TextInput', forms.toRichText('Good job'), 'Three', true, 'Equals', 'Two');
        explorationEditorMainTab.getResponseEditor('default').setFeedback(forms.toRichText('Try again'));
        explorationEditorMainTab.moveToState('Three');
        explorationEditorMainTab.setContent(forms.toRichText('Please write 3 in words.'));
        explorationEditorMainTab.setInteraction('TextInput');
        explorationEditorMainTab.addResponse('TextInput', forms.toRichText('Good job'), 'End', true, 'Equals', 'Three');
        explorationEditorMainTab.addResponse('TextInput', forms.toRichText('Try 2 again'), 'Two', false, 'Equals', 'Two');
        explorationEditorMainTab.getResponseEditor('default').setFeedback(forms.toRichText('Try again'));
        explorationEditorMainTab.moveToState('End');
        explorationEditorMainTab.setInteraction('EndExploration');
        explorationEditorPage.saveChanges();
        workflow.publishExploration();
        adminPage.editConfigProperty('The set of exploration IDs for recording playthrough issues', 'List', function (elem) {
            elem.addItem('Unicode').setValue(expId);
        });
        adminPage.editConfigProperty('The probability of recording playthroughs', 'Real', function (elem) {
            elem.setValue(1.0);
        });
    });
    it('records early quit issue.', function () {
        ...
        explorationEditorStatsTab.expectIssueTitleToBe('Several learners exited the exploration in less than a minute.');
    ...
    it('records multiple incorrect issue.', function () {
        ...
        explorationPlayerPage.expectLatestFeedbackToMatch(forms.toRichText('Try again'));
        ...
        explorationEditorPage.navigateToStatsTab();
        explorationEditorStatsTab.clickIssue(0, 'Issue 1');
        explorationEditorStatsTab.expectIssueTitleToBe('Several learners submitted answers to card "One" several times, ' +
    ...
    it('records cyclic transitions issue.', function () {
        ...
        explorationPlayerPage.expectContentToMatch(forms.toRichText('Please write 2 in words.'));
        explorationPlayerPage.clickThroughToNextCard();
        explorationPlayerPage.submitAnswer('TextInput', 'Two');
        explorationPlayerPage.expectContentToMatch(forms.toRichText('Please write 3 in words.'));
        explorationPlayerPage.clickThroughToNextCard();
        explorationPlayerPage.submitAnswer('TextInput', 'Two');
        explorationPlayerPage.expectContentToMatch(forms.toRichText('Please write 2 in words.'));
        explorationPlayerPage.clickThroughToNextCard();
        ...
        explorationEditorPage.navigateToStatsTab();
        explorationEditorStatsTab.clickIssue(0, 'Issue 1');
        explorationEditorStatsTab.expectIssueTitleToBe('Several learners ended up in a cyclic loop revisiting card ' +
            '"Two" many times.');
        explorationEditorStatsTab.markResolved();
        users.logout();
    });
...
describe('Statistics tab', function () {
    ...
    it('checks statistics tab for an exploration', function () {
        ...
        var EXPLORATION_CATEGORY = 'Algorithms';
        var EXPLORATION_LANGUAGE = 'English';
        users.createUser('user1@statisticsTab.com', 'statisticsTabCreator');
        users.createUser('user2@statisticsTab.com', 'statisticsTabLearner1');
        users.createUser('user3@statisticsTab.com', 'statisticsTabLearner2');
