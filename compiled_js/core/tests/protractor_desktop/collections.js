...
describe('Collections', function () {
    ...
    beforeAll(function () {
        ...
        workflow.createAndPublishExploration('First Exploration', 'Languages', 'First Test Exploration.');
        general.getExplorationIdFromEditor().then(function (expId) {
            firstExplorationId = expId;
            workflow.createAndPublishExploration('Second Exploration', 'Languages', 'Second Test Exploration.');
            general.getExplorationIdFromEditor().then(function (expId) {
                secondExplorationId = expId;
            });
            workflow.createAndPublishExploration('Third Exploration', 'Languages', 'Third Test Exploration.');
            general.getExplorationIdFromEditor().then(function (expId) {
                thirdExplorationId = expId;
            });
            workflow.createAndPublishExploration('Fourth Exploration', 'Languages', 'Fourth Test Exploration.');
            general.getExplorationIdFromEditor().then(function (expId) {
                fourthExplorationId = expId;
            });
            // Create searchable explorations.
            workflow.createAndPublishExploration('The Lazy Magician', 'Algorithms', 'discover the binary search algorithm');
            workflow.createAndPublishExploration('Root Linear Coefficient Theorem', 'Algebra', 'discover the Root Linear Coefficient Theorem');
            workflow.createAndPublishExploration('Test Exploration', 'Languages', 'discover the Protractor Testing');
            users.logout();
    ...
    it('visits the collection player', function () {
        users.login('alice@collections.com');
        protractor_1.browser.get('/collection/' + collectionId);
    ...
    it('checks for errors in a collection with varying node count', function () {
        ...
        protractor_1.browser.get('/collection_editor/create/' + collectionId);
        waitFor.pageToFullyLoad();
        collectionEditorPage.addExistingExploration(secondExplorationId);
        collectionEditorPage.saveDraft();
