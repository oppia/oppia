...
describe('Core exploration functionality', function () {
    ...
    beforeEach(function () {
        ...
        users.createUser("user" + userNumber + "@stateEditor.com", "user" + userNumber + "StateEditor");
    ...
    it('should create content and multiple choice interactions', function () {
        ...
        explorationEditorMainTab.setInteraction('MultipleChoiceInput', [forms.toRichText('option A'), forms.toRichText('option B')]);
        ...
        general.moveToPlayer();
        explorationPlayerPage.expectExplorationToNotBeOver();
        explorationPlayerPage.expectInteractionToMatch('MultipleChoiceInput', [forms.toRichText('option A'), forms.toRichText('option B')]);
    ...
    it('should obey numeric interaction rules and display feedback', function () {
        ...
        explorationEditorMainTab.addResponse('NumericInput', function (richTextEditor) {
            richTextEditor.appendBoldText('correct');
        }, 'final card', true, 'IsInclusivelyBetween', -1, 3);
        explorationEditorMainTab.getResponseEditor(0).expectRuleToBe('NumericInput', 'IsInclusivelyBetween', [-1, 3]);
        explorationEditorMainTab.getResponseEditor(0)
            .expectFeedbackInstructionToBe('correct');
        explorationEditorMainTab.getResponseEditor('default').setFeedback(forms.toRichText('out of bounds'));
        explorationEditorMainTab.getResponseEditor('default')
            .expectFeedbackInstructionToBe('out of bounds');
        explorationEditorMainTab.getResponseEditor('default').setDestination(null, false, null);
        // Setup a terminating state.
        explorationEditorMainTab.moveToState('final card');
        explorationEditorMainTab.setInteraction('EndExploration');
        explorationEditorPage.saveChanges();
        ...
        explorationPlayerPage.expectLatestFeedbackToMatch(forms.toRichText('out of bounds'));
        explorationPlayerPage.expectExplorationToNotBeOver();
        ...
        explorationPlayerPage.expectLatestFeedbackToMatch(function (richTextChecker) {
    ...
    it('should open appropriate modal on re-clicking an interaction to ' +
        ...
        var okayBtn = element(by.css('.protractor-test-close-no-customization-modal'));
    ...
    it('should correctly display contents, rule parameters, feedback' +
        ...
        explorationEditorMainTab.expectContentToMatch(forms.toRichText('Happiness Checker'));
        // Verify interaction's details.
        explorationEditorMainTab.setInteraction('TextInput', 'How are you?', 5);
        explorationEditorMainTab.expectInteractionToMatch('TextInput', 'How are you?', 5);
        // Verify rule parameter input by checking editor's response tab.
        // Create new state 'I am happy' for 'happy' rule.
        explorationEditorMainTab.addResponse('TextInput', forms.toRichText('You must be happy!'), 'I am happy', true, 'FuzzyEquals', 'happy');
        explorationEditorMainTab.getResponseEditor(0).expectRuleToBe('TextInput', 'FuzzyEquals', ['"happy"']);
        explorationEditorMainTab.getResponseEditor(0)
        ...
        explorationEditorMainTab.addResponse('TextInput', forms.toRichText('You cannot be sad!'), '(try again)', false, 'FuzzyEquals', 'sad');
        explorationEditorMainTab.getResponseEditor(1).expectRuleToBe('TextInput', 'FuzzyEquals', ['"sad"']);
        explorationEditorPage.saveChanges();
    });
    ...
    it('should show warnings when the length of goal < 15', function () {
        ...
        explorationEditorSettingsTab.expectWarningsColorToBe('rgba(115, 115, 115, 1)');
        explorationEditorSettingsTab.setObjective('short goal');
        explorationEditorSettingsTab.expectWarningsColorToBe('rgba(169, 68, 66, 1)');
    });
