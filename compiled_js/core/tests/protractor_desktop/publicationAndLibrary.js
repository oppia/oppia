...
describe('Library index page', function () {
    ...
    it('should display private and published explorations', function () {
        ...
        users.createModerator('varda@publicationAndLibrary.com', 'vardaPublicationAndLibrary');
        users.createUser('feanor@publicationAndLibrary.com', 'feanorPublicationAndLibrary');
        users.createUser('celebrimor@publicationAndLibrary.com', 'celebriorPublicationAndLibrary');
        users.createUser('earendil@publicationAndLibrary.com', 'earendilPublicationAndLibrary');
        users.login('feanor@publicationAndLibrary.com');
        workflow.createAndPublishExploration(EXPLORATION_SILMARILS, CATEGORY_ARCHITECTURE, 'hold the light of the two trees', LANGUAGE_DEUTSCH);
        users.logout();
        users.login('earendil@publicationAndLibrary.com');
        workflow.createAndPublishExploration(EXPLORATION_VINGILOT, CATEGORY_BUSINESS, 'seek the aid of the Valar');
        users.logout();
        ...
        explorationEditorMainTab.setContent(forms.toRichText('Celebrimbor wrote this'));
        explorationEditorMainTab.setInteraction('EndExploration');
        explorationEditorPage.navigateToSettingsTab();
        explorationEditorSettingsTab.setObjective('preserve the works of the elves');
        ...
        protractor_1.browser.get('/search/find?q=&language_code=("en")');
        ...
        expect(libraryPage.getExplorationObjective(EXPLORATION_VINGILOT)).toBe('Seek the aid of the Valar');
    ...
    it('should not have any non translated strings', function () {
        ...
        workflow.createAndPublishExploration(EXPLORATION_SILMARILS, CATEGORY_BUSINESS, 'hold the light of the two trees', LANGUAGE_FRANCAIS);
        workflow.createAndPublishExploration(EXPLORATION_VINGILOT, CATEGORY_ENVIRONMENT, 'seek the aid of the Valar');
        users.logout();
        libraryPage.get();
        libraryPage.expectMainHeaderTextToBe('Imagine what you could learn today...');
...
describe('Permissions for private explorations', function () {
    ...
    it('should be correct for translators', function () {
        ...
        general.getExplorationIdFromEditor().then(function (explorationId) {
            ...
            general.openEditor(explorationId);
            explorationEditorMainTab.expectContentToMatch(forms.toRichText('this is card 1'));
            expect(protractor_1.element(protractor_1.by.css('.protractor-test-save-changes')).isPresent()).toBeTruthy();
            users.logout();
