...
var DictionaryEditor = function (elem) {
    return {
        editEntry: function (index, objectType) {
            var entry = elem.element(protractor_1.by.repeater('property in propertySchemas()').
                row(index));
...
var GraphEditor = function (graphInputContainer) {
    ...
    var vertexElement = function (index) {
        ...
        return graphInputContainer.element(protractor_1.by.css('.protractor-test-graph-vertex-' + index));
    };
    var createVertex = function (xOffset, yOffset) {
        var addNodeButton = graphInputContainer.element(protractor_1.by.css('.protractor-test-Add-Node-button'));
        addNodeButton.click();
    ...
    var createEdge = function (vertexIndex1, vertexIndex2) {
        var addEdgeButton = graphInputContainer.element(protractor_1.by.css('.protractor-test-Add-Edge-button'));
        addEdgeButton.click();
        protractor_1.browser.actions()
    ...
    return {
        ...
        clearDefaultGraph: function () {
            var deleteButton = graphInputContainer.element(protractor_1.by.css('.protractor-test-Delete-button'));
            deleteButton.click();
        ...
        expectCurrentGraphToBe: function (graphDict) {
            ...
            if (edgesList) {
                ...
                // dict's edges.
                var allEdgesElement = protractor_1.element.all(protractor_1.by.css('.protractor-test-graph-edge'));
...
var ListEditor = function (elem) {
    ...
    var _getLength = function () {
        return elem.all(protractor_1.by.repeater('item in localValue track by $index'))
            .then(function (items) {
            return items.length;
        });
    };
    ...
    var addItem = function (objectType) {
        ...
        elem.element(protractor_1.by.css('.protractor-test-add-list-entry')).click();
        if (objectType !== null) {
            return getEditor(objectType)(elem.element(protractor_1.by.repeater('item in localValue track by $index').row(listLength)));
        }
    };
    var deleteItem = function (index) {
        elem.element(protractor_1.by.repeater('item in localValue track by $index').row(index)).element(protractor_1.by.css('.protractor-test-delete-list-entry')).click();
    };
    return {
        editItem: function (index, objectType) {
            var item = elem.element(protractor_1.by.repeater('item in localValue track by $index').row(index));
            var editor = getEditor(objectType);
            return editor(item);
        },
        addItem: addItem,
        ...
        setLength: function (desiredLength) {
            elem.all(protractor_1.by.repeater('item in localValue track by $index')).count().then(function (startingLength) {
...
var RichTextEditor = function (elem) {
    ...
    elem.all(protractor_1.by.css('.oppia-rte')).first().click();
    var _appendContentText = function (text) {
        elem.all(protractor_1.by.css('.oppia-rte')).first().sendKeys(text);
    };
    var _clickToolbarButton = function (buttonName) {
        elem.element(protractor_1.by.css('.' + buttonName)).click();
    };
    var _clearContent = function () {
        expect(elem.all(protractor_1.by.css('.oppia-rte')).first().isPresent()).toBe(true);
        elem.all(protractor_1.by.css('.oppia-rte')).first().clear();
    };
    return {
        clear: function () {
            _clearContent();
        ...
        addRteComponent: function (componentName) {
            ...
            var modal = protractor_1.element.all(protractor_1.by.css('.modal-dialog')).last();
            ...
            modal.element(protractor_1.by.css('.protractor-test-close-rich-text-component-editor')).click();
            ...
            if (['Video', 'Image', 'Collapsible', 'Tabs'].includes(componentName)) {
                elem.all(protractor_1.by.css('.oppia-rte')).first().sendKeys(protractor.Key.DOWN);
            }
            // Ensure that the cursor is at the end of the RTE.
            elem.all(protractor_1.by.css('.oppia-rte')).first().sendKeys(protractor.Key.chord(protractor.Key.CONTROL, protractor.Key.END));
        }
    };
};
...
var AutocompleteDropdownEditor = function (elem) {
    return {
        setValue: function (text) {
            elem.element(protractor_1.by.css('.select2-container')).click();
            ...
            protractor_1.element(protractor_1.by.css('.select2-dropdown')).element(protractor_1.by.css('.select2-search input')).sendKeys(text + '\n');
        },
        expectOptionsToBe: function (expectedOptions) {
            elem.element(protractor_1.by.css('.select2-container')).click();
            protractor_1.element(protractor_1.by.css('.select2-dropdown')).all(protractor_1.by.tagName('li')).map(function (optionElem) {
                return optionElem.getText();
            }).then(function (actualOptions) {
                expect(actualOptions).toEqual(expectedOptions);
            });
            // Re-close the dropdown.
            protractor_1.element(protractor_1.by.css('.select2-dropdown')).element(protractor_1.by.css('.select2-search input')).sendKeys('\n');
        }
    };
};
var AutocompleteMultiDropdownEditor = function (elem) {
    return {
        setValues: function (texts) {
            ...
            elem.element(protractor_1.by.css('.select2-selection__rendered'))
                .all(protractor_1.by.tagName('li')).map(function (choiceElem) {
                return choiceElem.element(protractor_1.by.css('.select2-selection__choice__remove'));
            ...
            for (var i = 0; i < texts.length; i++) {
                elem.element(protractor_1.by.css('.select2-container')).click();
                elem.element(protractor_1.by.css('.select2-search__field')).sendKeys(texts[i] + '\n');
            }
        },
...
var MultiSelectEditor = function (elem) {
    ...
    var _toggleElementStatusesAndVerifyExpectedClass = function (texts, expectedClassBeforeToggle) {
        ...
        elem.element(protractor_1.by.css('.protractor-test-search-bar-dropdown-toggle')).click();
        elem.element(protractor_1.by.css('.protractor-test-search-bar-dropdown-menu'))
            .all(protractor_1.by.tagName('span')).filter(function (choiceElem) {
    ...
    return {
        ...
        expectCurrentSelectionToBe: function (expectedCurrentSelection) {
            ...
            elem.element(protractor_1.by.css('.protractor-test-search-bar-dropdown-menu'))
                .all(protractor_1.by.css('.protractor-test-selected')).map(function (selectedElem) {
...
var expectRichText = function (elem) {
    var toMatch = function (richTextInstructions) {
        ...
        elem.all(protractor_1.by.xpath(XPATH_SELECTOR)).map(function (entry) {
            ...
            elem.all(protractor_1.by.xpath(XPATH_SELECTOR)).then(function (arrayOfElements) {
...
var CodeMirrorChecker = function (elem) {
    ...
    var _compareTextAndHighlightingFromLine = function (currentLineNumber, scrollTo, compareDict) {
        ...
        // large.
        protractor_1.browser.executeScript("$('.CodeMirror-vscrollbar').first().scrollTop(" + String(scrollTo) +
            ');');
        elem.all(protractor_1.by.xpath('./div')).map(function (lineElement) {
            return lineElement.element(protractor_1.by.css('.CodeMirror-linenumber')).getText()
                ...
                expect(lineElement.element(protractor_1.by.xpath('./pre')).getText())
                    .toEqual(compareDict[lineNumber].text);
                expect(lineElement.element(protractor_1.by.css('.CodeMirror-linebackground')).isPresent())
