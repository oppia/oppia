...
var isInDevMode = function () {
    protractor_1.browser.get('/library');
    waitFor.pageToFullyLoad();
    var devModeElement = protractor_1.element(protractor_1.by.css('.oppia-dev-mode'));
    ...
    return {
        then: function (callbackFunction) {
            protractor_1.browser.getCurrentUrl().then(function (url) {
                ...
                var explorationId = url.slice(currentUrlPrefix.length, currentUrlPrefix.length + EXPLORATION_ID_LENGTH);
...
var openEditor = function (explorationId) {
    protractor_1.browser.get(EDITOR_URL_SLICE + explorationId);
    waitFor.pageToFullyLoad();
...
var openPlayer = function (explorationId) {
    protractor_1.browser.get(PLAYER_URL_SLICE + explorationId);
    waitFor.pageToFullyLoad();
};
...
var expect404Error = function () {
    expect(protractor_1.element(protractor_1.by.css('.protractor-test-error-container')).getText()).
        toMatch('Error 404');
};
...
var ensurePageHasNoTranslationIds = function () {
    ...
    protractor_1.element(protractor_1.by.css('.oppia-base-container')).getAttribute('innerHTML').then(function (promiseValue) {
