...
var ExplorationEditorMainTab = function () {
     ...
     */
    var addResponseDetails = element(by.css('.protractor-test-add-response-details'));
    var addResponseHeader = element(by.css('.protractor-test-add-response-modal-header'));
    var multipleChoiceAnswerOptions = function (optionNum) {
        return element(by.cssContainingText('.protractor-test-html-select-option', optionNum));
    };
    var neutralElement = element.all(by.css('.protractor-test-neutral-element'))
        .first();
    var defaultResponseTab = element(by.css('.protractor-test-default-response-tab'));
    ...
    var editOutcomeDestStateInput = editOutcomeDestBubble.element(by.css('.protractor-test-add-state-input'));
    var editOutcomeDestAddExplorationId = element(by.css('.protractor-test-add-refresher-exploration-id'));
    ...
    var editParamChanges = element(by.css('.protractor-test-state-edit-param-changes'));
    ...
    var interactionEditor = element(by.css('.protractor-test-interaction-editor'));
    ...
    var interactionTile = function (interactionId) {
        return element(by.css('.protractor-test-interaction-tile-' + interactionId));
    };
    var openOutcomeDestEditor = element(by.css('.protractor-test-open-outcome-dest-editor'));
    var openOutcomeFeedBackEditor = element(by.css('.protractor-test-open-outcome-feedback-editor'));
    ...
    var stateContentDisplay = element(by.css('.protractor-test-state-content-display'));
    var stateNameContainer = element(by.css('.protractor-test-state-name-container'));
    var stateNameInput = element(by.css('.protractor-test-state-name-input'));
    /*
     * Buttons
     */
    ...
    var addNewResponseButton = element(by.css('.protractor-test-add-new-response'));
    var addParamButton = element(by.css('.protractor-test-add-param-button'));
    var addResponseButton = element(by.css('.protractor-test-open-add-response-modal'));
    var addSolutionButton = element(by.css('.protractor-test-oppia-add-solution-button'));
    var addInteractionButton = element(by.css('.protractor-test-open-add-interaction-modal'));
    var cancelOutcomeDestButton = element(by.css('.protractor-test-cancel-outcome-dest'));
    var closeAddResponseButton = element(by.css('.protractor-test-close-add-response-modal'));
    var confirmDeleteInteractionButton = element(by.css('.protractor-test-confirm-delete-interaction'));
    var confirmDeleteResponseButton = element(by.css('.protractor-test-confirm-delete-response'));
    var confirmDeleteStateButton = element(by.css('.protractor-test-confirm-delete-state'));
    var deleteAnswerButton = element(by.css('.protractor-test-delete-answer'));
    var deleteInteractionButton = element(by.css('.protractor-test-delete-interaction'));
    var deleteResponseButton = element(by.css('.protractor-test-delete-response'));
    var dismissWelcomeModalButton = element(by.css('.protractor-test-dismiss-welcome-modal'));
    var saveAnswerButton = element(by.css('.protractor-test-save-answer'));
    var saveHintButton = element(by.css('.protractor-test-save-hint'));
    var saveHintEditButton = element(by.css('.protractor-test-save-hint-edit'));
    var saveInteractionButton = element(by.css('.protractor-test-save-interaction'));
    var saveOutcomeDestButton = element(by.css('.protractor-test-save-outcome-dest'));
    var saveOutcomeFeedbackButton = element(by.css('.protractor-test-save-outcome-feedback'));
    var saveParamChangesButton = element(by.css('.protractor-test-save-param-changes-button'));
    var saveStateContentButton = element(by.css('.protractor-test-save-state-content'));
    var stateNameSubmitButton = stateNameContainer.element(by.css('.protractor-test-state-name-submit'));
    var answerCorrectnessToggle = element(by.css('.protractor-test-editor-correctness-toggle'));
    /*
     * Actions
     */
    // TUTORIAL
    ...
    this.playTutorial = function () {
        ...
        tutorialTabHeadings.forEach(function (heading) {
            var tutorialTabHeadingElement = element(by.cssContainingText('.popover-title', heading));
    ...
    this.startTutorial = function () {
        ...
        waitFor.visibilityOf(element(by.css('.ng-joyride-title')), 'Tutorial modal takes too long to appear');
    };
    // RESPONSE EDITOR
    /**
    ...
    this.getResponseEditor = function (responseNum) {
        ...
        return {
            ...
            expectAvailableDestinationsToBe: function (stateNames) {
                ...
                editOutcomeDestBubble.all(by.tagName('option')).map(function (optionElem) {
    ...
    this.setContent = function (richTextInstructions) {
        ...
        var stateContentEditor = stateEditorTag.element(by.css('.protractor-test-state-content-editor'));
    ...
    this.addHint = function (hint) {
        addHintButton.click();
        var addHintModal = element(by.cssContainingText('.protractor-test-hint-modal', 'Add Hint'));
    ...
    this.getHintEditor = function (hintNum) {
        var confirmDeteletHintButton = element(by.css('.protractor-test-confirm-delete-hint'));
        var headerElem = element.all(by.css('.protractor-test-hint-tab')).get(hintNum);
        var deleteHintIcon = headerElem.element(by.css('.protractor-test-delete-response'));
        var hintBodyElem = element(by.css('.protractor-test-hint-body-' + hintNum));
        ...
        return {
            setHint: function (hint) {
                var editHintIcon = element(by.css('.protractor-test-open-hint-editor'));
                editHintIcon.click();
    ...
    this.addSolution = function (interactionId, solution) {
        ...
        var addOrUpdateSolutionModal = element(by.css('.protractor-test-add-or-update-solution-modal'));
        waitFor.visibilityOf(addOrUpdateSolutionModal, 'Add/Update Solution modal takes to long to appear');
        interactions.getInteraction(interactionId).submitAnswer(element(by.css('.protractor-test-interaction-html')), solution.correctAnswer);
        ...
        var submitSolutionButton = element(by.css('.protractor-test-submit-solution-button'));
    ...
    this.addMultipleChoiceParameterChange = function (paramName, paramValues) {
        ...
        var editorRowElem = element.all(by.css('.protractor-test-param-changes-list')).last();
        forms.AutocompleteDropdownEditor(editorRowElem).setValue(paramName);
        var editorRowOption = editorRowElem.element(by.cssContainingText('option', 'to one of'));
    ...
    var _setRuleParameters = function (ruleElement, interactionId, ruleName) {
        ...
        var answerDescriptionFragment = element.all(by.css('.protractor-test-answer-description-fragment'));
    ...
    var _selectRule = function (ruleElem, interactionId, ruleName) {
        ...
        var answerDescription = element(by.css('.protractor-test-answer-description'));
        expect(answerDescription.isDisplayed()).toBe(true);
        answerDescription.click();
        var ruleDropdownElement = element.all(by.cssContainingText('.select2-results__option', ruleDescriptionInDropdown)).first();
    ...
    this.deleteState = function (stateName) {
        general.scrollToTop();
        var nodeElement = explorationGraph.all(by.cssContainingText('.protractor-test-node', stateName)).first();
