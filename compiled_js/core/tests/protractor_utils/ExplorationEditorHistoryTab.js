...
var ExplorationEditorHistoryTab = function () {
     ...
     */
    var historyCheckboxSelector = protractor_1.element.all(protractor_1.by.css('.protractor-test-history-checkbox-selector'));
    var historyGraph = protractor_1.element(protractor_1.by.css('.protractor-test-history-graph'));
    var stateNodes = historyGraph.all(protractor_1.by.css('.protractor-test-node'));
    var stateNodeBackground = function (nodeElement) {
        return nodeElement.element(protractor_1.by.css('.protractor-test-node-background'));
    };
     ...
     * Buttons
     */
    var closeStateHistoryButton = protractor_1.element(protractor_1.by.css('.protractor-test-close-history-state-modal'));
    var showHistoryGraphButton = protractor_1.element(protractor_1.by.css('.protractor-test-show-history-graph'));
    var revertVersionButton = protractor_1.element.all(protractor_1.by.css('.protractor-test-revert-version'));
    var confirmRevertVersionButton = protractor_1.element(protractor_1.by.css('.protractor-test-confirm-revert'));
    /*
     * Links
     */
    var historyGraphLink = historyGraph.all(protractor_1.by.css('.protractor-test-link'));
    /*
     * Workflows
     */
        ...
        return {
            openStateHistory: function (stateName) {
                stateNodes.map(function (stateElement) {
                    ...
                    if (!matched) {
                        throw Error('State ' + stateName + ' not found by getHistoryGraph.');
                    }
                });
            },
            closeStateHistory: function () {
                waitFor.elementToBeClickable(closeStateHistoryButton, 'Close State History button is not clickable');
                expect(closeStateHistoryButton.isDisplayed()).toBe(true);
                closeStateHistoryButton.click();
                waitFor.invisibilityOf(closeStateHistoryButton, 'Close State History button takes too long to disappear.');
            },
             ...
             */
            expectNumberOfLinksToMatch: function (totalLinks, addedLinks, deletedLinks) {
            ...
            expectTextToMatch: function (v1StateContents, v2StateContents) {
                forms.CodeMirrorChecker(protractor_1.element.all(protractor_1.by.css('.CodeMirror-code')).first()).expectTextToBe(v1StateContents);
                forms.CodeMirrorChecker(protractor_1.element.all(protractor_1.by.css('.CodeMirror-code')).last()).expectTextToBe(v2StateContents);
            },
            /*
             ...
             */
            expectTextWithHighlightingToMatch: function (v1StateContents, v2StateContents) {
                forms.CodeMirrorChecker(protractor_1.element.all(protractor_1.by.css('.CodeMirror-code')).first()).expectTextWithHighlightingToBe(v1StateContents);
                forms.CodeMirrorChecker(protractor_1.element.all(protractor_1.by.css('.CodeMirror-code')).last()).expectTextWithHighlightingToBe(v2StateContents);
            }
        };
    };
