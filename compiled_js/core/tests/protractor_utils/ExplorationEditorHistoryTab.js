...
var ExplorationEditorHistoryTab = function () {
     ...
     */
    var historyCheckboxSelector = element.all(by.css('.protractor-test-history-checkbox-selector'));
     ...
     * Buttons
     */
    var closeStateHistoryButton = element(by.css('.protractor-test-close-history-state-modal'));
    var showHistoryGraphButton = element(by.css('.protractor-test-show-history-graph'));
    var revertVersionButton = element.all(by.css('.protractor-test-revert-version'));
    var confirmRevertVersionButton = element(by.css('.protractor-test-confirm-revert'));
    /*
     * Links
     */
    ...
    this.getHistoryGraph = function () {
        return {
            ...
            expectTextToMatch: function (v1StateContents, v2StateContents) {
                forms.CodeMirrorChecker(element.all(by.css('.CodeMirror-code')).first()).expectTextToBe(v1StateContents);
                forms.CodeMirrorChecker(element.all(by.css('.CodeMirror-code')).last()).expectTextToBe(v2StateContents);
            },
            /*
            ...
            expectTextWithHighlightingToMatch: function (v1StateContents, v2StateContents) {
                forms.CodeMirrorChecker(element.all(by.css('.CodeMirror-code')).first()).expectTextWithHighlightingToBe(v1StateContents);
                forms.CodeMirrorChecker(element.all(by.css('.CodeMirror-code')).last()).expectTextWithHighlightingToBe(v2StateContents);
            }
        };
    };
