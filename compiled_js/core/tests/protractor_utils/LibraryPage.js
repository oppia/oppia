...
var LibraryPage = function () {
    ...
    var allCollectionSummaryTile = protractor_1.element.all(protractor_1.by.css('.protractor-test-collection-summary-tile'));
    var allExplorationSummaryTile = protractor_1.element.all(protractor_1.by.css('.protractor-test-exp-summary-tile'));
    var allCollectionsTitled = function (collectionName) {
        return protractor_1.element.all(protractor_1.by.cssContainingText('.protractor-test-collection-summary-tile-title', collectionName));
    };
    var allExplorationsTitled = function (explorationName) {
        return protractor_1.element.all(protractor_1.by.cssContainingText('.protractor-test-exp-summary-tile-title', explorationName));
    };
    var categorySelector = forms.MultiSelectEditor(protractor_1.element(protractor_1.by.css('.protractor-test-search-bar-category-selector')));
    var createActivityButton = protractor_1.element(protractor_1.by.css('.protractor-test-create-activity'));
    var languageSelector = forms.MultiSelectEditor(protractor_1.element(protractor_1.by.css('.protractor-test-search-bar-language-selector')));
    var searchInputs = protractor_1.element.all(protractor_1.by.css('.protractor-test-search-input'));
    var mainHeader = protractor_1.element(protractor_1.by.css('.protractor-test-library-main-header'));
    // Returns a promise of all explorations with the given name.
    var _getExplorationElements = function (name) {
        return protractor_1.element.all(protractor_1.by.css('.protractor-test-exp-summary-tile')).filter(function (tile) {
    ...
    var _submitSearchQuery = function (searchQuery) {
        ...
        var searchInput = (protractor_1.browser.isMobile ? searchInputs.get(1) : searchInputs.first());
        searchInput.clear();
        searchInput.sendKeys(searchQuery);
    };
    this.get = function () {
        protractor_1.browser.get(LIBRARY_URL_SUFFIX);
    ...
    this.playCollection = function (collectionName) {
        ...
        waitFor.visibilityOf(allCollectionSummaryTile.first(), 'Library Page does not have any collections');
        waitFor.visibilityOf(allCollectionsTitled(collectionName).first(), 'Unable to find collection ' + collectionName);
        allCollectionsTitled(collectionName).first().click();
        waitFor.pageToFullyLoad();
    };
    this.playExploration = function (explorationName) {
        waitFor.pageToFullyLoad();
        waitFor.visibilityOf(allExplorationSummaryTile.first(), 'Library Page does not have any explorations');
        waitFor.visibilityOf(allExplorationsTitled(explorationName).first(), 'Unable to find exploration ' + explorationName);
    ...
    this.getExplorationObjective = function (name) {
        return _getExplorationElements(name).then(function (elems) {
            return elems[0].element(protractor_1.by.css('.protractor-test-exp-summary-tile-objective')).getText();
        });
    };
    this.expectExplorationRatingToEqual = function (name, ratingValue) {
        _getExplorationElements(name).then(function (elems) {
            ...
            elems[0].element(protractor_1.by.css('.protractor-test-exp-summary-tile-rating')).getText().then(function (value) {
