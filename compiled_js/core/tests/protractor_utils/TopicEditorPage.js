...
var TopicEditorPage = function () {
    ...
    var createStoryButton = element(by.css('.protractor-test-create-story-button'));
    var newStoryTitleField = element(by.css('.protractor-test-new-story-title-field'));
    var confirmStoryCreationButton = element(by.css('.protractor-test-confirm-story-creation-button'));
    var storyListItems = element.all(by.css('.protractor-test-story-list-item'));
    var topicNameField = element(by.css('.protractor-test-topic-name-field'));
    var topicDescriptionField = element(by.css('.protractor-test-topic-description-field'));
    var saveTopicButton = element(by.css('.protractor-test-save-topic-button'));
    var publishTopicButton = element(by.css('.protractor-test-publish-topic-button'));
    var commitMessageField = element(by.css('.protractor-test-commit-message-input'));
    var closeSaveModalButton = element(by.css('.protractor-test-close-save-modal-button'));
    var subtopicsTabButton = element(by.css('.protractor-test-subtopics-tab-button'));
    var addSubtopicCard = element(by.css('.protractor-test-add-subtopic-card'));
    var newSubtopicTitlefield = element(by.css('.protractor-test-new-subtopic-title-field'));
    var confirmSubtopicCreationButton = element(by.css('.protractor-test-confirm-subtopic-creation-button'));
    var subtopics = element.all(by.css('.protractor-test-subtopic'));
    var deleteSubtopicButtons = element.all(by.css('.protractor-test-delete-subtopic-button'));
    var uncategorizedSkillItems = element.all(by.css('.protractor-test-uncategorized-skill-item'));
    var editSubtopicButtons = element.all(by.css('.protractor-test-edit-subtopic-button'));
    var subtopicTitleField = element(by.css('.protractor-test-subtopic-title-field'));
    var saveSubtopicButton = element(by.css('.protractor-test-save-subtopic-button'));
    var pageEditor = element(by.css('.protractor-test-subtopic-page-contents'));
    var subtopicTitles = element.all(by.css('.protractor-test-subtopic-title'));
    var questionsTabButton = element(by.css('.protractor-test-questions-tab-button'));
    var createQuestionButton = element(by.css('.protractor-test-create-question-button'));
    var skillItems = element.all(by.css('.protractor-test-skill-item'));
    var confirmSkillButton = element(by.css('.protractor-test-confirm-skill-button'));
    var saveQuestionButton = element(by.css('.protractor-test-save-question-button'));
    var questionItems = element.all(by.css('.protractor-test-question-list-item'));
    ...
    this.expectNumberOfQuestionsToBe = function (count) {
        waitFor.visibilityOf(questionItem, 'Question takes too long to appear');
    ...
    this.createQuestionForSkillWithIndex = function (index) {
        ...
        skillItems.then(function (elem) {
            elem[index].click();
            waitFor.elementToBeClickable(confirmSkillButton, 'Confirm Skill button takes too long to be clickable');
    ...
    this.expectSubtopicPageContentsToMatch = function (contents) {
        var subtopicPageEditor = forms.RichTextEditor(pageEditor);
        expect(element.all(by.css('.oppia-rte')).first().getText()).toEqual(contents);
    };
    ...
    this.expectStoryTitleToBe = function (title, index) {
        storyListItems.then(function (elems) {
            expect(elems[index].all(by.css('.protractor-test-story-title')).first().getText()).toEqual(title);
        });
    };
    ...
    this.createStory = function (storyTitle) {
        waitFor.elementToBeClickable(createStoryButton, 'Create Story button takes too long to be clickable');
        createStoryButton.click();
        newStoryTitleField.sendKeys(storyTitle);
        waitFor.elementToBeClickable(confirmStoryCreationButton, 'Confirm Create Story button takes too long to be clickable');
    ...
    this.saveTopic = function (commitMessage) {
        waitFor.elementToBeClickable(saveTopicButton, 'Save topic button takes too long to be clickable');
        saveTopicButton.click();
        commitMessageField.sendKeys(commitMessage);
        waitFor.elementToBeClickable(closeSaveModalButton, 'Close save modal button takes too long to be clickable');
