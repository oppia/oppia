...
var PreferencesPage = function () {
    ...
    var editorRoleEmailsCheckbox = element(by.css('.protractor-test-editor-role-email-checkbox'));
    var feedbackMessageEmailsCheckbox = element(by.css('.protractor-test-feedback-message-email-checkbox'));
    ...
    var preferredAudioLanguageSelector = element(by.css('.protractor-test-preferred-audio-language-selector'));
    var selectedAudioLanguageElement = preferredAudioLanguageSelector.element(by.css('.select2-selection__rendered'));
    var subscriptions = element.all(by.css('.protractor-test-subscription-name'));
    var systemLanguageSelector = element.all(by.css('.protractor-test-system-language-selector')).first();
    this.get = function () {
    ...
    this.selectSystemLanguage = function (language) {
        ...
        var options = element.all(by.css('.select2-dropdown li')).filter(function (elem) {
    ...
    this.selectPreferredAudioLanguage = function (language) {
        ...
        var correctOptions = languageOptionsList.all(by.tagName('li')).filter(function (elem) {
            return elem.getText().then(function (text) {
                return text === language;
            });
        });
    ...
    this.expectPreferredSiteLanguageToBe = function (language) {
        var selectedLanguageElement = systemLanguageSelector.element(by.css('.select2-selection__rendered'));
