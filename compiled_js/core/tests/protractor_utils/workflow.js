...
var createCollectionAsAdmin = function () {
    ...
    waitFor.visibilityOf(activityCreationModal, 'Activity Creation modal takes too long to appear');
...
var publishExploration = function () {
    protractor_1.element(protractor_1.by.css('.protractor-test-publish-exploration')).isDisplayed().then(function () {
        protractor_1.element(protractor_1.by.css('.protractor-test-publish-exploration')).click();
    });
    var prePublicationButtonElem = protractor_1.element(protractor_1.by.css('.protractor-test-confirm-pre-publication'));
    prePublicationButtonElem.isPresent().then(function () {
        prePublicationButtonElem.click();
    });
    waitFor.invisibilityOf(prePublicationButtonElem, 'prePublicationButtonElem taking too long to disappear while publishing');
    protractor_1.element(protractor_1.by.css('.protractor-test-confirm-publish')).click();
    var sharePublishModal = protractor_1.element(protractor_1.by.css('.protractor-test-share-publish-modal'));
    var closePublishModalButton = protractor_1.element(protractor_1.by.css('.protractor-test-share-publish-close'));
    waitFor.visibilityOf(sharePublishModal, 'Share Publish Modal takes too long to appear');
    waitFor.elementToBeClickable(closePublishModalButton, 'Close Publish Modal button is not clickable');
    closePublishModalButton.click();
};
// Creates and publishes a minimal exploration
var createAndPublishExploration = function (title, category, objective, language) {
    createExploration();
    var explorationEditorPage = new ExplorationEditorPage.ExplorationEditorPage();
    var explorationEditorMainTab = explorationEditorPage.getMainTab();
...
var _addExplorationRole = function (roleName, username) {
    protractor_1.element(protractor_1.by.css('.protractor-test-edit-roles')).click();
    protractor_1.element(protractor_1.by.css('.protractor-test-role-username')).sendKeys(username);
    protractor_1.element(protractor_1.by.css('.protractor-test-role-select')).
        element(protractor_1.by.cssContainingText('option', roleName)).click();
    protractor_1.element(protractor_1.by.css('.protractor-test-save-role')).click();
};
...
var _getExplorationRoles = function (roleName) {
    ...
    return protractor_1.element.all(protractor_1.by.repeater(itemName + ' in ExplorationRightsService.' + listName + ' track by $index')).map(function (elem) {
        return elem.getText();
    });
};
