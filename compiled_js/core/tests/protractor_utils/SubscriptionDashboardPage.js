...
var SubscriptionDashboardPage = function () {
    var subscriptionButton = protractor_1.element(protractor_1.by.css('.protractor-test-subscription-button'));
    var subscriptionName = protractor_1.element.all(protractor_1.by.css('.protractor-test-subscription-name'));
    this.navigateToUserSubscriptionPage = function (userName) {
        protractor_1.browser.get('/profile/' + userName);
        return waitFor.pageToFullyLoad();
    };
    this.expectSubscriptionFirstNameToMatch = function (name) {
        waitFor.visibilityOf(subscriptionName.first(), 'First Subscriber Name is not visible');
        expect(subscriptionName.first().getText()).toMatch(name);
    };
    this.expectSubscriptionLastNameToMatch = function (name) {
        waitFor.visibilityOf(subscriptionName.last(), 'Last Subscriber Name is not visible');
        expect(subscriptionName.last().getText()).toMatch(name);
    };
    this.expectSubscriptionCountToEqual = function (value) {
        waitFor.visibilityOf(subscriptionName.first(), 'Subscriber Name Card takes too long to appear');
        expect(subscriptionName.count()).toEqual(value);
    };
    this.navigateToSubscriptionButton = function () {
        waitFor.elementToBeClickable(subscriptionButton, 'Subscription button is not clickable');
