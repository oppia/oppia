...
describe('screenreader and keyboard user accessibility features', function () {
    ...
    it('should skip to the main content element', function () {
        libraryPage.get();
        protractor_1.browser.actions().sendKeys(protractor.Key.TAB).perform();
        var skipLink = protractor_1.element(protractor_1.by.css('.protractor-test-skip-link'));
        waitFor.elementToBeClickable(skipLink, 'Could not click skip link');
        skipLink.click();
        var mainContent = protractor_1.element(protractor_1.by.css('.protractor-test-main-content'));
        expect(mainContent.getAttribute('id'))
            .toEqual(protractor_1.browser.driver.switchTo().activeElement().getAttribute('id'));
    });
    afterEach(function () {
...
describe('Cache Slugs', function () {
    it('should check that errors get logged for missing resources', function () {
        protractor_1.browser.get('/console_errors');
        var expectedErrors = [
