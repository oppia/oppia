...
describe('State Rules Stats Service', function () {
    ...
    it('should claim text-input interaction states support issues overview', function () {
        // Only including properties required to identify supported states.
        var TEXT_INPUT_STATE = { interaction: { id: 'TextInput' } };
        expect(StateRulesStatsService.stateSupportsImprovementsOverview(TEXT_INPUT_STATE)).toBe(true);
    });
    describe('Stats Computation', function () {
        ...
        it('should respond with answer frequencies', function () {
            ...
            $httpBackend.expectGET('/createhandler/state_rules_stats/7/Hola').respond(HOLA_STATE_RULES_STATS_RESPONSE);
            StateRulesStatsService.computeStateRulesStats(HOLA_STATE).then(successHandler, failureHandler);
            $httpBackend.flush();
        ...
        it('should convert FractionInput into readable strings', function () {
            var successHandler = jasmine.createSpy('success');
            var failureHandler = jasmine.createSpy('failure');
            $httpBackend.expectGET('/createhandler/state_rules_stats/7/Fraction').respond({ visualizations_info: [{
                        data: [
                            {
                                answer: {
                                    isNegative: false,
            ...
            StateRulesStatsService.computeStateRulesStats({ name: 'Fraction', interaction: { id: 'FractionInput' } }).then(successHandler, failureHandler);
            $httpBackend.flush();
            expect(successHandler).toHaveBeenCalledWith(jasmine.objectContaining({
                visualizations_info: [jasmine.objectContaining({
                        data: [
