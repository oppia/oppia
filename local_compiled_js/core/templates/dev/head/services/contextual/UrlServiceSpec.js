...
describe('Url Service', function () {
    ...
    it('should return correct query value list for each query field', function () {
        ...
        expect(UrlService.getQueryFieldValuesAsList('field1')).toEqual(expectedList1);
        expect(UrlService.getQueryFieldValuesAsList('field2')).toEqual(expectedList2);
    });
    it('should correctly decode special characters in query value in url', function () {
        var expectedObject = {
            field1: '?value=1',
            field2: '?value&1'
        };
    ...
    it('should correctly encode and add query field and value to url', function () {
        var queryValue = '&value=1?';
        var queryField = 'field 1';
        var baseUrl = '/sample';
        var expectedUrl1 = baseUrl + '?field%201=%26value%3D1%3F';
        expect(UrlService.addField(baseUrl, queryField, queryValue)).toBe(expectedUrl1);
        baseUrl = '/sample?field=value';
        var expectedUrl2 = baseUrl + '&field%201=%26value%3D1%3F';
        expect(UrlService.addField(baseUrl, queryField, queryValue)).toBe(expectedUrl2);
    });
    ...
    it('should correctly retrieve topic name from url', function () {
        ...
        expect(UrlService.getTopicNameFromLearnerUrl()).toBe('abcdefgijklm');
        mockLocation.pathname = '/topic/topic%20name';
        expect(UrlService.getTopicNameFromLearnerUrl()).toBe('topic name');
    ...
    it('should correctly retrieve story id from url', function () {
        ...
        expect(UrlService.getStoryIdFromUrl()).toEqual('012345678901');
    });
    ...
    it('should correctly retrieve story id from url in player', function () {
        ...
        expect(UrlService.getStoryIdInPlayer()).toBe('mnopqrstuvwx');
        mockLocation.search = '?story=mnopqrstuvwx';
        expect(UrlService.getStoryIdInPlayer()).toBe(null);
    });
});
