...
describe('Thread Status Display Service', function () {
    ...
    it('should give human readable status for status choice', function () {
        ...
        for (var i = 0; i < mockStatusChoices.length; i++) {
            ...
            expect(ThreadStatusDisplayService.getHumanReadableStatus(mockStatusID)).toBe(mockStatusChoices[i].text);
        }
        var mockStatusID = 'INVALID_STATUS';
        expect(ThreadStatusDisplayService.getHumanReadableStatus(mockStatusID)).toBe('');
    });
    it('should give appropriate label class for status id', function () {
        var mockStatusID = 'open';
        expect(ThreadStatusDisplayService.getLabelClass(mockStatusID)).toBe('label label-info');
        mockStatusID = 'fixed';
        expect(ThreadStatusDisplayService.getLabelClass(mockStatusID)).toBe('label label-default');
        mockStatusID = 'ignored';
        expect(ThreadStatusDisplayService.getLabelClass(mockStatusID)).toBe('label label-default');
        mockStatusID = 'not_actionable';
        expect(ThreadStatusDisplayService.getLabelClass(mockStatusID)).toBe('label label-default');
        mockStatusID = 'compliment';
        expect(ThreadStatusDisplayService.getLabelClass(mockStatusID)).toBe('label label-success');
    });
});
// Change
