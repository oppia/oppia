...
oppia.factory('ExplorationSaveService', [
    ...
    function ($log, $q, $rootScope, $timeout, $uibModal, AlertsService, AutosaveInfoModalsService, ChangeListService, ExplorationCategoryService, ExplorationDataService, ExplorationDiffService, ExplorationInitStateNameService, ExplorationLanguageCodeService, ExplorationObjectiveService, ExplorationRightsService, ExplorationStatesService, ExplorationTagsService, ExplorationTitleService, ExplorationWarningsService, FocusManagerService, RouterService, SiteAnalyticsService, StatesObjectFactory, UrlInterpolationService) {
        ...
        var showCongratulatorySharingModal = function () {
            return $uibModal.open({
                ...
                controller: [
                    ...
                    'ContextService',
                    function ($scope, $window, $uibModalInstance, ContextService) {
                        $scope.congratsImgUrl = UrlInterpolationService.getStaticImageUrl('/general/congrats.svg');
                        $scope.DEFAULT_TWITTER_SHARE_MESSAGE_EDITOR = (GLOBALS.DEFAULT_TWITTER_SHARE_MESSAGE_EDITOR);
        ...
        var openPublishExplorationModal = function (onStartSaveCallback, onSaveDoneCallback) {
            ...
            publishModalInstance.result.then(function () {
                ...
                ExplorationRightsService.publish().then(function () {
                    if (onSaveDoneCallback) {
                        onSaveDoneCallback();
                    }
                    showCongratulatorySharingModal();
                    SiteAnalyticsService.registerPublishExplorationEvent(ExplorationDataService.explorationId);
        ...
        var saveDraftToBackend = function (commitMessage) {
            ...
            if (ExplorationRightsService.isPrivate()) {
                SiteAnalyticsService.registerCommitChangesToPrivateExplorationEvent(ExplorationDataService.explorationId);
            }
            else {
                SiteAnalyticsService.registerCommitChangesToPublicExplorationEvent(ExplorationDataService.explorationId);
            }
            if (ExplorationWarningsService.countWarnings() === 0) {
                SiteAnalyticsService.registerSavePlayableExplorationEvent(ExplorationDataService.explorationId);
            }
            saveIsInProgress = true;
            ExplorationDataService.save(changeList, commitMessage, function (isDraftVersionValid, draftChanges) {
        ...
        return {
            isExplorationSaveable: function () {
                return (ChangeListService.isExplorationLockedForEditing() &&
                    !saveIsInProgress && ((ExplorationRightsService.isPrivate() &&
            ...
            showPublishExplorationModal: function (onStartLoadingCallback, onEndLoadingCallback) {
                // This is resolved after publishing modals are closed,
                // so we can remove the loading-dots.
                var whenModalsClosed = $q.defer();
                SiteAnalyticsService.registerOpenPublishExplorationModalEvent(ExplorationDataService.explorationId);
                AlertsService.clearWarnings();
                ...
                if (isAdditionalMetadataNeeded()) {
                    var modalInstance = $uibModal.open({
                        ...
                        controller: [
                            ...
                            function ($scope, $uibModalInstance, ExplorationObjectiveService, ExplorationTitleService, ExplorationCategoryService, ExplorationStatesService, ALL_CATEGORIES, ExplorationLanguageCodeService, ExplorationTagsService) {
                                ...
                                $scope.objectiveHasBeenPreviouslyEdited = (ExplorationObjectiveService.savedMemento.length > 0);
                                $scope.requireTitleToBeSpecified = (!ExplorationTitleService.savedMemento);
                                $scope.requireObjectiveToBeSpecified = (ExplorationObjectiveService.savedMemento.length < 15);
                                $scope.requireCategoryToBeSpecified = (!ExplorationCategoryService.savedMemento);
                                $scope.askForLanguageCheck = (ExplorationLanguageCodeService.savedMemento ===
                                    constants.DEFAULT_LANGUAGE_CODE);
                                $scope.askForTags = (ExplorationTagsService.savedMemento.length === 0);
                                ...
                                $scope.isSavingAllowed = function () {
                                    return Boolean(ExplorationTitleService.displayed &&
                    ...
                    modalInstance.result.then(function (metadataList) {
                        if (metadataList.length > 0) {
                            ...
                            saveDraftToBackend(commitMessage).then(function () {
                                ...
                                openPublishExplorationModal(onStartLoadingCallback, onEndLoadingCallback)
                                    .then(function () {
            ...
            saveChanges: function (onStartLoadingCallback, onEndLoadingCallback) {
                ...
                ExplorationDataService.getLastSavedData().then(function (data) {
                    var oldStates = StatesObjectFactory.createFromBackendDict(data.states).getStateObjects();
                    var newStates = ExplorationStatesService.getStates()
                        .getStateObjects();
                    var diffGraphData = ExplorationDiffService.getDiffGraphData(oldStates, newStates, [{
                    ...
                    diffData = {
                        ...
                        v2InitStateId: diffGraphData.stateIds[ExplorationInitStateNameService.displayed],
                        v1States: oldStates,
                        v2States: newStates
                    };
                        ...
                        controller: [
                            ...
                            function ($scope, $uibModalInstance, isExplorationPrivate) {
                                ...
                                $scope.onClickToggleDiffButton = function () {
                                    ...
                                    if ($scope.showDiff) {
                                        $('.oppia-save-exploration-modal').addClass('oppia-save-exploration-wide-modal');
                                    }
                                    else {
                                        $('.oppia-save-exploration-modal').removeClass('oppia-save-exploration-wide-modal');
                                    }
                                };
