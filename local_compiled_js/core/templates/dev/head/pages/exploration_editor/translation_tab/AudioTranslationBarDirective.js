        ...
        return {
            ...
            link: function (scope, elm) {
                ...
                $('.oppia-translation-tab').on('drop', function (evt) {
                    evt.preventDefault();
                    if (evt.target.classList.contains('oppia-drop-area-message') && scope.dropAreaIsAccessible) {
            ...
            templateUrl: UrlInterpolationService.getDirectiveTemplateUrl('/pages/exploration_editor/translation_tab/' +
            ...
            controller: [
                ...
                function ($filter, $rootScope, $scope, $timeout, $uibModal, AlertsService, AssetsBackendApiService, AudioPlayerService, ContextService, EditabilityService, ExplorationStatesService, IdGenerationService, SiteAnalyticsService, StateContentIdsToAudioTranslationsService, StateEditorService, TranslationLanguageService, recorderService, TranslationTabActiveContentIdService, RECORDING_TIME_LIMIT) {
                    ...
                    var saveContentIdsToAudioTranslationChanges = function () {
                        ...
                        ExplorationStatesService.saveContentIdsToAudioTranslations(stateName, value);
                    };
                    ...
                    $scope.toggleAudioNeedsUpdate = function () {
                        StateContentIdsToAudioTranslationsService.displayed
                            .toggleNeedsUpdateAttribute($scope.contentId, $scope.languageCode);
                    ...
                    $scope.saveRecordedAudio = function () {
                        ...
                        var recordedAudioFile = new File([$scope.audioBlob], filename, { type: fileType });
                        $scope.showRecorderWarning = false;
                        AssetsBackendApiService.saveAudio(ContextService.getExplorationId(), filename, recordedAudioFile).then(function () {
                            ...
                            StateContentIdsToAudioTranslationsService.displayed
                                .addAudioTranslation(contentId, languageCode, filename, recordedAudioFile.size);
                            saveContentIdsToAudioTranslationChanges();
                            AlertsService.addSuccessMessage('Succesfuly uploaded recorded audio.');
                            $scope.audioIsCurrentlyBeingSaved = false;
                            $scope.initAudioBar();
                    ...
                    var loadAndPlayAudioTranslation = function () {
                        ...
                        var audioTranslation = getAvailableAudio($scope.contentId, $scope.languageCode);
                        if (audioTranslation) {
                    ...
                    $scope.initAudioBar = function () {
                        ...
                        $scope.contentId = (TranslationTabActiveContentIdService.getActiveContentId());
                        var audioTranslationObject = getAvailableAudio($scope.contentId, $scope.languageCode);
                    ...
                    $scope.openAddAudioTranslationModal = function (audioFile) {
                        ...
                        $uibModal.open({
                            ...
                            controller: [
                                ...
                                'audioFile',
                                function ($scope, $uibModalInstance, AlertsService, languageCode, ContextService, generatedFilename, isAudioAvailable, audioFile) {
                                    var ERROR_MESSAGE_BAD_FILE_UPLOAD = ('There was an error uploading the audio file.');
                                    ...
                                    $scope.save = function () {
                                        if ($scope.isAudioTranslationValid()) {
                                            ...
                                            AssetsBackendApiService.saveAudio(explorationId, generatedFilename, uploadedFile).then(function () {
                                                $uibModalInstance.close({
                                                ...
                                                $scope.errorMessage = (errorResponse.error || ERROR_MESSAGE_BAD_FILE_UPLOAD);
                                                uploadedFile = null;
                                                $scope.saveButtonText = BUTTON_TEXT_SAVE;
                                                $scope.saveInProgress = false;
                                            });
                            ...
                            StateContentIdsToAudioTranslationsService.displayed
                                .addAudioTranslation($scope.contentId, $scope.languageCode, result.filename, result.fileSizeBytes);
                            saveContentIdsToAudioTranslationChanges();
                            $scope.initAudioBar();
                        });
                    };
