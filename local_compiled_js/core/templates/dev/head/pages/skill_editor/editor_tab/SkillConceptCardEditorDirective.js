...
oppia.directive('skillConceptCardEditor', [
    ...
    function (GenerateContentIdService, SkillEditorStateService, SkillUpdateService, SubtitledHtmlObjectFactory, UrlInterpolationService, COMPONENT_NAME_EXPLANATION, COMPONENT_NAME_WORKED_EXAMPLE) {
        return {
            restrict: 'E',
            scope: {},
            templateUrl: UrlInterpolationService.getDirectiveTemplateUrl('/pages/skill_editor/editor_tab/' +
                ...
                function ($scope, $filter, $uibModal, EVENT_SKILL_REINITIALIZED) {
                    ...
                    var initBindableFieldsDict = function () {
                        $scope.bindableFieldsDict = {
                            displayedConceptCardExplanation: $scope.skill.getConceptCard().getExplanation().getHtml(),
                            displayedWorkedExamples: $scope.skill.getConceptCard().getWorkedExamples()
                        };
                    };
                    ...
                    $scope.WORKED_EXAMPLES_SORTABLE_OPTIONS = {
                        ...
                        stop: function () {
                            ...
                            SkillUpdateService.updateWorkedExamples($scope.skill, newWorkedExamples);
                        }
                    };
                    ...
                    $scope.saveConceptCardExplanation = function () {
                        $scope.conceptCardExplanationEditorIsShown = false;
                        SkillUpdateService.setConceptCardExplanation($scope.skill, SubtitledHtmlObjectFactory.createDefault($scope.bindableFieldsDict.displayedConceptCardExplanation, COMPONENT_NAME_EXPLANATION));
                    ...
                    $scope.openAddWorkedExampleModal = function () {
                        $uibModal.open({
                            ...
                            SkillUpdateService.addWorkedExample($scope.skill, SubtitledHtmlObjectFactory.createDefault(result.workedExampleHtml, GenerateContentIdService.getNextId($scope.skill.getConceptCard()
                                .getContentIdsToAudioTranslations().getAllContentId(), COMPONENT_NAME_WORKED_EXAMPLE)));
                            $scope.bindableFieldsDict.displayedWorkedExamples =
                                $scope.skill.getConceptCard().getWorkedExamples();
                        });
                    };
