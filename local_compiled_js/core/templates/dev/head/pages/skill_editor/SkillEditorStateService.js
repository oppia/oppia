...
oppia.constant('EVENT_QUESTION_SUMMARIES_INITIALIZED', 'questionSummariesInitialized');
oppia.factory('SkillEditorStateService', [
    ...
    function ($rootScope, AlertsService, EditableSkillBackendApiService, SkillObjectFactory, SkillRightsBackendApiService, SkillRightsObjectFactory, UndoRedoService, EVENT_QUESTION_SUMMARIES_INITIALIZED, EVENT_SKILL_INITIALIZED, EVENT_SKILL_REINITIALIZED) {
        ...
        var _updateSkill = function (newBackendSkillObject) {
            _setSkill(SkillObjectFactory.createFromBackendDict(newBackendSkillObject));
        };
        ...
        var _updateSkillRights = function (newBackendSkillRightsObject) {
            _setSkillRights(SkillRightsObjectFactory.createFromBackendDict(newBackendSkillRightsObject));
        };
        ...
        return {
            loadSkill: function (skillId) {
                ...
                EditableSkillBackendApiService.fetchSkill(skillId).then(function (newBackendSkillObject) {
                    _updateSkill(newBackendSkillObject);
                    EditableSkillBackendApiService.fetchQuestions(skillId, _nextCursorForQuestions).then(function (returnObject) {
                ...
                SkillRightsBackendApiService.fetchSkillRights(skillId).then(function (newBackendSkillRightsObject) {
            ...
            saveSkill: function (commitMessage, successCallback) {
                if (!_skillIsInitialized) {
                    AlertsService.fatalWarning('Cannot save a skill before one is loaded.');
                }
                if (!UndoRedoService.hasChanges()) {
                    return false;
                }
                _skillIsBeingSaved = true;
                EditableSkillBackendApiService.updateSkill(_skill.getId(), _skill.getVersion(), commitMessage, UndoRedoService.getCommittableChangeList()).then(function (skillBackendObject) {
                    ...
                    AlertsService.addWarning(error || 'There was an error when saving the skill');
                    _skillIsBeingSaved = false;
                });
                return true;
            },
