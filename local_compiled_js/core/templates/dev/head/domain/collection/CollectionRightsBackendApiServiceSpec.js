...
describe('Collection rights backend API service', function () {
    ...
    beforeEach(angular.mock.inject(function ($injector) {
        CollectionRightsBackendApiService = $injector.get('CollectionRightsBackendApiService');
    ...
    it('should successfully set a collection to be public', function () {
        ...
        $httpBackend.expect('PUT', '/collection_editor_handler/publish/0').respond(200);
        CollectionRightsBackendApiService.setCollectionPublic('0', 1).then(successHandler, failHandler);
        $httpBackend.flush();
        $rootScope.$digest();
    ...
    it('should call the provided fail handler on HTTP failure', function () {
        var successHandler = jasmine.createSpy('success');
        var failHandler = jasmine.createSpy('fail');
        $httpBackend.expect('PUT', '/collection_editor_handler/publish/0').respond(500, 'Error loading collection 0.');
        CollectionRightsBackendApiService.setCollectionPublic('0', 1).then(successHandler, failHandler);
        $httpBackend.flush();
        $rootScope.$digest();
    ...
    it('should report a cached collection rights after caching it', function () {
        ...
        CollectionRightsBackendApiService.loadCollectionRights('0').then(successHandler, failHandler);
