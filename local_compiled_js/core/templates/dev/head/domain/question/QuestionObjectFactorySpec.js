...
describe('Question object factory', function () {
    ...
    beforeEach(angular.mock.inject(function ($injector) {
        ...
        _sampleQuestion = QuestionObjectFactory.createFromBackendDict(_sampleQuestionBackendDict);
    }));
    it('should correctly get various fields of the question', function () {
        ...
        expect(interaction.solution.explanation.getHtml()).toEqual('Solution explanation');
        expect(interaction.solution.correctAnswer).toEqual('This is the correct answer');
    ...
    it('should correctly validate question', function () {
        ...
        var misconception1 = MisconceptionObjectFactory.create('id', 'name', 'notes', 'feedback');
        var misconception2 = MisconceptionObjectFactory.create('id_2', 'name_2', 'notes', 'feedback');
        expect(_sampleQuestion.validate([misconception1, misconception2])).toEqual('The following misconceptions should also be caught: name, name_2.' +
        ...
        expect(_sampleQuestion.validate([])).toEqual('At least one answer should be marked correct');
        interaction.solution = null;
        expect(_sampleQuestion.validate([])).toEqual('A solution must be specified');
        interaction.hints = [];
        expect(_sampleQuestion.validate([])).toEqual('At least 1 hint should be specfied');
    });
});
// Change
