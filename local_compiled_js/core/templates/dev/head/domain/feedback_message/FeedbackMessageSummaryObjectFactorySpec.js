...
describe('Feedback message object factory', function () {
    ...
    beforeEach(angular.mock.inject(function ($injector) {
        FeedbackMessageSummaryObjectFactory = $injector.get('FeedbackMessageSummaryObjectFactory');
    }));
    it('should create a new message', function () {
        var feedbackMessageSummary = (FeedbackMessageSummaryObjectFactory.createNewMessage(0, 'Sample message', 'Test user', 'profile_picture_url'));
        ...
        expect(feedbackMessageSummary.authorPictureDataUrl).toEqual('profile_picture_url');
    });
    it('should fetch the feedback message domain object from the backend ' +
        ...
        var feedbackMessageSummary = (FeedbackMessageSummaryObjectFactory.createFromBackendDict(messageSummary));
        expect(feedbackMessageSummary.text).toEqual('Sample text');
        expect(feedbackMessageSummary.authorUsername).toEqual('User 1');
        expect(feedbackMessageSummary.authorPictureDataUrl).toEqual('sample_picture_url_1');
    });
});
// Change
