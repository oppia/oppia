...
oppia.directive('sharingLinks', [
    'UrlInterpolationService', function (UrlInterpolationService) {
        return {
            ...
            templateUrl: UrlInterpolationService.getDirectiveTemplateUrl('/components/share/' +
                'sharing_links_directive.html'),
            controller: [
                '$scope', '$window', 'HtmlEscaperService',
                'ExplorationEmbedButtonService', 'SiteAnalyticsService',
                function ($scope, $window, HtmlEscaperService, ExplorationEmbedButtonService, SiteAnalyticsService) {
                    ...
                    if ($scope.shareType === 'exploration') {
                        ...
                        $scope.registerShareEvent = (SiteAnalyticsService.registerShareExplorationEvent);
                        ...
                        $scope.registerShareEvent = (SiteAnalyticsService.registerShareCollectionEvent);
                    }
                    else {
                    ...
                    $scope.escapedTwitterText = (HtmlEscaperService.unescapedStrToEscapedStr($scope.getTwitterText()));
                    $scope.classroomUrl = UrlInterpolationService.getStaticImageUrl('/general/classroom.png');
                }
            ]
        };
    }
