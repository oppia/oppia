...
oppia.directive('oppiaInteractiveDragAndDropSortInput', [
    ...
    function (DragAndDropSortInputRulesService, HtmlEscaperService, UrlInterpolationService) {
        return {
            restrict: 'E',
            scope: {},
            ...
            controller: [
                ...
                function ($scope, $attrs, UrlService, CurrentInteractionService) {
                    $scope.choices = HtmlEscaperService.escapedJsonToObj($attrs.choicesWithValue);
                    var answers = [];
                    $scope.list = [];
                    $scope.dataMaxDepth = 1;
                    $scope.allowMultipleItemsInSamePosition = ($attrs.allowMultipleItemsInSamePositionWithValue === 'true');
