...
var customizeInteraction = function (elem, recommendedExplorationIdArray) {
    if (recommendedExplorationIdArray) {
        ...
        recommendedExplorationIdArray.forEach(function (explorationId) {
            var addExplorationIdButton = element(by.css('.protractor-test-add-list-entry'));
            addExplorationIdButton.click();
            objects.UnicodeStringEditor(elem.element(by.tagName('schema-based-unicode-editor'))).setValue(explorationId);
        });
    }
};
var expectInteractionDetailsToMatch = function (elem) {
    expect(elem.element(by.tagName('oppia-interactive-end-exploration')).isPresent()).toBe(true);
};
var testSuite = [];
