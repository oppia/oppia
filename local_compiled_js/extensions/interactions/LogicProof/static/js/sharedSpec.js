...
describe('Parse then display expressions', function () {
    var parseThenDisplay = function (expressionString) {
        return logicProofShared.displayExpression(logicProofParser.parse(expressionString, 'expression'), logicProofData.BASE_STUDENT_LANGUAGE.operators);
    };
    var parseThenDisplayControl = function (expressionString) {
        return logicProofShared.displayExpression(logicProofParser.parse(expressionString, 'expression'), logicProofData.BASE_CONTROL_LANGUAGE.operators);
    };
...
describe('Pre-parse lines', function () {
    var preParse = function (lineString, isTemplate) {
        return logicProofShared.preParseLineString(lineString, logicProofData.BASE_STUDENT_LANGUAGE.operators, isTemplate);
    };
    ...
    it('should reject lines that are entirely whitespace', function () {
        expect(errorWrapper(preParse, '   ', false)).toThrowError('This line is blank.');
    });
    it('should reject unknown symbols', function () {
        expect(errorWrapper(preParse, 'from p and p{q we see q', false)).toThrowError('The symbol { was not recognised.');
    });
    describe('Parse lines', function () {
        var parse = function (lineString, isTemplate) {
            return logicProofShared.parseLineString(lineString, logicProofData.BASE_STUDENT_LANGUAGE.operators, logicProofData.BASE_VOCABULARY, isTemplate);
        };
        it('should parse examples correctly', function () {
        ...
        it('should reject unknown phrases', function () {
            expect(errorWrapper(parse, 'from p we havw p\u2227q')).toThrowError('The phrase starting \'we\' could not be identified; ' +
                'please make sure you are only using phrases from the given list of ' +
                'vocabulary.');
        });
        it('should reject consecutive expressions', function () {
            expect(errorWrapper(parse, 'from A=>B B have B')).toThrowError('We could not identify \'B\'; please make sure you are using ' +
    ...
    describe('Assign types to expressions', function () {
        var assignTypes = function (expressionString, newKindsPermitted) {
            return logicProofShared.assignTypesToExpression(logicProofParser.parse(expressionString, 'expression'), ['boolean'], logicProofData.BASE_STUDENT_LANGUAGE, newKindsPermitted)[0].typedExpression;
        };
        var assignTypesControl = function (expressionString, newKindsPermitted) {
            return logicProofShared.assignTypesToExpression(logicProofParser.parse(expressionString, 'expression'), ['boolean'], logicProofData.BASE_CONTROL_LANGUAGE, newKindsPermitted)[0].typedExpression;
        };
        ...
        it('should reject type mismatches', function () {
            expect(errorWrapper(assignTypes, 'p<=>2+x', ['variable'])).toThrowError('addition yields a element but you are trying to use it to give a ' +
                'boolean.');
            expect(errorWrapper(assignTypes, 'x\u2227f(x)', ['variable', 'prefix_function', 'constant'])).toThrowError('x yields a boolean but you are trying to use it to give a element.');
            expect(errorWrapper(assignTypesControl, '\'a\'=2', ['prefix_function', 'constant'])).toThrowError('2 yields a integer but you are trying to use it to give a string.');
        });
        it('should forbid quantification over pre-existing variables', function () {
            expect(errorWrapper(assignTypes, 'a\u2227\u2203a.f(2)', ['variable', 'prefix_function', 'constant'])).toThrowError('The name \'a\' is already in use and so cannot be quantified ' +
                'over in \u2203a.f(2).');
        });
        it('should reject kind mismatches', function () {
            expect(errorWrapper(assignTypes, 'f(f)', ['variable', 'prefix_function', 'constant'])).toThrowError('f is supposed to be a prefix_function.');
        });
        it('should reject unknown operators of an un-addable kind', function () {
            expect(errorWrapper(assignTypes, '\u2200m<n.A(n)', ['variable', 'prefix_function', 'constant'])).toThrowError('The operator bounded_for_all could not be identified.');
            expect(errorWrapper(assignTypes, '\u2203x.A(x)<=>x=2', ['prefix_function', 'constant'])).toThrowError('The operator x could not be identified.');
        });
    });
});
...
describe('Check equality between expression constructs', function () {
    it('should recognise when expressions are equal', function () {
        expect(logicProofShared.checkExpressionsAreEqual(logicProofParser.parse('p\u2227r\u2228\u2200x.s', 'expression'), logicProofParser.parse('(p\u2227r)\u2228(\u2200x.s)', 'expression'))).toBe(true);
    });
    it('should recognise when expressions are not equal', function () {
        expect(logicProofShared.checkExpressionsAreEqual(logicProofParser.parse('p\u2227r\u2228\u2200x.s', 'expression'), logicProofParser.parse('(p\u2227r)\u2228(\u2200y.s)', 'expression'))).toBe(false);
    });
    it('should recognise when an expression is not in a set', function () {
        expect(logicProofShared.checkExpressionIsInSet(logicProofParser.parse('p\u2227q', 'expression'), [
    ...
    it('should recognise when sets of expressions are equal', function () {
        expect(logicProofShared.checkSetsOfExpressionsAreEqual([
...
describe('Get operators from expression', function () {
    it('should identify all operators', function () {
        expect(logicProofShared.getOperatorsFromExpression(logicProofParser.parse('f(x+2)=y+x\u2227p', 'expression'))).toEqual(['x', 2, 'addition', 'f', 'y', 'equals', 'p', 'and']);
    });
    it('should identify all operators of a given kind', function () {
        expect(logicProofShared.getOperatorsFromExpression(logicProofParser.parse('x+2=y+x', 'expression'), ['variable'])).toEqual(['x', 'y']);
    });
});
