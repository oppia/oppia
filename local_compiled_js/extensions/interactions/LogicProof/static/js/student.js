...
var logicProofStudent = (function () {
    ...
    var BASE_CONTROL_MODEL = {
        evaluation_rules: {
            ...
            equals: {
                ...
                evaluateExpression: function (args, types) {
                    return (types[0] === 'formula') ?
                        logicProofShared.checkExpressionsAreEqual(args[0], args[1]) :
                        (args[0].type === 'set_of_formulas') ?
                            logicProofShared.checkSetsOfExpressionsAreEqual(args[0], args[1]) :
                            (args[0] === args[1]);
                }
            },
            not_equals: {
            ...
            is_in: {
                format: 'bottom_up',
                evaluateExpression: function (args) {
                    return logicProofShared.checkExpressionIsInSet(args[0], args[1]);
                }
            },
            addition: {
                format: 'bottom_up',
            ...
            question_variables: {
                format: 'bottom_up',
                evaluateExpression: function (args, types, evaluationParameters) {
                    var names = logicProofShared.getOperatorsFromExpressionArray(evaluationParameters.assumptions.concat([evaluationParameters.target]), ['variable']);
    ...
    var buildInstance = function (questionData) {
        ...
        var controlOperators = angular.copy(logicProofData.BASE_CONTROL_LANGUAGE.operators);
    ...
    var matchExpression = function (expression, template, oldMatchings) {
        ...
        if (template.top_kind_name === 'variable') {
            if (oldMatchings.hasOwnProperty(template.top_operator_name)) {
                if (logicProofShared.checkExpressionsAreEqual(expression, oldMatchings[template.top_operator_name])) {
                    return matchings;
                }
                else {
            ...
            for (var i = 0; i < messages.length; i++) {
                ...
                for (var j = 0; j < messages[i].length; j++) {
                    if (messages[i][j].format === 'string') {
                        ...
                        renderedMessages[i] += logicProofShared.displayExpression(computeExpressionFromTemplate(messages[i][j].content, matchings), operators);
                    }
                }
            }
            throw new logicProofShared.PreRenderedUserError(renderedMessages, templateName);
        }
    };
    /**
    ...
    var requireValidMatching = function (matchings, templateReaderView, language) {
        ...
        for (var i = 0; i < templateReaderView.length; i++) {
            if (templateReaderView[i].format === 'expression') {
                ...
                if (templateReaderView[i].content.hasOwnProperty('kind') &&
                    ...
                    throw new logicProofShared.UserError('wrong_kind_in_line', {
        ...
        logicProofShared.assignTypesToExpressionArray(expressionsToCheck, typesRequired, language, ['variable', 'constant']);
    };
    /**
    ...
    var requireIdentifiableLine = function (lineString, lineTemplates, language, vocabulary, generalMessages) {
        try {
            var protoLines = logicProofShared.parseLineString(lineString.trim(), language.operators, vocabulary, false);
        }
        catch (err) {
            throw {
            ...
            for (var j = 0; j < lineTemplates.length; j++) {
                try {
                    ...
                    if (errorMessage === undefined) {
                        var errorMessage = logicProofShared.renderError(err, generalMessages, language);
                    }
                }
            }
        }
    ...
    var buildLine = function (lineString, lineTemplates, language, vocabulary) {
        ...
        var indentation = n / 2;
        var protoLines = logicProofShared.parseLineString(lineString.slice(n, lineString.length), language.operators, vocabulary, false);
    ...
    var buildProof = function (proofString, questionInstance) {
        ...
        for (var i = 0; i <= lastLineNum; i++) {
            try {
                ...
                throw {
                    message: logicProofShared.renderError(err, questionInstance.general_messages, questionInstance.language),
                    line: i,
                    code: err.code,
                ...
                for (var j = 0; j < questionInstance.mistake_table[i].entries.length; j++) {
                    ...
                    if (check) {
                        ...
                        var error = (mistakeMessages.length > 0) ?
                            new logicProofShared.PreRenderedUserError(mistakeMessages, mistake.name) :
                            new logicProofShared.UserError('unspecified_mistake', {
                                section: questionInstance.mistake_table[i].name,
                                entry: mistake.name
                            });
                        throw {
                            message: logicProofShared.renderError(error, questionInstance.general_messages, questionInstance.language),
                            line: lineNumber,
                            code: mistake.name,
