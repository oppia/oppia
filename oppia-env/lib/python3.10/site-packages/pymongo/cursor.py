...
from bson.py3compat import integer_types, iteritems, string_type
...
class Cursor(object):
    ...
    def __init__(
        ...
        if modifiers is not None:
            warnings.warn(
                "the 'modifiers' parameter is deprecated", DeprecationWarning, stacklevel=2
            )
    ...
    def count(self, with_limit_and_skip=False):
        ...
        return self.__collection._count(cmd, self.__collation, session=self.__session)
    ...
    def _refresh(self):
        ...
        if self.__id is None:  # Query
            if (self.__min or self.__max) and not self.__hint:
                warnings.warn(
                    "using a min/max query operator without "
