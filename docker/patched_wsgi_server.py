...
class WsgiHostCheck(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if (
            self._is_local_host(canonical_host)
            or self._is_whitelisted_host(canonical_host)
            or self._is_whitelisted_wildcard_host(canonical_host)
            or self._is_whitelisted_deep_wildcard_host(canonical_host)
        ):
            return self.app(environ, start_response)
        else:
            logging.error(
...
class WsgiServer(object):
    ...
    def start(self):
        ...
        ssl_host_ports = (
            self._get_all_host_ports(host, self._ssl_port) if self.ssl_enabled else []
        )
