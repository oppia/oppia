        ...
        if fds:
            if _HAS_POLL:
                ...
                while failed:
                    try:
                        ready_file_descriptors = [
                            fd for fd, _ in poll.
                            poll(_READINESS_TIMEOUT_SECONDS * _SECONDS_TO_MILLISECONDS)
                        ]
                        failed = False
                    except select.error:
                        pass
                ...
                ready_file_descriptors, _, _ = select.select(
                    fds, [], [], _READINESS_TIMEOUT_SECONDS
                )
...
class _SingleAddressWsgiServer(wsgiserver.CherryPyWSGIServer):
    ...
    """
        ...
        try:
            ...
            if ':' in host:
                info = [(
                    socket.AF_INET6, socket.SOCK_STREAM, 0, '', self.bind_addr + (0, 0)
                )]
            else:
        ...
        for res in info:
            ...
            break
        if not self.socket:
            raise BindError('Unable to bind %s:%s' % self.bind_addr, saved_socket_error)
...
class WsgiHostCheck(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if (
            self._is_local_host(canonical_host) or
            ...
            logging.error(('Request Host %s not whitelisted. Enabled hosts are %s'),
                          canonical_host, self.whitelisted_hosts)
            start_response('400 Bad Request', [])
            return [(
                'Request host is not whitelist enabled for this server. '
    ...
    def _get_base_host_name(self, host):
        ...
        return six_subset.ensure_str(host).split('.', 1)[-1] if '.' in host else None
...
class WsgiServer(object):
    ...
    """
        ...
        ssl_host_ports = self._get_all_host_ports(host, self._ssl_port
        ...
        if port != 0:
            ...
            for _ in range(_PORT_0_RETRIES):
                if self._start_all_dynamic_port(
                    host_ports
                ) and self._start_all_dynamic_port(ssl_host_ports, ssl_adapter):
                    break
            else:
